...
function(op_library TARGET)
    ...
    if (WITH_GPU)
        if(WITH_OP_UNITY_BUILD AND op_library_UNITY)
            create_cu_unity_build_file(${UNITY_TARGET} SRCS ${cu_srcs} ${cudnn_cu_srcs})
            if(TARGET ${UNITY_TARGET})
                target_sources(${UNITY_TARGET} PRIVATE  ${cc_srcs} ${cu_cc_srcs} ${cudnn_cu_cc_srcs} ${mkldnn_cc_srcs} ${CU_UNITY_BUILD_FILE})
            else()
                nv_library(${UNITY_TARGET} SRCS ${cc_srcs} ${cu_cc_srcs} ${cudnn_cu_cc_srcs} ${cudnn_cu_srcs} ${mkldnn_cc_srcs} ${cu_srcs} DEPS ${op_library_DEPS}
                    ${op_common_deps})
            ...
            add_library(${TAREGT} ALIAS ${UNITY_TARGET})
        else()
...
function(create_cu_unity_build_file TARGET)
    set(option "")
    set(oneValueArgs "")
    set(options "")
    set(oneValueArgs "")
    set(multiValueArgs SRCS)
    cmake_parse_arguments(cu_unity_build "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    set(SKIP_CU_SRCS "dgc_momentum_op.cu" "proximal_gd_op.cu" "modified_huber_loss_op.cu" "rank_loss_op.cu" "smooth_l1_loss_op.cu" "squared_l2_distance_op.cu")
    set(cu_unity_build_srcs "")

    set(INDEX_DESC "cache unity target file index")
    set(COUNT_DESC "cache unity file inlcude file count")
    if(NOT TARGET ${TARGET})
        set(${TARGET}_UNITY_FILE_INDEX 0 CACHE INTERNAL ${INDEX_DESC})
        set(${TARGET}_UNITY_FILE_COUNT 0 CACHE INTERNAL ${COUNT_DESC})
    endif()
    set(CU_UNITY_BUILD_FILE ${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_${${TARGET}_UNITY_FILE_INDEX}.cu)
    if(NOT TARGET ${TARGET})
        file(WRITE ${CU_UNITY_BUILD_FILE} "/* generated by CMake */\n")
        file(APPEND ${CU_UNITY_BUILD_FILE} "${UNITY_BEFORE_CODE}\n")
    endif()

    foreach(cu_src ${cu_unity_build_SRCS})
        if(${${TARGET}_UNITY_FILE_COUNT} GREATER_EQUAL 8)
            math(EXPR ${TARGET_UNITY_FILE_INDEX} "${${TARGET}_UNITY_FILE_INDEX}+1")
            set(${TARGET}_UNITY_FILE_COUNT 0)
            set(CU_UNITY_BUILD_FILE ${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_${${TARGET}_UNITY_FILE_INDEX}.cu)
            file(WRITE ${CU_UNITY_BUILD_FILE} "/* generated by CMake */\n")
            file(APPEND ${CU_UNITY_BUILD_FILE} "${UNITY_BEFORE_CODE}\n")
        endif()
        if(${cu_src} IN_LIST SKIP_CU_SRCS)
            file(APPEND ${CU_UNITY_BUILD_FILE} "//#include \"${CMAKE_CURRENT_SOURCE_DIR}/${cu_src}\"\n")
            set(cu_unity_build_srcs ${cu_unity_build_srcs} ${cu_src})
        elseif(IS_ABSOLUTE ${cu_src})
            file(APPEND ${CU_UNITY_BUILD_FILE} "#include \"${cu_src}\"\n")
        else()
            file(APPEND ${CU_UNITY_BUILD_FILE} "#include \"${CMAKE_CURRENT_SOURCE_DIR}/${cu_src}\"\n")
        endif()
        math(EXPR ${TARGET}_UNITY_FILE_COUNT "${${TARGET}_UNITY_FILE_COUNT}+1")
    endforeach()

    set(${TARGET}_UNITY_FILE_INDEX ${${TARGET}_UNITY_FILE_INDEX} CACHE INTERNAL ${INDEX_DESC})
    set(${TARGET}_UNITY_FILE_COUNT ${${TARGET}_UNITY_FILE_COUNT} CACHE INTERNAL ${COUNT_DESC})
    set(CU_UNITY_BUILD_FILE ${CU_UNITY_BUILD_FILE} ${cu_unity_build_srcs} PARENT_SCOPE)
endfunction()
