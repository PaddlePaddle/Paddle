...
function(find_empty_grad_operators)
    set(options "")
    set(oneValueArgs "")
    set(multiValueArgs EXCLUDES DEPS)
    cmake_parse_arguments(find_empty_grad_operators "${options}" "${oneValueArgs}"
            "${multiValueArgs}" ${ARGN})

    set(mkldnn_cc_srcs)
    set(manual_empty_grad_op "logical_op" "compare_op" "isfinite_op" "multiclass_nms_op")
    set(manual_empty_grad_mkldnn_op "conv_transpose_mkldnn_op")
    file(GLOB_RECURSE OPS RELATIVE "${CMAKE_CURRENT_SOURCE_DIR}" "*_op.cc")
    foreach(src ${OPS})
        get_filename_component(op_filename ${src} NAME_WE)
        if(WITH_MKLDNN)
            string(REPLACE "_op" "_mkldnn_op" MKLDNN_FILE "${op_filename}")
            if (EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/mkldnn/${MKLDNN_FILE}.cc)
                list(APPEND mkldnn_cc_srcs mkldnn/${MKLDNN_FILE}.cc)
                continue()
            endif()
        endif()
        list(FIND manual_empty_grad_op ${op_filename} _index)
        if (${_index} EQUAL -1)
            file(READ ${CMAKE_CURRENT_SOURCE_DIR}/${src} CONTENT)
            string(REGEX MATCH "EmptyGradOpMaker|REGISTER_OP_WITHOUT_GRADIENT" regex_result "${CONTENT}")
            if (NOT regex_result STREQUAL "")
                string(REPLACE "_op" "" op_name "${op_filename}")
                file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(${op_name});\n")
            endif()
        elseif(${op_filename} STREQUAL "logical_op")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(logical_and);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(logical_or);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(logical_not);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(logical_xor);\n")
        elseif(${op_filename} STREQUAL "compare_op")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(less_than);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(less_equal);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(greater_than);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(greater_equal);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(equal);\n")
            file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(not_equal);\n")
        elseif(${op_filename} STREQUAL "isfinite_op")
          file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(isinf);\n")
          file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(isnan);\n")
          file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(isfinite);\n")
        elseif(${op_filename} STREQUAL "multiclass_nms_op")
          file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(multiclass_nms);\n")
          file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_OP(multiclass_nms2);\n")
        else()
            message(FATAL_ERROR "error: ${op_filename} should be manually appended to empty_grap_op_list.")
        endif()
    endforeach()

    foreach(src ${mkldnn_cc_srcs})
        get_filename_component(op_filename ${src} NAME_WE)
        list(FIND manual_empty_grad_mkldnn_op ${op_filename} _index)
        if (${_index} EQUAL -1)
            string(REPLACE "_mkldnn_op" "" op_name "${op_filename}")
            file(READ ${CMAKE_CURRENT_SOURCE_DIR}/${src} CONTENT)
            string(REGEX MATCH "${op_name}.*_grad" regex_result "${CONTENT}")
            if (regex_result STREQUAL "")
              file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_MKLDNN_OP(${op_name});\n")
            endif()
        elseif(${op_filename} STREQUAL "conv_transpose_mkldnn_op")
          file(APPEND ${empty_grap_op_list} "EMPTY_GRAD_MKLDNN_OP(conv2d_transpose);\n")
        else()
            message(FATAL_ERROR "error: ${op_filename} should be manually appended to empty_grap_op_list.")
        endif()
    endforeach()
