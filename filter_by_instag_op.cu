...
__global__ void filter_by_instag_cuda_kernel(const size_t N,
                                             const size_t* x2_lods_data,
...
template <typename T>
__global__ void copy_kernel(const size_t N,
                            T* out_data,
                            const T* x1_data,
  ...
  const T* src_end =
      x1_data + (map_data[idx * 3 + 1] + map_data[idx * 3 + 2]) * x1_embed_size;
  ...
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (out_lods.size() - 1 > 0) {
      ...
      thrust::fill(loss_weight_data_ptr,
                   loss_weight_data_ptr + loss_weight->numel(),
                   1.0);
      // only one kernel launch
      size_t N = out_lods.size() - 1;
      dim3 block_dim_2(block_size);
      dim3 grid_dim_2((N + block_size - 1) / block_size);
      copy_kernel<<<grid_dim_2, block_dim_2, 0, current_stream>>>(
          N, out_data, x1_data, map_data, x1_embed_size);
