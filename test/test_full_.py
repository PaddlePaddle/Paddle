...
class TestFP16Full_(unittest.TestCase):
    ...
    def test_cuda_api(self):
        ...
        _C_ops.full_(data, data.shape, float(self.value), data.dtype, core.CUDAPlace(0))

        assert np.array_equal(data, np.full(self.shape, self.value, dtype=self.type))

    def test_cpu_api(self):
        data = paddle.rand(self.shape, dtype=self.type)
        _C_ops.full_(data, data.shape, float(self.value), data.dtype, core.CPUPlace())
...
class TestBF16Full_(unittest.TestCase):
    ...
    def test_cuda_api(self):
        ...
        assert np.array_equal(data, np.full(self.shape, convert_float_to_uint16(self.value), dtype='uint16'))
