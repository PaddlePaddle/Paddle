...
class SIR171(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_804,  # (shape: [1, 384, 32, 32], dtype: paddle.float32, stop_gradient: False)
        var_805,  # (shape: [1, 384, 1, 1], dtype: paddle.float32, stop_gradient: False)
    ):
        var_808 = paddle.nn.functional.conv._conv_nd(
            var_805,
            self.var_806,
            bias=self.var_807,
            stride=[1, 1],
        ...
        var_809 = paddle.tensor.ops.sigmoid(var_808)
        var_810 = var_804.__mul__(var_809)
        return var_810


class TestSIR171(unittest.TestCase):
    def setUp(self):
        self.inputs = (
