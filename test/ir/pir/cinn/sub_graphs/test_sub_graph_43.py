...
# api||paddle.nn.functional.activation.relu
import unittest
...
class SIR59(paddle.nn.Layer):
    ...
    def forward(
        self,
        var_174,  # (shape: [43, 16, 14, 14], dtype: paddle.float32, stop_gradient: False)
    ):
...
class TestSIR59(unittest.TestCase):
    def setUp(self):
        self.inputs = (
            paddle.rand(shape=[43, 16, 14, 14], dtype=paddle.float32),
        )
        self.net = SIR59()
