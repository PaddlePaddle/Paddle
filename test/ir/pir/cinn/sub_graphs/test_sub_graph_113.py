...
class SIR101(paddle.nn.Layer):
    def __init__(self):
        super().__init__()
        self.var_857 = self.create_parameter(
            shape=[27],
    ...
    def forward(
        self,
        var_855,  # (shape: [1, 256, 28, 40], dtype: paddle.float32, stop_gradient: False)
    ):
        var_858 = paddle.nn.functional.conv._conv_nd(
            var_855,
            self.var_856,
            bias=self.var_857,
            stride=[1, 1],
        ...
        out = paddle.tensor.manipulation.split(
            var_858, num_or_sections=[18, 9], axis=1
        )
        var_859 = out[0]
        var_860 = out[1]
        var_861 = paddle.tensor.ops.sigmoid(var_860)
        var_864 = paddle.vision.ops.deform_conv2d(
            x=var_855,
            offset=var_859,
            weight=self.var_862,
            bias=self.var_863,
