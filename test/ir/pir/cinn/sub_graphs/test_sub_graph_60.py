...
class SIR115(paddle.nn.Layer):
    def __init__(self):
        super().__init__()
        self.var_758 = self.create_parameter(
            shape=[120, 20, 1, 1],
    ...
    def forward(
        self,
        var_751,  # (shape: [22, 40, 28, 28], dtype: paddle.float32, stop_gradient: False)
    ):
        out = paddle.tensor.manipulation.split(var_751, [20, 20], axis=1)
        var_752 = out[0]
        var_753 = out[1]
        out = paddle.tensor.manipulation.split(var_751, [20, 20], axis=1)
        var_754 = out[0]
        var_755 = out[1]
        var_757 = paddle.nn.functional.conv._conv_nd(
            var_754,
            self.var_756,
            bias=None,
            stride=[1, 1],
        ...
        var_759 = paddle.nn.functional.conv._conv_nd(
            var_755,
            self.var_758,
            bias=None,
            stride=[1, 1],
        ...
        var_760 = paddle.tensor.manipulation.concat((var_757, var_759), axis=1)
        return var_760
