...
class LayerCase(paddle.nn.Layer):
    ...
    def forward(
        ...
        var_3 = ppdiffusers.utils.paddle_utils.randn_pt(
            [1, 4, 1, 1], generator=None, dtype='float32'
        )
        var_4 = var_1 * var_3
        var_5 = var_0 + var_4
        var_6 = var_5 * 0.18215
        var_7 = ppdiffusers.utils.paddle_utils.randn_pt([1, 4, 1, 1])
        var_8 = ppdiffusers.utils.paddle_utils.randint_pt(0, 1000, (1,))
...
def create_numpy_inputs():
    inputs = (
        np.random.random(size=[1, 4, 1, 1]).astype('float32'),
        np.random.random(size=[1, 4, 1, 1]).astype('float32'),
        np.random.random(size=[1000]).astype('float32'),
    )
    return inputs
