...
class TestMinimumTRTPattern(PassTest):
    def is_program_valid(self, program=None):
        return True
...
class TestSubtractTRTPattern(PassTest):
    def is_program_valid(self, program=None):
        return True
        ...
        with paddle.pir_utils.IrGuard():
            ...
            with paddle.pir.core.program_guard(main_prog, start_prog):
                ...
                self.feeds = {
                    ...
                    "y": np.array([[5, 6], [3, 4]]).astype("float32"),
                }
                self.fetch_list = [out]
                self.valid_op_map = {
                    "pd_op.fusion_transpose_flatten_concat": 0,
...
class TestMatmulTRTPattern(PassTest):
    ...
    def sample_program(self):
        with paddle.pir_utils.IrGuard():
            ...
            start_prog = paddle.static.Program()
            with paddle.pir.core.program_guard(main_prog, start_prog):
                x = paddle.static.data(name='x', shape=[10,5,2], dtype='float32')
                y = paddle.static.data(name='y', shape=[10,2,5], dtype='float32')
