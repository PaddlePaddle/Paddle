...
class TestPredictorRunWithTensor(unittest.TestCase):
    ...
    def pir_setup(self):
        with paddle.pir_utils.DygraphPirGuard():
            ...
            paddle.jit.save(
                model,
                os.path.join(
                    self.temp_dir.name, 'test_predictor_run_model/pir_inference'
                ),
            )

    def oldir_setup(self):
    ...
    def init_pir_predictor(self):
        config = Config(
            os.path.join(
                self.temp_dir.name,
                'test_predictor_run_model/pir_inference.json',
            ),
            os.path.join(
                self.temp_dir.name,
                'test_predictor_run_model/pir_inference.pdiparams',
            ),
        )
        config.enable_use_gpu(256, 0)
        config.switch_ir_optim(False)
