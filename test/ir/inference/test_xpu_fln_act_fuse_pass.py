...
class TestXpuFLNActFusePass(PassAutoScanTest):
    def sample_predictor_configs(self, program_config):
        config = self.create_inference_config(use_xpu=True)
        yield config, ["fln_act_xpu"], (1e-3, 1e-3)
    ...
    def sample_program_config(self, draw):
        ...
        alpha = draw(st.floats(min_value=0.0000001, max_value=0.001))
        relu_op = OpConfig(
            "leaky_relu",
            inputs={
                "X": ["layer_norm_out"],
