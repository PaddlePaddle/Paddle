...
class TrtConvertConv2dTest(TrtLayerAutoScanTest):
    ...
    def sample_program_configs(self):
        ...
        def generate_input1(batch, attrs: List[Dict[str, Any]]):
            return (
                np.ones([batch, 64, 64, attrs[0]['groups'] * 3]).astype(
                    np.float32
                )
                / 4
            )
    ...
    def sample_predictor_configs(
        ...
        def generate_dynamic_shape(attrs):
            ...
            self.dynamic_shape.min_input_shape = {
                "input_data": [1, 32, 32, input_groups],
                "output_data": [1, 24, 32, 32],
            }
            self.dynamic_shape.max_input_shape = {
                "input_data": [4, 64, 64, input_groups],
                "output_data": [4, 24, 64, 64],
            }
            self.dynamic_shape.opt_input_shape = {
                "input_data": [1, 64, 64, input_groups],
