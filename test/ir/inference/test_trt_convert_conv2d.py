...
class TrtConvertConv2dPluginTest(TrtLayerAutoScanTest):
    def is_program_valid(self, program_config: ProgramConfig) -> bool:
        inputs = program_config.inputs
        weights = program_config.weights
        attrs = [
    ...
    def sample_program_configs(self):
        ...
        def generate_data(shape, attrs: List[Dict[str, Any]]):
            return np.random.random([9, 3, 3, 3]).astype(np.float32) - 0.5
            ...
            program_config = ProgramConfig(
                ...
                inputs={
                    ...
                    "input1_data": TensorConfig(
                        data_gen=partial(generate_data, [9, 3, 3, 8], attrs)
                    ),
                    "input2_data": TensorConfig(
                        data_gen=partial(generate_data, [8, 3], attrs)
                    ),
                },
                outputs=["output_data"],
            )
    ...
    def sample_predictor_configs(
        ...
        def generate_dynamic_shape(attrs):
            ...
            self.dynamic_shape.opt_input_shape["input1_data"] = [9, 3, 3, 8]
            self.dynamic_shape.opt_input_shape["input2_data"] = [8, 3]
