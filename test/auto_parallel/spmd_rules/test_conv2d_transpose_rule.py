...
class TestConv2dTransposeSPMDRule(unittest.TestCase):
    def setUp(self):
        self.rule = core.get_phi_spmd_rule("conv2d_transpose")

    def test_conv2d_transpose_nchw_infer_forward(self):
        # forward setup
        input_shape = [2, 4, 8, 8]
        ...
        self.dilations = [1, 1]
        # case 1
        # input: NCHinWin[0, -1, -1, -1], filter: CMHkWk[-1, -1, -1, -1]
        ...
        # case 4
        # input: NCHinWin[-1, 0, -1, -1], filter: CMHkWk[0, -1, -1, -1] --->
        # output: NMHoutWout[-1, -1, -1, -1]
        ...
        # case 5
        # input: NCHinWin[0, 2, -1, -1], filter: CMHkWk[2, 1, -1, -1] --->
        # output: NMHoutWout[0, 1, -1, -1]
        ...
        self.assertEqual(
            inferred_input_dist_attrs[1].dims_mapping, [2, 1, -1, -1]
        )
        self.assertEqual(
            inferred_output_dist_attrs[0].dims_mapping, [0, 1, -1, -1]
        )
    ...
    def test_conv2d_transpose_nhwc_infer_forward(self):
        ...
        self.dilations = [1, 1]
        # case 6
        # input: NHinWinC[-1, -1, -1, 0], filter: CMHkWk[0, -1, -1, -1] --->
        # output: NMHoutWout[-1, -1, -1, -1]
        ...
        self.assertEqual(
            inferred_input_dist_attrs[0].dims_mapping, [0, -1, -1, 2]
        )
        self.assertEqual(
            inferred_input_dist_attrs[1].dims_mapping, [2, 1, -1, -1]
        )
    ...
    def test_conv2d_transpose_nchw_infer_backward(self):
        # backward setup
        input_shape = [2, 4, 8, 8]
        self.data_format = "NCHW"
        filter_shape = [4, 6, 3, 3]
        output_shape = [2, 6, 10, 10]
        process_mesh = auto.ProcessMesh(
            mesh=[[[0, 1], [2, 3]], [[4, 5], [6, 7]]]
        )
        ...
        # Output: NMHoutWout[0, 1, -1, -1] --->
        # input: NCHinWin[0, -1, -1, -1], filter: CMHkWk[-1, 1, -1, -1]
        # input_grad: NCHinWin[0, -1, -1, -1], filter_grad: CMHkWk[-1, 1, -1, -1]
        ...
        # case 2:
        # Output: NMHoutWout[0, 1, -1, -1] partial_dim=[2]--->
        # input: NCHinWin[0, 2, -1, -1], filter: CMHkWk[2, 1, -1, -1]
        # input_grad: NCHinWin[0, 2, -1, -1], filter_grad: MCHkWk[2, 1, -1, -1]
        ...
        self.assertEqual(
            inferred_output_dist_attrs[0].dims_mapping, [0, 2, -1, -1]
        )
        self.assertEqual(
            inferred_output_dist_attrs[1].dims_mapping, [2, 1, -1, -1]
        )
        self.assertEqual(inferred_input_dist_attrs[0]._is_partial(), False)
        self.assertEqual(inferred_input_dist_attrs[1]._is_partial(), False)
    ...
    def test_conv2d_transpose_nhwc_infer_backward(self):
        # backward setup
        input_shape = [2, 8, 8, 4]
        self.data_format = "NHWC"
        filter_shape = [4, 6, 3, 3]
        ...
        # case 1:
        # Output: NMHoutWout[0, 1, -1, -1] --->
        # input: NHinWinC[0, -1, -1, -1], filter: CMHkWk[-1, 1, -1, -1]
        # input_grad: NHinWinC[0, -1, -1, -1], filter_grad: CMHkWk[-1, 1, -1, -1]
        result_dist_attrs = self.rule.infer_backward(
            self.input_dist_tensor_spec,
            self.filter_dist_tensor_spec,
            self.output_dist_tensor_spec,
        ...
        # Output: NMHoutWout[0, 1, -1, -1] partial_dim=[2]--->
        # input: NHinWinC[0, -1, -1, 2], filter: CMHkWk[2, 1, -1, -1]
        # input_grad: NHinWinC[0, -1, -1, 2], filter_grad: MCHkWk[2, 1, -1, -1]
        output_tensor_dist_attr._set_partial_dims([2])
        self.output_dist_tensor_spec = DistTensorSpec(
            output_shape, output_tensor_dist_attr
        )
        ...
        self.assertEqual(
            inferred_output_dist_attrs[0].dims_mapping, [0, -1, -1, 2]
        )
        self.assertEqual(
            inferred_output_dist_attrs[1].dims_mapping, [2, 1, -1, -1]
        )
