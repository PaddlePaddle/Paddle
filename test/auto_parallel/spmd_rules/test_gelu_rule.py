...
class TestGeluSPMDRule(unittest.TestCase):
    def setUp(self):
        self.gelu_rule = core.get_phi_spmd_rule("gelu")
        ...
        self.attrs = OrderedDict([('approximate', False)])

    def test_infer_forward(self):
        # [-1, 0]--> [-1, 0], [-1, 0]
        self.x_dist_tensor_spec.set_dims_mapping([-1, 0])

        result_dist_attrs = self.gelu_rule.infer_forward(
            self.x_dist_tensor_spec, self.attrs['approximate']
        )
    ...
    def test_infer_backward(self):
        ...
        result_dist_attrs = self.gelu_rule.infer_backward(
