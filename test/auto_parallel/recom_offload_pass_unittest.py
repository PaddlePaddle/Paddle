...
def apply_pass(
    use_recompute=False, use_recom_offload=False, no_recompute_segments=[]
):
    ...
    if use_recompute:
        ...
        if use_recom_offload:
            offload = strategy.offload
            offload.enable = True
            offload.recom_offload = True
    return strategy
...
class TestRecomOffloadPass(unittest.TestCase):
    def setUp(self):
        self.rtol = 1e-6
        self.atol = 1e-8
        self.batch_size = 1
    ...
    def test_recom_offload_pass(self):
        # mp2 training
