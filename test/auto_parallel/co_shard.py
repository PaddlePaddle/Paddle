...
class TestCoShard:
    ...
    def run_test_case_0(self):
        ...
        placements = [dist.Shard(0, co_shard_order=0), dist.Shard(0, co_shard_order=1)]
        input = dist.shard_tensor(a, mesh, placements)

        idx = dist.get_rank()
        np.testing.assert_equal(input._local_value().numpy().flatten(), a[idx].numpy().flatten())
    ...
    def run_test_case_1(self):
        a = paddle.to_tensor([[1,2],
                              ...
                              [7,8]])
        mesh = dist.ProcessMesh([[0, 1], [2, 3]],  dim_names=['x', 'y'])
        placements = [dist.Shard(0, co_shard_order=1), dist.Shard(0, co_shard_order=0)]
        input = dist.shard_tensor(a, mesh, placements)

        idx = dist.get_rank()
        new_idx = idx % 2 * 2 + idx // 2
        np.testing.assert_equal(input._local_value().numpy().flatten(), a[new_idx].numpy().flatten())
