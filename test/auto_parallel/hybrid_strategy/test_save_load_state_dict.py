...
class TestSaveLoadStateDict(test_base.CommunicationTestDistBase):
    def setUp(self):
        self._default_envs = {}
        self._changeable_envs = {"device_num": ['2','4','8']}

    def test_save_load_state_dict(self):
        # save with 4 devices
        os.system(f"rm -rf {ckpt_path()}")
        super().setUp(num_of_devices=4, timeout=120, nnode=1)
        self.run_test_case("save_state_dict.py")
        # load with 2, 4, 8 devices
        envs_list = test_base.gen_product_envs_list(
            self._default_envs, self._changeable_envs
        )
        for envs in envs_list:
            super().setUp(save_log_dir="./log", num_of_devices=int(envs["device_num"]), timeout=120, nnode=1)
            self.run_test_case(
                "load_state_dict.py",
