...
from paddle.base.framework import pir_op_role_guard, auto_complete_op_role
from paddle.distributed.auto_parallel.static.pir_pass import ReshardPasses, apply_partition_pass
from paddle.distributed.auto_parallel.static.mix_to_dist_pass import apply_mix2dist_pass
...
class TestOpRole(unittest.TestCase):
    def test_signle(self):
        ...
        # check global shape
        std_ops = ["pd_op.data:-1", "pd_op.data:1", "pd_op.relu:-1", "pd_op.relu:-1",
            "pd_op.relu:1",
            "pd_op.add:1",
            "pd_op.full:1",
            "pd_op.scale:1",
        ...
        cur_ops = ["{}:{}".format(op.name(), op.op_role) for op in main_program.global_block().ops]
