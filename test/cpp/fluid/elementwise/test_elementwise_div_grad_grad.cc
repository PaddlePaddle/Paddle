...
// template <typename T>
// class TestElementwiseDivGradGradWithoutDX
//     : public TestElementwiseOpGradGrad<T> {
//  public:
//   TestElementwiseDivGradGradWithoutDX(const platform::Place &place,
//                                         const framework::DDim &dims)
//       : TestElementwiseOpGradGrad<T>(
//             "elementwise_div_grad_grad",
//             place,
//             dims,
//             {"Y", "Out", "Out@GRAD", "DDX", "DDY"},
//             {"Y@GRAD", "DDOut", "DOut"}) {}
...
//     for (size_t i = 0; i < numel; ++i) {
//       // dY(Y@GRAD) = Out * dX * ddY / Y - dX * ddX / Y
//       auto dx = feed_datas_["Out@GRAD"][i] / feed_datas_["Y"][i];
//       dy[i] = (dx / feed_datas_["Y"][i]) *
//               (feed_datas_["Out"][i] * feed_datas_["DDY"][i] -
//                feed_datas_["DDX"][i]);
//       // ddOut = ddX / Y - Out * ddY / Y = (ddX - Out * ddY) / Y
//       ddout[i] = (feed_datas_["DDX"][i] -
...
// TEST(test_elementwise_div_grad_grad_without_dx, cpu_place) {
//   framework::DDim dims({32, 64});
//   platform::CPUPlace p;
//   TestElementwiseDivGradGradWithoutDX<float> test(p, dims);
//   ASSERT_TRUE(test.Check());
// }

// #if defined(PADDLE_WITH_CUDA) || defined(PADDLE_WITH_HIP)
// TEST(test_elementwise_div_grad_grad_without_dx, gpu_place) {
//   framework::DDim dims({32, 64});
//   platform::CUDAPlace p(0);
//   TestElementwiseDivGradGradWithoutDX<float> test(p, dims);
//   ASSERT_TRUE(test.Check());
