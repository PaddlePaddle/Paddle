...
T* CreateForSaveCombineOp(int x,
  ...
  paddle::memory::Copy(place, expect, cpu_place, ground_truth_cpu.data(), sizeof(T) * ground_truth_cpu.size());
  return expect;
}
...
int SaveLoadCombineOpTest(Place place) {
  ...
  paddle::memory::Copy(cpu_place, expect1_cpu.data(), place, expect1, sizeof(T) * numel1);
  std::vector<T> expect2_cpu(numel2);
  paddle::memory::Copy(cpu_place, expect2_cpu.data(), place, expect2, sizeof(T) * numel2);
  std::vector<T> expect3_cpu(numel3);
  paddle::memory::Copy(cpu_place, expect3_cpu.data(), place, expect3, sizeof(T) * numel3);
  std::vector<T> expect4_cpu(numel4);
  paddle::memory::Copy(cpu_place, expect4_cpu.data(), place, expect4, sizeof(T) * numel4);

  std::vector<U> actual1_cpu(numel1);
  paddle::memory::Copy(cpu_place, actual1_cpu.data(), place, actual1, sizeof(U) * numel1);
  std::vector<U> actual2_cpu(numel2);
  paddle::memory::Copy(cpu_place, actual2_cpu.data(), place, actual2, sizeof(U) * numel2);
  std::vector<U> actual3_cpu(numel3);
  paddle::memory::Copy(cpu_place, actual3_cpu.data(), place, actual3, sizeof(U) * numel3);
  std::vector<U> actual4_cpu(numel4);
  paddle::memory::Copy(cpu_place, actual4_cpu.data(), place, actual4, sizeof(U) * numel4);

  CheckValues(expect1_cpu.data(), actual1_cpu.data(), expect_lod1, actual_lod1, numel1);
  CheckValues(expect2_cpu.data(), actual2_cpu.data(), expect_lod2, actual_lod2, numel2);
  CheckValues(expect3_cpu.data(), actual3_cpu.data(), expect_lod3, actual_lod3, numel3);
  CheckValues(expect4_cpu.data(), actual4_cpu.data(), expect_lod4, actual_lod4, numel4);

  return 0;
}

TEST(SaveLoadCombineOp, XPU) {
  // phi::InitXPU();  // Ensure XPU is initialized
  ...
  r = SaveLoadCombineOpTest<phi::XPUPlace, paddle::platform::float16, paddle::platform::float16>(xpu_place);
  EXPECT_EQ(r, 0);
  r = SaveLoadCombineOpTest<phi::CPUPlace, paddle::platform::float16, paddle::platform::float16>(cpu_place);
  EXPECT_EQ(r, 0);

  r = SaveLoadCombineOpTest<phi::XPUPlace, paddle::platform::bfloat16, paddle::platform::bfloat16>(xpu_place);
  EXPECT_EQ(r, 0);
  r = SaveLoadCombineOpTest<phi::CPUPlace, paddle::platform::bfloat16, paddle::platform::bfloat16>(cpu_place);
  EXPECT_EQ(r, 0);
}
