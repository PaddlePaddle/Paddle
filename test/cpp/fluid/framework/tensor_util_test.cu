...
static __global__ void FillNAN(phi::float16* buf) {
  buf[0] = 0.0;
  buf[1] = 0.1;
  buf[2].x = 0x7fff;
}

static __global__ void FillInf(phi::float16* buf) {
  buf[0] = 0.0;
  buf[1].x = 0x7c00;
  buf[2] = 0.5;
}
...
static __global__ void FillFinite(phi::float16* buf) {
  buf[0] = 0.0;
  buf[1] = 0.1;
  buf[2] = 0.2;
}
...
TEST(TensorContainsNAN, GPU) {
  ...
  {
    phi::DenseTensor tensor;
    phi::float16* buf = tensor.mutable_data<phi::float16>({3}, gpu);
#ifdef PADDLE_WITH_HIP
    hipLaunchKernelGGL(FillNAN, dim3(1), dim3(1), 0, cuda_ctx->stream(), buf);
#else
    FillNAN<<<1, 1, 0, cuda_ctx->stream()>>>(buf);
