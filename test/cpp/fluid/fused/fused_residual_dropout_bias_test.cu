...
TEST(FusedDropout, GPUFusedResidualDropoutBiasLargeShapeFp16) {
  ...
  FusedResidualDropoutBiasTester<phi::float16> test(
      rows, cols, 0, 0.0, true, true);
  test.Run();
  test.CheckOut(static_cast<phi::float16>(1e-1));
  test.CheckGrad(static_cast<phi::float16>(1e-1));
}
