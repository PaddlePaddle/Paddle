...
TEST(StandaloneExecutor, run_inplace_sqrt) {
  ir::IrContext* ctx = ir::IrContext::Instance();
  VLOG(0) << "ir::IrContext* ctx = ir::IrContext::Instance();";
  ir::Program program((ctx));
  VLOG(0) << "ir::Program program((ctx));";
  ctx->GetOrRegisterDialect<paddle::dialect::PaddleDialect>();
  VLOG(0) << "ctx->GetOrRegisterDialect<paddle::dialect::PaddleDialect>();";
  ir::Builder builder = ir::Builder(ctx, program.block());
  VLOG(0) << "ir::Builder builder = ir::Builder(ctx, program.block());";
  ...
  VLOG(0) << "paddle::dialect::FullOp full";

  builder.Build<paddle::dialect::Sqrt_Op>(full->result(0));
  VLOG(0) << "builder.Build<paddle::dialect::Sqrt_Op>(full->result(0));";

  auto kernel_program = paddle::dialect::PdOpLowerToKernelPass(&program);
  VLOG(0) << "auto kernel_program = "
             "paddle::dialect::PdOpLowerToKernelPass(&program);";

  auto place = platform::CPUPlace();
  VLOG(0) << "auto place = platform::CPUPlace();";
  Scope scope;
  VLOG(0) << "Scope scope;";
  InterpreterCore test_core(place, std::move(kernel_program), &scope);
  VLOG(0)
