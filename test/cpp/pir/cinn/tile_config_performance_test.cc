...
/**
 * @brief Test case for comparing schedule configurations between default and optimized.
 *
 */
TEST(ConfigSearcher, TestReducePipeline) {
  ...
  //build csv file
  std::string dump_path = GenCSVFilePath(cinn::common::DefaultTarget(),iter_space_type);
  ...
  for (int s_dimension_lower = spatial_left_bound;
    ...
    for (int r_dimension_lower = reduce_left_bound;
      ...
      // Write to csv file
      cinn::ir::search::ScoreType optim_percentage = best_score / baseline_score;
      std::stringstream ss;
      ss << " { " ;
      for(int i=0;i<iter_space_type.size();++i){
        ss << iter_space_type[i].first << "_" << iter_space_type[i].second << ": " <<  bucket_info.space[i].lower_bound << "-" << bucket_info.space[i].upper_bound << "  ";
      }
      ss << "} " ;
      os << ss.str() << ",";
      os << " { " << best_candidate[0] << " "
                 << best_candidate[1] << " " << best_candidate[2] << " } " << ",";
      os << std::setprecision(3) << "  " << baseline_score << "," << best_score << "," << optim_percentage << "\n";
    }
  }
  os.close();
}
