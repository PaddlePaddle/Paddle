...
ProgramInfo BuildSoftmax() {
  ...
  auto x = builder
               .Build<paddle::dialect::FullOp>(std::vector<int64_t>({8, 8, 16}),
                                               1.0,
                                               phi::DataType::FLOAT32,
                                               phi::GPUPlace(0))
               .result(0);
  ...
  auto broadcast_1 =
      builder
          .Build<cinn::dialect::BroadcastOp>(max,
  ...
  auto broadcast_2 =
      builder
          .Build<cinn::dialect::BroadcastOp>(sum,
                                             std::vector<int64_t>({0, 1, 2}),
                                             std::vector<int64_t>({8, 8, 16}))
          .result(0);
  auto divide =
...
TEST(PIRCompier, CompileSoftmax) {
  ...
  std::cerr << "out tensor" << out_tensor << std::endl;
