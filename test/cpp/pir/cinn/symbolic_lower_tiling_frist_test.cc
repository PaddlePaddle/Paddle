...
BuildGroupProgramForLowering() {
  ...
  const std::vector<int64_t> x_shape = {-1, 64};
  ...
  auto group_op = builder.Build<cinn::dialect::GroupOp>(
      CreateDenseTensorTypes(common::make_ddim({-1, 64})));
  builder.SetInsertionPointToBlockEnd(group_op.block());
  auto exp = builder.Build<paddle::dialect::ExpOp>(x);

  builder.Build<::pir::YieldOp>(std::vector<::pir::Value>{exp.result(0)});
  ...
  groups[0]->loop_ranges = std::vector<int64_t>({-1, 64});
...
TEST(ReshapeOpGroup, CINNLowering) {
  ...
  ir_compiler.BuildCUDAJITInfo(groups);
}
