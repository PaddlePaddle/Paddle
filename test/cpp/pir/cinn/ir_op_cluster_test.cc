...
int GetFuseOpNum(::pir::Block* block) {
  int num = 0;
  for (auto& op : (*block)) {
...
//   auto x = builder
//                .Build<paddle::dialect::FullOp>(std::vector<int64_t>({10,
//                10}),
//                                                1.0,
...
//   axes1, false).result(0); auto out1 =
//   builder.Build<paddle::dialect::AddOp>(inputs[2], e).result(0); auto out2 =
...
//   auto c =
//       builder.Build<paddle::dialect::AddOp>(inputs[0], inputs[1]).result(0);
//   auto out1 = builder.Build<cinn::dialect::ReduceSumOp>(inputs[0], axes,
//   false)
//                   .result(0);
//   auto out2 = builder.Build<cinn::dialect::ReduceSumOp>(inputs[1], axes,
//   false)
//                   .result(0);
//   auto out3 =
...
//   auto scale =
//       builder.Build<cinn::dialect::BroadcastOp>(inputs[1], all_axes,
//       out_shape2)
//           .result(0);
//   auto bias =
//       builder.Build<cinn::dialect::BroadcastOp>(inputs[2], all_axes,
//       out_shape2)
//           .result(0);
...
TEST(IROpFusionPass, broadcast_with_add) {
  ::pir::IrContext* ctx = ::pir::IrContext::Instance();
  ctx->GetOrRegisterDialect<paddle::dialect::OperatorDialect>();
  ctx->GetOrRegisterDialect<cinn::dialect::OperatorDialect>();
  ...
  auto x =
      builder
          .Build<paddle::dialect::UniformOp>(std::vector<int64_t>({10, 10}),
                                             phi::DataType::FLOAT32,
                                             0.0,
                                             1.0,
                                             0,
  ...
  auto y =
      builder
          .Build<paddle::dialect::UniformOp>(std::vector<int64_t>({100, 10}),
                                             phi::DataType::FLOAT32,
                                             0.0,
                                             1.0,
                                             0,
  ...
  auto t2 =
      builder
          .Build<paddle::dialect::ReshapeOp>(t1, std::vector<int64_t>({100, 1}))
          .result(0);
  auto out1 = builder.Build<paddle::dialect::AddOp>(t2, y).result(0);
  builder.Build<paddle::dialect::FetchOp>(out1, "out1", 0);
  ...
  ASSERT_EQ(GetFuseOpNum(program.block()), 2);
}
