...
//   auto sum = builder
//                  .Build<paddle::dialect::SumOp>(
//                      x, std::vector<int64_t>{0}, phi::DataType::FLOAT32,
//                      true)
//                  .result(0);
...
std::shared_ptr<::pir::Program> BuildReshapeSumProgram() {
  ...
  auto end = builder.Build<paddle::dialect::AddOp>(d1, d2).result(0);
  auto start = builder.Build<paddle::dialect::SubtractOp>(d1, d2).result(0);

  auto sum = builder.Build<paddle::dialect::ArangeOp>(start, end, d3).result(0);
...
TEST(GroupOp, TestBuildReshapeSum) {
  ...
  RunAndCheckResult(program.get(), false, 0.0);
}
