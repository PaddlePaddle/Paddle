...
std::shared_ptr<::pir::Program> BuildGroupProgram() {
  ...
  const std::vector<int64_t> shape = {108, 4, 16, 16};
  auto x = builder
...
// TEST(GroupOp, TestSharedBuffer1) {
//   // Step 1: Construct pir::Program
//   ::pir::IrContext* ctx = ::pir::IrContext::Instance();
...
//   auto x =
//       builder
//           .Build<paddle::dialect::UniformOp>(std::vector<int64_t>({43, 2, 7,
//           2, 7, 384}),
//                                           phi::DataType::FLOAT32, -0.5, 0.5,
//                                           0, phi::GPUPlace())
//           .result(0);
...
//     auto out =
//         builder.Build<paddle::dialect::ReshapeOp>(x,
//         std::vector<int64_t>({-1, 14, 14, 384}))
//             .result(0);
