...
TEST(DimExpr, value_shape_expr) {
  ...
  auto op2 =
      ::test::CreateDenseTensorOp(ctx, {2, -1}, {"op_attr"}, {"op_name"});
  pir::OpResult x = op1->result(0);
  pir::OpResult y = op2->result(0);
  pir::shape::ShapeOfOp shape_of_op = builder.Build<pir::shape::ShapeOfOp>(x);
  pir::Value extend_x = shape_of_op.out();
