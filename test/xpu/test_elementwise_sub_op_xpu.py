...
class XPUTestElementwiseSubOp(XPUOpTestWrapper):
    ...
    class TestElementwiseOp(XPUOpTest):
        ...
        def init_input_output(self):
            ...
            if self.dtype == np.uint16:
                self.x = self.reshape_data(self.x, self.y)
                self.y = self.reshape_data(self.y, self.x)
                self.outputs = {'Out': self.x - self.y}
                self.x = convert_float_to_uint16(self.x)
                self.y = convert_float_to_uint16(self.y)
            else:
                self.x = self.reshape_data(self.x, self.y).astype(self.dtype)
                self.y = self.reshape_data(self.y, self.x).astype(self.dtype)
                self.outputs = {'Out': self.x - self.y}
            self.inputs = {
                'X': self.x,
                'Y': self.y,
    ...
    @skip_check_grad_ci(
        reason="[skip shape check] Use x_shape(1) to test broadcast."
    )
