...
def ref_rms_norm(
    ...
    y = 1.0 / np.sqrt(variance + epsilon).reshape([left, 1])

    y = x * norm_weight.reshape([1, right]) * y
...
class XPUTestRmsNormOp(XPUOpTestWrapper):
    ...
    class TestXPURmsNormOp(XPUOpTest):
        def setUp(self):
            ...
            self.__class__.op_type = 'rms_norm'
            ...
            self.use_norm_bias = True  # optional input
            ...
            ref_y_np, ref_residual_out, ref_variance_np = ref_rms_norm(
                deepcopy(x_np),
                deepcopy(norm_weight_np),
                deepcopy(norm_bias_np),
                self.epsilon,
                self.begin_norm_axis,
                deepcopy(residual_np),
                deepcopy(bias_np),
            )
            ...
            # print(x_np)
            # print(norm_weight_np)
            self.outputs = {'Y': ref_y_np, 'Inv_var': ref_variance_np}
            if self.use_residual:
                self.outputs['Residual_out'] = ref_residual_out
...
for stype in support_types:
    # if not (stype == 'bfloat16' and core.get_xpu_device_version(0) == core.XPUVersion.XPU2):
    if stype == 'float16':
