...
class TestTorchProxySpecLookupReentrancy(unittest.TestCase):
    ...
    finder is registered once: ``enable_compat`` appends it to ``sys.meta_path``
    unconditionally, so nested guards leave duplicates behind, while
    ``disable_compat`` drops a single occurrence. With a duplicate left in place
    the lookup re-enters the finder for the very same name and starts over, and
    a name that has no spec at all never converges, so the lookups fan out --
    each one sweeping all of ``sys.modules`` to toggle the proxy.
    """
    ...
    def _count_lookups(self, name, importer):
        ...
        try:
            with paddle.use_compat_guard(scope=self.PACKAGE):
