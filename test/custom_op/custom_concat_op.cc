...
int64_t ComputeAxis(int64_t axis, int64_t rank) {
  PD_CHECK(axis >= -rank && axis < rank, "The axis is excepted to be in range of [", -rank, ", ", rank, "].");
  PADDLE_ENFORCE_EQ(axis >= -rank && axis < rank, true, common::errors::InvalidArgument("The axis is excepted to be in range of [ %d , %d ].", -rank, rank));
  if (axis < 0) {
    axis = axis + rank;
  }
...
std::vector<int64_t> ComputeOutShape(
  ...
  for (size_t i = 1; i < n; ++i) {
    PADDLE_ENFORCE_EQ(in_shapes[i].size() == out_shape.size(), true, common::errors::InvalidArgument("Input dimension must be same."));
    for (size_t j = 0; j < zero_dim_size; ++j) {
      if (j == axis) {
        out_shape[axis] += in_shapes[i][j];
      } else {
        PADDLE_ENFORCE_EQ(in_shapes[0][j] == in_shapes[i][j], true, common::errors::InvalidArgument("The %d-th dimension of input must be same.", j)); }
    }
  }
  return out_shape;
}
...
std::vector<paddle::Tensor> ConcatForwardDynamicAxis(
  ...
  // check inputs
  PADDLE_ENFORCE_EQ(inputs.size() >= 1, true,common::errors::InvalidArgument("No Tensor need to be concat."));
  for (auto& t : inputs) {
    CHECK_INPUT(t);
  }
  CHECK_INPUT(axis_t);
