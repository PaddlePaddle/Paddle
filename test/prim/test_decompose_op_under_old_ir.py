...
#     newir_program, param_mappings = pir.translate_to_new_ir_with_param_map(mp.desc)
#     check_param_mappings(param_mappings)
#     new_ir_grad_var_to_var_map = get_new_ir_grad_var_to_var_map(param_mappings, old_ir_grad_var_to_var_map)
...
#         newir_program, grad_var_to_var_map = get_gelu_pir_program_and_param_map()

#         with paddle.pir_utils.IrGuard(), paddle.pir.core.program_guard(newir_program):
#             core._set_prim_forward_enabled(True)
#             core._set_prim_backward_enabled(True)

#             newir_ops = newir_program.global_block().ops
#             decompose_bwd_ops_names = ["pd_op.gelu_grad", "pd_op.add_grad", "pd_op.multiply_grad"]

#             global_outputs = [newir_ops[5].result(0)]
#             global_grads = [newir_ops[-1].result(0), newir_ops[-1].result(1), newir_ops[-2].result(1)]
...
#             ) = get_global_grads_infos(newir_program.global_block(), global_grads)

#             for bwd_op in newir_ops:
#                 if flag == "decompose" and bwd_op.name() in decompose_bwd_ops_names:
...
def get_layer_norm_pir_program_and_param_map():
    ...
    with paddle.static.program_guard(mp):
        ...
        old_ir_grad_var_to_var_map = {
            ...
            # "fill_constant_1.tmp_0@GRAD" : "fill_constant_1.tmp_0",
