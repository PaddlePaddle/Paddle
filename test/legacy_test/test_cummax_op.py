...
class TestCummaxGradDeterministicGPU(unittest.TestCase):
    ...
    These tests cover forward+backward, duplicate-maximum run merging,
    ...
    def setUp(self):
        if not (base.core.is_compiled_with_cuda() or is_custom_device()):
            self.skipTest("deterministic cummax grad path is GPU only")
        paddle.disable_static(get_device_place())
    ...
    def _grad(self, x_np, g_np, axis, indices_dtype):
        ...
        out, ind = paddle.cummax(xt, axis=axis, dtype=indices_dtype)
    ...
    def test_long_run_merge(self):
        ...
        x_np[:, 0] = 1e4  # global maximum at position 0 of every row
