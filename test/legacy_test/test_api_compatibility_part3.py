...
class TestLinearAPI(unittest.TestCase):
    def setUp(self):
        ...
        self.shape = [2, 3]
        self.np_x = np.array(
            [[1.0, -2.0, 0.5], [3.0, 4.0, -1.0]], dtype=self.dtype
        )
        self.np_weight = np.array(
            [[0.25, -0.5, 1.5], [-1.0, 0.75, 0.5]], dtype=self.dtype
        )
        self.np_bias = np.array([0.1, -0.2], dtype=self.dtype)
        self.bound = 1 / np.sqrt(self.shape[-1])

    def _set_weight_and_bias(self, layer, bias=True):
        layer.weight.set_value(paddle.to_tensor(self.np_weight))
        if bias:
            layer.bias.set_value(paddle.to_tensor(self.np_bias))

    def _expected_linear(self, bias=True):
        expected = np.matmul(self.np_x, self.np_weight.T)
        if bias:
            expected = expected + self.np_bias
        return expected
    ...
    def test_dygraph_Compatibility(self):
        ...
        # 1. Paddle Positional arguments
        layer1 = paddle.compat.nn.Linear(3, 2, True, dtype=self.dtype)
        self._set_weight_and_bias(layer1)
        out1 = layer1(x)
        # 2. Paddle keyword arguments
        layer2 = paddle.compat.nn.Linear(
            in_features=3,
            out_features=2,
            bias=True,
            device="cpu",
        ...
        self._set_weight_and_bias(layer2)
        out2 = layer2(input=x)
        # 3. Mixed arguments
        layer3 = paddle.compat.nn.Linear(
            3, out_features=2, bias=False, dtype=self.dtype
        )
        self._set_weight_and_bias(layer3, bias=False)
        out3 = layer3(x)

        # Verify all outputs
        expected = self._expected_linear()
        expected_no_bias = self._expected_linear(bias=False)
        for out, ref_out in [
            (out1, expected),
            (out2, expected),
            (out3, expected_no_bias),
        ]:
            np.testing.assert_allclose(out.numpy(), ref_out, rtol=1e-6)
            self.assertEqual(out.dtype, paddle.float64)
        ...
        self.assertEqual(layer1.bias.shape, [2])
        self.assertIsNone(layer3.bias)
        self.assertEqual(
            layer2.weight.place, paddle.zeros([1], device="cpu").place
        )

        paddle.seed(2025)
        init_layer = paddle.compat.nn.Linear(3, 2, bias=True)
        with self.subTest("PyTorch weight initialization range"):
            weight_abs_max = float(np.max(np.abs(init_layer.weight.numpy())))
            self.assertLessEqual(weight_abs_max, self.bound)
            self.assertFalse(
                np.allclose(
                    init_layer.weight.numpy(),
                    np.zeros([2, 3], dtype="float32"),
                )
            )

        with self.subTest("PyTorch bias initialization range"):
            bias_abs_max = float(np.max(np.abs(init_layer.bias.numpy())))
            self.assertLessEqual(bias_abs_max, self.bound)

        with self.subTest("PyTorch bias initialization nonzero"):
            self.assertFalse(
                np.allclose(
                    init_layer.bias.numpy(), np.zeros([2], dtype="float32")
                )
            )

        with self.subTest("zero out_features"):
            zero_layer = paddle.compat.nn.Linear(
                3, 0, bias=True, dtype=self.dtype
            )
            zero_out = zero_layer(x)
            self.assertEqual(zero_layer.weight.shape, [0, 3])
            self.assertEqual(zero_layer.bias.shape, [0])
            self.assertEqual(zero_out.shape, [2, 0])
    ...
    def test_static_Compatibility(self):
        ...
        with paddle.static.program_guard(main, startup):
            ...
            out1_np, out2_np, out3_np = fetches[:3]
            weight1, bias1, weight2, bias2, weight3 = fetches[3:]

            # Verify all outputs
            for out, weight, bias in [
                (out1_np, weight1, bias1),
                (out2_np, weight2, bias2),
                (out3_np, weight3, None),
            ]:
                ref_out = np.matmul(self.np_x, weight.T)
                if bias is not None:
                    ref_out = ref_out + bias
                np.testing.assert_allclose(out, ref_out, rtol=1e-6)

            for weight in [weight1, weight2, weight3]:
                self.assertEqual(list(weight.shape), [2, 3])
            self.assertEqual(list(bias1.shape), [2])
            self.assertEqual(weight1.dtype, np.float64)

            with self.subTest("PyTorch weight initialization range"):
                weight_abs_max = float(np.max(np.abs(weight1)))
                self.assertLessEqual(weight_abs_max, self.bound)
                self.assertFalse(np.allclose(weight1, np.zeros([2, 3])))

            with self.subTest("PyTorch bias initialization range"):
                bias_abs_max = float(np.max(np.abs(bias1)))
                self.assertLessEqual(bias_abs_max, self.bound)

            with self.subTest("PyTorch bias initialization nonzero"):
                self.assertFalse(np.allclose(bias1, np.zeros([2])))

        with self.subTest("zero out_features"):
            zero_main = paddle.static.Program()
            zero_startup = paddle.static.Program()
            with paddle.static.program_guard(zero_main, zero_startup):
                zero_x = paddle.static.data(
                    name="zero_x", shape=self.shape, dtype=self.dtype
                )
                zero_layer = paddle.compat.nn.Linear(
                    3, 0, bias=True, dtype=self.dtype
                )
                zero_out = zero_layer(zero_x)
                exe = paddle.static.Executor(paddle.CPUPlace())
                exe.run(zero_startup)
                zero_fetch = exe.run(
                    zero_main,
                    feed={"zero_x": self.np_x},
                    fetch_list=[zero_out, zero_layer.weight, zero_layer.bias],
                )
                self.assertEqual(list(zero_fetch[0].shape), [2, 0])
                self.assertEqual(list(zero_fetch[1].shape), [0, 3])
                self.assertEqual(list(zero_fetch[2].shape), [0])
