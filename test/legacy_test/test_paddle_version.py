...
"""
Tests for ``paddle.paddle_version`` -- the Paddle mirror of
``torch.torch_version``. Verifies that ``PaddleVersion`` behaves as a
``str`` while also supporting comparisons against
``packaging.version.Version``, version-tuple iterables, and version
strings.
"""
...
class TestPaddleVersion(unittest.TestCase):
    ...
    def test_paddle_version_alias(self):
        # paddle.__version__ should be the PaddleVersion instance.
    ...
    def test_fallback_on_invalid_version(self):
        # Comparison against a non-version string falls back to str.
        self.assertFalse(self.v == 'parrot')
        self.assertNotEqual(self.v, 'parrot')

    def test_hashable(self):
        # Hashability is required for use as dict key / set member.
        d = {self.v: 'paddle'}
