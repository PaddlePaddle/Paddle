    ...
    def _geqrf(A):
        ...
        for i in range(min(n, m)):
            ...
            if normx == 0: tau[i] = 0
                ...
                A[i, i] = s * beta
            w = w.reshape([-1, 1])
            A[i:, i + 1:] = A[i:, i + 1:] - (tau[i] * w) @ (w.T @ A[i:, i + 1:])
        return A, tau[:min(m, n)]
    if len(A.shape) == 2: return _geqrf(A)
    m, n = A.shape[-2:]
    org_A_shape = A.shape
...
def ref_qr(A):
    def _ref_qr(A): 
        q, _ = np.linalg.qr(A)
        return q
    if len(A.shape) == 2: return _ref_qr(A)
    m, n = A.shape[-2:]
    org_shape = A.shape
    A = A.reshape((-1, A.shape[-2], A.shape[-1]))
    n_batch = A.shape[0]
...
class TestHouseholderProductAPI(unittest.TestCase):
    ...
    def test_static_api(self):
        ...
        with paddle.static.program_guard(paddle.static.Program()):
            x = paddle.static.data('x', self.geqrf_x.shape, dtype=self.geqrf_x.dtype)
            tau = paddle.static.data('tau', self.tau.shape, dtype=self.tau.dtype)
            out = paddle.linalg.householder_product(x, tau)
            exe = paddle.static.Executor(self.place)
            res = exe.run(feed={'x': self.geqrf_x, 'tau': self.tau}, fetch_list=[out])
