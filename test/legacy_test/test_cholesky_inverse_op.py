...
class TestCholeskyInverseOp(OpTest):
    def setUp(self):
        self.op_type = "cholesky_inverse"
        self.python_api = paddle.cholesky_inverse
        ...
        input_data = np.linalg.cholesky(input_data).astype("float64")
        if self._upper:
            input_data = input_data.transpose(self.trans_dims)
            output_data = np.linalg.inv(
                np.matmul(input_data.transpose(self.trans_dims), input_data)
            )
        else:
            output_data = np.linalg.inv(
                np.matmul(input_data, input_data.transpose(self.trans_dims))
            )
    ...
    def func(self, place):
        ...
        with base.program_guard(prog):
            ...
            cholesky_factor = paddle.cholesky(x, upper=self.attrs["upper"])
            out = paddle.cholesky_inverse(
                cholesky_factor, upper=self.attrs["upper"]
            )
...
class TestCholeskyInverseOpLower(TestCholeskyInverseOp):
    def init_config(self):
        self._upper = False


class TestCholeskyInverseOp2D(TestCholeskyInverseOp):
    def init_config(self):
...
class TestDygraph(unittest.TestCase):
    def test_dygraph(self):
        ...
        cholesky_factor = paddle.cholesky(x, upper=False)
        out = paddle.cholesky_inverse(cholesky_factor, upper=False)


class TestCholeskyInverseSingularAPI(unittest.TestCase):
    def setUp(self):
    ...
    def check_static_result(self, place, with_out=False):
        with base.program_guard(base.Program(), base.Program()):
            ...
            result = paddle.cholesky_inverse(input)
