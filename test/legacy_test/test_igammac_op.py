    ...
    def test_api_static(self):
        def run(place):
            ...
            with paddle.static.program_guard(paddle.static.Program()):
                ...
                out_ref = ref_igammac(self.x, self.ORDER)
...
class TestIgammacFloat32Order1(TestIgammacAPI):
    DTYPE = "float32"
    DATA = [2, 3, 5, 2.25, 7, 7.25]
    ORDER = 1


class TestIgammacFloat32Order2(TestIgammacAPI):
    DTYPE = "float32"
    DATA = [2, 3, 5, 2.25, 7, 7.25]
    ORDER = 2


class TestIgammacFloat32Order3(TestIgammacAPI):
    DTYPE = "float32"
    DATA = [2, 3, 5, 2.25, 7, 7.25]
    ORDER = 3


class TestIgammacFloat64Order1(TestIgammacAPI):
    DTYPE = "float64"
    DATA = [2, 3, 5, 2.25, 7, 7.25]
    ORDER = 1


class TestIgammacFloat64Order2(TestIgammacAPI):
    DTYPE = "float64"
    DATA = [2, 3, 5, 2.25, 7, 7.25]
    ORDER = 2


class TestIgammacFloat64Order3(TestIgammacAPI):
    DTYPE = "float64"
    DATA = [2, 3, 5, 2.25, 7, 7.25]
    ORDER = 3


class TestIgammacNegativeInputOrder1(TestIgammacAPI):
    DTYPE = "float64"
    DATA = [-2, 3, 5, 2.25, 7, 7.25]
    ORDER = 1


class TestIgammacMultiDimOrder1(TestIgammacAPI):
    DTYPE = "float64"
    DATA = [[-2, 3, 5, 2.25, 7, 7.25], [0, 1, 2, 3, 4, 5]]
    ORDER = 1


class TestIgammacMultiDimOrder2(TestIgammacAPI):
    DTYPE = "float64"
    DATA = [
...
class TestIgammacOp(OpTest):
    ...
    def init_config(self):
        ...
        self.target = ref_igammac(self.inputs['x'], self.inputs['a'])
    ...
    def test_check_grad(self):
        self.check_grad(
            ...
            'out',
            user_defined_grads=[
                ref_igammac_grad(self.case, 1 / self.case.size, self.order)
            ],
            check_pir=True,
        )
