...
class TestLegacyLossArgs(unittest.TestCase):
    def assertReduction(self, loss, expected_reduction):
        reduction = getattr(loss, 'reduction', None)
        if reduction is None:
            reduction = loss._reduction
        self.assertEqual(reduction, expected_reduction)

    def assertSuggests(self, loss_ctor, expected_reduction, **legacy_kwargs):
        with self.assertWarnsRegex(
            Warning, f"reduction='{expected_reduction}'"
        ):
            loss = loss_ctor(**legacy_kwargs)
        self.assertReduction(loss, expected_reduction)
    ...
    def test_ce_positional_legacy_reduce_trigger(self):
        ...
        with self.assertWarnsRegex(Warning, "reduction='mean'"):
            w = paddle.ones([3], dtype='float32')
            loss = CrossEntropyLoss(w, True, -100, True)
        self.assertReduction(loss, 'mean')
