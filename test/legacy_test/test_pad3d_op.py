    ...
    def test_errors(self):
        ...
        for _ in self.places:
            self.assertRaisesRegex(
                ValueError,
                r"\(InvalidArgument\) pad3d\(\): argument 'x' \(position 0\) must be Tensor, but got numpy.ndarray",
                test_variable,
            )
            self.assertRaisesRegex(
                ValueError,
            ...
            self.assertRaisesRegex(
                ValueError,
                r"\(InvalidArgument\) The height of Input\(X\)'s dimension should be greater than pad_top in reflect mode",
                test_reflect_2,
            )
            self.assertRaisesRegex(
                ValueError,
                r"\(InvalidArgument\) The depth of Input\(X\)'s dimension should be greater than pad_back in reflect mode",
                test_reflect_3,
            )
