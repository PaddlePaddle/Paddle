...
class TestActQuantDequant(unittest.TestCase):
    ...
    def act_quant(self, x: paddle.Tensor, block_size: int = 128) -> Tuple[paddle.Tensor, paddle.Tensor]:
        """
        ...
        new_shape = list(original_shape[:-1]) + [original_shape[-1] // block_size, block_size]
        x_reshaped = x_float.reshape(new_shape)
        
        # Compute scaling factors: max(abs(x)) / 448.0 for each block
        abs_x = paddle.abs(x_reshaped)
        max_vals = paddle.max(abs_x, axis=-1, keepdim=False)  # (..., num_blocks)
        s = max_vals / 448.0
    ...
    def dequant_ref(self, x: paddle.Tensor, s: paddle.Tensor, block_size: int = 128) -> paddle.Tensor:
        """
        ...
        if s.shape[1] == N // block_size:
            ...
            s_expanded = paddle.tile(s_expanded, [1, 1, block_size])  # (M, N//block_size, block_size)
    ...
    def _test_single_case(self, height: int, width: int, rtol: float = 1e-2, atol: float = 1e-2):
        """
        ...
        if hasattr(paddle.nn.functional, 'fused_act_dequant'):
            dequant_result_fused = paddle.nn.functional.fused_act_dequant(x_fp8, scale)
        else:
            # Skip fused test if not available
            self.skipTest("fused_act_dequant not available in this Paddle version")
        ...
        self.assertEqual(nan_cnt_fused, 0, f"Fused result contains {nan_cnt_fused} NaN values")
        self.assertEqual(nan_cnt_ref, 0, f"Reference result contains {nan_cnt_ref} NaN values")
        
        # Compare results
        try:
            np.testing.assert_allclose(fused_np, ref_np, rtol=rtol, atol=atol)
        except AssertionError as e:
            self.fail(f"Results don't match for shape [{height}, {width}]: {str(e)}")

    def test_invalid_inputs(self):
        """Test error handling for invalid inputs."""
        # Test non-divisible block size
        x = paddle.randn([4, 255]).astype("bfloat16")  # 255 is not divisible by 128
