...
class TestCompatSDPACPUFallback(unittest.TestCase):
    def setUp(self):
        paddle.disable_static()
        self.rng = np.random.default_rng(2026)

    def _tensor(self, shape, dtype=paddle.float32):
        data = self.rng.standard_normal(shape).astype(np.float32)
        return paddle.to_tensor(data, dtype=dtype, place=paddle.CPUPlace())

    def test_nonstandard_ranks_and_scale_zero(self):
        for shape in ((4, 3), (2, 3, 2, 4, 3)):
            q, k, v = (self._tensor(shape) for _ in range(3))
            q.stop_gradient = False
            expected = _attention_naive_bnsd(q, k, v)
            out = compat_sdpa(q, k, v)
            np.testing.assert_allclose(
                out.numpy(), expected.numpy(), rtol=1e-5, atol=1e-6
            )
            out.sum().backward()
            self.assertIsNotNone(q.grad)

        q, k, v = (self._tensor((2, 3, 4, 5)) for _ in range(3))
        expected = _attention_naive_bnsd(q, k, v, scale=0.0)
        out = compat_sdpa(q, k, v, scale=0.0)
        np.testing.assert_allclose(
            out.numpy(), expected.numpy(), rtol=1e-6, atol=1e-6
        )

        q = self._tensor((1, 4, 3, 5))
        k = self._tensor((1, 2, 3, 5))
        v = self._tensor((1, 1, 3, 5))
        expected = _attention_naive_bnsd(q, k, v, scale=0.0, enable_gqa=True)
        out = compat_sdpa(q, k, v, scale=0.0, enable_gqa=True)
        np.testing.assert_allclose(
            out.numpy(), expected.numpy(), rtol=1e-6, atol=1e-6
        )

    def test_low_precision_and_fully_masked_rows(self):
        q32, k32, v32 = (self._tensor((2, 3, 4, 5)) for _ in range(3))
        expected = _attention_naive_bnsd(q32, k32, v32)
        ...
        q, k, v = (self._tensor((4, 3)) for _ in range(3))
        mask = paddle.zeros([4, 4], dtype="bool")
        out = compat_sdpa(q, k, v, attn_mask=mask)
        np.testing.assert_array_equal(out.numpy(), np.zeros([4, 3]))

    def test_rank4_fully_masked_rows_and_gradients(self):
        masks = (
            paddle.to_tensor([[False, False], [True, False]]),
            paddle.to_tensor(
                [[-float("inf"), -float("inf")], [0.0, -float("inf")]]
            ),
        )
        expected_output = np.array([[[[0.0, 0.0], [2.0, 1.0]]]])
        expected_value_grad = np.array([[[[1.0, 1.0], [0.0, 0.0]]]])
        for mask in masks:
            query = paddle.to_tensor(
                [[[[1.0, 0.0], [0.0, 1.0]]]], stop_gradient=False
            )
            key = query.detach().clone()
            key.stop_gradient = False
            value = paddle.to_tensor(
                [[[[2.0, 1.0], [3.0, -1.0]]]], stop_gradient=False
            )
            output = compat_sdpa(query, key, value, attn_mask=mask)
            output.sum().backward()

            np.testing.assert_allclose(
                output.numpy(), expected_output, rtol=1e-5, atol=1e-6
            )
            np.testing.assert_array_equal(
                query.grad.numpy(), np.zeros_like(query.grad.numpy())
            )
            np.testing.assert_array_equal(
                key.grad.numpy(), np.zeros_like(key.grad.numpy())
            )
            np.testing.assert_array_equal(
                value.grad.numpy(), expected_value_grad
            )

    def test_invalid_dropout(self):
        q = self._tensor((4, 3))
        for dropout_p in (-0.1, 1.1):
            with self.assertRaises(ValueError):
                compat_sdpa(q, q, q, dropout_p=dropout_p)
        with self.assertRaises(IndexError):
            compat_sdpa(q, q, q, enable_gqa=True)
