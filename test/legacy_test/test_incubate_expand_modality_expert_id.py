...
import paddle.incubate.nn.functional.expand_modality_expert_id as expand_modality_expert_id 
...
def test_expand_modality_expert_id():
    def expand_id_one(expert_id, num_expert_per_modality, k, group_size, modality_offset, is_group_expert):
        ...
        ret = rank * (num_expert_per_modality * 2) + expert_id_in_rank + modality_offset * num_expert_per_modality
    ...
    fused = expand_modality_expert_id(expert_id_mm, num_expert_per_rank_per_modality, group_size, 1, True)

    nonfused = expand_id_one(expert_id_mm, num_expert_per_rank_per_modality, k, group_size, 1, True)
    # num_expert_per_rank_per_modality, group_size
    assert (fused == nonfused).all().item()

    Config = namedtuple("Config", ["moe_world_size"])
    Self = namedtuple("Self", ["config", "k", "gate", "group_experts", "moe_statics", "use_correction_bias"])
    ...
    w_and_e_ref, prob_lm_ref, prob_mm_ref = fused_gate_logits_process_ref(fake_self, fake_logits, fake_logits_mm, None)
