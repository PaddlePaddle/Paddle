...
class TestNeg(unittest.TestCase):
    ...
    def test_dygraph_compatibility(self):
        ...
        out3 = paddle.neg(x)
        # 4. PyTorch keyword arguments (alias)
        out4 = paddle.neg(input=x)
        # 5. Mixed arguments
        out5 = paddle.neg(x, name=None)
        # 6. out parameter test
        out6 = paddle.empty_like(x)
        out7 = paddle.neg(x=x, out=out6)
        # 7. Tensor method - args
        out8 = x.neg()
        # 8. Tensor method - kwargs
        out9 = x.neg(name=None)

        for out in [out1, out2, out3, out4, out5, out6, out7, out8, out9]:
            np.testing.assert_allclose(out.numpy(), -self.np_x, rtol=1e-6)
    ...
    def test_static_compatibility(self):
        ...
        with paddle.static.program_guard(main, startup):
            ...
            for out in fetches:
                np.testing.assert_allclose(out, -self.np_x, rtol=1e-6)
