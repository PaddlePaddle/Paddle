...
def test_reducescatter(ep_group:Group, mode: str):
    ...
    expected_tensor = paddle.ones(shape=[m, n], dtype=paddle.float32) * (local_rank + 1) * num_local_ranks
...
def test_alltoall(ep_group:Group, mode: str):
    ...
    expected_tensor = paddle.ones(shape=[m, n], dtype=paddle.float32) * (local_rank + 1)
...
def test_scatter(ep_group:Group, mode: str):
    ...
    expected = paddle.ones(shape=[m, n], dtype=paddle.float32) * (local_rank + 1)
    assert paddle.allclose(recv_tensor, expected), f"rank {local_rank}: scatter validation failed"
...
def test_reduce(ep_group:Group, mode: str):
    ...
    if local_rank == 0:
        ...
        res = paddle.ones(shape=[m, n], dtype=paddle.float32) * (num_local_ranks * (num_local_ranks + 1) / 2)
        assert paddle.allclose(gbl_x, res), f"rank {local_rank}: reduce validation failed"
...
def test_broadcast(ep_group:Group, mode: str):
    ...
    assert paddle.allclose(gbl_x, res), f"rank {local_rank}: broadcast validation failed"
...
def test_all_reduce(ep_group:Group, mode: str):
    ...
    assert paddle.allclose(gbl_x, res), f"rank {local_rank}: all reduce validation failed"
...
def test_primitive():
    ...
    # alog_list = ["Tree", "Ring", "CollNetDirect", "CollNetChain", "NVLS", "NVLSTree", "PAT"]

    alog_list = ["Tree", "Ring", "NVLS"]

    proto_list = ["LL", "LL128", "Simple"]

    ll_size = [i for i in range(32768, 1048576, 1024 * 400)]
    ll128_size = [i for i in range(32768 * 4, 1048576, 1024 * 400)]
    simple_size = [i for i in range(32768 * 4, 1048576, 1024 * 400)]
    ...
    ep_group1 = [paddle.distributed.new_group(ranks, nccl_config=nccl_config1[i]) for i in range(config_len1)]

    ep_group2 = [paddle.distributed.new_group(ranks, nccl_config=nccl_config2[i]) for i in range(config_len2)]

    ep_group3 = [paddle.distributed.new_group(ranks, nccl_config=nccl_config3[i]) for i in range(config_len3)]

    ep_group4 = [paddle.distributed.new_group(ranks, nccl_config=nccl_config4[i]) for i in range(config_len4)]

    ep_group5 = [paddle.distributed.new_group(ranks, nccl_config=nccl_config5[i]) for i in range(config_len5)]

    ep_group6 = [paddle.distributed.new_group(ranks, nccl_config=nccl_config6[i]) for i in range(config_len6)]

    ep_group7 = [paddle.distributed.new_group(ranks, nccl_config=nccl_config7[i]) for i in range(config_len7)]
