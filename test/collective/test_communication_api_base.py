...
class CommunicationTestDistBase(unittest.TestCase):
    ...
    def run_test_case(self, script_file, user_defined_envs=None):
        ...
        if self._nnode > 1:
            start_command = f"{self._python_interp} -u -m paddle.distributed.launch --nnode={self._nnode} --master=127.0.0.1:{self._find_free_port()} --log_dir log_tmp --devices {self._devices} {script_file}"
        else:
            start_command = f"{self._python_interp} -u -m paddle.distributed.launch --log_dir log_tmp --devices {self._devices} {script_file}"
