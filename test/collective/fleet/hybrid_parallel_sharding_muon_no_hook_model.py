...
#
# Real single-machine 8-card Muon test for the sharding-stage1 "no-hook shared
# color" mechanism in MuonShardingOptimizer.
#
# A 3-layer fully connected network is trained under pure sharding (degree=8).
...
class TestMuonNoHookOverlapVsBaseline(unittest.TestCase):
    ...
    def _run(self, weights, comm_overlap):
        """Train the 8-card model for STEPS and return final params as numpy."""
        ...
        if comm_overlap:
            ...
            assert len(inner_opt._comm_buffers) > 0, (
