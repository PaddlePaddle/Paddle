...
class TestShardingV2Rebuild(unittest.TestCase):
    def setUp(self):
        random.seed(2021)
        np.random.seed(2021)
        paddle.seed(2021)
    ...
    def optimizer_clear(self, opt):
        for param_name in opt._inner_opt._slice_params.keys():
            opt._inner_opt._slice_params[param_name]._clear_to_zero_allocation()
        opt._inner_opt._slice_params = {}

        for buffer in opt._comm_buffer_list:
            buffer.param_storage._clear_to_zero_allocation()
    ...
    def sharding_model(self, Optimizer, sharded_accumulators, amp_level=None):
        ...
        for idx in range(STEPS):
            ...
            self.optimizer_clear(optimizer_a)
            optimizer_a.rebuild()
