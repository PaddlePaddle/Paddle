...
class EmbeddingPipe(nn.Layer):
    ...
    def forward(self, input_ids):
        print(f"liyurui, the input of embeding pipe is {input_ids.dtype}, {input_ids.shape}")
...
class TestDistPPTraining(unittest.TestCase):
    ...
    def test_unified_pp_model(self):
        ...
        unified_model_nonpp, unified_optimizer_nonpp = self.wrapper_mix_precision(unified_model_nonpp, unified_optimizer_nonpp)
        unified_model_nonpp = fleet.distributed_model(unified_model_nonpp)
        unified_optimizer_nonpp = fleet.distributed_optimizer(unified_optimizer_nonpp)
        ...
        for _, (input_ids, label) in enumerate(train_reader()):
            ...
            nonpp_loss = unified_model_nonpp.train_batch([input_ids, label], unified_optimizer_nonpp, unified_scheduler_nonpp)
            #print(f"liyurui, pp_loss is {pp_loss}, nonpp_loss is {nonpp_loss}")
