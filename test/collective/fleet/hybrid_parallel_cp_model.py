...
class CPModel(paddle.nn.Layer):
    ...
    def forward(self, x):
        x = Split.apply(x, axis=1, group=self._hcg.get_cp_parallel_group())
        x = self._net.forward(x)
        x = Concat.apply(x, axis=1, group=self._hcg.get_cp_parallel_group())
        loss = x.mean()
        return loss
...
class TestDistCPTraining(unittest.TestCase):
    def setUp(self):
    ...
    def test_basic_hcg(self):
        ...
        assert hcg.get_cp_parallel_world_size() == 2
        assert hcg.get_cp_parallel_group_src_rank() == 0
        assert hcg.get_cp_parallel_group() is not None
        assert hcg.get_dp_cp_parallel_group() is not None
    ...
    def test_cp_train(self):
        cp_model = self.build_model(CPModel)
        cp_model = fleet.distributed_model(cp_model)
        cp_optimizer = self.build_optimizer(cp_model)
        cp_optimizer = fleet.distributed_optimizer(cp_optimizer)
        ...
        for _ in range(5):
            ...
            loss_cp = self.train_batch(batch, cp_model, cp_optimizer)
            loss_dp = self.train_batch(batch, dp_model, dp_optimizer)

            np.testing.assert_allclose(
                loss_cp.numpy(), loss_dp.numpy(), rtol=1e-3
            )
