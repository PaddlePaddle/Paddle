...
int main(int argc, char* argv[]) {
  ...
  // PD_ConfigSwitchIrOptim(config, FALSE);
  ...
  PD_OneDimArrayCstr* input_name = PD_PredictorGetInputNames(predictor);
  PD_Tensor* input_tensor =
      PD_PredictorGetInputHandle(predictor, input_name->data[0]);
  PD_OneDimArrayCstrDestroy(input_name);
  input_name = NULL;
  ...
  PD_OneDimArrayCstr* output_name = PD_PredictorGetOutputNames(predictor);
  PD_Tensor* output_tensor =
      PD_PredictorGetOutputHandle(predictor, input_name->data[0]);
  PD_OneDimArrayCstrDestroy(output_name);
  output_name = nullptr;
