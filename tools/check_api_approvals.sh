...
if [ "$api_annotation_diff" != "" ]; then
    echo_line="You must have one member of Typing group (zrr1999, megemini, sunzhongkai588) approval for API annotation change.\n"
    check_approval 1 zrr1999 megemini sunzhongkai588
fi
...
if [ "$op_type_spec_diff" != "" ]; then
    echo_line="You must have one RD (wanghuancoder or zrr1999) approval for the data_type registration of new operator. More data_type of new operator should be registered in your PR. Please make sure that both float/double (or int/int64_t) have been registered.\n For more details, please click [https://github.com/PaddlePaddle/Paddle/wiki/Data-types-of-generic-Op-must-be-fully-registered].\n"
...
if [ "$op_kernel_dtype_spec_diff" != "" ]; then
    echo_line="You have added or modified Op Kernel, resulting in inconsistent data types supported by the forward and backward kernels of the same op, such modifications are not allowed in principle. If it is a mismatch, please request one RD (wanghuancoder or zrr1999) review and approve. Including the following kernels:\n${op_kernel_dtype_spec_diff}\n"
    check_approval 1 wanghuancoder zrr1999
fi
