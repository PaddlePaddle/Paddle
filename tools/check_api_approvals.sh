...
function check_approval(){
    ...
    APPROVALS=`${approval_line}|python ${PADDLE_ROOT}/tools/check_pr_approval.py $1 $person_num`
...
if [[ $git_files -gt 19 || $git_count -gt 999 ]];then
    ...
    APPROVALS=`check_approval 1 38231817`
fi    
...
if [ "$api_spec_diff" != "" ]; then
    ...
    APPROVALS=`check_approval 1 46782768 47554610 2870059` 
fi
...
if [ "$api_doc_spec_diff" != "" ]; then
    ...
    APPROVALS=`check_approval 1 31623103 2870059`
fi
...
for API_FILE in ${API_FILES[*]}; do
  ...
  if [ "${API_CHANGE}" ] && [ "${GIT_PR_ID}" != "" ]; then
      ...
      if [ "${API_FILE}" == "paddle/fluid/op_use_default_grad_op_maker.spec" ];then
          ...
          APPROVALS=`check_approval 1 32832641 6836917`
          ...
          APPROVALS=`check_approval 1 6836917 46782768`
          ...
          APPROVALS=`check_approval 1 6836917 47554610`
          ...
          APPROVALS=`check_approval 1 6836917 22361972`
          ...
          APPROVALS=`check_approval 1 10721757 5442383`
      else
          ...
          APPROVALS=`check_approval 1 3048612 46782768 12538138 6836917 32832641`
      fi
  fi
done
...
if [ ${HAS_DEFINE_FLAG} ] && [ "${GIT_PR_ID}" != "" ]; then
    ...
    APPROVALS=`check_approval 1 47554610` 
fi
...
if [ "${ALL_PADDLE_ENFORCE}" != "" ] && [ "${GIT_PR_ID}" != "" ]; then
    ...
    APPROVALS=`check_approval 1 6836917 47554610 22561442`
fi
