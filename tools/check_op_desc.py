...
def diff_vars(origin_vars, new_vars):
    ...
    for var_name in common_vars_name:
        if operator.eq(origin_vars.get(var_name), new_vars.get(var_name)):
            continue
        else:
...
def diff_attr(ori_attrs, new_attrs):
    ...
    for attr_name in common_attrs:
        if operator.eq(ori_attrs.get(attr_name), new_attrs.get(attr_name)):
            continue
        else:
...
def compare_op_desc(origin_op_desc, new_op_desc):
    ...
    if operator.eq(origin_op_desc, new_op_desc):
