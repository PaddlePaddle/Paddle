...
function coverage_process_before() {
    # run paddle coverage
...
function coverage_process_after() {
    # gen html report
    # download coverage info file from run test step
    # wget 
    cd /paddle
...
function main() {
    local CMD=$1
    case $CMD in
      coverage_process_before)
        coverage_process_before
        ;;
      coverage_process_after)
        coverage_process_after
        ;;
}
