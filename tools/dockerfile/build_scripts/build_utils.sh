...
function do_cpython_build {
    ...
    if [ "$(lex_pyver "$py_ver")" -ge "$(lex_pyver 3.6)" ]; then
        ...
        cd sqlite-autoconf-3250300 || exit
    ...
    cd /
    ls "${MY_DIR}"
    LD_LIBRARY_PATH="${prefix}/lib" "${prefix}"/bin/python "${MY_DIR}"/python-tag-abi-tag.py
    local abi_tag="$LD_LIBRARY_PATH"
