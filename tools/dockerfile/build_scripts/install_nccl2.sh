...
if [ "$VERSION" == "10.0" ]; then
  ...
  if [ -f "/etc/redhat-release" ];then
    rm -f /usr/local/lib/libnccl.so
    wget --no-check-certificate -q https://nccl2-deb.cdn.bcebos.com/libnccl-2.16.5-1+cuda12.0.x86_64.rpm
    wget --no-check-certificate -q https://nccl2-deb.cdn.bcebos.com/libnccl-devel-2.16.5-1+cuda12.0.x86_64.rpm
    wget --no-check-certificate -q https://nccl2-deb.cdn.bcebos.com/libnccl-static-2.16.5-1+cuda12.0.x86_64.rpm
    rpm -Fivh libnccl-2.10.3-1+cuda11.4.x86_64.rpm
    rpm -Fivh libnccl-devel-2.10.3-1+cuda11.4.x86_64.rpm
    rpm -Fivh libnccl-static-2.10.3-1+cuda11.4.x86_64.rpm && rm -f libnccl-*
    exit 0
  fi
  DEB="nccl-repo-ubuntu2004-2.16.5-cuda12.0_1.0-1_amd64.deb"
else
  echo "nccl not found"
  exit 2
fi
