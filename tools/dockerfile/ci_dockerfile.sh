...
function make_ubuntu_dockerfile(){
  ...
  sed -i "s#bash /build_scripts/install_nccl2.sh#wget -q --no-proxy https://nccl2-deb.cdn.bcebos.com/nccl-repo-ubuntu1604-2.7.8-ga-cuda10.1_1-1_amd64.deb \\
    ...
    RUN apt update \&\& apt remove -y libnccl* --allow-change-held-packages \&\&  apt-get install -y
    libnccl2=2.7.8-1+cuda10.1 libnccl-dev=2.7.8-1+cuda10.1 pigz numactl --allow-change-held-packages #g" ${dockerfile_name}
}
