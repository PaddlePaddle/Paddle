...
function make_ubuntu20_cu12_dockerfile(){
  ...
  sed -i 's#RUN apt-get install -y gcc-12 g++-12#RUN add-apt-repository ppa:ubuntu-toolchain-r/test \&\& apt-get update \&\& apt-get install -y gcc-15 g++-15#g' ${dockerfile_name}
  sed -i 's#/usr/bin/gcc-12#/usr/bin/gcc-15#g' ${dockerfile_name}
  sed -i 's#/usr/bin/g++-12#/usr/bin/g++-15#g' ${dockerfile_name}
  sed -i 's#ENV PATH=/usr/local/gcc-12/bin:$PATH##g' ${dockerfile_name}
}
