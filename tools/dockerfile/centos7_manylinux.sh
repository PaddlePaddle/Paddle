...
function make_cuda124cudnn911trt8616() {
  ...
  sed -i "s#<install_gcc>#RUN dnf install -y gcc-toolset-12-gcc* \&\& scl enable gcc-toolset-12 bash \&\& echo 'source /opt/rh/rh-gcc-toolset12/enable' >>~/.bashrc #g" Dockerfile.tmp
