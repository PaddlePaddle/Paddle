...
function run_test {
    ...
    if [ ${TESTING_DEBUG_MODE:-OFF} == "ON" ] ; then
        ctest --parallel 10 -V -E "test_frontend_interpreter|test_cinn_fake_resnet" --repeat until-pass:100
    else
        ctest --parallel 10 --output-on-failure -E "test_frontend_interpreter|test_cinn_fake_resnet" --repeat until-pass:100
    fi
}
