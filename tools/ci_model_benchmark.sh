...
function check_whl {
    ...
    pip install build/pr_whl/paddlepaddle_gpu*.whl
    [ $? -ne 0 ] && echo "install paddle failed." && exit 1
    
    unzip -q build/pr_whl/paddlepaddle_gpu*.whl -d /tmp/pr
    unzip -q build/dev_whl/paddlepaddle_gpu*.whl -d /tmp/develop
