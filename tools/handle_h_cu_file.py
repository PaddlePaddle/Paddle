...
def insert_pile_to_h_file(rootPath):
    ...
    for line in lines:
        ...
        if line.endswith('paddle/fluid/operators/detection/poly_util.h'):
            os.system('echo "\n#ifndef _ZCLPRECISE_H_\n" >> %s' %line)
            os.system('echo "#define _ZCLPRECISE_H_" >> %s' %line)
            os.system('echo "\n#include <cstdio>\n" >> %s' %line)
            os.system('echo "__attribute__((constructor)) static void calledFirst%s()\n{" >> %s' %(func, line))
            os.system('echo \'    printf("precise test map fileeee: %%s\\\\n", __FILE__);\n}\' >> %s' %line)
            os.system('echo "\n#endif" >> %s' %line)
        else:  
            os.system('echo "\n#include <cstdio>\n" >> %s' %line)
            os.system('echo "__attribute__((constructor)) static void calledFirst%s()\n{" >> %s' %(func, line))
...
def get_h_cu_file(file_path):
    ...
    os.system("cat %s/%s | grep 'precise test map fileeee:'| uniq >> %s/build/ut_map/%s/%s.txt" %(dir_path, filename, rootPath, ut, ut))
