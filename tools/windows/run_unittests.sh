...
if [ -f "$PADDLE_ROOT/added_ut" ];then
    ...
    compute-sanitizer --tool memcheck --target-processes all ctest -R "(${added_uts})" -E "${disable_win_inference_test}" --output-on-failure -C Release --repeat-until-fail 3;added_ut_error=$?
...
function run_unittest_gpu() {
    ...
    tmpfile=$tmp_dir/$RANDOM
    (compute-sanitizer --tool memcheck --target-processes all ctest -R "$test_case" -E "$disable_ut_quickly|$disable_wingpu_test|$disable_win_trt_test|$long_time_test" -LE "${nightly_label}" --output-on-failure -C Release -j $parallel_job | tee $tmpfile ) &
    wait;
}
    ...
    if [ $need_retry_ut_counts -lt $exec_retry_threshold ];then
            ...
            while ( [ $exec_times -lt $retry_time ] )
                do
                    ...
                    rm -f $tmp_dir/*
                    failed_test_lists=''
                    (compute-sanitizer --tool memcheck --target-processes all ctest -R "($retry_unittests_regular)" --output-on-failure -C Release -j 1 | tee $tmpfile ) &
                    wait;
