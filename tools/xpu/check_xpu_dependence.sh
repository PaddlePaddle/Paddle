...
function check_xhpc() {
    ...
    local xhpc_files=("xblas/include/cublasLt.h" "xblas/so/libxpu_blas.so" "xblas/dependency_so/libxpujitc.so" "xblas/dependency_so/libLLVM-15.so" "xblas/dependency_so/libclang-cpp.so" "xblas/dependency_so/libclang-cpp.so.15" "xdnn/include/xpu/xdnn.h" "xdnn/so/libxpuapi.so" "xfa/include/flash_api.h" "xfa/so/libxpu_flash_attention.so" "xpudnn/include/xpudnn.h" "xpudnn/so/libxpu_dnn.so")
