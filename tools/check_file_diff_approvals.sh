...
if [ "${HAS_MODIFIED_DY2ST_TEST_FILES}" != "" ] && [ "${GIT_PR_ID}" != "" ]; then
    python ${PADDLE_ROOT}/test/dygraph_to_static/check_approval.py ${HAS_MODIFIED_DY2ST_TEST_FILES}
    if [ $? -ne 0 ]; then
        ...
        echo_line=${echo_line}"You must have one RD (SigureMo, Aurelius84, 2742195759 or gouzil) approval for the changes.\n"
