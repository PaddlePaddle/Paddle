...
for API_FILE in ${API_FILES[*]}; do
  ...
  if [ "${API_CHANGE}" ] && [ "${GIT_PR_ID}" != "" ]; then
      ...
      if [ "${API_FILE}" == "CMakeLists.txt" ];then
          ...
          echo_line="You must have one RD (luotao1, lanxianghit, phlrain) approval for the ${API_FILE}, which manages the white list of instance size 0 input for sequence op test. For more information, please refer to [https://github.com/PaddlePaddle/Paddle/wiki/It-is-required-to-include-Tensor-input-with-instance_size=0-in-sequence-OP-test]. \n"
          ...
          echo_line="You must have one RD (luotao1, lanxianghit or phlrain) approval for ${API_FILE}, which manages the white list of batch size 1 input for sequence op test. For more information, please refer to [https://github.com/PaddlePaddle/Paddle/wiki/It-is-required-to-include-Tensor-input-with-batch_size=1-in-sequence-OP-test]. \n"
          check_approval 1 6836917 43953930 47554610
