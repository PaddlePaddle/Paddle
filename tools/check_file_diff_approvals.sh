...
DEPS_PHI_IN_IR=`git diff --name-only upstream/develop | grep -E "paddle/ir/" | grep "CMakeList" |xargs -I param sh -c "cd .. && git diff -U0 upstream/develop param| grep '^\+' | grep 'phi'" || true`
