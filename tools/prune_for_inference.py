...
import os
import re
from paddle.inference import get_phi_kernel_name
...
def get_model_phi_kernels(ops_set):
    phi_set = set()
    for op in ops_set:
        phi_set.add(get_phi_kernel_name(op))
...
if __name__ == '__main__':
    ...
    ops_set = np.array(list(ops_set))
    phi_set = np.array(list(phi_set))
    np.savetxt("op_list.txt", ops_set, fmt='%s')
    np.savetxt("kernel_list.txt", phi_set, fmt='%s')
