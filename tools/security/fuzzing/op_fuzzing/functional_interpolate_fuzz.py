...
def TestOneInput(input_bytes):
    ...
    x, x_rank = m.tensor_with_diff_shape(
        min_val=0.0, max_val=30.0, min_dim=0, max_dim=10, max_rank=5)
    size, size_rank = m.tensor_with_diff_shape(
        min_val=0.0, max_val=30.0, min_dim=0, max_dim=10, max_rank=5)
    scale_factor, sf_rank = m.tensor_with_diff_shape(
        min_val=0.0, max_val=30.0, min_dim=0, max_dim=10, max_rank=5)

    mode_list = [
        "bilinear", "trilinear", "nearest", "bicubic", "linear", "area"
    ]
