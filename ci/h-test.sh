...
distribute_list="^test_buffer_shared_memory_reuse_pass$|\
...
for (( i = 0; i < $NUM_PROC; i++ )); do
    cuda_list="$((i*2)),$((i*2+1))"
    (env CUDA_VISIBLE_DEVICES=$cuda_list ctest -I $i,,$NUM_PROC --output-on-failure -R "($fp8_list)" -E "($disable_ut_quickly)" --timeout 120 -j4 | tee -a $tmpfile; test ${PIPESTATUS[0]} -eq 0)&
    pids+=($!)
done
...
for (( i = 0; i < $NUM_PROC; i++ )); do
    cuda_list="$((i*2)),$((i*2+1))"
    (env CUDA_VISIBLE_DEVICES=$cuda_list ctest -I $i,,$NUM_PROC --output-on-failure -R "($distribute_list)" -E "($disable_ut_quickly)" --timeout 120 -j2 | tee -a $tmpfile1; test ${PIPESTATUS[0]} -eq 0)&
    pids+=($!)
done
