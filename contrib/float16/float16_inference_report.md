...
We can then run various inference experiments in float16 mode and save the float16 program and weights on disk for future deployment. To make it easier to write and test code, we maintain a consistent API so that user can use the same float32 input data to run inference program in either float32 and float16 mode and obtain output data both of float32 data type. The details of why we need this transpiler and how it is implemented can be found [here](https://github.com/PaddlePaddle/Paddle/blob/develop/doc/fluid/design/data_type/float16.md#float16-inference).
...
We can see that the accuracy of float16 inference is very close to that of float32 inference in every experiment (within 0.05% difference) and is 0.01% better than its float32 counterpart if averaged over 10 tests. 
...
We thus benchmark the float16 inference performance on a single Nvidia V100 GPU and compare it with its float32 counterpart. All the following results are in ms (millisecond) averaged over 1000 mini-batches with respective to different mini-batch(mb) sizes.
...
Usually in CNN, the convolution operation is the computational bottleneck, so we can check the average time spent on the convolution operator for one mini-batch as follows:
...
Our convolution operator uses cuDNN 7 to implement the kernel and we can see that with the help of tensor core, float16 convolution is significantly faster than its float32 counterpart, which makes the overall float16 inference performance much better.
