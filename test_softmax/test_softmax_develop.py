...
class TestSoftmax_FP32(unittest.TestCase):
    def setUp(self):
        self.dtype = "bfloat16"
        self.rtol = 1e-3
        self.atol = 1e-3
    ...
    def gen_paddle_inputs(self):
        ...
        p_x = paddle.to_tensor(self.np_x, place="gpu").astype(self.dtype)
        p_x.stop_gradient = False
        # print(p_x)
        # p_dout = paddle.to_tensor(self.np_dout, dtype=self.dtype, place="gpu")
        p_dout = paddle.to_tensor(self.np_dout, place="gpu").astype(self.dtype)
        p_dout.stop_gradient = False
        return p_x, p_dout

    def gen_paddle_static_inputs(self):
        p_x_static = paddle.static.data("x", self.shape, dtype="float32")
        # p_x_static = paddle.static.data("x", self.shape, dtype=self.dtype if self.dtype != "bfloat16" else "float32")
        p_x_static.stop_gradient = False
        p_dout_static = paddle.static.data("dout", self.shape, dtype="float32")
        # p_dout_static = paddle.static.data("dout", self.shape, dtype=self.dtype if self.dtype != "bfloat16" else "float32")
    ...
    def cal_paddle_res(self, x, dout):
        out = paddle.nn.functional.softmax(x, axis=self.axis, dtype=self.dtype)
    ...
    def compare_paddle_res_to_torch(self):
        ...
        del p_x, p_dout
        p_out_np = p_out.numpy()
        p_grads_np = [grad.numpy() for grad in p_grads]
        del p_out, p_grads
        ...
        print(np.isnan(p_out_np).any())
        print(np.isnan(t_out.detach().cpu().numpy()).any())
...
# class TestSoftmax_FP16(TestSoftmax_FP32):
#     def setUp(self):
#         self.dtype = "float16"
#         self.rtol = 1e-4
#         self.atol = 1e-4
...
# class TestSoftmax_BF16(TestSoftmax_FP32):
#     def setUp(self):
#         self.dtype = "bfloat16"
#         self.rtol = 1e-3
#         self.atol = 1e-3
