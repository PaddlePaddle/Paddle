...
template <typename T>
static  T* VectorToTensor(const std::vector<T>& selected_indices,const framework::ExecutionContext& ctx, int selected_num) {
...
class MergedMomentumOpXPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    paddle::framework::Tensor lr_vec;
    lr_vec.Resize({sum});
    ...
    for(size_t i=0;i<n;i++){
      if(params[i] && params[i]->numel()>0){
        ...
        auto* y_data = params[i]->mutable_data<T>(ctx.GetPlace());       	
	...
	idx +=params[i]->numel();
      ...
      if(params_out[i] && params_out[i]->numel()>0){
        ...
        auto* y_data = params_out[i].mutable_data<T>(ctx.GetPlace());       	
	...
	idx +=params_out[i]->numel();
      ...
      if(grad[i] && grad[i]->numel()>0){
        ...
        auto* y_data = grad[i]->mutable_data<T>(ctx.GetPlace());       	
	...
	idx +=grad[i]->numel();
      ...
      if(lr[i] && lr[i]->numel()>0){
        int idx=0;
        auto* keep_data = lr_vec.mutable_data<T>(ctx.GetPlace());       	
        auto* y_data = lr[i]->mutable_data<T>(ctx.GetPlace());       	
        for(int j=0;j<lr[i]->numel();j++){
	   keep_data[j+idx]=y_data[j];
      } 
	idx +=lr[i]->numel();
        std::cout<<"7. ******************"<<std::endl;
   ...
   */ 
    ...
    int r = xpu::merged_momentum(dev_ctx.x_context(),params_vec.data<T>(),
                          ...
                          sum,lr_vec.data<T>(), use_nesterov,mu_ptr,len,sizes_ptr);
