...
The PaddlePaddle framework supports almost all popular optimizers. Varies optimizer has diﬀerent memory requirement. For example, the :code:`adadelta` consumes approximately 5 times memory
...
Some optimization algorithms such as :code:`momentum` are worth to give a shot.
...
* Reduce the time consuming of data loading
...
4. How to ﬁx training termination caused by :code:`Floating point exception` during training.
------------------------------------------------------------------------
...
Moreover, Such problems may be ﬁxed with smaller learning rates or data normalization.
