...
class BashJob(KubeJob):
    ...
    def run(self, filename, trainner_count, docker_image=DOCKER_IMAGE):
        ...
        k8s_filename = os.path.join(CONFIG_MAP_MOUNT_PATH, os.path.basename(filename))
        config_map = self._prepare_config_map(filename)
        api_instance = client.CoreV1Api()
        try:
            config_maps = api_instance.list_namespaced_config_map(self._namespace)
            updated = False
            for item in config_maps.items:
                if self._name == item.metadata.name:
                    updated = True
                    api_instance.patch_namespaced_config_map(self._name, self._namespace, config_map)
                    break
            if not updated:
                api_instance.create_namespaced_config_map(self._namespace, config_map)
