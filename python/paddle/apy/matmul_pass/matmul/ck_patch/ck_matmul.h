...
#include "ck_patch/default_config_id.h"
...
template <typename T, int VecSize>
__device__ __forceinline__ auto load_vector(const T* ptr,
                                            int64_t offset,
                                            bool valid,
                                            int64_t size) {
  using CKType = typename ck::CKDataType<T>::Type;
  return ck::amd_buffer_load_invalid_element_return_zero<CKType, VecSize>(
      reinterpret_cast<const CKType*>(ptr), offset, valid, size);
}

template <typename T, int VecSize, int I>
__device__ __forceinline__ constexpr const auto& extract_scalar(
    const ck::vector_type<T, VecSize>& vec, ck::Number<I> i) {
  return vec.template AsType<T>()(i);
}

template <typename T, int VecSize, int I>
__device__ __forceinline__ constexpr auto& extract_scalar(
    ck::vector_type<T, VecSize>& vec, ck::Number<I> i) {
  return vec.template AsType<T>()(i);
}
...
void MatmulAddVariadic(
  ...
  using AlignedABlockTransferSrcVectorTensorLengths =
      typename ck::AlignBlockTransferTensor<
          typename Config::ABlockTransferSrcVectorTensorLengths,
          AlignA,
          0,
          3>::type;
  using AlignedBBlockTransferSrcVectorTensorLengths =
      typename ck::AlignBlockTransferTensor<
          typename Config::BBlockTransferSrcVectorTensorLengths,
          AlignB,
          1,
          2>::type;

  static constexpr int ALignedCThreadTransferDstScalarPerVector =
      std::min(Config::CThreadTransferDstScalarPerVector, AlignB);
  static constexpr int VecSize = ALignedCThreadTransferDstScalarPerVector;

  using AElementOp = PassThrough;
  using BElementOp = PassThrough;
  using CDEElementOp = ck::tensor_operation::element_wise::
      VariadicElementwiseOp<VariadicFunctor, AccDataType, VecSize>;

  static constexpr auto GemmDefault =
      ck::tensor_operation::device::GemmSpecialization::MNPadding;

  using DeviceOpInstance =
      ck::tensor_operation::device::DeviceBatchedGemmMultipleD_Dl_WithVariadic<
          ALayout,
          BLayout,
          DsLayout,
          ELayout,
  ...
  auto argument = device_op.MakeArgument(params.input,
                                         params.weight,
                                         {},  // TODO check bias
                                         params.output,
                                         params.m,
  ...
  if (!device_op.IsSupportedArgument(argument)) {
    std::cout << "wrong! this device_op instance does not support this problem"
              << std::endl;
    exit(-1);
  }
