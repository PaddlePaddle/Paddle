...
def array_write(x, i, array=None):
    """
    This OP writes the input ``x`` to the i-th position of the ``array``
    ...
    Args:
        ...
        i (Variable): 1-d Tensor with shape [1], which represents the position to which
                      ``x`` is written. data type: int64.
        array (Variable, optional): the LoDTensorArray to which ``x`` is written. 
                                    The default value is None, when a new LoDTensorArray
                                    will be created and returned as a result.

    Returns:
        Variable: the input ``array`` after ``x`` is written.
    ...
    Examples:
        .. code-block:: python
            ...
            #Write tmp to the position of arr with subscript 10 and return arr.
            arr = fluid.layers.array_write(tmp, i=i)

            #Now, arr is a LoDTensorArray of length 11. We can use array_read OP
            #to view the data with subscript 10 and print it out.
            ...
            #dtype is the corresponding C++ data type, which may vary in different
            #environments, but the essence is the same.
            #Eg: if the data type of tensor is int64, then the corresponding C++ data type is int64_t, 
            #     so the dtype value is typeid(int64_t). Name (), which is 'x' in MacOS, 'l' in Linux, 
            #     and '__int64' in Windows. They both represent 64-bit integer variables.
...
def array_read(array, i):
    """
    This OP is used to read data at the specified position in the input array 
    ...
    ::
        Input:
            The shape of first three tensors are [1], the last one is [1,2]:
    ...
    Args:
        array (Variable): the input LoDTensorArray.
        i (Variable): 1-D Tensor, whose shape is [1] and dtype is int64. It represents the
                    specified read position of ``array`` .

    Returns:
        Variable: The LoDTensor or Tensor that is read from the specified position of ``array`` .

    Examples:
        .. code-block:: python

            #First we're going to create a LoDTensorArray, then we're going to write the Tensor into
            #the specified position, and finally we're going to read the Tensor from that position.
            import paddle.fluid as fluid
...
def array_length(array):
    ...
    Args:
        array (LoDTensorArray): The input array that will be used
    ...
    Examples:
        .. code-block:: python
            ...
            i = fluid.layers.fill_constant(shape=[1], dtype='int64', value=10)
            #tmp is 1-D Tensor with shape [10], writing tmp into arr on subscript 10,
            #and the length of arr becomes 11
            arr = fluid.layers.array_write(tmp, i=i)
            #View the length of arr
            ...
            #1-D Tensor with shape [1], whose value is 11. It means that the
            #length of LoDTensorArray is 11.
            #dtype is the corresponding C++ data type, which may vary in different
            #environments, but the essence is the same.
            #Eg: if the data type of tensor is int64, then the corresponding C++ data type is int64_t, 
            #     so the dtype value is typeid(int64_t). Name (), which is 'x' in MacOS, 'l' in Linux, 
