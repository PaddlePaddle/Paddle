...
def density_prior_box(input,
    ...
    **Density Prior Box Operator**
    ...
    determined by the count of min_sizes, max_sizes, aspect_ratios, 
    densities, fixed_sizes and fixed_ratios. Besides boxes generated by
    prior_box operator, boxes center at grid points around each input 
    position is also generated by this operator, and the grid points is
    determined by densities and the count of density prior box is determined 
    by fixed_sizes and fixed_ratios. Obviously, the number of fixed_sizes 
    is equal to the number of densities.
    N = N_prior_box + N_density_prior_box,
    if fixed_ratios is supported, for densities_i in densities:
    N_density_prior_box =sum(N_fixed_ratios * densities_i^2),
    if fixed_ratios is not supported, use the N_aspect_ratios instead.

    Args:
       input(Variable): The Input Variables, the format is NCHW.
       image(Variable): The input image data of PriorBoxOp,
            the layout is NCHW.
       min_sizes(list|tuple|float value): min sizes of generated density prior boxes.
       max_sizes(list|tuple|None): max sizes of generated density prior boxes.
            Default: None.
       aspect_ratios(list|tuple|float value): the aspect ratios of generated
            prior boxes. Default: [1.].
       ...
       min_max_aspect_ratios_order(bool): If set True, the output density prior box is
            in order of [min, max, aspect_ratios], which is consistent with
            Caffe. Please note, this order affects the weights order of
            convolution layer followed by and does not affect the final
            detection results. Default: False.
