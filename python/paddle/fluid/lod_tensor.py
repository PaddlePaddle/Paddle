...
def create_lod_tensor(data, recursive_seq_lens, place):
    """
    Create a Tensor from a numpy array, list or existing Tensor.
    ...
    3. Based on :code:`place` , copy the :code:`data` from a numpy array, list
       or existing Tensor to CPU or GPU device.

    4. Set offset-based LoD to the output Tensor.

    Suppose we want to create a Tensor to hold data for word sequences,
    where each word is represented by an integer. If we want to create
    a Tensor to represent two sentences, one of 2 words, and one of 3 words.
    ...
    Args:
        data (numpy.ndarray|list|Tensor): a numpy array, a list or ad Tensor
        ...
        place (CPUPlace|CUDAPlace): CPU or GPU place indicating where the data
                in the created Tensor will be stored.

    Returns:
         A Tensor with tensor data and recursive_seq_lens info.
    ...
    """
    if isinstance(data, core.Tensor):
        ...
        raise TypeError(
            "data should be either a Tensor, a Numpy array or a list")
...
def create_random_int_lodtensor(recursive_seq_lens, base_shape, place, low,
    ...
    Create a Tensor containing random integers.

    The implementation is as follows:

    1. Obtain the shape of output Tensor based on :code:`recursive_seq_lens`
    ...
    Suppose we want to create a Tensor to hold data for 2 sequences, where
    ...
    Therefore, the shape of the output Tensor would be [5, 30], where
    ...
    Args:
        recursive_seq_lens (list[list[int]]): a list of lists indicating the
                length-based LoD info.
        base_shape (list[int]): the shape of the output Tensor excluding
                the first dimension.
        place (CPUPlace|CUDAPlace): CPU or GPU place indicating where
                the data in the created Tensor will be stored.
        low (int): the lower bound of the random integers.
        high (int): the upper bound of the random integers.

    Returns:
        A Tensor with tensor data and recursive_seq_lens info, whose data
        is inside [low, high].
