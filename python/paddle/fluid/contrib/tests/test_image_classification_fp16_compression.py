...
def train(net_type, use_cuda, save_dirname, is_local):
    ...
    with fluid.program_guard(train_program, startup_prog):
        ...
        optimizer.register_gradients_hook(fluid.contrib.fp16_compression)
        ...
        for pass_id in range(PASS_NUM):
            for batch_id, data in enumerate(train_reader()):
                loss, = exe.run(main_program,
                                ...
                                fetch_list=[avg_cost])
                print('PassID {0:1}, BatchID {1:04}, train loss {2:2.4}'.format(
                    pass_id, batch_id, float(loss)))
                if (batch_id % 10) == 0:
                    acc_list = []
                    avg_loss_list = []
...
def main(net_type, use_cuda, is_local=True):
    ...
    # save_dirname = "image_classification" + net_type + ".inference.model"
    save_dirname = None
