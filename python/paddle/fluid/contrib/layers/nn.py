...
def embedding(input,
    ...
    **Note:** The id in :attr:`input` must satisfy :math:`0 =< id < size[0]` ,
    ...
    Args:
        ...
        is_test (bool): A flag indicating whether it is in test phrase or not. when is_test is True, the id do not exist in lookup table will return 0.
    ...
    if not isinstance(input, Variable):
        raise TypeError(
            "The type of 'input' in contrib.layers.embedding must be Variable, but received %s"
            % (type(input)))
    if convert_dtype(input.dtype) not in ['int64']:
        raise TypeError(
            "The data type of 'input' in contrib.layers.embedding must be int64, but received %s."
