...
class UniformPruneStrategy(PruneStrategy):
    ...
    def on_compression_begin(self, context):
        ...
        #        logger.info('Pruned metric: {:.2f}'.format(metric-self.metric))
        logger.info(
...
class SensitivePruneStrategy(Strategy):
        ...
        for param in self.sensitivities.keys():
            ...
            while ratio < 1:
                ...
                if self.metric is None:
                    self.metric = self._eval_graph(context)
                ...
                logger.debug("pruned param: {}; {}; loss={}".format(
                    param, ratio, loss))
    ...
    def _get_best_ratios(self, context):
        ...
        coefficients = {}
        for param in self.sensitivities:
            losses = np.array(self.sensitivities[param]['loss'])
            precents = np.array(self.sensitivities[param]['pruned_percent'])
