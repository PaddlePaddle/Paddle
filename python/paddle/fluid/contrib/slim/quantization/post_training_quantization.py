...
class PostTrainingQuantization(object):
    ...
    def _collect_target_varnames(self):
        ...
        for index in range(self._program.num_blocks):
            print("clollect target varnames in block {}".format(index))
    ...
    def _update_program(self):
        ...
        if only_process_one_block:
            print("only quantize one block")
            graph = IrGraph(core.Graph(self._program.desc, 0), for_test=True)
            self._draw_graph(graph, save_path, "one_block_origin")
            graph = _apply_pass(graph)
            self._draw_graph(graph, save_path, "one_block_processed")
