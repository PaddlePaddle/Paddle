...
class TransformForMkldnnPass(object):
    def __init__(self, scope=None, place=None):
        """
        Convert QuantizationFreezePass generated IrGraph to MKL-DNN suppport INT8
        runnable IrGraph. Following transformations have been done in this pass:
            1. Convert int8 range weights with fload dtype, which generated by the
               QuantizationFreezePass, to fp32 range weights with float dtype by the
               corresponding scales. This is because MKL-DNN INT8 conv2d kernelnow
               only support fp32 weights input, will do the fp32 -> int8  inside the kernel.
            2. Create the new conv2d op with the converted weights and link its output
               to fake_dequantize_abs_max output and set conv2d's attribute "force_fp32
               _output" as true
    ...
    def apply(self, graph):
        ...
        activation quantization type, the graph will transform fake 
        quantize operators to quantize operators and remove the fake 
    ...
    def _transform_to_conv_mkldnn(self, graph, op_node):
        ...
        #Convert int8 range weights to fp32 range 
        ...
        # fake_dequantize_abs_max's output as new output of conv2d
