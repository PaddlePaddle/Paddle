    ...
    def reset(self, range_table, constrain_func=None):
        ...
        Args:
            range_table(list<int>): It is used to define the searching space of controller.
                                    The tokens[i] generated by controller shold be in [0, range_table[i]).
...
class SAController(EvolutionaryController):
    ...
    def next_tokens(self, control_token=None):
        ...
        if control_token:
            ...
            tokens = self._tokens
        new_tokens = tokens[:]
        indice = int(len(self._range_table) * np.random.random())
        new_tokens[indice] = (
            new_tokens[indice] + np.random.randint(self._range_table[indice] - 1)
            + 1) % self._range_table[indice]
        _logger.info("change indice[{}] from {} to {}".format(indice, tokens[
            indice], new_tokens[indice]))
        ...
        for _ in range(self._max_iter_number):
            if not self._constrain_func(new_tokens):
                indice = int(len(self._range_table) * np.random.random())
                new_tokens = tokens[:]
                new_tokens[indice] = np.random.randint(self._range_table[indice])
            else:
                break
        return new_tokens
