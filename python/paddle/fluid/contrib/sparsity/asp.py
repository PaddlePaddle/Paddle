...
class ASPHelper(object):
    ...
    def decorate(optimizer):
        r"""
        Wrap the given optimizer as a ASP Optimizer.
        The wrapper allows to use the same way of calling ASPHelper.minimize() 
        as calling Optimizer.minimize().

        *Note*: This function is used to replace `ASPHelper.minimize` when applying distributed training
        with `Fleet` due to there is a invisiable graphs optimization in `Fleet.minimize()` which make 
        training graph cannot be modified anymore..

        Args:
            optimizer (Optimizer): A Optimizer used for training.
        Returns:
            ASPOptimizerWrapper: A wrapper for ASP to decorate `minimize` function of the given optimizer.
        Examples:
            .. code-block:: python
              ...
              with fluid.program_guard(main_program, startup_program):
                    ...
                    # Equal to call ASPHelper.minimize(optimizer, loss, main_program, startup_program)
              ...
              optimizer = fluid.optimizer.SGD(learning_rate=0.1)
              optimizer = ASPHelper.decorate(optimizer) # Need to be called before `fleet.distributed_optimizer`
    ...
    def minimize(cls,
        ...
        *Note*: Please use `ASP.decorate` instead when applying distributed training with `Fleet`. 
        ...
        Examples:
            .. code-block:: python
              ...
              with fluid.program_guard(main_program, startup_program):
                    ...
                    optimizer = fluid.optimizer.SGD(learning_rate=0.1)
                    ASPHelper.minimize(optimizer, loss, main_program, startup_program)
        ...
        cls.create_mask_variables(main_program, startup_program,
                                  params_and_grads)
        cls.insert_sparse_mask_ops(main_program, params_and_grads)
        return optimizer_ops, params_and_grads

    @classmethod
    def create_mask_variables(cls, main_program, startup_program,
                              params_and_grads):
        r"""
        ...
        Examples:
            .. code-block:: python
              ...
              with fluid.program_guard(main_program, startup_program):
                    ...
                    # Equal to ASPHelper.minimize(optimizer, loss, main_program, startup_program)
            ...
            for param_and_grad in params_and_grads:
                if ASPHelper.is_supported_layer(main_program,
                    ...
                    mask_param = layers.create_parameter(
                        ...
                        ASPHelper.MASKE_APPENDDED_NAME,
    ...
    @classmethod
    def insert_sparse_mask_ops(cls, main_program, param_grads):
        r"""
        ...
        *Note*: If calling this function with :attr:`with_mask`, it should call `ASPHelper.minimize` 
        ...
        Examples:
            .. code-block:: python
              ...
              import paddle.fluid as fluid
              from paddle.fluid.contrib.sparsity import ASPHelper, MaskAlgo
              ...
              ASPHelper.prune_model(place, main_program, func_name=MaskAlgo.MASK_2D_BEST)
        """
            ...
            if ASPHelper.is_supported_layer(main_program, param.name):
                ...
                if with_mask:
                    weight_mask_param = global_scope().find_var(
                        ASPHelper.get_mask_name(param.name))
                    assert weight_mask_param is not None, \
                        'Cannot find {} variable, please call ASPHelper.minimize' \
                        ' and initialization (exe.run(startup_program)) first!'.format(ASPHelper.get_mask_name(param.name))
...
class ASPOptimizerWrapper(object):
    ...
    to be the same interface as `Optimizer.minimize`
    """
