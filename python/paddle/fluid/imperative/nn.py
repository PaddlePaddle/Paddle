...
class Dropout(layers.Layer):
    def __init__(self,
                 ...
                 is_test=False,
                 seed=None,
                 dropout_implementation="downgraph_in_infer"):
        super(Dropout, self).__init__(name_scope)
    ...
    def forward(self, input):
        out = self._helper.create_variable_for_type_inference(dtype=self._dtype)

        mask = self._helper.create_variable_for_type_inference(
            dtype=self._dtype, stop_gradient=True)

        if (self._seed is None or
                self._seed == 0) and self._helper.main_program.random_seed != 0:
            self._seed = self._helper.main_program.random_seed
        ...
        self._helper.append_op(
            ...
            outputs={'Out': [out],
                     'Mask': [mask]},
            attrs={
                'dropout_prob': self._droput_prob,
                'is_test': self._is_test,
