...
_name_mapping = {
    "graph_send_recv": {
        "final_op_name": "_graph_send_recv",
        "x": "X",
        "src_index": "Src_index",
        "dst_index": "Dst_index",
        "out": "Out",
    ...
    # },
    "trunc": {
        "final_op_name": "_trunc",
        "x": "X",
        "out": "Out",
    },
    # "pool2d": {
    #     "final_op_name": "_pool2d",
    #     "x": "X",
    #     "kernel_size": "ksize",
    #     "out": "Out",
    # },
    ...
    "digamma": {
        "final_op_name": "_digamma",
        "x": "X",
        "out": "Out",
    },
    "diagonal": {
        "final_op_name": "_diagonal",
        "x": "Input",
        "offset": "offset",
        "axis1": "axis1",
        "axis2": "axis2",
...
class Tracer(core.Tracer):
    ...
    def eager_trace_op(self,
        ...
        core_ops_args_info = _C_ops.get_intermediate_state_core_ops_args_info()
        core_ops_args_type_info = _C_ops.get_intermediate_state_core_ops_args_type_info(
        )
        core_ops_returns_info = _C_ops.get_intermediate_state_core_ops_returns_info(
        )
    ...
    def eager__trace_op(self,
                        ...
                        stop_gradient=False,
                        inplace_map=None):
        assert type in _name_mapping.keys()

        op_type = _name_mapping[type]["final_op_name"]
        function_ptr = _C_ops.__dict__[op_type]

        core_ops_args_info = _C_ops.get__core_ops_args_info()
        core_ops_args_type_info = _C_ops.get__core_ops_args_type_info()
        core_ops_returns_info = _C_ops.get__core_ops_returns_info()
        ...
        for i in range(len(op_args)):
            ...
            assert eager_arg_name in _name_mapping[type].keys()
            arg_name = _name_mapping[type][eager_arg_name]
        ...
        if isinstance(returns, tuple):
            for i in range(len(op_returns)):
                ...
                assert eager_retname in _name_mapping[type].keys()
                retname = _name_mapping[type][eager_retname]
                if retname in outputs.keys():
                    # Replaced outputs by function returns
                    if isinstance(returns[i], list):
                        for j in range(len(returns[i])):
    ...
    def trace_op(self,
        ...
        if not framework._in_legacy_dygraph():
            # inputs : {"sum": [tensor], ...}
            # outputs : {"sum": [tensor], ...}
            if type in _name_mapping.keys():
                op_type = _name_mapping[type]["final_op_name"]

                assert op_type in _C_ops.__dict__
                self.eager__trace_op(type, inputs, outputs, attrs,
