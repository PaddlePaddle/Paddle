...
from paddle.fluid.dygraph.jit.static_analysis import AstNodeWrapper, NodeVarType, StaticAnalysisVisitor
from paddle.fluid.dygraph.jit.utils import ast_to_source_code, is_control_flow_to_transform
...
class ListTransformer(gast.NodeTransformer):
    ...
    def _transform_slice_to_tensor_write(self, node):
        ...
        if isinstance(slice_node, gast.Slice):
            ...
            i = "fluid.layers.cast(" \
                "x=fluid.dygraph.jit.converter.variable_trans_func.to_static_variable({})," \
    ...
    def _replace_list_pop(self, node):
        ...
        new_call_str = "fluid.dygraph.jit.transformer.list_transformer.convert_list_pop({}, {})".format(
            target_str, idx_str)
