...
_already_patch_eagertensor = False
...
def monkey_patch_math_eagertensor():
    """
    Similar to monkey_patch_eagertensor.
    The difference is, in dygraph mode, use auto-generated op functions for better performance.
    """
    ...
    def create_tensor(value, dtype, shape):
        out = _varbase_creator(dtype=dtype)
        out = core.eager.ops.fill_constant(out, 'dtype', dtype, 'shape', shape,
    ...
    def astype(self, dtype):
        ...
        if not isinstance(dtype, core.VarDesc.VarType):
            dtype = convert_np_dtype_to_dtype_(dtype)
        return core.eager.ops.cast(self, 'in_dtype', self.dtype, 'out_dtype',
                                   dtype)

    def _scalar_elementwise_op_(var, scale, bias):
        return core.eager.ops.scale(var, 'scale', scale, 'bias', bias)
    ...
    def _T_(var):
        ...
        for i in range(len(var.shape)):
            perm.insert(0, i)
        out, _ = core.eager.ops.transpose2(var, 'axis', perm)
        return out
    ...
    def _binary_creator_(method_name,
        ...
        def __impl__(self, other_var):
            ...
            # 2. create varbase for scalar
            lhs_dtype = self.dtype
            if not isinstance(other_var, core.eager.EagerTensor):
                if isinstance(other_var, complex):
                    import paddle
                    other_var = paddle.to_tensor(other_var, dtype='complex64')
                else:
            ...
            # 4. calculation
            axis = -1
            math_op = getattr(core.eager.ops, op_type)
            return math_op(self, other_var, 'axis', axis)
    ...
    if hasattr(core, "eager"):
        global _already_patch_eagertensor
        if not _already_patch_eagertensor:
            for method in varbase_methods:
                method_name = method[0]
                method_impl = method[1]
        ...
        _already_patch_eagertensor = True
