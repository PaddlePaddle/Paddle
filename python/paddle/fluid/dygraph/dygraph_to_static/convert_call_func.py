...
from paddle.fluid.dygraph.dygraph_to_static.program_translator import StaticWrapper
...
def convert_call(func):
    ...
    if inspect.isfunction(func):
        ...
        try:
            ...
            #          return x
            #
            # `foo` will be converted into a wrapper class, suppose as `StaticWrapper`.
            # And `foo.__globals__['foo']` will still return this `StaticWrapper` instead of
            # `foo` function. So `isinstance(fn, StaticWrapper)` is added here. 
            global_functions = set()
            for fn in func.__globals__.values():
                if inspect.isfunction(fn):
                    global_functions.add(fn)
                elif isinstance(fn, StaticWrapper):
