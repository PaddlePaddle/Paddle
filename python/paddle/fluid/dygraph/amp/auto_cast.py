...
def amp_guard(enable=True,
    ...
    # check device_type:
    # NOTE: Now, amp only support gpu for float16 and bfloat16, xpu for float16, mlu for float16.
    ...
    if enable and not (tracer._expected_place.is_gpu_place() or
                       ...
                       tracer._expected_place.is_mlu_place()):
        warnings.warn(
            'amp_guard can only be enabled on CUDAPlace, XPUPlace and MLUUPlace, current place is %s, so it makes no effect.'
