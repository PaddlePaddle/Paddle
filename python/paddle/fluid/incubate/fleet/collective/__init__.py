...
class CollectiveOptimizer(DistributedOptimizer):
    ...
    def _check_collective_mode(self, main_program, optimizer, strategy):
        ...
        if self._strategy.collective_mode=="local_sgd" \
            ...
            assert self._strategy.mode == "collective", \
                "local_sgd, grad_allreduce and pipeline_grad_allreduce can be used under collective mode"
    ...
    def minimize(self,
        ...
        if self._use_pipeline:
            self._optimizer = fluid.optimizer.PipelineOptimizer(
                self._optimizer,
                num_microbatches=self._num_microbatches,
                start_cpu_core_id=0)
            optimize_ops, param_grads, prog_list = self._optimizer.minimize(
                loss,
                startup_program=startup_program,
