...
class DownpourServer(Server):
    ...
    def add_sparse_table(self, table_id, strategy):
        ...
        if strategy.get('sparse_table_class') is not None:
            ...
            if table_class not in support_table_calss:
                raise ValueError("support sparse_table_class: [ 'DownpourSparseTable' ], \
        ...
        if table_class == 'DownpourSparseTable':
            table.compress_in_save = strategy.get('sparse_compress_in_save', True)
            table.shard_num = strategy.get('sparse_shard_num', 1000)
            
            support_accessor_class = ['DownpourFeatureValueAccessor']
            if strategy.get('sparse_accessor_class') is not None:
                ...
                if accessor_class not in support_accessor_class:
                    raise ValueError("support sparse_accessor_class: ['DownpourFeatureValueAccessor'], \
            ...
            if accessor_class == 'DownpourFeatureValueAccessor':
                table.accessor.sparse_sgd_param.learning_rate = strategy.get('sparse_learning_rate', 0.05) 
                table.accessor.sparse_sgd_param.initial_g2sum = strategy.get('sparse_initial_g2sum', 3) 
                table.accessor.sparse_sgd_param.initial_range = strategy.get('sparse_initial_range', 1e-4) 
                if strategy.get('sparse_weight_bounds') is None:
                    table.accessor.sparse_sgd_param.weight_bounds.extend([-10, 10])
                else:
                    table.accessor.sparse_sgd_param.weight_bounds.extend(strategy.get('sparse_weight_bounds'))
                table.accessor.embedx_dim = strategy.get('sparse_embedx_dim', 8)
                table.accessor.embedx_threshold = strategy.get('sparse_embedx_threshold', 5) 
                table.accessor.fea_dim = int(table.accessor.embedx_dim) + 3 
                table.accessor.downpour_accessor_param.nonclk_coeff = strategy.get('sparse_nonclk_coeff', 0.1) 
                table.accessor.downpour_accessor_param.click_coeff = strategy.get('sparse_click_coeff', 2) 
                table.accessor.downpour_accessor_param.base_threshold = strategy.get('sparse_base_threshold', 0.2) 
                table.accessor.downpour_accessor_param.delta_threshold = strategy.get('sparse_delta_threshold', 0.15) 
                table.accessor.downpour_accessor_param.delta_keep_days = strategy.get('sparse_delta_keep_days', 31)
                table.accessor.downpour_accessor_param.show_click_decay_rate = strategy.get('sparse_show_click_decay_rate', 0.999)
                table.accessor.downpour_accessor_param.delete_threshold = strategy.get('sparse_delete_threshold', 0.8)
    ...
    def add_dense_table(self, table_id, param_var, grad_var, strategy):
        ...
        for key in strategy:
            if key.startswith('dense_') and key not in support_dense_key_list:
                raise ValueError("strategy key '%s' not support" % (key))

        table.table_class = strategy.get('dense_table_class', "DownpourDenseTable") 
        table.type = pslib.PS_DENSE_TABLE
        table.compress_in_save = strategy.get('dense_compress_in_save', True) 
        table.accessor.accessor_class = strategy.get('dense_accessor_class', "DownpourDenseValueAccessor") 
        table.accessor.dense_sgd_param.name = strategy.get('dense_optimizer', "adam") 
        table.accessor.dense_sgd_param.adam.learning_rate = strategy.get('dense_learning_rate', 5e-06) 
        table.accessor.dense_sgd_param.adam.avg_decay_rate = strategy.get('dense_avg_decay', 0.999993) 
        table.accessor.dense_sgd_param.adam.ada_decay_rate = strategy.get('dense_ada_decay', 0.9999) 
        table.accessor.dense_sgd_param.adam.ada_epsilon = strategy.get('dense_ada_epsilon', 1e-8) 
        table.accessor.dense_sgd_param.adam.mom_decay_rate = strategy.get('dense_mom_decay', 0.99) 
        table.accessor.dense_sgd_param.naive.learning_rate = strategy.get('dense_naive_lr', 0.0002) 
        table.accessor.fea_dim = fea_dim

    def add_data_norm_table(self, table_id, learning_rate, param_var, grad_var, strategy):
        """
        Args:
            table_id(int): id of sparse params table
            learning_rate(float): the learning rate used to update parameters. \
        ...
        for key in strategy:
            if key.startswith('datanorm_') and key not in support_datanorm_key_list:
                raise ValueError("strategy key '%s' not support" % (key))
