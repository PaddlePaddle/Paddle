...
class DistributeTranspiler(object):
        ...
        for v in self.param_grad_ep_mapping[endpoint]["grads"]:
            ...
            single_trainer_var = \
                pserver_program.global_block().create_var(
                    ...
                    dtype=v.dtype,
                    shape=v.shape)
            if self.config.sync and self.trainers > 1:
    ...
    def _init_splited_vars(self):
        ...
        self.grad_var_mapping = self._create_vars_from_blocklist(
            self.origin_program,
            grad_blocks,
            add_trainer_suffix=self.trainers > 1)
    ...
    def _create_table_optimize_block(self, pserver_index, pserver_program,
        ...
        grad_var = pserver_program.global_block()._rename_var(origin_grad_name,
                                                              splited_grad_name)
    ...
    def _create_vars_from_blocklist(self,
        ...
        for varname, split in six.iteritems(block_map):
            ...
            if len(split) == 1:
                var_mapping[
                    varname] = [program.global_block().var(orig_var.name)]
                continue
            var_mapping[varname] = []
            ...
            if role_id == int(LR_SCHED_OP_ROLE_ATTR_VALUE) or \
                ...
                if op.type == 'increment':
                    ...
                    all_trainer_counter_inputs = [
                        ...
                        for id_ in range(self.trainers)
                    ]
