...
__all__ = [
    ...
    'RecomputeOptimizer', 'Adam', 'AdamScale'
]
...
class AdamScaleOptimizer(Optimizer):
    """
    The Adam scale optimizer uses an optimization described at the end
    of section 2 of `Adam paper <https://arxiv.org/abs/1412.6980>`_ ,
    it can dynamically adjusts the learning rate of each parameter using
    the 1st moment estimates and the 2nd moment estimates of the gradient.
    ...
    def __init__(self,
        ...
        assert beta2 is not None
        assert epsilon is not None
        super(AdamScaleOptimizer, self).__init__(
            learning_rate=learning_rate,
            parameter_list=parameter_list,
            regularization=regularization,
            grad_clip=grad_clip,
            name=name)
        self.type = "adam_scale"
        self._beta1 = beta1
        self._beta2 = beta2
        self._epsilon = epsilon
        self._lazy_mode = lazy_mode
        self._new_param_scale = new_param_scale
    ...
    def _append_optimize_op(self, block, param_and_grad):
        ...
        if framework.in_dygraph_mode():
            ...
            _beta2 = self._beta2 if not isinstance(
                self._beta2, Variable) else self._beta2.numpy().item(0)
            _new_param_scale = self._new_param_scale if not isinstance(
                self._new_param_scale, Variable) else self._new_param_scale.numpy().item(0)
            _, _, _, _, _ = core.ops.adam_scale(
                param_and_grad[0], param_and_grad[1], lr, moment1, moment2,
                beta1_pow_acc, beta2_pow_acc, param_and_grad[0], moment1,
                moment2, beta1_pow_acc, beta2_pow_acc, 'epsilon', self._epsilon,
                'lazy_mode', self._lazy_mode, 'min_row_size_to_use_multithread',
                1000, 'beta1', _beta1, 'beta2', _beta2, 'new_param_scale', _new_param_scale)
        ...
        if isinstance(self._beta2, Variable):
            ...
            attrs['beta2'] = self._beta2
        if isinstance(self._new_param_scale, Variable):
            inputs['NewParamScaleTensor'] = self._new_param_scale
        else:
            attrs['new_param_scale'] = self._new_param_scale

        adam_scale_op = block.append_op(
            type=self.type,
            inputs=inputs,
            outputs=outputs,
            attrs=attrs,
