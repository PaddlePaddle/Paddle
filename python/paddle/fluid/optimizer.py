...
class PipelineOptimizer(object):
    ...
    Train with pipeline mode. The program will be split by devices
    where ops run. 
    ...
    Examples:
        .. code-block:: python
            ...
            optimizer = fluid.optimizer.SGD(learning_rate=0.1)
            optimizer = fluid.optimizer.PipelineOptimizer(optimizer, num_microbatches=4)
            optimizer.minimize(loss)
            place = fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(fluid.default_startup_program())
    ...
    def _split_program(self, main_program):
        ...
        for op in block.ops:
            device_spec = op.attr(self._op_device_key)
    ...
    def _add_default_opdevice_attr(self, block):
        ...
        # set op_device attr for optimizer related ops
