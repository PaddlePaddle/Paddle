...
class Optimizer(object):
    ...
    def register_gradients_hook(self, hook):
        """
        Registers a gradients hook on the optimizer.
        The hook will be called before apply_optimize, after backward.
        The signature of the hook should be:
            hook(params_grads) -> new_params_grads
        The :attr:`params_grads` is list of (param, grad) variable pairs, param is ``Parameter``,
        grad is the gradient value corresponding to the parameter. The :attr:`new_params_grads`
        is a new list of (param, grad) variable pairs generated by hook, it will be used in
        place of :attr:`params_grads` in apply_optimize.

        Args:
            hook(callable): A hook for processing gradients
        """
        assert callable(hook), "hook should be callable with \
                hook(params_grads) -> new_prams_grads signature"

        self._gradients_hooks.append(hook)
    ...
    def apply_optimize(self, loss, startup_program, params_grads):
        ...
        if framework.in_dygraph_mode():
            ...
            program = loss.block.program
            with program_guard(program, startup_program):
                for hook in self._gradients_hooks:
                    new_params_grads = hook(params_grads)
