...
class CompiledProgram(object):
    """
    CompiledProgram converts and optimizes the input Program or Graph according
    ...
    def with_data_parallel(self,
                           ...
                           places=None):
        """
        This interface is used to convert the input Program or Graph to run the model
        in data parallel mode. Users can use the build_strategy and exec_strategy
        to set some optimizations that can be performed during the construction and
        computation of the Graph, such as reducing the number of AllReduce operations,
        specifying the size of the thread pool used in the computation Graph running
        the model, and so on. Note: If build_strategy is specified when building
        CompiledProgram and calling with_data_parallel, build_strategy in CompiledProgram
        will be overwritten, therefore, if it is data parallel training, it is recommended
        to set build_strategy when calling with_data_parallel interface.
