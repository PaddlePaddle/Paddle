...
template <typename data_t>
void relu_xpu_forward_kernel(const data_t* x_data,
                             data_t* out_data,
                             int64_t x_numel) {
  ...
  int r = xpu::relu<XPUType>(dev_ctx.x_context(),
...
std::vector<paddle::Tensor> relu_xpu_forward(const paddle::Tensor& x) {
  ...
  PD_DISPATCH_FLOATING_TYPES(
      x.type(), "relu_xpu_forward", ([&] {
        relu_xpu_forward_kernel<data_t>(
            x.data<data_t>(), out.data<data_t>(), x.numel());
      }));
...
std::vector<paddle::Tensor> ReluDoubleBackward(const paddle::Tensor& out,
  ...
  } else if (out.place() == phi::Place(phi::AllocationType::XPU)) {
