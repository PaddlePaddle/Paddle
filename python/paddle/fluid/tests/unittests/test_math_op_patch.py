...
class TestMathOpPatches(unittest.TestCase):
    ...
    def test_rdiv_scalar(self):
        ...
        place = fluid.CPUPlace()
        exe = fluid.Executor(place)
        a_np = numpy.random.random(size=self.a_shape).astype('float32') + 1e-2
    ...
    def test_div_two_tensor(self):
        ...
        exe = fluid.Executor(place)
        a_np = numpy.random.random(size=self.a_shape).astype('float32')
        b_np = numpy.random.random(size=self.b_shape).astype('float32') + 1e-2
    ...
    def test_mul_two_tensor(self):
        ...
        exe = fluid.Executor(place)
        a_np = numpy.random.random(size=self.a_shape).astype('float32')
        b_np = numpy.random.random(size=self.b_shape).astype('float32')
        c_np = exe.run(fluid.default_main_program(),
                       feed={
                           "a": a_np,
                           'b': b_np
    ...
    def test_neg(self):
        ...
        place = fluid.CPUPlace()
        exe = fluid.Executor(place)
        a_np = numpy.random.uniform(-1, 1, size=self.a_shape).astype('float32')
    ...
    def test_astype(self):
        ...
        place = fluid.CPUPlace()
        exe = fluid.Executor(place)
        a_np = numpy.random.uniform(-1, 1, size=self.a_shape).astype('float64')
    ...
    def test_bitwise_and(self):
        ...
        x = fluid.layers.data(name="x", shape=[2, 3, 5], dtype="int32")
        y = fluid.layers.data(name="y", shape=[2, 3, 5], dtype="int32")
        z = x & y
    ...
    def test_ndim(self):
        ...
        b = paddle.static.data(name="b", shape=[])
        self.assertEqual(b.dim(), 0)
        self.assertEqual(b.ndimension(), 0)
...
class TestMathOpPatches3(TestMathOpPatches):
    ...
    @prog_scope()
    def test_equal(self):
        a = paddle.static.data(name="a", shape=[], dtype='float32')
        b = paddle.static.data(name="b", shape=[], dtype='float32')
        c = (a == b)

        place = fluid.CPUPlace()
        exe = fluid.Executor(place)
        a_np = numpy.array(3).astype('float32')
        b_np = numpy.array(3).astype('float32')
    ...
    def test_equal_and_cond(self):
        ...
        one = fluid.layers.ones(shape=[], dtype='int32')
        zero = fluid.layers.zeros(shape=[], dtype='int32')
        cond = (one == zero)
        c = fluid.layers.cond(cond, lambda: a + b, lambda: a - b)
