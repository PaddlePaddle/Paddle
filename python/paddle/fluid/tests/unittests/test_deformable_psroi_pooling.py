...
#class TestDeformablePSROIPoolOp():
    def set_data(self):
        self.init_test_case()
        self.make_rois()
        self.calc_deformable_psroi_pooling()
        self.inputs = {'Input': self.input, "ROIs": (self.rois[:, 1:5], self.rois_lod), "Trans": self.trans}
        self.attrs = {
    ...
    def calc_deformable_psroi_pooling(self):
        ...
        self.trans = np.random.rand(self.rois_num, 2, self.part_size, self.part_size).astype('float32')
        ...
        for index in range(count):
            ...
            if self.no_trans:
                ...
                trans_x = self.trans[n][0][part_h][part_w] * self.trans_std# only deformable roi pooling
            ...
            for iw in range(self.sample_per_part):
                for ih in range(self.sample_per_part):
                    ...
                    if w < -0.5 or w > self.width - 0.5 or h < -0.5 or h > self.height - 0.5:
                        continue
    ...
    def test_check_grad(self):
        # self.check_grad(['trans'], 'out', max_relative_error=0.0008)
