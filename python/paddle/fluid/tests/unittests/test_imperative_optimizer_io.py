...
class TestOptimizerIO(unittest.TestCase):
            ...
            for batch_id, data in enumerate(self.train_reader()):
                ...
                for i, optimizer_loaded in enumerate(optimizers_loaded):
                    optimizer_loaded.load_from_path(
                        os.path.join("./", self.save_dir, self.save_filenames[i]))
    ...
    def test_optmizer_io_exception(self):
        with fluid.dygraph.guard():
            ...
            moment_loaded = fluid.optimizer.MomentumOptimizer(
                ...
                momentum=random.uniform(0.001, 1.0),
                use_nesterov=True)
            self.assertRaises(TypeError , moment_loaded.load_from_path, os.path.join(self.save_dir, "saved_sgd"))
    ...
    def test_io_in_static_exception(self):
        ...
        for i, optimizer in enumerate(optimizers):
            self.assertRaises(TypeError , optimizer.save, self.save_dir, self.save_filenames[i])
            self.assertRaises(TypeError , optimizer.load_from_path, os.path.join(self.save_dir, "random_file_name"))
