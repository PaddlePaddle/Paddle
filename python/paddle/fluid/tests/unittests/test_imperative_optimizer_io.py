...
class TestOptimizerIO(unittest.TestCase):
    ...
    def test_optmizer_io(self):
        with fluid.dygraph.guard():
            ...
            model_average.save(self.save_dir, "model_average")
            model_average_loaded = fluid.optimizer.ModelAverage(
                random.uniform(0.001, 1.0),
                min_average_window=random.randint(1000, 10000),
                max_average_window=random.randint(10000, 20000))
            model_average_loaded.load_from_path(
                os.path.join(self.save_dir, "model_average"))
            self.assertTrue(
                compare_approximate(model_average.state_dict()['properties'],
                                    model_average_loaded.state_dict()[
                                        'properties']))
            self.assertEqual(
                model_average.state_dict()['accumulators_state'],
                model_average_loaded.state_dict()['accumulators_state'])
    ...
    def test_optmizer_io_exception(self):
        with fluid.dygraph.guard():
            ...
            for batch_id, data in enumerate(self.train_reader()):
                ...
                sgd.save(self.save_dir)
                sgd.save("random_dir" + str(random.randint(0, 1000)))
                break
