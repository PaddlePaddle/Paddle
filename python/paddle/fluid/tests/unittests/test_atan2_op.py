        ...
        def run(place):
            with paddle.static.program_guard(paddle.static.Program()):
                X1 = paddle.paddle.static.data(
                    'X1', self.shape, dtype=self.dtype
                )
                X2 = paddle.paddle.static.data(
                    'X2', self.shape, dtype=self.dtype
                )
...
class TestAtan2Error(unittest.TestCase):
    def test_mismatch(self):
        ...
        def test_mismatch_numel():
            X = paddle.paddle.static.data('X', (1,), dtype=np.float64)
            Y = paddle.paddle.static.data('Y', (0,), dtype=np.float64)
