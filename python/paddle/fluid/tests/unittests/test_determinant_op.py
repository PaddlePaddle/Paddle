...
class TestDeterminantOpFP16Op(OpTest):
    def setUp(self):
        self.python_api = paddle.linalg.det
        self.init_data()
        self.op_type = "determinant"
    ...
    def init_data(self):
        np.random.seed(0)
        # not invertible matrix
        self.case = np.random.rand(3, 3, 3, 5, 5).astype(np.float16)
        self.inputs = {'Input': self.case}
        self.target = np.linalg.det(
            np.random.rand(3, 3, 3, 5, 5).astype(np.float32)
        )
