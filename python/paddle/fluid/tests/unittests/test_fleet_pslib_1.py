...
class TestPSlib(unittest.TestCase):
    ...
    def test_fleet1(self):
        ...
        if fleet.is_server():
            ...
            time.sleep(1)
            fleet.load_one_table(0, "./fleet_model_0")
            time.sleep(1)
            fleet._fleet_ptr.copy_table(0, 1)
            time.sleep(1)
            fleet.clear_model()
            fleet.init_server("./fleet_model_0")
            with fluid.scope_guard(scope):
                var_names = ["fc_0.b_0", "fc_0.w_0"]
                cur_vars = [train_program.global_block().var(i) for i in \
                    var_names]
                fluid.io.save_vars(exe, "./fleet_paddle_model_0", \
                    train_program, cur_vars)
                with open("./fleet_program_test_0.bin", "wb") as fout:
                    fout.write(train_program.desc.serialize_to_string())
                fleet.load_one_table(2, "./fleet_paddle_model_0", scope=scope,\
