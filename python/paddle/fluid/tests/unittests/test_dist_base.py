...
def runtime_main(test_class):
    ...
    if len(sys.argv) < 8:
        # must have >= 8 args
        print(
            "Usage: python dist_se_resnext.py [pserver/trainer] [endpoints] [trainer_id] [current_endpoint] [trainers] [is_dist] [sync_mode] [dc_asgd]"
        )
    role = sys.argv[1]
    ...
    dc_asgd = False
    if len(sys.argv) > 8:
        dc_asgd = True if sys.argv[8] == "TRUE" else False
...
class TestDistBase(unittest.TestCase):
    ...
    def start_pserver(self, model_file, check_error_log):
        ...
        dc_asgd_str = "TRUE" if self._dc_asgd else "FALSE"
        ps0_ep, ps1_ep = self._ps_endpoints.split(",")
        ps0_cmd = "%s %s pserver %s 0 %s %d TRUE %s %s" % \
            (self._python_interp, model_file, self._ps_endpoints, ps0_ep,
             self._trainers, sync_mode_str, dc_asgd_str)
        ps1_cmd = "%s %s pserver %s 0 %s %d TRUE %s %s" % \
    ...
    def check_with_place(self, model_file, delta=1e-3, check_error_log=False):
        ...
        ps0_ep, ps1_ep = self._ps_endpoints.split(",")
        tr0_cmd = "%s %s trainer %s 0 %s %d TRUE %s %s" % \
            (self._python_interp, model_file, self._ps_endpoints, ps0_ep,
             self._trainers, sync_mode_str, dc_asgd_str)
        tr1_cmd = "%s %s trainer %s 1 %s %d TRUE %s %s" % \
            (self._python_interp, model_file, self._ps_endpoints, ps1_ep,
             self._trainers, sync_mode_str, dc_asgd_str)
