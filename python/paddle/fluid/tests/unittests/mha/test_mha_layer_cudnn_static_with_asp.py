...
class TestCUDNNMHALayerWithASP(unittest.TestCase):
    def setUp(self):
        ...
        self.Q, self.K, self.V, self.W = _generate_data(self.batch_size, self.seq_len, self.vec_size)
    ...
    def test_cudnn_1D_mask(self):
        ...
        pre_pruned_mat = np.array(fluid.global_scope().find_var(param_name).get_tensor())
        ...
        post_pruned_mat = np.array(fluid.global_scope().find_var(param_name).get_tensor())
    ...
    def test_asp_workflow(self):
        ...
        param_name = "{}.w_0".format(self.cudnn_mha.full_name())
        cudnn_weight = np.array(fluid.global_scope().find_var(param_name).get_tensor())
