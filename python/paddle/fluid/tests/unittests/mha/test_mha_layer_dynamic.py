...
class TestFP32cuDNNMultiHeadAttentionDynamic(unittest.TestCase):
    def setUp(self):
        ...
        self.seq_data = seq_infer(self.attn_tensor)
    ...
    def test_fwd_output(self):
        ...
        with paddle.amp.auto_cast(enable=enable_amp, custom_white_list={'mha'}):
            ...
            cudnn_output = self.cudnn_mha(self.q_tensor_cudnn,
                                          self.k_tensor_cudnn,
                                          self.v_tensor_cudnn, self.seq_data)
        self.assertTrue(
            compare(ref_output.numpy(),
                    cudnn_output.numpy(), self.atol, self.rtol),
            "[Test{}cuDNNMultiHeadAttention] outputs are miss-matched.".format(
                get_dtype_str(self.dtype)))
