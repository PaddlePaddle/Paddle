...
class TestLogNormal(unittest.TestCase):
    ...
    def setUp(self):
        ...
        [self.mean, self.var, self.entropy, self.samples] = executor.run(
            main_program,
            feed=self.feeds,
    ...
    def test_entropy(self):
        ...
        self.assertEqual(str(self.entropy.dtype).split('.')
                         [-1], self.scale.dtype)
...
class LogNormalTestSample(unittest.TestCase):
    ...
    def setUp(self):
        ...
        executor.run(startup_program)
        [self.mean, self.samples] = executor.run(
            main_program,
            feed=self.feeds,
            fetch_list=fetch_list)
    ...
    def test_sample(self):
        for i in range(len(self.scale)):
            self.assertTrue(self._kstest(
                self.loc[i], self.scale[i], self.samples[:, i]))
...
class TestLognormalKL(unittest.TestCase):
    ...
    def setUp(self):
        ...
        executor = paddle.static.Executor(self.place)
        with paddle.static.program_guard(main_program, startup_program):
            loc1 = paddle.static.data('loc1', self.loc1.shape,
                                      self.loc1.dtype)
            scale1 = paddle.static.data('scale1', self.scale1.shape,
                                        self.scale1.dtype)
            loc2 = paddle.static.data('loc2', self.loc2.shape,
                                      self.loc2.dtype)
        ...
        self.feeds = {'loc1': self.loc1, 'scale1': self.scale1,
                      'loc2': self.loc2, 'scale2': self.scale2}

        executor.run(startup_program)
        [self.kl1, self.kl2] = executor.run(
            main_program,
            feed=self.feeds,
            fetch_list=fetch_list)
