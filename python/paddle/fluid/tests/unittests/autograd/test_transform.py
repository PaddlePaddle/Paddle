...
class TestAutoGradTransformForAdd(unittest.TestCase):
    ...
    def test_run(self):
        ...
        with paddle.static.program_guard(
            ...
            flatten_xs_dot = paddle.utils.layers_utils.flatten(xs_dot)
            for k, v in self.xs_shape_map.items():
                self.assertEqual(flatten_xs_dot[k].shape, v)
            flatten_ys_dot = paddle.utils.layers_utils.flatten(ys_dot)
            ...
            flatten_xs_bar = paddle.utils.layers_utils.flatten(xs_bar)
            for k, v in self.xs_shape_map.items():
            ...
            flatten_ys_bar = paddle.utils.layers_utils.flatten(ys_bar)
            for k, v in self.ys_shape_map.items():
