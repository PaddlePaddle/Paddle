    ...
    def test_prim_mode(self):
        def fun(x, y):
            ...
            with paddle.static.program_guard(main, startup):
                ...
                jac = paddle.incubate.autograd.Jacobian(paddle.matmul,
                                                        [static_x, static_y])[:]
        ...
        np.testing.assert_allclose(
            orig_jac, prim_jac, rtol=self.rtol, atol=self.atol)
...
class TestHessianFloat32(unittest.TestCase):
    ...
    def test_prim_mode(self):
        def fun(x, y):
            ...
            with paddle.static.program_guard(main, startup):
                ...
                hessian = paddle.incubate.autograd.Hessian(
                    lambda x, y: paddle.matmul(x, y), [static_x, static_y])[:]
                if paddle.incubate.autograd.prim_enabled():
                    paddle.incubate.autograd.prim2orig(main.block(0))
            feed = {'x': x, 'y': y}
            fetch_list = [hessian]
        ...
        np.testing.assert_allclose(
            orig_hessian, prim_hessian, rtol=self.rtol, atol=self.atol)
    ...
    def test_forward_gradients(self):
        ...
        def actual():
            ...
            with paddle.static.program_guard(mp, sp):
                ...
                ys = self.fun(*static_xs) if isinstance(
                    static_xs, typing.Sequence) else self.fun(static_xs)
                ys_grad = forward_gradients(ys, static_xs, static_v)
