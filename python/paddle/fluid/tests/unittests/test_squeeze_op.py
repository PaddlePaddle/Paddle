...
class TestSqueezeDygraphReuseAllocation(unittest.TestCase):
    def test_squeeze_reuse_allocation(self):
        paddle.disable_static()
        var = paddle.rand([1, 2, 3, 1])
        squeeze_var = paddle.squeeze(var)
        squeeze_var[0] = 2.
        self.assertNotEqual(var.shape, squeeze_var.shape)

        var_numpy = var.numpy()
        squeeze_var_numpy = squeeze_var.numpy()
        self.assertTrue(np.array_equal(var_numpy.squeeze(), squeeze_var_numpy))
        paddle.enable_static()
    ...
    def test_forward_version(self):
        ...
        squeeze_var = paddle.squeeze(var)
        self.assertEqual(squeeze_var.inplace_version, 0)

        var[0] = 2.
        self.assertEqual(var.inplace_version, 1)
        self.assertEqual(squeeze_var.inplace_version, 1)

        squeeze_var_2 = paddle.squeeze(var)
        self.assertEqual(squeeze_var_2.inplace_version, 1)

        var[0] = 3.
        self.assertEqual(squeeze_var.inplace_version, 2)
        self.assertEqual(squeeze_var_2.inplace_version, 2)
        paddle.enable_static()
    """
    ...
    def test_backward_error(self):
        ...
        paddle.disable_static()
        var_a = paddle.ones(shape=[1, 2, 3, 1], dtype="float32")
        ...
        var_c = var_b**2
        squeeze_var_b = var_b.squeeze()
        squeeze_var_b[0] = 2.  # var_b is modified inplace
