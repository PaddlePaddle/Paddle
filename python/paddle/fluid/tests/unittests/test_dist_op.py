...
class TestDistOpFP16(OpTest):
    def setUp(self):
        ...
        self.inputs = {
            ...
            "Y": np.random.random(self.y_shape).astype("float16"),
        }
...
class TestDistAPIFP16(unittest.TestCase):
    ...
    def test_api(self):
        ...
        with fluid.program_guard(main_program, startup_program):
            x = fluid.data(name='x', shape=[2, 3, 4, 5], dtype="float16")
            y = fluid.data(name='y', shape=[3, 1, 5], dtype="float16")
            p = 2
            x_i = np.random.random((2, 3, 4, 5)).astype("float16")
            y_i = np.random.random((3, 1, 5)).astype("float16")
            ...
            np.testing.assert_allclose(dist(x_i, y_i, p), out[0], rtol=1e-3)
