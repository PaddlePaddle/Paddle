...
class TestPSlib(unittest.TestCase):
    ...
    def test_fleet2(self):
        ...
        if fleet.is_server():
            ...
            dataset.set_filelist(["test_fleet2_a.txt", "test_fleet2_b.txt"])
            ...
            with fluid.scope_guard(scope):
                exe.train_from_dataset(train_program, dataset, scope, thread=3)
            ...
            fleet.save_persistables(exe, "./fleet_model_2")
            fleet.shrink_sparse_table()
            fleet.shrink_dense_table(0.98, scope=scope)
            time.sleep(1)
            fleet.load_one_table(0, "./fleet_model_2")
            time.sleep(1)
            fleet._fleet_ptr.copy_table(0, 1)
            time.sleep(1)
            fleet.clear_model()

            os.remove("./test_fleet2_a.txt")
            os.remove("./test_fleet2_b.txt")
