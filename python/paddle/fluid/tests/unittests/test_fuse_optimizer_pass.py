...
class TestSpareFuseAdamOps(TestParallelExecutorBase):
    def setUp(self):
        os.environ['CPU_NUM'] = str(4)
        self.word_dict_len = 5147
        self.BATCH_SIZE = 64
        reader = fake_imdb_reader(self.word_dict_len, self.BATCH_SIZE * 100)
        reader = paddle.batch(reader, batch_size=self.BATCH_SIZE)()
        self.train_data = next(reader)
    ...
    def test_simple_bow_net_with_fuse_op(self):
        ...
        self._compare_fused_optimizer_ops(model, True, optimizer=self.optimizer)
        self._compare_fused_optimizer_ops(
