...
class TestListenAndServOp(OpTest):
    ...
    def _raise_signal(self, parent_pid, raised_signal)
        time.sleep(self.sleep_time)
 		...
 		ps_command = subprocess.Popen("ps -o pid --ppid %d --noheaders" % parent_pid,
	...
	def _start_pserver(self, use_cuda, sync_mode):
        p = Process(target=run_pserver,
                args=(use_cuda, sync_mode, self.ip, self.port, self.trainer_count, self.trainer_id))
        p.start()
