    ...
    def test_errors(self):
        def test_dtype1():
            with fluid.program_guard(fluid.Program(), fluid.Program()):
                ...
                fluid.layers.sum(data, dtype="int32")
        ...
        def test_dtype2():
            with fluid.program_guard(fluid.Program(), fluid.Program()):
                data = fluid.data(name="data", shape=[10], dtype="float32")
                fluid.layers.sum(data, dtype="float32")
        ...
        def test_dtype3():
            with fluid.program_guard(fluid.Program(), fluid.Program()):
                ...
                fluid.layers.sum(data, dtype="bool")
...
class API_TestSumOp(unittest.TestCase):
    def test_1(self):
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            ...
            result_sum = fluid.layers.sum(input=data, dim=1, dtype="float64")
        ...
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            ...
            result_sum = fluid.layers.sum(input=data, dim=1, dtype="int64")
            place = fluid.CPUPlace()
            exe = fluid.Executor(place)
        ...
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            data = fluid.data("data", shape=[10, 10], dtype="int32")
            result_sum = fluid.layers.sum(input=data, dim=1)
            place = fluid.CPUPlace()
            exe = fluid.Executor(place)
            input_data = np.random.randint(10, size=(10, 10)).astype(np.int32)
            res, = exe.run(feed={"data": input_data}, fetch_list=[result_sum])
