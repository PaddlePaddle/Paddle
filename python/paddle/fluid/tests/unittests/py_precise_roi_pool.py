...
class PyPrRoIPool(object):
    ...
    def _PrRoIPoolingDistributeDiff(self, diff, top_diff, h, w, height, width,
                                    coeff):
        # print(top_diff, diff)
        overflow = (h < 0) or (w < 0) or (h >= height) or (w >= width)
        if not overflow:
            #     # atomicAdd(diff + h * width + w, top_diff * coeff);
            diff[h][w] += top_diff * coeff

    def _PrRoIPoolingMatDistributeDiff(self, diff, top_diff, s_h, s_w, e_h, e_w,
                                       y0, x0, y1, x1, h0, w0):
        ...
        tmp = (
            ...
            alpha) * (
                lim_beta - 0.5 * lim_beta * lim_beta - beta + 0.5 * beta * beta)
        self._PrRoIPoolingDistributeDiff(diff, top_diff, s_h, s_w, h0, w0, tmp)
        ...
        tmp = (
            ...
            alpha) * (
                lim_beta - 0.5 * lim_beta * lim_beta - beta + 0.5 * beta * beta)
        self._PrRoIPoolingDistributeDiff(diff, top_diff, s_h, e_w, h0, w0, tmp)
        ...
        tmp = (
            ...
            alpha) * (
                lim_beta - 0.5 * lim_beta * lim_beta - beta + 0.5 * beta * beta)
        self._PrRoIPoolingDistributeDiff(diff, top_diff, e_h, s_w, h0, w0, tmp)
        ...
        tmp = (
            ...
            alpha) * (
                lim_beta - 0.5 * lim_beta * lim_beta - beta + 0.5 * beta * beta)
        self._PrRoIPoolingDistributeDiff(diff, top_diff, e_h, e_w, h0, w0, tmp)
    ...
    def compute_grad(self,
                     ...
                     pooled_height=1,
                     pooled_width=1):
        (batch_size, channels, height, width) = np.array(x_grad).shape
        rois_num = len(rois)
        # output_shape = (rois_num, output_channels, pooled_height, pooled_width)
        # out_data = np.zeros(output_shape)
        for i in range(rois_num):
            ...
            roi = rois[i]
            roi_batch_id = int(roi[0])
            # print(rois_num, batch_size, roi_batch_id)
            ...
            # win_end_h = win_start_h + bin_size_h;
            #
            # win_size = max(float(0.0), bin_size_w * bin_size_h);
            #
            # # sum_out = win_size == float(0) ? float(0) : *this_out_grad / win_size;
            # if win_size == 0.0:
            #     sum_out = 0.0
            # else:
            #     c_in = (c * pooled_height + ph) * pooled_width + pw
            #     sum_out = x_grad[ci] / win_size;
            ...
            # for w_iter in range(int(s_w), int(e_w)):
            #     for h_iter in range(int(s_h), int(e_h)):
            #         self._PrRoIPoolingMatDistributeDiff(this_data_grad, sum_out, h_iter, w_iter, h_iter + 1, w_iter + 1,
                ...
                for ph in range(pooled_height):
                    for pw in range(pooled_width):
                        ...
                        if win_size == 0.0:
                            ...
                            # print(roi_batch_id, c, ph, pw, y_grad.shape)
                            sum_out = y_grad[i][c][ph][pw] / win_size
                        ...
                        for w_iter in range(int(s_w), int(e_w)):
                            for h_iter in range(int(s_h), int(e_h)):
                                self._PrRoIPoolingMatDistributeDiff(
                                    x_grad[roi_batch_id][c_in], sum_out, h_iter,
                                    w_iter, h_iter + 1, w_iter + 1,
                                    max(win_start_h, float(h_iter)),
                                    max(win_start_w, float(w_iter)),
                                    min(win_end_h, float(h_iter) + 1.0),
                                    min(win_end_w, float(w_iter + 1.0)), height,
                                    width)

                            # x_grad[i, c, ph, pw] = sum_out / win_size
                            # print(x_grad)
        return x_grad
