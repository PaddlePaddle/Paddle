...
def test_static_layer(place,
    ...
    with paddle.static.program_guard(prog, startup_prog):
        ...
        if weight_np is not None:
            ...
            bce_loss = paddle.nn.loss.AdaptiveLogSoftmaxWithLoss(
                weight=weight, reduction=reduction)
        else:
            bce_loss = paddle.nn.loss.AdaptiveLogSoftmaxWithLoss(reduction=reduction)
...
def test_dygraph_layer(place,
    ...
    if weight_np is not None:
        ...
        bce_loss = paddle.nn.loss.AdaptiveLogSoftmaxWithLoss(weight=weight, reduction=reduction)
    else:
        bce_loss = paddle.nn.loss.AdaptiveLogSoftmaxWithLoss(reduction=reduction)
...
def calc_adaptivelogsoftmaxwithloss(input_np, label_np, reduction='mean', weight_np=None):
...
class TestAdaptiveLogSoftmaxWithLoss(unittest.TestCase):
        ...
        for place in places:
            for reduction in reductions:
                ...
                dy_result = test_dygraph_layer(place, input_np, label_np,
                                               reduction)
                expected = calc_adaptivelogsoftmaxwithloss(input_np, label_np, reduction)
    ...
    def test_AdaptiveLogSoftmaxWithLoss_weight(self):
        ...
        for reduction in ['sum', 'mean', 'none']:
            ...
            dy_result = test_dygraph_layer(
                place, input_np, label_np, reduction, weight_np=weight_np)
            expected = calc_adaptivelogsoftmaxwithloss(
    ...
    def test_AdaptiveLogSoftmaxWithLoss_error(self):
        paddle.disable_static()
        self.assertRaises(
            ValueError, paddle.nn.loss.AdaptiveLogSoftmaxWithLoss, reduction="unsupport reduction")
...
class TestAdaptiveLogSoftmaxWithLossOpCase1(OpTest):
    def init_test_cast(self):
        self.shape = [2, 3, 4, 5]


class TestAdaptiveLogSoftmaxWithLossOpCase2(OpTest):
    def init_test_cast(self):
        self.shape = [2, 3, 20]
