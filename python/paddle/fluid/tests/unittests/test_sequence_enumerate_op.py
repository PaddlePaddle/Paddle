...
def sequence_enumerate(input_seq, in_lod, win_size, pad_value, need_pad=True):
    lod0 = [0]
    ...
    if need_pad:
        ...
        return out_seq, in_lod
    else:
        offset = 0
        out_lod = []
        for i in range(0, len(in_lod[0])):
            if in_lod[0][i] - win_size + 1 > 0:
                out_lod.append(in_lod[0][i] - win_size + 1)
            else:
                out_lod.append(0)
        for i in range(0, len(lod0) - 1):
            for idx in range(lod0[i], lod0[i + 1] - win_size + 1):
                single_seq = []
                for word_idx in range(win_size):
                    word_pos = idx + word_idx
                    dat = input_seq[word_pos][0]
                    single_seq.append(dat)
                out_seq.append(single_seq)
        return out_seq, [out_lod]
...
class TestSequenceEnumerateOp(OpTest):
    ...
    def init_test_case(self):
        ...
        self.win_size = 2
        self.pad_value = 0
        out_seq, lod = sequence_enumerate(self.in_seq, self.lod, self.win_size,
                                     self.pad_value)
...
class TesSequenceEnumerateOpNoPad(TestSequenceEnumerateOp):
    def setUp(self):
        self.op_type = "sequence_enumerate"
        self.init_test_case()
        self.inputs = {'X': (self.in_seq, self.lod)}
        self.attrs = {'win_size': self.win_size, 'pad_value': self.pad_value, 'need_pad':False}
        self.outputs = {'Out': (self.out_seq, self.out_lod)}

    def init_test_case(self):
        self.in_seq = np.random.randint(0, 10, (7, 1)).astype("int32")
        self.lod = [[4, 3]]
        self.win_size = 2
        self.pad_value = 0
        out_seq, lod = sequence_enumerate(self.in_seq, self.lod, self.win_size,
                                     self.pad_value, need_pad=False)
        self.out_seq = np.array(out_seq).astype("int32")
        self.out_lod = lod

class TestSequenceEnumerateOpNoPadLargeWinSize(TesSequenceEnumerateOpNoPad):
    def init_test_case(self):
        self.in_seq = np.random.randint(0, 10, (7, 1)).astype("int32")
        self.lod = [[4, 3]]
        self.win_size = 5
