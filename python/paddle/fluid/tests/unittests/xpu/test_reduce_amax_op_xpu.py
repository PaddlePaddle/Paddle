...
class XPUTestReduceAmaxOp(XPUOpTestWrapper):
    ...
    class XPUTestReduceAmaxBase(XPUOpTest):
        ...
        def set_case(self):
            ...
            self.attrs = {'use_xpu': True, 'keep_dim': self.keep_dim}
            in_arr = np.array([[3., 4., 6.], [2., 5., 7.], [5, .3, .6]])
            # self.inputs = {'X': np.random.random(self.shape).astype("float32")}
            self.inputs = {'X': in_arr.astype("float32")}
            print("intpu: >>>\n", self.inputs['X'])
    ...
    # class XPUTestReduceAmaxCase1(XPUTestReduceAmaxBase):
    #     def init_case(self):
    #         self.shape = (5, 6)
    #         self.axis = 1
    #         self.keep_dim = True
