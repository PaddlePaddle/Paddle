...
class XPUTestUnpoolOp(XPUOpTestWrapper):
    def __init__(self):
        self.op_name = 'unpool'
        self.use_dynamic_create_class = False

    class TestUnpoolOp(XPUOpTest):
        def setUp(self):
            self.op_type = "unpool"
            ...
            self.outputs = {'Out': output.astype(self.dtype)}
        ...
        def test_case(self):
            ...
            with fluid.dygraph.guard(place):
                ...
                np.testing.assert_allclose(
                    out_pp.numpy(), expect_res, rtol=1e-05
                )
    ...
    class TestUnpoolOpAPI_st(unittest.TestCase):
        def test_case(self):
            ...
            x = fluid.data(name="x", shape=[1, 1, 4, 4], dtype=self.dtype)
            output, indices = F.max_pool2d(
...
support_types = get_xpu_op_support_types('unpool')
for stype in support_types:
    create_test_class(globals(), XPUTestUnpoolOp, stype)
