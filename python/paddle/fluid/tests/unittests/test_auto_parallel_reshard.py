...
from paddle.distributed.auto_parallel.context import DistributedContext, get_default_distributed_context
...
PP_MESH_0 = auto.ProcessMesh([0], parent=ROOT_MESH)
PP_MESH_1 = auto.ProcessMesh([1], parent=ROOT_MESH)
...
class MLPLayer(nn.Layer):
    ...
    def forward(self, input):
        if _global_parallel_strategy == "pp":
            ...
            _global_process_mesh = auto.ProcessMesh([0, 1], parent=ROOT_MESH)
            auto.shard_tensor(
...
def get_dist_prog(train_program, startup_program, dist_context, rank_id):
    ...
    # auto completion
    complete_train_program = auto.complete_annotation(train_program, dist_context)

    dist_strategy = fleet.DistributedStrategy()
    partitioner = Partitioner(dist_strategy, dist_context, rank_id)
    # logical partition
    auto_parallel_main_prog, auto_parallel_startup_prog = partitioner.transpile_forward(complete_train_program, startup_program)
    dist_params_grads = partitioner.apply_backward(loss, complete_train_program, startup_program, auto_parallel_main_prog, auto_parallel_startup_prog)
    optimizer = paddle.fluid.optimizer.AdamOptimizer()
    opt_ops = partitioner.apply_optimize(optimizer, dist_params_grads, auto_parallel_main_prog, auto_parallel_startup_prog)
...
def check_backward_dist_attr(dist_context, dist_main_prog, op_need_check):
    ...
    op_dist_attr = dist_context.get_op_distributed_attr_for_program(op_need_check)
...
class TestMLPReshard(unittest.TestCase):
    def test_complete_backward_annotation(self):
        ...
        rank_id = 0
        dist_main_prog, dist_startup_prog = get_dist_prog(train_program, startup_program, dist_context, 0)
    ...
    def test_mlp_pp(self):
        ...
        dist_context = get_default_distributed_context()
        rank_id = 1
        dist_main_prog, dist_startup_prog = get_dist_prog(train_program, startup_program, dist_context, rank_id)
        complete_backward_annotation(dist_main_prog)
        reshard(dist_main_prog, dist_startup_prog, rank_id)
    ...
    def test_allgather(self):
        train_program = paddle.static.Program()
        startup_program = paddle.static.Program()
        global _global_process_mesh, ROOT_MESH
        _global_process_mesh = auto.ProcessMesh(mesh=[0, 1], parent=ROOT_MESH)
        with static.program_guard(train_program, startup_program):
            x = paddle.static.data(name="x", shape=[4, 4], dtype='float32')
            x = auto.shard_tensor(x, _global_process_mesh, dim_mapping=[0, -1])

            w = paddle.static.data(name="w", shape=[4, 4], dtype='float32')
            w = auto.shard_tensor(w, _global_process_mesh, dim_mapping=[-1, -1])

            y = paddle.distributed.shard_op(paddle.matmul, _global_process_mesh, {x.name: [-1, -1], w.name: [-1, -1]}, **{"x": x, "y": w})[0]
        ...
        complete_train_program = auto.complete_annotation(train_program, dist_context)
        auto_parallel_main_prog, auto_parallel_startup_prog = partitioner.transpile_forward(complete_train_program, startup_program)
        reshard(auto_parallel_main_prog, startup_program, rank_id)
