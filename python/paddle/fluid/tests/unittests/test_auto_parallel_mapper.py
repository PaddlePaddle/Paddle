    ...
    def _prepare_qkv(self, query, key, value, use_cache=False, cache=None):
        ...
        if _global_parallel_strategy == "mp":
            ...
            auto.shard_tensor(
                self.q_proj.weight,
                dist_attr={
                    "process_mesh": _global_process_mesh[self.stage],
                    "dims_mapping": [-1, 1]
                })
...
class TransformerDecoder(nn.Layer):
    ...
    def forward(self,
        ...
        new_caches = []
        self.checkpoints = []
        assert cache is None and use_cache == False
                ...
                if use_cache:
                    ...
                    if _global_parallel_strategy == "dp_mp_pp":
                        output = auto.shard_op(
                            mod,
                            dist_attr={
                                "process_mesh": _global_process_mesh[mod.stage]
...
class GPTModel(nn.Layer):
    ...
    def __init__(self,
        ...
        layer_per_stage = num_hidden_layers // _global_num_stages
        decoder_layers = nn.LayerList()
        for i in range(num_hidden_layers):
            stage = i // layer_per_stage
...
def get_dist_prog(train_program, startup_program, dist_context, rank_id):
    train_program, startup_program, loss = gpt_pretrain_forward(train_program,
                                                                startup_program)
...
class TestAutoParallelMapper(unittest.TestCase):
    def test_mapper_dp_mp(self):
        ...
        process_graph = build_process_graph(dist_programs)
