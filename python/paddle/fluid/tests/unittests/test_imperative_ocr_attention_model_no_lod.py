...
class ConvBNPool(fluid.dygraph.Layer):
    def __init__(self,
        ...
        self.conv_0_layer = Conv2D(
                        self.full_name(), channels[0], out_ch[0],
                        3, padding=1, param_attr=conv_param_0,
                        bias_attr=bias_attr_0, act=None,
                        use_cudnn=use_cudnn)
        self.bn_0_layer = BatchNorm(self.full_name(), out_ch[0],
        ...
        self.conv_1_layer = Conv2D(
                        ...
                        bias_attr=bias_attr_1, act=None,
                        use_cudnn=use_cudnn)
        self.bn_1_layer  = BatchNorm(self.full_name(), out_ch[1],
                        act=act, is_test=is_test)
...
class OCRConv(fluid.dygraph.Layer):
    def __init__(self,
        ...
        self.conv_bn_pool_4 = ConvBNPool(
            self.full_name(), 2, [128, 128], [64, 128], is_test=is_test, pool=False,
            use_cudnn=use_cudnn)
...
class DynamicGRU(fluid.dygraph.Layer):
    def __init__(self,
        ...
        self.gru_unit = GRUUnit(self.full_name(),
                               size * 3,
        ...
        #self.fc_test = FC(self.full_name(), size, act=None, bias_attr=False)
        self.h_0 = h_0
    ...
    def forward(self, inputs):
        ...
        for i in range(inputs.shape[1]):
            ...
            input_ = fluid.layers.slice(inputs, axes=[1], starts=[i], ends=[i+1])
            input_ = fluid.layers.reshape(input_, [-1, input_.shape[2]])
            hidden, reset, gate  = self.gru_unit(input_, hidden)
            #hidden = self.fc_test(input_)
...
class EncoderNet(fluid.dygraph.Layer):
    def __init__(self,
        ...
        if fluid.framework._in_dygraph_mode():
            h_0 = np.zeros((batch_size, rnn_hidden_size), dtype="float32")
            h_0 = to_variable(h_0)
        else:
            h_0 = fluid.layers.fill_constant(shape=[batch_size, rnn_hidden_size],
                    dtype='float32', value=0)
        self.ocr_convs = OCRConv(self.full_name(),
                                 is_test=is_test,
                                 use_cudnn=use_cudnn)
        # self.conv_test = Conv2D(self.full_name(), 128, 128, 3, padding=1)
        ...
        self.fc_2_layer  = FC(self.full_name(),
                              ...
                              bias_attr=False,
                              num_flatten_dims=2)
        self.gru_forward_layer = DynamicGRU(self.full_name(),
                                            size=rnn_hidden_size,
                                            h_0=h_0,
                                            param_attr=para_attr,
                                            bias_attr=bias_attr,
                                            candidate_activation='relu')
        self.gru_backward_layer = DynamicGRU(self.full_name(),
                                            size=rnn_hidden_size,
                                            h_0=h_0,
                                            param_attr=para_attr,
                                            bias_attr=bias_attr,
    ...
    def forward(self, inputs):
        ...
        # conv_features = self.conv_test(conv_features)
        sliced_feature = fluid.layers.im2sequence(
            input=conv_features,
            stride=[1, 1],
            filter_size=[conv_features.shape[2], 1])

        sliced_feature = fluid.layers.reshape(conv_features,
                [-1, 48, sliced_feature.shape[1]], inplace=False)
        #sliced_feature = fluid.layers.slice(sliced_feature, axes=[1], starts=[0],
        #  ends=[1])
...
class SimpleAttention(fluid.dygraph.Layer):
    def __init__(self,
        ...
        self.fc_1 = FC(self.full_name(), decoder_size,
                act=None, param_attr=para_attr,  bias_attr=bias_attr)
        self.fc_2 = FC(self.full_name() , 1,
                act=None, param_attr=para_attr,  bias_attr=bias_attr)
    ...
    def forward(self, encoder_vec, encoder_proj, decoder_state):
        ...
        '''
        if fluid.framework._in_dygraph_mode():
            decoder_state = to_variable(decoder_state)
        else:
            decoder_state = fluid.layers.assign(decoder_state)
        '''

        decoder_state_fc = self.fc_1(decoder_state)
        decoder_state_proj_reshape = fluid.layers.reshape(decoder_state_fc,
                [-1, 1, decoder_state_fc.shape[1]], inplace=False)
        decoder_state_expand = fluid.layers.expand(decoder_state_proj_reshape,
        ...
        weights_reshape = fluid.layers.reshape(x=attention_weight, shape=[-1],
                inplace=False)
        scaled = fluid.layers.elementwise_mul(
                x=encoder_vec, y=weights_reshape, axis=0)
        scaled = fluid.layers.transpose(scaled, [0, 2, 1])
        scaled = fluid.layers.reshape(scaled,
                [-1, scaled.shape[1], scaled.shape[2], 1])
        context = fluid.layers.pool2d(input=scaled,
                pool_size=[scaled.shape[2], scaled.shape[3]], pool_type='avg')
        context = fluid.layers.reshape(context, [-1, context.shape[1]], inplace=False)
...
class GRUDecoderWithAttention(fluid.dygraph.Layer):
    def __init__(self,
        ...
        self.fc_1_layer = FC(self.full_name(), size=decoder_size*3,
                param_attr=para_attr, bias_attr=bias_attr)
        self.fc_2_layer = FC(self.full_name(), size=decoder_size*3,
                param_attr=para_attr, bias_attr=bias_attr)
        self.gru_unit = GRUUnit(self.full_name(), size=decoder_size*3,
                param_attr=para_attr,
                bias_attr=bias_attr )
    ...
    def _build_once(self, target_embedding, encoder_vec, encoder_proj, decoder_boot):
        pass
    def forward(self, target_embedding, encoder_vec, encoder_proj, decoder_boot ):
        ...
        for i in range(target_embedding.shape[1]):
            current_word  = fluid.layers.slice(target_embedding, axes=[1],
                    starts=[i], ends=[i+1])
            current_word = fluid.layers.reshape(current_word,
                    [-1, current_word.shape[2]], inplace=False)
            '''
            if fluid.framework._in_dygraph_mode():
                encoder_vec  = to_variable(encoder_vec)
                encoder_proj = to_variable(encoder_proj)
            else:
                encoder_vec = fluid.layers.assign(encoder_vec)
                encoder_proj  = fluid.layers.assign(encoder_proj)
            '''

            context, test1  = self.simple_attention(encoder_vec, encoder_proj, hidden_mem)
            ...
            # h = self.fc_test(decoder_inputs)
            hidden_mem = h
            '''
            if fluid.framework._in_dygraph_mode():
                hidden_mem = to_variable(hidden_mem)
            else:
                hidden_mem = fluid.layers.assign(hidden_mem)
            '''
...
class OCRAttention(fluid.dygraph.Layer):
    def __init__(self,
        ...
        self.fc = FC(self.full_name(),
                    ...
                    bias_attr=False,
                    act='relu')
        self.embedding = Embedding(self.full_name(), [num_classes+2, word_vector_dim],
                                   dtype='float32')
        self.gru_decoder_with_attention = GRUDecoderWithAttention(self.full_name(),
    ...
    def forward(self, inputs, label_in):
        ...
        # return gru_backward
        backward_first = fluid.layers.slice(gru_backward, axes=[1], starts=[0], ends=[1])
        backward_first = fluid.layers.reshape(backward_first,
                [-1, backward_first.shape[2]], inplace=False)
        decoder_boot = self.fc(backward_first)
        #label_in = fluid.layers.cast(x=label_in, dtype='int64')
        #label_in._stop_gradient = True
        #label_in.trainable = False
        label_in = fluid.layers.reshape(label_in, [-1, 1])
        trg_embedding = self.embedding(label_in)
        trg_embedding = fluid.layers.reshape(trg_embedding,
                [-1, max_length, trg_embedding.shape[1]], inplace=False)
        prediction, test  = self.gru_decoder_with_attention(trg_embedding, encoded_vector,
...
class TestDygraphOCRAttention(unittest.TestCase):
    def test_while_op(self):
        ...
        with fluid.dygraph.guard(fluid.CPUPlace()):
            ...
            train_images_dir = os.path.join(DATA_DIR_NAME, TRAIN_DATA_DIR_NAME)
            train_list_file = os.path.join(DATA_DIR_NAME, TRAIN_LIST_FILE_NAME)
            train_reader = data_generator.train_reader(train_images_dir,
                train_list_file, batch_size, 0)
            dy_param_init_value = {}
            ...
            for epoch in range(epoch_num):
                for batch_id, data in enumerate(train_reader()):
                    if(batch_id == batch_num):
                        break
                    img, label_in_tensor, label_out_tensor = data_generator.get_attention_data(data, place)
                    label_in_array = np.array(label_in_tensor).astype('int64')
                    label_in = to_variable(value=label_in_array)

                    label_out_array = np.array(label_out_tensor).astype('int64')
                    label_out = to_variable(label_out_array)

                    # label_in._stop_gradient = True
                    # label_in.trainable = False
                    label_out._stop_gradient = True
                    label_out.trainable = False
                    dy_img = np.array(img).astype('float32')
                    img = to_variable(dy_img)
                    dy_prediction, dy_grad  = ocr_attention(img,  label_in)
                    #fluid.clip.set_gradient_clip(fluid.clip.GradientClipByValue(
                    #    gradient_clip)
                    #    )
                    ...
                    if epoch == 0 and batch_id == 0:
                        for param in ocr_attention.parameters():
                            if param.name not in dy_param_init_value:
                                dy_param_init_value[param.name] = param._numpy()
                    avg_loss._backward()
                    dy_grad_test = dy_grad._gradient()
        ...
        with new_program_scope():
            ...
            '''
            static_label_in._stop_gradient = True
            static_label_in.trainable = False
            images._stop_gradient = True
            images.trainable = False
            static_label_out._stop_gradient = True
            static_label_out.trainable = False
            '''
            #static_label_in = fluid.layers.cast(x=static_label_in, dtype='int64')
            static_prediction, static_grad  = ocr_attention(images, static_label_in)
            # fluid.clip.set_gradient_clip(fluid.clip.GradientClipByValue(gradient_clip))
            static_label_out = fluid.layers.cast(x=static_label_out, dtype='int64')
            ...
            for epoch in range(epoch_num):
                for batch_id, data in enumerate(train_reader()):
                    if (batch_id == batch_num):
                        break
                    static_imgs, static_label_in, static_label_out  = data_generator.get_attention_data(data, place)
                    #fetch_list = [static_trg_embedding.name + '@GRAD']
                    fetch_list = [static_prediction.name, static_grad.name+'@GRAD']
                    fetch_list.extend(static_param_name_list)
                    fetch_list.extend(static_grad_name_list)
                    static_label_in = np.array(static_label_in).astype('int64')
                    static_label_out = np.array(static_label_out).astype('int64')
                    static_label_out = static_label_out.reshape((-1,1))
                    out = exe.run(
                        fluid.default_main_program(),
                        feed={"pixel": static_imgs,
            ...
            for op in fluid.default_main_program().block(0).ops:
                ...
                for key, value in six.iteritems(op.all_attrs()):
                    if key not in ['op_callstack', 'op_role_var']:
                        print(" [ attrs: {}\n   {} ]".format(key, value))
            '''

        self.assertTrue(np.allclose(np.array(dy_img).all(),np.array(static_imgs).all()))
        ...
        if not np.array_equal(static_grad_test, dy_grad_test):
            print(static_grad_test[0])
            print(dy_grad_test[0])
            print("not ok")
        else:
            print("ok")
        ...
        for key, value in six.iteritems(static_grad_value):
            if not np.array_equal(value, dy_grad_value[key]):
                ...
                if len(value.shape) > 1:
                    print(value[0][:5])
                    print(dy_grad_value[key][0][:5])
                else:
                    print(value[0])
                    print(dy_grad_value[key][0])
                '''
