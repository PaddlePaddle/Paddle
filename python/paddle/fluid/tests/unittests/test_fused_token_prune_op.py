...
class TestFusedTokenPruneOp(unittest.TestCase):
    def setUp(self):
        ...
        # attn = np.expand_dims(attn, axis=0) # [1,1,2,2] bsz = 1, nd_head=1, max_seq_len=2
        # mask = [[1, 1], [-1, -1]]
        # mask = np.array(mask, dtype='float32')
        # mask = np.expand_dims(mask, axis=0)
        # mask = np.expand_dims(mask, axis=0) # same as attn
        # x = [[1, 2, 3], [4, 5, 6]]
        # x  = np.array(x, dtype='float32')
        # x = np.expand_dims(x, axis=0) # [1, 2, 3] bsz = 1, max_seq_len=2, c=3
        self.factor = 0.5
    ...
    def test_fused_token_prune_op(self):
        out_py = fused_token_prune(self.attn, self.mask, self.x, self.factor)
        attn = paddle.to_tensor(self.attn)
        mask = paddle.to_tensor(self.mask)
        x = paddle.to_tensor(self.x)
        out = _C_ops.fused_token_prune(attn, x, mask, 'factor', self.factor)
        self.assertTrue(
