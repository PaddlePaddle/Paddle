...
def index_add_add_value_grad_np(data: np.array, axis: int, index: List[int]):
    ...
    outer_loop = int(np.prod(data.shape[:axis]))
    new_shape = [outer_loop] + list(data.shape[axis:])
    x_np_reshape = data.reshape(tuple(new_shape))
    for i in range(outer_loop):
        for j in index:
            add_value_grad += np.sum(x_np_reshape[i, j])
    return add_value_grad
...
#         add_value_grad = index_add_add_value_grad_np(np.full_like(self.x_np, fill_value=1.0), self.axis,
#                                                         self.index_np)
#         self.assertTrue(
#             np.allclose(
