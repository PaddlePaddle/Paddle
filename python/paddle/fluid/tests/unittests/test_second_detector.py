...
def test():
    with _test_eager_guard():
        ...
        dense_model = MiddleExtractor(num_input_features=in_channels)
        sparse_model = SparseMiddleExtractor(num_input_features=in_channels)
        spconv_model = SpconvMiddleExtractor(num_input_features=in_channels).to(device)
        ...
        np.allclose(paddle.transpose(out2, [0, 4, 1, 2, 3]).numpy(), out3.detach().cpu().numpy(), atol=1e-3, rtol=1e-3)
