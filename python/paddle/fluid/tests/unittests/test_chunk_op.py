...
class TestChunkOpError(unittest.TestCase):
    def test_errors(self):
        with program_guard(Program(), Program()):
            ...
            def test_axis_type():
                x1 = paddle.paddle.static.data(
                    shape=[4], dtype='float16', name='x3'
                )
            ...
            def test_axis_variable_type():
                x2 = paddle.paddle.static.data(
                    shape=[4], dtype='float16', name='x9'
                )
                x3 = paddle.paddle.static.data(
                    shape=[1], dtype='float16', name='x10'
                )
            ...
            def test_chunks_type():
                x4 = paddle.paddle.static.data(
                    shape=[4], dtype='float16', name='x4'
                )
            ...
            def test_axis_type_tensor():
                x5 = paddle.paddle.static.data(
                    shape=[4], dtype='float16', name='x6'
                )
...
class API_TestChunk(unittest.TestCase):
    def test_out(self):
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            data1 = paddle.paddle.static.data(
                'data1', shape=[4, 6, 6], dtype='float64'
            )
            data2 = paddle.paddle.static.data('data2', shape=[1], dtype='int32')
