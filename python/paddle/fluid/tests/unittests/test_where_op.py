...
class TestWhereAPI(unittest.TestCase):
        ...
        for x_stop_gradient in [False, True]:
            for y_stop_gradient in [False, True]:
                with fluid.program_guard(Program(), Program()):
                    cond = fluid.layers.data(name='cond', shape=self.shape, dtype='bool')
                    x = fluid.layers.data(name='x', shape=self.shape, dtype='float32')
                    y = fluid.layers.data(name='y', shape=self.shape, dtype='float32')
                    ...
                    for use_cuda in [False, True]:
                        ...
                        out = exe.run(fluid.default_main_program(), feed={'cond': self.cond, 'x': self.x, 'y': self.y}, fetch_list=fetch_list)
    ...
    def __test_where_with_broadcast_static(self, cond_shape, x_shape, y_shape):
        ...
        main_program = Program()
        with fluid.program_guard(main_program):
            cond = fluid.layers.data(name='cond', shape=cond_shape, dtype='bool')
            ...
            for use_cuda in [False, True]:
                ...
                place = (fluid.CUDAPlace(0) if use_cuda else fluid.CPUPlace())
                exe = fluid.Executor(place)
                out = exe.run(fluid.default_main_program(), feed={'cond': cond_data, 'x': x_data, 'y': y_data}, fetch_list=[result])
