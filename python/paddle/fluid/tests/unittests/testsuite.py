...
def append_input_output(
    block, op_proto, np_list, is_input, dtype, is_cal_ref=False
):
    '''Insert VarDesc and generate Python variable instance'''
    proto_list = op_proto.inputs if is_input else op_proto.outputs

    def create_var(block, name, np_list, var_proto, is_cal_ref=False):
        dtype = None
        shape = None
        lod_level = None
    ...
    for var_proto in proto_list:
        ...
        if var_proto.duplicable:
            ...
            for (name, np_value) in np_list[var_name]:
                var_list.append(
                    create_var(
                        block, name, {name: np_value}, var_proto, is_cal_ref
                    )
                )
            var_dict[var_name] = var_list
        else:
            var_dict[var_name] = create_var(
                block, var_name, np_list, var_proto, is_cal_ref
            )
