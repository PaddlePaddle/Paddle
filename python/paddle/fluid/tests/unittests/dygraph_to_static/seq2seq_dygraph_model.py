...
class AttentionModel(fluid.dygraph.Layer):
    ...
    def beam_search(self, inputs):
        print(" -" * 10, 1)
        temp = fluid.layers.zeros(shape=[2], dtype="int64")
        src, tar, label, src_sequence_length, tar_sequence_length = inputs
        if src.shape[0] < self.batch_size:
            self.batch_size = src.shape[0]
        ...
        print(" -" * 10, 6)
        print(dec_hidden, " \n")
        for i, state in enumerate(dec_hidden):
            fluid.layers.array_write(
                self._expand_to_beam_size(state), i, dec_hidden)

        # dec_hidden = [self._expand_to_beam_size(state) for state in dec_hidden]
        print(" -" * 10, 7)

        for i, state in enumerate(dec_cell):
            fluid.layers.array_write(
                self._expand_to_beam_size(state), i, dec_cell)
        ...
        for step_idx in range(self.beam_max_step_num):
            ...
            new_dec_hidden = fluid.layers.create_array("float32")
            new_dec_cell = fluid.layers.create_array("float32")
            # dec_hidden = [self._merge_batch_beams(state) for state in dec_hidden]
            # dec_cell = [self._merge_batch_beams(state) for state in dec_cell]
            print(" dec_hidden : ", dec_hidden)
            print(" dec_cell : ", dec_cell)
            for j, state in enumerate(dec_hidden):
                fluid.layers.array_write(
                    self._merge_batch_beams(state), j, dec_hidden)
            for j, state in enumerate(dec_cell):
                fluid.layers.array_write(
                    self._merge_batch_beams(state), j, dec_cell)
            ...
            for i in range(self.num_layers):
                ...
                new_hidden, new_cell = self.dec_units[i](
                    step_input, dec_hidden[i], dec_cell[i])
                # new_dec_hidden.append(new_hidden)
                # new_dec_cell.append(new_dec_cell)

                arr_len = fluid.layers.array_length(new_dec_hidden)
                new_dec_hidden = fluid.layers.array_write(new_hidden, arr_len,
                                                          new_dec_hidden)
                new_dec_cell = fluid.layers.array_write(new_dec_cell, arr_len,
                                                        new_dec_cell)
            ...
            noend_array = [-self.kinf] * self.tar_vocab_size
            noend_array[
                self.beam_end_token] = 0  # [-kinf, -kinf, ..., 0, -kinf, ...]
            noend_mask_tensor = to_variable(
                np.array(
                    noend_array, dtype='float32'))
            ...
            new_dec_hidden = self._change_size_for_array(
                self._split_batch_beams, new_dec_hidden)
            new_dec_cell = self._change_size_for_array(self._split_batch_beams,
                                                       new_dec_cell)
            ...
            print(" -" * 10, 13)
            for j, state in enumerate(new_dec_hidden):
                fluid.layers.array_write(
                    self._gather(state, beam_indices, batch_pos), j,
                    new_dec_hidden)
            for j, state in enumerate(new_dec_cell):
                fluid.layers.array_write(
                    self._gather(state, beam_indices, batch_pos), j,
                    new_dec_cell)
