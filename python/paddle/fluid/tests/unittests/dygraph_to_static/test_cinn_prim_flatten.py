...
class TestPrimForward(unittest.TestCase):
    ...
    def setUp(self):
        paddle.seed(2022)
        self.shapes = [[1, 2, 3, 4, 5], [64, 32, 16, 8, 4]]
        self.dtypes = ["float32", "float64"]

    def train(self, use_prim, data):
        for start in start_axes:
            for stop in stop_axes:
                return self._train(use_prim, data, start, stop)

    def _train(self, use_prim, data, start, stop):
        paddle.seed(2022)
        net = PrimeNet()
    ...
    def check_prim(self, net, use_prim):
        ...
        self.assertTrue('flatten_contiguous_range_grad' not in fwd_ops)
