...
class TestBook(unittest.TestCase):
    ...
    def test_tree_conv_layer(self):
        ...
        with program_guard(program):
            data = layers.data(
                name='Vector', shape=[None, 5, 7], dtype="float32")
            edges = layers.data(name='Edges', shape=[None, 5, 2], dtype="int64")
            w_attr = ParamAttr(initializer=Constant(1.0))
            bias_attr = ParamAttr(initializer=Constant(1.0))
            out = layers.tree_conv(data, edges, 10, 'tree_conv', 1, 2, 'tanh',
                                   w_attr, bias_attr)
