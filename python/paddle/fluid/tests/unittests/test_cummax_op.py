...
# define cummax grad function.
def cummax_grad(x, y, dy, dx, shape, dim):
    if dim < 0:
        dim += len(shape)
    mid_dim = shape[dim]
    outer_dim = 1
...
class TestCummax(OpTest):
    ...
    def prepare_inputs_outputs_attrs(self, dim, zero_num):
        ...
        self.out = np.maximum.accumulate(self.x, axis=dim)
...
class TestCummaxAPI(unittest.TestCase):
    ...
    def test_dygraph_api(self):
        def run(place):
            ...
            out = paddle.cummax(x, 1)
            out_ref = np.maximum.accumulate(self.x, 1)
            self.assertEqual(np.allclose(out_ref, out[0].numpy()), True)
            self.assertEqual(
                np.allclose(out_ref, x.index_select(out[1]).numpy(), True))
            paddle.enable_static()
