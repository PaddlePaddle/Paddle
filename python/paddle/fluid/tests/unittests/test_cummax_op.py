...
class TestCummax(OpTest):
    ...
    def setUp(self):
        ...
        self.op_type = "cummax"
        self.inputs = {'X': None}
        self.outputs = {'Out': None, 'Indices': None}
        self.attrs = {'axis': None}

    # def prepare_inputs_outputs_attrs(self, axis, zero_num):
    ...
    #     self.out = np.maximum.accumulate(self.x, axis=axis)
    #     self.inputs = {'X': self.x}
    #     self.outputs = {'Out': self.out}
    #     self.attrs = {'axis': axis}

    # def init_grad_input_output(self, axis):
    ...
    #     cummax_grad(reshape_x, out_data, self.grad_out, self.grad_x, self.shape,
    #                 axis)
    ...
    #     for axis in range(-len(self.shape), len(self.shape)):
    #         for zero_num in self.zero_nums:
    #             self.prepare_inputs_outputs_attrs(axis, zero_num)
    #             self.check_output()
    ...
    #         for zero_num in self.zero_nums:
    #             self.prepare_inputs_outputs_attrs(axis, zero_num)
    #             self.init_grad_input_output(axis)
...
# test float32 case.
class TestCummax_float32(TestCummax):
    def init_dtype(self):
        self.dtype = np.float32


# test complex64 case.
class TestCummax_complex64(TestCummax):
    def init_dtype(self):
        self.dtype = np.complex64


# test complex128 case.
class TestCummax_complex128(TestCummax):
    def init_dtype(self):
...
class TestCummaxAPI(unittest.TestCase):
    ...
    def test_static_api(self):
        ...
        def run(place):
            with paddle.static.program_guard(paddle.static.Program()):
                ...
                out, idx = paddle.cummax(x, -2)
                exe = paddle.static.Executor(place)
                res = exe.run(feed={'X': self.x}, fetch_list=[out])
            out_ref = np.maximum.accumulate(self.x, -2)
    ...
    def test_dygraph_api(self):
        def run(place):
            ...
            out, idx = paddle.cummax(x, 1)
            out_ref = np.maximum.accumulate(self.x, 1)
            self.assertEqual(np.allclose(out_ref, out.numpy()), True)
            self.assertEqual(
                np.allclose(out_ref, x.index_select(x[idx]).numpy(), True))
            paddle.enable_static()
