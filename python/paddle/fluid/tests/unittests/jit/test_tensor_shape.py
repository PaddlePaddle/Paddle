...
def dyfunc_tensor_shape_5(x):
    ...
    #           fluid.dygraph.jit.converter.convert_operators.convert_var_shape(x)[0]))`
    x = fluid.dygraph.to_variable(x)
    s = x.shape[0]
...
def dyfunc_with_if_1(x):
    ...
    if x_shape_0 < 1:
        ...
        #   `fluid.dygraph.jit.converter.convert_operators.convert_var_shape(res)[0]`
        if res.shape[0] > 1:
...
def dyfunc_with_for_1(x):
    ...
    # `x.shape[0]` is transformed into `fluid.dygraph.jit.converter.convert_operators.convert_var_shape(x)[0]`
...
def dyfunc_with_for_2(x):
    ...
    # `x_shape_0` is transformed into `fluid.dygraph.jit.converter.convert_operators.convert_var_shape(x)[0]`
