        ...
        for i in range(H_out):
            ...
            for j in range(W_out):
                ...
                for n in range(N):
                    for c in range(C):
                        arr = x_masked[n, c, :, :, :]
                        indice = np.where(arr == np.max(arr))
                        sub_deep = indice[0][0]
                        sub_row = indice[1][0]
                        sub_col = indice[2][0]
                        indice = ((d_start + sub_deep) * H +
                                 (h_start + sub_row)) * W + w_start + sub_col
                        mask[n, c, k, i, j] = indice
        ...
        for j in range(W_out):
            ...
            for n in range(N):
                for c in range(C):
                    ...
                    sub_row = indice[0][0]
                    sub_col = indice[1][0]
                    indice = (r_start + sub_row) * W + c_start + sub_col
                    mask[n, c, i, j] = indice
...
class TestMaxPoolWithIndex_Op(OpTest):
    ...
    def init_test_case(self):
        self.op_type = "max_pool3d_with_indice"
...
class TestCase4(TestMaxPoolWithIndex_Op):
    def init_test_case(self):
        self.op_type = "max_pool2d_with_indice"
