...
class TestFusedGateAttentionOp(OpTest):
    ...
    def GetFusedGateAttentionOut(self):
        ...
        _, _, _, _, _, final_out = _C_ops.fused_gate_attention(
            x, qkv_weight, nonbatched_bias, src_mask, gating_w, gating_b,
            output_w, output_b, 'is_gating', self.is_gating)
...
class TestFusedGateAttentionOpBF16(TestFusedGateAttentionOp):
    def config(self):
        super().config()
