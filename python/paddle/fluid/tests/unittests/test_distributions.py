...
class CategoricalNumpy(DistributionNumpy):
    ...
    def entropy(self):
        ...
        prob = e_logits / z
        return -1. * np.sum(prob * (logits - np.log(z)), 
            axis=-1, keepdims=True)

    def kl_divergence(self, other):
        logits = self.logits - np.max(self.logits, axis=-1, keepdims=True)
        other_logits = other.logits - np.max(other.logits, axis=-1, 
            keepdims=True)
        e_logits = np.exp(logits)
...
class MultivariateNormalDiagNumpy(DistributionNumpy):
    ...
    def entropy(self):
        return 0.5 * (self.scale.shape[0] * ( 1.0 + np.log(np.array(
                2 * math.pi).astype('float32'))) + np.log(self._det(self.scale)))

    def kl_divergence(self, other):
        tr_cov_matmul = np.sum(self._inv(other.scale) * self.scale)
        loc_matmul_cov = np.matmul((other.loc - self.loc), self._inv(other.scale))
...
class DistributionTest(unittest.TestCase):
    ...
    def test_categorical_distribution(self, batch_size=2, dims=3, tolerance=1e-6):
        test_program = fluid.Program()
        ...
        with fluid.program_guard(test_program):
            ...
            other_logits = layers.data(name='other_logits', shape=[dims], 
                dtype='float32')
        ...
        np.testing.assert_allclose(
            output_kl_np, gt_kl_np, rtol=tolerance)
    ...
    def test_multivariateNormalDiag_distribution(self, 
        ...
        other_scale_np = np.diag(np.random.random(batch_size, )).astype('float32')

        with fluid.program_guard(test_program):
            loc = layers.data(name='loc', shape=[batch_size, ], dtype='float32',
                              append_batch_size=False)
            scale = layers.data(name='scale', shape=[batch_size, batch_size], 
                                dtype='float32', append_batch_size=False)
            other_loc = layers.data(name='other_loc', shape=[batch_size, ], 
                                    dtype='float32', append_batch_size=False)
            other_scale = layers.data(name='other_scale', shape=[batch_size, batch_size], 
                                      dtype='float32', append_batch_size=False)

            multivariate_np = MultivariateNormalDiag(loc, scale)
            other_multivariate_np = MultivariateNormalDiag(other_loc, other_scale)
