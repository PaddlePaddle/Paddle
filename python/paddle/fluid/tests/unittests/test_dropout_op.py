...
class TestRandomValue(unittest.TestCase):
    def test_fixed_random_number(self):
        ...
        def _check_random_value(dtype, expect, expect_sum, expect_value):
            x = paddle.rand([3, 1024, 1024], dtype=dtype)
            ...
            self.assertTrue(np.allclose(np.sum(out), expect_sum))
        ...
        paddle.seed(100)
        expect_index = [786513, 402105982, 402073640]
        expect_sum = 1571383.125
        expect_value = [
            0.5845404, 0., 0., 2.1652932, 0., 0., 0.040565785, 0., 0., 0.
        ]
        _check_random_value(core.VarDesc.VarType.FP32, expect_index, expect_sum,
                            expect_value)

        expect_index = [785804, 401834444, 402346505]
        expect_sum = 1573329.79780755960382521152
        expect_value = [
            0., 0., 0., 0., 1.57558365192, 0.27930011, 1.388266437,
            2.4822938722, 0., 0.
        ]
        _check_random_value(core.VarDesc.VarType.FP64, expect_index, expect_sum,
                            expect_value)
