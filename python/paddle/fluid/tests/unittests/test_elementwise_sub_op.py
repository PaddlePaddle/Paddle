...
class TestSubTypePromoteAPI(unittest.TestCase):
    def setUp(self):
    ...
    def check_static_result(self, place):
        ...
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            ...
            y = np.array([1, 2, 3])
            self.assertRaises(TypeError, paddle.sub, x=x, y=y)
        ...
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            ...
            np_z = exe.run(fluid.default_main_program(),
                           feed={},
                           fetch_list=[res])
            self.assertEqual(np_z[0] == -2., True)
    ...
    def test_dygraph(self):
        for place in self.places:
            with fluid.dygraph.guard(place):
                ...
                self.assertRaises(TypeError, paddle.sub, x=x, y=np_y)
