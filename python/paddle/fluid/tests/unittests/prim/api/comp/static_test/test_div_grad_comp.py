...
@param.parameterized_class(
    ('primal0', 'primal1', 'cotangent', 'dtype'),
    [
        (
...
class TestDivGradComp(unittest.TestCase):
    ...
    def test_tanh_grad_comp(self):
        def actual(primal0, primal1, cotangent):
            ...
            return exe.run(
                ...
                fetch_list=[x.name + "@GRAD", y.name + "@GRAD"],
            )[0]

        def desired(primal0, primal1, cotangent):
        ...
        np.testing.assert_allclose(
            actual=actual(self.primal0, self.primal1, self.cotangent),
            desired=desired(self.primal0, self.primal1, self.cotangent),
            rtol=1e-6,
            atol=0,
        )
