...
class TestInterpOpAPI(unittest.TestCase):
    def test_case(self):
        x = fluid.data(name="x", shape=[2, 3, 6, 6], dtype="float32")
        ...
        out1 = interpolate(
            x, out_shape=[12, 12], resample='NEAREST', align_corners=False)
        out2 = fluid.layers.resize_nearest(
            x, out_shape=[12, 12], align_corners=False)

        out3 = interpolate(
            x, out_shape=[12, 12], resample='BILINEAR', align_corners=False)
        out4 = fluid.layers.resize_bilinear(
            x, out_shape=[12, 12], align_corners=False)

        out5 = interpolate(
            y, out_shape=[4, 4, 8], resample='TRILINEAR', align_corners=False)
        out6 = fluid.layers.resize_trilinear(
            y, out_shape=[4, 4, 8], align_corners=False)

        x_data = np.random.random((2, 3, 6, 6)).astype("float32")
        y_data = np.transpose(x_data, (0, 2, 3, 4, 1))
        ...
        self.assertTrue(np.allclose(results[2], results[3]))
        self.assertTrue(np.allclose(results[4], results[5]))
