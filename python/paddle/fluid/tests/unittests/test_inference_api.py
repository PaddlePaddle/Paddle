...
class TestInferenceApi(unittest.TestCase):
    def test_inference_api(self):
        ...
        self.assertEqual(type(paddletensor32.data.tolist('int32')), type(tensor32.tolist()))
        ...
        self.assertEqual(type(paddletensor64.data.tolist('int64')), type(tensor64.tolist()))
        ...
        value_float = np.array(paddletensor_float.data.float_data()).reshape(* [20, 2])
        ...
        self.assertEqual(type(paddletensor_float.data.tolist('float32')), type(tensor_float.tolist()))
        self.assertEqual(paddletensor_float.data.tolist('float32'), tensor_float.ravel().tolist())
        self.assertEqual(type(paddletensor_float.as_ndarray()), type(tensor_float))
        paddletensor_float.data.reset(tensor_float)
        self.assertEqual(paddletensor_float.as_ndarray().all(), tensor_float.all())
