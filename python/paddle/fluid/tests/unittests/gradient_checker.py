...
def get_eager_double_grad(func,
    ...
    if return_mid_result:
        return [dd_input for dd_input in dd_inputs if dd_input is not None], inputs + ddys
    else:
        return [
...
def get_eager_triple_grad(func,
    ...
    for dd_yi in dd_y:
        ...
        dddys.append(dddy)
    ddd_inputs = paddle.grad(outputs=dd_y, inputs=dd_x, grad_outputs=dddys, allow_unused = True)
    return [ddd_input.numpy() for ddd_input in ddd_inputs if ddd_input is not None]
