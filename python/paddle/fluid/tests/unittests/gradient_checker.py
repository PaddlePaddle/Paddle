...
def triple_grad_check(x,
    ...
    # append first order grads
    target_grads = fluid.gradients(y, x, y_grads)  # grad
    ...
    if y_grads_grads is None:
        ...
        y_grads_grads = _as_list(y_grads_grads)
        y_grads_grads_init = [
            var_to_np_array_in_scope(scope, place, v.name)
            for v in y_grads_grads
        ]
    # append second order grads
    target_grads_grads = fluid.gradients(target_grads, x,
                                         y_grads_grads)  # double grad

    ddx = []
    ddx_init = []
    for xi in x:
        dxi_name = _append_grad_suffix_(_append_grad_suffix_(xi.name))
        np_type = dtype_to_np_dtype(xi.dtype)
