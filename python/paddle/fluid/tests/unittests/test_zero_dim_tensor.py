    ...
    def test_static_reduce(self):
        ...
        for api in reduce_api_list:
            ...
            with paddle.static.program_guard(
                ...
                fetch_list = [out]
                if block.has_var(x.grad_name):
                    fetch_list.extend([out.grad_name])
...
class TestSundryAPI(unittest.TestCase):
    ...
    def test_median(self):
        ...
        x1 = paddle.rand([3, 5])
        x1.stop_gradient = False
        out1 = paddle.median(x1, None)
        out1.backward()

        self.assertEqual(out1.shape, [])
        self.assertEqual(x1.grad.shape, [3, 5])

    def test_std(self):
        # 1) x is OD
        x = paddle.rand([])
        x.stop_gradient = False
        out1 = paddle.std(x, None)
        out2 = paddle.std(x, [])
        out1.backward()
        out2.backward()
    ...
    def test_numpy(self):
        ...
        x_np = x.numpy()
        self.assertEqual(x_np.shape, ())
        self.assertEqual(x_np.item(), x.item())
    ...
    def _test_getitem(self):
        ...
        # 2) index=1D not reduce dimension
        # index=0D reduce dimension
        x1 = paddle.full([3, 5], 0.5)
        out1 = x1[paddle.to_tensor(1), paddle.to_tensor(2)]
        self.assertEqual(out1.shape, [])

        x2 = paddle.full([3, 5], 0.5)
        out2 = x2[paddle.to_tensor([1]), paddle.to_tensor([2])]
        self.assertEqual(out2.shape, [1, 1])

        # 3) 0D can not getitem or setitem
        with self.assertRaises(IndexError):
            x = paddle.rand([])
            out = x[:]

    def _test_setitem(self):
        # 1) ND slice to 0D
        x = paddle.full([3, 5], 0.5)
        x.stop_gradient = False
        x[1, 2] = 2.0

        self.assertEqual(x[1, 2], 2.0)

        # 2) index=1D not reduce dimension
        # index=0D reduce dimension
        x1 = paddle.full([3, 5], 0.5)
        x1[paddle.to_tensor(1), paddle.to_tensor(2)] = 2.0
        self.assertEqual(x1[1, 2], 2.0)

        x2 = paddle.full([3, 5], 0.5)
        x2[paddle.to_tensor([1]), paddle.to_tensor([2])] = 2.0
        self.assertEqual(
            x2[paddle.to_tensor([1]), paddle.to_tensor([2])], [[2.0]]
        )
...
class TestSundryAPIStatic(unittest.TestCase):
    ...
    def test_static_data(self):
        x = paddle.static.data('X', [], dtype='float32')
        res = self.exe.run(feed={'X': 0.5}, fetch_list=[x])
        self.assertEqual(res[0].shape, ())
        self.assertEqual(res[0].item(), 0.5)
    ...
    def test_std(self):
        ...
        self.assertEqual(res[6].shape, (3, 5))
