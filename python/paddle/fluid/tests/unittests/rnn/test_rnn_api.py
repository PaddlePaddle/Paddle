...
# class TestTinyRNNBackward(unittest.TestCase):
#     def __init__(self, time_major=True, direction="forward", place="cpu"):
#         super().__init__("runTest")
#         self.time_major = time_major
#         self.direction = direction
...
#         paddle.seed(self.seed)
#         paddle.framework.random._manual_program_seed(self.seed)
#         cell_dy = TinyRNN(self.input_size)
#         self.rnn_net = paddle.nn.RNN(cell_dy, time_major=self.time_major)
...
#                 cell_st = TinyRNN(self.input_size)
#                 self.rnn_st = paddle.nn.RNN(cell_st, time_major=self.time_major)
...
#                 sgd = paddle.optimizer.SGD(0.1)
#                 sgd.minimize(loss)
#                 self.fetch_list = [st_out, st_last_h,  "x@GRAD"]
