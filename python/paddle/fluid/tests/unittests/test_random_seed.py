...
class TestGeneratorSeed(unittest.TestCase):
    ...
    def test_generator_sampling_id_dygraph(self):
        ...
        gen.manual_seed(12312321111)
        x = paddle.uniform([10, 10], dtype="float32", min=0.0, max=1.0)
        y = fluid.layers.sampling_id(x)

        st1 = gen.get_state()
        x1 = paddle.uniform([10, 10], dtype="float32", min=0.0, max=1.0)
        y1 = fluid.layers.sampling_id(x)

        gen.set_state(st1)
        x2 = paddle.uniform([10, 10], dtype="float32", min=0.0, max=1.0)
        y2 = fluid.layers.sampling_id(x)

        gen.manual_seed(12312321111)
        x3 = paddle.uniform([10, 10], dtype="float32", min=0.0, max=1.0)
