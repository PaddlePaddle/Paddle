...
class TestMergeLayernormFusePass(PassAutoScanTest):
    ...
    def sample_program_config(self, draw):
        ...
        layernorm_40_epsilon = draw(st.floats(min_value=0.0000001, max_value=0.001))
        ...
        def generate_input(attrs):
            return np.random.random(
                [attrs[3]['batch_size'],
                 attrs[3]['input_H_W']*
                 attrs[3]['input_H_W'],
                 attrs[3]['input_n']]).astype(np.float32)
        def generate_weight(attrs):
            return np.random.random(
                [attrs[3]['input_n']*4]).astype(np.float32)
        attrs = [
        ...
        program_config = ProgramConfig(
            ops=[reshape2_00_op,
                 strided_slice_10_op,strided_slice_11_op,
                 strided_slice_12_op,strided_slice_13_op,
                 concat_20_op,
                 reshape2_30_op,
                 layernorm_40_op],
            weights={
