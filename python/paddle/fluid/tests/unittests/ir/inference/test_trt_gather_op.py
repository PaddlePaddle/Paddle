...
class TRTGatherTest2(InferencePassTest):
    def setUp(self):
        self.set_params()
        with fluid.program_guard(self.main_program, self.startup_program):
            data = fluid.data(name='data', shape=[-1, 256, 8], dtype='float32')
            index = fluid.data(name='index', shape=[-1], dtype='int32')
            scale_out = fluid.layers.gather(data, index=index)
            out = fluid.layers.softmax(input=scale_out)

        self.feeds = {
            "data": np.random.random([self.bs, 256, 8]).astype("float32"),
            "index": self.index
        }
        ...
        self.dynamic_shape_params = TRTGatherTest2.DynamicShapeParam({
            ...
            'index': [32]}, {'data': [10, 256, 8],
                             'index': [10]}, False)
        self.fetch_list = [out]

    def set_params(self):
        self.index = np.array([2, 4, 5, 7, 9], dtype='int32')
        self.bs = 10
