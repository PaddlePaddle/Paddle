...
class TestOneDNNConvElementwiseAddFusePass(PassAutoScanTest):
    ...
    def test(self):
        self.run_and_statis(
            quant=False, passes=['conv_elementwise_mkldnn_fuse_pass']
        )


class TestOneDNNConvElementwiseMulFusePass(PassAutoScanTest):
    def is_program_valid(self, program_config: ProgramConfig) -> bool:
        attrs = [
            program_config.ops[i].attrs for i in range(len(program_config.ops))
        ]
    ...
    def sample_program_config(self, draw):
        ...
        program_config = ProgramConfig(
            ops=[relu_op, conv2d_op1, conv2d_op2, elementwise_op],
            weights={
                'conv_weight1': TensorConfig(data_gen=partial(generate_weight)),
                'conv_weight2': TensorConfig(data_gen=partial(generate_weight)),
            },
