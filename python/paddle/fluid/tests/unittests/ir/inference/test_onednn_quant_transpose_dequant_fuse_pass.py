...
class TestQuantTranspose2OneDNNFusePass(PassAutoScanTest):
...
class TestTranspose2DequantOneDNNFusePass(PassAutoScanTest):
    def is_program_valid(self, program_config: ProgramConfig) -> bool:
        return True
    ...
    def sample_program_config(self, draw):
        ...
        def generate_input():
            if transpose_X:
                ...
                shape_x = [batch_size, channel, 32, input_dim]
            return np.random.random(shape_x).astype(np.int8)
        ...
        dequantize_op = OpConfig(
            type='dequantize',
            inputs={'Input': ['transpose2_output']},
        ...
        program_config = ProgramConfig(
            ops=[transpose2_op, dequantize_op],
            weights={},
            inputs={
                'input_data': TensorConfig(data_gen=partial(generate_input))
            },
