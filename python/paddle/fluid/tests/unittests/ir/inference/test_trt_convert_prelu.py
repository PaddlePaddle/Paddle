...
class TrtConvertPreluTest_all(TrtLayerAutoScanTest):
    def init(self):
        self.mode = "all"
    ...
    def update_program_input_and_weight_with_attr(self, op_attr_list):
        ...
        alpha = np.random.random(size=(1)).astype("float32")
        alpha_weight = TensorConfig(shape=[1], data=alpha)

        self.program_weights = {"alpha_weight": alpha_weight}
...
class TrtConvertPreluTest_channel(TrtConvertPreluTest_all):
    def init(self):
        self.mode = "channel"
        self.trt_engine_num = 1
        self.paddle_op_num = 2
    ...
    def update_program_input_and_weight_with_attr(self, op_attr_list):
        input_data = TensorConfig(shape=[-1, 3, 64, 64])
        alpha = np.random.random(size=(1, 3, 1, 1)).astype("float32")
        alpha_weight = TensorConfig(shape=[1, 3, 1, 1], data=alpha)
...
class TrtConvertPreluTest_element(TrtConvertPreluTest_all):
    def init(self):
        self.mode = "element"
        self.trt_engine_num = 1
        self.paddle_op_num = 2
    ...
    def update_program_input_and_weight_with_attr(self, op_attr_list):
        input_data = TensorConfig(shape=[-1, 3, 64, 64])
        alpha = np.random.random(size=(1, 3, 64, 64)).astype("float32")
        alpha_weight = TensorConfig(shape=[1, 3, 64, 64], data=alpha)
