...
class TestSequencePadOpError(unittest.TestCase):
    def test_error(self):
        ...
        def test_pad_value_variable():
            x1 = fluid.layers.data(name='x1', shape=[10, 5], dtype='float32', lod_level=1)
        ...
        def test_dtype():
            x2 = fluid.layers.data(name='x2', shape=[10, 5], dtype='int16', lod_level=1)
            pad_value2 = fluid.layers.assign(input=np.array([0.0], dtype=np.int32))
