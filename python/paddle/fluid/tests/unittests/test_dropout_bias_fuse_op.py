...
class TestDropoutBiasFuseOp(OpTest):
    # test for dropout_prob = 0 (No drop)
    def setUp(self):
...
class TestDropoutBiasFuseOp1(TestDropoutBiasFuseOp):
    # test for upscale_in_train version of dropout with dropout_prob=1
    def setUp(self):
        self.op_type = "dropout_bias_fuse"
        self.inputs = {
            'X': np.random.random((16, 128)).astype("float64"),
...
class TestDropoutBiasFuseOp2(TestDropoutBiasFuseOp):
    # test for upscale_in_train version of dropout with dropout_prob=0
    def setUp(self):
        self.op_type = "dropout_bias_fuse"
        self.inputs = {
...
class TestDropoutBiasFuseOp5(unittest.TestCase):
    # test the python side API for dropout_bias_fuse op
    def setUp(self):
        np.random.seed(123)
