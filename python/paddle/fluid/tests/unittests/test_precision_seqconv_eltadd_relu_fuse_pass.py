...
class TestSeqconvEltaddReluFusePass(unittest.TestCase):
    def test_seqconv_eltadd_relu_fuse_pass_cpu_precision(self):
    ...
    def test_seqconv_eltadd_relu_fuse_pass_gpu_precision(self):
        if fluid.is_compiled_with_cuda():
            ...
            np_x = np.array([i for i in range(10)]).reshape(
                [10, 1]).astype('float32')
            in_data = fluid.create_lod_tensor(np_x, [[2, 3, 5]],
                                              fluid.CPUPlace())
            fw_output = exe.run(feed={"x": in_data},
                                fetch_list=[seqconv_res],
                                return_numpy=False)
