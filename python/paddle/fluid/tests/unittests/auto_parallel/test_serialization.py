...
class MLPLayer(nn.Layer):
    ...
    def forward(self, input):
        ...
        out = auto.shard_op(self.linear1,
                            dist_attr={"process_mesh": _g_process_mesh})(out)
...
class TestDistAttrSerialization(unittest.TestCase):
    ...
    def test_serialization_program(self):
        ...
        for original_block, block in zip(original_program.blocks,
                                         program.blocks):
            self.assertEqual(len(original_block.vars.values()),
                             len(block.vars.values()))
            for original_tensor in original_block.vars.values():
                self.assertEqual(original_tensor.dist_attr,
