...
def make_program_dp2():
    ...
    with paddle.static.program_guard(main_program, start_program):
        cond = paddle.static.data(name='cond', shape=[4, 5, 1], dtype='bool')
        x = paddle.static.data(name='x', shape=[4, 5, 6], dtype='float32')
        y = paddle.static.data(name='y', shape=[4, 5, 6], dtype='float32')

        auto.shard_tensor(
            x, auto.ProcessMesh([0], dim_names=["x"]), [None, None, None]
        )

        x = paddle.where(cond, x, y)
        loss = paddle.mean(x)
...
class TestDistSlice(unittest.TestCase):
    def test_dist_slice_dp2(self):
        # for rank in range(2):
        dist_main_prog, dist_context = parallelizer(make_program_dp2, 0)
