...
class TestNPUIdentityOp(unittest.TestCase):
    ...
    def test_api_static(self):
        ...
        with paddle.static.program_guard(main_program, startup_program):
            ...
            output = paddle.npu_identity(x=x_data, format=self.format)
        ...
        np.testing.assert_allclose(result[0].shape, self.out_shape, rtol=1e-08)

    def test_api_dygraph(self):
        paddle.disable_static(self.place)

        x_tensor = paddle.to_tensor(self.x)
        out = paddle.npu_identity(x_tensor, self.format)

        np.testing.assert_allclose(out.shape, self.out_shape, rtol=1e-08)
        paddle.enable_static()
