...
class TestFlattenParamGradsAdam(unittest.TestCase):
    def _test_static(
        self,
        place,
    ...
    def _test_adam_api_dygraph(self):
        # NOTE(zhiqiu): cpu and gpu has different seed, so should compare separatly.
        self._test_with_place('cpu', is_dygraph=True)
        # self._test_with_place(paddle.CPUPlace(), is_dygraph=False)
        if core.is_compiled_with_cuda():
            self._test_with_place('gpu:0', is_dygraph=True)
            self._test_with_place('gpu:0', is_dygraph=True)
            # self._test_with_place(paddle.CUDAPlace(0), is_dygraph=False)
    
    # def test_adam_api_static(self):
    #     # NOTE(zhiqiu): cpu and gpu has different seed, so should compare separatly.
    #     self._test_with_place(paddle.CPUPlace(), is_dygraph=False)
    #     if core.is_compiled_with_cuda():
    #         self._test_with_place(paddle.CUDAPlace(0), is_dygraph=False)
...
class TestFlattenParamGradsAdamw(unittest.TestCase):
    ...
    #     for flatten_param_grads in [True, False]:
    #         # for func in [self._test_static, self._test_dygraph]:
    #         # for func in [self._test_static]:
    #         for func in [self._test_dygraph]:
    #             pred, loss = func(
    #                 place,
    #                 flatten_param_grads,
    #             )
    ...
    # def test_adamw_flatten_param_grads(self):
    #     self._test_with_place('cpu')
    #     if core.is_compiled_with_cuda():
    #         self._test_with_place('gpu:0')
"""
