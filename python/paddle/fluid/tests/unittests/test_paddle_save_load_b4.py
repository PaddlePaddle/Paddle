...
class TestSaveLoadBinaryFormat(unittest.TestCase):
    ...
    def test_save_load_lod_tensor(self):
        ...
        with new_program_scope():
            ...
            dirname = 'b4_test_save_load_lod_tensor1/tensor_'
            ...
            msg = 'start up:' + str(time.time() - start)
            loaded_tensor = paddle.load(dirname + 'fc_vars.w_0')
            msg += '\n load1:' + str(time.time() - start)
            ...
            msg += '\n to array:' + str(time.time() - start)
            self.assertTrue(np.array_equal(origin, to_array))
            msg += '\n assertTrue1:' + str(time.time() - start)

        with self.assertRaises(NotImplementedError):
            path = 'b4_test_save_load_error/temp'
            paddle.save({}, path, use_binary_format=True)
        msg += '\n save1:' + str(time.time() - start)
        ...
        with self.assertRaises(ValueError):
            ...
            with open(path, "w") as f:
                f.write('\0')
            paddle.load(path)
        msg += '\n load2:' + str(time.time() - start)

        with self.assertRaises(ValueError):
            temp_lod = fluid.core.LoDTensor()
            paddle.save(temp_lod, path, use_binary_format=True)
        msg += '\n save2:' + str(time.time() - start)

        with self.assertRaises(RuntimeError):
            fluid.core._save_lod_tensor(
                temp_lod,
                'b4_test_save_load_error_not_exist_file/not_exist_file')
        msg += '\n save3:' + str(time.time() - start)

        with self.assertRaises(RuntimeError):
            fluid.core._load_lod_tensor(
                temp_lod,
                'b4_test_save_load_error_not_exist_file/not_exist_file')
        msg += '\n load3:' + str(time.time() - start)
