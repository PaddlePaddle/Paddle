...
class ApiHeavisideTest(OpTest):
    def setUp(self):
        ...
        self.np_expected1 = np.heaviside(self.input_x, self.input_y)
        self.np_expected2 = np.heaviside(self.input_x, self.input_z)
        self.np_expected3 = np.heaviside(self.input_a, self.input_c)
        self.np_expected4 = np.heaviside(self.input_b, self.input_c)
    ...
    def test_static_api(self):
        ...
        with paddle.static.program_guard(paddle.static.Program(),
            ...
            result_heaviside = paddle.heaviside(data_x, data_y)
            exe = paddle.static.Executor(self.place)
            res, = exe.run(feed={"x": self.input_x,
                                 "y": self.input_y},
                           fetch_list=[result_heaviside])
        ...
        with paddle.static.program_guard(paddle.static.Program(),
            ...
            result_heaviside = paddle.heaviside(data_x, data_z)
            exe = paddle.static.Executor(self.place)
            res, = exe.run(feed={"x": self.input_x,
                                 "z": self.input_z},
                           fetch_list=[result_heaviside])
        ...
        with paddle.static.program_guard(paddle.static.Program(),
            ...
            result_heaviside = paddle.heaviside(data_a, data_c)
            exe = paddle.static.Executor(self.place)
        ...
        with paddle.static.program_guard(paddle.static.Program(),
            ...
            data_c = paddle.static.data("c", shape=[3], dtype="int64")
            result_heaviside = paddle.heaviside(data_b, data_c)
            exe = paddle.static.Executor(self.place)
    ...
    def test_dynamic_api(self):
        ...
        res = paddle.heaviside(x, y)
        res = res.numpy()
        self.assertTrue(np.allclose(res, self.np_expected1))

        # test broadcast
        res = paddle.heaviside(x, z)
        res = res.numpy()
        self.assertTrue(np.allclose(res, self.np_expected2))

        res = paddle.heaviside(a, c)
        res = res.numpy()
        self.assertTrue(np.allclose(res, self.np_expected3))
