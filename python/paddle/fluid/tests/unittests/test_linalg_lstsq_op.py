...
class LinalgLstsqTestCase(unittest.TestCase):
    ...
    def generate_output(self):
        if len(self._input_shape_1) == 2:
            ...
            out = np.linalg.lstsq(self._input_data_1[0], self._input_data_2[0], rcond=self.rcond)
    ...
    def test_dygraph(self):
        ...
        if self.driver in ("gelsd", "gelss"):
            if (np.abs(res_singular_values - self._output_sg_values) < 1e-6).any():
                pass
            else:
        ...
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            ...
            if self.driver in ("gelsd", "gelss"):
                if (np.abs(fetches[3] - self._output_sg_values) < 1e-6).any():
                    ...
                    print("EXPECTED: \n", self._output_sg_values)
                    print("GOT     : \n", fetches[3])
                    raise RuntimeError("Check LSTSQ singular values static Failed")
...
class LinalgLstsqTestCaseLarge2(LinalgLstsqTestCase):
    def init_config(self):
        ...
        self.rcond = 1e-15
        self.driver = "gelss"
        self._input_shape_1 = (500, 600)
        self._input_shape_2 = (500, 300)
