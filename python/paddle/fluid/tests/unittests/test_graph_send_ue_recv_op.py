...
class API_GeometricSendUERecvTest(unittest.TestCase):
    def test_compute_all_with_sum(self):
        ...
        res_sub = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "sub", "sum"
        )
        res_mul = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "mul", "sum"
        )
    ...
    def test_compute_all_with_mean(self):
        ...
        res_sub = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "sub", "mean"
        )
        res_mul = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "mul", "mean"
        )
        res_div = paddle.geometric.send_ue_recv(
    ...
    def test_compute_all_with_max(self):
        ...
        res_sub = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "sub", "max"
        )
        res_mul = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "mul", "max"
        )
        res_div = paddle.geometric.send_ue_recv(
    ...
    def test_compute_all_with_max_fp16(self):
        ...
        if core.is_compiled_with_cuda():
            ...
            if core.is_float16_supported(place):
                ...
                np_add = np.array(
                    [[1, 3, 4], [3, 7, 8], [2, 5, 6]], dtype="float16"
                )
                np_sub = np.array(
                    [[-1, 1, 2], [1, 5, 6], [0, 3, 4]], dtype="float16"
                )
                np_mul = np.array(
                    [[0, 2, 3], [2, 6, 7], [1, 4, 5]], dtype="float16"
                )
                np_div = np.array(
                    [[0, 2, 3], [2, 6, 7], [1, 4, 5]], dtype="float16"
                )
                ...
                np.testing.assert_allclose(
                    np_sub, res_sub, rtol=1e-05, atol=1e-06
                )
    ...
    def test_compute_all_with_min(self):
        ...
        res_sub = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "sub", "min"
        )
        res_mul = paddle.geometric.send_ue_recv(
            x, y, src_index, dst_index, "mul", "min"
        )
        res_div = paddle.geometric.send_ue_recv(
    ...
    def test_compute_all_with_min_fp16(self):
        ...
        if core.is_compiled_with_cuda():
            ...
            if core.is_float16_supported(place):
                ...
                np_sub = np.array(
                    [[-1, 1, 2], [-1, 1, 2], [0, 3, 4]], dtype="float16"
                )
                np_mul = np.array(
                    [[0, 2, 3], [0, 2, 3], [1, 4, 5]], dtype="float16"
                )
                np_div = np.array(
                    [[0, 2, 3], [0, 2, 3], [1, 4, 5]], dtype="float16"
                )
