...
def train_main(use_cuda, is_sparse, is_local=True):
    ...
    train_reader = paddle.batch(
        paddle.reader.shuffle(
            paddle.dataset.wmt14.train(dict_size), buf_size=1024 * 10),
        batch_size=batch_size)

    test_reader = paddle.batch(
        paddle.reader.shuffle(
            paddle.dataset.wmt14.test(test_dict_size), buf_size=1024 * 10),
        batch_size=batch_size)
        ...
        if isinstance(event, fluid.EndStepEvent):
            if (event.step % 5) == 0:
                out = trainer.test(
                    ...
                    data_feed_handler=partial(func_feed, place))
                avg_cost_val = out[0]
                print('pass_id=' + str(event.epoch) + ' batch=' +
                      str(event.step) + " avg_cost=" + str(avg_cost_val))
            else:
                print(
                    'pass_id=' + str(event.epoch) + ' batch=' + str(event.step))
