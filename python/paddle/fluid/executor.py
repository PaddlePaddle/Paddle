...
class Executor(object):
    ...
    def __init__(self, place):
        ...
        if isinstance(place, core.CPUPlace):
            self.async_executor = core.AsyncExecutor(global_scope(), p)
        else:
    ...
    def run_from_files(self,
                       ...
                       debug=False):
        """
        Run program `program`, training dataset will be set in filelist.

        This method utilizes an Asynchronous Executor in Python. Through
        exploiting the power of multi-core processor and data queueing,
        it makes data reading and cosuming decoupled, each run in multiple
        threads in parallel.

        Instead of reading data in python side, this method accepts a training
        ...
        :code:`fetch`, like in fluid.Executor.run(). Unlike fluid.Executor,
        however, AsyncExecutor doesn't return fetched variables, instead, it
        will dump the values of each fetched variable to stdandard output.
        ...
        :code:`run_from_files` is in active development and the API might
        ...
        self.async_executor.run_from_files(program_desc,
                                           data_feed.desc(), filelist,
