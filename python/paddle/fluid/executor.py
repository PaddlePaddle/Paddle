...
def _get_program_cache_key(feed, fetch_list):
    ...
    fetch_var_names = map(_var_to_name, fetch_list)
...
class Executor(object):
    ...
    def run(self,
        ...
        if self.log_level >= 5:
            params_grads = program.block(0).vars.keys()
            for key in params_grads:
                if isinstance(key, basestring):
                    fetch_list.append(str(key))
                elif isinstance(key, Variable):
                    fetch_list.append(key.desc.name)
                else:
                    raise ValueError("The var name of parameter error")
            # filter the variable not exsits in startup program
            def filter_vars(var):
            ...
            fetch_list = filter(filter_vars, map(_var_to_name, fetch_list))
        ...
        if self.log_level >= 5:
            ...
            vars_list = []
            loggings = []
            for idx, var_name in enumerate(fetch_list):
                ...
                for i, op in enumerate(ops):
                    ...
                    if var_name in outputs:
                        # NOTE(dzhwinter): there are loop in graph
                        # here output is generated by forward/backward op
                        # ignore the optimize ones.
                        ops_list.append(op.type)
                        index = i
                        break
                if index == -1:
                    raise ValueError("Not found in output!")
                loggings.append((index, ops_list[idx], vars_list[idx],
                                 fetch_list[idx], outs[idx]))
            loggings.sort(key=lambda x: x[0], reverse=False)
            for _, op, var, fetch_var_name, fetch_var_value in loggings:
                print "{0}->({1}:{3})->{2}".format(op, fetch_var_name,
