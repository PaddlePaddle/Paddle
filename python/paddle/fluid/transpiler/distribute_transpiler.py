...
to transpile() call.")
    ...
    def append_bn_repeat_init_op(main_prog, startup_prog, num_repeats):
        """
            For distributed training using multi batch merging, and the
            have batch normalization ops in the program, you need to call
            this function to append initialize operations for mean and
            variance for each repeat.

            Args:
                main_prog (Program): main program for training
                startup_prog (Program): startup program, this function call
                    will append initialize ops for bn mean and variance params
                    for each repeat
                num_repeats (int): must use same value passed to BuildStrategy
            
            Examples:
                .. code-block:: python

                    num_repeats = 4
                    fluid.DistributeTranspiler.append_bn_repeat_init_op(
                    ...
                    build_st.multi_batch_merge_repeats = num_repeats
                    parallel_exe = fluid.ParallelExecutor(
                        True,
                        loss.name,
                        build_strategy=build_st
                    )
                    parallel_exe.run(fluid.default_main_program())
        """
        repeat_vars = set()
                ...
                if op.type == "fill_constant":
                    for oname in op.output_arg_names:
                        if oname in repeat_vars:
                            ...
                            print("append init ops for ", repeat_var_name,
                                  var.shape)
