...
def _generate_doc_string_(
    ...
    for each_input in op_proto.inputs:
        line_begin = '    {}'.format(_convert_(each_input.name))
...
def generate_layer_fn(op_type):
    ...
    if len(not_intermediate_outputs) != 1:
        raise ValueError(
            ...
            "automatically generated. {}".format(op_type),
        )
        ...
        for ipt in op_proto.inputs:
            ...
            for each in val:
                if not isinstance(each, Variable):
                    raise ValueError(
                        "input of {} must be variable".format(op_type)
                    )
    ...
    * ${{name}_type}: The type of ${name}.
    ...
    def __impl__(func):
        ...
        for each_input in op_proto.inputs:
            ...
            args["{}_comment".format(input_name)] = trim_ending_dot(
                each_input.comment
            )
            args["{}_type".format(input_name)] = "Variable"
        for each_attr in op_proto.attrs:
            input_name = _convert_(each_attr.name)
            args["{}_comment".format(input_name)] = trim_ending_dot(
                each_attr.comment
            )
            args["{}_type".format(input_name)] = _type_to_str_(each_attr.type)

        for each_opt in op_proto.outputs:
            output_name = _convert_(each_opt.name)
            args["{}_comment".format(output_name)] = trim_ending_dot(
                each_opt.comment
            )
            args["{}_type".format(output_name)] = "Variable"
