...
def forward_grad(outputs, inputs, grad_inputs=None):
    ...
    Args:
        outputs: The target tensor or tensors
    ...
    if any(x.block != block for x in xs + ys):
        raise RuntimeError(
            'Variable in inputs and outputs should exist in current block of '
            'main program.')
...
def grad(outputs, inputs, grad_outputs=None):
    ...
    Args:
        outputs: The target tensor or tensors
        inputs: The input tensor or tensors
        grad_outputs: The gradient Tensor or Tensors of outputs which has 
            the same shape with outputs, Defaults to None, in this case is 
            equivalent to all ones.

    Returns:
        grad_inputs (Tensor|Sequence[Tensor]): The gradients for inputs.
