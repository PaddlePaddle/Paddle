...
def prepare_python_api_arguments(op):
    ...
    if op.input_names is None:
        ...
        if set(inputs) != set(op.input_names):
            raise ValueError(
                f"op input names {inputs} does not match op {op.type} input names {op.input_names}."
            )
        res = [get_var_block(op.block, op.input(n)) for n in sorted(inputs)]
        if attrs:
            res.extend(attrs)
        return res
