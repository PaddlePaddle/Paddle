...
def int4_gemm_cutlass(x, y, bias=None, activation='none'):
    """
    ...
    Examples:
        .. code-block:: python
            ...
            # required: gpu
            import paddle
            from paddle.incubate.nn.functional import fused_int4_gemm_epilogue

            x = paddle.randn([3, 4])
            y = paddle.randn([4, 5])
            bias = paddle.randn([5])
            out = fused_int4_gemm_epilogue(x, y, bias)
            print(out.shape) # [3, 5]
    """
    helper = LayerHelper('int4_gemm_cutlass', **locals())
    out = helper.create_variable_for_type_inference(dtype=x.dtype)
    helper.append_op(
        type='int4_gemm_cutlass',
        inputs={
            'X': x,
            'Y': y,
            'Bias': bias,
        ...
        outputs={'Out': out},
        attrs={'activation': activation, 'trans_x': False, 'trans_y': False},
    )
    return out
