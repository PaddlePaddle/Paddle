...
class FusedMultiTransformerInt8(Layer):
    ...
    def __init__(self,
                 ...
                 ffn2_weight_attrs=None,
                 ffn2_bias_attrs=None,
                 qkv_out_scales_attr=None,
                 out_linear_out_scales_attr=None,
                 ffn1_out_scales_attr=None,
                 ffn2_out_scales_attr=None,
                 qkv_in_scale=None,
        ...
        self.qkv_out_scales = self.create_parameter(
            shape=[num_layers, 3 * embed_dim],
            attr=qkv_out_scales_attr,
            dtype="float32",
            is_bias=False)

        self.out_linear_out_scales = self.create_parameter(
            shape=[num_layers, embed_dim],
            attr=out_linear_out_scales_attr,
            dtype="float32",
            is_bias=False)
        self.ffn1_out_scales = self.create_parameter(
            shape=[num_layers, 4 * embed_dim],
            attr=ffn1_out_scales_attr,
            dtype="float32",
            is_bias=False)
        self.ffn2_out_scales = self.create_parameter(
            shape=[num_layers, embed_dim],
    ...
    def forward(self, src, attn_mask=None, caches=None, time_step=None):
        ...
        if caches is not None:
            assert len(caches) == len(self.qkv_weights)
        out = incubate_f.fused_multi_transformer_int8(
            src,
            self.ln_scales,
            self.ln_biases,
            self.qkv_weights,
