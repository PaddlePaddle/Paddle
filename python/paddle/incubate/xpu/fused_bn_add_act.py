...
def fused_bn_add_act(x,
    ...
    if fluid.framework.in_dygraph_mode():
        attrs = ('momentum', momentum, "epsilon", epsilon, 'act_type', act_type,
                 'use_global_stats', use_global_stats, 'is_test', not training,
                 "trainable_statistics", trainable_statistics)
        out, _, _, _, _, _ = \
                              getattr(_C_ops, "fused_bn_add_activation")(x, z, scale, bias, mean, var, mean, var, *attrs)
        return out

    helper = LayerHelper('fused_bn_add_activation', **locals())
...
class FusedBNAddAct(Layer):
    def __init__(self,
                 ...
                 name=None,
                 trainable_statistics=False):
        super(FusedBNAddAct, self).__init__()
