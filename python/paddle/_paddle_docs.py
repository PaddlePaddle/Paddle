...
add_doc_and_signature(
    ...
    .. note::
        ...
        For example, ``gcd(input=tensor_x, other=tensor_y)`` is equivalent to ``gcd(x=tensor_x, y=tensor_y)``.
...
add_doc_and_signature(
    ...
    .. note::
        Alias Support: The parameter name ``input`` can be used as an alias for ``x``, and the parameter name ``other`` can be used as an alias for ``y``.
        For example, ``lcm(input=tensor_x, other=tensor_y)`` is equivalent to ``lcm(x=tensor_x, y=tensor_y)``.
