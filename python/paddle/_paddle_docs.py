...
add_doc_and_signature(
    ...
    .. note::
        Alias Support: The parameter name ``input`` can be used as an alias for ``x``.
        For example, ``asinh(input=tensor_x)`` is equivalent to ``asinh(x=tensor_x)``.
...
add_doc_and_signature(
    ...
    .. note::
        Alias Support: The parameter name ``input`` can be used as an alias for ``x``.
        For example, ``atan(input=tensor_x)`` is equivalent to ``atan(x=tensor_x)``.
    ...
    Returns:
        Tensor. Output of Atan operator, a Tensor with shape same as input
            (integer types are autocasted into float32).
...
add_doc_and_signature(
    ...
    Examples:
        .. code-block:: python
            ...
            >>> x = paddle.to_tensor([-0.4, -0.2, 0.1, 0.3])
            >>> y = paddle.to_tensor([0.2, 0.3, 0.4, 0.5])
            >>> out = paddle.atan2(x, y)
            >>> print(out)
            Tensor(shape=[4], dtype=float32, place=Place(cpu), stop_gradient=True,
            [-1.10714877, -0.58800262,  0.24497866,  0.54041950])
    """,
    """
    def atan2(
        x: Tensor,
