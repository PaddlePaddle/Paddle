        ...
        for value in input_values:
            ...
            if defining_op.name() == "builtin.parameter":
                ...
                if value.get_defining_op().name() == "builtin.split":
                    # TODO if the input value is generated by the other trt_engine_op, maybe the shape is unknown.
                    min_shape = tuple(dim if dim != -1 else 1 for dim in shape)
                    opt_shape = tuple(dim if dim != -1 else 4 for dim in shape)
                    max_shape = tuple(dim if dim != -1 else 8 for dim in shape)
                else:
