...
class DenseLayer(nn.Layer):
    ...
    def __init__(self,in_c,growth_rate,bn_size):
        ...
        self.layers=nn.Sequential(nn.BatchNorm2D(in_c),nn.ReLU(),nn.Conv2D(in_c,out_c,1),nn.BatchNorm2D(out_c),nn.ReLU(),nn.Conv2D(out_c,growth_rate,3,padding=1))
...
def DenseNet201(pretrained=False, batch_norm=False, **kwargs):
    ...
    if batch_norm:
        model_name += ('_bn')
    return _DenseNet(model_name, (6,12,64,48), batch_norm, pretrained, **kwargs)
