...
class BeamSearchDecoder(Decoder):
    ...
    def initialize(self, initial_cell_states):
        ...
        """
        self.kinf = 1e9
        state = paddle.utils.layers_utils.flatten(initial_cell_states)[0]
        ...
        init_cell_states = paddle.utils.layers_utils.map_structure(
    ...
    def _beam_search_step(self, time, logits, next_cell_states, beam_state):
        ...
        next_cell_states = paddle.utils.layers_utils.map_structure(
    ...
    def step(self, time, inputs, states, **kwargs):
        ...
        """
        inputs = paddle.utils.layers_utils.map_structure(
            self._merge_batch_beams, inputs
        )
        cell_states = paddle.utils.layers_utils.map_structure(
            self._merge_batch_beams, states.cell_states
        )
        cell_outputs, next_cell_states = self.cell(
            inputs, cell_states, **kwargs
        )
        cell_outputs = paddle.utils.layers_utils.map_structure(
...
def _dynamic_decode_imperative(
    ...
    **kwargs
    ...
    while cond.numpy():
        ...
        if not decoder.tracks_own_finished:
            ...
            if impute_finished:  # rectify the states for the finished.
                next_states = paddle.utils.layers_utils.map_structure(
        ...
        outputs = (
            paddle.utils.layers_utils.map_structure(
                lambda x: ArrayWrapper(x), step_outputs
            )
            if step_idx == 0
            else paddle.utils.layers_utils.map_structure(
    ...
    final_outputs = paddle.utils.layers_utils.map_structure(
...
def _dynamic_decode_declarative(
    ...
    **kwargs
    ...
    if is_test:
        ...
        inputs = paddle.utils.layers_utils.map_structure(
            lambda x: x, initial_inputs
        )
        states = paddle.utils.layers_utils.map_structure(
            lambda x: x, initial_states
        )
    else:
        # inputs and states of all steps must be saved for backward and training
        inputs_arrays = paddle.utils.layers_utils.map_structure(
            lambda x: paddle.tensor.array.array_write(x, step_idx),
            initial_inputs,
        )
        states_arrays = paddle.utils.layers_utils.map_structure(
            lambda x: paddle.tensor.array.array_write(x, step_idx),
            initial_states,
        )
    ...
    with while_op.block():
        ...
        outputs_arrays = paddle.utils.layers_utils.map_structure(
    ...
    if is_test:
        ...
        final_states = paddle.utils.layers_utils.map_structure(
            lambda array: paddle.tensor.array.array_read(array, step_idx),
            states_arrays,
        )
