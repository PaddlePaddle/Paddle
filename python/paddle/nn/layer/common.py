...
class EmbeddingBag(Layer):
    ...
    This Layer is used to calculate the sum, mean or max of the specified bag in the embeddings vector by : attr:'input' .
    It automatically constructs a 2D embedding matrix based on the input :attr:`num_embeddings` and :attr:`embedding_dim`.
    ...
    Parameters:
        num_embeddings (int): Just one element which indicate the size of the dictionary of embeddings.
        embedding_dim (int):  Just one element which indicate the size of each embedding vector respectively.
        mode(str): The string indicates calculation mode. mode can be set "sum", "mean" or "max".
    ...
    Examples:
        .. code-block:: python
            ...
            >>> embedding_sum  = paddle.nn.EmbeddingBag(10, 3, mode='sum')
            ...
            >>> adam = paddle.optimizer.Adam(parameters=[embedding_sum._embedding], learning_rate=0.01)
            >>> adam.clear_grad()
            >>> output = embedding_sum(input, weight=weight)
            >>> print(output)
            Tensor(shape=[2, 3], dtype=float32, place=Place(gpu:0), stop_gradient=False,
            [[ 0.40701598,  0.42892793, -0.72934294],
             [-0.33902842,  0.76669604,  0.27275681]])
            >>> output.backward()
            >>> adam.step()
    """
    ...
    def __init__(
        ...
        if self._num_embeddings <= 0:
            raise ValueError("num_embedding must be gather than 0")
        if self._embedding_dim <= 0:
            raise ValueError("embedding_dim must be gather than 0")
        ...
        self.weight.stop_gradient = True  # tmp
        if in_dynamic_mode() and padding_idx != -1:
            with paddle.no_grad():
                self.weight[padding_idx] = 0.0

    def forward(self, input, per_sample_weight=None):
        if per_sample_weight is None:
            per_sample_weight = paddle.ones_like(input, dtype=self.weight.dtype)
            per_sample_weight.stop_gradient = True  # tmp
        elif self._mode != 'sum':
            raise ValueError(
                "mode must be 'sum' when per_sample_weight are supplied to EmbeddingBag"
            )
