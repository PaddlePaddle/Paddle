...
from paddle.fluid.layers import fill_constant_batch_size_like
from paddle.fluid.layers import utils, control_flow
from paddle.fluid.layers.nn import elementwise_mul
...
class RNNCellBase(Layer):
    ...
    def get_initial_states(self,
        ...
        init_states = map_structure(
            lambda shape, dtype: fill_constant_batch_size_like(
                input=batch_ref,
                shape=shape.shape,
                dtype=dtype,
                value=init_value,
...
def _maybe_copy(state, new_state, step_mask):
    ...
    new_state = elementwise_mul(new_state, step_mask, axis=0) \
              + elementwise_mul(state, (1 - step_mask), axis=0)
    return new_state
...
def _rnn_dynamic_graph(cell,
    ...
    if sequence_length is not None:
        mask = F.sequence_mask(sequence_length,
                               maxlen=time_steps,
                               dtype=inputs.dtype)
    ...
    if is_reverse:
        ...
        mask = paddle.reverse(mask, axis=[0]) \
...
def _rnn_static_graph(cell,
    ...
    if is_reverse:
        final_outputs = map_structure(lambda x: paddle.reverse(x, axis=[0]),
                                      final_outputs)
...
def birnn(cell_fw,
    ...
    outputs = map_structure(lambda x, y: paddle.concat([x, y], -1), outputs_fw,
                            outputs_bw)
    ...
    def forward(self, inputs, initial_states=None, sequence_length=None):
        ...
        if initial_states is None:
            ...
            initial_states = tuple([
                fill_constant_batch_size_like(inputs, state_shape, dtype, 0,
                                              batch_index, 1)
