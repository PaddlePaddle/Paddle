...
class CrossEntropyLoss(Layer):
    ...
    By default, implements the cross entropy loss function with softmax. This function
    combines the calculation of the softmax operation and the cross entropy loss function
    to provide a more numerically stable computing.

    Calculate the cross entropy loss function without softmax when use_softmax=False.

    By default, will calculate the mean of the result, and you can also affect
...
class CosineEmbeddingLoss(Layer):
    ...
    Shape:
        ...
        input2 (Tensor): tensor with shape: [N, M], 'N' means batch size, which can be 0, 'M' means the length of input array.
                         Available dtypes are float32, float64.
        label (Tensor): tensor with shape: [N]. 'N' means the length of input array, The target labels values should be -1 or 1.
