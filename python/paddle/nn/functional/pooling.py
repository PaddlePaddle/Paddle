...
from ...utils import layers_utils
...
def _update_padding_nd(padding, num_dims, channel_last=False, ceil_mode=False):
    if isinstance(padding, str):
        ...
        if len(padding) == 2 + num_dims and _is_list_or_tuple(padding[0]):
            ...
            if layers_utils._is_symmetric_padding(padding, num_dims):
                padding = padding[0::2]
        # for padding like [pad_before, pad_after, pad_before, pad_after, ...]
        elif len(padding) == 2 * num_dims and isinstance(padding[0], int):
            padding_algorithm = "EXPLICIT"
            padding = layers_utils.convert_to_list(
                padding, 2 * num_dims, 'padding'
            )
            if layers_utils._is_symmetric_padding(padding, num_dims):
                padding = padding[0::2]
        # for padding like [pad_d1, pad_d2, ...]
            ...
            padding_algorithm = "EXPLICIT"
            padding = layers_utils.convert_to_list(padding, num_dims, 'padding')
        else:
...
def avg_pool1d(
    ...
    _check_input(x, 3)
    x = unsqueeze(x, [2])
    kernel_size = layers_utils.convert_to_list(kernel_size, 1, 'kernel_size')
    kernel_size = [1] + kernel_size
    if stride is None:
        stride = kernel_size
    else:
        stride = layers_utils.convert_to_list(stride, 1, 'pool_stride')
        stride = [1] + stride
...
def avg_pool2d(
    ...
    Examples:
        .. code-block:: python
            ...
            # out.shape [1, 3, 16, 16]
    """
    kernel_size = layers_utils.convert_to_list(kernel_size, 2, 'pool_size')
    if stride is None:
        stride = kernel_size
    else:
        stride = layers_utils.convert_to_list(stride, 2, 'pool_stride')
...
def avg_pool3d(
    ...
    """
    kernel_size = layers_utils.convert_to_list(kernel_size, 3, 'pool_size')
    if stride is None:
        stride = kernel_size
    else:
        stride = layers_utils.convert_to_list(stride, 3, 'pool_stride')
...
def max_pool1d(
    ...
    _check_input(x, 3)
    x = unsqueeze(x, [2])
    kernel_size = [1] + layers_utils.convert_to_list(
        kernel_size, 1, 'pool_size'
    )
    if stride is None:
        stride = kernel_size
    else:
        stride = [1] + layers_utils.convert_to_list(stride, 1, 'pool_stride')
...
def _unpool_output_size(x, kernel_size, stride, padding, output_size):
    ...
    if output_size is None:
        return default_size
    elif layers_utils._contain_var(output_size):
        if not in_dygraph_mode():
            has_static_var = True
            output_size = layers_utils._convert_to_tensor_list(output_size)
        else:
...
def max_unpool2d(
    ...
    if stride is None:
        ...
        stride = layers_utils.convert_to_list(stride, 2, 'pool_stride')
    padding = layers_utils.convert_to_list(padding, 2, 'padding')
...
def max_unpool3d(
    ...
    if stride is None:
        ...
        stride = layers_utils.convert_to_list(stride, 3, 'pool_stride')
    padding = layers_utils.convert_to_list(padding, 3, 'padding')
...
def adaptive_avg_pool1d(x, output_size, name=None):
    ...
    pool_type = 'avg'
    _check_input(x, 3)
    pool_size = [1] + layers_utils.convert_to_list(output_size, 1, 'pool_size')
...
def adaptive_avg_pool2d(x, output_size, data_format='NCHW', name=None):
    ...
    if isinstance(output_size, int):
        output_size = layers_utils.convert_to_list(
