...
def reshape(x, shape, name=None):
    ...
    Examples:
        .. code-block:: python
            ...
            out = paddle.nn.functional.reshape(x, [-1, 0, 3, 2])
            print(out)
            # the shape is [2,4,3,2].
            out = paddle.nn.functional.reshape(x, shape=[positive_four, 12])
            print(out)
            # the shape of out_2 is [4, 12].
            shape_tensor = paddle.to_tensor([8, 6], dtype=paddle.int32)
            out = paddle.nn.functional.reshape(x, shape=shape_tensor)
            print(out.shape)
            # the shape is [8, 6].
