...
def nll_loss(input,
    ...
    if in_dygraph_mode():
        if input_dims != 2 and input_dims != 4:
            input, _ = core.ops.reshape2(input, None, None, 'shape',
                                         [n, c, 1, -1])
            label, _ = core.ops.reshape2(label, None, None, 'shape', [n, 1, -1])
            out_shape = [n] + input_shape[2:]
        out, total_weight = core.ops.nll_loss(input, label, weight,
                                              'ignore_index', ignore_index,
                                              'reduction', reduction)
        if input_dims != 2 and input_dims != 4 and reduction == 'none':
            out, _ = core.ops.reshape2(out, None, None, 'shape', out_shape)
        return out
    ...
    if in_dygraph_mode():
        ...
        if weight is not None:
            ...
            weight_gather_reshape, _ = core.ops.reshape2(
                weight_gather, None, None, 'shape', input_shape)
