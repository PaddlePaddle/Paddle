...
def margin_softmax_with_cross_entropy(logits,
    ...
    Args:
    	logits: shape[N, local_num_classes], the output of the normalized X multiply the normalized W.
                The logits is shard_logits when using model parallel.
    	label: shape[N] or shape[N, 1], the groud truth label.
    	margin1: (1.0), m1 of margin loss.
    	margin2: (0.5), m2 of margin loss.
    	margin3: (0.0), m3 of margin loss.
    	scale: (64.0), s of margin loss.
        group: (None) nccl communication group.
        return_softmax: (False), whether return softmax probability.
        reduction : ('mean'), The candicates are ``'none'`` | ``'mean'`` | ``'sum'``.
    ...
    Return:
    	loss: if reduction==None, shape[N, 1], else shape[1], the cross entropy loss.
    	softmax: softmax probability. The softmax is shard_softmax when using model parallel.
