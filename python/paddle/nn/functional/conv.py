...
def conv1d(input,
    ...
    Output are in NCL or NLC format, where N is batch size, C is the number of
    channels, L is a length of signal sequence.
    
    Filter is in MCL format, where M is the number of output signal channels,
    C is the number of input signal channels, L is a length of signal sequence.

    If the groups is greater than 1,C will equal the number of input image channels divided by the groups.
    ...
    * :math:`X`: Input value, a tensor with NCL or NLC format.
    ...
    * :math:`Out`: Output value, the shape of :math:`Out` and :math:`X` may be different.
    ...
    Args:
        ...
        weight (Variable): The convolution kernel with shape [M, C/g, kL], where M is
            the number of output channels, g is the number of groups, kL is the filter's
            length. 
        bias (Variable, optional): The bias with shape [M,].
        padding (string|int|list|tuple): The padding size. It means the number of zero-paddings
            on both sides for each dimension. If `padding` is a string, either 'VALID' or
            'SAME' which is the padding algorithm. If padding size is a tuple or list,
            it could be in three forms: `[pad_length]` or
            `[pad_length_head, pad_length_bottom]`, and when 
            `data_format` is `"NCL"`, `padding` can be in the form `[[0,0], [0,0], 
            [pad_length_head, pad_length_bottom]`.
            when `data_format` is `"NHWC"`, `pool_padding` can be in the form
            `[[0,0], [pad_length_head, pad_length_bottom], [0,0]]`.
            Default: padding = 0.
        stride (int|tuple): The stride size. It means the stride in convolution. 
            Default: stride = 1.
        dilation (int|tuple): The dilation size. It means the spacing between the kernel
            points, Default: dilation = 1.
    ...
    Examples:
        .. code-block:: python
          ...
          with dg.guard(place):
              ...
              y_var = F.conv1d(x_var, w_var, act="relu")
              y_np = y_var.numpy()
          print(y_np.shape)

          # (2, 6, 6)
    """
    input = nn.unsqueeze(input, [2])
    weight = nn.unsqueeze(input, [2])
    if data_format == "NCL":
        data_format = "NCHW"
    ...
    out = nn.squeeze(out, [2], name=name)
    return out
