...
def hz_linesearch(state, func, k, x, p, f0=None, g0=None, a1=None, gtol=1e-8, max_iters=50, params=None):
    r"""
    ...
    Reference:
        ...
        W. W. Hager and H. Zhang, ACM Transactions on Mathematical Software 32: 113-137, 2006

    The Hager-Zhang method successfully addresses two issues. The first is with numerical
    stability, where the standard strong Wolfe (SW) condition fails to test the difference of
    function values in the neighborhood of a minimum. The Hager-Zhang method uses a relaxed
    form of SW and is able to check value difference more reliably. The second issue is with
    convergence speed. A special secant step is the key feature that helps achieve fast
    convergence.
    ...
    **Termination conditions**
        ...
        T1. the standard SW condition.
            ...
            phi(a) - phi(0) >= delta * a * phi'(0),  sufficient decrease condition, 
        ...
        T2. the approximate Wolf (AW) condition.
            ...
            sigma * phi'(0) <= phi'(a) <= (2*delta - 1) phi'(0),
                                                     where 0 < delta < .5 and delta <= sigma < 1
            phi(a) <= phi(0) + epsilon_k,            epsilon_k is error estimation at
                                                     iteration k

    **Termination strategy**

        At initial iterates which are fairly distant from the minimum, line search stops
        when a step size is found satisfying either T1 or T2. If at some iteration k,
        the following switching condition holds, then the iterates afterwards must use
        the AW condition to terminate the line search.

            |f(x_k+1) - f(x_k)| <= omiga * C_k,

        where C_k is a weighted average of all the function values on the located iterates
        so far.
    ...
    **Initial(k)**
        ...
        I0. if k = 0 and no starting point is given, then generate the initial step size c
            using the following rules.
            (a) If x_0 is not 0, c = psi_0 * infnorm(x_0) / infnorm(g_0) 
            (b) If f(x_0) is not 0, then c = psi_0 * |f(x_0)| / vnorm(g_0)^2
            (c) Otherwise, c = 1
        
        I1. If QuadStep is true, phi(psi_1 * a_k-1) <= phi(0), and the quadratic interpolant
            q() matches phi(0), phi'(0) and phi(psi * a_k-1) is strongly convex with a minimizer
            a_q, then c = a_k
    ...
    **Bracket(c)**
        ...
        This procedure is used to generate an opposite slope interval, kicking start
        a bracketing process that eventually narrows down to a point satfisfying T1 or T2.

        B0. c_-1 = c

        B1. If phi'(c) >= 0, then return [c_-1, c]

        B2. If phi'(c) < 0 and phi(c) > phi(0) + epsilon_k, then return Bisect([0, c])
    ...
    **Secant2(a, b)**
        ...
        This step uses a function called secant which generates a new point from 
        two existing points using the slopes at the two points.  
    ...
    **Update(a, b, c)**
        ...
        U3. If phi'(c) < 0 and phi(c) > phi(0) + epsilon_k, return Bisect([a, c])
    ...
    phi0, phiprime0 = f0, paddle.dot(g0, p)
    a1 = paddle.ones_like(p)

    # Two maintanence arrays are used to represent the linesearch
    # progress and the final status.
    # - ls_active. True indicates line search is in progress.
    #      False means line search has stopped.
    # - ls_status. False means either line search is invalid or
    #       has failed. True if line search succeeds or is still
    #       active.
    # 
    # ls_status is maintained across iterates while is_active is
    # initialized by copying from ls_status for each line search
    # procedure.

    # Select those from the batch with negative initial gradients 
