...
class Optimizer:
    ...
    def _create_master_weight(self, param):
        if param.name in self._master_weights:
            ...
            if in_pir_mode():
                with paddle.static.program_guard(self.helper.startup_program):
                    ...
                    startup_param = get_param_from_startup(
                        self.helper.startup_program, param.name
                    )
                    var = paddle.cast(startup_param, 'float32')
                    var.persistable = True
                    paddle._pir_ops.set_parameter(var, var_name)
                self.helper.main_program.set_parameters_from(
                    self.helper.startup_program
                )
                with paddle.static.program_guard(self.helper.main_program):
