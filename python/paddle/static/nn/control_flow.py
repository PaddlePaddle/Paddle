...
def case(pred_fn_pairs, default=None, name=None):
    ...
    Args:
        pred_fn_pairs(list|tuple): A list or tuple of (pred, fn) pairs. ``pred`` is a boolean Tensor whose numel should be 1 (scalar Tensor or shape [1]), ``fn`` is a callable. All callables return the same structure of Tensors.
...
def switch_case(branch_index, branch_fns, default=None, name=None):
    ...
    Args:
        branch_index(Tensor): A Tensor whose numel should be 1 (scalar Tensor or shape [1]) to specify which branch to execute. The data type is ``int32``, ``int64`` or ``uint8``.
...
def cond(pred, true_fn=None, false_fn=None, name=None, return_names=None):
    ...
    Args:
        pred(Tensor): A boolean tensor whose numel should be 1 (scalar Tensor
