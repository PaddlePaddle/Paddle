...
def while_loop(cond, body, loop_vars, is_test=False, name=None):
    ...
    if reduce(lambda a, b: a * b, pre_cond.shape, 1) != 1:
        raise TypeError(
            f"the shape of the variable returned by cond should be [1], but given shape as {list(pre_cond.shape)}."
        )
    ...
    with while_loop_block.block():
        ...
        try:
            ...
            raise ValueError(
                f"body in while_loop should return the same arity (length and structure) as loop_vars: {e}"
            )
