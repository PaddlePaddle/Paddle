...
def D(x):
    ...
    pool1 = fluid.layers.pool2d(input=conv1, pool_size=2, pool_type='avg')
    conv2 = fluid.layers.conv2d(
        ...
        param_attr='D_{0}'.format(c),
        bias_attr='D_{0}'.format(c))
    pool2 = fluid.layers.pool2d(input=conv2, pool_size=2, pool_type='avg')
    logits = fluid.layers.fc(input=pool2,
                             size=1,
                             act=None,
                             param_attr='D_{0}'.format(c),
                             bias_attr='D_{0}'.format(c))
