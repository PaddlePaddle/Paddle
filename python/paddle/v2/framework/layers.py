...
def batch_norm(input,
    ...
    def create_persistable_var(dtype, shape, initializer=None):
        ...
        var = startup_program.global_block().create_var(
            dtype=dtype, shape=shape, name=name, persistable=True)
        if initializer is not None:
            initializer(var, var.block)
        return main_program.global_block().create_var(
...
class BlockGuard(object):
    """
    BlockGuard used to create sub-block in main_program by using Python `with`
    keyword.
    """

    def __init__(self, main_program):
        if not isinstance(main_program, Program):
            raise TypeError("BlockGuard takes a main_program")
