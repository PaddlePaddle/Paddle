...
class CommonAccessor(Accessor):
    def __init__(self):
        super(CommonAccessor).__init__(self)
...
class Tensor:
    ...
    def _set(self, tensor_proto):
        tensor_proto.main_program_id = self.tensor_dict["main_program_id"]
        tensor_proto.startup_program_id = self.tensor_dict["startup_program_id"]
        tensor_proto.feed_var_name = self.tensor_dict["feed_var_name"]
        tensor_proto.fetch_var_name = self.tensor_dict["fetch_var_name"]
        tensor_proto.tensor_table_class = self.tensor_dict["tensor_table_class"]
...
class BarrierTable(Table):
    def __init__(self, context, send_ctx, idx):
    ...
    def _set(self, table_proto):
        table_proto.id = self.idx
        table_proto.type = "PS_OTHER_TABLE"
...
class TensorTable(Table):
    ...
    def _set(self, table_proto):
        table_proto.id = self.idx
        table_proto.type = "PS_OTHER_TABLE"
        table_proto.table_class = self.tensor_dict["tensor_table_class"]

        table_proto.accessor.accessor_class = "CommMergeAccessor"

        table_proto.common.table_name = self.tensor_dict["feed_var_name"]
        table_proto.common.trainer_num = get_trainers(self.role_maker)
        table_proto.common.trainer_num = trainer_num
...
class SparseTable(Table):
    ...
    def _set(self, table_proto):
        ...
        if ctx.is_tensor_table() or len(ctx.origin_varnames()) < 1 or (
                ctx.is_sparse() == False):
            return
        table_proto.id = ctx.table_id()
        table_proto.table_class = self.table_class
        table_proto.type = self.type
        ...
        check_embedding_dim(usr_table_proto.accessor, self.common.table_name,
                            ctx.program_id(), self.context)
...
class GeoSparseTable(SparseTable):
    ...
    def _set(self, table_proto):
        ...
        table_proto.accessor.accessor_class = 'CommMergeAccessor'
        table_proto.accessor.feature_dim = ctx.sections()[0]
        table_proto.accessor.embedding_dim = ctx.sections()[1]
...
class DenseTable(Table):
    ...
    def _set(self, table_proto):
        ...
        table_proto.type = "PS_DENSE_TABLE"
        table_proto.table_class = "CommonDenseTable"
        table_proto.shard_num = 256

        table_proto.accessor.accessor_class = 'CommMergeAccessor'
        table_proto.accessor.feature_dim = ctx.sections()[0]
        table_proto.accessor.embedding_dim = 1
...
class DownpourServer(Server):
    def __init__(self):
        super(DownpourServer).__init__(self)
        self.self.proto_desc = ps_pb2.ServerParameter().downpour_server_param
...
class DownpourWorker(Worker):
    def __init__(self):
        super(DownpourWorker).__init__(self)
        self.proto_desc = ps_pb2.WorkerParameter().downpour_worker_param
...
class fsClient:
    def __init__(self):
        self.proto_desc = ps_pb2.FsClientParameter()

    def _set(self, fs_client_proto):
        self.proto_desc.uri = fs_client_proto.uri
        self.proto_desc.user = fs_client_proto.user
        self.proto_desc.passwd = fs_client_proto.passwd
        self.proto_desc.hadoop_bin = fs_client_proto.hadoop_bin
...
class PsDescBuilder(object):
    ...
    def _get_tables(self):
        ...
        for idx, (name, ctx) in enumerate(self.send_ctx.items()):
            if ctx.is_sparse():
                ...
                tables.append(globals()['DenseTable'](self.context,
                                                      self.send_ctx))
        tensor_dict = {}  # TODO
        tables.append(globals()['TensorTable'](len(tables), tensor_dict,
                                               self.context['role_maker']))
        tables.append(globals()['BarrierTable'](self.context, self.send_ctx,
                                                len(tables)))
        return tables
    ...
    def build_worker_desc(self):
        _worker = DownpourWorker()
        for table in self.tables:
            table_proto = _worker.downpour_table_param.add()
            table._set(table_proto)

        return _worker.proto_desc.SerializeToString()

    def build_server_desc(self):
        _server = DownpourServer()
        for table in self.tables:
            table_proto = _server.downpour_table_param.add()
            table._set(table_proto)
        self.service._set(_server.service_param)
        self.fs_client._set(self.fs_client.proto_desc)

        return (_server.proto_desc.SerializeToString() + '\n' +
                self.fs_client.proto_desc.SerializeToString())
...
class TheOnePSRuntime(RuntimeBase):
    ...
    def _set_basic_info(self, context):
        ...
        self.ps_desc_builder = PsDescBuilder(self.is_sync)
