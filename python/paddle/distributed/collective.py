    ...
    if group is None:
        for pg_name, pg in _get_group_map_by_name().items():
            if pg.process_group is not None and pg_name not in shutdown_groups:
                ...
                print("shutdown_process_group", shutdown_groups)
    else:
        if group.process_group is not None and group.name not in shutdown_groups:
...
def restart_process_group(group: Group | None = None) -> None:
    ...
    if group is None:
        ...
        print("restart_process_group", shutdown_groups)
    else:
