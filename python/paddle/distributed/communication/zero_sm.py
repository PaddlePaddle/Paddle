...
def all_gather(
    ...
    tensor list; ``tensor`` may come from anywhere and is staged when needed.
    The list form gathers into one pooled buffer, as the native implementation
    does anyway. An empty list is filled with views over that buffer and costs
    no copy; a list that already holds tensors is written back element by
    element once the collective has landed.
    """
    ...
    if isinstance(out, (list, tuple)):
        if len(out) not in (0, group.nranks):
            # Checked before the allocation below, which votes with the whole
            # group: a rejected call must not cost a collective.
            raise ValueError(
