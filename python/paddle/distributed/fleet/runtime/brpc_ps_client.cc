...
DEFINE_int32(pslib_sparse_table_shard_num,
             1000,
             "sparse table shard for save & load");
DEFINE_int32(pslib_push_dense_merge_limit,
             12,
             "limit max push_dense local merge requests");
DEFINE_bool(pslib_scale_gradient_by_merge,
            false,
            "scale dense gradient when merged");
DEFINE_int32(pslib_async_push_dense_interval_ms,
             10,
             "async push_dense to server interval");
        ...
        for (size_t i = 0; i < shard_sorted_kvs->size(); ++i) {
          ...
          for (size_t kv_idx = 0; kv_idx < request_kvs.size(); ++kv_idx) {
            ...
            if (kv_pair->first == last_key) {
              memcpy(
                  (void *)kv_pair->second, (void *)last_value_data, value_size);
            } else {
...
int32_t BrpcPsClient::recv_and_save_table(const uint64_t table_id,
  ...
  auto status = pull_sparse((float **)save_vec.data(),
                            table_id,
                            save_key.data(),
                            save_key.size(),
                            true);
...
void BrpcPsClient::push_sparse_task_consume() {
  ...
  size_t request_call_num = _server_channels.size();
  ThreadPool<int> async_push_sparse_shard_threads(
  ...
  while (_running) {
    ...
    for (auto &push_sparse_task_itr : _push_sparse_task_queue_map) {
      ...
      if (_push_sparse_merge_count_map[table_id] >= merge_size ||
        ...
        for (int shard_idx = 0; shard_idx < request_call_num; ++shard_idx) {
          merge_status[shard_idx] = async_push_sparse_shard_threads.AddTask(
              std::bind(&BrpcPsClient::push_sparse_async_shard_push,
                        this,
                        task_list,
                        request_kv_num,
                        table_id,
      ...
      //未达到阈值 只做多路归并
        ...
        for (int shard_idx = 0; shard_idx < request_call_num; ++shard_idx) {
          merge_status[shard_idx] = async_push_sparse_shard_threads.AddTask(
              std::bind(&BrpcPsClient::push_sparse_async_shard_merge,
                        this,
                        task_list,
                        request_kv_num,
                        table_id,
...
int BrpcPsClient::push_sparse_async_shard_merge(
  ...
  for (int i = 1; i < task_list.size(); ++i) {
    ...
    for (int j = 0; j < kv_num; ++j) {
      ...
      sorted_kv_list.push_back({key_list[j], (float *)task_data_ptr});
    }
  }
      ...
      size_t total_send_data_size = total_send_data_vec.size();
      {
        ...
        while (!task_queue->empty() && merge_count < merge_size) {
          ...
          merge_status[merge_count] =
              async_merge_dense_threads.AddTask([closure,
                                                 accessor,
                                                 &total_send_data,
...
#pragma optimize("", on)
        ...
        DownpourBrpcClosure *closure = new DownpourBrpcClosure(
          ...
          push_dense_raw_gradient(
              task, total_send_data, total_send_data_size, closure);
          }
          if (scale_gradient && merge_count > 1) {
          Eigen::Map<Eigen::MatrixXf> mat(
              total_send_data, 1, total_send_data_size);
