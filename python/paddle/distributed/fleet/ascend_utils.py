...
import json
import paddle
from paddle.distributed.fleet.launch_utils import get_cluster, logger, get_host_name_ip
...
def get_cloud_cluster(rank_table_file_path, device_mode, devices_per_proc):
    """
    Args:
    rank_table_file_path: string, ascend npu rank file path
    device_mode: DeviceMode(Int)
    devices_per_proc:list
    """
    node_ips, device_count = _get_ascend_rankfile(rank_table_file_path)
