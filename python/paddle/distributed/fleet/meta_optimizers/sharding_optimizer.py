...
class ShardingOptimizer(MetaOptimizerBase):
    ...
    def minimize_impl(self,
        ...
        with open("main_after_gm_%d" % self.role_maker._worker_index(),
                  'w') as f:
            f.writelines(str(self._main_program))
    ...
    def _wait(self, ):
        ...
        assert current_endpoint == self.current_endpoint
                ...
                if int(op.attr('op_role')) == int(OpRole.Backward):
                    for input_name in op.desc.input_arg_names():
                        ...
                        if 'AMPOptimizer' in fleet._get_applied_meta_list():
                            ...
                            if (op.type != "cast" and op.type != "layer_norm"
                                ) or "@Fetch_0" not in input_name:
                                continue
                            else:
...
# find broadcast vars
    ...
    def _create_gm_cond(self, main_block):
        ...
        paddle.static.Print(
            current_step_var, message="in FWBW last conditional")
        return cond_var
