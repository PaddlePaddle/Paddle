...
class RawProgramOptimizer(MetaOptimizerBase):
    ...
    def _transpile_main_program(self, loss):
        ...
        if self.fuse_all_reduce_ops:
            self._calc_stream = True  # default use calc stream as commute stream
            ...
            if idx == start:
                ...
                for id_ in range(len(done_output_vars)):
                    x = self.vars['FusedOutput_{}'.format(done_output_vars[id_][
                        0].name)]
                    out = self.vars['FusedOutput_{}'.format(done_output_vars[
                        id_][0].name)]
                    ...
                    block._insert_op(
                        ...
                        if self._calc_stream else end + id_ + pos + 2,
                        type='c_allreduce_sum',
                        inputs={'X': x},
                        outputs={'Out': out},
                        attrs={
