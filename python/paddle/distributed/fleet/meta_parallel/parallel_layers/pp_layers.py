...
class PipelineLayer(Layer):
    def __init__(self,
        ...
        self.shared_comm = self._index_shared_layers()
    ...
    def _index_shared_layers(self):
        ...
        shared_keys = set(
            s.key for s in layers_desc if isinstance(s, SharedLayerDesc))
        for key in shared_keys:
            # Find the layers that the tied module appears in
            shared_layers = []
            for idx, layer in enumerate(layers_desc):
                if isinstance(layer, SharedLayerDesc) and layer.key == key:
                    shared_layers.append(idx)

            shared_stages = set(self.stage_owner(idx) for idx in shared_layers)
            ...
            for dp in range(self._dp_degree):
                for mp in range(self._mp_degree):
                    ...
                    # logger.info("info of shared layer: ", group)
                    ...
                    if self.global_rank in shared_ranks:
                        ...
                        if key in self.shared_layers:
                            ...
                            # if self.global_rank != shared_ranks[0]:
                            #     for p in self.shared_layers[key].parameters():
    ...
    def allreduce_shared_weight_gradients(self):
        ...
        for key, comm in self.shared_comm.items():
            ...
            # paddle.distributed.all_reduce(weight.grad, group=comm['group'])
    ...
    def _build_layer(self):
        ...
        for index, layer in enumerate(self._layers_desc[start:end]):
            ...
            if isinstance(layer, Layer):
                ...
                if layer.key not in self.shared_layers:
                    self.shared_layers[layer.key] = layer.build_layer()
                    self.shared_weight_attrs[
                        layer.key] = layer.shared_weight_attr

                if layer.forward_func is None:
                    self.run_function.append(self.shared_layers[layer.key])
