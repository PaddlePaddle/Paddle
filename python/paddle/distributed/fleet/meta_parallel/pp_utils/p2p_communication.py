...
class SendRecvMeta:
    ...
    def recv_meta(self, group):
        ...
        for _ in range(tensor_num):
            ...
            shapes.append(data[:shape_len])
            dtypes.append(number_2_dtype(data.pop(0)))
            stop_grads.append(bool(data.pop(0)))
    ...
    def send_meta(self, tensor, group):
        ...
        for t in tensors_to_send:
            assert isinstance(t, (paddle.Tensor, framework.core.eager.Tensor))
            data.extend(
                [
                    len(t.shape),
                    *t.shape,
