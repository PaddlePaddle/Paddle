...
class Fleet(object):
    ...
    def merge_embedding_shards_to_binary(self, input_path, output_path,
                                         **kwargs):
        """
        ...
        hadoop_fs_name = kwargs.get("hadoop_fs_name", None)
        hadoop_fs_ugi = kwargs.get("hadoop_fs_ugi", None)
        hadoop_home = kwargs.get("hadoop_home", "$HADOOP_HOME")
        upload_flag = False

        print("going to merge embedding shards to binary!")

        if os.path.isdir("output_embedding/"):
            shutil.rmtree("output_embedding/", True)

        # get files from hdfs
        if input_path.startswith("hdfs:") or input_path.startswith("afs:"):
            print("going to download embedding shards from hdfs!")
            configs = {
                "fs.default.name": hadoop_fs_name,
                "hadoop.job.ugi": hadoop_fs_ugi
            }
            client = HDFSClient(hadoop_home, configs)
            client.download(input_path, "output_embedding/")
            input_path = "output_embedding/"

        if not os.path.exists(input_path):
            raise ValueError \
                ("{} is not exist, pleast confirm your input_path.".format(input_path))

        if output_path.startswith("hdfs:") or output_path.startswith("afs:"):
            tmp_output_path = output_path
            output_path = "merged_embedding/"
            upload_flag = True
        ...
        # get var names from input path
        var_names = []
        for f in os.listdir(input_path):
            f_path = os.path.join(input_path, f)
        if os.path.isdir(f_path) and f.endswith(".shard"):
            var_names.append(f[:-6])

        # merge embedding shards for each var
        for varname in var_names:
            shard_txt = os.path.join(input_path, "{}.shard".format(varname))
            save_path = os.path.join(output_path, varname)

            if not os.path.exists(shard_txt):
                raise ValueError \
                    ("{} is not exist, pleast confirm your input_path.".format(shard_txt))

            if os.path.exists(save_path):
                raise ValueError("{} is exist, pleast delete.".format(
                    save_path))

            print("searching Param/Meta from {} and will merge to {}".format(
                shard_txt, save_path))

            shards, param_dim = self.get_distributed_shard(shard_txt, varname)
            sharding_merge = paddle.fluid.core.ShardingMerge()
            sharding_merge.merge(shards, [feasign_nums for i in shards],
                                 save_path, param_dim)

            print("save {} with {} shards to {}".format(varname,
                                                        len(shards), save_path))

        # upload files to hdfs
        if upload_flag:
            print("going to upload merged embedding shards to hdfs!")
            configs = {
                "fs.default.name": hadoop_fs_name,
                "hadoop.job.ugi": hadoop_fs_ugi
            }
            ...
            if not client.is_exist(tmp_output_path):
                client.mkdirs(tmp_output_path)
            client.upload(
                output_path, tmp_output_path, multi_processes=5, overwrite=True)

        print("merge embedding shards done!")

    def get_distributed_shard(self, shard_dirname, shard_varname):
        def get_meta(shard_meta):
            ...
            if varname is None or param_dim == -1:
                raise ValueError \
                    ("can not get right information from {}".format(shard_meta))
        ...
        if len(shards) == 0:
            raise ValueError(
                "can't find embedding shard txt files, please check")
        ...
        if meta_varname != shard_varname:
            raise ValueError("get_distributed_shard meta error, please check.")
