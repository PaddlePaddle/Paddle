...
class RNGStatesTracker:
    ...
    def add(self, name, seed):
        ...
        orig_rng_state_index = paddle.get_rng_state(use_index=True)
        # register a new state and set that state with the seed, store the indices into states_
        self.states_[name] = paddle.register_rng_state_as_index()
        paddle.seed(seed)
    ...
    def rng_state(self, name=MODEL_PARALLEL_RNG):
        ...
        try:
            ...
            self.states_[name] = paddle.get_rng_state(use_index=True)
            paddle.incubate.set_rng_state(orig_rng_state_index, use_index=True)
