...
class Fleet:
    ...
    def perf_test(self, round=50, test_comm=[], context={}, hcg=None):
        ...
        # test allgather perf
        def allgather_test(iteration, x, group):
            if group is None or group.nranks <= 1:
                return -1
            paddle.distributed.barrier()
            paddle.device.cuda.synchronize()
        ...
        def reduce_scatter_test(iteration, x, group):
            if group is None or group.nranks <= 1:
                return -1
            paddle.distributed.barrier()
            paddle.device.cuda.synchronize()
        ...
        while nbytes <= final_nbytes:
            ...
            if "allgather" in test_comm:
                ...
                ret = allgather_test(iteration=round, x=x, group=mp_group)
                logger.info(
            ...
            if "reduce_scatter" in test_comm:
                ...
                ret = reduce_scatter_test(iteration=round, x=x, group=mp_group)
                logger.info(
                    f"[ReduceScatterTest] nbytes {reduce_scatter_size}B test result: {ret} s/iter"
                )
                if (
                    reduce_scatter_thres_time > -1
                    and ret > reduce_scatter_thres_time
                ):
                    logger.warning(
