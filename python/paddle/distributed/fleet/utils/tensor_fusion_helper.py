...
class FusedCommBuffer:
    ...
    def _copy_grad_to_buffer(self, param, param_grad_is_none=False):
        ...
        if param_grad_is_none:
            assert (
                grad_var is None
            ), f"The current parameter[{param.name}] has gradient, its stop_grdient is {param.stop_gradient} , but we excepte it^s grad is None ,  Here are some examples: user marked_unsed_param incorrect ,like marked_unsed_param when backward unfinished,marked_unsed_param which param should have grad."
            return
