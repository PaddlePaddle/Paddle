...
def copy_parameters(block_, params):
    for param in params:
        new_p = Parameter(
            ...
            if param.type == core.VarDesc.VarType.LOD_TENSOR else None,
            ...
            name=param.name)
        assert param.is_distributed is False, "Try to sync Distribted Parameter: {}".format(param)
...
def insert_sync_op(
    ...
    if sync_mode == "broadcast":
        ...
        raise NotImplementedError('Sync mode of [{}] is NOT supported.'.format(sync_mode))
