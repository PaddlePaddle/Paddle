...
class DataParallelOptimizationPass(PassBase):
    ...
    def _add_dependencies(self, grad_groups):
        ...
        for idx, op in reversed(list(enumerate(block.ops))):
            ...
            if is_data_parallel_reduce_op(op):
                coalesce_var_name = op.out_arg_names[0]
            ...
            for out_name in op.out_arg_names:
        ...
        dep_var_pairs = list(
            set(prior_allreduce_deps.values() + post_allreduce_deps.values())
        )
        dep_var_pairs.sort(key=lambda x: x[0], reversed=True)
