...
class Engine:
    ...
    def _build(self, mode):
        if _non_static_mode():
            ...
            self.static_model = to_static(
                self.model, input_spec=self.inputs_spec)
            ...
            if isinstance(self._loss, paddle.nn.Layer):
                self.static_loss = to_static(
                    self._loss.forward,
                    input_spec=outputs_spec + self.labels_spec)
                loss_main_prog = self.static_loss.main_program
            elif callable(self._loss):
                self.static_loss = to_static(
                    self._loss, input_spec=outputs_spec + self.labels_spec)
                loss_main_prog = self.static_loss.main_program
    ...
    def _initialize(self, mode):
        ...
        if self._dygraph_mode:
            ...
            for param in self.concrete_program.parameters:
                ...
                sliced_param = Converter.slice_with_dist_attr(param.numpy(),
                                                              dist_attr)
                target_tensor = paddle.to_tensor(
                    sliced_param, place=self._place)
                param_tensor._share_data_with(target_tensor.value().get_tensor(
                ))
            paddle.enable_static()
