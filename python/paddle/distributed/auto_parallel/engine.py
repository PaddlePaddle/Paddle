...
class Engine:
    ...
    def __init__(self,
        ...
        if loss and not isinstance(
                loss, (paddle.nn.Layer, Variable)) and not callable(loss):
            raise TypeError(
    ...
    def _build(self, mode):
            ...
            if not self._skip_build:
                with static.program_guard(serial_main_prog, serial_startup_prog), \
                    ...
                    if mode != "predict" and self._loss:
                        assert isinstance(self._loss, paddle.nn.Layer) or callable(self._loss), \
        ...
        self._dist_contexts[mode] = DistributedContext(
            serial_main_prog, serial_startup_prog, self._optimizer,
            self._losses, feed_vars, fetch_vars, self._cluster, self._strategy)
