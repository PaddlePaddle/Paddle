...
class Engine:
    ...
    def _initialize(self, mode):
        if self._nranks > 1:
            ...
            all_process_groups = sorted(list(all_process_groups), key=lambda x: (-len(x.ranks), x.ranks))
            has_recv_by_socket = []
            ...
            print("The cur_rank_ip is {}, cur_rank_port is {}".format(cur_rank_ip, cur_rank_port))
            ...
            for process_group in all_process_groups:
                if self._cur_rank not in process_group.ranks:
                    continue
                print("start init process group: ", process_group.ranks)
                if len(process_group.ranks) == 2:
                    ...
                    if is_send:
                        ...
                        recv_rank_ip, recv_rank_port = genv.trainer_endpoints[recv_rank].split(":")
                        ...
                        print("has send {} to {}:{}.".format(self._cur_rank, recv_rank_ip, connect_port))
                        rank = client_socket.recv(buff_size).decode('utf-8')
                        print("engine.py rank: ", rank)
                        rank = int(rank)
                        if rank != recv_rank:
                            raise ValueError("Please check comm pair, the recv rank should be {} but got {}.".format(recv_rank, rank))
                        else:
                            print("engine.py It is able instantiate {} as sender now.".format(process_group.ranks))
                        client_socket.close()
                    else:
                        # [0, 7]
                        ...
                        while True:
                            if send_rank not in has_recv_by_socket:
                                ...
                                print("has recv {} from {}.".format(rank, recv_addr))
                            else:
                                client_sockets[send_rank].send(str(self._cur_rank).encode("utf-8"))
                                client_sockets[send_rank].close()
                                print("engine.py It is able instantiate {} as recver now.".format(process_group.ranks))
                                break
