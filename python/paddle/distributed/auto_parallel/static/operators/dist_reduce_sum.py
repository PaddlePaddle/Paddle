...
from ..utils import infer_with_spmd
from .common import (
...
class DistributedReduceSumImpl0(DistributedOperatorImpl):
    ...
    def is_input_compatible(self, dist_op):
        ...
        x_var = dist_op.serial_op.block._var_recursive(x_name)
        x_dims_mapping = op_dist_attr.get_input_dims_mapping(x_name)
        x_ndim = len(x_var.shape)
        if isinstance(axes, int):
            if axes < 0:
                axes += x_ndim
            return x_dims_mapping[axes] == -1
        else:
            if axes == [] and op_desc.attr("reduce_all"):
                axes = list(range(x_ndim))
            norm_axes = [axis if axis >= 0 else axis + x_ndim for axis in axes]
            return all(x_dims_mapping[axis] == -1 for axis in norm_axes)
    ...
    def is_output_compatible(self, dist_op):
        ...
        out_ndim = len(out_var.shape)
        if keep_dim:
            if isinstance(axes, int):
                if axes < 0:
                    axes += out_ndim
                return out_dims_mapping[axes] == -1
            else:
                if axes == [] and op_desc.attr("reduce_all"):
                    axes = list(range(out_ndim))
                norm_axes = [
                    axis if axis >= 0 else axis + out_ndim for axis in axes
                ]
                return all(out_dims_mapping[axis] == -1 for axis in norm_axes)
        return True
    ...
    def update_dims_mapping(self, dist_op):
        ...
        if os.getenv("ENABLE_SPMD_RULE") == 'true':
            print("################ reduce_sum spmd ####################")
            x_name = op_desc.input('X')[0]
            out_name = op_desc.output('Out')[0]
            return infer_with_spmd(
                dist_op, [x_name], [out_name], ["dim", "keep_dim"], "reduction"
            )
        else:
...
register_distributed_operator_impl(
    "reduce_sum", DistributedReduceSumImpl0("reduce_sum")
)
