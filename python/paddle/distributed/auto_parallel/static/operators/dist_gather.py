...
class DistributedGather(DistributedOperatorImplContainer):
    ...
    def update_dims_mapping(dist_op):
        ...
        # step2: infer spmd
        rule = get_phi_spmd_rule("gather")
        # tensor order following order in PHI definition
        fw_results = rule.infer_forward(input_specs[0], input_specs[1], axis)
        bw_results = rule.infer_backward(
            input_specs[0], input_specs[1], output_spec, axis
        )
...
register_distributed_operator_impl_container(DistributedGather("gather"))
