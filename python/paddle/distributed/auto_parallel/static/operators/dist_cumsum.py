...
class DistributedCumsum(DistributedOperatorImplContainer):
    ...
    def update_dims_mapping(dist_op):
        ...
        # step2: infer spmd
        rule = get_phi_spmd_rule("cumsum")
        # tensor order following order in PHI definition
        fw_results = rule.infer_forward(
            input_specs[0], axis, False, False, False
        )
        bw_results = rule.infer_backward(
            input_specs[0], output_spec, axis, False, False, False
        )
...
register_distributed_operator_impl_container(DistributedCumsum("cumsum"))
