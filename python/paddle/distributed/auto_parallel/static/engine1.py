...
class Engine:
    ...
    def _validate_batch_size(self, batch_size):
        ...
        if auto_utils.use_new_executor():
            ...
            ), f"batch_size [{batch_size!s}] is not divisible by dp_world_size [{self._dp_world_sizes[0]!s}]"
    ...
    def _set_state_dict(self, mode, strict, state_dict, dist_attr):
        ...
        for name, param in program.state_dict().items():
            ...
            if param_array.dtype != state_dict[name].dtype:
                self._logger.info(
                    f"cast {name}'s dtype from '{state_dict[name].dtype!s}' to '{param_array.dtype!s}'"
                )
    ...
    def main_program(self):
        if self._in_pir_mode:
            print(self._pir_main_progs, self._mode)
