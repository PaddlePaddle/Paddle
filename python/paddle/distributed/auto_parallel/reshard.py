...
def _compute_partition_index(process, complete_shape, dims_mapping, process_shape, process_group):
    """Compute the partition index in complete tensor."""
    partition_shape = _compute_partition_shape(complete_shape, dims_mapping, process_shape)
    process_index = _compute_process_index(process, process_group, process_shape)
    partition_index = []
    ...
    for i in range(len(complete_shape)):
        if dims_mapping[i] == -1:
            ...
            partition_index.append([process_index[dims_mapping[i]]*partition_shape[i],
...
def _compute_concat_info(partition_index_x, partition_index_y):
    ...
    for idx, item in enumerate(partition_index_x):
        if item != partition_index_y[idx]:
            differ_count += 1
            if item[1] == partition_index_y[idx][0] and item[0] < partition_index_y[idx][1]:
                concat_axis = idx
                new_partition.append([item[0], partition_index_y[idx][1]])
            elif item[0] == partition_index_y[idx][1] and item[1] > partition_index_y[idx][0]:
                first_order = 1
                concat_axis = idx
...
def _concat_partitions(partition_index_list, partition_index):
    ...
    if not partition_index_list:
        ...
        while i < len(partition_index_list):
            concat_axis, _, new_partition = _compute_concat_info(partition_index_list[i], partition_index)
            if concat_axis != -1:
                has_concat = True
...
def _is_overlapped(shape_x, shape_y):
    ...
    overlapped = False
    if (shape_y[0] <= shape_x[0] < shape_y[1]) or (shape_x[0] <= shape_y[0] < shape_x[1]):
        overlapped = True
    return overlapped
...
def _need_reshard(tensor_dist_attr, op_dist_attr):
    ...
    op_input_dims_mapping = op_dist_attr.get_input_dims_mapping(tensor_dist_attr.get_owner_tensor().name)
    op_process_mesh = op_dist_attr.get_process_mesh()
    if all(map(lambda x: x is not None,
               [tensor_dims_mapping, tensor_process_mesh, op_input_dims_mapping, op_process_mesh])):
...
def find_op_desc_seq(source_tensor, tensor_dist_attr, op_dist_attr):
    ...
    target_dims_mapping = op_dist_attr.get_input_dims_mapping(tensor_dist_attr.get_owner_tensor().name)
    target_process_group = target_process_mesh.process_group
    target_process_shape = target_process_mesh.topology

    complete_shape = _compute_complete_shape(source_tensor.shape, source_process_shape, source_dims_mapping)
    op_desc_seq = {}

    # TODO: if the target process group has the same process with source process group
    if set(target_process_group).intersection(set(source_process_group)) and set(target_process_group).difference(
    ...
    # in the different process group, it will use send, recv, concat and slice op
        ...
        for source_process in source_process_group:
            ...
            if not partition_process_mapping_list:
                partition_process_mapping_list.append([source_partition_index, [source_process], [False]])
            else:
                partition_list = list([item[0] for item in partition_process_mapping_list])
                process_list = list([item[1] for item in partition_process_mapping_list])
                has_used = list([item[2] for item in partition_process_mapping_list])
        ...
        for target_process in target_process_group:
            has_sent = []
            target_partition_index = _compute_partition_index(target_process, complete_shape, target_dims_mapping,
            ...
            for source_process in source_process_group:
                ...
                if all(_ for _ in list(map(_is_overlapped, source_partition_index, target_partition_index))) \
                    ...
                    idx = list([item[0] for item in partition_process_mapping_list]).index(source_partition_index)
                    has_used = list([item[2] for item in partition_process_mapping_list])[idx]
                    process_list = list([item[1] for item in partition_process_mapping_list])[idx]
                    i = 0
                    ...
                    send_op_desc = SendOpDesc(source_partition_index, target_process)
                    recv_op_desc = RecvOpDesc(source_partition_index, to_send_process)
                    ...
                    _concat_partitions(partition_index_list, source_partition_index)

            # append concat op desc
            op_desc_seq[target_process].append(ConcatOpDesc(all_partition_index_list))
            ...
            op_desc_seq[target_process].append(SliceOpDesc(slice_starts, slice_ends, slices_axes))
    ...
    # in the same process group, it will use allgahther and slice op
        ...
        for source_process in source_process_group:
            ...
            if source_partition_index not in partition_index_list:
                ...
                process_index.append([[source_process, ], source_partition_index])
            else:
                process_index[partition_index_list.index(source_partition_index)][0].append(source_process)
        ...
        for i in range(len(process_index[0][0])):
            ...
            for process in group:
                ...
                slice_ends = []
                slices_axes = []
                target_partition_index = _compute_partition_index(process, complete_shape, target_dims_mapping,
                                                                  target_process_shape, target_process_group)
...
def _insert_slice_op(block, idx, tensor, starts, ends, axes, new_var_name):
    ...
    attrs = {'axes': axes, "starts": starts, "ends": ends, "infer_flags": infer_flags}
...
def _insert_allgather_op(block, idx, tensor, ranks):
    ...
    def _insert_fill_constant_op(block, idx):
        ...
        utils.get_shape_tensor_inputs(inputs=inputs, attrs=attrs, shape=[0], op_type='fill_constant')
        block._insert_op(
            idx,
            type='fill_constant',
            inputs=inputs,
...
def _concat_partitions_with_op(partition_tensor_list, tensor, partition_index, block, idx):
    ...
    if not partition_tensor_list:
        ...
        while i < len(partition_tensor_list):
            concat_axis, first_order, new_partition = _compute_concat_info(partition_tensor_list[i][1], partition_index)
            if concat_axis != -1:
                has_concat = True
                ...
                idx[0] += 1
                _concat_partitions_with_op(partition_tensor_list, _, new_partition, block, idx)
                break
            i += 1
        if not has_concat:
            partition_tensor_list.append((tensor, partition_index))
...
def parse_op_desc(program, rank_id, op_desc_seq, var_name, reshard_op, dist_context):
    ...
    assert var_name in block.vars.keys(), "The {} cannot be found in the {} program.".format(var_name, rank_id)
    ...
    for index, op in list(enumerate(block.ops)):
        if op.desc.id == reshard_op.desc.id:
            idx = index
            break
    assert idx is not None, "The op for reshard cannot be found in the rank {} program.".format(rank_id)
    ...
    for op_desc in op_desc_list:
        if isinstance(op_desc, AllGatherOpDesc): # noqa: F401
            ...
            if op_desc.src not in HAS_RECV[var_name].keys():
                ...
                recv_tensor = block.create_var(name=unique_name.generate(var_name + "@recv"), shape=shape,
            ...
            for index, tensor in enumerate(tensor_list):
                _concat_partitions_with_op(partition_tensor_list, tensor, partition_index_list[index], block, idx_list)
            idx = idx_list[0]

        elif isinstance(op_desc, SliceOpDesc):
            assert len(partition_tensor_list) == 1 or not partition_tensor_list
            to_slice_tensor = partition_tensor_list[0][0] if len(partition_tensor_list) == 1 else source_tensor
            new_name = unique_name.generate(var_name + "@RESHARD")
            target_tensor = _insert_slice_op(block, idx, to_slice_tensor, starts=op_desc.starts,
                                             ends=op_desc.ends, axes=op_desc.axes, new_var_name=new_name)

            tensor_attr = TensorDistributedAttribute(target_tensor, dist_context)
            process_mesh = dist_context.get_op_distributed_attr_for_program(matched_op).get_process_mesh()
            dims_mapping = dist_context.get_op_distributed_attr_for_program(matched_op).get_input_dims_mapping(var_name)
            tensor_attr.set_dims_mapping(dims_mapping)
            tensor_attr.set_process_mesh(process_mesh)
            dist_context.set_tensor_distributed_attr_for_program(target_tensor, tensor_attr)
            ...
            for op in block.ops:
                for name in op.input_arg_names:
                    ...
                    if name == var_name and op_dist_attr is not None:
                        op_process_mesh = op_dist_attr.get_process_mesh()
                        op_input_dims_mapping = op_dist_attr.get_input_dims_mapping(var_name)
                        if op_process_mesh._id == process_mesh._id and op_input_dims_mapping == dims_mapping:
                            ...
                            op_dist_attr.set_input_dims_mapping(target_tensor.name, dims_mapping)
...
def _remove_no_need_ops(auto_parallel_main_prog, dist_context, rank_id):
    ...
    not_remove_op_ref = ["create_py_reader", "create_double_buffer_reader", "read"]
    remove_op_idx = []
    ...
    for idx, op in enumerate(ops):
        ...
        if op.type == "c_sync_comm_stream":
            ...
            for var_name in op.input_arg_names:
                process_mesh = dist_context.get_tensor_distributed_attr_for_program(vars[var_name]).get_process_mesh()
...
def reshard(auto_parallel_main_prog, auto_parallel_startup_prog, rank_id):
    ...
    while idx < len(block.ops):
        ...
        if op_dist_attr is not None:
            ...
            for var_name in op.input_arg_names:
                ...
                tensor_dist_attr = dist_context.get_tensor_distributed_attr_for_program(var)
                if tensor_dist_attr is not None and _need_reshard(tensor_dist_attr, op_dist_attr):
                    reshard_op_desc = find_op_desc_seq(var, tensor_dist_attr, op_dist_attr)
                    parse_op_desc(auto_parallel_main_prog, rank_id, reshard_op_desc, var_name, op, dist_context)
    ...
    remove_no_need_in_startup(auto_parallel_main_prog, auto_parallel_startup_prog)
