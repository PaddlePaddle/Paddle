...
class CostEstimator:
    ...
    def _estimate_core(self, dist_context, resharder, block):
        # print("estimate_cost.py print program to estimate cost*****")
        # print_program_with_dist_attr(block, dist_context)
        ...
        for i in range(loop_count):
            for idx, op in enumerate(ops):
                ...
                for var_name in op.output_arg_names:
                    output_dims_mapping = op_dist_attr.get_output_dims_mapping(var_name)
                    dist_op_cost_key += self._convert_pm_and_dm_to_str(op_dist_attr.process_mesh, output_dims_mapping)
                ...
                for var_name in op.input_arg_names:
                    ...
                    op_key = self._convert_pm_and_dm_to_str(op_dist_attr.process_mesh, op_dist_attr.get_input_dims_mapping(var_name))
                    tensor_key = self._convert_pm_and_dm_to_str(tensor_dist_attr.process_mesh, tensor_dist_attr.dims_mapping)
                    ...
                    if key not in cached_reshard_cost[idx]:
                        ...
                        tmp_end = time.time()
                        # print("estimate_cost.py get_cost********", tmp_end-tmp_start, key, idx)
                    else:
                        tmp_start = time.time()
                        reshard_cost = cached_reshard_cost[idx][key]
                        tmp_end = time.time()
                        # print("estimate_cost.py get_cost_cached********", tmp_end-tmp_start)
                                ...
                                for rank in group_ranks:
                                    ...
                                    if max_time is None:
                                        ...
                                        if max_time < rank_cost.time:
                                            max_time = rank_cost.time
                                # print("estimate_cost.py max_time****", max_time, group_ranks)
                                
                                # print("estimate_cost.py comm_cost", cost, cost.op_desc)
                                for rank in group_ranks:
                                    self.local_cost(
                                        rank).time = max_time + cost.time
                                    # print("estimate_cost.py rank, local_cost", rank, self.local_cost(rank).time, cost.time)
                ...
                if dist_op_cost_key not in cached_dist_op_cost[idx]:
                    ...
                    tmp_end = time.time()
                    # print("estimate_cost.py dist_op_cost********", tmp_end-tmp_start, dist_op_cost_key, idx)
                else:
                    tmp_start = time.time()
                    dist_op_cost = cached_dist_op_cost[idx][dist_op_cost_key]
                    tmp_end = time.time()
                    # print("estimate_cost.py cached_dist_op_cost********", tmp_end-tmp_start)
                ...
                # print("estimate_cost.py global cost at op****", self.global_cost.time, op)
    ...
    def ordered_ops(self):
        ...
        tmp_end = time.time()
        print("estimate_cost.py memory cost step1 need {}s".format(tmp_end-tmp_start))
    ...
    def _estimate_max_memory_by_dist_op(self, dist_context):
        ...
        for op_id, op in self.ordered_ops:
            ...
            for var_name in op.input_arg_names:
                ...
                if var_name not in var_memory:
                    var_memory[var_name] = {}
                key = self._convert_pm_and_dm_to_str(process_mesh, input_dims_mapping)
                ...
                if "memory" not in var_info[var_name][key]:
                    if "memory" not in var_memory[var_name][key]:
                        ...
                        dtype = var.dtype
                        sizes = DistributedTensor.get_local_sizes(global_sizes, input_dims_mapping, process_mesh.topology, process_mesh.processes, False)
                        var_memory[var_name][key]["memory"] = self._calculate_bytes(sizes, dtype)
                        # tmp_end = time.time()
                        # print("estimate_cost.py calc memory need {}s".format(tmp_end-tmp_start))
                    # tmp_start = time.time()
                    var_info[var_name][key]["memory"] = var_memory[var_name][key]["memory"]
                    tmp_end = time.time()
                    # print("estimate_cost.py calc memory cached need {}s".format(tmp_end-tmp_start))
            ...
            for var_name in op.output_arg_names:
                ...
                if var_name not in var_memory:
                    var_memory[var_name] = {}
                key = self._convert_pm_and_dm_to_str(process_mesh, output_dims_mapping)
                if key not in var_info[var_name]:
                    var_info[var_name][key] = {}
                if key not in var_memory[var_name]:
                    var_memory[var_name][key] = {}
                if "memory" not in var_info[var_name][key]:
                    if "memory" not in var_memory[var_name][key]:
                        ...
                        global_sizes = var.shape
                        dtype = var.dtype
                        sizes = DistributedTensor.get_local_sizes(global_sizes, output_dims_mapping, process_mesh.topology, process_mesh.processes, False)
                        var_memory[var_name][key]["memory"] = self._calculate_bytes(sizes, dtype)
                    var_info[var_name][key]["memory"] = var_memory[var_name][key]["memory"]
                if "position" not in var_info[var_name][key]:
                    var_info[var_name][key]["position"] = []
        ...
        tmp_end = time.time()
        print("estimate_cost.py memory cost step2 need {}s".format(tmp_end-tmp_start))
        ...
        tmp_end = time.time()
        print("estimate_cost.py memory cost step3 need {}s".format(tmp_end-tmp_start))
        # Calculate the max memory in all ranks
        print("estimate_cost.py max_memories********", max_memories)
        max_memory = max(max_memories.values())
        end_time = time.time()
        print("estimate_cost.py estimate max memory need {}".format(end_time-start_time))
        return max_memory
    ...
    def estimate(self, dist_context, resharder=None):
        ...
        end_time = time.time()
        print("estimate_cost.py estimate cost need {}".format(end_time-start_time))
