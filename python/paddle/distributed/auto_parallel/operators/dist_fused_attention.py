...
class DistributedFusedAttentionImpl(DistributedOperatorImpl):
    ...
    def is_auto_compatible(self, dist_op):
        ...
        op_desc = dist_op.serial_op.desc
        op_dist_attr = dist_op.dist_attr
        in_names = op_desc.input('X') + op_desc.input('CacheKV')
        out_names = op_desc.output('Y') + op_desc.output('CacheKVOut')
        for i, in_name in enumerate(in_names):
            out_name = out_names[i]
            in_dims_mapping = op_dist_attr.get_input_dims_mapping(in_name)
            out_dims_mapping = op_dist_attr.get_output_dims_mapping(out_name)
            if in_dims_mapping != out_dims_mapping:
                return False
        return True
        ...
        for i, in_name in enumerate(in_names):
            ...
            for i in range(len(in_dims_mapping)):
                dim_changed = compute_compatible_and_update_dim_mapping(
                    [in_dims_mapping, out_dims_mapping], [i, i]
                )
                if dim_changed:
                    changed = True
        return changed
