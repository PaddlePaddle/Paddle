...
class ShardingOptimizerStage1(Optimizer):
    ...
    def apply_gradients(self, params_grads):
            ...
            for slice_param, param_info in slice_param_dict.items():
                ...
                slice_grad.get_defining_op().dist_attr = (
                    paddle.base.libpaddle.pir.create_op_dist_attribute(
                        slice_grad.process_mesh,
                        [shard_fused_grad.dist_attr()],
                        [slice_grad_dist_attr],
                    )
                )
                slice_grad.set_type(
        ...
        for (
            ...
            # paddle.assign(allgather_value, fused_param)
            allgather_value.persistable = True
            paddle._pir_ops.set_persistable_value(
                allgather_value, fused_param.name
            )
            ...
            for index, param in enumerate(group_param_list):
                ...
                if param_begin < param_end:
                    ...
                    with paddle.static.program_guard(main_program):
                        ...
                        slice_param.get_defining_op().dist_attr = (
                            paddle.base.libpaddle.pir.create_op_dist_attribute(
                                init_slice_param.process_mesh,
                                [],
                                [init_slice_param.dist_attr()],
                            )
                        )
