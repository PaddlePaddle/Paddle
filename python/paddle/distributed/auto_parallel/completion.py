        ...
        if grad_op.desc.id() in dist_op_context.gradopidx2opidx:
            ...
            for input_name in grad_op.input_arg_names:
                ...
                if "@GRAD" in input_name:
                    forward_name = _get_forward_varname_from_grad_varname(input_name)
                    ref_dims_mapping = forward_op_dist_attr.get_output_dims_mapping(forward_name)
                else:
            ...
            for output_name in grad_op.desc.output_names():
                ...
                if len(grad_op.desc.output(output_name)) == 1:
                    ...
                    forward_name = _get_forward_varname_from_grad_varname(output_var.name)
                    ref_dims_mapping = forward_op_dist_attr.get_input_dims_mapping(forward_name)
