...
class MergedNode(NodeBase):
    def __init__(self, node_type,id = None, base_node_list = None):
        assert node_type == NodeType.MERGED
...
class CommNode(NodeBase):
    def __init__(self, node, node_type, id = None, comm_node_list= None):
        assert node_type == NodeType.COMMUNICATION
    ...
    def set_cost(self, cluster = None):
        ...
        if 'allreduce' in self.comm_type:
            self.cost = comm_volumn / (BANDWIDTH * num_ranks / (2 * (num_ranks - 1) ) )
...
class CompNode(NodeBase):
    def __init__(self, node, node_type,id = None):
        assert node_type == NodeType.COMPUTATION
...
class CostModelContext():
    ...
    def _parse_sub_program(self, program, nodes, graph, cost_data):
        assert len(program.blocks) == 1, "Program more than 1 block not supported."
        ...
        for op in block.ops:
            ...
            if op.type.startswith('c_') or op.type.startswith('send') or op.type.startswith('recv'):
        ...
        for node_id, node in nodes.items():
            if node.type == NodeType.VARIABLE and node.node.persistable:
                ...
                for pred_id in graph[node_id][PRED]:
                    pred = nodes[pred_id]
                    if pred.type == NodeType.COMPUTATION and (pred_id in graph[node_id][SUCC]): 
                        ...
                        new_var = VarNode(node.node, NodeType.VARIABLE, new_var_id,shared_node_id=node_id)
    ...
    def parse_program(self, distributed_program):
        ...
        for sub_idx, sub_prog in enumerate(distributed_program):
            ...
            self._parse_sub_program(sub_prog, 
                                    self.nodes[sub_idx],
    ...
    def merge_comm(self):
        for sub_idx in range(len(self.distributed_program)):
            ...
            end_idx =   (dim[1]+1) * self.process_mesh.topology[2] + \
                        dim[0] * self.process_mesh.topology[2] * self.process_mesh.topology[1]
            mp_group = list(map(lambda x: self.process_mesh.process_group[x],range(start_idx,end_idx)))
        
            start_idx = dim[2] + dim[0] * self.process_mesh.topology[2] * self.process_mesh.topology[1]
            end_idx =  start_idx + self.process_mesh.topology[2] * self.process_mesh.topology[1]
            dp_group = list(map(lambda x: self.process_mesh.process_group[x],
            ...
            if dim[0] != 0: # first stage
                pp_pred_peer = self.process_mesh.process_group[rank - self.process_mesh.topology[2] * self.process_mesh.topology[1]]

            if dim[0] != ( self.process_mesh.topology[0] - 1 ): # last stage
                pp_succ_peer = self.process_mesh.process_group[rank + self.process_mesh.topology[2] * self.process_mesh.topology[1]]
    ...
    def _merge_node(self, to_merge_node_list, merge_type = 'linear', nodes=None):
        ...
        merged_node = MergedNode(NodeType.MERGED,
                                id = merged_node_id,
    ...
    def merge_linear(self):
        cnt = 0
        for sub_idx in range(len(self.distributed_program)):
            cnt += self._merge_linear(self.nodes[sub_idx], self.rt_graph[sub_idx])
        return cnt

    def merge_branch(self):
        cnt = 0
        for sub_idx in range(len(self.distributed_program)):
            cnt += self._merge_branch(self.nodes[sub_idx], self.rt_graph[sub_idx])
        return cnt
    ...
    def _merge_linear(self, nodes, rt_graph):
        ...
        for node_id in rt_nodes_id:
            if node_id not in rt_graph.keys(): # the node has already been deleted because of merging
                continue
            ...
            if ind == 1: # only depend on one node
                ...
                node = nodes[node_id]
                pred = nodes[pred_id]
                merged_node_id, merged_node = self._merge_node([node, pred], merge_type='linear',
                                                                            nodes = nodes)
    ...
    def _merge_branch(self, nodes, rt_graph):
        ...
        for node_id in rt_nodes_id:
            ...
            if outd > 1: # branch out
                ...
                to_merge = True
                # print(edges)
                if len(edges[SUCC]) < 1 or len(rt_graph[edges[SUCC][0]][SUCC]) < 1 :
                    continue
                end_node_id = rt_graph[edges[SUCC][0]][SUCC][0]
                for i in succ_nodes_id:
                    if len(rt_graph[i][SUCC]) > 1 or \
                        rt_graph[i][SUCC][0] != end_node_id:
                        to_merge = False # if branches has different end node, we don' merge them
                        break
                if to_merge:
                    to_merge_node_list = [nodes[i] for i in succ_nodes_id]
                    merged_node_id, merged_node = self._merge_node(to_merge_node_list,
                                                                  merge_type='branch',
                                                                  nodes = nodes)
                    nodes[merged_node_id] = merged_node
                    rt_graph[merged_node_id] = [[],[]]
    ...
    def get_mem(self):
        ...
        top_list = []
        for sub_idx in range(len(self.distributed_program)):
            static_mem, cur_mem, top_mem = self._mem_sim(self.nodes[sub_idx],self.origin_graph[sub_idx])
    ...
    def _mem_sim(self,nodes, origin_graph):
        ...
        while not q.empty():
            ...
            if node.type == NodeType.VARIABLE:
                ...
                pass
            elif node.type == NodeType.COMMUNICATION:
                pass
            ...
            for pred_id in edges[PRED]:
                ...
                if pred.type == NodeType.VARIABLE:
                    ...
                    if len(sim_graph[pred_id][SUCC]) == 0 and not pred.node.persistable:
    ...
    def get_pipeline_time(self):
        if len(self.time_list) <= 1:
            return self.time_list[0] + self.time_list[0] * self.bwd_ratio + self.update_time
        else:
            pp = list(map(lambda x: self.process_mesh.process_group[x],
                            range(0, self.process_mesh.topology[2]* self.process_mesh.topology[1],
                                    self.process_mesh.topology[2])))
            
            self.fwd_time = [self.time_list[pp[i]] for i in range(len(pp))]
            self.bwd_time = [self.time_list[pp[i]]* self.bwd_ratio for i in range(len(pp))]
        ...
        while not q.empty():
            ...
            if e.name == 'fwd':
                if fwd_cnt[stid] > 0:
                    ...
                    event_list.append(e)
                    if stid != stage_num - 1:
                        q.put(PipeEvent(stid + 1, 'fwd', self.fwd_time[stid + 1],
                                        start_time = e.e_time))
                    else:
                        q.put(PipeEvent(stid, 'bwd', self.bwd_time[stid], 
                                        start_time = e.e_time ))
                    fwd_cnt[stid] -= 1
                ...
                event_list.append(e)
                if stid != 0:
                    q.put(PipeEvent(stid - 1, 'bwd', self.bwd_time[stid - 1], 
                                    start_time = e.e_time))
                fwd_cnt[stid] += 1
                bwd_cnt[stid] -= 1
                if bwd_cnt[stid] == 0:
                    q.put(PipeEvent(stid, 'optim', self.optim_time[stid], 
                                    start_time = e.e_time))
                global_time[stid]  = e.e_time
            elif e.name == 'optim':
                e.s_time = max(global_time[stid], e.s_time)
...
def estimate_cost(distributed_program, cluster = None, process_mesh = None, single_cost_data = None, batch_size = 1):
    # the following line is left for now, cluster model will be involved in the future
    assert cluster is None

    cost = Cost()
    cm_ctx = CostModelContext(cluster = cluster,
