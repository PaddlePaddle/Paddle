...
class ParallelTuner:
                    ...
                    if var.is_data or var.is_parameter:
                        if var_name not in has_set_dist_attr:
                            ...
                            tensor_dist_attr.dims_mapping = self._dist_context.get_dist_op_for_program(op).dist_attr.get_input_dims_mapping(
                                var_name) if var.is_parameter else [-1 for i in var.shape]
    ...
    def _eval_trial(self, trial):
        ...
        print("Parallel Tuner: process_mesh_list is ", process_mesh_list)
        ...
        end_time = time.time()
        print("Parallel Tuner: time of sampling a trial is {}s".format(end_time - start_time))
        ...
        complete_tensor_time = time.time()
        self._complete_tensor_dist_attr_by_op()
        end_time = time.time()
        print("Parallel Tuner: time of completing tensor dist attr by op is {}s".format(end_time - complete_tensor_time), flush=True)
        ...
        end_time = time.time()
        print("Parallel tuner: completing time is ", end_time - start_time, flush=True)

        start_time = time.time()
        estimate_time = self._estimate_trial()
        print("Parallel Tuner: cost by cost model is ", estimate_time, "ms", flush=True)
        end_time = time.time()
        print("Parallel Tuner: time of estimating a trial by cost model is {}s".format(end_time - start_time), flush=True)
    ...
    def erine_pass(self, dist_context):
    ...
    def _estimate_trial(self):
        ...
        print("Parallel Tuner: max_memory", "{:,}".format(max_memory), flush=True)
        ...
        self.erine_pass(self._dist_context)
        global_cost = self._estimator.estimate(self._dist_context)
        return global_cost.time
    ...
    def _store_init_parallel_strategy(self):
        ...
        end_time = time.time()
        print("Parallel Tuner: time of completing forward annotation is {}s".format(end_time-start_time))
        self._dist_context.block_state.parse_forward_blocks(
            self._dist_context.serial_main_program)
    ...
    def tune(self):
        ...
        best_time = init_time
        # For debug, the best time is by searched
        best_time = 1000000000 
        start_time = time.time()
        self.construct_space()
        end_time = time.time()
        print("Parallel Tuner: time of constructing space is {}s".format(end_time - start_time), flush=True)
        times = 0
        while True:
            times += 1
            ...
            end_time = time.time()
            print("Parallel Tuner: time of evaling a trial is ".format(end_time - start_time), flush=True)
            ...
            if cur_time < best_time:
                ...
                best_time = cur_time
            print("Parallel Tuner: {} cur_time: {}ms, best_time: {}ms".format(times, cur_time, best_time), flush=True)
        ...
        # Calc cost again after erine_pass
        self._estimator = CostEstimator(
            self._dist_context.serial_main_program,
            self._cluster,
            loop_count=self._loop_count)
        global_cost = self._estimator.estimate(self._dist_context)
        end_time = time.time()
        print("Parallel Tuner: searching {} times cost {}s and best time is {}ms".format(self._max_trials, end_time-s_time, global_cost.time))
        print("Parallel Tuner: best strategy program")
        print_program_with_dist_attr(self._dist_context.serial_main_program, self._dist_context)
