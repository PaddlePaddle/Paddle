...
class ParallelTuner:
    ...
    def __init__(self,
        ...
        self._init_parallel_strategy = [None, None, None, None]
        self._best_parallel_strategy = [None, None, None, None]
    ...
    def _estimate_trial(self):
        ...
        if self._mode == "eval":
            ...
            # print("parallel_tuner.py serial_main_program with optimizer****")
            # print_program_with_dist_attr(serial_main_program, self._dist_context)
            self._estimator = CostEstimator(serial_main_program,
                                            self._cluster,
                                            loop_count=self._loop_count)

        self._max_memory = self._estimator._estimate_max_memory_by_dist_op(self._dist_context)
        print("\tmax_memory", "{:,}".format(self._max_memory), flush=True)
        # The max memory must be less than 80% of 32GB (hard code)
        if self._max_memory > 32 * 1024 * 1024 * 1024:
    ...
    def _store_init_parallel_strategy(self):
        ...
        self._init_parallel_strategy[3] = self._max_memory
        ...
        self._best_parallel_strategy[3] = self._max_memory
    ...
    def tune(self):
        ...
        self._init_parallel_strategy[3] = self._max_memory
        self._best_parallel_strategy[3] = self._max_memory
        print("init cost and max_memory: ", init_time, self._init_parallel_strategy[3])
        ...
        max_memory = self._best_parallel_strategy[3]
        print("The best strategy max memory is {}".format(max_memory))
