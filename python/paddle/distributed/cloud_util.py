...
node_ip: {} from paddlecloud environment.".format(args_node_ip, node_ip))
    ...
    if args_node_ips != "127.0.0.1" and args_cluster_node_ips != ",".join(
            node_ips):
        logger.warning(
...
paddlecloud environment.".format(args_node_ips, node_ips))
    ...
    if num_nodes > 1:
        try:
            paddle_port = int(os.getenv("TRAINER_PORT", ""))
            paddle_port_num = int(os.getenv("TRAINER_PORTS_NUM", ""))

            if paddle_port_num >= selected_gpus_num and paddle_port != args_port:
