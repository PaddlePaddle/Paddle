...
class DGCModel(layers.Layer):
    def __init__(self,
        ...
        self._global_learning_rate = paddle.to_tensor(
            learning_rate, dtype="float32")
        ...
        def reduce_grad(grad):
            ...
            def reduce_buffer(comm_param, comm_grad):
                ...
                paddle.distributed.wait(
                    comm_grad, group=self.new_group, use_calc_stream=True)
                #NOTE: dgc comm: allgather.
                if self._global_step_var[
                        0] >= self._rampup_begin_step and self._is_use_dgc(
                ...
                paddle.distributed.wait(
                    comm_grad, group=self.new_group, use_calc_stream=False)
    ...
    def init_params(self):
        ...
        assert len(self.trainable_params) == len(
            self.
            _reduce_var_ready), "there are at least two params with same name."
        ...
        self._nranks_var = paddle.to_tensor(
            [self._num_trainers], dtype='float32')
    ...
    def _get_auxiliary_var(self, param):
        ...
        encoded_var = paddle.empty(
            shape=[0],
            dtype=param.dtype,
            name=param.name + core.dgc.kDGCEncodedName())
        gather_var = paddle.empty(
            shape=[0],
            dtype=param.dtype,
    ...
    def _add_accumulator(self,
        ...
        var = self.helper.create_global_variable(
            ...
            if _in_legacy_dygraph() else (param.type if type is None else type),
            shape=shape,
    ...
    def step(self):
        ...
        for param in self.trainable_params:
            paddle.distributed.wait(
                param.grad, group=self.new_group, use_calc_stream=False)
            paddle.distributed.wait(
                param.grad, group=self.new_group, use_calc_stream=True)
...
class DGCDygraphOptimizer(paddle.optimizer.Optimizer):
    def __init__(self, dgc_model):
        super(DGCDygraphOptimizer, self).__init__(
            learning_rate=dgc_model._learning_rate,
