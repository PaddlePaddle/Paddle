...
def prune_ops(total_ops, inputs_set, outputs_set, no_grad_set):
    ...
    if inputs_set:
        for i, op in enumerate(total_ops):
            ...
            op_inputs = get_used_external_value(op) if op.name() in["pd_op.if", "pd_op.while"] else op.operands_source()
...
def append_backward_ops(
    ...
    def make_output_with_output_grad(op):
        ...
        for i, value in enumerate(op.results()):
            new_value = [control_flow_value_to_copyvalue_map[value]] if value in control_flow_value_to_copyvalue_map else [value]
    ...
    def make_input_with_input_stopgradient(op):
        origin_inputs = get_used_external_value(op) if op.name() in ["pd_op.if", "pd_op.while"] else op.operands_source()
        inputs = []
        input_grad_stopgradients = []
        if op.name() in ["builtin.combine" , "pd_op.if", "pd_op.while" , "cf.tuple_push"]:
        ...
        for input, grad_semantic in zip(
            origin_inputs, grad_semantic_info
        ):
            if not grad_semantic:
                if (
                    ...
                    for tmp in input.get_defining_op().operands_source():
                        tmp_input.append(control_flow_value_to_copyvalue_map[tmp] if tmp in control_flow_value_to_copyvalue_map else tmp)
                    
                    inputs.append(tmp_input)
                else:
                    tmp_input = [control_flow_value_to_copyvalue_map[input]] if input in control_flow_value_to_copyvalue_map else [input]
                    inputs.append(tmp_input)
                continue
    ...
    def update_input_grad_map(op, input_grads, origin_inputs):
        i = 0
        if op.name() == "builtin.combine" or op.name() == "pd_op.if" or op.name() == "pd_op.while":
            grad_semantic_info = [True for _ in range(len(origin_inputs))]
        else:
            grad_semantic_info = op.get_input_grad_semantics()
        for input, grad_semantic in zip(
            ...
            if (
                ...
                update_input_grad_map(input.get_defining_op(), input_grads[i], input.get_defining_op().operands_source())
            else:
    ...
    control_flow_value_to_copyvalue_map = dict()
    #tuple_push value to pop value
    control_flow_copyvalue_to_value_map = dict()
    #sub_block op output to parent_block op output
    inside_value_to_outside_value_map= dict()
    if effective_forward_ops[-1].name() == "cf.yield":
        yield_op = effective_forward_ops[-1]
        for outside_output, inside_output in zip(base_op.results(), yield_op.operands_source()):
        ...
        for op in clear_effective_forward_ops:
            if paddle.framework.core.has_vjp(op):
                ...
                if op.name() == "cf.tuple_push":
                    copy_out = paddle.framework.core.call_vjp(op, inputs, outputs, output_grads, input_grad_stopgradients)
                    pop_op = bwd_block.ops[-1]
                    bwd_ops = [pop_op]
                    for output, copy_output in zip(inputs[1:], copy_out[1:]):
                        control_flow_value_to_copyvalue_map[output[0]] = copy_output[0]
                        control_flow_copyvalue_to_value_map[copy_output[0]] = output[0]
                    ...
                    if op.name() == "pd_op.if" or op.name() == "pd_op.while":
                        ...
                        for sub_fwd_block, sub_bwd_block in zip(op.blocks(), grad_op.blocks()): 
                            sub_state = state.copy(sub_fwd_block)
                            sub_backward_ops = []
                            append_backward_ops(op, sub_fwd_block, sub_bwd_block, sub_fwd_block.ops, no_grad_set, sub_backward_ops, sub_state)
...
def calc_gradient_helper(outputs, inputs, grad_outputs, no_grad_set):
    ...
    append_backward_ops(
        None, block, block, effective_forward_ops, no_grad_set, backward_ops, state
    )
