...
def MakeLayerNameInSubmodel(name, submodel_name=None):
    ...
    if (submodel_name is None and not g_add_submodel_suffix
...
def parse_conv(conv, input_layer_name, conv_conf):
    ...
    config_assert((conv_conf.img_size**2) == img_pixels, (
        ...
        "image pixels %d") % (input_layer_name, conv_conf.img_size, img_pixels))
...
class ConvLayerBase(LayerBase):
    ...
    def __init__(self,
        ...
        if (use_gpu == 1 and self.layer_type != "exconv"
                and (parallel_nn == 0 or self.config.device > -1)):
