...
TEST(inference, recognize_digits_combine) {
  if (FLAGS_dirname.empty() || FLAGS_prog_filename.empty() ||
      FLAGS_param_filename.empty()) {
    LOG(FATAL) << "Usage: ./example --dirname=path/to/your/model "
                  "--prog_filename=pd_file --param_filename=param.dat";
  }

  LOG(INFO) << "FLAGS_dirname: " << FLAGS_dirname << std::endl;
  std::string dirname = FLAGS_dirname;
  std::string prog_filename = FLAGS_prog_filename;
  std::string param_filename = FLAGS_param_filename;
  ...
  TestInference<paddle::platform::CPUPlace, float>(
      dirname, prog_filename, param_filename, cpu_feeds, cpu_fetchs1);
