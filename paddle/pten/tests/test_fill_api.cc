...
TEST(API, full_like) {
  ...
  // 2. test API
  auto out = paddle::experimental::full_like(x, val, pten::DataType::kFLOAT32);
...
TEST(API, zeros_like) {
  ...
  // 2. test API
  auto out = paddle::experimental::zeros_like(x, pten::DataType::kFLOAT32);
...
TEST(API, ones_like) {
  ...
  // 2. test API
  auto out = paddle::experimental::ones_like(x, pten::DataType::kINT32);
  ...
  ASSERT_EQ(out.numel(), 6);
  ASSERT_EQ(out.is_cpu(), true);
  ASSERT_EQ(out.type(), pten::DataType::kINT32);
  ASSERT_EQ(out.layout(), pten::DataLayout::kNCHW);
  ASSERT_EQ(out.initialized(), true);
