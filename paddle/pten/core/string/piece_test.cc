...
TEST(StringPiece, Construct) {
  ...
  { EXPECT_THROW(pten::string::Piece s(NULL, 10000U), std::invalid_argument); }
  {
    pten::string::Piece s(NULL);
    EXPECT_EQ(0U, s.len());
  }
  {
    std::string a;
    EXPECT_EQ(0U, a.size());
    pten::string::Piece s(a);
    EXPECT_EQ(0U, s.len());
  }
}

TEST(StringPiece, CopyAndAssign) {
  pten::string::Piece empty;
  EXPECT_EQ(0U, empty.len());

  pten::string::Piece a("hello");
  pten::string::Piece b = a;
  EXPECT_EQ(b.len(), strlen("hello"));
  EXPECT_EQ(a, b);

  std::string storage("hello");
  pten::string::Piece c(storage);
  EXPECT_EQ(a, c);
...
TEST(StringPiece, ToString) {
  ...
  {
    pten::string::Piece s("hello");
    EXPECT_EQ(std::string("hello"), s.ToString());
  }
}

TEST(StringPiece, HasPrefixSuffix) {
  using pten::string::HasPrefix;
  using pten::string::HasSuffix;
  {
    pten::string::Piece s;
...
TEST(StringPiece, SkipPrefixSuffix) {
  using pten::string::SkipPrefix;
  using pten::string::SkipSuffix;
  {
    pten::string::Piece s;
...
TEST(StringPiece, TrimPrefixSuffix) {
  using pten::string::TrimPrefix;
  using pten::string::TrimSuffix;
  {
    pten::string::Piece s;
...
TEST(StringPiece, Index) {
  using pten::string::Index;
  auto npos = pten::string::Piece::npos;
  {
    pten::string::Piece s;
...
TEST(StringPiece, StreamOutput) {
  using pten::string::Piece;

  std::stringstream o;
  o << pten::string::Piece();
  EXPECT_EQ("", o.str());

  o << pten::string::Piece("hello");
