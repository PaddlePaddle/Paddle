...
StringTensor::StringTensor(const std::shared_ptr<Allocator>& a,
...
platform::pstring* StringTensor::mutable_data(size_t request_bytes /* = 0 */) {
  PADDLE_ENFORCE(
      valid(),
  ...
  return reinterpret_cast<platform::pstring*>(storage_->data());
}

const platform::pstring* StringTensor::data() const {
  PADDLE_ENFORCE_NOT_NULL(
      storage_,
      paddle::platform::errors::PreconditionNotMet(
          "The storage must be valid when call the mutable data function."));
