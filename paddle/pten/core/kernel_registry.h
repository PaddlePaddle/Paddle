...
#define DATALAYOUT(arg__) pten::DataLayout::k##arg__
#define DATATYPE(arg__) pten::DataType::k##arg__
...
struct KernelArgsParseFunctor<Return_ (*)(Args_...)> {
  ...
  static void Parse(const KernelKey& default_key, KernelArgsDef* args_def) {
    ...
    // fluid Tensor
    auto default_tensor_layout = pten::DataLayout::kNCHW;
    if (default_key.layout() != pten::DataLayout::kAny) {
...
#define _PT_KERNEL_REGISTRAR_INIT_1(kernel_name,       \
  ...
  static const ::pten::KernelRegistrar PT_CONCATENATE( \
      ...
      BACKEND(backend),                                \
      DATALAYOUT(layout),                              \
      ::pten::CppTypeToDataType<cpp_dtype>::Type(),    \
