...
PADDLE_API Tensor add(const Tensor& x, const Tensor& y) {
  ...
  auto dense_y = PrepareData(y, kernel.InputAt(1), {});

  Tensor out;
  auto dense_out = SetKernelOutput(kernel_backend, &out);
  pten::MetaTensor meta_out(dense_out);

  pten::ElementwiseInferMeta(MakeMetaTensor(*dense_x), MakeMetaTensor(*dense_y), &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, const pten::DenseTensor&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, *dense_y, dense_out);
...
PADDLE_API Tensor cast(const Tensor& x, DataType out_dtype) {
  ...
  pten::CastInferMeta(MakeMetaTensor(*dense_x), out_dtype, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, DataType, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, out_dtype, dense_out);
...
PADDLE_API Tensor concat(const std::vector<Tensor>& x, const Scalar& axis) {
  ...
  pten::ConcatInferMeta(MakeMetaTensor(*dense_x), axis, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const std::vector<pten::DenseTensor>&, const pten::Scalar&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, pten::Scalar(axis), dense_out);
...
PADDLE_API Tensor conj(const Tensor& x) {
  ...
  pten::UnchangedInferMeta(MakeMetaTensor(*dense_x), &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, dense_out);
...
PADDLE_API Tensor dot(const Tensor& x, const Tensor& y) {
  ...
  pten::DotInferMeta(MakeMetaTensor(*dense_x), MakeMetaTensor(*dense_y), &meta_out);
...
PADDLE_API Tensor empty(const ScalarArray& shape, DataType dtype, Backend place, DataLayout layout) {
  ...
  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::ScalarArray&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, pten::ScalarArray(shape), dense_out);

  return out;
}

PADDLE_API Tensor empty_like(const Tensor& x, DataType dtype, Backend place, DataLayout layout) {
  ...
  pten::CreateLikeInferMeta(MakeMetaTensor(*dense_x), dtype, layout, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, dense_out);
...
PADDLE_API Tensor flatten(const Tensor& x, int start_axis, int stop_axis) {
  ...
  pten::FlattenInferMeta(MakeMetaTensor(*dense_x), start_axis, stop_axis, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, int, int, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, start_axis, stop_axis, dense_out);
...
PADDLE_API Tensor full(const ScalarArray& shape, const Scalar& value, DataType dtype, Backend place, DataLayout layout) {
  ...
  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::ScalarArray&, const pten::Scalar&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, pten::ScalarArray(shape), pten::Scalar(value), dense_out);
...
PADDLE_API Tensor full_like(const Tensor& x, const Scalar& value, DataType dtype, Backend place, DataLayout layout) {
  ...
  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::Scalar&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, pten::Scalar(value), dense_out);
...
PADDLE_API Tensor matmul(const Tensor& x, const Tensor& y, bool transpose_x, bool transpose_y) {
  ...
  pten::MatmulInferMeta(MakeMetaTensor(*dense_x), MakeMetaTensor(*dense_y), transpose_x, transpose_y, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, const pten::DenseTensor&, bool, bool, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, *dense_y, transpose_x, transpose_y, dense_out);
...
PADDLE_API Tensor mean(const Tensor& x, const std::vector<int64_t>& axis, bool keep_dim) {
  ...
  pten::ReduceInferMeta(MakeMetaTensor(*dense_x), axis, keep_dim, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, const std::vector<int64_t>&, bool, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, axis, keep_dim, dense_out);
...
PADDLE_API Tensor reshape(const Tensor& x, const ScalarArray& shape) {
  ...
  pten::ReshapeInferMeta(MakeMetaTensor(*dense_x), shape, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, const pten::ScalarArray&, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, pten::ScalarArray(shape), dense_out);
...
PADDLE_API Tensor scale(const Tensor& x, const Scalar& scale, float bias, bool bias_after_scale) {
  ...
  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, const pten::Scalar&, float, bool, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, pten::Scalar(scale), bias, bias_after_scale, dense_out);
...
PADDLE_API Tensor sum(const Tensor& x, const std::vector<int64_t>& axis, DataType dtype, bool keep_dim) {
  ...
  pten::SumInferMeta(MakeMetaTensor(*dense_x), axis, dtype, keep_dim, &meta_out);

  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, const std::vector<int64_t>&, DataType, bool, pten::DenseTensor*);
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, *dense_x, axis, dtype, keep_dim, dense_out);
