...
PADDLE_API Tensor add(const Tensor& x, const Tensor& y) {
  ...
  pten::ElementwiseInferMeta(MakeMetaTensor(*input_x), MakeMetaTensor(*input_y), &meta_out);
...
PADDLE_API Tensor dot(const Tensor& x, const Tensor& y) {
  ...
  pten::DotInferMeta(MakeMetaTensor(*input_x), MakeMetaTensor(*input_y), &meta_out);
...
PADDLE_API Tensor flatten(const Tensor& x, int start_axis, int stop_axis) {
  ...
  pten::FlattenInferMeta(MakeMetaTensor(*input_x), start_axis, stop_axis, &meta_out);
...
PADDLE_API Tensor full(const ScalarArray& shape, const Scalar& value, DataType dtype, Backend place, DataLayout layout) {
  ...
  auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
  (*kernel_fn)(*dev_ctx, pten::ScalarArray(shape), pten::Scalar(value), kernel_out);
...
PADDLE_API Tensor matmul(const Tensor& x, const Tensor& y, bool transpose_x, bool transpose_y) {
  ...
  pten::MatmulInferMeta(MakeMetaTensor(*input_x), MakeMetaTensor(*input_y), transpose_x, transpose_y, &meta_out);
...
Tensor ones_like(const Tensor& x, DataType dtype, Backend place, DataLayout layout) {
...
PADDLE_API Tensor scale(const Tensor& x, const Scalar& scale, float bias, bool bias_after_scale) {
  ...
  if(kernel_type == KernelType::DENSE_TENSOR_KENREL){
    ...
    auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
    (*kernel_fn)(*dev_ctx, *input_x, pten::Scalar(scale), bias, bias_after_scale, kernel_out);
    ...
    VLOG(6) << "scale API SelectedRows kernel key: [" << kernel_backend << ", " << kernel_layout << ", "<< kernel_data_type << "]";
    ...
    using kernel_signature = void(*)(const platform::DeviceContext&, const pten::SelectedRows&, const pten::Scalar&, float, bool, pten::SelectedRows*);
    auto* kernel_fn = kernel.GetVariadicKernelFn<kernel_signature>();
    (*kernel_fn)(*dev_ctx, *input_x, pten::Scalar(scale), bias, bias_after_scale, kernel_out);
...
PADDLE_API Tensor sum(const Tensor& x, const std::vector<int64_t>& axis, DataType dtype, bool keep_dim) {
  ...
  pten::SumInferMeta(MakeMetaTensor(*input_x), axis, dtype, keep_dim, &meta_out);
...
Tensor zeros_like(const Tensor& x, DataType dtype, Backend place, DataLayout layout) {
