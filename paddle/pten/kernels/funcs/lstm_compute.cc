...
#include "paddle/pten/kernels/funcs/detail/lstm_cpu_kernel.h"
#include "paddle/pten/kernels/funcs/detail/lstm_kernel.h"
  ...
  static void compute(const paddle::platform::CPUDeviceContext& context,
    ...
    for (int b = 0; b < batch_size; b++) {
      detail::cpu_lstm_forward(context,
                               pten::funcs::detail::forward::lstm<T>(),
                               value,
                               frame_size,
                               cell_clip,
                               cand_act,
  ...
  static void compute(const paddle::platform::CPUDeviceContext& context,
    ...
    for (int b = 0; b < batch_size; b++) {
      detail::cpu_lstm_backward(context,
                                pten::funcs::detail::backward::lstm<T>(),
                                value,
                                grad,
                                frame_size,
                                cell_clip,
