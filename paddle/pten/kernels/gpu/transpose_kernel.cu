...
using Dim3 = paddle::framework::Dim3;
using Index3 = paddle::framework::Index3;
...
void LaunchNarrowDims2TransposeKernel(const pten::GPUContext& d,
                                      int tile_size_i,
                                      int tile_size_j,
...
struct NarrowDims2TransposeDispatch {
  static void DoTranspose(const pten::GPUContext& d,
                          int tile_size_i,
                          int tile_size_j,
                          int total_tiles_count,
                          const T* input,
...
void SwapDim1And2InNarrow(const pten::GPUContext& d,
                          const T* input,
                          const Dim3& input_dims,
                          T* output,
  ...
  for (auto tile_size_pair : tile_sele) {
    ...
    int num_wasted_threads = input_long_edge -
                             paddle::framework::CeilOrFloor<int, false>(
                                 input_long_edge, proposed_tile_long_edge) *
                                 proposed_tile_long_edge;

    int num_full_tiles = paddle::framework::CeilOrFloor<int, false>(
        input_long_edge, proposed_tile_long_edge);
  ...
  Dim3 input_dims_aligned = {
      input_dims[0],
      paddle::framework::CeilOrFloor<int, true>(input_dims[1],
                                                select_tile_size_i),
      paddle::framework::CeilOrFloor<int, true>(input_dims[2],
                                                select_tile_size_j),
  };
...
template <typename T>
void SendSwapDim1And2InTranspose(const pten::GPUContext& d,
                                 const T* input,
                                 const Dim3& input_dims,
                                 T* output) {
  ...
  if (large_tile) {
    ...
    Dim3 input_dims_aligned = {
        input_dims[0],
        paddle::framework::CeilOrFloor<int, true>(input_dims[1], kTileSize),
        paddle::framework::CeilOrFloor<int, true>(input_dims[2], kTileSize),
    };
    ...
    auto config = pten::backends::gpu::GetGpuLaunchConfig1D(d, total_elements);
    TransposeSimpleKernel<T, 0, 2, 1><<<config.block_per_grid.x,
...
struct SwapDim1And2InTranspose {
  typedef pten::GPUContext Device;
...
struct SwapDim0And2InTranspose {
  ...
  void operator()(const Device& d,
    ...
    auto config = pten::backends::gpu::GetGpuLaunchConfig1D(d, total_size);

    TransposeSimpleKernel<T, 2, 1, 0><<<config.block_per_grid.x,
                                        config.thread_per_block.x,
                                        0,
                                        d.stream()>>>(
...
struct TransposeSimple {
  static bool run(const pten::GPUContext& ctx,
                  const Tensor& in,
...
template <typename T>
void TransposeGPUKernelDriver(const pten::GPUContext& dev_ctx,
                              const int ndims,
                              const Tensor& in,
                              const std::vector<int32_t> perm,
                              Tensor* out) {
  ...
  if (!ret) {
    TransposeKernelImpl<T>(dev_ctx, in, perm, out);
  }
}
