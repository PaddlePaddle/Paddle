...
static __global__ void BNForwardInference(
  ...
  for (int i = gid; i < num; i += stride) {
    const int c = layout == paddle::framework::DataLayout::kNCHW ? i / HxW % C : i % C;
...
void BatchNormKernel(const Context& ctx, const DenseTensor& x, \
                    const DenseTensor& scale, const DenseTensor& bias,
                    const DenseTensor& mean, const DenseTensor& variance,
                    float momentum, float epsilon_f, const std::string& data_layout_str,
...
PT_REGISTER_KERNEL(batch_norm, GPU, ALL_LAYOUT, pten::BatchNormKernel, float, double) {}
