...
template <typename T>
static __global__ void KeBNRestoreData(const paddle::framework::DataLayout layout, T *x,
                                       const BatchNormParamType<T> *scale,
  ...
  int stride = blockDim.x * gridDim.x;
  for (int i = gid; i < num; i += stride) {
    const int c = layout == paddle::framework::DataLayout::kNCHW ? (i / M) % C : i % C;
...
class InplaceHelper {
 public:
  void operator()(const paddle::framework::DataLayout layout, T *x,
                  const BatchNormParamType<T> *scale,
                  const BatchNormParamType<T> *bias,
                  const BatchNormParamType<T> *mean,
    ...
    PADDLE_ENFORCE_EQ(x, y, pten::errors::InvalidArgument(
...
// auto compute_format = DataLayout::kNCHW;
    ...
    if (!use_global_stats) {
      if ((N * H * W * D) == 1) {
        ...
        paddle::operators::math::SetConstant<paddle::platform::CUDADeviceContext, BatchNormParamType<T>>
            functor;
...
//                                                       data_desc_, mode_));
#else
      PADDLE_ENFORCE_GPU_SUCCESS(paddle::platform::dynload::cudnnSetTensorNdDescriptor(
          data_desc_, CudnnDataType<T>::type,
          x_dims.size() > 3 ? x_dims.size() : 4, dims.data(), strides.data()));
      PADDLE_ENFORCE_GPU_SUCCESS(
          paddle::platform::dynload::cudnnDeriveBNTensorDescriptor(bn_param_desc_,
                                                           data_desc_, mode_));
#endif
...
//         epsilon, saved_mean_data, saved_var_data));
        ...
        if (compute_format == DataLayout::kNCHW) {
          if (d_x) {
            BNBackwardData<T, block, paddle::framework::DataLayout::kNCHW><<<
          ...
          if (d_x) {
            BNBackwardData<T, block, paddle::framework::DataLayout::kNHWC><<<
                grid2, block, 0, ctx.stream()>>>(
                d_y->data<T>(), scale.data<BatchNormParamType<T>>(),
                saved_mean_data, x.data<T>(), saved_var_data, C, N, H * W * D,
                d_x->data<T>());
...
//     platform::dynload::miopenDestroyTensorDescriptor(bn_param_desc_));
      ...
      if (compute_format == DataLayout::kNCHW) {
        if (d_x) {
          KeBNBackwardData<
              T, paddle::framework::DataLayout::kNCHW><<<grid1, block, 0, stream>>>(
              d_y->data<T>(), scale.data<BatchNormParamType<T>>(),
        ...
        if (d_x) {
          KeBNBackwardData<
              T, paddle::framework::DataLayout::kNHWC><<<grid1, block, 0, stream>>>(
              d_y->data<T>(), scale.data<BatchNormParamType<T>>(),
              running_var_data, epsilon, C, H * W, num, d_x->data<T>());
        }
        if (d_scale && d_bias) {
...
PT_REGISTER_KERNEL(batch_norm_grad, GPU, ALL_LAYOUT, pten::BatchNormGradKernel, float, double) {}

PT_REGISTER_KERNEL(batch_norm_grad_raw, GPU, ALL_LAYOUT, pten::BatchNormGradRawKernel, float, double) {}
