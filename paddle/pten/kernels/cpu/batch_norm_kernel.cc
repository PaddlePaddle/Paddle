...
void BatchNormKernel(const Context& ctx, const DenseTensor& x, \
                    ...
                    float momentum, float epsilon, std::string data_layout_str,
    ...
    if (global_stats) {
      ...
      EigenVectorArrayMap<T> saved_inv_std(
          saved_variance->data<T>(), C);
      // inverse SavedVariance first, gradient will use it too.
      saved_inv_std = (saved_inv_std + epsilon).inverse().sqrt();
      inv_std = saved_inv_std;
    }
    ConstEigenVectorArrayMap<T> mean_arr(
        global_stats ? mean.data<T>()
                     : saved_mean->data<T>(),
        C);
...
PT_REGISTER_KERNEL(batch_norm, CPU, ALL_LAYOUT, pten::BatchNormKernel, float, double) {}
