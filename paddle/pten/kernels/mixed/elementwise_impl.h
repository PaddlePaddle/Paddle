...
void ElementwiseCompute(const paddle::platform::CPUDeviceContext &dev_ctx,
  ...
  if (is_xsize_larger) {
    auto y_dims_trimed = functions::trim_trailing_singular_dims(y_dims);
    axis_trim = (y_dims_trimed.size() == 0) ? x_dims.size() : axis;
    functions::get_mid_dims(x_dims,
                            y_dims_trimed,
                            axis_trim,
                            &pre,
                            &n,
    ...
    auto x_dims_trimed = functions::trim_trailing_singular_dims(x_dims);
    axis_trim = (x_dims_trimed.size() == 0) ? y_dims.size() : axis;
    functions::get_mid_dims(y_dims,
                            x_dims_trimed,
                            axis_trim,
                            &pre,
                            &n,
