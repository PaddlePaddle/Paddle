...
@REM call :build || goto build_error
...
@REM cmake .. -G %GENERATOR% -DCMAKE_BUILD_TYPE=Release -DWITH_AVX=%WITH_AVX% -DWITH_GPU=%WITH_GPU% -DWITH_MKL=%WITH_MKL% ^
@REM -DWITH_TESTING=%WITH_TESTING% -DWITH_PYTHON=%WITH_PYTHON% -DPYTHON_EXECUTABLE=%PYTHON_EXECUTABLE% -DON_INFER=%ON_INFER% ^
@REM -DWITH_INFERENCE_API_TEST=%WITH_INFERENCE_API_TEST% -DTHIRD_PARTY_PATH=%THIRD_PARTY_PATH% ^
@REM -DINFERENCE_DEMO_INSTALL_DIR=%INFERENCE_DEMO_INSTALL_DIR% -DWITH_STATIC_LIB=%WITH_STATIC_LIB% ^
@REM -DWITH_TENSORRT=%WITH_TENSORRT% -DTENSORRT_ROOT="%TENSORRT_ROOT%" -DMSVC_STATIC_CRT=%MSVC_STATIC_CRT% ^
@REM -DWITH_UNITY_BUILD=%WITH_UNITY_BUILD% -DCUDA_ARCH_NAME=%CUDA_ARCH_NAME% -DCUB_PATH=%THIRD_PARTY_HOME%/cub ^
@REM -DCUDA_TOOLKIT_ROOT_DIR="%CUDA_TOOLKIT_ROOT_DIR%" -DNEW_RELEASE_ALL=%NEW_RELEASE_ALL% -DNEW_RELEASE_PYPI=%NEW_RELEASE_PYPI% ^
@REM -DNEW_RELEASE_JIT=%NEW_RELEASE_JIT% -DWITH_ONNXRUNTIME=%WITH_ONNXRUNTIME%
set CMAKE_GENERATOR=%GENERATOR% CMAKE_BUILD_TYPE=Release WITH_AVX=%WITH_AVX% WITH_GPU=%WITH_GPU% WITH_MKL=%WITH_MKL% ^
WITH_TESTING=%WITH_TESTING% WITH_PYTHON=%WITH_PYTHON% PYTHON_EXECUTABLE=%PYTHON_EXECUTABLE% ON_INFER=%ON_INFER% ^
WITH_INFERENCE_API_TEST=%WITH_INFERENCE_API_TEST% THIRD_PARTY_PATH=%THIRD_PARTY_PATH% ^
INFERENCE_DEMO_INSTALL_DIR=%INFERENCE_DEMO_INSTALL_DIR% WITH_STATIC_LIB=%WITH_STATIC_LIB% ^
WITH_TENSORRT=%WITH_TENSORRT% TENSORRT_ROOT="%TENSORRT_ROOT%" MSVC_STATIC_CRT=%MSVC_STATIC_CRT% ^
WITH_UNITY_BUILD=%WITH_UNITY_BUILD% CUDA_ARCH_NAME=%CUDA_ARCH_NAME% CUB_PATH=%THIRD_PARTY_HOME%/cub ^
CUDA_TOOLKIT_ROOT_DIR="%CUDA_TOOLKIT_ROOT_DIR%" NEW_RELEASE_ALL=%NEW_RELEASE_ALL% NEW_RELEASE_PYPI=%NEW_RELEASE_PYPI% ^
NEW_RELEASE_JIT=%NEW_RELEASE_JIT% WITH_ONNXRUNTIME=%WITH_ONNXRUNTIME%
cd ..
