...
set GENERATOR=Ninja && set CMAKE_BUILD_TYPE=Release && set WITH_AVX=%WITH_AVX% && set WITH_GPU=%WITH_GPU% && set WITH_MKL=%WITH_MKL% 
set WITH_TESTING=%WITH_TESTING% && set WITH_PYTHON=%WITH_PYTHON% && set PYTHON_EXECUTABLE=%PYTHON_EXECUTABLE% && set ON_INFER=%ON_INFER% 
set WITH_INFERENCE_API_TEST=%WITH_INFERENCE_API_TEST% && set THIRD_PARTY_PATH=%THIRD_PARTY_PATH%
set INFERENCE_DEMO_INSTALL_DIR=%INFERENCE_DEMO_INSTALL_DIR% && set WITH_STATIC_LIB=%WITH_STATIC_LIB%
set WITH_TENSORRT=%WITH_TENSORRT% && set TENSORRT_ROOT="%TENSORRT_ROOT%" && set MSVC_STATIC_CRT=%MSVC_STATIC_CRT%
set WITH_UNITY_BUILD=%WITH_UNITY_BUILD% && set CUDA_ARCH_NAME=%CUDA_ARCH_NAME% && set CUB_PATH=%THIRD_PARTY_HOME%/cub
set CUDA_TOOLKIT_ROOT_DIR="%CUDA_TOOLKIT_ROOT_DIR%" && set NEW_RELEASE_ALL=%NEW_RELEASE_ALL% && set NEW_RELEASE_PYPI=%NEW_RELEASE_PYPI%
set NEW_RELEASE_JIT=%NEW_RELEASE_JIT% && set WITH_ONNXRUNTIME=%WITH_ONNXRUNTIME%
