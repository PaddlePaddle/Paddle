...
"""
    ...
    self.python_version = [self.python27, self.python35, self.python36, self.python37]
    ...
    self.cuda_str = ["cuda8.0", "cuda9.0", "cuda10.0"]
    self.py_str = ["py27", "py35", "py36", "py37"]
    self.pip_end = ".whl --no-deps"
    self.pip_post = [".post87", ".post97", ".post107"]
    self.pip_prefix_linux = "pip install /package/paddlepaddle" 
    self.pip_prefix_windows = "pip install C:\package\paddlepaddle"
    self.pip_gpu = "_gpu-"
    self.pip_cpu = "-"
    self.mac_pip = ["-cp27-cp27m-macosx_10_6_intel", "-cp35-cp35m-macosx_10_6_intel", "-cp36-cp36m-macosx_10_6_intel", "-cp37-cp37m-macosx_10_6_intel"]
    self.linux_pip = ["-cp27-cp27mu-manylinux1_x86_64", "-cp35-cp35m-manylinux1_x86_64", "-cp36-cp36m-manylinux1_x86_64", "-cp37-cp37m-manylinux1_x86_64"]
    self.windows_pip = ["-cp27-cp27m-win_amd64", "-cp35-cp35m-win_amd64", "-cp36-cp36m-win_amd64", "-cp37-cp37m-win_amd64"]
  ...
  if(sysstr =="Windows"):
      ...
      for i in range(len(var.python_version)):
          for j in range(len(var.cuda_version)):
            ...
            python_str = var.python_version[i]
            meta_build_windows(var, python_str,  paddle_version, blt_var, name, cuda_version)
            os.system("conda build .")
      ...
      for i in range(len(var.python_version)):
          for j in range(len(var.cuda_version)):
            ...
            cuda_version = var.cuda_version[j]
            python_str = var.python_version[i]
            meta_build_linux(var, python_str,  paddle_version, build_var, name, cuda_version)
            os.system("conda build .")
