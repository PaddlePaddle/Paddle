...
function assert_api_spec_approvals() {
    ...
    for API_FILE in ${API_FILES[*]}; do
      ...
      if [ ${API_CHANGE} ] && [ "${GIT_PR_ID}" != "" ]; then
          ...
          if [ "${API_CHANGE}" == "paddle/fluid/API.spec" ];then
          ...
          elif [ "${API_CHANGE}" == "CMakeLists.txt" ];then
            APPROVALS=`curl -H "Authorization: token ${GITHUB_API_TOKEN}" https://api.github.com/repos/PaddlePaddle/Paddle/pulls/${GIT_PR_ID}/reviews?per_page=10000 | \
          ...
          if [ "${APPROVALS}" == "FALSE" ]; then
            if [ "${API_CHANGE}" == "paddle/fluid/API.spec" ];then
              echo "You must have one RD (chengduoZH or XiaoguangHu01) and one PM (shanyi15) approval for the api change! ${API_CHANGE}"
            elif [ "${API_CHANGE}" == "CMakeLists.txt" ];then
              echo "You must have one RD (luotao1 or chengduoZH or XiaoguangHu01) approval for the cmakelist change! ${API_CHANGE}"
            else
              echo "You must have one RD (velconia,XiaoguangHu01,chengduoZH,Xreki,luotao1,sneaxiy,tensor-tang,jacquesqiao,typhoonzero) approval for the api change! ${API_CHANGE}"
            fi
            exit 1
          fi
      fi
