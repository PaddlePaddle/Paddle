...
EOF
    ...
    case $LIB_TYPE in
      full)
        # Build full Paddle Python module. Will timeout without caching 'copy_paddle_core' first
        make -j `nproc` framework_py_proto copy_paddle_core paddle_python
        ;;
      pybind)
        # Build paddle pybind library. Takes 49 minutes to build. Might timeout
        make -j `nproc` copy_paddle_core
        ;;
      proto)
