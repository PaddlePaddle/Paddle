...
set +x
        ...
        if [[ "$on_precision" == "0" ]];then
            ...
            ctest -R "$UT_list_prec" -E "$disable_ut_quickly" -LE ${nightly_label} --timeout 500 --output-on-failure -j $2 | tee $tmpfile
            tmpfile_rand=`date +%s%N`
            tmpfile=$tmp_dir/$tmpfile_rand
            ctest -R "$UT_list_prec_1" -E "$disable_ut_quickly" -LE ${nightly_label} --timeout 500 --output-on-failure -j $2 | tee $tmpfile
        fi
            ...
            while ( [ $exec_times -lt $retry_time ] )
                do
                    ...
                    if [[ "$is_retry_execuate" == "0" ]];then
                        ...
                        failed_test_lists=''
                        ctest -R "$retry_unittests_regular" --timeout 500 --output-on-failure -j 2 | tee $tmpfile
                        collect_failed_tests
                        rm -f $tmp_dir/*
                        exec_times=$[$exec_times+1]
                    else 
