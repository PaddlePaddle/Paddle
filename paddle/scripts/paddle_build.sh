...
function precise_card_test() {
    ...
    env CUDA_VISIBLE_DEVICES=$cuda_list ctest -I 0,,1 -R "($testcases)" --timeout 500 --output-on-failure -V -j 1 | tee $tmpfile; test ${PIPESTATUS[0]} -eq 0
    set +m
}
...
function insert_pile_to_h_cu_diff {
    ...
    cd ${PADDLE_ROOT}
    find ${PADDLE_ROOT} -name '*.h' >> ${PADDLE_ROOT}/tools/h_cu_files.log
    find ${PADDLE_ROOT} -name '*.cu' >> ${PADDLE_ROOT}/tools/h_cu_files.log
...
function get_precise_tests_map_file {
    ...
    mkdir -p ${PADDLE_ROOT}/build/h_cu_file
    ...
    python pyCov_multithreading.py ${PADDLE_ROOT}
