...
function build_base() {
    ...
    if [ "$WITH_ARM" == "ON" ]; then
        make TARGET=ARMV8 -j ${parallel_number}
        build_error=$?
    else
        ...
        if [ -n "$failed_test_lists" ]; then
            ...
            if [ $need_retry_ut_count -lt $exec_retry_threshold ]; then
                while ([ $exec_times -lt $retry_time ]); do
                    ...
                    for line in ${retry_unittests[@]}; do
...
EOF
        ...
        if [ -a "$PADDLE_ROOT/added_ut" ]; then
            ...
            ctest -R "(${added_uts})" -LE "RUN_TYPE=DIST|RUN_TYPE=EXCLUSIVE" --output-on-failure --repeat-until-fail 3 --timeout 15
            added_ut_error=$?
            ctest -R "(${added_uts})" -L "RUN_TYPE=DIST|RUN_TYPE=EXCLUSIVE" --output-on-failure --repeat-until-fail 3 --timeout 15
            added_ut_error_1=$?
...
function assert_file_diff_approvals() {
    /bin/bash ${PADDLE_ROOT}/tools/check_file_diff_approvals.sh
    file_approval_error=$?
    ...
    for ((i = 0; i < $NUM_PROC; i++)); do
        ...
        if [ ${TESTING_DEBUG_MODE:-OFF} == "ON" ]; then
            if [[ $cardnumber == $CUDA_DEVICE_COUNT ]]; then
                (
                    ctest -I $i,,$NUM_PROC -R "($testcases)" -E "($disable_ut_quickly)" ${run_label_mode} -V --timeout 120 -j $parallel_job | tee $tmpfile
                    test ${PIPESTATUS[0]} -eq 0
                ) &
            else
                (
                    env CUDA_VISIBLE_DEVICES=$cuda_list ctest -I $i,,$NUM_PROC -R "($testcases)" -E "($disable_ut_quickly)" ${run_label_mode} --timeout 120 -V -j $parallel_job | tee $tmpfile
                    test ${PIPESTATUS[0]} -eq 0
                ) &
            fi
        else
            if [[ $cardnumber == $CUDA_DEVICE_COUNT ]]; then
                (
                    ctest -I $i,,$NUM_PROC -R "($testcases)" -E "($disable_ut_quickly)" ${run_label_mode} --timeout 120 --output-on-failure -j $parallel_job | tee $tmpfile
                    test ${PIPESTATUS[0]} -eq 0
                ) &
            else
                (
                    env CUDA_VISIBLE_DEVICES=$cuda_list ctest -I $i,,$NUM_PROC -R "($testcases)" -E "($disable_ut_quickly)" ${run_label_mode} --timeout 120 --output-on-failure -j $parallel_job | tee $tmpfile
                    test ${PIPESTATUS[0]} -eq 0
                ) &
            fi
        fi
...
EOF
        ...
        if [ -a "$PADDLE_ROOT/added_ut" ]; then
            added_uts=^$(awk BEGIN{RS=EOF}'{gsub(/\n/,"$|^");print}' $PADDLE_ROOT/added_ut)$
            env CUDA_VISIBLE_DEVICES=0 ctest -R "(${added_uts})" -LE "RUN_TYPE=DIST|RUN_TYPE=EXCLUSIVE" --output-on-failure --repeat-until-fail 3 --timeout 15
            added_ut_error=$?
            ctest -R "(${added_uts})" -L "RUN_TYPE=DIST|RUN_TYPE=EXCLUSIVE" --output-on-failure --repeat-until-fail 3 --timeout 15
            added_ut_error_1=$?
            if [ "$added_ut_error" != 0 ] && [ "$added_ut_error_1" != 0 ]; then
...
EOF
    ...
    cmake .. -DWITH_DISTRIBUTE=OFF -DON_INFER=ON -DWITH_TENSORRT=ON -DCUDA_ARCH_NAME=${CUDA_ARCH_NAME:-Auto} -DWITH_PYTHON=${WITH_PYTHON:-ON} -DWITH_ONNXRUNTIME=${WITH_ONNXRUNTIME:-OFF} -DWITH_MKL=${WITH_MKL:-ON} -DWITH_MKLDNN=${WITH_MKLDNN:-ON}
    build_error=$?
...
function exec_samplecode_test() {
    ...
    if [ "$1" = "cpu" ]; then
        ...
        python sampcd_processor.py --threads=${SAMPLE_CODE_EXEC_THREADS} gpu
        example_error=$?
    fi
...
EOF
    ...
    if [ "${PYTHON_EXECUTABLE}" != "" ]; then
        if [ "$SYSTEM" == "Darwin" ]; then
            ${PYTHON_EXECUTABLE} setup.py $2 --plat-name=macosx_10_9_x86_64
            build_error=$?
        else
            ${PYTHON_EXECUTABLE} setup.py $2
            build_error=$?
        fi
    else
        if [ "$SYSTEM" == "Darwin" ]; then
            python setup.py $2 --plat-name=macosx_10_9_x86_64
            build_error=$?
        else
            python setup.py $2
            build_error=$?
