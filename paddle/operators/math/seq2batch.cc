  ...
  void operator()(const platform::DeviceContext& context,
    ...
    for (size_t i = 0; i < max_sequence_length; ++i) {
      for (size_t j = 0; j < num_sequences; ++j) {
        size_t start_pos = lod[level][j];
        size_t sequence_length = lod[level][j + 1] - start_pos;
        T scale = norm_by_times ? (1.0 / static_cast<T>(sequence_length)) : 1.0;
 ...
 public:
  void operator()(const platform::DeviceContext& context,
    ...
    for (size_t i = 0; i < num_sequences; ++i) {
      size_t start_pos = lod[level][i];
      size_t sequence_length = lod[level][i + 1] - start_pos;
      T scale = norm_by_times ? (1.0 / static_cast<T>(sequence_length)) : 1.0;
