...
class DetectionOutputKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    float c_threshold = context.template Attr<float>("confidence_threshold");
    ...
    framework::Tensor loc_tr;
    framework::Tensor conf_tr;
    loc_tr.mutable_data<T>(loc_shape, context.GetPlace());
    conf_tr.mutable_data<T>(conf_shape, context.GetPlace());
    framework::Tensor loc_cpu;
    framework::Tensor conf_cpu;
    framework::Tensor priorbox_cpu;
    const T* p_data = in_p->data<T>();
    transpose_fun<DeviceContext, T>(context, *in_loc, &loc_tr);
    transpose_fun<DeviceContext, T>(context, *in_conf, &conf_tr);
    conf_tr.Resize(conf_shape_softmax);
    math::SoftmaxFunctor<DeviceContext, T>()(
        context.template device_context<DeviceContext>(), &conf_tr, &conf_tr);
    T* loc_data = loc_tr.data<T>();
    T* conf_data = conf_tr.data<T>();
    if (platform::is_gpu_place(context.GetPlace())) {
      loc_cpu.mutable_data<T>(loc_tr.dims(), platform::CPUPlace());
      framework::Copy(loc_tr, platform::CPUPlace(),
                          context.device_context(), &loc_cpu);
      loc_data = loc_cpu.data<T>();
      conf_cpu.mutable_data<T>(conf_tr.dims(), platform::CPUPlace());
      framework::Copy(conf_tr, platform::CPUPlace(),
                          context.device_context(), &conf_cpu);
      conf_data = conf_cpu.data<T>();
      priorbox_cpu.mutable_data<T>(in_p->dims(), platform::CPUPlace());
      framework::Copy(*in_p, platform::CPUPlace(), context.device_context(),
                          &priorbox_cpu);
      p_data = priorbox_cpu.data<T>();
    }
    size_t num_p = in_p->numel() / 8;
    ...
    int num_kept = math::GetDetectionIndices<T>(
        conf_data, num_p, classes, label_id, batch_size, c_threshold, nms_top_k,
