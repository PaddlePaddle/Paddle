      ...
      case 2: {
        if (pooling_type == "max") {
          paddle::operators::math::Pool2dFunctor<
              DeviceContextType, paddle::operators::math::MaxPool<T>, T>
              pool2d_forward;
          ...
          paddle::operators::math::Pool2dFunctor<
              DeviceContextType, paddle::operators::math::AvgPool<T>, T>
              pool2d_forward;
 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (in_x_grad) {
      ...
      temp.device(*context.template device_context<Place>().eigen_device()) =
          temp.constant(static_cast<T>(0));

      switch (ksize.size()) {
        case 2: {
          if (pooling_type == "max") {
            paddle::operators::math::MaxPool2dGradFunctor<DeviceContextType, T>
                pool2d_backward;
            pool2d_backward(dev_ctx, *in_x, *out, *out_grad, ksize, strides,
                            paddings, in_x_grad);
          } else if (pooling_type == "avg") {
            paddle::operators::math::Pool2dGradFunctor<
                DeviceContextType, paddle::operators::math::AvgPoolGrad<T>, T>
                pool2d_backward;
        ...
        case 3: {
          if (pooling_type == "max") {
            paddle::operators::math::MaxPool3dGradFunctor<DeviceContextType, T>
                pool3d_backward;
