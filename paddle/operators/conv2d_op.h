...
void GemmConv2DCompute(const framework::ExecutionContext& context) {
...
template <typename Place, typename T>
void GemmConvGrad2DCompute(const framework::ExecutionContext& context) {
  const Tensor* input = context.Input<Tensor>("Input");
  ...
  framework::DDim output_matrix_shape = {
      output_grad->dims()[1], output_grad->dims()[2] * output_grad->dims()[3]};
  ...
  if (filter_grad) {
    ...
    for (int i = 0; i < batch_size; i++) {
      ...
      for (int g = 0; g < groups; g++) {
        ...
        math::matmul<Place, T>(context.device_context(), out_grad_slice, false,
