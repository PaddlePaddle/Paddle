...
void AdadeltaInferMeta(const MetaTensor& param,
                       ...
                       const MetaTensor& master_param UNUSED,
                       float rho UNUSED,
                       ...
                       MetaTensor* master_param_out UNUSED) {
...
void AdamInferMeta(const MetaTensor& param,
                   ...
                   const MetaTensor& master_param UNUSED,
                   const MetaTensor& skip_update UNUSED,
                   const Scalar& beta1 UNUSED,
                   const Scalar& beta2 UNUSED,
                   const Scalar& epsilon UNUSED,
                   bool lazy_mode UNUSED,
                   int64_t min_row_size_to_use_multithread UNUSED,
                   bool multi_precision UNUSED,
                   bool use_global_beta_pow UNUSED,
                   MetaTensor* param_out,
                   MetaTensor* moment1_out,
                   MetaTensor* moment2_out,
                   MetaTensor* beta1_pow_out,
                   MetaTensor* beta2_pow_out,
                   MetaTensor* master_param_outs UNUSED) {
  auto lr_dims = learning_rate.dims();
  PADDLE_ENFORCE_EQ(
      phi::product(lr_dims),
      1,
...
void AdamwInferMeta(const MetaTensor& param,
                    ...
                    float lr_ratio UNUSED,
                    float coeff UNUSED,
                    bool with_decay UNUSED,
                    bool lazy_mode,
...
void AucInferMeta(const MetaTensor& input,
                  ...
                  const MetaTensor& stat_pos UNUSED,
                  const MetaTensor& stat_neg UNUSED,
                  const MetaTensor& ins_tag_weight UNUSED,
                  const std::string& curve UNUSED,
                  int num_thresholds,
                  int slide_steps,
                  MetaTensor* auc,
...
void AverageAccumulatesInferMeta(const MetaTensor& param UNUSED,
                                 ...
                                 float average_window UNUSED,
                                 int64_t max_average_window UNUSED,
                                 int64_t min_average_window UNUSED,
...
void BatchNormInferMeta(const MetaTensor& x,
                        const MetaTensor& mean UNUSED,
                        const MetaTensor& variance UNUSED,
...
void CoalesceTensorInferMeta(const std::vector<const MetaTensor*>& input,
                             DataType dtype,
                             bool copy_data UNUSED,
                             bool set_constant UNUSED,
                             bool persist_output UNUSED,
                             float constant UNUSED,
                             bool use_align,
                             int align_size,
                             int size_of_dtype,
                             const std::vector<int64_t>& concated_shapes UNUSED,
                             const std::vector<int64_t>& concated_ranks UNUSED,
                             std::vector<MetaTensor*> output UNUSED,
...
void CheckMemoryContinueInferMeta(const std::vector<const MetaTensor*>& input,
                                  MetaTensor* output,
                                  std::vector<MetaTensor*> xout UNUSED,
                                  MetaConfig config) {
  if (config.is_runtime) {
    return;
  }
...
void GenerateProposalsV2InferMeta(const MetaTensor& scores UNUSED,
                                  const MetaTensor& bbox_deltas UNUSED,
                                  const MetaTensor& im_shape UNUSED,
                                  const MetaTensor& anchors UNUSED,
                                  const MetaTensor& variances UNUSED,
                                  int pre_nms_top_n UNUSED,
                                  int post_nms_top_n UNUSED,
                                  float nms_thresh UNUSED,
                                  float min_size UNUSED,
                                  float eta UNUSED,
                                  bool pixel_offset UNUSED,
                                  MetaTensor* rpn_rois,
                                  MetaTensor* rpn_roi_probs,
                                  MetaTensor* rpn_rois_num UNUSED) {
...
void HSigmoidLossInferMeta(const MetaTensor& x,
                           const MetaTensor& label,
                           const MetaTensor& w UNUSED,
                           const MetaTensor& bias UNUSED,
                           const MetaTensor& path UNUSED,
                           const MetaTensor& code UNUSED,
                           int num_classes UNUSED,
                           bool is_sparse UNUSED,
                           MetaTensor* out,
                           MetaTensor* pre_out UNUSED,
                           MetaTensor* w_out UNUSED) {
...
void IndexPutInferMeta(const MetaTensor& x,
                       const std::vector<const MetaTensor*>& indices UNUSED,
                       const MetaTensor& value UNUSED,
                       bool accumulate UNUSED,
                       MetaTensor* out) {
  auto in_dims = x.dims();
  PADDLE_ENFORCE_LT(
      in_dims.size(),
...
void MergedAdamInferMeta(
    const std::vector<const MetaTensor*>& param UNUSED,
    const std::vector<const MetaTensor*>& grad UNUSED,
    const std::vector<const MetaTensor*>& learning_rate UNUSED,
    const std::vector<const MetaTensor*>& moment1 UNUSED,
    const std::vector<const MetaTensor*>& moment2 UNUSED,
    const std::vector<const MetaTensor*>& beta1_pow UNUSED,
    const std::vector<const MetaTensor*>& beta2_pow UNUSED,
    const paddle::optional<std::vector<const MetaTensor*>>& master_param UNUSED,
    const Scalar& beta1 UNUSED,
    const Scalar& beta2 UNUSED,
    const Scalar& epsilon UNUSED,
    bool multi_precision UNUSED,
    bool use_global_beta_pow UNUSED,
    std::vector<MetaTensor*> param_out UNUSED,
    std::vector<MetaTensor*> moment1_out UNUSED,
    std::vector<MetaTensor*> moment2_out UNUSED,
    std::vector<MetaTensor*> beta1_pow_out UNUSED,
    std::vector<MetaTensor*> beta2_pow_out UNUSED,
    std::vector<MetaTensor*> master_param_out UNUSED) {}

void MergedMomentumInferMeta(
    const std::vector<const MetaTensor*>& param UNUSED,
    const std::vector<const MetaTensor*>& grad UNUSED,
    const std::vector<const MetaTensor*>& velocity UNUSED,
    const std::vector<const MetaTensor*>& learning_rate UNUSED,
    const paddle::optional<std::vector<const MetaTensor*>>& master_param UNUSED,
    float mu UNUSED,
    bool use_nesterov UNUSED,
    const std::vector<std::string>& regularization_method UNUSED,
    const std::vector<float>& regularization_coeff UNUSED,
    bool multi_precision UNUSED,
    float rescale_grad UNUSED,
    std::vector<MetaTensor*> param_out UNUSED,
    std::vector<MetaTensor*> velocity_out UNUSED,
    std::vector<MetaTensor*> master_param_out UNUSED) {}
...
void MemoryEfficientAttentionInferMeta(const MetaTensor& query,
                                       ...
                                       const MetaTensor& bias UNUSED,
                                       const MetaTensor& cu_seqlens_q UNUSED,
                                       const MetaTensor& cu_seqlens_k UNUSED,
                                       const MetaTensor& causal_diagonal UNUSED,
                                       const MetaTensor& seqlen_k UNUSED,
                                       const Scalar& max_seqlen_q UNUSED,
                                       const Scalar& max_seqlen_k UNUSED,
                                       const bool causal UNUSED,
                                       const double dropout_p UNUSED,
                                       const float scale UNUSED,
                                       const bool is_test UNUSED,
                                       MetaTensor* output,
...
void MomentumInferMeta(const MetaTensor& param,
                       const MetaTensor& grad UNUSED,
                       const MetaTensor& velocity UNUSED,
                       const MetaTensor& learning_rate,
                       const MetaTensor& master_param UNUSED,
                       float mu UNUSED,
                       bool use_nesterov UNUSED,
                       const std::string& regularization_method UNUSED,
                       float regularization_coeff UNUSED,
                       bool multi_precision UNUSED,
                       float rescale_grad UNUSED,
                       MetaTensor* param_out,
...
void RnnInferMeta(const MetaTensor& x,
                  ...
                  const std::vector<const MetaTensor*>& weight_list UNUSED,
                  const MetaTensor& sequence_length,
                  float dropout_prob UNUSED,
                  bool is_bidirec,
                  int input_size UNUSED,
                  int hidden_size,
                  int num_layers UNUSED,
                  const std::string& mode,
                  int seed UNUSED,
                  bool is_test UNUSED,
                  MetaTensor* out,
                  MetaTensor* dropout_state UNUSED,
                  std::vector<MetaTensor*> state,
                  MetaTensor* reserve UNUSED) {
  auto in_dims = x.dims();
...
void SendUERecvInferMeta(const MetaTensor& x,
                         ...
                         const std::string& message_op UNUSED,
                         const std::string& reduce_op,
                         const IntArray& out_size UNUSED,
                         MetaTensor* out,
...
void ShareBufferInferMeta(const std::vector<const MetaTensor*>& xs,
                          ...
                          std::vector<MetaTensor*> outs,
                          std::vector<MetaTensor*> xouts UNUSED) {
...
void UpdateLossScalingInferMeta(const std::vector<const MetaTensor*>& xs,
                                const MetaTensor& found_infinite UNUSED,
                                const MetaTensor& prev_loss_scaling UNUSED,
                                const MetaTensor& in_good_steps UNUSED,
                                const MetaTensor& in_bad_steps UNUSED,
                                std::vector<MetaTensor*> outs,
...
void WarpctcInferMeta(const MetaTensor& logits,
                      const MetaTensor& label UNUSED,
                      const MetaTensor& logits_length,
                      const MetaTensor& labels_length UNUSED,
                      int blank,
                      bool norm_by_times UNUSED,
                      MetaTensor* loss,
                      MetaTensor* warpctcgrad UNUSED) {
...
void WarprnntInferMeta(const MetaTensor& input,
                       const MetaTensor& label UNUSED,
                       const MetaTensor& input_lengths UNUSED,
                       const MetaTensor& label_lengths UNUSED,
                       int blank,
...
void YoloLossInferMeta(const MetaTensor& x,
                       ...
                       int downsample_ratio UNUSED,
                       bool use_label_smooth UNUSED,
...
void FusedAdamInferMeta(
    ...
    const std::vector<const MetaTensor*>& grads UNUSED,
    const MetaTensor& learning_rate UNUSED,
    ...
    const Scalar& beta2 UNUSED,
    const Scalar& epsilon UNUSED,
    int chunk_size UNUSED,
    float weight_decay UNUSED,
    bool use_adamw UNUSED,
    bool multi_precision UNUSED,
    bool use_global_beta_pow UNUSED,
...
void MoeInferMeta(const MetaTensor& x,
                  const MetaTensor& gate UNUSED,
                  const MetaTensor& bmm0 UNUSED,
                  const MetaTensor& bias0 UNUSED,
                  const MetaTensor& bmm1 UNUSED,
                  const MetaTensor& bias1 UNUSED,
                  const std::string& act_type UNUSED,
                  MetaTensor* out) {
  out->set_dims(x.dims());
  out->share_lod(x);
  out->set_dtype(x.dtype());
