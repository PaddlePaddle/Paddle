...
void AdamInferMeta(const MetaTensor& param,
                   ...
                   const paddle::optional<MetaTensor>& master_param,
                   const paddle::optional<MetaTensor>& skip_update,
                   const Scalar& beta1,
                   const Scalar& beta2,
...
void HierarchicalSigmoidInferMeta(const MetaTensor& x,
                                  const MetaTensor& w,
                                  const MetaTensor& label,
                                  const paddle::optional<MetaTensor>& path,
                                  const paddle::optional<MetaTensor>& code,
                                  const paddle::optional<MetaTensor>& bias,
                                  int num_classes,
...
static void Interpolate1DInferShapeCheck(
    const MetaTensor& x,
    const paddle::optional<MetaTensor>& out_size,
    paddle::optional<const std::vector<const MetaTensor*>> size_tensor,
    const paddle::optional<MetaTensor>& scale_tensor,
    const std::string& data_layout_str,
    int out_d,
    int out_h,
    int out_w,
...
void RmspropInferMeta(const MetaTensor& param,
                      ...
                      const MetaTensor& moment,
                      const MetaTensor& learning_rate,
                      const paddle::optional<MetaTensor>& mean_grad,
                      float epsilon,
                      float decay,
                      float momentum,
                      bool centered,
...
void WarpctcInferMeta(const MetaTensor& logits,
                      const MetaTensor& label,
                      const paddle::optional<MetaTensor>& logits_length,
                      const paddle::optional<MetaTensor>& labels_length,
                      int blank,
                      bool norm_by_times,
...
void GraphReindexInferMeta(const MetaTensor& x,
                           ...
                           const paddle::optional<MetaTensor>& hashtable_value,
                           const paddle::optional<MetaTensor>& hashtable_index,
...
void GraphSampleNeighborsInferMeta(
    ...
    const MetaTensor& x,
    const paddle::optional<MetaTensor>& eids,
    const paddle::optional<MetaTensor>& perm_buffer,
    int sample_size,
    bool return_eids,
...
void Yolov3LossInferMeta(const MetaTensor& x,
                         ...
                         const paddle::optional<MetaTensor>& gt_score,
