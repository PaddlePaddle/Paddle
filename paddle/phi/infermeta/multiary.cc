...
void AdadeltaInferMeta(const MetaTensor& param,
  ...
  auto MPType = (param.dtype() == DataType::FLOAT16 ||
                 param.dtype() == DataType::BFLOAT16)
                    ? DataType::FLOAT32
                    : param.dtype();
...
void BroadcastTensorsInferMeta(const std::vector<const MetaTensor*>& x,
  ...
  for (size_t i = 0; i < out.size(); i++) {
    out[i]->set_dims(make_ddim(target_dims));
    out[i]->share_lod(*(x[i]));
...
void CoalesceTensorInferMeta(const std::vector<const MetaTensor*>& input,
  ...
  if (config.is_runtime) {
    ...
    if (fused_output) {
      fused_output->set_dims(make_ddim({numel}));
...
void CheckMemoryContinueInferMeta(const std::vector<const MetaTensor*>& input,
  ...
  output->set_dtype(DataType::INT8);
}
...
void ConcatInferMeta(const std::vector<const MetaTensor*>& x,
  ...
  DDim out_dim = funcs::ComputeAndCheckShape(config.is_runtime, x_dims, axis);
...
void ChunkEvalInferMeta(const MetaTensor& inference,
  ...
  precision->set_dtype(DataType::FLOAT32);
  recall->set_dtype(DataType::FLOAT32);
  f1_score->set_dtype(DataType::FLOAT32);
  num_infer_chunks->set_dtype(DataType::INT64);
  num_label_chunks->set_dtype(DataType::INT64);
  num_correct_chunks->set_dtype(DataType::INT64);
}
...
void CudnnLSTMInferMeta(
  ...
  reserve->set_dtype(DataType::UINT8);
  state_out->set_dtype(DataType::UINT8);
}
...
void LSTMInferMeta(const MetaTensor& input,
  ...
  DDim out_dims({in_dims[0], frame_size});
  hidden->set_dims(out_dims);
  cell->set_dims(out_dims);
  if (!is_test) {
...
inline int64_t ConvOutputSize(
  ...
  int64_t output_size = (input_size + 2 * padding - dkernel) / stride + 1;
  PADDLE_ENFORCE_GT(
      output_size,
      0,
      common::errors::InvalidArgument(
...
void EditDistanceInferMeta(const MetaTensor& hyps,
  ...
  sequencenum->set_dims(make_ddim({1}));
...
void Fp8GemmBlockwiseInferMeta(const MetaTensor& A,
  ...
  PADDLE_ENFORCE_EQ(input_result.dtype() == DataType::FLOAT32 ||
                        input_result.dtype() == DataType::BFLOAT16,
                    true,
                    errors::InvalidArgument(
...
void FusedBiasActInferMeta(const MetaTensor& x,
  ...
  if (act_method == "geglu" || act_method == "swiglu") {
    ...
    out->set_dims(make_ddim(x_shapes));
...
void FusedLayerNormInferMeta(const MetaTensor& x,
  ...
  mean->set_dims(make_ddim({rows}));
  mean->set_dtype(DataType::FLOAT32);
  mean->set_layout(x.layout());

  variance->set_dims(make_ddim({rows}));
...
void FusedLinearParamGradAddInferMeta(const MetaTensor& x,
  ...
  const DDim& weight_dims = {x_dims[rank - 1], dout_dims[rank - 1]};
  if (dweight) {
    PADDLE_ENFORCE_EQ(
        weight_dims,
        dweight.dims(),
...
void GenerateProposalsV2InferMeta(const MetaTensor& scores,
  ...
  rpn_rois->set_dims(make_ddim({-1, 4}));
  rpn_roi_probs->set_dims(make_ddim({-1, 1}));
  if (rpn_rois_num) {
    rpn_rois_num->set_dims(make_ddim({scores.dims()[0]}));
  }
}
...
void GraphKhopSamplerInferMeta(const MetaTensor& row,
  ...
  auto GKSShapeCheck = [](const DDim& dims, std::string tensor_name) {
    if (dims.size() == 2) {
      PADDLE_ENFORCE_EQ(dims[1],
                        1,
                        common::errors::InvalidArgument(
...
void GraphReindexInferMeta(const MetaTensor& x,
  ...
  auto GraphReindexShapeCheck = [](const DDim& dims, std::string tensor_name) {
    if (dims.size() == 2) {
      PADDLE_ENFORCE_EQ(dims[1],
                        1,
                        common::errors::InvalidArgument(
...
void GraphSampleNeighborsInferMeta(const MetaTensor& row,
  ...
  auto GSNShapeCheck = [](const DDim& dims, std::string tensor_name) {
    if (dims.size() == 2) {
      PADDLE_ENFORCE_EQ(dims[1],
                        1,
                        common::errors::InvalidArgument(
  ...
  if (scale_tensor) {
    ...
    if (!scale.empty()) {
      ...
      if (scale_w > 0.) {
        // round down
        out_w_tmp =
            static_cast<int64_t>(data_layout == DataLayout::NCHW
...
static void Interpolate2DInferShapeCheck(
  ...
  int64_t out_h_tmp = 0, out_w_tmp = 0;
...
static void Interpolate3DInferShapeCheck(
  ...
  int64_t out_d_tmp = 0, out_h_tmp = 0, out_w_tmp = 0;
  if (scale_tensor) {
    auto scale_tensor_dim = scale_tensor.dims();
    PADDLE_ENFORCE_EQ(
        scale_tensor_dim.size() == 1 || scale_tensor_dim.size() == 0,
...
void LarsMomentumInferMeta(
  ...
  for (size_t i = 0; i < param_out.size(); i++) {
    auto MPType = (param[i]->dtype() == DataType::FLOAT16 ||
                   param[i]->dtype() == DataType::BFLOAT16)
                      ? DataType::FLOAT32
                      : param[i]->dtype();
...
void MemoryEfficientAttentionInferMeta(const MetaTensor& query,
  ...
  output->set_dims(make_ddim(out_dims));
  output->share_lod(query);
  output->set_dtype(query.dtype());
  output->set_layout(query.layout());

  logsumexp->set_dims(make_ddim(logsumexp_dims));
  logsumexp->set_dtype(DataType::FLOAT32);

  seed_and_offset->set_dims(make_ddim(seed_and_offset_dims));
  seed_and_offset->set_dtype(DataType::INT64);
}
...
void MeshgridInferMeta(const std::vector<const MetaTensor*>& inputs,
  ...
  std::vector<int64_t> out_shape = std::vector<int64_t>(inputs_num);
...
void NceInferMeta(const MetaTensor& input,
  ...
  cost->set_dims(make_ddim(out_dims));
  ...
  if (!is_test) {
    ...
    sample_logits->set_dims(make_ddim(sample_out_dims));
    sample_labels->set_dims(make_ddim(sample_out_dims));
  }
}
...
void PyramidHashInferMeta(const MetaTensor& x,
  ...
  if (config.is_runtime) {
    ...
    // compile time
    out->set_dims(make_ddim({-1, num_emb}));
...
void FusedRmsNormQuantInferMeta(const MetaTensor& x,
  ...
  if (inv_var != nullptr) {
    inv_var->set_dtype(DataType::FLOAT32);
    ...
    inv_var->set_dims(make_ddim(inv_var_dims));
...
void SigmoidCrossEntropyWithLogitsInferMeta(const MetaTensor& x,
  ...
  if (check) {
    PADDLE_ENFORCE_EQ(
        slice_ddim(x_dims, 0, rank),
        slice_ddim(labels_dims, 0, rank),
        common::errors::InvalidArgument(
...
void StackInferMeta(const std::vector<const MetaTensor*>& x,
  ...
  auto out_dim = funcs::ComputeAndCheckShape(config.is_runtime, input_dims, -1);
...
void YoloBoxPostInferMeta(const MetaTensor& boxes0,
  ...
  out->set_dims(make_ddim({1, 6}));
  nms_rois_num->set_dims(make_ddim({batch}));
  out->set_dtype(DataType::FLOAT32);
...
void MoePermuteInferMeta(const MetaTensor& X,
  ...
  PADDLE_ENFORCE_EQ(expert_routemap_topk.dtype() == DataType::INT32,
                    true,
                    common::errors::InvalidArgument(
                        "Input expert_routemap_topk's dtype should be INT32"));
  PADDLE_ENFORCE_EQ(expert_prob_topk.dtype() == DataType::FLOAT32,
                    true,
                    common::errors::InvalidArgument(
...
void MoeUnpermuteInferMeta(const MetaTensor& unzipped_tokens,
  ...
  PADDLE_ENFORCE_EQ(
      unzipped_token_probs.dtype() == DataType::FLOAT32,
      true,
      common::errors::InvalidArgument(
...
void FusedRopeInferMeta(const MetaTensor& q,
                        ...
                        MetaTensor* out_k,
                        MetaTensor* out_v) {
  // Validate q: must be 4-D [batch_size, seq_len, num_heads, head_dim]
  // or [seq_len, batch_size, num_heads, head_dim]
  auto q_dims = q.dims();
  PADDLE_ENFORCE_EQ(q_dims.size(),
                    4,
                    common::errors::InvalidArgument(
                        "Input q should be a 4-D tensor of format "
                        "[batch_size, seq_len, num_heads, head_dim] "
                        "or [seq_len, batch_size, num_heads, head_dim], "
                        "but got %d-D.",
                        q_dims.size()));

  auto batch_size = time_major ? q_dims[1] : q_dims[0];
  auto seq_len = time_major ? q_dims[0] : q_dims[1];
  auto num_heads = q_dims[2];
  auto head_dim = q_dims[3];
  ...
  if (k) {
    ...
    PADDLE_ENFORCE_EQ(k_dims.size(),
                      4,
                      common::errors::InvalidArgument(
                          "Input k should be a 4-D tensor of format "
                          "[batch_size, seq_len, num_heads, head_dim] "
                          "or [seq_len, batch_size, num_heads, head_dim], "
                          "but got %d-D.",
                          k_dims.size()));
    ...
    PADDLE_ENFORCE_EQ(k_head_dim % 2,
                      0,
                      common::errors::InvalidArgument(
                          "The head_dim of k must be a multiple of 2, "
                          "but got head_dim = %d.",
                          k_head_dim));

    PADDLE_ENFORCE_EQ(k_head_dim,
                      head_dim,
                      common::errors::InvalidArgument(
                          "The head_dim of k (%d) must be equal to "
                          "the head_dim of q (%d).",
                          k_head_dim,
                          head_dim));

    auto k_batch_size = time_major ? k_dims[1] : k_dims[0];
    auto k_seq_len = time_major ? k_dims[0] : k_dims[1];
    auto k_num_heads = k_dims[2];

    PADDLE_ENFORCE_GT(k_batch_size,
                      0,
                      common::errors::InvalidArgument(
                          "The batch_size of k must be greater than 0, "
                          "but got %d.",
                          k_batch_size));

    PADDLE_ENFORCE_GT(k_seq_len,
                      0,
                      common::errors::InvalidArgument(
                          "The seq_len of k must be greater than 0, "
                          "but got %d.",
                          k_seq_len));

    PADDLE_ENFORCE_GT(k_num_heads,
                      0,
                      common::errors::InvalidArgument(
                          "The num_heads of k must be greater than 0, "
                          "but got %d.",
                          k_num_heads));

    PADDLE_ENFORCE_EQ(k_seq_len,
                      seq_len,
                      common::errors::InvalidArgument(
                          "The seq_len of k (%d) must be equal to "
                          "the seq_len of q (%d).",
                          k_seq_len,
                          seq_len));
  }
  ...
  if (v) {
    ...
    PADDLE_ENFORCE_EQ(v_dims.size(),
                      4,
                      common::errors::InvalidArgument(
                          "Input v should be a 4-D tensor of format "
                          "[batch_size, seq_len, num_heads, head_dim] "
                          "or [seq_len, batch_size, num_heads, head_dim], "
                          "but got %d-D.",
                          v_dims.size()));
    ...
    PADDLE_ENFORCE_EQ(v_head_dim % 2,
                      0,
                      common::errors::InvalidArgument(
                          "The head_dim of v must be a multiple of 2, "
                          "but got head_dim = %d.",
                          v_head_dim));

    PADDLE_ENFORCE_EQ(v_head_dim,
                      head_dim,
                      common::errors::InvalidArgument(
                          "The head_dim of v (%d) must be equal to "
                          "the head_dim of q (%d).",
                          v_head_dim,
                          head_dim));

    auto v_batch_size = time_major ? v_dims[1] : v_dims[0];
    auto v_seq_len = time_major ? v_dims[0] : v_dims[1];
    auto v_num_heads = v_dims[2];

    PADDLE_ENFORCE_GT(v_batch_size,
                      0,
                      common::errors::InvalidArgument(
                          "The batch_size of v must be greater than 0, "
                          "but got %d.",
                          v_batch_size));

    PADDLE_ENFORCE_GT(v_seq_len,
                      0,
                      common::errors::InvalidArgument(
                          "The seq_len of v must be greater than 0, "
                          "but got %d.",
                          v_seq_len));

    PADDLE_ENFORCE_GT(v_num_heads,
                      0,
                      common::errors::InvalidArgument(
                          "The num_heads of v must be greater than 0, "
                          "but got %d.",
                          v_num_heads));

    PADDLE_ENFORCE_EQ(v_seq_len,
                      seq_len,
                      common::errors::InvalidArgument(
                          "The seq_len of v (%d) must be equal to "
                          "the seq_len of q (%d).",
                          v_seq_len,
                          seq_len));
  }
...
void FullWithTensorInferMeta(const IntArray& shape,
                             DataType dtype,
                             MetaTensor* out) {
  out->set_dims(make_ddim(shape.GetData()));
  out->set_dtype(dtype);
}
