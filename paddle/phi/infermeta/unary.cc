...
void DecodeJpegInferMeta(const MetaTensor& x,
  ...
  void DiagInferMeta(
      const MetaTensor& x, int offset, float padding_value, MetaTensor* out) {
    auto x_dims = x.dims();
  ...
  void EigvalsInferMeta(
      const MetaTensor& x, MetaTensor* out, MetaConfig config) {
    auto x_dims = x.dims();
  ...
  void ExpandInferMeta(
      const MetaTensor& x, const IntArray& shape, MetaTensor* out) {
#define MAX_RANK_SUPPORTED 6
  ...
  void FlattenInferMeta(
      const MetaTensor& x, int start_axis, int stop_axis, MetaTensor* out) {
  ...
  void FlipInferMeta(
      const MetaTensor& x, const std::vector<int>& axis, MetaTensor* out) {
    auto x_dims = x.dims();
    auto flip_dims = axis;
  ...
  void InferMetaFromVecValue(
      const MetaTensor& x, const std::vector<int64_t>& shape, MetaTensor* out) {
  ...
  void MaxOutInferMeta(
      const MetaTensor& x, int groups, int axis, MetaTensor* out) {
  ...
  void MultinomialInferMeta(
      const MetaTensor& x, int num_samples, bool replacement, MetaTensor* out) {
    auto x_dim = x.dims();
  ...
  void RReluGradInferMeta(
      const MetaTensor& out_grad, const MetaTensor& noise, MetaTensor* x_grad) {
    auto do_dims = out_grad.dims();
  ...
  void SqueezeInferMeta(
      const MetaTensor& x, const std::vector<int>& axes, MetaTensor* out) {
    const auto& x_dims = x.dims();
...
#define MAX_RANK_SUPPORTED 6
  ...
  void TransferLayoutInferMeta(
      const MetaTensor& x, DataLayout layout, MetaTensor* out) {
    out->set_dims(x.dims());
    out->set_dtype(x.dtype());
    out->set_layout(layout);
  }

  void TransposeInferMeta(
      const MetaTensor& x, const std::vector<int>& axis, MetaTensor* out) {
    auto x_dims = x.dims();
  ...
  void UnbindInferMeta(
      const MetaTensor& x, int axis, std::vector<MetaTensor*> outs) {
    auto in_dims = x.dims();
  ...
  void TrilTriuInferMeta(
      const MetaTensor& x, int diagonal, bool lower, MetaTensor* out) {
    const auto& x_dims = x.dims();
    PADDLE_ENFORCE_GE(x_dims.size(),
                      2,
                      phi::errors::InvalidArgument(
  ...
  void UnchangedInferMetaCheckAxis(
      const MetaTensor& x, int axis, MetaTensor* out) {
  ...
  void UnStackInferMeta(
      const MetaTensor& x, int axis, int num, std::vector<MetaTensor*> outs) {
    auto x_dim = x.dims();
    int rank = x_dim.size();
    PADDLE_ENFORCE_GE(axis,
                      -rank,
  ...
  void OneHotInferMeta(
      const MetaTensor& x, const Scalar& depth_t, MetaTensor* out) {
    auto x_dims = x.dims();
    PADDLE_ENFORCE_GE(
        x_dims.size(),
        1,
  ...
  void IdentityLossInferMeta(
      const MetaTensor& x, int reduction, MetaTensor* out) {
    if (reduction == 2) {
      out->set_dtype(x.dtype());
      out->set_dims(x.dims());
    } else {
