...
// The target shape in squeeze may contains a -1 dimension,
...
std::vector<DimTrans*> MakeSqueezeDimTrans(
...
SpmdInfo SqueezeInferSpmd(const DistMetaTensor& x,
  ...
  std::vector<DimTrans*> trans = MakeSqueezeDimTrans(src_shape, tgt_shape);
...
SpmdInfo SqueezeInferSpmdReverse(const DistMetaTensor& x,
  ...
  std::vector<DimTrans*> trans = MakeSqueezeDimTrans(out_shape, x_shape);
