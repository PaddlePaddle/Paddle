...
SpmdInfo ExpandAsGradInferSpmd(const DistMetaTensor& x,
  ...
  const auto& x_dims_mapping = x_dist_attr.get_dims_mapping();
  const auto& y_dims_mapping = y_dist_attr.get_dims_mapping();

  // handle partial grad
  auto x_grad_dist_attr = CopyTensorDistAttrForOutput(x_dist_attr);
  ...
  x_grad_dist_attr.set_partial(partial);
