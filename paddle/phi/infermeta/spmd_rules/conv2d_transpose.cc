...
SpmdInfo Conv2dTransposeInferSpmd(const DistMetaTensor& input,
  ...
  // Step0: verify input args based on conv2d_transpose logic
  VLOG(4) << "step 0: verify input args based on conv2d_transpose logic";
  ...
  PADDLE_ENFORCE_EQ(input_ndim,
                    ...
                    common::errors::InvalidArgument(
                        ...
                        "conv2d_transpose, for NCHW or NHWC format."
                        "But now it's [%d]",
                        input_ndim));
  ...
  PADDLE_ENFORCE_EQ(
      ...
      common::errors::InvalidArgument("The Tensor Filter's rank must be 4 in "
                                      "conv2d_transpose, for CMHW format."
                                      "But now it's [%d]",
                                      filter_ndim));
  ...
  VLOG(6) << "Conv2DTranspose InferForward Inputs: "
  ...
  std::string input_axes = (data_format == "NCHW") ? "nchw" : "nhwc";
  std::string filter_axes = "cmhw";
  ...
  VLOG(4) << "Conv2DTransposeSPMDRule InferForward: "
...
SpmdInfo Conv2dTransposeGradInferSpmd(const DistMetaTensor& input,
                                      const DistMetaTensor& filter,
