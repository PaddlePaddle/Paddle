...
ShardingMergeForTensorsOnePair(
    const std::vector<
        std::pair<std::string, std::vector<std::vector<int64_t>>>>&
        tensor_axes_to_dim_pairs,
    const bool merge_conflicts) {
  // TODO(ooooo): ShardingMergeForTensorsOnePair is just to check whether a mesh
  // dim sharding on more than one tensor axes.
  PADDLE_ENFORCE_EQ(
      tensor_axes_to_dim_pairs.size(),
      1,
      phi::errors::InvalidArgument(
          "The size of tensor_axes_to_dim_pairs in "
          "ShardingMergeForTensorsOnePair should be 1, but got [%d].",
          tensor_axes_to_dim_pairs.size()));
  const auto& axes = tensor_axes_to_dim_pairs[0].first;
  const auto& dim_map = tensor_axes_to_dim_pairs[0].second;
  std::unordered_map<std::string, std::vector<int64_t>> axis_to_dim_map;
  // tensor axis -> mesh dims
  for (size_t i = 0; i < dim_map.size(); ++i) {
    axis_to_dim_map[axes.substr(i, 1)] = dim_map[i];
  }

  std::unordered_map<int64_t, std::string> dim_to_axis_map;
  // mesh dim -> tensor axes
  for (size_t i = 0; i < dim_map.size(); ++i) {
    const auto tensor_axis = axes.substr(i, 1);
    const auto& mesh_dims = dim_map[i];
    for (const auto dim : mesh_dims) {
      if (dim_to_axis_map.count(dim) == 0) {
        dim_to_axis_map.insert({dim, tensor_axis});
      } else if (dim_to_axis_map[dim].find(tensor_axis) == std::string::npos) {
        dim_to_axis_map[dim] += tensor_axis;
      }
    }
  }
  // ["a": [1], "b": [0, 1]]
  // {0: "a", 1: "ab"}, the mesh dim is mapped to the two tensor axes.
  // We keep the first one -> ["a": [1], "b": [0]]
  for (const auto& it : dim_to_axis_map) {
    const auto& conflicting_axes_str = it.second;
    const auto& mesh_dim = it.first;

    if (conflicting_axes_str.size() > 1) {
      if (merge_conflicts) {
        VLOG(4) << "Sharding Conflict: Mesh_Dim [" << mesh_dim
                << "] are Sharding Multiple Tensor Axis: ["
                << conflicting_axes_str << "]. The Axis: ["
                << conflicting_axes_str[0] << "] is Picked.";

        for (size_t i = 1; i < conflicting_axes_str.size(); ++i) {
          const auto axis_to_modify = conflicting_axes_str.substr(i, 1);
          auto& dim_vector_to_modify = axis_to_dim_map.at(axis_to_modify);
          auto new_end = std::remove(dim_vector_to_modify.begin(),
                                     dim_vector_to_modify.end(),
                                     mesh_dim);
          dim_vector_to_modify.erase(new_end, dim_vector_to_modify.end());
        }
      } else {
        PADDLE_THROW(common::errors::PreconditionNotMet(
            "Multiple Tensor Axes [%s] is sharded by same mesh dimension [%d].",
            str_join(conflicting_axes_str),
            mesh_dim));
      }
    }
  }
