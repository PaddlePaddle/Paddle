...
inline void check_shape(const DDim& dims) {
  ...
  PADDLE_ENFORCE(
      valid,
      phi::errors::InvalidArgument("the SparseCsrTensor only supports 2-D or "
                                   "3-D Tensor, but gets %d-D Tensor",
                                   dims.size()));
}
...
void SparseCsrTensor::SetMembers(const DenseTensor& non_zero_crows,
                                 const DenseTensor& non_zero_cols,
                                 const DenseTensor& non_zero_elements,
                                 const DDim& dims) {
