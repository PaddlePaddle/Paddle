...
void IpcCollect() {
  ...
  while (iter != ipc_handle_to_baseptr_.end()) {
    if (iter->second.expired()) {
      VLOG(6) < "XpuIpcCollect: removing expired IPC handle: " < iter->first;
  ...
  VLOG(3) < "XpuIpcCollect: collected " < collected_count <
      " expired IPC handles " < "out of " < total_count < " total handles";
}
