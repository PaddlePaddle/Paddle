...
struct DenseTensorMeta {
  ...
  /// \brief Determine whether using OneDNN speed-up library in the new dygraph.
  /// It will also support CuDNN library in the near future.
  bool use_dnn{false};
  DDim dims;
...
inline bool operator==(const DenseTensorMeta& lhs, const DenseTensorMeta& rhs) {
  return (lhs.is_scalar == rhs.is_scalar) && (lhs.use_dnn == rhs.use_dnn) &&
