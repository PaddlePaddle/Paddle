...
ProcessGroupNCCL::NCCLTask::NCCLTask(const Place& place,
    ...
    : TaskStream(rank, comm_type, sync_op, use_calc_stream),
      comm_event_(place, paddle::platform::GenerateDeviceEventFlag()),
      task_place_(place),
      gid_(gid) {}
...
bool ProcessGroupNCCL::NCCLTask::Wait(std::chrono::milliseconds timeout) {
  ...
  comm_event_.Wait(paddle::platform::Place2DeviceType(task_place_), calc_ctx);
  ...
  return Collective(
      [&](phi::distributed::NCCLCommContext* comm_context, gpuStream_t stream) {
        ...
        VLOG(3) << "[AllToAll] "
                ...
                << paddle::string::join_strings(out_size_each_rank, ',')
                << ", in_size_each_rank: "
                << paddle::string::join_strings(in_size_each_rank, ',')
                << ", sync_op: " << sync_op
                << ", use_calc_stream: " << use_calc_stream << ", "
                << GetGroupMessage();
...
void ProcessGroupNCCL::CreateNCCLEnvCache(const Place& place,
  ...
  place_to_calc_event_.emplace(
      ...
      paddle::platform::DeviceEvent(
          place, paddle::platform::GenerateDeviceEventFlag()));
...
void ProcessGroupNCCL::SyncCalcStream(const Place& place,
  ...
  calc_event.Wait(paddle::platform::Place2DeviceType(place), comm_ctx);
}
...
std::shared_ptr<ProcessGroup::Task> ProcessGroupNCCL::Collective(
  ...
  paddle::platform::CUDADeviceGuard cuda_guard(place);
  ...
  if (!use_calc_stream) {
    if (!is_coalescing_) {
      if (FLAGS_use_stream_safe_cuda_allocator ||
        ...
        paddle::memory::RecordStream(tensor.Holder(), nccl_stream);
      }
