...
void CommTaskManager::CommTaskLoop() {
  ...
  while (!terminated_.load() || !done) {
      ...
      VLOG(1) << "task loop begin, comm_task_list_ size: " << comm_task_list_.size();
      for (auto task = comm_task_list_.begin(); task != comm_task_list_.end();) {
          VLOG(1) << "process task " << CommTypeToString((*task)->GetCommType()) << ", trace: " << (*task)->GetTraceMsg();
          (*task)->CheckAndSetException();
          if ((*task)->IsTimeout()) {
              VLOG(0) << "debug timeout task " << CommTypeToString((*task)->GetCommType()) << ", curr rank: " << (*task)->GetRank();
          ...
          VLOG(1) << "process task " << CommTypeToString((*task)->GetCommType()) << ", started " << (*task)->IsStarted() << ", completed " << (*task)->IsCompleted() << ", store err " << store_error_;
          VLOG(1) << "process task updated " << (*task)->GetTraceUpdated()  << ", terminated_ " << terminated_.load();
          ...
          if ((*task)->IsCompleted()) {
              ...
              VLOG(1) << "process erase task " << CommTypeToString((*task)->GetCommType());
          } else {
              ++task;
          }
      }
