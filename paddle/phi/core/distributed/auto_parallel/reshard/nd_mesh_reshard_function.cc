...
//   int64_t first_diff_axis = FindFirstDiffShardAxis(in_dist_attr,
//   out_dist_attr);
  ...
  for (int64_t i = first_diff_axis; i >= 0; --i) {
    ...
    if (in_mesh_axis == -1 || in_mesh_axis == out_mesh_axis) {
      ...
      VLOG(3) << "Step1: shard to replicated on axis " << i
              << ". in_mesh_axis: " << in_mesh_axis
              << " , out_mesh_axis: " << out_mesh_axis;
      // 1.1 将该轴标记为非分片 Calculate the dist_attr after this transform
      ...
      // 1.2 获取该轴对应的进程 Calculate the process_mesh on specific axis
      ProcessMesh sub_mesh = GetSubProcessMesh(process_mesh, in_mesh_axis);

      // 1.3 设置该轴为分片，其他轴为复制 Calculate the input one dim dist attr
