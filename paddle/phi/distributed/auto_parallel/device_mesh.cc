...
DeviceMesh::DeviceMesh(const std::string &name,
  ...
  PADDLE_ENFORCE_EQ(
      size,
      device_ids.size(),
      phi::errors::InvalidArgument("The size %d of this device mesh must be "
  ...
  PADDLE_ENFORCE_EQ(
      ...
      false,
      phi::errors::InvalidArgument("The device ids [%s] must be unique.",
                                   str_join(device_ids)));
...
void DeviceMesh::add_link(const Link &link) {
  PADDLE_ENFORCE_EQ(
      ...
      true,
      phi::errors::InvalidArgument("The source id of the added link [%s] "
