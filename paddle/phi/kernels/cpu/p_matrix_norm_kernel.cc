...
//   if (porder == INFINITY || porder == -INFINITY || porder == 1 || porder ==
//   -1) {
...
//       x_sum.Resize(detail::GetEigenvalueDim(x_input.dims(),trans_dims[x_rank
//       - 2])); dev_ctx.template Alloc<T>(&x_sum); phi::SumKernel<T, Context>(
//           dev_ctx, x_input, {-1}, x_input.dtype(), false, &x_sum);
//     } else if(porder == 1 || porder == -1){
//       x_sum.Resize(detail::GetEigenvalueDim(x_input.dims(),trans_dims[x_rank
//       - 1])); dev_ctx.template Alloc<T>(&x_sum); phi::SumKernel<T, Context>(
...
//     std::cout<<x.data<T>()[2]<<" ";
//     std::cout<<x_transposed.data<T>()[1]<<" ";
//     std::cout<<"\n";
...
//     MatmulKernel<T, Context>(dev_ctx, x_transposed, x_input, false, false,
//     &x_2);
...
//     SvdKernel<T,
//     Context>(dev_ctx,x_2,false,&u_tensor,&singular_tensor,&vh_tensor);
...
//     phi::AbsKernel<T, Context>(dev_ctx, singular_tensor,
//     &singular_tensor_abs);

//     DenseTensor max_singular_tensor;
//     max_singular_tensor.Resize(detail::RemoveLastDim(singular_tensor_abs.dims()));
