...
template <typename T>
static inline T chbevl(const T x, const T array[], size_t len) {
  T b0, b1, b2;
...
void CalcI0e(const Context& ctx, const DenseTensor& x, DenseTensor *out) {
  ...
  for (int64_t i = 0; i < size; ++i) {
    ...
    if (x <= 8.0) {
      auto y = (x / 2.0) - 2.0;
      out_data[i] = static_cast<T>(chbevl(y, A, len_A));
    } else {
      auto y = (32.0 / x) - 2.0;
      out_data[i] = static_cast<T>(chbevl(y, B, len_B) / std::sqrt(x));
    }
  }
}
...
void I0eKernel(const Context& ctx, const DenseTensor& x, DenseTensor* out) {
  ...
  CalcI0e<T, Context>(ctx, x, out_data);
}
