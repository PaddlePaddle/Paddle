...
const std::vector<long int> get_slice_strides(const std::vector<long int>& out_vec_dims, const dnnl::memory::desc& full_md, int axis)
{
  ...
  for (unsigned short i=0; i< ndims;++i) {
    slice_strides[i] = strides[i] > splitted_stride ?  (strides[i]/full_dims[axis])*out_vec_dims[axis]: strides[i];
  }
...
void SplitKernel(const Context& dev_ctx,
  ...
  for (size_t i = 0; i < outs_number; ++i) {
    ...
    auto reorder_dst_memory_p = reorder_handler.AcquireDstMemory(
        out[i], out_vec_dims, get_slice_strides(out_vec_dims,x.mem_desc(), axis), dev_ctx.GetPlace());
    auto reorder_p =
