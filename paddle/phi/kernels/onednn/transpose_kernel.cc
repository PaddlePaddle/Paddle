...
std::vector<int64_t> FakeTranposeStrides(
    const dnnl::memory::desc& dst_md, const std::vector<int>& transpose_axis) {
...
std::vector<int> TransposeToPermuteAxis(
    const std::vector<int>& transpose_axis) {
...
void TransposeKernel(const Context& dev_ctx,
  ...
  auto dst_strides = FakeTranposeStrides(dst_md, axis);
  dst_md =
