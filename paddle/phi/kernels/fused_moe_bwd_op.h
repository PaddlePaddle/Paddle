...
#ifdef DEBUG_MOE_OP
  std::cerr << "[DEBUG-BWD] launch kernel, num_active=" << num_active
            << ", num_rows=" << num_rows << ", dim=" << dim << std::endl;
#endif
...
#ifdef DEBUG_MOE_OP
    std::cerr << "[DEBUG-BWD] gather_with_mask with vectorized, s_shared_num="
