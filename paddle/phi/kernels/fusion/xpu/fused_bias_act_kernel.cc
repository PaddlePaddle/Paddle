...
static void ComputeImpl(const phi::XPUContext *xpu_ctx,
  ...
  if (bias) {
    r = baidu::xpu::api::broadcast_add<XPUType>(
        ...
        {1, cols});
    PADDLE_ENFORCE_EQ(
        r, 0, phi::errors::Fatal("baidu::xpu::api::broadcast_add failed."));
  }
  if (act_method == "geglu") {
    PD_THROW(
        "NOT supported GeGLU. "
    ...
    r = baidu::xpu::api::swiglu<XPUType>(
        ...
        true);
    PADDLE_ENFORCE_EQ(
        r, 0, phi::errors::Fatal("baidu::xpu::api::swiglu failed."));
    ...
    r = baidu::xpu::api::gelu<XPUType>(
        ...
        rows * cols);
    PADDLE_ENFORCE_EQ(
        r, 0, phi::errors::Fatal("baidu::xpu::api::gelu failed."));
    ...
    r = baidu::xpu::api::relu<XPUType>(
        ...
        rows * cols);
    PADDLE_ENFORCE_EQ(
        r, 0, phi::errors::Fatal("baidu::xpu::api::relu failed."));
  } else {
    PD_THROW(
        "NOT supported. "
        "Currently Only Support SwiGLU, GeLU, ReLU");
