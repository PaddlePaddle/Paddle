...
void Pad2dXPUKernel(const Context& dev_ctx,
  ...
  PADDLE_ENFORCE_EQ(
      pads.size(),
      6,
      errors::InvalidArgument("Input(X)'s rank of PadOp should be 6,"
                              "but received paddings's size is %d.",
                              pads.size()));
  PADDLE_ENFORCE_EQ(
      pads[4],
      0,
      errors::InvalidArgument("5 Input(X)'s rank of PadOp should equal to 0,"
                              "but received paddings's data is %d.",
                              pads[4]));
  PADDLE_ENFORCE_EQ(
      pads[5],
      0,
      errors::InvalidArgument("6 Input(X)'s rank of PadOp should equal to 0,"
                              "but received paddings's data is %d.",
                              pads[5]));
  ...
  if (dev_version == phi::backends::xpu::XPUVersion::XPU1) {
    if (mode_xpu == "constant" || mode_xpu == "edge" || mode_xpu == "reflect") {
      ...
      PADDLE_THROW(phi::errors::External(
          "XPU is not support other padding mode in pad2d"));
    }
    ...
    PADDLE_THROW(
        phi::errors::External("not support other XPU version in pad2d"));
  }
}
