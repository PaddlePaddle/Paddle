...
void FastAddLayernormXPUKernel(const Context& ctx,
                               const DenseTensor& x,
                               const DenseTensor& y,
  ...
  int r = xpu::plugin::fast_add_layer_norm_fusion<XPUType>(
  ...
  PADDLE_ENFORCE_XDNN_SUCCESS(r, "fast_add_layer_norm_fusion");
#else
  int r = xpu::add_layer_norm_fusion<XPUType>(
      /* baidu::xpu::api::Context* ctx */ ctx.x_context(),
      /* const T* x */ x_data,
      /* const T* y */ y_data,
      /* T* z */ out_data,
...
PD_REGISTER_KERNEL(fast_add_layernorm_xpu,
                   XPU,
                   ALL_LAYOUT,
                   phi::fusion::FastAddLayernormXPUKernel,
                   float,
                   phi::dtype::float16) {}
