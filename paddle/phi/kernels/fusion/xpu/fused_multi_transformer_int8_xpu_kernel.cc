...
void FusedMultiTransformerInt8XpuKernel(
  ...
  r = xft::fused_multi_transformer<XPUTypeT, TW, int8_t>(ctx.x_context(),
                                                         xft_x,
                                                         xft_cache_kv,
                                                         xft_src_mask,
                                                         xft_ln_scale,
                                                         ...
                                                         &xft_out,
                                                         xft_cache_kv_out);
  PADDLE_ENFORCE_XDNN_SUCCESS(r, "xft::fused_multi_transformer_int8");
#else
  LOG(FATAL)
      << "fused_multi_transformer_int8_xpu is not supported since it's not "
