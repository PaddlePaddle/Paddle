...
void FusedAttentionGradKernel(
  ...
  if (pre_layer_norm) {
    ...
    if (ln_0_size) {
      ...
      LaunchLayernormResidualDropoutBiasBackward<T>(
          dev_ctx,
          bsz_seq,
          dim_embed,
          dropout_param2,
  ...
  LaunchAttentionMatMulBackward<T>(dev_ctx,
                                   false,
                                   false,
                                   bsz_seq,
                                   input_size,
  ...
  if (pre_layer_norm) {
    ...
    LaunchAttentionLayerNormBackward<T>(dev_ctx,
                                        epsilon,
                                        bsz_seq,
                                        dim_embed,
                                        x_data,
  ...
  if (add_residual) {
    LaunchAdd<T>(dev_ctx, d_residual, *x_grad, x_grad);
  }
}
