...
void ConvFusionKernel(const Context& ctx,
  ...
  bool exhaustive_search_flag =
      FLAGS_cudnn_exhaustive_search || exhaustive_search;
  bool deterministic = FLAGS_cudnn_deterministic;
  PADDLE_ENFORCE_EQ(exhaustive_search_flag && deterministic,
                    false,
  ...
  phi::backends::gpu::DataLayout layout = phi::backends::gpu::DataLayout::kNHWC;
  ...
  std::vector<int> b_dims{1, 1, 1, static_cast<int>(bias.dims()[0])};
  std::function<void(cudnnConvolutionFwdAlgo_t*,
      ...
      search_func = [&](cudnnConvolutionFwdAlgo_t* cudnn_algo,
        ...
        // TODO(wilber): get workspace limit.
        int workspace_size_limit = 128 * 1024 * 1024;
  ...
  //         " "batch size is %d, Input's shape is [%s].", x_dims[0],
