...
void FFNGrad(const GPUContext& dev_ctx,
             ...
             const FusedDropoutConfig& dropout_param1,
             const FusedDropoutConfig& dropout_param2,
  ...
  if (pre_layer_norm) {
    LaunchResidualDropoutBiasBackward<T>(dev_ctx,
                                         bsz_seq,
                                         d_model,
                                         dropout_param2,
                                         epsilon2,
    ...
    LaunchLayernormResidualDropoutBiasBackward<T>(dev_ctx,
                                                  bsz_seq,
                                                  d_model,
                                                  dropout_param2,
                                                  epsilon2,
  ...
  LaunchDropoutActBiasBackward<T>(dev_ctx,
                                  bsz_seq,
                                  dim_feedforward,
                                  dropout_param1,
  ...
  if (pre_layer_norm) {
    ...
    LaunchLayerNormBackward<T>(dev_ctx,
                               bsz_seq,
                               d_model,
                               epsilon1,
                               d_ln1_out.data<T>(),
  ...
  if (add_residual) {
    LaunchAdd<T>(dev_ctx, d_residual, *d_x, d_x);
  }
}
...
void FusedFeedForwardGradKernel(const Context& dev_ctx,
  ...
  FusedDropoutConfig dropout_param1{dropout1_fix_seed,
                                    is_test,
                                    is_upscale_in_train1,
                                    dropout1_prob,
                                    nullptr,
                                    dropout1_seed_val};
  FusedDropoutConfig dropout_param2{dropout2_fix_seed,
                                    is_test,
