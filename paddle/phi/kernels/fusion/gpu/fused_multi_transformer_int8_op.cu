...
void FusedMultiTransformerINT8Kernel(
    ...
    const std::vector<const DenseTensor *> &qkv_out_scales,
    const std::vector<const DenseTensor *> &out_linear_out_scales,
    const std::vector<const DenseTensor *> &ffn1_out_scales,
    const std::vector<const DenseTensor *> &ffn2_out_scales,
    bool pre_layer_norm,
    float epsilon,
    float dropout_rate,
    bool is_test,
