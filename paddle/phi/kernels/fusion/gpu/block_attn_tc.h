...
template <typename T>
void WriteEncoderCacheKvHdim128<T>::operator()(const T* qkv_out,
                                               const int* cu_seqlens_q,
                                               const int* block_tables,
                                               const int* seq_lens,
                                               uint8_t* cache_k,
  ...
  kernel<<<grid_dim, kNThreads, smem_size, stream>>>(qkv_out,
                                                     cu_seqlens_q,
                                                     block_tables,
                                                     seq_lens,
                                                     cache_k,
...
__global__ void writeDecoderCacheKVKernel(T* qkv_input,
  ...
  using T_vec_4 = typename block_attn::Vec<cuteType, 4>;
  T_vec_4 scale_k = *reinterpret_cast<const T_vec_4*>(
      cache_k_quant_scale + head_idx * kHeadDim + col * 32 + row * 4);

  T_vec_4 scale_v = *reinterpret_cast<const T_vec_4*>(
  ...
  if (kv_idx == 0) {
    ...
    if (warp_id == 0) {
      ...
      cache_k_cur[row * kHeadDim * 2 + col * 2 + copy_idx * 2 + offset] = value;
    }
  } else {
    if (warp_id == 0) {
      T_vec kv_src;
...
void set_params(const phi::GPUContext& dev_ctx,
  ...
  if constexpr (cachenbits == 16) {
    ...
    params.cache_v = const_cast<uint8_t*>(v_cache->data<uint8_t>());
    params.cache_k_dequant_scale = cache_k_dequant_scales->data<T>();
    params.cache_v_dequant_scale = cache_v_dequant_scales->data<T>();
    params.cache_k_quant_scale = cache_k_quant_scales->data<T>();
...
void blha_tc(const phi::GPUContext& dev_ctx,
  ...
  auto set_params_func = set_params<T, int8_t, max_seq_per_block, 16>;

  dev_ctx.template Alloc<T>(out_tensor);
  if (use_cachekv_int8 > 0) {
    set_params_func = set_params<T, T, max_seq_per_block, 8>;
  } else {
    set_params_func = set_params<T, T, max_seq_per_block, 16>;
  }
