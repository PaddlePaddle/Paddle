...
namespace phi {
  ...
  float conv2d_diff_cpu(COMMON_CONV_PARAMS,
    ...
    naive_conv_cpu(cpu_input, cpu_weight, cpu_bias, cpu_output, batch, ic, ih, iw, kh, kw, oc, pad_h, pad_w, \
        stride_h, stride_w, residual, op_name);
...
__device__ int gpu_nhwc(struct logical_struct shape, struct logical_struct index) {
  return index.n * shape.h * shape.w * shape.c + index.h * shape.w * shape.c +
         index.w * shape.c + index.c;
}

__global__ void naive_conv2d_kernel(const half *input,  const half *weight,
const half *bias, float *output,
int batch, int ic, int ih, int iw,
int kh, int kw, int oc, int pad_h, int pad_w,
int stride_h, int stride_w,
  int oh, int ow, const half *residual, int op_type) {
...
float conv2d_diff_gpu(COMMON_CONV_PARAMS, const half *residual, std::string op_name) {
  ...
  int output_size = batch * oc * oh * ow;
  half *output_from_cutlass = (half*)malloc(sizeof(half) * output_size);
  cudaMemcpy(output_from_cutlass, output, output_size * sizeof(half), cudaMemcpyDeviceToHost);

  float *gpu_output;
  cudaMalloc((void **)&gpu_output, output_size * sizeof(float));
  naive_conv2d_kernel<<<grid, block>>>(input, weight, bias, gpu_output, batch, ic, ih, iw, kh, kw,
                             oc, pad_h, pad_w, stride_h, stride_w, oh, ow, residual, op_type);
  float *output_from_gpu = (float*)malloc(sizeof(float) * output_size);
  cudaMemcpy(output_from_gpu, gpu_output, output_size * sizeof(float), cudaMemcpyDeviceToHost);
