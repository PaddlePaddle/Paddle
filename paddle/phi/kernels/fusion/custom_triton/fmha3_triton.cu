...
#include "generated/aot/fmha3/fp16/fmha3_fp16.h"
#include <cuda_fp16.h>
#include <cmath>  
#include <iostream>

std::vector<paddle::Tensor> TritonFMHA3(const paddle::Tensor& q, 
...
std::vector<std::vector<int64_t>> TritonFMHA3InferShape(const std::vector<int64_t>& q_shape,
                                                              const std::vector<int64_t>& k_shape, 
                                                              const std::vector<int64_t>& v_shape) {
    return {q_shape};
}

std::vector<paddle::DataType> TritonFMHA3InferDtype(const paddle::DataType& q_dtype,
                                                        const paddle::DataType& k_dtype,
                                                        const paddle::DataType& v_dtype) {
    return {q_dtype};
}

PD_BUILD_OP(triton_fmha3)
    .Inputs({"q", "k", "v"})
    .Outputs({"out"})
    .Attrs({"scale: float"})
    .SetKernelFn(PD_KERNEL(TritonFMHA3))
    .SetInferShapeFn(PD_INFER_SHAPE(TritonFMHA3InferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(TritonFMHA3InferDtype));
