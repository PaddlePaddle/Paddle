...
std::vector<paddle::Tensor> TritonFc2(const paddle::Tensor& a, 
...
std::vector<std::vector<int64_t>> TritonFc2InferShape(const std::vector<int64_t>& a_shape,
                                                              const std::vector<int64_t>& b_shape) {
    auto out_shape = a_shape;
    out_shape.back() = b_shape[0];
    // out_shape.pop_back();
...
std::vector<paddle::DataType> TritonFc2InferDtype(const paddle::DataType& a_dtype,
                                                        const paddle::DataType& b_dtype) {
    return {a_dtype};
}

PD_BUILD_OP(triton_Fc2)
    .Inputs({"a", "b"})
    .Outputs({"c"})
    .SetKernelFn(PD_KERNEL(TritonFc2))
    .SetInferShapeFn(PD_INFER_SHAPE(TritonFc2InferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(TritonFc2InferDtype));
