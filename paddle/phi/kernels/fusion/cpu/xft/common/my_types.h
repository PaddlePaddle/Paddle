...
struct MatData<T, true> {
  ...
  void Resize(int rows, int cols, int stride) {
    ...
    if ((this->qscheme == per_channel_symmetric ||
         this->qscheme == per_channel_affine) &&
        this->qparam.per_c.alloc_size < rows) {
      if (this->qparam.per_c.scales) {
        free(this->qparam.per_c.scales);
      }
      this->qparam.per_c.scales =
          reinterpret_cast<float *>(aligned_alloc(64, sizeof(float) * rows));
      ...
      if (this->qscheme == per_channel_affine) {
        ...
        this->qparam.per_c.zps = reinterpret_cast<int32_t *>(
            aligned_alloc(64, sizeof(int32_t) * rows));
  ...
  void Release() {
    ...
    if (!shadow && (this->qscheme == per_channel_symmetric ||
                    this->qscheme == per_channel_affine)) {
      free(this->qparam.per_c.scales);
  ...
  void SetQScheme(QuantizationScheme scheme) {
    if (scheme == per_tensor_symmetric || scheme == per_tensor_affine) {
      ...
      if (unlikely(this->qscheme == per_channel_symmetric ||
                   this->qscheme == per_channel_affine)) {
        if (this->qparam.per_c.scales) {
          free(this->qparam.per_c.scales);
        }
    ...
    } else if (scheme == per_channel_symmetric ||
  ...
  float *Scales() {
    if (this->qscheme == per_tensor_symmetric ||
        this->qscheme == per_tensor_affine) {
      return &qparam.per_t.scale;
    } else if (this->qscheme == per_channel_symmetric ||
               this->qscheme == per_channel_affine) {
...
class Matrix {
 ...
 public:
  ...
  Matrix(Matrix &m, int start_row, int rows, int start_col, int cols)  // NOLINT
  ...
  // Create dilated matrix, for example, if dilation = 2, then select the 1st,
  // 3rd, 5th, ... lines
  Matrix(Matrix &m, int start_row, int dilation, bool unused)  // NOLINT
      : data(m.data.buf + start_row * m.stride) {
    this->rows = m.rows / dilation;
    this->cols = m.cols;
    this->stride = m.stride * dilation;
  }

  Matrix(Matrix &m, int start_row, int rows)  // NOLINT
      : data(m.data.buf + start_row * m.stride) {
    this->rows = rows;
    this->cols = m.cols;
    this->stride = m.stride;
  ...
  void Resize(int rows, int cols) {
    ...
    // Previously, we used to pad the matrix when the columns aligned with the
    // boundary of 1024. However, we discovered that this did not enhance the
    // performance. As a result, we have decided to remove this approach.
    this->stride = cols;
    this->rows = rows;
    this->cols = cols;
  ...
  void Resize(int rows, int cols, int stride) {
    ...
    if (this->rows == rows && this->cols == cols && this->stride == stride) {
      return;
    }
