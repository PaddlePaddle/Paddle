...
struct DecoderContext {
  ...
  // Please look into the comments in resize function to see how buffers are
  // arranged
...
#pragma omp parallel
    ...
    this->rawBufSize = 4 * 32 * intermediateSize +
                       4 * attHeadNum * 32 * 32;  // assume bs=4, seq=32
    this->rawBuffer = reinterpret_cast<float *>(
        aligned_alloc(64, sizeof(float) * rawBufSize));
  ...
  void resize(int batchSize, int inputSeqLen, bool preSeqLen) {
    ...
    int responsibleHead = splitIdx < (attHeadNum % numSplit)
                              ? (attHeadNum / numSplit + 1)
                              : (attHeadNum / numSplit);
    ...
    int qkvStride =
        (qkvCols % 512 == 0 ? qkvCols + pad
                            : qkvCols);  // stride for the concated QKV
    int mlpFactor = (this->actType == SILU || this->actType == SWIGLU) ? 2 : 1;
    int imCols = splitIdx < (intermediateSize % numSplit)
                     ? (intermediateSize / numSplit + 1)
                     : (intermediateSize / numSplit);
    int imStride =
        (imCols % 512 == 0 ? imCols + pad
                           : imCols);  // stride for intermediate output
    ...
    int scoreBufSize =
        preSeqLen > 0 ? batchSize * responsibleHead * inputSeqLen * paddedSize
    ...
    normBuf.Assign(
        this->rawBuffer, batchSize * inputSeqLen, hiddenSize, hiddenStride);
    tmpBuf.Assign(
        this->qkScores, batchSize * inputSeqLen, hiddenSize, hiddenStride);
    imOut.Assign(
        this->rawBuffer + size1, batchSize * inputSeqLen, imCols, imStride);
    qkvMatMul.Assign(
        this->rawBuffer + size1, batchSize * inputSeqLen, qkvCols, qkvStride);
  }
