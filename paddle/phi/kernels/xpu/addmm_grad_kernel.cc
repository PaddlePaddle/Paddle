...
void AddmmGradKernel(const Context& dev_ctx,
  ...
  if (x_grad) {
    MatMulXPUFunction<XPUType, XPUType>(xpu_ctx, a_1, b_1, nullptr, c_1, info_x_grad, alpha, 0.f);
  ...
  if (y_grad) {
    MatMulXPUFunction<XPUType, XPUType>(xpu_ctx, a_2, b_2, nullptr, c_2, info_y_grad, alpha, 0.f);
