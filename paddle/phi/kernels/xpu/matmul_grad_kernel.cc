...
void MatmulGradKernel(const Context& dev_ctx,
  ...
  if (dx) {
    MatMulXPUFunction<XPUType, XPUType>(xpu_ctx, a_1, b_1, nullptr, c_1, info_dx, 1.0f);
    if (info_forward.is_x_need_broadcast) {
  ...
  if (dy) {
    MatMulXPUFunction<XPUType, XPUType>(xpu_ctx, a_2, b_2, nullptr, c_2, info_dy, 1.0f);
    if (info_forward.is_y_need_broadcast) {
      int r = xpu::reduce_sum<XPUType>(
          xpu_ctx,
          c_2,
...
void MatmulWithFlattenGradKernel(const Context& dev_ctx,
  ...
  std::tie(info_dx, info_dy, a_1, b_1, a_2, b_2) = fc_info;
  if (x_grad) {
    phi::MatMulXPUFunction<XPUType, XPUType>(xpu_ctx, a_1, b_1, nullptr, c_1, info_dx, 1.0f);
  }
  if (y_grad) {
    phi::MatMulXPUFunction<XPUType, XPUType>(xpu_ctx, a_2, b_2, nullptr, c_2, info_dy, 1.0f);
  }
}
