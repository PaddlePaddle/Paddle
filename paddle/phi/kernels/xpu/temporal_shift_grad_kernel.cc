...
void TemporalShiftGradKernel(const Context& dev_ctx,
  ...
  if (data_layout == DataLayout::kNCHW) {
    int r = xpu::temporal_shift_grad(dev_ctx.x_context(), output_grad_data, input_grad_data, n, c, h, w, t, shift_ratio, false);
    PADDLE_ENFORCE_EQ(
      r,
      XPU_SUCCESS,
      phi::errors::External(
          "XPU temporal_shift_grad_nchw kernel return wrong value[%d %s]", r, XPUAPIErrorMsg[r]));
  } else {
    int r = xpu::temporal_shift_grad(dev_ctx.x_context(), output_grad_data, input_grad_data, n, c, h, w, t, shift_ratio, true);
    PADDLE_ENFORCE_EQ(
      r,
      XPU_SUCCESS,
      phi::errors::External(
          "XPU temporal_shift_grad_nhwc kernel return wrong value[%d %s]", r, XPUAPIErrorMsg[r]));
  }
}
