...
void DeformableConvGradKernel(const Context& dev_ctx,
  ...
  // Match the forward workaround for legal large-padding cases rejected by XDNN:
  // run grad on the materialized padded input, then crop dx back to x's shape.
  ...
  if (padded_dx_data != nullptr) {
    r_dx = xpu::constant<T>(dev_ctx.x_context(),
                            padded_dx_data,
                            batch_size * conv_input_dim,
                            zero);
  }
  ...
  if (groups > 1 && deformable_groups == 1) {
    ...
    for (int64_t i = 0; i < batch_size / n; ++i) {
      for (int64_t g = 0; g < groups; ++g) {
        ...
        T* temp_dw = RAII_GUARD.alloc_l3_or_gm<T>(
            f_per_group * c_per_group * ksize[0] * ksize[1]);
        ...
        PADDLE_ENFORCE_NOT_NULL(
            temp_out_grad, errors::ResourceExhausted("XPU has no enough memory"));
        PADDLE_ENFORCE_NOT_NULL(
            temp_doffset, errors::ResourceExhausted("XPU has no enough memory"));
        PADDLE_ENFORCE_NOT_NULL(
            temp_dmask, errors::ResourceExhausted("XPU has no enough memory"));

        int r = xpu::slice<T>(dev_ctx.x_context(),
                              conv_input_ptr + i * n * conv_input_dim,
                              temp_x,
                              std::vector<int64_t>{n, c, conv_h, conv_w},
                              std::vector<int64_t>{0, g * c_per_group, 0, 0},
                              std::vector<int64_t>{
                                  n, (g + 1) * c_per_group, conv_h, conv_w});
        ...
        r = xpu::constant<T>(dev_ctx.x_context(), temp_dx, n * temp_input_dim, zero);
        ...
        PADDLE_ENFORCE_NOT_NULL(
            padded_temp_dx, errors::ResourceExhausted("XPU has no enough memory"));
        PADDLE_ENFORCE_NOT_NULL(
            padded_temp_dw, errors::ResourceExhausted("XPU has no enough memory"));
        r = xpu::pad<T>(dev_ctx.x_context(),
                        temp_dx,
                        padded_temp_dx,
        ...
        r = baidu::xpu::api::add<T>(
            dev_ctx.x_context(), padded_temp_dw, dw_data, dw_data, filter.numel());
        PADDLE_ENFORCE_XDNN_SUCCESS(r, "add");
        r = baidu::xpu::api::add<T>(dev_ctx.x_context(),
                                    temp_doffset,
    ...
    // shape family. Each call uses one deformable group and full output channels.
    ...
    for (int64_t i = 0; i < batch_size / n; ++i) {
      for (int64_t g = 0; g < deformable_groups; ++g) {
        ...
        T* temp_doffset = RAII_GUARD.alloc_l3_or_gm<T>(n * offset_dim_per_group);
        ...
        T* padded_temp_dx = RAII_GUARD.alloc_l3_or_gm<T>(n * conv_input_dim);
        T* padded_temp_dw = RAII_GUARD.alloc_l3_or_gm<T>(filter.numel());
        T* padded_temp_doffset = RAII_GUARD.alloc_l3_or_gm<T>(n * input_offset_dim);
        ...
        r = xpu::pad<T>(
            ...
            std::vector<int64_t>{0,
                                 (deformable_groups - g - 1) * 2 * ksize[0] * ksize[1],
                                 0,
                                 0},
            static_cast<T>(0));
        PADDLE_ENFORCE_XDNN_SUCCESS(r, "pad");
  ...
  if (padded_dx_data != nullptr) {
    int r_slice = xpu::slice<T>(dev_ctx.x_context(),
                                padded_dx_data,
                                dx_data,
