...
void MergedAdamKernel(
  ...
  float lr_;
  xpu_memcpy(&lr_, lr_ptr, sizeof(float), XPUMemcpyKind::XPU_DEVICE_TO_HOST);
  ...
  if (beta1_pow[0]->place() == CPUPlace()) {
    DenseTensor xpu_beta1_pow;
    phi::Copy(
        dev_ctx, *beta1_pow[0], dev_ctx.GetPlace(), false, &xpu_beta1_pow);
    if (xpu_beta1_pow.dtype() == DataType::FLOAT16)
      funcs::GetDataPointer<Context, float>(
          xpu_beta1_pow, &beta1_pow_ptr, dev_ctx);
    beta1_pow_ptr != nullptr ? beta1_pow_ptr
                             : xpu_beta1_pow.template data<float>();
    beta1_pow_data = *(beta1_pow_ptr);
  } else {
    if (beta1_pow[0]->dtype() == DataType::FLOAT16)
      funcs::GetDataPointer<Context, float>(
          *beta1_pow[0], &beta1_pow_ptr, dev_ctx);
  ...
  if (beta2_pow[0]->place() == CPUPlace()) {
    DenseTensor xpu_beta2_pow;
    phi::Copy(
        dev_ctx, *beta2_pow[0], dev_ctx.GetPlace(), false, &xpu_beta2_pow);
    if (xpu_beta2_pow.dtype() == DataType::FLOAT16)
      funcs::GetDataPointer<Context, float>(
          xpu_beta2_pow, &beta2_pow_ptr, dev_ctx);
    beta2_pow_ptr != nullptr ? beta2_pow_ptr
                             : xpu_beta2_pow.template data<float>();
    beta2_pow_data = *(beta2_pow_ptr);
  } else {
    if (beta2_pow[0]->dtype() == DataType::FLOAT16)
      funcs::GetDataPointer<Context, float>(
          *beta2_pow[0], &beta2_pow_ptr, dev_ctx);
  ...
  if (param_num > 0) {
    for (int j = 0; j < param_num; j++) {
      ...
      param_list[j] != nullptr ? param_ptr
                               : const_cast<float*>(param[j]->data<float>());
      grad_list[j] != nullptr ? grad_ptr
                              : const_cast<float*>(grad[j]->data<float>());
      moment1_list[j] != nullptr
          ? mom1_ptr
          : const_cast<float*>(moment1[j]->data<float>());
      moment2_list[j] != nullptr
          ? mom2_ptr
          : const_cast<float*>(moment2[j]->data<float>());
      shape_list[j] = static_cast<int64_t>(param[j]->numel());
