...
void MergedAdamKernel(
  ...
  float lr_;
  const float* lr_ptr = learning_rate[0]->data<float>();
  ...
  if (beta1_pow[0]->place() == CPUPlace()) {
    ...
    beta1_const_pow_ptr = xpu_beta1_pow.template data<float>();
    beta1_pow_data = *(beta1_const_pow_ptr);
  } else {
    beta1_const_pow_ptr = beta1_pow[0]->data<float>();
  ...
  if (beta2_pow[0]->place() == CPUPlace()) {
    ...
    beta2_const_pow_ptr = xpu_beta2_pow.template data<float>();
    beta2_pow_data = *(beta2_const_pow_ptr);
  } else {
    beta2_const_pow_ptr = beta2_pow[0]->data<float>();
