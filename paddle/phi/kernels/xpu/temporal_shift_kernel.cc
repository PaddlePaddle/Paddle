...
void TemporalShiftKernel(const Context& dev_ctx,
  ...
  if (data_layout == DataLayout::kNCHW) {
    int r = xpu::temporal_shift(dev_ctx.x_context(), input_data, output_data, n, c, h, w, t, shift_ratio, false);
    PADDLE_ENFORCE_EQ(
      r,
      XPU_SUCCESS,
      phi::errors::External(
          "XPU temporal_shift_nchw kernel return wrong value[%d %s]", r, XPUAPIErrorMsg[r]));
  } else {
    int r = xpu::temporal_shift(dev_ctx.x_context(), input_data, output_data, n, c, h, w, t, shift_ratio, true);
    PADDLE_ENFORCE_EQ(
      r,
      XPU_SUCCESS,
      phi::errors::External(
          "XPU temporal_shift_nhwc kernel return wrong value[%d %s]", r, XPUAPIErrorMsg[r]));
  }
}
