...
void ContiguousKernel(const Context& dev_ctx,
  ...
  if (std::is_same<T, float>::value) {
    ...
    auto input_data = reinterpret_cast<const float16*>(input.data<T>());
    auto output_data =
        reinterpret_cast<float16*>(dev_ctx.template Alloc<T>(out));
    r = xpu::as_strided<float16>(dev_ctx.x_context(),
                                 input_data,
                                 output_data,
                                 phi::vectorize<int64_t>(input.dims()),
                                 phi::vectorize<int64_t>(input.strides()),
