  ...
  switch (x.dtype()) {
    case phi::DataType::FLOAT16: {
      ...
      if (weight_dtype == "int8") {
        ...
        r = baidu::xpu::api::cast_v2<uint8_t, int32_t>(
            xpu_ctx->x_context(),
        ...
        // std::vector<float> w_scale_cpu(n);
        // xpu_memcpy(w_scale_cpu.data(), max_value.data<float>(), sizeof(float)
        // * max_value.numel(), XPU_DEVICE_TO_HOST); std::vector<int8_t> w_cpu(n
        // * k); xpu_memcpy(w_cpu.data(), permuted_weight.data<int8_t>(),
        // sizeof(int8_t) * n * k, XPU_DEVICE_TO_HOST); for (int i = 0; i < n;
        // ++i) {
        //   for (int j = 0; j < k; j++) {
        //     std::cout << w_scale_cpu[i] / 127 * static_cast<int32_t>(w_cpu[i
        //     * k + j]) << ", ";
        //   }
        //   std::cout << std::endl;
        // }
