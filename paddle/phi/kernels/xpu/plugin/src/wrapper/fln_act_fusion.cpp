...
namespace plugin {
template <typename T>
__attribute__((global)) void fln_act_tiny_common(float epsilon,
                                                         int64_t m,
                                                         int64_t n,
                                                         const T* x,
                                                         T* y,
                                                         ...
                                                         float act_param);
template <typename T>
__attribute__((global)) void fln_act_tiny_align32(float epsilon,
                                                          int64_t m,
                                                          int64_t n,
                                                          const T* x,
                                                          T* y,
                                                          ...
                                                          float act_param);
template <typename T>
__attribute__((global)) void leaky_relu_fwd(const T* x,
                                            T* y,
                                            int len,
                                            float alpha);
}  // namespace plugin
...
static int xpu2_wrapper(Context* ctx,
  ...
  if (n <= 832) {
    if (n % 32 == 0 && n < 128) {
      xpu2::plugin::fln_act_tiny_align32<T>
          <<<ctx->ncluster(), 64, ctx->xpu_stream>>>(
              eps, m, n, x, y, scale, bias, act_param);
    } else {
      xpu2::plugin::fln_act_tiny_common<T>
          <<<ctx->ncluster(), 64, ctx->xpu_stream>>>(
              eps, m, n, x, y, scale, bias, act_param);
    }
  } else {
    layer_norm(ctx, x, y, m, n, eps, scale, bias, nullptr, nullptr);
    xpu2::plugin::leaky_relu_fwd<T><<<ctx->ncluster(), 64, ctx->xpu_stream>>>(y, y, m*n, act_param);
  }
...
template <typename T>
int fln_act_fusion(Context* ctx,
                    const T* x,
                    T* y,
                    int64_t m,
                    int64_t n,
  ...
  WRAPPER_DUMP_FUNCTION_T1(ctx, "fln_act_fusion", T);
...
template int fln_act_fusion(Context*,
                             const float*,
                             float*,
                             int64_t,
                             int64_t,
