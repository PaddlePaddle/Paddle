  ...
  for (auto i1 = blockIdx.y; i1 < n1; i1 += gridDim.y) {
    ...
    if (gamma != NULL && (beta != NULL || rms_only) ) {
      for (int i = thrx; i < n2; i += numx) {
        ...
        if (!rms_only) {
          ovals[i] = static_cast<V>(static_cast<U>(gamma[i]) * c_invvar * (curr - mu) + static_cast<U>(beta[i]));
        } else {
...
__device__ void cuLoadWriteStridedInputs(const int i1_block,
                                         ...
                                         const U* __restrict__ invvar, bool rms_only) {
...
__global__ void cuComputePartGradGammaBeta(const V* __restrict__ dout,
                                           ...
                                           U epsilon,
                                           U* part_grad_gamma,
                                           U* part_grad_beta, bool rms_only) {
...
void cuda_layer_norm(const paddle::Tensor& x,
...
void cuda_rms_norm(const paddle::Tensor& x,
                            const paddle::Tensor& scale,
                            int rows,
                            int cols,
                            float epsilon,
...
void cuda_layer_norm_gradient(const paddle::Tensor& x,
                                     const paddle::Tensor& scale,
                                     const paddle::Tensor& bias,
...
void cuda_rms_norm_gradient(const paddle::Tensor& x,
                                     const paddle::Tensor& scale,
                                     const paddle::Tensor& invvar,
                                     const paddle::Tensor& dy,
                                     int rows,
