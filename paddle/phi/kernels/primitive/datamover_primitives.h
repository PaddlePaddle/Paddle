...
struct FastDivMod {
  ...
  HOSTDEVICE FastDivMod(uint64_t d) : divisor(d) {
    if (divisor <= INT32_MAX) {
      int num_bits = 32;
      for (shift_val = 0; shift_val < num_bits; ++shift_val) {
      ...
      uint64_t long_one = 1;
      uint64_t temp_div =
          ((long_one << num_bits) * ((long_one << shift_val) - divisor)) /
              divisor +
          1;
      multiplier = temp_div;
    }
  ...
  __device__ __forceinline__ uint32_t FastDiv(uint32_t n) const {
    uint32_t t = __umulhi(n, multiplier);
    return (t + n) >> shift_val;
  }

  template <bool UseFastDiv = true>
  __device__ __forceinline__ DivModT Divmod(uint64_t n) const {
    uint32_t q = 0;
    if (UseFastDiv) {
      PADDLE_ENFORCE(divisor <= INT32_MAX && n <= INT32_MAX, "");
      q = FastDiv(static_cast<uint32_t>(n));
    } else {
      q = Div(n);
    }
...
struct BroadcastConfig {
  ...
  __device__ uint64_t GetOffset(uint64_t broadcasted_offset) const {
    uint64_t div_res = broadcasted_offset;
    uint64_t offset = 0;
#pragma unroll
    for (int i = 0; i < phi::DDim::kMaxRank; ++i) {
      ...
      auto divmod_res = divmoders[i].Divmod<UseFastDiv>(div_res);
...
__device__ __forceinline__ void ReadDataBc(
  ...
  bool can_use_fast_div = total_num_output <= INT32_MAX;

#pragma unroll
  for (int ny = 0; ny < NY; ++ny) {
#pragma unroll
    for (uint32_t nx = 0; nx < NX; ++nx) {
      uint64_t index_dst = thread_offset + ny * stride_ny + nx * stride_nx;
      if (IsBoundary) {
      ...
      if (can_use_fast_div) {
        index_src = config.GetOffset<true>(index_dst);
      } else {
        index_src = config.GetOffset<false>(index_dst);
      }
...
#pragma unroll
  for (uint32_t nx = 0; nx < NX; ++nx) {
    uint64_t index_dst = thread_offset + nx;
    if (IsBoundary) {
      if (index_dst >= total_num_output) {
        break;
      }
