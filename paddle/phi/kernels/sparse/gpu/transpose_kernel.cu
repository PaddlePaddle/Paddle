...
__global__ void TransposeCsr2DCudaKernel(const IntT *x_crows_data,
                                         ...
                                         IntT *out_cols_data,
                                         T *out_values_data) {
  IntT __index__ = static_cast<IntT>(blockIdx.x) * blockDim.x + threadIdx.x;
  // compute out_crows_data by x_cols_data
  for (IntT i = __index__; i <= out_dims[0]; i += blockDim.x * gridDim.x) {
    out_crows_data[i] = 0;
  }
  __syncthreads();
  if (__index__ == 0) {
    for (IntT i = 0; i < x_nnz; ++i) {
      int j = x_cols_data[i];
      out_crows_data[j + 2]++;
    }
    for (IntT i = 0; i < out_dims[0]; i += 1) {
...
void TransposeCsrImpl(const Context &dev_ctx,
  ...
  DDim out_dims = x.dims().transpose(perm);
  if (n_dim == 2) {
    out_crows = Empty<IntT, Context>(dev_ctx, {out_dims[0] + 1});
  } else {
    out_crows =
        Empty<IntT, Context>(dev_ctx, {out_dims[0] * (out_dims[1] + 1)});
  }
  out_cols = EmptyLike<IntT, Context>(dev_ctx, x.cols());
  ...
  auto d_x_dims_tensor = memory_utils::Alloc(
      dev_ctx.GetPlace(),
      sizeof(IntT) * x.dims().size(),
      phi::Stream(reinterpret_cast<phi::StreamId>(dev_ctx.stream())));
  d_x_dims = reinterpret_cast<IntT *>(d_x_dims_tensor->ptr());
  memory_utils::Copy(dev_ctx.GetPlace(),
                     d_x_dims,
                     phi::CPUPlace(),
                     x.dims().Get(),
  ...
  auto d_out_dims_tensor = memory_utils::Alloc(
      ...
      phi::Stream(reinterpret_cast<phi::StreamId>(dev_ctx.stream())));
  d_out_dims = reinterpret_cast<IntT *>(d_out_dims_tensor->ptr());
  memory_utils::Copy(dev_ctx.GetPlace(),
                     d_out_dims,
                     phi::CPUPlace(),
                     out_dims.Get(),
  ...
  if (perm.size() == 2) {
    TransposeCsr2DCudaKernel<T, IntT><<<config.block_per_grid.x,
                                        config.thread_per_block.x,
                                        0,
    ...
    TransposeCsr3DCudaKernel<T, IntT>
...
void TransposeCsrKernel(const Context &dev_ctx,
  ...
  PD_VISIT_BASE_INTEGRAL_TYPES(x.crows().dtype(), "TransposeCsrKernel", ([&] {
                                 TransposeCsrImpl<T, Context, data_t>(
                                     dev_ctx, x, perm, out);
                               }));
}
