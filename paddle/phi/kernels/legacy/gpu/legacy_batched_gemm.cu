...
void CublasGemm(cublasHandle_t cublas_handle, phi::bfloat16 *a, int64_t a_rows,
                int64_t a_cols, bool trans_a, phi::bfloat16 *b, int64_t b_rows,
                int64_t b_cols, bool trans_b, phi::bfloat16 *c, int64_t c_rows,
                int64_t c_cols) {
...
void grouped_gemm_cuda_forward(
                          const Context &dev_ctx,
                          const DenseTensor &a, const DenseTensor &b,
  ...
  if constexpr (std::is_same<T, paddle::bfloat16>::value) {
    ...
    for (int64_t i = 0; i < num_experts; ++i) {
      ...
      CublasGemm(handle, a_data, expert_bs, input_hidden_size, false, b_data,
                 b_shape[1], b_shape[2], false, output_data, expert_bs,
                 output_hidden_size);
...
void LegacyBatchedGEMM(const Context &dev_ctx,
                       const DenseTensor &lhs, const DenseTensor &rhs,
                       const std::vector<int64_t> &batch_sizes,
                       DenseTensor* output) {
  ...
  switch (lhs.dtype()) {
  case paddle::DataType::BFLOAT16:
    grouped_gemm_cuda_forward<paddle::bfloat16>(dev_ctx,lhs, rhs, batch_sizes,
                                                       output);
  default:
    PD_CHECK(false, "Unsupported data type");
  }
