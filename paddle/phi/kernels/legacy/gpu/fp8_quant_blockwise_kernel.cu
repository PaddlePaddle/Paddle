...
template <bool use_pow2_scale, bool using_ue8m0_scale>
...
__global__ void __launch_bounds__(512)
  ...
  if constexpr (!return_transpose_only) {
    ...
    ComputeRowScale<use_pow2_scale, using_ue8m0_scale>(
  ...
  if constexpr (input_transpose) {
    ...
    ComputeColumnScale<use_pow2_scale, using_ue8m0_scale>(
        x, block_scale, reinterpret_cast<__nv_bfloat16 *>(shm), epsilon);
...
void FP8QuantBlockWiseKernelImpl(const Context &dev_ctx,
  ...
  auto kernel = using_1x128_vec_quant
                    ? quantize_1x128_kernel<ScaleT,
                                            input_transpose,
                                            output_scale_transpose,
                                            return_transpose_only,
                                            using_pow2_scale,
                                            using_ue8m0_scale>
                    : quantize_128x128_kernel<ScaleT,
                                              input_transpose,
                                              output_scale_transpose,
                                              return_transpose_only,
                                              using_pow2_scale,
