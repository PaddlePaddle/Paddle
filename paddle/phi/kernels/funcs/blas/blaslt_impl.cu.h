...
struct MatmulDescriptor {
 public:
  ...
  void Create(const int M,
              ...
              const bool trans_x,
              const bool trans_y,
              phi::autotune::MatmulPlanner* planner,
  ...
  void SetFusedEpiloguePtr(phi::autotune::MatmulPlanner* planner) {
    if (planner->bias) {
      ...
      PADDLE_ENFORCE_GPU_SUCCESS(
        dynload::cublasLtMatmulDescSetAttribute(
          op_desc,
...
#define GET_DESC_DATA_INFO(src)                      \
 ...
 private:
  ...
  void SetFusedEpilogueOpDescriptor(
            phi::autotune::MatmulPlanner* planner, 
            int64_t lead_dim) {
      if (planner->bias) {
...
struct DescriptorSetter {
  ...
  DescriptorSetter(phi::autotune::MatmulPlanner* planner,
                   const int M,
                   const int N,
                   const int K,
                   const bool trans_x,
    ...
    if (planner != nullptr) {
      sub_key =
          planner->GenSubKey(static_cast<size_t>(planner->ImplType()));
    }
...
struct MatmulWithCublasLt {
 public:
  ...
  static void Run(const phi::GPUContext& ctx,
                  ...
                  const bool trans_x,
                  const bool trans_y,
                  phi::autotune::MatmulPlanner* planner = nullptr) {
