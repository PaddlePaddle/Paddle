...
template <typename T>
class FCFunctor<paddle::platform::CPUDeviceContext, T> {
 public:
  void operator()(const paddle::platform::CPUDeviceContext& context,
    ...
    auto blas = GetBlas<paddle::platform::CPUDeviceContext, T>(context);
...
class FCFunctor<CPUContext, T> {
 public:
  void operator()(const CPUContext& context,
                  ...
                  bool relu = false,
                  bool padding_weights = false) {
    auto blas = GetBlas<CPUContext, T>(context);
    paddle::framework::Tensor Y1;
    T* Y1_data = nullptr;
    if (padding_weights) {
      const int NN = N + 4;
