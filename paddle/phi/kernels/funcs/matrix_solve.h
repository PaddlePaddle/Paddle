...
static std::vector<int64_t> getNewDimsVec(const DDim& b_dims) {
  std::vector<int64_t> b_dims_vec = vectorize(b_dims);
...
void compute_solve_eigen(const Context& dev_ctx,
  ...
  int64_t a_batch_size = a_rank > 2 ? a.numel() / ((int64_t)n * n) : 1;
  ...
  int64_t b_batch_size = b_rank > 2 ? b.numel() / ((int64_t)b_h * b_w) : 1;
  ...
  if (a_batch_size == b_batch_size) {
    for (int64_t i = 0; i < a_batch_size; ++i) {
      ConstEigenMatrixMap a_mat(a_ptr + (int64_t)i * n * n, n, n);
      ConstEigenMatrixMap b_mat(b_ptr + (int64_t)i * b_h * b_w, b_h, b_w);
      EigenMatrixMap out_mat(out_ptr + (int64_t)i * b_h * b_w, b_h, b_w);
