...
__global__ void batch_transpose_kernel(
    T* output, const T* input, const int batch, const int M, const int N , int swizzle) {
  const int num = M * N;
...
template <typename T>
void BatchTranspose(T* output, const T* input, int batch, int m, int n, int max_grid_y) {
  ...
  dim3 physical_grid(logical_grid.x * swizzle, (logical_grid.y + swizzle - 1) / swizzle, batch);
  batch_transpose_kernel<<<physical_grid, block>>>(output, input, batch, m, n, swizzle);
}
...
template void BatchTranspose(
    float16* output, const float16* input, int batch, int m, int n, int max_grid_y);
template void BatchTranspose(
    float* output, const float* input, int batch, int m, int n, int max_grid_y);
template void BatchTranspose(
    bfloat16* output, const bfloat16* input, int batch, int m, int n, int max_grid_y);
