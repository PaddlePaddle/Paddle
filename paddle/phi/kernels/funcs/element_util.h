...
struct AnyFunctor {
  __device__ T operator()(const T *args) const { return f(args); }
  // NB: scalar is stored in higher precision!
  explicit AnyFunctor(func_t f_) : f(f_) {}
...
int GetVectorizedSizeForIO(const std::vector<const phi::DenseTensor *> &ins,
                           const std::vector<DenseTensor *> &outs) {
  int vec_size = 4;
  for (auto iter = ins.begin(); iter != ins.end(); ++iter) {
    vec_size =
        std::min<int>(vec_size, phi::GetVectorizedSize((*iter)->data<InT>()));
  }
...
struct ElementwiseDataWrapper {
  using InVecType = phi::AlignedVector<InT, VecSize>;
  using OutVecType = phi::AlignedVector<OutT, VecSize>;

  const InT *__restrict__ in_data[ET];
  OutT *out_data;
  uint32_t scalar_cal_offset;

  HOSTDEVICE ElementwiseDataWrapper(const std::vector<const DenseTensor *> &ins,
...
template <ElementwiseType ET,
          ...
          typename OutT,
          typename Functor>
__device__ inline void ScalarKernelImpl(ElementwiseWrapper data,
                                        Functor func,
                                        int tid) {
  InT ins[ET];
  OutT out;
...
template <ElementwiseType ET,
          ...
          int VecSize,
          typename Functor>
__global__ void VectorizedKernel(ElementwiseWrapper data,
                                 int main_tid,
                                 int tail_tid,
                                 Functor func) {
  ...
  if (tid < tail_tid) {
    ScalarKernelImpl<ET, ElementwiseWrapper, InT, OutT, Functor>(
        data, func, tid);
  }
}
...
void LaunchSameDimsElementwiseCudaKernel(
  ...
  switch (vec_size) {
    case 4: {
      ...
      VectorizedKernel<ET, decltype(data_wrapper), InT, OutT, 4>
    ...
    case 2: {
      ...
      VectorizedKernel<ET, decltype(data_wrapper), InT, OutT, 2>
          <<<grid_size, block_size, 0, stream>>>(
              data_wrapper, main_tid, tail_tid, func);
      break;
    }
    case 1: {
      ...
      ScalarKernel<ET, decltype(data_wrapper), InT, OutT>
          <<<grid_size, block_size, 0, stream>>>(data_wrapper, numel, func);
      break;
    }
    default: {
