...
// template <typename T, typename IndexT, typename DataArrayT, typename
// ValArrayT>
...
//       <<<grid_dims, block_dims, 0, ctx.stream()>>>(
//           input_data, out_row, cumulative_col, setter.array,
//           setter.val_array);
// }
...
// #ifdef PADDLE_WITH_HIP
//   // Prevent the pinned memory value from being covered and release the
//   memory
//   // after the launch kernel of the stream is executed (reapply pinned memory
//   // next time)
//   auto* data_alloc_released = data_alloc.release();
