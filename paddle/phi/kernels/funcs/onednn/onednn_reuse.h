...
class ReductionMKLDNNHandler
    : public MKLDNNHandlerNoCachingT<T, dnnl::reduction> {
 public:
  ReductionMKLDNNHandler(const dnnl::algorithm algo,
      ...
      : MKLDNNHandlerNoCachingT<T, dnnl::reduction>(engine, cpu_place) {
    const auto out_md = memory::desc(
        out_tz, MKLDNNGetDataType<T>(), dnnl::memory::format_tag::any);
...
class BroadcastDataMKLDNNHandler
    : public MKLDNNHandlerNoCachingT<T, dnnl::binary> {
 public:
  BroadcastDataMKLDNNHandler(const dnnl::algorithm algo,
      ...
      : MKLDNNHandlerNoCachingT<T, dnnl::binary>(engine, cpu_place) {
    const auto src0_tz = phi::vectorize(out->dims());
    const auto src0_md = dnnl::memory::desc(
        src0_tz, MKLDNNGetDataType<T>(), GetPlainMKLDNNFormat(src0_tz.size()));
