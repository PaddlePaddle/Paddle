...
struct ReduceConfig {
 ...
 private:
...
#else
    int device_id = phi::GetCurrentDeviceId();
  ...
  void SetBlockDimForReduceAny(dim3* block_dim, dim3* grid_dim) {
    ...
    int device_id = phi::GetCurrentDeviceId();
    int max_mp = phi::GetGPUMultiProcessors(device_id);
    int max_threads_per_mp = phi::GetGPUMaxThreadsPerMultiProcessor(device_id);
