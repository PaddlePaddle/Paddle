...
template <typename T>
struct HalfMulAdd {
  static __device__ __forceinline__ T apply(const T& x, const T& y, const T &z) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 530
...
struct HalfMulAdd<__nv_bfloat162> {
  static __device__ __forceinline__ __nv_bfloat162 apply(const __nv_bfloat162& x, const __nv_bfloat162& y, const __nv_bfloat162 &z) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
    return __hfma2(x, y, z);
#else
    assert(0&&"HalfMulAdd cuda version error");  
...
template <typename T, bool Bias, bool Gelu, int Batch>
__global__ void int8_weight_only_gemv_batched(const T* input,
                                      const int8_t* weight,
                                      const T* scale_list,
                                      const T* bias,
                                      T* output,
  ...
  constexpr int kWarpSize = 32;
  constexpr int kVecSize = 16;
  constexpr int kThreadPerBlock = kPerBlockWarpNum*kWarpSize;
  T vec_input[kVecSize];
  int8_t vec_weight[kVecSize];
  T vec_weight_f16[kVecSize];
  ...
  const int row_id = tile_id * 2 + ((lane_id % 8) > 3 ? 1 : 0);
  weight += tile_id * k * 2;
  // __shared__ float4 vec_input_smem[Batch][kThreadPerBlock][2];
  float v_batched[Batch];
  #pragma unroll
  for (int b = 0; b < Batch; ++b) {
    v_batched[b] = 0.0f;
  }

  float scale = static_cast<float>(scale_list[row_id]), v_bias;
#ifdef WEIGHT_ONLY_GEMV_CUDA_DEBUG
  if(threadIdx.x == 0 && blockIdx.x == 0) {
    printf("%d-%d scale:%f",
            threadIdx.x, blockIdx.x, scale); 
  }
#endif
  if (Bias) {
    v_bias = ConvertFloatFunc<T>::apply(bias[row_id]);
  ...
  for (int i = lane_id * kVecSize; i < k * 2; i += kVecSize * kWarpSize) {
    *(int4*)vec_weight = *(int4*)(weight + i);            // NOLINT
    ...
    for (int b = 0; b < Batch; ++b) {
        *(float4*)vec_input =                                 // NOLINT
            *(float4*)(input + i / 128 * 64 + (i % 64) + b*k);      // NOLINT
        *(float4*)(vec_input + 8) =                           // NOLINT
            *(float4*)(input + i / 128 * 64 + (i % 64) + 8 + b*k);  // NOLINT
#ifdef WEIGHT_ONLY_GEMV_CUDA_DEBUG
        if(threadIdx.x == 0 && blockIdx.x == 0) {
          printf("%d-%d, batch: %d, vec_input: %f ... %f \n",
                threadIdx.x, blockIdx.x, b, static_cast<float>(vec_input[0]), static_cast<float>(vec_input[15]));
        }
#endif

        #pragma unroll
        for (int p = 0; p < kVecSize; ++p) {
          v_batched[b] += ConvertFloatFunc<T>::apply(
              HalfMul<T>::apply(vec_input[p], vec_weight_f16[p / 8 + (p % 8) * 2]));
        }
#ifdef WEIGHT_ONLY_GEMV_CUDA_DEBUG
        if(threadIdx.x == 0 && blockIdx.x == 0) {
          printf("%d-%d, batch: %d, v_batched: %f \n",
                threadIdx.x, blockIdx.x, b, static_cast<float>(v_batched[b]));
        }
#endif
    }
  }
  ...
  for (int b = 0; b < Batch; ++b) {
    // Do WarpReduceSum.
    v_batched[b] += __shfl_xor_sync(0xffffffff, v_batched[b], 16);
    v_batched[b] += __shfl_xor_sync(0xffffffff, v_batched[b], 8);
    v_batched[b] += __shfl_xor_sync(0xffffffff, v_batched[b], 2);
    v_batched[b] += __shfl_xor_sync(0xffffffff, v_batched[b], 1);
  }
  #pragma unrool
  for (int b = 0; b < Batch; ++b) {
    int batched_row_id = row_id + b * n;
    if (lane_id == 0 || lane_id == 4) {
      if (Bias) {
        output[batched_row_id] = ConvertDstFunc<T>::apply(
            GeluActivation<float, Gelu>::apply(v_batched[b] * scale + v_bias));
      } else {
        output[batched_row_id] = ConvertDstFunc<T>::apply(
            GeluActivation<float, Gelu>::apply(v_batched[b] * scale));
      }
    }
  }
...
__global__ void weight_only_batched_gemv_multi_warp(const int8_t* qweight, const T* scales, const T* zeros, const T* in,
    const T* bias, T* out, const int n, const int k)
{
...
#pragma unroll
        for (int idx = 0; idx < NPerBlock; ++idx)
        {
            ...
            *(int4*)weights_quantized = *(int4*)(qweight + idx * Interleave * k / Details::kElemsPerByte + local_k / Details::kElemsPerByte);            // NOLINT
...
#pragma unroll
            for (int i = 0; i < Details::kConvertIters; ++i)
            {
                ...
                fast_cvt_4_packed_signed_i8s_to_2_half2s<T>(weights_vec + i * Details::kConvertCount,
...
#pragma unroll
            for (int idx = 0; idx < Details::kActivationAccessNum; ++idx)
            {
              ...
              *(float4*)in_v =                                 // NOLINT
                  *(float4*)(in + b * k + scale_loader.offset() + idx * Details::kActivationElemNumPerAccess);      // NOLINT
              *(float4*)(in_v + 8) =                           // NOLINT
                  *(float4*)(in + b * k + scale_loader.offset() + idx * Details::kActivationElemNumPerAccess + 8);  // NOLINT
...
#pragma unroll
                ...
                {
                    v = HalfMulAdd<HALF_2_TYPE>::apply(*reinterpret_cast<HALF_2_TYPE*>(weights_f16 + y), *reinterpret_cast<HALF_2_TYPE*>(in_v + y), v);
                }
...
#pragma unroll
                    ...
                    {
                        *reinterpret_cast<HALF_2_TYPE*>(accumulator + b * NPerBlock + x * 2)
                            = HalfMulAdd<HALF_2_TYPE>::apply(*reinterpret_cast<HALF_2_TYPE*>(weights_f16 + y * NPerBlock + x * 2),
                                ConvertDstFunc_2<HALF_2_TYPE>::apply(in_v[y]), *reinterpret_cast<HALF_2_TYPE*>(accumulator + b * NPerBlock + x * 2));
                    }
                }
            }
        }
...
void GemvWeightonlyInt8Wrapper(const Context& ctx,
  ...
  batched_int8_weight_only_gemv_launcher<DataType>(
      reinterpret_cast<const DataType*>(x),
      weight,
      reinterpret_cast<const DataType*>(weight_scale),
      reinterpret_cast<const DataType*>(bias),
