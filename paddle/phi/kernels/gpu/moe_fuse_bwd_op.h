...
__global__ void gather_with_mask_permute_kernel(const T* dy, // [s*k, d]
    ...
    float* combine_weights_shared = reinterpret_cast<float*>(shared + s_shared_num * k * sizeof(int));
    int64_t shared_idx_begin = blockIdx.x * blockDim.x * vec_size;

    for (int64_t idx = (blockIdx.x * blockDim.x + threadIdx.x) * vec_size; \
            idx < N; idx += blockDim.x * gridDim.x * vec_size) {
      int64_t si = idx / dim;
      int64_t di_begin = idx % dim;
      int64_t si_shared_begin = shared_idx_begin / dim;
      int64_t shared_stride = min(static_cast<int64_t>(blockDim.x), N - shared_idx_begin);
      ...
      for (int64_t i = 0; i < k; ++i) {
        ...
        if (combine_weights_shared[scatter_offset] > 0.f){
          ...
          int64_t permuted_id = local_iexpert * (world_size * capacity) + irank * capacity + row_in_expert;
...
template<typename T, int64_t vec_size>
__global__ void gather_with_mask_kernel(const T* dy, // [s*k, d]
                                const int* scatter_index, // [s, k]
                                const float* combine_weights, // [s, k]
                                T* dx, // [s, d]
                                int64_t num_rows, // s
...
inline int64_t max_shared_s_num(int64_t num_rows, int64_t dim, int64_t threads, int64_t vec_size) {
...
// #ifdef DEBUG_MOE_OP
//     std::cerr << "[DEBUG-BWD] launch kernel, num_active=" << num_active << ", num_rows=" << num_rows << ", dim=" << dim << std::endl;
// #endif
...
// #ifdef DEBUG_MOE_OP
//       std::cerr << "[DEBUG-BWD] gather_with_mask with vectorized, s_shared_num=" << s_shared_num << ", block=" << blocks << std::endl;
// #endif
      if (!use_all2all_permute) {
        gather_with_mask_kernel<T, 4><<<blocks, threads, shared_size, stream>>>(
                                                  dy,
                                                  scatter_index,
                                                  combine_weights,
                                                  dx,
        ...
        gather_with_mask_permute_kernel<T, 4><<<blocks, threads, shared_size, stream>>>(
                                                  dy,
                                                  scatter_index,
                                                  combine_weights,
                                                  dx,
...
// #ifdef DEBUG_MOE_OP
//       std::cerr << "[DEBUG-BWD] gather_with_mask without vectorized, s_shared_num=" << s_shared_num << ", block=" << blocks << std::endl;
// #endif

      if (!use_all2all_permute) {
        gather_with_mask_kernel<T, 1><<<blocks, threads, shared_size, stream>>>(
                                                  dy,
                                                  scatter_index,
                                                  combine_weights,
                                                  dx,
        ...
        gather_with_mask_permute_kernel<T, 1><<<blocks, threads, shared_size, stream>>>(
                                                  dy,
                                                  scatter_index,
                                                  combine_weights,
                                                  dx,
