...
template <typename T, typename Context>
void FlashAttnUnpaddedWithMaskKernel(
    const Context& ctx,
    const DenseTensor& q,
    const DenseTensor& k,
    const DenseTensor& v,
  ...
  int64_t first_dim = 1;
  const auto& origin_dims = rand_mask.dims();
...
template <typename T, typename Context>
void ScaledDotProductAttenionKernel(
    const Context& ctx,
    const DenseTensor& q,
    const DenseTensor& k,
    const DenseTensor& v,
  ...
  FlashAttnUnpaddedWithMaskKernel<T, Context>(ctx,
                                              q_t_s,
                                              k_t_s,
                                              v_t_s,
                                              cu_seqlens_q,
...
PD_REGISTER_KERNEL(scaled_dot_product_attention,
                   GPU,
                   ALL_LAYOUT,
                   phi::ScaledDotProductAttenionKernel,
                   phi::dtype::float16,
                   phi::dtype::bfloat16) {
