...
void IntBincountImpl(const Context& ctx, const T *x, int64_t n, T min_v, T max_v, BinsT *bins) {
...
#pragma unroll
  for (int i = 0; i < 2; ++i) {
    if (workspace_size > 0) {
      workspace = phi::Empty<uint8_t>(ctx, {static_cast<int64_t>(workspace_size)});
...
void IntBincount(const Context& ctx, const DenseTensor &x, int64_t low, int64_t high, int64_t out_dtype, DenseTensor* out) {
  PD_CHECK(low < high);
