...
void EncodeRotaryQkKernel(const Context &dev_ctx,
  ...
  const float *sin_emb =
      rotary_emb.data<float>() + batch_size * seq_len * dim_head;
  ...
  DataType_ *q_out_data =
      reinterpret_cast<DataType_ *>(const_cast<T *>(q.data<T>()));
  DataType_ *k_out_data =
      reinterpret_cast<DataType_ *>(const_cast<T *>(kv.data<T>()));
