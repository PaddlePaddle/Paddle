  ...
  if (is_half2) {                                              \
    if (grid.x % 4 == 0) {                                     \
      ...
      softmax_kernel_v5_half2<__half, ITEMS_PER_THREAD, 4>     \
          <<<grid, block, 0, stream>>>((half*)buffer,          \
...
void MoeKernel(const Context& ctx,
  ...
  long int bytes = getWorkspaceSize<T>(num_rows,
                                       hidden_size,
                                       inter_size,
                                       num_experts,
                                       k,
  ...
  permuted_data_ = (T*)(permuted_experts_ + num_experts * k);
  padded_expert_scales_ = (T*)(permuted_data_ + buf_size);
  total_rows_before_expert_ =
      (int64_t*)(padded_expert_scales_ + padded_num_moe_inputs);
  sorted_softmax_output_ = (T*)(total_rows_before_expert_ + padded_experts);
  attr_mask_ = (T*)(sorted_softmax_output_ + padded_num_moe_inputs);
  fc1_result_ = (T*)(attr_mask_ + num_moe_inputs);
