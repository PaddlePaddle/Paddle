...
__global__ void ReduceSumWithSubtract(
  ...
  __syncthreads();
  sum_val = phi::funcs::BlockReduceSum<MPType>(sum_val, FULL_MASK);
  if (threadIdx.x == 0) {
...
__global__ void ReduceMaxWithSubtract(const T* x,
  ...
  __syncthreads();
  max_val = phi::funcs::BlockReduceMax<MPType>(max_val, FULL_MASK);
  if (threadIdx.x == 0) {
...
__global__ void ReduceMinWithSubtract(const T* x,
  ...
  __syncthreads();
  min_val = phi::funcs::BlockReduceMin<MPType>(min_val, FULL_MASK);
  if (threadIdx.x == 0) {
