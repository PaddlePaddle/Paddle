  ...
  if (xdim == y.dims()) {  // same shape
    ...
    if (p == 0) {
      ...
      ReduceMaxWithSubtract<T>
          <<<config.block_per_grid.x, config.thread_per_block.x, 0, stream>>>(
              x_ptr, y_ptr, i_ptr, n);
      // phi::funcs::ReduceKernel<T, T, kps::MaxFunctor,
      // kps::IdentityFunctor<T>>(
      //     dev_ctx, intermediate, out, kps::IdentityFunctor<T>(),
      //     reduce_axis);
      ...
      // phi::funcs::ReduceKernel<T, T, kps::MinFunctor,
      // kps::IdentityFunctor<T>>(
      //     dev_ctx, intermediate, out, kps::IdentityFunctor<T>(),
      //     reduce_axis);
      phi::MinRawKernel<T, Context>(
          dev_ctx, intermediate, reduce_axis, false, true, out);
