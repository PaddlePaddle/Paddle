...
void DistributedFusedLambInitOpKernel(
  ...
  auto *param_order_t = dev_ctx.template Alloc<int>(param_order);
  ...
  TensorFillConstant<float>(dev_ctx, beta2_pow, {1}, beta1);
  ...
  param_info->Resize({8});
  auto *param_info_t = dev_ctx.template Alloc<int>(param_info);
