...
void apply_moe_dispatch_fwd(const Context& dev_ctx,
  ...
  DenseTensor xpanded_source_row_to_expanded_dest_row_tensor = phi::Empty<int, Context>(dev_ctx, IntArray({num_rows, k}));
  ...
  DenseTensor active_cnt_tensor = phi::Empty<int, Context>(dev_ctx, IntArray({1}));
  ...
  DenseTensor ws_ptr_tensor = phi::Empty<int8_t, Context>(dev_ctx, IntArray({bytes}));
...
*/
 #ifdef DEBUG_MOE_OP
  print_to_screen1<int>(expert_id, 8, 16, std::string("expert-id after modified"));
#endif
  ...
  cudaMemcpy(&num_active_host_v2, expert_offset + num_experts - 1, sizeof(int64_t), cudaMemcpyDeviceToHost);  
  std::cerr << "[DEBUG] num_active v2: " << num_active_host_v2 << std::endl;
  print_to_screen1<int>(permuted_experts_, 8, num_active_host_v2+2, std::string("<reprint>expert-id after permute"));
  ...
  // );
#ifdef DEBUG_MOE_OP
  print_to_screen1<int>(scatter_index, 8, 16, std::string("scatter_index after pad"));
#endif
  // cudaMemcpy(scatter_index, permuted_rows_, sizeof(int64_t) * k * num_rows, cudaMemcpyDeviceToDevice);
  // cudaMemcpy(combine_weights, expert_scales_float, sizeof(float) * k * num_rows, cudaMemcpyDeviceToDevice);
  return;
}
...
void moe_dispatch_fwd(const Context& dev_ctx,
  ...
  apply_moe_dispatch_fwd<T, Context>(dev_ctx,
                                      x.data<T>(),
