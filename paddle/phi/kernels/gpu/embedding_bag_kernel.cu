...
__global__ void EmbeddingBag(T *output, const T *params, const IdT *input,
                             const T *weight, const int output_dim,
                             const int sequence_length, CalMode mode ) {
...
struct EmbeddingBagCUDAFunctor {
    ...
    void apply() {
        ...
        EmbeddingBag<T, IdT, kThreadsPerBlock>
            <<<grids, kThreadsPerBlock, 0, stream>>>(
