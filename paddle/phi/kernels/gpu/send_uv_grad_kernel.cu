...
void CalculateGrad(const Context& dev_ctx,
  ...
  if (message_op == "ADD") {
    if (!reduce) {
      ...
      const dim3 block_tmp(ntx, nty);
      GraphSendUVGradCUDAKernel<T, IndexT>
          <<<grid_tmp, block_tmp, 0, dev_ctx.stream()>>>(out_grad,
                                                         d_index,
                                                         s_index,
                                                         index_size,
                                                         bcast_info.out_len,
...
template <typename Context, typename T, typename IndexT>
void GraphSendUVGradOpCUDAKernelLaunchHelper(const Context& dev_ctx,
                                             const DenseTensor& x,
                                             const DenseTensor& y,
  ...
  cudaMemsetAsync(y_grad_data, 0, memset_bytes_y, dev_ctx.stream());
#endif
