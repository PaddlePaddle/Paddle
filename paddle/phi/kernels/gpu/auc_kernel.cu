...
__global__ void ClearObsoleteDataKernel(int64_t *pos,
  ...
  int64_t cur_step_begin = (int64_t)cur_step_index * bucket_length;
  int64_t sum_step_begin = (int64_t)slide_steps * bucket_length;
...
void AucKernel(const Context &dev_ctx,
  ...
  statAuc<T, Context>(dev_ctx,
                      ...
                      origin_stat_neg,
                      is_fake_data);
  int64_t sum_offset = (int64_t)slide_steps * (num_thresholds + 1);
