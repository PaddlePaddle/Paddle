...
void QkvTransposeSplitKernel(const Context& dev_ctx,
    ...
    // const auto* input_input = &input_ids;
    // const auto& input_dims = input_input->dims();

    const int token_num = qkv_dims[0];
    const int bsz = seq_lens.dims()[0];
    const int max_seq_len = input_ids.dims()[1]; //max_seq_len_tensor.copy_to(paddle::CPUPlace(), false).data<int>()[0];
    ...
    // }
    // // cudaMemsetAsync(q_out_ptr,
    // //                   0,
    // //                   2 * num_head*max_seq_len*head_size,
    // //                   cu_stream);
    // funcs::SetConstant<Context, T> set_zero;
    // set_zero(ctx, dx, static_cast<T>(0));
