...
__global__ void fusedQKV_transpose_split_kernel(T* q_buf,
  ...
  const int32_t fused_hidden_size =
  ...
  for (int32_t linear_index = global_thread_idx * VecSize,
    ...
    const int32_t qkv_id =
        bias_idx < hidden_size
            ? 0
            : (bias_idx - hidden_size) / (kv_head_num * size_per_head) + 1;
    const int32_t head_id =
        qkv_id == 0 ? bias_idx / size_per_head
                    : (bias_idx - hidden_size) / size_per_head % kv_head_num;
    ...
    if (qkv_id == 0) {
      ...
      phi::Store<T, VecSize>(
          src_vec,
          &k_buf[target_batch_id * kv_head_num * max_len_this_time *
                     size_per_head +
                 head_id * max_len_this_time * size_per_head +
                 seq_id * size_per_head + size_id]);
    } else {
      phi::Store<T, VecSize>(
          src_vec,
          &v_buf[target_batch_id * kv_head_num * max_len_this_time *
                     size_per_head +
                 head_id * max_len_this_time * size_per_head +
                 seq_id * size_per_head + size_id]);
    }
...
void QkvTransposeSplitKernel(const Context& dev_ctx,
  ...
  fusedQKV_transpose_split_kernel<T, PackSize>
      <<<grid_size, blocksize, 0, cu_stream>>>(q_out_ptr,
                                               k_out_ptr,
                                               v_out_ptr,
                                               qkv.data<T>(),
