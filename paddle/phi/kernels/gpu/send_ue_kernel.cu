...
#include "paddle/phi/kernels/send_ue_kernel.h"
...
__global__ void GraphSendUECUDAKernel(const T* x_data,
                                      const T* y_data,
...
void GraphSendUEOpCUDAKernelLaunchHelper(const Context& ctx,
                                         const DenseTensor& x,
                                         const DenseTensor& y,
  ...
  if (message_op == "ADD") {
    ...
    GraphSendUECUDAKernel<T, IndexT, funcs::AddFunctor<T>>
    ...
    GraphSendUECUDAKernel<T, IndexT, funcs::MultiplyFunctor<T>>
        <<<grid, block, 0, ctx.stream()>>>(
            x_data,
            y_data,
            s_index,
...
void SendUEKernel(const Context& ctx,
                  const DenseTensor& x,
                  const DenseTensor& y,
                  const DenseTensor& src_index,
                  const DenseTensor& dst_index,
  ...
  if (index_type == phi::DataType::INT32) {
    GraphSendUEOpCUDAKernelLaunchHelper<Context, T, int32_t>(
        ctx, x, y, src_index, dst_index, message_op, out);
  } else if (index_type == phi::DataType::INT64) {
    GraphSendUEOpCUDAKernelLaunchHelper<Context, T, int64_t>(
        ctx, x, y, src_index, dst_index, message_op, out);
  }
}
