...
void PerSort(const auto exec_policy,
  ...
  if (stable) {
    ...
    thrust::sort_by_key(exec_policy, out_data + start, out_data + end, ids_data + start);
    if (descending) {
...
template <typename T, typename IndType>
__global__ void merge_kernel(const T* A, size_t sizeA,
                             const T* B, size_t sizeB,
                             const IndType* ids_A,
                             const IndType* ids_B,
                             T* out,
                             IndType* out_ids) {
...
void ArgsortKernel(const Context& dev_ctx,
  ...
  if (size == in_dims[axis]) {
    ...
    const int64_t per_number = 1 << 30;
    int64_t start = 0;
    int64_t end = std::min(start + per_number, size);
    if (end == size) {
      PerSort<T, int64_t>(exec_policy, out_data, ids_data, start, end, stable, descending);
    }
    else{
      // Sorting the segments and then merging them
      T* temp_data;
      int64_t* temp_ids;
      cudaMalloc((void**)&temp_data, size * sizeof(T));
      cudaMalloc((void**)&temp_ids, size * sizeof(int64_t));
      while(start != size){
        PerSort<T, int64_t>(exec_policy, out_data, ids_data, start, end, stable, false);
        if(start != 0){
          ...
          merge_kernel<<<config.block_per_grid.x,
                        ...
                        cu_stream>>>(
            out_data, start, 
            out_data + start, end - start, 
            ids_data, ids_data + start, 
            temp_data, temp_ids);
