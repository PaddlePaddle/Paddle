...
void ScatterGradKernel(const Context &ctx,
  ...
  if (reducer == "add") {
    ...
    if (updates_grad) {
      ...
      DenseTensor flag_cpu;
      phi::Copy(ctx,
                src_num_zeros_greater_one_any,
                phi::CPUPlace(),
                false,
                &flag_cpu);
      bool out_data = flag_cpu.template data<bool>()[0];

      // auto src_num_zeros_greater_one_any = Sum<bool, Context>(ctx,
      // src_num_zeros_greater_one, {}, x.dtype(), false); auto out_data =
