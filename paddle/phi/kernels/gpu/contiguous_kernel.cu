...
__global__ void ContiguousCaseZeroFunc(
    const T* input_data,
    T* out_data,
    phi::Array<int64_t, phi::DDim::kMaxRank> input_stride) {
...
__global__ void ContiguousCaseOneFunc(
  ...
  if (x < x_max) {
    ...
    int64_t coordinate[phi::DDim::kMaxRank];
...
void ContiguousKernel(const Context& dev_ctx,
  ...
  phi::Array<int64_t, phi::DDim::kMaxRank> input_stride;
  phi::Array<int64_t, phi::DDim::kMaxRank> input_dims;
  ...
  for (int i = 0; i < 3 && i < input_dims.size(); i++) {
  ...
  if (rank <= 6 && tmp <= 1024 &&
    ...
    phi::Array<int64_t, 3> cur_input_dims;
    ...
    switch (rank) {
      case 1:
        grid.x = (input_dims[rank - 1] + block.x - 1) / block.x;
        ...
        break;
      case 2:
        grid.x = (input_dims[rank - 1] * input_dims[rank - 2] + block.x - 1) /
                 block.x;
        cur_input_dims[0] = input_dims[rank - 1];
