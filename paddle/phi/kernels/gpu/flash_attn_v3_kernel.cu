...
template <typename T, typename Context>
void FlashAttnV3VarLenWrapper(const Context &ctx,
                            const DenseTensor &q,
                            const DenseTensor &k,
                            const DenseTensor &v,
                            const DenseTensor &cu_seqlens_q,
                            ...
                            DenseTensor *out,
                            DenseTensor *softmax_lse) {
  FlashAttnV3VarLenKernel<T, Context>(
    ctx,
    q,
    k,
    v,
...
template void FlashAttnV3VarLenWrapper<phi::dtype::bfloat16, phi::GPUContext>(
                            ...
                            const paddle::optional<phi::DenseTensor> &q_v_,
                            const paddle::optional<phi::DenseTensor> &q_descale_,
                            const paddle::optional<phi::DenseTensor> &k_descale_,
                            const paddle::optional<phi::DenseTensor> &v_descale_,
                            const float softmax_scale,
                            bool is_causal,
                            int window_size_left,
                            int window_size_right,
...
template void FlashAttnV3VarLenWrapper<phi::dtype::float16, phi::GPUContext>(
                            const phi::GPUContext &ctx,
                            const phi::DenseTensor &q,
                            const phi::DenseTensor &k,
                            const phi::DenseTensor &v,
