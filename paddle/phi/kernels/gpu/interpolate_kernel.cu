...
inline backends::gpu::GpuLaunchConfig GetGpuLaunchConfig3D(
    const phi::GPUContext& context, int num_img, int height, int width) {
...
static void Interpolate1DCUDAFwd(
  ...
  auto list_new_shape_tensor = *size_tensor;
  float scale_w = -1;
  if (list_new_shape_tensor.size() > 0) {
    // have size tensor
    auto new_size = funcs::get_new_shape(list_new_shape_tensor);
    out_w = new_size[0];
  } else {
  ...
  if ("nearest" == interp_method) {
    if (data_layout == DataLayout::kNCHW) {
      ...
      backends::gpu::GpuLaunchConfig config_3d =
          GetGpuLaunchConfig3D(dev_ctx, nc, out_h, out_w);
