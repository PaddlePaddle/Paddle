...
__global__ void IndexElementwiseGetGradKernel(
    int64_t N,
    const char* in_ptr,
    char* out_ptr,
...
void GPUIndexElementwiseGetGrad(const phi::GPUContext& ctx,
  ...
  IndexElementwiseGetGradKernel<T, IndexT, nt, vt><<<grid, block, 0, stream>>>(
      N, in_ptr, out_ptr, index_ptrs, sizes, strides, num_indices, offset_calc);
}
