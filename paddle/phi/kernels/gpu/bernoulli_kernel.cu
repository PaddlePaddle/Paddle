...
void BernoulliKernel(const Context& dev_ctx,
  ...
  size_t grid_size64 = gpu_config.GetGridSize();
  size_t block_size64 = gpu_config.GetBlockSize();
  PADDLE_ENFORCE_LE_UINT32_MAX(grid_size64, "bernoulli grid.x");
  PADDLE_ENFORCE_LE_UINT32_MAX(block_size64, "bernoulli block.x");
