...
// 'curand_uniform4' generate 4 random number each time
template <typename T>
__global__ void BernoulliCudaKernel(
...
void BernoulliKernel(const Context& ctx,
  ...
  auto gpu_config = phi::backends::gpu::GetGpuLaunchConfig1D(ctx, size, 4);
  size_t grid_size = gpu_config.GetGridSize();
  size_t block_size = gpu_config.GetBlockSize();

  BernoulliCudaKernel<<<grid_size, block_size, 0, ctx.stream()>>>(
      size, seed, offset, x_data, out_data);
}
