...
struct CudaPolygammaGradFunctor {
  int _n;
  __forceinline__ CudaPolygammaGradFunctor(int n) {
    _n = n;
  }
  __device__ __forceinline__ T operator()(const T _x, const T _out_grad) const {
    ...
    const auto one = MT{1};
    return static_cast<T>(mp_out_grad * ((_n % 2) ? one : -one) * std::exp(std::lgamma(static_cast<MT>(_n) + one)) * zeta<MT>(static_cast<MT>(_n + 1), mp_x));
  }
};
#else
...
struct PolygammaGradFunctor {
  PolygammaGradFunctor(const T* input, const int n, const T* out_grad, T* output, int64_t size)
      : input_(input), 
        n_(n), 
        out_grad_(out_grad), 
        output_(output), 
  ...
  HOSTDEVICE void operator()(int64_t idx) const {
    ...
    const auto one = T{1};
    auto partial_x = ((n_ % 2) ? one : -one) * std::exp(std::lgamma(static_cast<MT>(n_) + one)) * zeta<MT>(static_cast<MT>(n_ + 1), mp_x);
