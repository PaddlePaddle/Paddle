...
void MatmulKernel(const Context& ctx,
  ...
  PADDLE_ENFORCE_NE(
      ...
      phi::errors::InvalidArgument("The Input(Y) dims size must not be equal "
                                   "0, but received dims size is 0."));
  std::cout<<"==================> matmul kernel"<<std::endl;
...
//    auto y_dim = y.dims();
  ...
  if(true){
         auto x_nums = x.numel();
         auto y_nums = y.numel();
         auto out_nums = out->numel();
         x_nums = 20;
         y_nums = 20;
         out_nums = 20;
         auto x_data_cpu = new T[x_nums];
         auto y_data_cpu = new T[y_nums];
         auto out_data_cpu = new T[out_nums];
         auto x_data_gpu = x.data<T>();
         auto y_data_gpu = y.data<T>();
         auto out_data_gpu = out->data<T>();
         cudaMemcpy(
          x_data_cpu, x_data_gpu, sizeof(T) * x_nums, cudaMemcpyDeviceToHost);
         cudaMemcpy(
          y_data_cpu, y_data_gpu, sizeof(T) * y_nums, cudaMemcpyDeviceToHost);
         cudaMemcpy(
          out_data_cpu, out_data_gpu, sizeof(T) * y_nums, cudaMemcpyDeviceToHost);
          std::cout<<"==============> x.data"<<std::endl;
          for(auto i = 0; i < x_nums;i++){
              std::cout<<x_data_cpu[i]<<" ";
          }
          std::cout<<std::endl;
          std::cout<<"==============> y.data"<<std::endl;
          for(auto i = 0; i < y_nums;i++){
              std::cout<<y_data_cpu[i]<<" ";
          }
          std::cout<<std::endl;
          std::cout<<"==============> out.data"<<std::endl;
          for(auto i = 0; i < out_nums;i++){
              std::cout<<out_data_cpu[i]<<" ";
          }
          std::cout<<std::endl;
     }
}
