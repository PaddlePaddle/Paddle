...
template <typename ScalarT, typename AccscalarT>
HOSTDEVICE ScalarT digamma(ScalarT x_) {
  using MPTypeAccscalar = typename phi::dtype::MPTypeTrait<AccscalarT>::Type;

  MPTypeAccscalar x = static_cast<MPTypeAccscalar>(x_);
  constexpr MPTypeAccscalar PSI_10 = 2.25175258906672110764;
  constexpr MPTypeAccscalar PI = 3.14159265358979323846;

  if (x == 0) {
    return INFINITY;
  }
  MPTypeAccscalar additional_summand = 0;
  ...
  static const MPTypeAccscalar A[] = {
  ...
  MPTypeAccscalar y = 0;
  if (x < 1.0e17f) {
    MPTypeAccscalar z = 1.0 / (x * x);
    y = z * polevl<MPTypeAccscalar>(z, A, 6);
  }
...
HOSTDEVICE ScalarT gamma_grad(ScalarT alpha_, ScalarT x_) {
  ...
  MPTypeAccscalar x = static_cast<MPTypeAccscalar>(x_);
  MPTypeAccscalar alpha = static_cast<MPTypeAccscalar>(alpha_);

  if (x < 0.8f) {
    MPTypeAccscalar numer = 1.0f;
    MPTypeAccscalar denom = alpha;
    auto series1 = numer / denom;
    auto series2 = numer / (denom * denom);
    for (int i = 1; i <= 5; ++i) {
      numer *= -x / static_cast<MPTypeAccscalar>(i);
      denom += 1;
      series1 += numer / denom;
    ...
    const auto gamma_cdf_alpha =
        (COMPAT_LOG(x) - digamma<MPTypeAccscalar, MPTypeAccscalar>(alpha)) *
            gamma_cdf -
        pow_x_alpha * series2;
  ...
  if (alpha > 8.0f) {
    ...
    const auto term3 = COMPAT_POW(x - alpha - alpha * COMPAT_LOG(x / alpha),
                                  static_cast<MPTypeAccscalar>(-1.5));
  ...
  static const MPTypeAccscalar coef_uv[3][8] = {
      {0.16009398,
       -0.094634809,
       0.025146376,
       -0.0030648343,
...
struct GammaGradFunctor {
  ...
  HOSTDEVICE void operator()(int64_t index) {
    grad_[index] = gamma_grad<T, T>(alpha_[index], x_[index]);
