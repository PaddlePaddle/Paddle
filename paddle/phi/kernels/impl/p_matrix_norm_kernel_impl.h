...
void PMatrixNormKernel(const Context& dev_ctx,
  ...
  if (porder == INFINITY || porder == -INFINITY || porder == 1 ||
    ...
    if (porder == INFINITY || porder == -INFINITY) {
      ...
      x_sum.Resize(detail::GetSumvalueDim(
          x_input.dims(), trans_dims[x_rank - 1], keepdim, 0));
      dev_ctx.template Alloc<T>(&x_sum);
      phi::SumKernel<T, Context>(
    ...
    if (!keepdim) {
      if (porder == INFINITY || porder == 1) {
        phi::MaxKernel<T, Context>(
            dev_ctx, x_sum, phi::IntArray({-1}), keepdim, out);
      } else {
      ...
      if (porder == INFINITY) {
        phi::MaxKernel<T, Context>(
            dev_ctx, x_sum, phi::IntArray({-2}), keepdim, out);
      } else if (porder == 1) {
        phi::MaxKernel<T, Context>(
            dev_ctx, x_sum, phi::IntArray({-1}), keepdim, out);
    ...
    SqrtKernel<T, Context>(dev_ctx, max_min_singular_tensor, out);
  }
}
}  // namespace phi
