...
template <typename T>
__host__ __device__ T lanczos_sum_expg_scaled(T x) {
  ...
  static const T lanczos_sum_expg_scaled_denom[13] = {1.,
                                                      66.,
                                                      1925.,
                                                      32670.,
                                                      357423.,
...
template <typename T>
__host__ __device__ T _igam_helper_fac(T a, T x) {
...
template <typename T>
__host__ __device__ T _igam_helper_series(T a, T x) {
...
template <typename T>
__host__ __device__ T _igam_helper_asymptotic_series(T a, T x, bool igam) {
  // Compute igam/igamc using DLMF 8.12.3/8.12.4 [igam1]

  static const T d[25][25] = {
  ...
  T lambda = x / a;
  T sigma = (x - a) / a;
  T eta, res, ck, ckterm, term, absterm;
  T absoldterm = INFINITY;
  T etapow[25] = {1};
  T sum = 0;
  T afac = 1;
...
template <typename T>
__host__ __device__ T _igamc_helper_series(T a, T x) {
...
template <typename T>
__host__ __device__ T _igamc_helper_continued_fraction(T a, T x) {
  // Compute igamc using DLMF 8.9.2. [igam1]
  int i;
  T ans, ax, c, yc, r, t, y, z;
  T pk, pkm1, pkm2, qk, qkm1, qkm2;
  int MAXITER = 2000;
  static T MACHEP = std::is_same<T, double>::value ? 1.11022302462515654042E-16
                                                   : 5.9604644775390625E-8;
  static T BIG =
      std::is_same<T, double>::value ? 4.503599627370496e15 : 16777216.;
  static T BIGINV = std::is_same<T, double>::value ? 2.22044604925031308085e-16
                                                   : 5.9604644775390625E-8;
...
template <typename T>
__host__ __device__ T compute_igammac(T x, T a) {
  static T SMALL = 20.0;
...
template <typename T>
__host__ __device__ T compute_igamma(T x, T a) {
  static T SMALL = 20.0;
  static T LARGE = 200.0;
  static T SMALLRATIO = 0.3;
  static T LARGERATIO = 4.5;
...
template <typename T>
struct CudaIgammaFunctor {
  __device__ __forceinline__ T operator()(const T _x, const T _a) const {
    using MT = typename phi::dtype::MPTypeTrait<T>::Type;
    const MT mp_x = static_cast<MT>(_x);
    const MT mp_a = static_cast<MT>(_a);
    return compute_igamma<MT>(mp_x, mp_a);
  }
};

template <typename T>
struct CudaIgammacFunctor {
  __device__ __forceinline__ T operator()(const T _x, const T _a) const {
    using MT = typename phi::dtype::MPTypeTrait<T>::Type;
    const MT mp_x = static_cast<MT>(_x);
    const MT mp_a = static_cast<MT>(_a);
...
template <typename T>
static inline T _igam_helper_asymptotic_series(T a, T x, bool igam) {
  // Compute igam/igamc using DLMF 8.12.3/8.12.4 [igam1]
...
template <typename T>
static inline T _igamc_helper_series(T a, T x) {
  // Compute igamc using DLMF 8.7.3 [igam1]. This is related to the series in
  // _igam_helper_series but extra care is taken to avoid cancellation.
...
struct IgammacFunctor {
  IgammacFunctor(const T* input, const T* other, T* output, int64_t numel)
      : input_(input), other_(other), output_(output), numel_(numel) {}
