...
void MatMulFunctionLt(const Context &dev_ctx,
                      const DenseTensor &X,
                      const DenseTensor &Y,
  ...
  PADDLE_ENFORCE_GPU_SUCCESS(
      phi::dynload::cublasLtMatmulDescSetAttribute(operation_desc,
                                                   CUBLASLT_MATMUL_DESC_TRANSA,
                                                   &op_transpose,
                                                   sizeof(op_transpose)));
...
void MatmulKernel(const Context &dev_ctx,
  ...
  PADDLE_ENFORCE_NE(
      ...
      phi::errors::InvalidArgument("The Input(Y) dims size must not be equal 0,"
                                   " but reviced dims size is 0. "));
  MatMulFunctionLt<Context, T>(dev_ctx, x, y, out, transpose_x, transpose_y);
}
