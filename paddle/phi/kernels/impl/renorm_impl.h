...
template <typename T>
void RenormFunc(const phi::GPUContext& dev_ctx,
                const T* x_data,
                T* out_data,
                float p,
                int dim,
  ...
  pow_value.Resize(make_ddim({pre_mul, dimension_each, dim_divisor}));
  dim_value.Resize(make_ddim({dimension_each}));
...
template <typename T>
void RenormGradFunc(const phi::GPUContext& dev_ctx,
                    const T* x_data,
                    const T* dout_data,
                    T* dx_data,
                    float p,
  ...
  pow_value.Resize(make_ddim({pre_mul, dimension_each, dim_divisor}));
  mul_value.Resize(make_ddim({pre_mul, dimension_each, dim_divisor}));
  dim_value.Resize(make_ddim({dimension_each}));
  dim_power_sum.Resize(make_ddim({dimension_each}));
  weight_derivative.Resize(make_ddim({dimension_each}));
  T* pow_value_data = dev_ctx.template Alloc<T>(&pow_value);
