...
#include "paddle/phi/kernels/huber_loss_div_delta_grad_kernel.h"

namespace phi {

template <typename T>
struct HuberLossDivDeltaBackward {
  HOSTDEVICE HuberLossDivDeltaBackward(const T& delta, T sign)
      : sign(sign), delta(delta) {}

  HOSTDEVICE T operator()(const T& val) const {
    T abs_val = abs(val);
    if (abs_val <= delta) {
      return sign * val / delta;
    } else {
      if (val > static_cast<T>(0)) {
        return sign;
      } else {
        return static_cast<T>(-1) * sign;
      }
    }
  }
...
void HuberLossDivDeltaGradKernel(const Context& dev_ctx,
  ...
  if (input_grad) {
    ...
    eigen_input_grad.device(place) = eigen_residual.unaryExpr(
        HuberLossDivDeltaBackward<T>(delta_, static_cast<T>(-1.0)));
