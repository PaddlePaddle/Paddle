...
void LUGradKernel(const Context& dev_ctx,
    ...
    L_narrow_mH = Transpose2DTo6D(dev_ctx, L_narrow_mH);
    U_narrow_mH = Transpose2DTo6D(dev_ctx, U_narrow_mH);
    ...
    if (m <= n) {
        ...
        DenseTensor U_complement_mH = Transpose2DTo6D(dev_ctx, U_complement);

        Tensor_Conj<Context, T>(
        ...
        phi_complement_l.template Alloc<T>(&phi_complement_l);
        const auto H = phidims[phidims.size() - 2];
        const auto W = phidims[phidims.size() - 1];
        phi::funs::ForRange<Context> x_for_range(dev_ctx,
        ...
        phi_mH = Transpose2DTo6D(dev_ctx, phi_mH);
        ...
        psi_principal = Transpose2DTo6D(dev_ctx, psi_principal);
        slice_starts[0] = 0;
        slice_starts[1] = 0;
        slice_ends[0] = k;
        slice_ends[1] = k;
        ...
        DenseTensor L_complement, L_grad_complement, phi_complement, phi_complement_u;
        Tensor_narrow<Context, T>(dev_ctx, &L, &L_complement, k, m, 0, k);
        Tensor_narrow<Context, T>(
            dev_ctx, &out_grad, &L_grad_complement, k, m, 0, k);
        DenseTensor L_complement_mH = Transpose2DTo6D(dev_ctx, L_complement);
        Tensor_Conj<Context, T>(dev_ctx, L_complement_mH, &L_complement_mH);

        auto mat_dim_g = phi::funcs::CreateMatrixDescriptor(
            L_grad_complement.dims(), 0, false);
        auto mat_dim_u = phi::funcs::CreateMatrixDescriptor(L_complement_mH.dims(), 0, false);
        ...
        blas.MatMul(Pmat,
                    ...
                    &psi_tmp,
                    static_cast<T>(0));
        psi_tmp = Transpose2DTo6D(dev_ctx, psi_tmp);

        Tensor_Conj<Context, T>(dev_ctx, U_narrow, &U_narrow_mH);
        phi::TriangularSolveKernel<T, Context>(
            dev_ctx, U_narrow_mH, psi_tmp, true, false, false, &psi);
        *x_grad = Transpose2DTo6D(dev_ctx, psi);
    }
}
