...
void AddNArrayKernel(const Context& dev_ctx,
  ...
  for (size_t i = in_place ? 1 : 0; i < x.size(); ++i) {
    ...
    for (size_t j = 0; j < in_array.size(); ++j) {
      if (in_array[j]->IsInitialized() && (in_array[j]->numel() != 0)) {
        if (j >= out.size()) {
          out.resize(j + 1);
        }
        if (!out[j]->IsInitialized() || (out[j]->numel() == 0)) {
          Copy<Context>(
              dev_ctx, *in_array[j], in_array[j]->place(), false, out[j]);
          out[j]->set_lod(in_array[j]->lod());
        } else {
          PADDLE_ENFORCE_EQ(
              out[j]->lod(),
              in_array[j]->lod(),
              phi::errors::InvalidArgument(
                  ...
                  j,
                  j));
          auto in = EigenVector<T>::Flatten(*in_array[j]);
          auto result = EigenVector<T>::Flatten(*out[j]);
