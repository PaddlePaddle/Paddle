...
static void GetBroadcastFromDims(const int64_t x_ndim,
                                 const std::int64_t* x_dims,
                                 const int64_t y_ndim,
...
static int64_t GetIndexMessage(const int64_t n,
                               const int64_t* dims,
                               const int64_t* index) {
  int64_t sum = 0;
...
static void IndexIncreaseFromDims(const int64_t ndim,
                                  const int64_t* dims,
                                  int64_t* index) {
...
void MatMulFunctionImplWithBlas(
  ...
  const int64_t batch_dim = ndim - 2;
  // broadcast message
...
void MatMulFunctionImplWithCublasLt(
  ...
  if (x_ndim == 1) {
    ...
    if (trans_y) {
      ...
      const int64_t batch_size = Y.numel() / (static_cast<int64_t>(M) * N);
      if (batch_size == 1) {
