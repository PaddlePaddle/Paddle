  ...
  if (reduce_all) {
    ...
    if (dim_vec.size() == 1) {
      ...
      if (!keep_dim) {
        ...
        grad_expanded = phi::Reshape<T, Context>(dev_ctx, out_grad, grad_shape);

        auto result_shape = common::vectorize<int64_t>(out.dims());
        result_shape.insert(result_shape.begin() + dim, 1);
        result_expanded = phi::Reshape<T, Context>(dev_ctx, out, result_shape);
      }
      ...
      if (total_zeros == 0) {
        ...
        DenseTensor grad_result =
            phi::Multiply<T, Context>(dev_ctx, grad_expanded, result_div_input);
        phi::Copy(dev_ctx, grad_result, dev_ctx.GetPlace(), false, x_grad);
      } else {
