...
__global__ void weight_permute_kernel_wint8(
  ...
  int total_n){
    for(int linear_idx = blockIdx.x * blockDim.x + threadIdx.x; 
        ...
        output_data_dev[permute_index] = *reinterpret_cast<int8_t*>(&shift_quant_weight);
    }
}
...
template<typename GPUContext>
  ...
  __global__ void per_channel_quant_gpu(const T* weight_data,
    ...
    if(n<total_vec_n){
      ...
      #pragma unroll
      for(int i=0;i<VectorSize;++i){
        scale[i] = static_cast<T>(abs_max[i]/static_cast<float>(127.0));
      }
      *(int4*)(&vec_scale_data[n]) = *(int4*)(&scale);
      for(int k=0;k<total_k;++k){
        ...
        *(int4*)(&weight) = *(int4*)(&vec_weight_data_ptr[linear_index]);
        #pragma unroll
        for(int i=0;i<VectorSize;++i){
          float scaled_weight = (static_cast<float>(weight[i])/static_cast<float>(abs_max[i]))*static_cast<float>(127.0);
          int8_t clipped_weight = static_cast<int8_t>(lroundf(fmaxf(-127.0f, fminf(127.0f, scaled_weight))));
