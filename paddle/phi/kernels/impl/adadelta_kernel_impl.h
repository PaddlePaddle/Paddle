...
void AdadeltaKernel(const Context& dev_ctx,
  ...
  if (paddle::platform::is_cpu_place(dev_ctx.GetPlace())) {
    ...
    eigen_param_out.device(place) =
        (eigen_param.template cast<MPDType>() + lr.broadcast(m_dsize) * update).template cast<T>();
    } else {
      eigen_param_out.device(place) = eigen_param + lr.broadcast(m_dsize) * update.template cast<T>();
    }
  }
