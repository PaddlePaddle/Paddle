...
PADDLE_API std::tuple<Tensor, Tensor> add_grad(const Tensor& x, const Tensor& y, const Tensor& out_grad) {
  ...
  if (x.is_sparse_coo_tensor() && y.is_sparse_coo_tensor() && out_grad.is_sparse_coo_tensor()) {
    ...
    auto* kernel_out_1 = SetSparseKernelOutput(&std::get<1>(api_output), TensorType::SPARSE_COO);
    auto input_x = PrepareDataForSparseCooTensor(x);
  ...
  if (x.is_sparse_csr_tensor() && y.is_sparse_csr_tensor() && out_grad.is_sparse_csr_tensor()) {
    ...
    std::tuple<Tensor, Tensor> api_output;
    auto* kernel_out_0 = SetSparseKernelOutput(&std::get<0>(api_output), TensorType::SPARSE_CSR);
...
PADDLE_API std::tuple<Tensor, Tensor, Tensor> addmm_grad(const Tensor& input, const Tensor& x, const Tensor& y, const Tensor& out_grad, float alpha, float beta) {
  ...
  if (input.is_sparse_csr_tensor() && x.is_sparse_csr_tensor() && y.is_sparse_csr_tensor() && out_grad.is_sparse_csr_tensor()) {
    ...
    auto* kernel_out_0 = SetSparseKernelOutput(&std::get<0>(api_output), TensorType::SPARSE_CSR);
    auto* kernel_out_1 = SetSparseKernelOutput(&std::get<1>(api_output), TensorType::SPARSE_CSR);
    auto* kernel_out_2 = SetSparseKernelOutput(&std::get<2>(api_output), TensorType::SPARSE_CSR);
  ...
  if (input.is_sparse_coo_tensor() && x.is_sparse_coo_tensor() && y.is_sparse_coo_tensor() && out_grad.is_sparse_coo_tensor()) {
    ...
    auto* kernel_out_0 = SetSparseKernelOutput(&std::get<0>(api_output), TensorType::SPARSE_COO);
    auto* kernel_out_1 = SetSparseKernelOutput(&std::get<1>(api_output), TensorType::SPARSE_COO);
    auto* kernel_out_2 = SetSparseKernelOutput(&std::get<2>(api_output), TensorType::SPARSE_COO);
...
PADDLE_API Tensor sum_grad(const Tensor& x, const Tensor& out_grad, const IntArray& axis, bool keepdim) {
