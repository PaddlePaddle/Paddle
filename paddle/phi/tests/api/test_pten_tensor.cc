...
Tensor InitCPUTensorForTest() {
  ...
  auto t1 = Tensor(paddle::PlaceType::kCPU, tensor_shape);
...
void TestAPIPlace() {
  std::vector<int64_t> tensor_shape = {5, 5};
#if defined(PADDLE_WITH_CUDA) || defined(PADDLE_WITH_HIP)
  auto t1 = Tensor(paddle::PlaceType::kGPU, tensor_shape);
  t1.mutable_data<float>(paddle::PlaceType::kGPU);
  CHECK((paddle::PlaceType::kGPU == t1.place()));
#endif
  auto t2 = Tensor(paddle::PlaceType::kCPU, tensor_shape);
...
void TestAPISlice() {
  ...
  auto t4 = Tensor(paddle::PlaceType::kCPU, tensor_shape_origin2);
...
#if defined(PADDLE_WITH_CUDA) || defined(PADDLE_WITH_HIP)
  auto tg1 = Tensor(paddle::PlaceType::kGPU);
...
void TestInitilized() {
  Tensor test_tensor(paddle::PlaceType::kCPU, {1, 1});
