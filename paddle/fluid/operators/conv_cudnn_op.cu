...
class CUDNNConvOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    // ------------------- cudnn descriptors ---------------------
    bool need_set_args = (args_ == nullptr);
    VLOG(10) << "need_set_args = " << need_set_args;
    if (need_set_args) {
      args_ = std::make_unique<ConvArgs>(
          &transformed_input, &transformed_filter_channel, &transformed_output,
...
// VLOG(4) << "Conv: use_addto = " << ctx.Attr<bool>("use_addto");
 ...
 private:
  mutable std::unique_ptr<ConvArgs> args_;
};
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto compute_format = DataLayout::kNCHW;
      ...
      const bool compute_in_nhwc =
          dtype == CUDNN_DATA_HALF && IsVoltaOrLater(dev_ctx);
      auto compute_format = compute_in_nhwc && channel_last ? DataLayout::kNHWC
                                                            : DataLayout::kNCHW;
#endif
    ...
    bool need_set_args1 = args1_ == nullptr;
    bool need_set_args2 = args2_ == nullptr;
    if (!args1_) {
      args1_ = std::make_unique<ConvArgs>(
          &transformed_input_grad, &transformed_filter_channel,
          &transformed_output_grad_channel, strides, padding_common, dilations,
          dtype);
    }
    auto& args1 = *args1_;

    if (!args2_) {
      args2_ = std::make_unique<ConvArgs>(
          &transformed_input, &transformed_filter_grad_channel,
          &transformed_output_grad_channel, strides, padding_common, dilations,
          dtype);
    }
    auto& args2 = *args2_;
    ...
    if (filter_grad) {
      ...
      filter_algo = search2::Find<T>(args2, exhaustive_search, deterministic,
        ...
        workspace_size = std::max(
            workspace_size, search2::GetWorkspaceSize(args2, filter_algo));
#endif
    }
...
// Because beta is zero, it is unnecessary to reset filter_grad.
 ...
 private:
  mutable std::unique_ptr<ConvArgs> args1_, args2_;
};
  ...
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (ddO) {
      ...
      if (ddW) {
        ...
        fwd_algo2 = search2::Find<T>(args2, exhaustive_search, false,
          ...
          workspace_size = std::max(
              workspace_size, search2::GetWorkspaceSize(args2, fwd_algo2));
#endif
      }
    }
    ...
    if (dW && ddX) {
      ...
      filter_algo = search3::Find<T>(args3, exhaustive_search, deterministic,
        ...
        workspace_size = std::max(
            workspace_size, search3::GetWorkspaceSize(args3, filter_algo));
#endif
    }
    ...
    if (ddW && dX) {
      ...
      data_algo = search4::Find<T>(args4, exhaustive_search, deterministic,
        ...
        workspace_size = std::max(workspace_size,
                                  search4::GetWorkspaceSize(args4, data_algo));
#endif
    }
