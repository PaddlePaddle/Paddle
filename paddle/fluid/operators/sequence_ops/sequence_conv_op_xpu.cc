 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    int r = xpu::sequence_context_projection<T, int>(
        xpu_context, in->data<T>(), col.data<T>(), nullptr, lodx,
        sequence_width, context_start, context_length, context_stride, {2, 2});
    PADDLE_ENFORCE_EQ(r, xpu::Error_t::SUCCESS,
                      platform::errors::External(
                          "The sequence_context_projection XPU OP return wrong "
                          "value[%d %s]",
                          r, XPUAPIErrorMsg[r]));
    ...
    int n = filter.dims()[1];
    PADDLE_ENFORCE_EQ(k, k1, platform::errors::InvalidArgument(
                                 "Shape mistake in seq_conv_op"));
    int lda = (!trans_a) ? k : m;
    int ldb = (!trans_b) ? n : k;
    int ldc = n;
    T alpha = static_cast<T>(1.0);
    T beta = static_cast<T>(0.0);
    const T* data_a = col.data<T>();
...
class SequenceConvGradXPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (in_g || filter_g) {
      int r = xpu::constant<T>(xpu_context, col.data<T>(), col.numel(), T(0));
      ...
      PADDLE_ENFORCE_EQ(k, k1,
                        platform::errors::InvalidArgument("Shape mistake in "));
      int lda = (!trans_a) ? k : m;
      int ldb = (!trans_b) ? n : k;
      int ldc = n;
      T alpha = static_cast<T>(1.0);
      ...
      PADDLE_ENFORCE_EQ(r, xpu::Error_t::SUCCESS,
                        platform::errors::External(
                            "XPU fully connect return wrong value[%d %s]", r,
                            XPUAPIErrorMsg[r]));
    }
    ...
    if (in_g) {
      ...
      int r = xpu::sequence_context_projection_grad<T, int>(
          xpu_context, in_g->data<T>(), col.data<T>(), nullptr, lodx,
          sequence_width, context_start, context_length, context_stride,
          {2, 2});

      PADDLE_ENFORCE_EQ(r, xpu::Error_t::SUCCESS,
                        platform::errors::External(
                            "XPU sequence context projecton grad return wrong "
                            "value[%d %s]",
                            r, XPUAPIErrorMsg[r]));
    }
