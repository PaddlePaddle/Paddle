  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    if (ctx->IsRuntime()) {
      framework::Variable* ids_var =
          BOOST_GET_CONST(framework::Variable*, ctx->GetInputVarPtrs("Ids")[0]);
      framework::Variable* updates_var = BOOST_GET_CONST(
          framework::Variable*, ctx->GetInputVarPtrs("Updates")[0]);
