  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    PADDLE_ENFORCE_GE(x_dims.size(), 2,
                      platform::errors::InvalidArgument(
                          ...
                          x_dims.size()));
    auto time_step_dims = pten::slice_ddim(x_dims, 1, x_dims.size());
    auto pad_value_dims = ctx->GetInputDim("PadValue");
    PADDLE_ENFORCE_EQ(
        pad_value_dims == pten::make_ddim({1}) ||
    ...
    auto time_step_dims_vec = pten::vectorize<int>(time_step_dims);
    ...
    ctx->SetOutputDim("Length", pten::make_ddim(len_dims_vec));
  }
