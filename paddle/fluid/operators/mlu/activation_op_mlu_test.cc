...
void Compare(fw::Scope* scope, const plat::DeviceContext& ctx,
  ...
  // init
  auto x = scope->Var("X");
  auto tensor_x = x->GetMutable<fw::Tensor>();
  ...
  auto tensor_out = out->GetMutable<fw::Tensor>();
...
void CompareGrad(fw::Scope* scope, const plat::DeviceContext& ctx,
  ...
  auto tensor_dout = dout->GetMutable<fw::Tensor>();
  auto out = scope->Var("Out");
  auto tensor_out = out->GetMutable<fw::Tensor>();
  ...
  auto tensor_dx = dx->GetMutable<fw::Tensor>();
