...
template <typename T>
class MaskedSelectGradXPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    auto& dev_ctx =
        context.template device_context<paddle::platform::XPUDeviceContext>();
    auto x_shape = framework::vectorize<int>(out->dims());
    auto mask_shape = framework::vectorize<int>(mask->dims());
    int ret = xpu::masked_select_grad(dev_ctx.x_context(), input_data,
                                      mask_data, out_data, x_shape, mask_shape);
    PADDLE_ENFORCE_EQ(
        ret, XPU_SUCCESS,
        platform::errors::External(
            "XPU masked_select_grad kernel return wrong value[%d %s]", ret,
            XPUAPIErrorMsg[ret]));
  }
};
...
REGISTER_OP_XPU_KERNEL(masked_select_grad,
                       ops::MaskedSelectGradXPUKernel<float>,
                       ops::MaskedSelectGradXPUKernel<int>,
                       ops::MaskedSelectGradXPUKernel<int64_t>);
#endif
