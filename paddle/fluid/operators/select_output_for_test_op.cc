...
// SelectOutputForTestOp has one input, one integer mask and multiple outputs.
// It
// selects one output specified by the mask and copy the input to it.
class SelectOutputForTestOp : public framework::OperatorBase {
 public:
  SelectOutputForTestOp(const std::string &type,
...
class SelectOutputForTestOpProtoMaker
...
class SelectOutputForTestInferShape : public framework::InferShapeBase {
 public:
  void operator()(framework::InferShapeContext *context) const override {
    PADDLE_ENFORCE_EQ(context->HasInput("X"), true,
                      "SelectOutputForTestOp must have input X.");
    PADDLE_ENFORCE_EQ(context->HasInput("Mask"), true,
                      "SelectOutputForTestOp must have input Mask.");
    PADDLE_ENFORCE_EQ(context->HasOutputs("Out"), true,
                      "SelectOutputForTestOp must have output Out.");
  }
};

template <typename T>
class SelectOutputForTestGradMaker : public framework::SingleGradOpMaker<T> {
 public:
...
REGISTER_OPERATOR(
    select_output_for_test, ops::SelectOutputForTestOp,
    ops::SelectOutputForTestOpProtoMaker, ops::SelectOutputForTestInferShape,
    ops::SelectOutputForTestGradMaker<paddle::framework::OpDesc>,
    ops::SelectOutputForTestGradMaker<paddle::imperative::OpBase>);
