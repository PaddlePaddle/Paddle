...
class MatMulV2Op : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    std::vector<int64_t> dims_x = paddle::framework::vectorize(
        paddle::operators::GetDimForInput(*ctx, 'X'));
    std::vector<int64_t> dims_y = paddle::framework::vectorize(
        paddle::operators::GetDimForInput(*ctx, 'Y'));
