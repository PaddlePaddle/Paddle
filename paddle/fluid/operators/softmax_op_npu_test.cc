...
void Compare(f::Scope* scope, const p::DeviceContext& ctx) {
  ...
  std::string df = "AnyLayout";
  std::string mdt = "float32";

  // run
  int axis = 1;
  f::AttributeMap attrs = {{"axis", axis}, {"use_mkldnn", false}, {"data_format", df},
                           {"mkldnn_data_type", mdt}, {"is_test", false}};
...
void CompareGrad(f::Scope* scope, const p::DeviceContext& ctx) {
  ...
  for (int i = 0; i < 6; ++i) {
    dout_init.push_back(static_cast<T>(1.2));
  }
