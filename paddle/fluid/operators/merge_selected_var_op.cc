...
class MergeSelectedVarOp : public framework::OperatorBase {
 public:
  MergeSelectedVarOp(const std::string &type,
 ...
 private:
  void RunImpl(const framework::Scope &scope,
    ...
    if (selected_x->IsType<framework::LoDTensor>()) {
      ...
      PADDLE_THROW("Type %s of %s is not supported in MergeSelectedVarOp",
  ...
  int GetOutputBranch(const framework::LoDTensor &mask,
    ...
    PADDLE_ENFORCE_EQ(mask.numel(), 1,
                      "Mask in MergeSelectedVarOp must have numel 1.");
    ...
    PADDLE_THROW(
        ...
        "Mask in MergeSelectedVarOp. Please compile WITH_GPU option");
#endif
    return cpu_mask->data<int>()[0];
  }
};

class MergeSelectedVarOpProtoMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("X",
             "The input LoDTensors or LoDTensorArray or SelectedRows. All "
             "input must have same variable type")
        .AsDuplicable();
    AddInput("Mask",
...
template <typename T>
class MergeSelectedVarGradMaker : public framework::SingleGradOpMaker<T> {
 public:
...
REGISTER_OPERATOR(merge_selected_tensor, ops::MergeSelectedVarOp,
                  ...
                  ops::MergeSelectedVarGradMaker<paddle::imperative::OpBase>);
