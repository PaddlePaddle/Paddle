...
bool SortTopk(const platform::CUDADeviceContext& ctx,
  ...
  if (k < num_cols) {
    ...
    EigenSlice<typename std::remove_reference<decltype(dev)>::type, int64_t,
               2>::Eval(dev, e_indices, e_tmp_indices, slice_indices,
                        slice_sizes);
    EigenSlice<typename std::remove_reference<decltype(dev)>::type, T, 2>::Eval(
