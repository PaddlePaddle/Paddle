...
template <typename T>
  void MatMulGradMKLDNNKernel<T>::Compute(const ExecutionContext& ctx) const override {
    if (ctx.HasAttr("head_number")) {
      PADDLE_ENFORCE_EQ(
          ctx.Attr<int>("head_number"), 1,
          platform::errors::Unimplemented(
...
template <typename T>
  void MatMulGradMKLDNNKernel<T>::ExecuteMatMulGrad(const ExecutionContext& ctx,
...
template class paddle::operators::MatMulGradMKLDNNKernel<float>;
template class paddle::operators::MatMulGradMKLDNNKernel<paddle::platform::bfloat16>;
