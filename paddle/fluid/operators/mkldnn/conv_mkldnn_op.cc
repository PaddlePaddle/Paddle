 ...
 public:
  ConvMKLDNNHandlerT(const framework::ExecutionContext& ctx,
      ...
      : platform::MKLDNNHandlerT<T, dnnl::convolution_forward,
            ...
            platform::CreateKey(dev_ctx, pten::vectorize(input->dims()),
                                unique_name)) {
    if (unlikely(!this->isCached())) {
      ...
      if (bias) {
        auto bias_tz = pten::vectorize(bias->dims());
  ...
  ConvMKLDNNHandlerT(const framework::ExecutionContext& ctx,
      ...
      : platform::MKLDNNHandlerT<T, dnnl::convolution_forward,
            ...
            platform::CreateKey(dev_ctx, pten::vectorize(in->dims()),
                                unique_name)) {
    if (unlikely(!this->isBwdCached())) {
      ...
      auto data_dims = pten::slice_ddim(input_dims, 2, input_dims.size());
      auto filter_dims = filter->dims();
      auto filter_data_dims =
          pten::slice_ddim(filter_dims, 2, filter_dims.size());
      auto ksize = pten::vectorize(filter_data_dims);
      ...
      auto src_tz = pten::vectorize(in->dims());
      auto weights_tz = pten::vectorize(filter->dims());

      int groups = ctx.Attr<int>("groups");
      int g = std::max(groups, 1);
      platform::GetGroupConvWeightsTz(weights_tz, g);
      auto dst_tz = pten::vectorize(out_grad->dims());
  ...
  std::shared_ptr<std::tuple<float, std::vector<float>>> get_int8_bias_scales(
    ...
    const auto& weights_tz = pten::vectorize(filter->dims());
  ...
  std::shared_ptr<dnnl::memory> AcquireMemoryWithReorderPrimitive(
    ...
    if (!user_mem_p) {
      auto user_mem_md = platform::MKLDNNMemDesc(
          pten::vectorize(in_mem->dims()), platform::MKLDNNGetDataType<T>(),
          in_mem->format());
      return this->AcquireMemoryWithReorder(
  ...
  std::shared_ptr<dnnl::memory> AcquireResidualMemory(
    ...
    if (residual_mem_p) {
      ...
      auto user_residual_md = platform::MKLDNNMemDesc(
          pten::vectorize(residual_param->dims()),
