...
void eltwise_forward(const framework::ExecutionContext &ctx,
  ...
  if (algorithm == mkldnn::algorithm::eltwise_tanh) {
    platform::DumpComposit::execute("elemwise_fwd{tanh}", ctx, *y);
  } else if (algorithm == mkldnn::algorithm::eltwise_relu) {
    platform::DumpComposit::execute("elemwise_fwd{relu}", ctx, *y);
  } else {
    platform::DumpComposit::execute("elemwise_fwd", ctx, *y);
  }
}
