...
class LoDResetOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("X",
             "(Tensor, Tensor) Input variable of LoDResetOp which "
             "could be a Tensor or Tensor, where the data of output "
             "variable inherits from.");
    AddInput("Y",
             "(Tensor, Tensor, optional) If provided and Y is Tensor, "
             "lod of Input(Y) would be considered as the target lod first, "
             "otherwise data of Input(Y) would be considered as the "
             "target lod.")
        .AsDispensable();
    AddOutput("Out",
              "(Tensor) Output variable of LoDResetOp which should be a "
              "Tensor.");
...
provided and `Y` is a Tensor, `Y.lod` would be considered as target LoD
