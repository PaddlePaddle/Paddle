...
#if CUDNN_VERSION >= 90000
    PADDLE_ENFORCE_GPU_SUCCESS(platform::dynload::cudnnCreateRNNDataDescriptor(&x_desc_));
    PADDLE_ENFORCE_GPU_SUCCESS(platform::dynload::cudnnCreateRNNDataDescriptor(&y_desc_));
...
#if CUDNN_VERSION >= 90000
    PADDLE_ENFORCE_GPU_SUCCESS(platform::dynload::cudnnGetRNNTempSpaceSizes(
        handle,
        rnn_desc_,
