...
class MergedMomentumOpXPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto regularization_method = ctx.Attr<std::vector<std::string>>("regularization_method");
    auto regularization_coeff = ctx.Attr<std::vector<float>>("regularization_coeff");
    std::vector<T*> param_list(op_num);
    std::vector<T*> velocity_list(op_num);
    std::vector<T*> grad_list(op_num);
    std::vector<T*> velocity_out_list(op_num);
    std::vector<T*> param_out_list(op_num);
    std::vector<int> sizes(op_num);
    std::vector<float> l2_weight_decay(op_num);
    if(op_num>0){
       for(int j=0;j<op_num;j++){
        param_list[j]=const_cast<T*>(params[j]->data<T>());
        velocity_list[j]=const_cast<T*>(velocity[j]->data<T>());
        grad_list[j]=const_cast<T*>(grad[j]->data<T>());
        param_out_list[j]=params_out[j]->data<T>();
        velocity_out_list[j]=velocity_out[j]->data<T>();
        sizes[j]=static_cast<T>(params[j]->numel());
        if (regularization_method[j] != "l2_decay") {
             l2_weight_decay[j] = 0.0f;
           }else{
             l2_weight_decay[j] =static_cast<float>(regularization_coeff[j]);
           }    
        PADDLE_ENFORCE_EQ(params[j], params_out[j],
    ...
    int r = xpu::merged_momentum(dev_ctx.x_context(),param_list,
                          ...
                          l2_weight_decay,sizes, lr->data<T>(),mu,use_nesterov);
    int ret = xpu_wait();
    PADDLE_ENFORCE_XDNN_SUCCESS(r,"merged_momentum");
    PADDLE_ENFORCE_EQ(
          ret, xpu::Error_t::SUCCESS,
          platform::errors::InvalidArgument(
