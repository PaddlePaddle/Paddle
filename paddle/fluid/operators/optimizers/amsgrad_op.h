...
class AmsgradOp : public framework::OperatorWithKernel {
 public:
...
struct AmsgradFunctor;

template <typename T>
struct AmsgradFunctor<T, GPUAmsgrad> {
  T beta1_;
  T beta2_;
  T epsilon_;
  ...
  AmsgradFunctor(T beta1, T beta2, T epsilon, const T* beta1_pow,
                 const T* beta2_pow, const T* mom1, T* mom1_out, const T* mom2,
                 T* mom2_out, const T* max_mom2, T* max_mom2_out, const T* lr,
      ...
      : beta1_(beta1),
        ...
        max_moment2_(max_mom2),
        max_moment2_out_(max_mom2_out),
        lr_(lr),
        grad_(grad),
        param_(param),
  ...
  inline HOSTDEVICE void operator()(size_t i) const {
    ...
    max_mom2 = max_mom2 > mom2 ? max_mom2 : mom2;
    p -= lr * (mom1 / (sqrt(max_mom2) + epsilon_));

    // Write back to global memory
    moment1_out_[i] = mom1;
    moment2_out_[i] = mom2;
    max_moment2_out_[i] = max_mom2;
    param_out_[i] = p;
  }
};

template <typename T>
struct AmsgradFunctor<T, CPUAmsgrad> {
  T beta1_;
  T beta2_;
  T epsilon_;
  ...
  void operator()(size_t numel) const {
    ...
    Eigen::Map<const Eigen::Array<T, 1, Eigen::Dynamic>> max_mom2{
        max_moment2_, static_cast<Eigen::Index>(numel)};
    ...
    Eigen::Map<Eigen::Array<T, 1, Eigen::Dynamic>> max_moment2_out{
        max_moment2_out_, static_cast<Eigen::Index>(numel)};
    ...
    max_moment2_out = max_mom2.cwiseMax(moment2_out);
    param_out =
        param - lr * (moment1_out / (max_moment2_out.sqrt() + epsilon_));
  }
};

template <typename T, typename Flavour>
struct SparseAmsgradFunctor;

template <typename T>
struct SparseAmsgradFunctor<T, GPUAmsgrad> {
  T beta1_;
  T beta2_;
  T epsilon_;
  ...
  SparseAmsgradFunctor(T beta1, T beta2, T epsilon, const T* beta1_pow,
                       const T* beta2_pow, const T* mom1, T* mom1_out,
                       const T* mom2, T* mom2_out, const T* max_mom2,
                       T* max_mom2_out, const T* lr, const T* grad,
  ...
  inline HOSTDEVICE void amsgrad_update(size_t i, T g) const {
...
template <typename T>
struct SparseAmsgradFunctor<T, CPUAmsgrad> {
  T beta1_;
  T beta2_;
  T epsilon_;
    ...
    for (int64_t i = 0, j = 0; i != row_count; ++i) {
      if (i == *(rows_ + j)) {
        for (int64_t k = 0; k != row_numel_; ++k) {
          ...
          amsgrad_update(i * row_numel_ + k, g);
        }
        ++j;
      } else {
        for (int64_t k = 0; k != row_numel_; ++k) {
          ...
          T max_mom2 = max_moment2_[i * row_numel_ + k];
          ...
          max_moment2_out_[i * row_numel_ + k] = max_mom2;
...
class AmsgradOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto& max_mom2 =
        Ref(ctx.Input<LoDTensor>("MaxMoment2"), "Must set MaxMoment2");
    ...
    auto& max_mom2_out =
        Ref(ctx.Output<LoDTensor>("MaxMoment2Out"), "Must set MaxMoment2Out");

    if (grad_var->IsType<framework::LoDTensor>()) {
      auto& grad = Ref(ctx.Input<LoDTensor>("Grad"), "Must set Grad");

      if (platform::is_cpu_place(ctx.GetPlace())) {
        AmsgradFunctor<T, CPUAmsgrad> functor(
            beta1, beta2, epsilon, beta1_pow.template data<T>(),
            beta2_pow.template data<T>(), mom1.template data<T>(),
            mom1_out.template mutable_data<T>(ctx.GetPlace()),
            mom2.template data<T>(),
            mom2_out.template mutable_data<T>(ctx.GetPlace()),
            max_mom2.template data<T>(),
            max_mom2_out.template mutable_data<T>(ctx.GetPlace()),
            lr.template data<T>(), grad.template data<T>(),
            param.template data<T>(),
            param_out.template mutable_data<T>(ctx.GetPlace()));
        functor(param.numel());
      } else if (platform::is_gpu_place(ctx.GetPlace())) {
        AmsgradFunctor<T, GPUAmsgrad> functor(
            beta1, beta2, epsilon, beta1_pow.template data<T>(),
            beta2_pow.template data<T>(), mom1.template data<T>(),
            mom1_out.template mutable_data<T>(ctx.GetPlace()),
            mom2.template data<T>(),
      ...
      if (platform::is_cpu_place(ctx.GetPlace())) {
        SparseAmsgradFunctor<T, CPUAmsgrad> functor(
            beta1, beta2, epsilon, beta1_pow.template data<T>(),
            beta2_pow.template data<T>(), mom1.template data<T>(),
            mom1_out.template mutable_data<T>(ctx.GetPlace()),
            mom2.template data<T>(),
            ...
            fs.push_back(framework::Async([&functor, &row_id_to_grad_row_offset,
              ...
              for (int64_t row_id = start; row_id < end; ++row_id) {
                ...
                if (iter != row_id_to_grad_row_offset.end()) {
                  ...
                  for (size_t row_offset = 0U; row_offset < row_numel;
                       ++row_offset) {
                    functor.amsgrad_update(row_id * row_numel + row_offset, 0);
                  }
                }
              }
            }));
        ...
        SparseAmsgradFunctor<T, GPUAmsgrad> functor(
            beta1, beta2, epsilon, beta1_pow.template data<T>(),
            beta2_pow.template data<T>(), mom1.template data<T>(),
            mom1_out.template mutable_data<T>(ctx.GetPlace()),
            mom2.template data<T>(),
      ...
      PADDLE_THROW("Variable type not supported by amsgrad_op");
    }
  }
};
