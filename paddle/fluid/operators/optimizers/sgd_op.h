...
template <>
class SGDOpKernel<platform::CPUDeviceContext, platform::bfloat16>
    : public framework::OpKernel<platform::bfloat16> {
  using T = platform::bfloat16;
  ...
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    if (param_var->IsType<framework::LoDTensor>()) {
      ...
      if (grad_var->IsType<framework::LoDTensor>()) {
        ...
        // In Sparse SGD operator, in-place update is enforced.
        // This manual optimization brings difficulty to track data dependency.
        PADDLE_ENFORCE_EQ(param, param_out,
                          platform::errors::InvalidArgument(
                              "The input tensor Param of SgdOp "
        ...
        int64_t param_height = out_dims[0];
        int64_t param_width = param_out->numel() / param_height;
        // note: it is not grad->height()
        int64_t grad_val_height = static_cast<int64_t>(grad_rows.size());
        int64_t grad_width = grad_value.numel() / grad_val_height;
        PADDLE_ENFORCE_EQ(
      ...
      PADDLE_ENFORCE_EQ(
          param_row_width, grad_row_width,
          platform::errors::InvalidArgument(
              "The grad value's row width should be equal with param's row "
              "width. But received grad value's width [%s] and param's "
              "row width [%s]",
              grad_row_width, param_row_width));
