...
template <typename T>
static T *TensorFillConstant(const phi::XPUContext &dev_ctx,
...
static phi::DenseTensor CastDataForInitedTensor(const phi::XPUContext &dev_ctx,
...
template <typename T>
class DistributedFusedLambInitOpKernel<phi::XPUContext, T>
    : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    printf("====> starts to run DistributedFusedLambInitOpKernel\n");
    ...
    // Todo:
    (void)fused_fp32_param;
    (void)fused_fp32_grad;
    (void)fused_fp16_param;
    (void)fused_fp16_grad;
    ...
    for (const auto &info : fp32_infos) {
      auto sliced_tensor = CopyAndShareBufferForInitedTensor(
          info.param_t, fp32_p_t, info.numel_offset);
      ...
      if (info.grad_t->IsInitialized()) {
        CopyAndShareBufferForInitedTensor(
            info.grad_t, fp32_g_t, info.numel_offset);
      } else {
    ...
    for (const auto &info : fp16_infos) {
      ...
      CopyAndShareBufferForInitedTensor(
          info.param_t, fp16_p_t, info.numel_offset);
      PADDLE_ENFORCE_EQ(master_params[info.idx]->mutable_data<float>(place),
                        sliced_tensor.data<float>(),
                        platform::errors::InvalidArgument(
                            "Invalid master weight tensor pointer."));
      ...
      if (info.grad_t->IsInitialized()) {
        CopyAndShareBufferForInitedTensor(
            info.grad_t, fp16_g_t, info.numel_offset);
      } else {
        ShareBufferForNonInitedTensor(
    ...
    // Todo:
    // (void)fp32_wd_end_idx;
    // (void)fp16_wd_end_idx;
    // (void)fused_fp32_param;
    // (void)fused_fp32_grad;
    // (void)fused_fp16_param;
    // (void)fused_fp16_grad;
...
//     ops::DistributedFusedLambInitOpKernel<phi::XPUContext, float>);
