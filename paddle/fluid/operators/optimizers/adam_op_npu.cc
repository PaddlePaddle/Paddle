...
class AdamNPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    Tensor beta1_tmp(experimental::DataType::FP32);
    Tensor beta2_tmp(experimental::DataType::FP32);
    Tensor epsilon_tmp(experimental::DataType::FP32);
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (!skip_update && with_decay) {
      ...
      Tensor one(experimental::DataType::FP32);
      Tensor decay(experimental::DataType::FP32);
      Tensor tmp(experimental::DataType::FP32);
