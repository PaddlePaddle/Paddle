...
#include "paddle/fluid/operators/optimizers/adam_scale_op.h"

namespace paddle {
namespace operators {

template <typename T>
__global__ void AdamScaleKernelREG(T beta1, T beta2, T epsilon, T beta1_pow_,
                              ...
                              T new_param_scale, int ndim) {
  T lr = *lr_;
  T beta1_pow = beta1_pow_;
  T beta2_pow = beta2_pow_;

  T adam_ratio = 1.0 / (1.0 - sqrt(beta1_pow + beta2_pow));
  adam_ratio = (adam_ratio < new_param_scale) ? adam_ratio : new_param_scale;
...
template <typename T>
__global__ void AdamScaleKernelMEM(T beta1, T beta2, T epsilon,
                              const T* beta1_pow_, const T* beta2_pow_,
                              const T* moment1, T* moment1_out,
                              const T* moment2, T* moment2_out,
                              const T* lr_, const T* grad, const T* param,
                              T* param_out, T new_param_scale, int ndim) {
  T lr = *lr_;
  T beta1_pow = *beta1_pow_;
  T beta2_pow = *beta2_pow_;
...
template <typename T>
__global__ void SparseAdamScaleCUDAKernelREG(
    ...
    const T* grad_, const T* param_, T* param_out_, T new_param_scale,
    const int64_t* rows_, int64_t row_numel, int64_t row_count,
  ...
  lr *= adam_ratio * sqrt(1 - beta2_pow) / (1 - beta1_pow);
...
template <typename T>
class AdamScaleOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    T new_param_scale = static_cast<T>(ctx.Attr<float>("new_param_scale"));
    if (ctx.HasInput("NewParamScaleTensor")) {
      auto* new_param_scale_tensor =
          ctx.Input<framework::Tensor>("NewParamScaleTensor");
      PADDLE_ENFORCE_EQ(new_param_scale_tensor->numel(), 1,
                        platform::errors::InvalidArgument(
                            "Input(NewParamScaleTensor) size must be 1,"
                            " but get %d",
                            new_param_scale_tensor->numel()));
      new_param_scale =
          static_cast<T>(GetAttrFromTensor(new_param_scale_tensor));
    }
    ...
    if (grad_var->IsType<framework::LoDTensor>()) {
      ...
      if (beta1_pow->place() == platform::CPUPlace() &&
        ...
        AdamScaleKernelREG<T><<<blocks, threads, 0, dev_ctx.stream()>>>(
            ...
            param_out->mutable_data<T>(
                ctx.GetPlace()), new_param_scale, param->numel());
        // Cpu update
        ...
        AdamScaleKernelMEM<T><<<blocks, threads, 0, dev_ctx.stream()>>>(
            beta1, beta2, epsilon, beta1_pow->data<T>(), beta2_pow->data<T>(),
            mom1->data<T>(), mom1_out->mutable_data<T>(ctx.GetPlace()),
            mom2->data<T>(), mom2_out->mutable_data<T>(ctx.GetPlace()),
            lr->data<T>(), grad->data<T>(), param->data<T>(),
      ...
      if (beta1_pow->place() == platform::CPUPlace() &&
        ...
        SparseAdamScaleCUDAKernelREG<T><<<blocks,
                                              threads, 0, dev_ctx.stream()>>>(
            beta1, beta2, epsilon, *beta1_pow->data<T>(), *beta2_pow->data<T>(),
            mom1->data<T>(), mom1_out->mutable_data<T>(ctx.GetPlace()),
            mom2->data<T>(), mom2_out->mutable_data<T>(ctx.GetPlace()),
            lr->data<T>(), grad_data, param->data<T>(),
            param_out->mutable_data<T>(ctx.GetPlace()), new_param_scale,
            rows, row_numel, grad_merge.rows().size(), lazy_mode, ndim);
        // Update with cpu
        beta1_pow_out->mutable_data<T>(platform::CPUPlace())[0] =
            beta1 * beta1_pow->data<T>()[0];
        beta2_pow_out->mutable_data<T>(platform::CPUPlace())[0] =
        ...
        SparseAdamScaleFunctor<T, GPUAdamScale> functor(
            beta1, beta2, epsilon, beta1_pow->data<T>(), beta2_pow->data<T>(),
            mom1->data<T>(), mom1_out->mutable_data<T>(ctx.GetPlace()),
            mom2->data<T>(), mom2_out->mutable_data<T>(ctx.GetPlace()),
            lr->data<T>(), grad_data, param->data<T>(),
...
REGISTER_OP_CUDA_KERNEL(adam_scale, ops::AdamScaleOpCUDAKernel<float>,
                        ops::AdamScaleOpCUDAKernel<double>);
