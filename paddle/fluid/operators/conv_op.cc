...
void ConvOp::InferShape(framework::InferShapeContext* ctx) const {
  ...
  PADDLE_ENFORCE_EQ(
      ...
      platform::errors::InvalidArgument(
          ...
          "channels * groups for Op(conv). But received: the input's channels "
          "is "
          "[%d], the shape "
          "of input is [%s], the filter's channel is [%d], the shape of filter "
          "is "
  ...
  PADDLE_ENFORCE_EQ(filter_dims[0] % groups, 0,
                    platform::errors::InvalidArgument(
                        "ShapeError: The number of output channels of Op(conv) "
                        "should be divided "
                        "by groups. "
                        "But received: the output channels is [%d], the shape "
                        "of filter is [%s] "
                        "(the first dimension of filter is output channel), "
                        "the groups is [%d].",
