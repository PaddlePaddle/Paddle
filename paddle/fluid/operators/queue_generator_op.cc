...
class QueueGeneratorOp : public framework::OperatorBase {
 public:
  ...
  void RunImpl(const framework::Scope& scope,
    ...
    PADDLE_ENFORCE_GT(names.size(), 0,
                      "The attribute 'names' for QueueGeneratorOP must"
                      " be set.");
    int capacity = Attr<int>("capacity");
    PADDLE_ENFORCE_GT(capacity, 0,
                      "The attribute 'capacity' for QueueGeneratorOP must"
                      " be set a positive value, but the one you give is: "
                      "%d.",
                      capacity);
 ...
 private:
  void GenerateQueue(const framework::Scope* scope, const std::string& name,
    ...
    PADDLE_ENFORCE_NOT_NULL(var, "Can't find var named: %s", name);
    auto ptr = var->GetMutable<reader::LoDTensorBlockingQueueHolder>();
    ptr->InitOnce(capacity);

    VLOG(3) << "generated a queue var named: " << name;
  }
};
...
REGISTER_OPERATOR(queue_generator, ops::QueueGeneratorOp,
