...
__global__ void KernelBincount(const InputT* input, const int total_elements,
  ...
  if (!has_weights) {
    ...
    for (int i = threadIdx.x; i < total_elements; i += blockDim.x) {
      paddle::platform::CudaAtomicAdd(&output[input[i]], static_cast<OutT>(weights[i]));
    }
  }
}
...
void BincountCUDAInner(const framework::ExecutionContext& context) {
  ...
  int64_t output_size = static_cast<int64_t>(input_max_cpu.data<InputT>()[0]) + 1L;
  ...
  if (!has_weights) {
    ...
    KernelBincount<
        T, InputT, int64_t><<<GET_BLOCKS(input_numel), PADDLE_CUDA_NUM_THREADS,
                        0, stream>>>(
    ...
    if (weights_type == framework::proto::VarType::FP32) {
      ...
      KernelBincount<
          T, InputT, float><<<GET_BLOCKS(input_numel), PADDLE_CUDA_NUM_THREADS,
                          0, stream>>>(
          input_data, input_numel, has_weights, weights_data, output_data);
    } else {
      ...
      KernelBincount<
          T, InputT, double><<<GET_BLOCKS(input_numel), PADDLE_CUDA_NUM_THREADS,
                          0, stream>>>(
          input_data, input_numel, has_weights, weights_data, output_data);
    }
...
REGISTER_OP_CUDA_KERNEL(
    bincount,
    ops::BincountCUDAKernel<paddle::platform::CUDADeviceContext, int>,
