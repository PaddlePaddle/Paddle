...
static inline bool is_vector_rhs(const Tensor& input, const Tensor& other) {
  ...
  std::vector<int64_t> x_dims_vec = pten::vectorize(x_dim);
  std::vector<int64_t> y_dims_vec = pten::vectorize(y_dim);
...
get_broadcast_dims(const Tensor& x, const Tensor& y) {
  std::vector<int64_t> x_dims_vec = pten::vectorize(x.dims());
  std::vector<int64_t> y_dims_vec = pten::vectorize(y.dims());
...
void expand_impl(const DeviceContext& context, const Tensor& in, Tensor* out,
  ...
  auto vec_in_dims = pten::vectorize<int>(in.dims());
