...
template <typename T>
struct HuberLossV2Forward {
  HOSTDEVICE HuberLossV2Forward(const T& delta) : delta(delta) {}
...
class HuberLossV2Kernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    loss.device(place) = residual.unaryExpr(HuberLossV2Forward<T>(delta));
  }
};

template <typename T>
struct HuberLossV2Backward {
  HOSTDEVICE HuberLossV2Backward(const T& delta, T sign)
...
template <typename DeviceContext, typename T, typename AttrType = T>
class HuberLossV2GradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (out0) {
      ...
      x_grad.device(place) =
          residual.unaryExpr(HuberLossV2Backward<T>(delta, -1.0));
