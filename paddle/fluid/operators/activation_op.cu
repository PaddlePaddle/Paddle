...
template <typename T>
struct CudaRReluFunctor : public BaseActivationFunctor<T> {
  T zero = static_cast<T>(0.0f);
  float alpha;
...
template <typename T>
struct CudaRReluGradFunctor : public BaseActivationFunctor<T> {
  T zero = static_cast<T>(0.0f);
  float alpha;
...
/* ======================== leaky relu register  ============================ */
REGISTER_ACTIVATION_CUDA_KERNEL(rrelu, RRelu, CudaRReluFunctor,
                                CudaRReluGradFunctor);
...
REGISTER_OP_CUDA_KERNEL(
    ...
    ops::ActivationDoubleGradKernel<
        plat::CUDADeviceContext, ops::RReluGradGradFunctor<plat::float16>>);
/* ========================================================================== */
