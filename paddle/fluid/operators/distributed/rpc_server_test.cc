...
framework::BlockDesc* AppendSendAndRecvBlock(framework::ProgramDesc* program) {
  auto root_block = prgram->MutableBlock(0);
  auto* block = program->AppendBlock(*root_block);

  framework::VariableNameMap input({{"X", {"x"}});
    framework::VariableNameMap output({{"Out", {"out"}}});
    auto op = block->AppendOp();
    op->SetType("scale");
    op->SetInput("X", {"x"});
    op->SetOutput("Out", {"out"});
    op->SetAttr("scale", {0.5});
...
void InitTensorsOnClient(framework::Scope* scope, platform::CPUPlace* place,
    ...
    int64_t* ids_ptr = ids_var->mutable_data<int64_t>(
        framework::DDim({rows_numel, 1}), *place);
...
TEST(COMPLETE, CPU) {
    ...
    g_req_handler.reset(new distributed::RequestSendHandler(
