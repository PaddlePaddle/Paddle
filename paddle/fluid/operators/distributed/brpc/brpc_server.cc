...
class BRPCServiceImpl : public SendRecvService {
 public:
  ...
  void HandleRequest(distributed::RequestType req_type,
    ...
    RPCRequest req;
    auto get_var_callback = std::bind(&RequestHandler::GetOrCreateRequestVar,
                                      handler, std::placeholders::_1, &req);

    distributed::BRPCVariableResponse resp(get_var_callback_,
    ...
    if (request->out_var_) {
      ...
      distributed::SerializeToIOBuf(request->out_var_name_, request->out_var_,
                                    *request_get_h_->dev_ctx(), response,
                                    &cntl->response_attachment(),
                                    request->out_var_name_, var_is_stable);
    }
  }
