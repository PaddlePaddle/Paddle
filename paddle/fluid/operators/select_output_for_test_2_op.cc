...
// SelectOutputForTest2Op has one input, one integer mask and multiple outputs.
// It
// selects one output specified by the mask and copy the input to it.
class SelectOutputForTest2Op : public framework::OperatorBase {
 public:
  SelectOutputForTest2Op(const std::string &type,
...
class SelectOutputForTest2OpProtoMaker
...
class SelectOutputForTest2InferShape : public framework::InferShapeBase {
 public:
  void operator()(framework::InferShapeContext *context) const override {
    PADDLE_ENFORCE_EQ(context->HasInput("X"), true,
                      "SelectOutputForTest2Op must have input X.");
    PADDLE_ENFORCE_EQ(context->HasInput("Mask"), true,
                      "SelectOutputForTest2Op must have input Mask.");
    PADDLE_ENFORCE_EQ(context->HasOutputs("Out"), true,
                      "SelectOutputForTest2Op must have output Out.");
  }
};

template <typename T>
class SelectOutputForTest2GradMaker : public framework::SingleGradOpMaker<T> {
 public:
...
REGISTER_OPERATOR(
    select_output_for_test, ops::SelectOutputForTest2Op,
