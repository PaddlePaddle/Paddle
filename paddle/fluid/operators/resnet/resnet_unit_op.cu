...
// #include "paddle/fluid/operators/resnet/cudnn_scale_bias_add_relu.cu.h"
...
class ResNetUnitKernel<platform::CUDADeviceContext, T>
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    //     auto *save_invstd_x = ctx.Output<Tensor>("SavedInvstdX");
...
#define MALLOC_AND_GET_PTR(TR, Dtype, Place) \
  auto TR##_ptr = TR->mutable_data<Dtype>(Place);
    ...
    conv_x_op->Forward(ctx, input_x_ptr, filter_x_ptr, output_ptr, sum_x_ptr,
                       sum_of_squares_x_ptr);
//     // 2. BN
//     CuDNNBNStatsFinalizeOp<T> bn_x_op = new
//     CuDNNBNStatsFinalizeOp<T>(); bn_x_op.Init(ctx, output_shape);
//     bn_x_op.Forward(ctx, sum_x_ptr, sum_of_squares_x_ptr,
//     saved_mean_x_ptr,
//                     saved_invstd_x_ptr, running_mean_x_ptr,
//                     running_var_x_ptr,
//                     equiv_scale_x_ptr, equiv_bias_x_ptr);

//     // 3. scale + bias + add + relu
//     bool has_shortcut = ctx.Attr<bool>("has_shortcut");
//     bool fused_add = ctx.Attr<bool>("fused_add");
//     CuDNNScaleBiasAddReluOp<T> sbar_op = new
//     CuDNNScaleBiasAddReluOp<T>();
//     if (has_shortcut) {
//       // input z
...
//       auto *save_invstd_z = ctx.Output<Tensor>("SavedInvstdZ");
...
//       auto input_z_shape =
//       framework::vectorize<int>(input_z->dims()); auto filter_z_shape
//       = framework::vectorize<int>(filter_z->dims());
//       // 3.1 Conv for second input
//       auto input_z_ptr = input_z->data<T>();
//       auto filter_z_ptr = filter_z->data<T>();
//       CuDNNNormConvolutionOp<T> conv_z_op = new
//       CuDNNNormConvolutionOp<T>();
//       conv_z_op.Init(ctx, input_z_shape, filter_z_shape,
//       output_shape); conv_z_op.Forward(ctx, input_z_ptr,
//       filter_z_ptr, conv_out_z_ptr,
//                         sum_z_ptr, sum_of_squares_z_ptr);
//       // 3.2 BN for second input
//       CuDNNBNStatsFinalizeOp<T> bn_z_op = new
//       CuDNNBNStatsFinalizeOp<T>();
//       bn_z_op.Init(ctx, output_shape);
//       bn_z_op.Forward(ctz, sum_z_ptr, sum_of_squares_z_ptr,
//       saved_mean_z_ptr,
//                       saved_invstd_z_ptr, running_mean_z_ptr,
//                       running_var_z_ptr,
...
//         auto input_z_ptr = input_z->data<T>();
//         auto input_z_shape =
//         framework::vectorize<int>(input_z->dims()); sbar_op.Init(ctx,
//         output_shape, output_shape, bitmask_shape,
//                      input_z_shape);
//         sbar_op.Forward(ctx, conv_out_x_ptr, equiv_scale_x_ptr,
