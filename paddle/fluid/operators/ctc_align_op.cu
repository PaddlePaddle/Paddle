...
template <typename T>
__global__ void PaddingMergeAndDelCudaKernel(const int64_t num_token, const T* tokens,
                                             const int blank, const int merge_repeated, 
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (input->lod().empty()) {
        ...
        T* output_data = output->mutable_data<T>({input_dims[0], input_dims[1]}, ctx.GetPlace());
        PaddingMergeAndDelCudaKernel<T><<<32, (input_dims[0] + 32 - 1) / 32, 0, stream>>>(
            input_dims[1], tokens, blank, merge_repeated, padding_num, input_dims[0], output_data);
    } else {
