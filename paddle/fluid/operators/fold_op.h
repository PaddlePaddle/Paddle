...
class FoldOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    int input_channels = input_dims[1] / (kernel_sizes[0] * kernel_sizes[1]);
    int output_height =
    ...
    framework::DDim output_matrix_shape({input_channels, output_sizes[0], output_sizes[1]});
    framework::DDim input_shape({input_channels, kernel_sizes[0],
        kernel_sizes[1], output_height, output_width});
...
class FoldGradOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    int output_channels = input_dims[1] / (kernel_sizes[0] * kernel_sizes[1]);
    framework::DDim input_shape({output_channels, output_sizes[0], output_sizes[1]});
    ...
    int output_width =
        CalcOutputSize(output_sizes[1], kernel_sizes[1], dilations[1],
                       paddings[1], paddings[3], strides[1]);
    framework::DDim output_matrix_shape({output_height, output_width, output_channels, kernel_sizes[0], kernel_sizes[1]});
    ...
    for (int i = 0; i < batch_size; i++) {
      Tensor out_grad_batch = output_grad->Slice(i, i + 1).Resize(input_shape);
      Tensor in_grad_batch = input_grad->Slice(i, i + 1).Resize(output_matrix_shape);
      im2col(dev_ctx, out_grad_batch, dilations, strides, paddings, &in_grad_batch);
    }
  }
};
}  // namespace operators
