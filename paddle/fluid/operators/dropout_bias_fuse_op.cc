...
#include "paddle/fluid/operators/dropout_bias_fuse_op.h"
#include <memory>
#include <string>
...
class DropoutBiasFuseOp : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    // only support (1, h) or (h) shape of bias for the broadcasting
 ...
 public:
  void Make() override {
    ...
    AddAttr<std::string>(
        ...
        "1. downgrade_in_infer(default)"
        "   train: out = (input + bias) * mask"
        "   inference: out = (input + bias) * (1.0 - dropout_prob)"
        "2. upscale_in_train"
        "   train: out = (input + bias) * mask / ( 1.0 - dropout_prob )"
        "   inference: out = (input + bias)")
...
class DropoutBiasFuseGradOpMaker : public framework::SingleGradOpMaker<T> {
 ...
 protected:
  void Apply(GradOpPtr<T> op) const override {
    op->SetType("dropout_bias_fuse_grad");
...
REGISTER_OPERATOR(dropout_bias_fuse, ops::DropoutBiasFuseOp,
...
REGISTER_OPERATOR(dropout_bias_fuse_grad, ops::DropoutBiasFuseOpGrad);
