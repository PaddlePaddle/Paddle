...
class AffineGridOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto h_idx_t = EigenTensor<T, 1>::From(h_idx).reshape(Array2(1, h)).broadcast(Array2(w, 1)).shuffle(Array2(1,0)).reshape(Array3(h, w, 1));
    // Get indexes of width with shape [height, width, 1]
    auto w_idx = linspace((T)-1, (T)1, w, ctx);
    auto w_idx_t = EigenTensor<T, 1>::From(w_idx).reshape(Array2(1, w)).broadcast(Array2(h, 1)).reshape(Array3(h, w, 1));
    ...
    // Get grid tensor with shape [n, h, w, 3] by concatenating h_idx, w_idx and ones
    Tensor grid;
    grid.mutable_data<T>({n, h, w, 3}, ctx.GetPlace());
    auto grid_t = EigenTensor<T, 4>::From(grid);
    grid_t = w_idx_t.concatenate(h_idx_t, 2).eval().concatenate(ones_t, 2).reshape(Array4(1, h, w, 3)).broadcast(Array4(n,1,1,1));
    ...
    for (int i=0; i<n; ++i) {
        ...
        blas.MatMul(sliced_grid, false, sliced_theta, true, T(1), &sliced_out, T(0));
    }
  }
};
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    for (int i=0; i<n; ++i) {
        ...
        blas.MatMul(sliced_out_grad, true, sliced_grid, false, T(1), &sliced_theta_grad, T(0));
    }
  }
};
