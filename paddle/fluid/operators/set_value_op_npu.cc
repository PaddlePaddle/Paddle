...
class SetValueNPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const {
    ...
    int64_t stride_step = pten::product(in_dims);
    ...
    PADDLE_ENFORCE_EQ(
        ...
        pten::product(slice_dims_for_assign),
    ...
    if (slice_dims_for_assign == value_t.dims()) {
      ...
      runner_brd.SetType("BroadcastTo")
          .AddInput(value_t)
          .AddInput(pten::vectorize(slice_dims_for_assign))
          .AddOutput(value_temp)
          .Run(stream);
    }

    int64_t input_numel = pten::product(in_dims);
    ...
    in_temp.Resize(pten::make_ddim({input_numel}));
    out_temp.Resize(pten::make_ddim({input_numel}));
    val_temp.Resize(pten::make_ddim({index_numel}));
