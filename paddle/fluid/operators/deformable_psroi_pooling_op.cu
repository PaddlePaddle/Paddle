    ...
    const int height)
{
  ...
  T value = (1 - dist_x) * (1 - dist_y) * value11 + (1 - dist_x) * dist_y * value12 + dist_x * (1 - dist_y) * value21 + dist_x * dist_y * value22;
  return value;
}
...
public:
  void Compute(const framework::ExecutionContext& ctx)  const override{      
    ...
    PADDLE_ENFORCE_EQ(top_count->dims(), out->dims(), "number of bbox should be same with number of output");    
    ...
    PADDLE_ENFORCE_EQ(num_bbox, out->dims()[0], "number of bbox should be same with number of output");
    ...
    DeformablePSROIPoolForwardKernel<<<GET_BLOCKS(count), CUDA_NUM_THREADS, 0,dev_ctx.stream()>>>(//>>>(
        count, bottom_data, (T)spatial_scale, channels, height, width, pooled_height, pooled_width,
        bottom_rois, bottom_trans, no_trans, (T)trans_std, sample_per_part, output_dim,
        group_size, part_size, num_classes, channels_each_class, top_data, top_count_data, roi_id_data);
...
__global__ void DeformablePSROIPoolBackwardAccKernel(
    ...
    T* bottom_data_diff, T* bottom_trans_diff, const T *bottom_data,
    const T *bottom_rois, const T *bottom_trans, const int no_trans,
    const T trans_std, const int sample_per_part, const int group_size,
    const int part_size, const int num_classes, const int channels_each_class,
  ...
  CUDA_KERNEL_LOOP(index, count)
  {
    ...
    for (int ih = 0; ih < sample_per_part; ih++)
    {
      for (int iw = 0; iw < sample_per_part; iw++)
      {
        ...
        platform::CudaAtomicAdd(offset_bottom_data_diff + bottom_index_base + y0 * width + x0, q00 * diff_val);
        platform::CudaAtomicAdd(offset_bottom_data_diff + bottom_index_base + y1 * width + x0, q01 * diff_val);
        platform::CudaAtomicAdd(offset_bottom_data_diff + bottom_index_base + y0 * width + x1, q10 * diff_val);
        platform::CudaAtomicAdd(offset_bottom_data_diff + bottom_index_base + y1 * width + x1, q11 * diff_val);
        ...
        platform::CudaAtomicAdd(bottom_trans_diff + (((n * num_classes + class_id) * 2) * part_size + part_h) * part_size + part_w, diff_x);
        platform::CudaAtomicAdd(bottom_trans_diff + (((n * num_classes + class_id) * 2 + 1) * part_size + part_h) * part_size + part_w, diff_y);
      }
    }
  }
}
...
class DeformablePSROIPoolGradCUDAKernel : public framework::OpKernel<T>{
  public:  
    void Compute(const framework::ExecutionContext& ctx)  const override{
      ...
      DeformablePSROIPoolBackwardAccKernel<<<GET_BLOCKS(count), CUDA_NUM_THREADS, 0, dev_ctx.stream()>>>( 
        count, top_diff, top_count_data, num_bbox, (T)spatial_scale, channels, height, width,
        pooled_height, pooled_width, output_dim, bottom_data_diff, bottom_trans_diff,
        bottom_data, bottom_rois, bottom_trans, no_trans, (T)trans_std, sample_per_part,
        group_size, part_size, num_classes, channels_each_class, roi_id_data);
    }
};
