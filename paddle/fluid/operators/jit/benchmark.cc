...
template <jit::KernelType KT, typename T, typename PlaceType>
void BenchRAXNKernel() {
  for (int d : TestSizes()) {
    Tensor x;
    RandomVec<T>(d, x.mutable_data<T>({d}, PlaceType()));
    BenchAllImpls<KT, jit::RAXNTuples<T>, PlaceType>(d, x.data<T>(), d);
  }
}
...
// raxn
BENCH_FP32_CPU(kASum) { BenchRAXNKernel<jit::kASum, T, CPUPlace>(); }
BENCH_FP32_CPU(kAMax) { BenchRAXNKernel<jit::kAMax, T, CPUPlace>(); }
