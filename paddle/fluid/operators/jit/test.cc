...
template <typename T>
struct TestFuncWithRefer<jit::RAXNTuples<T>, std::vector<T>, T> {
  void operator()(const typename jit::RAXNTuples<T>::func_type tgt,
                  const std::vector<T>& x, const T ref_res) {
    EXPECT_TRUE(tgt != nullptr);
    T tgt_res = tgt(x.data(), x.size());
...
void TestRAXNKernel() {
  VLOG(10) << "===== Test JITKernel " << jit::to_string(KT);
  for (int d : TestSizes()) {
    auto ref = jit::GetRefer<KT, jit::RAXNTuples<T>>();
    EXPECT_TRUE(ref != nullptr);
    std::vector<T> x(d);
    RandomVec<T>(d, x.data());
    TestAllImpls<KT, jit::RAXNTuples<T>, PlaceType, std::vector<T>, T>(
        d, x, ref(x.data(), d));
  }
}
...
// RAXNTuples
TEST(JITKernel, kAMax) {
  TestRAXNKernel<jit::kAMax, float, CPUPlace>();
  TestRAXNKernel<jit::kAMax, double, CPUPlace>();
}

TEST(JITKernel, kASum) {
  TestRAXNKernel<jit::kASum, float, CPUPlace>();
  TestRAXNKernel<jit::kASum, double, CPUPlace>();
}
