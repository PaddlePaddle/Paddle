...
void AttentionLSTMOp::InferShape(framework::InferShapeContext* ctx) const {
  ...
  PADDLE_ENFORCE_EQ(
      ...
      platform::errors::InvalidArgument(
          "Expected input(X)'s rank is 2. But received %d.", x_dims.size()));
  ...
  PADDLE_ENFORCE_EQ(
      w_dims.size(), 2,
      platform::errors::InvalidArgument(
          "Expected input(LSTMWeight)'s rank is 2.But received %d.",
          w_dims.size()));
  PADDLE_ENFORCE_EQ(
      w_dims[0], D + M,
      platform::errors::InvalidArgument(
  ...
  if (ctx->HasInput("H0")) {
    ...
    PADDLE_ENFORCE_EQ(
        h_dims.size(), 2UL,
        platform::errors::InvalidArgument(
            "Expected input(H0)'s rank is 2. But received %d.", h_dims.size()));
    if (ctx->IsRuntime() ||
  ...
  PADDLE_ENFORCE_EQ(
      atten_w_dims[0], M + D,
      platform::errors::InvalidArgument("Expected `AttentionWeight` shapes is "
                                        "(%d + %d) * 1. But received %d * 1",
                                        M, D, atten_w_dims[0]));
