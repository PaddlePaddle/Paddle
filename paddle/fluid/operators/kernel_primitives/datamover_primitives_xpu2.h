...
struct BroadcastConfig {
  ...
  HOSTDEVICE BroadcastConfig(const std::vector<int64_t>& out_dims,
    ...
    //     std::cout<<"stride_in ####### "<< strides_in_tmp[i] <<std::endl;
    //     std::cout<<"########## stride_out "<< strides_out_tmp[i] <<std::endl;
    //     std::cout<<"in_dims "<< dim_tmp[i] <<std::endl;
    //	}
    //     std::cout<<"!!!!!!  printf end !!!!!!"<<std::endl;
...
__device__ int IndexCalculator(const int _global_ptr_* in_strides,
                               const int _global_ptr_* out_strides,
                               const int _global_ptr_* in_dims, int in_rank,
                               int offset) {
  constexpr int kMaxRank = framework::DDim::kMaxRank;
  __local__ int in_stride[kMaxRank];
  __local__ int out_stride[kMaxRank];
  __local__ int in_dim[kMaxRank];
  GM2LM(in_strides, in_stride, kMaxRank * sizeof(int));
  GM2LM(out_strides, out_stride, kMaxRank * sizeof(int));
  GM2LM(in_dims, in_dim, kMaxRank * sizeof(int));
  int index = 0;
#pragma unroll
  for (int i = 0; i < kMaxRank; i++) {
    ...
    index += (offset / in_stride[i]) * out_stride[in_dim[i]];
    offset = offset % in_stride[i];
  }
  return index;
}
...
//          bool IsBoundary = false>
//__device__  void ReadDataBc(
//    T* dst, const T _global_ptr_* src, uint32_t block_offset,
//    details::BroadcastConfig<Rank> config, int total_num_output, int
//    stride_nx,
//    int stride_ny) {
...
__device__ void ReadDataReduce(Ty* dst, const Tx _global_ptr_* src,
                               int block_offset,
                               const int _global_ptr_* reduce_strides,
                               const int _global_ptr_* output_strides,
                               const int _global_ptr_* reduce_dims,
                               int reduce_rank, int size_nx, int size_ny,
...
#pragma unroll
    for (int ny = 0; ny < NY; ++ny) {
      ...
      uint32_t index_src =
          IndexCalculator(reduce_strides, output_strides, reduce_dims,
                          reduce_rank, thread_offset + block_offset);
...
__device__ void ReadDataBc(T* dst, const T _global_ptr_* src,
                           const int _global_ptr_* strides_out,
                           const int _global_ptr_* strides_in,
                           const int _global_ptr_* in_dims, int block_offset,
                           int total_num_output) {
  int thread_offset = block_offset + core_id() * NX;
  int index_src = 0;
  __local__ T in_temp[1];
  __local__ int stride_out[Rank];
  __local__ int stride_in[Rank];
  __local__ int in_dim[Rank];
  for (int i = 0; i < Rank; i++) {
    stride_out[i] = 1;
    stride_in[i] = 1;
    in_dim[i] = 1;
  }
  GM2LM(strides_out, stride_out, Rank * sizeof(int));
  GM2LM(strides_in, stride_in, Rank * sizeof(int));
  GM2LM(in_dims, in_dim, Rank * sizeof(int));
...
#pragma unroll
  for (int nx = 0; nx < NX; ++nx) {
    ...
    index_src = 0;
    if (IsBoundary) {
      if (index_output >= (int)total_num_output) {
        break;
      }
    }
#pragma unroll
    for (int i = Rank - 1; i >= 0; --i) {
      int tmp_index = (index_output / stride_out[i]);
      index_output = index_output - tmp_index * stride_out[i];
      index_src += (tmp_index % in_dim[i]) * stride_in[i];
    }
    GM2LM(src + index_src, in_temp, sizeof(T));
    dst[nx] = in_temp[0];
  }
