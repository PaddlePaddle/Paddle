...
struct ReduceConfig {
 ...
 private:
  ...
  void SetStrides() {
    ...
    // fast divmod
    for (auto i : reduce_strides) {
      reduce_divmoders.push_back(FastDivMod(i));
    }
    for (auto i : left_strides) {
      left_divmoders.push_back(FastDivMod(i));
    }
  }
 ...
 public:
  ...
  std::vector<FastDivMod> reduce_divmoders;
  std::vector<FastDivMod> left_divmoders;
