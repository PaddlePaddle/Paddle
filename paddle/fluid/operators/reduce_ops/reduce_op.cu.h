...
void TensorReduceFunctorImpl(const framework::Tensor& x, framework::Tensor* y,
                             ...
                             gpuStream_t stream) {
  auto x_dim = framework::vectorize<int64_t>(x.dims());
  auto config = pten::detail::ReduceConfig<Ty>(origin_reduce_dims, x_dim);
  config.Run();
  int numel = x.numel();
  // after config.run()
  ...
  // launch pten::detail::ReduceHigherDimKernel
  ...
  if (config.reduce_type == pten::detail::ReduceType::kReduceHigherDim) {
    ...
    pten::detail::ReduceHigherDimKernel<Tx, Ty, MPType, ReduceOp<Tx, MPType>,
  ...
  // function will be used
  pten::detail::LaunchReduceKernel<Tx, Ty, MPType, ReduceOp<Tx, MPType>>(
