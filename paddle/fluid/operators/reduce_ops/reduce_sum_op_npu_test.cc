...
USE_OP(reduce_sum);
USE_OP_DEVICE_KERNEL(reduce_sum, NPU);
...
void Compare(f::Scope* scope, const p::DeviceContext& ctx, int size) {
  ...
  std::vector<T> vec_dims;
  vec_dims.push_back(1)
  // run
  f::AttributeMap attrs = {{"dims", vec_dims}, {"keep_dim", false}};
  //f::AttributeMap attrs = {};
  auto op =
      f::OpRegistry::CreateOp("reduce_sum", {"X", {"X"}},
                              {"Out", {"Out"}}, attrs);
  ...
  EXPECT_EQ((uint64_t)out_vec.size(), (uint64_t)(dim1));
  for (uint32_t i = 0; i < out_vec.size(); i++) {
    EXPECT_EQ(out_vec[i], static_cast<T>(1024));
  }
}


TEST(reduce_sum, NPU_fp32) {
  f::Scope scope;
