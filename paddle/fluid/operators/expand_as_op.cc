...
class ExpandAsGradOp : public framework::OperatorWithKernel {
 ...
 protected:
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    /*auto expand_tensor_dims = expand_tensor->dims();*/
  ...
  /* for (size_t i = start_pos; i < expand_tensor_dims.size(); ++i) {
  #    PADDLE_ENFORCE_EQ(x_dims[i] * expand_tensor[i], out_dims[i],
...
REGISTER_OPERATOR(expand_grad, ops::ExpandAsGradOp);
