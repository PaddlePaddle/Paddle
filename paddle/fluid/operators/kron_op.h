...
struct KronOpFunctor {
  void operator()(const DeviceContext& dev_ctx, const framework::Tensor& x,
    ...
    const framework::DDim stride_x = pten::stride(dim_x);
    const framework::DDim stride_y = pten::stride(dim_y);
    const framework::DDim stride_out = pten::stride(dim_out);
...
struct KronGradOpFunctor {
  void operator()(const DeviceContext& dev_ctx, const framework::Tensor& dout,
    ...
    const framework::DDim stride_x = pten::stride(dim_x);
    const framework::DDim stride_y = pten::stride(dim_y);
    const framework::DDim stride_dout = pten::stride(dim_dout);
...
inline framework::Tensor UnsqueezeTo(const framework::Tensor& src, int ndims) {
  ...
  if (rank < ndims) {
    ...
    res.Resize(pten::make_ddim(new_dim));
  }
  return res;
}
