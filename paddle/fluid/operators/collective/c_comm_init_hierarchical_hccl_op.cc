...
class CCommInitOpHierarchicalHCCL : public framework::OperatorBase {
 public:
  CCommInitOpHierarchicalHCCL(const std::string& type,
  ...
  void RunImpl(const framework::Scope& scope,
    ...
    PADDLE_ENFORCE_EQ(
        is_npu_place(place), true,
        platform::errors::PreconditionNotMet(
            "CCommInitOpHierarchicalHCCL can run on npu place only."));
    ...
    platform::HierarchicalHcclCommContext::Instance()
        .CreateHierarchicalHcclComm(group_name, rank_ids, rank_id, device_id,
                                    rid);
#else
...
class CCommInitOpHierarchicalHCCLMaker
...
Initialize collective communicatoin context within this trainer
    ...
    AddAttr<std::string>(
        ...
        "The group id used for HierarchicalHCCL, which may map to ringid!");
  }
};
...
REGISTER_OPERATOR(c_comm_init_hierarchical_hccl,
                  ops::CCommInitOpHierarchicalHCCL,
                  ops::CCommInitOpHierarchicalHCCLMaker);
