...
USE_OP_DEVICE_KERNEL(c_barrier, NPU);
...
void Prepare(f::Scope* scope, const p::DeviceContext& ctx,
             HcclRootInfo* hccl_id, int ring_id = 0) {
  auto x = scope->Var("X");
  ...
  comm_init_attrs["ring_id"] = ring_id;
...
void TestHCCLCBarrierOp(f::Scope* scope, const p::DeviceContext& ctx,
                        bool use_barrier = false, bool use_calc = true,
                        int ring_id = 0) {
  // init
  ...
  int num1 = 3;
  int num2 = 128;
  // FLAGS_selected_npus=0 or 1
  int proc = atoi(FLAGS_selected_npus.c_str());
  ...
  time_t now = time(0);
  struct tm tm0;
  tm* ltm = localtime_r(&now, &tm0);
  VLOG(2) << "Time start: " << ltm->tm_hour << ":" << ltm->tm_min << ":"
          << ltm->tm_sec;
  if (proc == 0) {
    sleep(10);
  }
  f::AttributeMap attrs_barrier;
  attrs_barrier["ring_id"] = ring_id;
  attrs_barrier["use_calc_stream"] = use_calc;
  auto op_barrier = f::OpRegistry::CreateOp("c_barrier", {{"X", {"Data"}}}, {},
                                            attrs_barrier);
  op_barrier->Run(*scope, place);

  time_t now1 = time(0);
  struct tm tm1;
  tm* ltm1 = localtime_r(&now1, &tm1);
  VLOG(2) << "Time end: " << ltm1->tm_hour << ":" << ltm1->tm_min << ":"
          << ltm1->tm_sec;
}
...
TEST(c_barrier, NPU) {
  ...
  // use barier, use calc stream
  VLOG(2) << "use_barrier: "
          << "true "
          << "use_comm_stream: "
          << "false";
  TestHCCLCBarrierOp(&scope, ctx, true);

  f::Scope scope1;
  HcclRootInfo hccl_id1;

  // only support one device, if more than one device, use first default
  p::NPUDeviceContext ctx1(p::NPUPlace(atoi(FLAGS_selected_npus.c_str())));

  PrepareUniqueId(&scope1, ctx1, &hccl_id1);
  Prepare(&scope1, ctx1, &hccl_id1, 1);
  // use barier, use comm stream
  VLOG(2) << "use_barrier: "
          << "true "
          << "use_comm_stream: "
          << "true";
  TestHCCLCBarrierOp(&scope, ctx, true, false, 1);
}
