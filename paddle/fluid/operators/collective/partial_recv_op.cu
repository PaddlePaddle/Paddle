...
class PartialRecvOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (map->has(rid)) {
      CPartialRecvProcessGroupFunctor<phi::GPUContext, T> functor_;
      functor_(ctx);
    } else {
      CPartialRecvFunctor<phi::GPUContext, T> functor_;
      functor_(ctx);
    }
  }
};
...
template <typename T>
struct CPartialRecvFunctor<phi::GPUContext, T> {
...
template <typename T>
struct CPartialRecvProcessGroupFunctor<phi::GPUContext, T> {
  void operator()(const framework::ExecutionContext& ctx) {
#if (defined(PADDLE_WITH_RCCL) || defined(PADDLE_WITH_NCCL)) && \
    NCCL_VERSION_CODE >= 2703
    auto out = ctx.Output<framework::LoDTensor>("Out");
    ...
    // PADDLE_ENFORCE_LT(
    //     peer, comm->nranks(),
    //     platform::errors::InvalidArgument("The value of peer (%d) you set
    //     must "
    //                                       "be less than comm->nranks (%d).",
    //                                       peer, comm->nranks()));
    ...
    VLOG(0) << "rank " << pg->GetRank() << " recv " << recv_numel
            << " from offset[" << offset << "] from " << peer;

    phi::DenseTensor shared_out;
    shared_out.ShareDataWith(*out);
    shared_out.Resize({numel});

    phi::DenseTensor slice_tensor =
        shared_out.Slice(offset, offset + recv_numel);
    std::vector<phi::DenseTensor> recv_out;
    recv_out.push_back(slice_tensor);
    pg->Recv(recv_out, peer)->Synchronize();
