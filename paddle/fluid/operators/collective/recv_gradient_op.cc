...
#include "paddle/fluid/operators/collective/recv_gradient_op.h"
#include <string>

namespace paddle {
namespace operators {

class RecvGradientOp : public framework::OperatorWithKernel {
 public:
  using framework::OperatorWithKernel::OperatorWithKernel;

  void InferShape(framework::InferShapeContext* ctx) const override {
    OP_INOUT_CHECK(ctx->HasOutput("Out"), "Output", "Out", "Recv_Gradient");
    int peer = ctx->Attrs().Get<int>("peer");
    int ring_id = ctx->Attrs().Get<int>("ring_id");
    PADDLE_ENFORCE_GE(
        peer, 0,
        platform::errors::InvalidArgument(
            "The peer (%d) for recv_gradient op must be non-negative.", peer));
    PADDLE_ENFORCE_GE(
        ring_id, 0,
        platform::errors::InvalidArgument(
            "The ring_id (%d) for recv_gradient op must be non-negative.",
            ring_id));
    ...
    if (ctx->GetOutputsVarType("Out").front() ==
      ...
      for (size_t i = 0; i < out_shape.size(); ++i) {
        PADDLE_ENFORCE_GE(
            out_shape[i], 1,
            platform::errors::InvalidArgument(
                "The shape attribute for recv_activation must be set "
...
class RecvGradientOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() {
...
REGISTER_OP_WITHOUT_GRADIENT(recv_gradient, ops::RecvGradientOp,
                             ops::RecvGradientOpMaker);

REGISTER_OP_CPU_KERNEL(recv_gradient, ops::RecvGradientOpCPUKernel<float>,
                       ops::RecvGradientOpCPUKernel<double>,
                       ops::RecvGradientOpCPUKernel<int>,
                       ops::RecvGradientOpCPUKernel<int64_t>,
                       ops::RecvGradientOpCPUKernel<plat::float16>);
