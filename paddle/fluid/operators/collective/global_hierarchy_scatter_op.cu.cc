...
class GlobalHierarchyScatterOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    // Step2: get dp_global_count, just a normal all_to_all.
    int dp_fwd_count = 0;
    ...
    PADDLE_ENFORCE_GPU_SUCCESS(platform::dynload::ncclGroupStart());
    for (auto i = 0; i < outside_nranks; ++i) {
      for (auto j = 0; j < n_expert; ++j) {
        int idx = j + i * n_expert;
        if (cpu_mp_fused_global_count_data[idx]) {
          PADDLE_ENFORCE_GPU_SUCCESS(platform::dynload::ncclSend(
              outside_send_buf + outside_send_ptr * in_feat,
              cpu_mp_fused_global_count_data[idx] * in_feat, dtype, i,
              outside_comm->comm(), stream));
          outside_send_ptr += cpu_mp_fused_global_count_data[idx];
        }
