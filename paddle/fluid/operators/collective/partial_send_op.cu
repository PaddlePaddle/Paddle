...
class PartialSendCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (map->has(rid)) {
      CPartialSendProcessGroupFunctor<phi::GPUContext, T> functor_;
      functor_(ctx);
    } else {
      CPartialSendFunctor<phi::GPUContext, T> functor_;
      functor_(ctx);
    }
  }
};
...
template <typename T>
struct CPartialSendFunctor<phi::GPUContext, T> {
...
template <typename T>
struct CPartialSendProcessGroupFunctor<phi::GPUContext, T> {
  void operator()(const framework::ExecutionContext& ctx) {
#if (defined(PADDLE_WITH_RCCL) || defined(PADDLE_WITH_NCCL)) && \
    NCCL_VERSION_CODE >= 2703
    auto x = ctx.Input<framework::LoDTensor>("X");
    ...
    VLOG(0) << "rank " << pg->GetRank() << " send " << send_numel
            << " from offset[" << offset << "] to " << peer;
    phi::DenseTensor shared_x;
    shared_x.ShareDataWith(*x);
    ...
    pg->Send(send_in, peer)->Synchronize();
#else
    PADDLE_THROW(platform::errors::Unavailable(
        "PaddlePaddle should be compiled with NCCL and "
        "NCCL version >= 2.7.3 is needed."));
