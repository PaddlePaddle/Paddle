...
class RecvActivationOp : public framework::OperatorWithKernel {
 public:
  using framework::OperatorWithKernel::OperatorWithKernel;

  void InferShape(framework::InferShapeContext* ctx) const override {
    OP_INOUT_CHECK(ctx->HasOutput("Out"), "Output", "Out", "Recv_Activation");
    int peer = ctx->Attrs().Get<int>("peer");
    int ring_id = ctx->Attrs().Get<int>("ring_id");
    PADDLE_ENFORCE_GE(
        peer, 0,
        platform::errors::InvalidArgument(
            "The peer (%d) for recv_activation op must be non-negative.",
            peer));
    PADDLE_ENFORCE_GE(
        ring_id, 0,
        platform::errors::InvalidArgument(
            "The ring_id (%d) for recv_activation op must be non-negative.",
            ring_id));
...
class RecvActivationOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() {
...
REGISTER_OP_WITHOUT_GRADIENT(recv_activation, ops::RecvActivationOp,
                             ops::RecvActivationOpMaker);

REGISTER_OP_CPU_KERNEL(recv_activation, ops::RecvActivationOpCPUKernel<float>,
                       ops::RecvActivationOpCPUKernel<double>,
                       ops::RecvActivationOpCPUKernel<int>,
                       ops::RecvActivationOpCPUKernel<int64_t>,
                       ops::RecvActivationOpCPUKernel<plat::float16>);
