 ...
 public:
  ...
  void RunImpl(const framework::Scope& scope,
    ...
    VLOG(3) << "begin c_comm_init on npu, parameters are: "
            << "ring id[" << rid << "], nranks[" << nranks << "], rank_id["
            << rank_id << "], device_id[" << device_id << "]";

    platform::HCCLCommContext::Instance().CreateHCCLComm(rank_ids, rank_id,
                                                         device_id, rid);
  }
};
...
REGISTER_OPERATOR(c_comm_init_hcom, ops::CCommInitOpNPU,
                  ops::CCommInitOpNPUMaker);
