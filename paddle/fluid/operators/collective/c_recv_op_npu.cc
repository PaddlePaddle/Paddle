...
class CRecvOpASCENDKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    int srcRank = ctx.Attr<int>("srcRank")
    ...
    VLOG(3) << "Begin hccl receive, parameter is: "<< "srRank" << srRank
      << ", group is " << group
      << ", tag is " << tag
      << ", srTag is " << srTag;
    ...
    platform::dynload::hcom_receive(
        ...
        srcRank, srTag, group.c_string(), stream);

    VLOG(3) << "srcRank " << srcRank << " invoke Receive. Receiving "
            << x->numel();
...
REGISTER_OP_NPU_KERNEL(c_receive, ops::CRecvOpASCENDKernel<float>,
