...
class CGenECCLIdOp : public framework::OperatorBase {
 public:
  CGenECCLIdOp(const std::string& type,
  ...
  void RunImpl(const framework::Scope& scope,
    ...
    VLOG(2) << "rank = " << rank << ", endpoint = " << endpoint
            << ", rank_count = " << rank_count
            << ", group_name = " << group_name
            << ", split_index = " << split_index;

    PADDLE_ENFORCE_NPU_SUCCESS(platform::dynload::eccl_gen_unique_id(
        rank, endpoint.c_str(), rank_count, split_index, group_name.c_str()));
  }
};
 ...
 public:
  ...
  void RunImpl(const framework::Scope& scope,
    ...
    PADDLE_THROW(platform::errors::PreconditionNotMet(
        "PaddlePaddle should compile with WITH_ECCL=ON."));
  }
};

#endif

class CGenECCLIdOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    // AddOutput("Out", "Raw variable contains a ECCL UniqueId instaces.");
    AddComment(R"DOC(
CGenECCLId operator
...
REGISTER_OPERATOR(c_gen_eccl_id, ops::CGenECCLIdOp, ops::CGenECCLIdOpMaker);
