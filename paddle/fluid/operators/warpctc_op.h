  ...
  void operator()(const framework::ExecutionContext& ctx, const float* input,
    ...
    /*PADDLE_ENFORCE_GT(workspace_bytes, 0UL,
                     platform::errors::InvalidArgument(
                          "Bytes of workspace got by warp-ctc function, "
                      "get_workspace_size(), should be larger than 0, but
       received %d",
                          workspace_bytes));
    */
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (ctx.HasInput("LogitsLength") && ctx.HasInput("LabelLength")) {
      ...
      /*
      PADDLE_ENFORCE_GT(logits->NumLevels, 0, platform::errors::InvalidArgument(
                                    "Input(logits) Tensor of WarpCTC "
                                    "does not contain LoD information."));
      PADDLE_ENFORCE_GT(label->NumLevels, 0, platform::errors::InvalidArgument(
      ...
      */
      ...
      PADDLE_ENFORCE_EQ(
          logits_dims[0], static_cast<int64_t>(logits_lod.back()),
          platform::errors::InvalidArgument(
                          "The first dimension of Input(Logits) should be equal
      to "
                          "the sum of all sequences' lengths = %d., but received
      %d. ",
