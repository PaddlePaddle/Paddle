...
class Reshape2NPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (shape_tensor_vector.size() > 0) {
      ...
      if (shape_tensor) {
        ...
        PADDLE_ENFORCE_GT(
          ...
          platform::errors::InvalidArgument(
              "The length of shape attribute should be larger than 0 when input ShapeTensor and Shape are empty!"));
      }
    }

    int num_negative = std::count(target_shape_vector.begin(), target_shape_vector.end(), -1);
    PADDLE_ENFORCE_LE(num_negative, 1,
                    platform::errors::InvalidArgument(
                        "The max number of -1 in shape attribute is 1 "
                        "but received %d.",
                        num_negative));
    auto it_zero = std::find(target_shape_vector.begin(), target_shape_vector.end(), 0);
        ...
        if (target_shape_vector[i] == 0) {
          PADDLE_ENFORCE_LT(
              ...
              platform::errors::InvalidArgument(
                  ...
                  "but the index is %d and input dim size is %d", i,
                  x_rank));
    ...
    auto it = std::find(target_shape_vector.begin(), target_shape_vector.end(), -1);
    if (it != target_shape_vector.end()) {
      ...
      int reshape_out_product = std::accumulate(
          target_shape_vector.begin(), target_shape_vector.end(), -1, std::multiplies<int>());
