 ...
 public:
  ...
  void InferShape(framework::InferShapeContext *ctx) const override {
    PADDLE_ENFORCE(ctx->HasInput("MaxProbs"),
                   "Input(MaxProbs) shold not be null.");
    PADDLE_ENFORCE(ctx->HasInput("Indices"),
                   "Input(Indices) shold not be null.");
    PADDLE_ENFORCE(ctx->HasInput("Labels"),
                   "Input(Labels) shold not be null.");
    PADDLE_ENFORCE(ctx->HasOutput("BatchMetrics"),
                   "Output(BatchMetrics) shold not be null.");
    PADDLE_ENFORCE(ctx->HasOutput("AccumMetrics"),
                   "Output(AccumMetrics) shold not be null.");
    PADDLE_ENFORCE(ctx->HasOutput("AccumStatesInfo"),
                   "Output(AccumStatesInfo) shold not be null.");
    ...
    if (ctx->IsRuntime()) {
      PADDLE_ENFORCE_EQ(max_probs_dims[1], 1,
                        ...
                        "shape of Input(MaxProbs) shold be [batch_size, 1].");
      PADDLE_ENFORCE_EQ(
          ctx->GetInputDim("Indices"), max_probs_dims,
          "The shape of Input(Indices) shold bes same with max_probs_dims");
      PADDLE_ENFORCE_EQ(
          max_probs_dims[0], labels_dims[0],
          "The 1st dimension of Input(MaxProbs) and "
          "Input(Labels) both are batch_size and the shape shold "
          "be the same.");
      PADDLE_ENFORCE_EQ(labels_dims[1], 1,
                        "The 2nd dimension of Input(Labels) contains instance "
                        "label and the shape shold be equal to 1.");
    }
    if (ctx->HasInput("Weights")) {
      ...
      if (ctx->IsRuntime()) {
        PADDLE_ENFORCE_EQ(weights_dims,
                          ...
                          "The shape of Input(Weights) shold be "
                          "[batch_size, 1].");
      }
    }
    if (ctx->HasInput("StatesInfo")) {
      ...
      if (ctx->IsRuntime()) {
        PADDLE_ENFORCE_EQ(states_dims, framework::make_ddim({cls_num, 4}),
                          "The shape of Input(StatesInfo) shold be "
                          "[class_number, 4].");
      }
    }
 ...
 public:
  void Make() override {
    ...
    AddInput("Indices",
             ...
             "indice which computed by the previous top_k (k=1) operator.");
    AddInput("Labels",
             "(Tensor, default Tensor<int>) A 2-D tensor with shape N x 1, "
             "where N is the batch size. Each element is a label and the "
             "value shold be in [0, class_number - 1].");
    AddInput("Weights",
             "(Tensor, default Tensor<float>) A 2-D tensor with shape N x 1, "
...
achieve this, Input(StatesInfo) shold be provided. State of current batch
