 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (label_dims.size() < rank) {
      ...
      labels_2d.Resize({pten::product(label_dims), 1});

      y_2d.ShareDataWith(*y);
      y_2d.Resize({pten::product(y->dims()), 1});
...
class CrossEntropyOpKernel2 : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto batch_size = pten::product(x->dims()) / feature_size;
...
class CrossEntropyGradientOpKernel2 : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    int64_t batch_size = pten::product(dx->dims()) / feature_size;
