...
void TensorFormatter::Print(const framework::LoDTensor& print_tensor,
  ...
  std::cout << Format(print_tensor, place, tensor_name, message);
}
...
std::string TensorFormatter::Format(const framework::LoDTensor& print_tensor,
  ...
  if (!tensor_name.empty()) {
    log_stream_ << "Variable: " << tensor_name << std::endl;
  }

  if (!message.empty()) {
    log_stream_ << "  - message: " << message << std::endl;
  }
  ...
  log_stream_ << "  - place: " << place << std::endl;

  if (print_tensor_shape_) {
    log_stream_ << "  - shape: " << print_tensor.dims().to_str() << std::endl;
  }
  ...
  if (print_tensor_type_) {
    log_stream_ << "  - dtype: " << platform::demangle(dtype.name())
                << std::endl;
  }

  if (framework::IsType<const float>(dtype)) {
    FormatData<float>(print_tensor);
  } else if (framework::IsType<const double>(dtype)) {
    FormatData<double>(print_tensor);
  } else if (framework::IsType<const int>(dtype)) {
    FormatData<int>(print_tensor);
  } else if (framework::IsType<const int64_t>(dtype)) {
    FormatData<int64_t>(print_tensor);
  } else if (framework::IsType<const bool>(dtype)) {
    FormatData<bool>(print_tensor);
  } else {
    log_stream_ << "  - data: unprintable type: " << dtype.name() << std::endl;
  }
...
void TensorFormatter::FormatData(const framework::LoDTensor& print_tensor) {
  ...
  const T* data = print_tensor.data<T>();
  log_stream_ << "  - data: [";
  if (print_size > 0) {
    log_stream_ << data[0];
