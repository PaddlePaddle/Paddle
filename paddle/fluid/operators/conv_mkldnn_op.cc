 ...
 public:
  void Compute(const paddle::framework::ExecutionContext& ctx) const override {
    ...
    if (is_INT8) {
      ...
      PADDLE_ENFORCE(
          input->dims().size() == 4 || input->dims().size() == 5,
          "Input must be with 4 or 5 dimensions, i.e. NCHW or NCDHW");
      PADDLE_ENFORCE(
          filter->dims().size() == 4 || filter->dims().size() == 5,
...
#pragma omp parallel for if (count > 1)
      ...
      auto user_src_md = platform::MKLDNNMemDesc(
          {src_tz}, paddle::framework::ToMKLDNNDataType(input->type()),
          input->format());
      auto user_weights_md = platform::MKLDNNMemDesc(
      ...
      */
      ...
      auto src_md = platform::MKLDNNMemDesc(src_tz, memory::data_type::u8,
      ...
      if (fuse_residual_conn) {
        ...
        auto residual_dt =
            paddle::framework::ToMKLDNNDataType(residual_param->type());
        PADDLE_ENFORCE_EQ(
            residual_param->format(), handler->GetDstFormat(),
        ...
        platform::SetDstMemory<float>(ctx, output, handler, dst_memory_p);
      }
