 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    PADDLE_ENFORCE_EQ(row_ids.size(), x_tensors.size(),
                      "the number of Rows and X shold be the same");
    PADDLE_ENFORCE_EQ(ids.size(), outs.size(),
                      "the number of Ids and Out shold be the same");
    ...
    for (size_t i = 0; i < x_tensors.size(); ++i) {
      ...
      PADDLE_ENFORCE_EQ(embedding_size, x_tensor->dims()[1],
                        "embedding size of all input shold be the same");
    ...
    PADDLE_ENFORCE_EQ(
        ...
        "the merged X dim[0] and merged Rows dim[0] shold be the same");
    ...
    PADDLE_ENFORCE_EQ(row_ids_size, selected_rows_idx_map.size(),
                      "the rows and tensor map size shold be the same");
