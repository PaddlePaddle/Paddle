...
void MatchMatrixTensorOP::InferShape(framework::InferShapeContext* ctx) const {
  ...
  if (ctx->IsRuntime()) {
    ...
    framework::Variable* y_var =
        BOOST_GET_CONST(framework::Variable*, ctx->GetInputVarPtrs("Y")[0]);
    ...
    framework::VarDesc* x_desc =
        BOOST_GET_CONST(framework::VarDesc*, ctx->GetInputVarPtrs("X")[0]);
    PADDLE_ENFORCE_GE(x_desc->GetLoDLevel(), 1);
    framework::VarDesc* y_desc =
        BOOST_GET_CONST(framework::VarDesc*, ctx->GetInputVarPtrs("Y")[0]);
