 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (!context.Attr<bool>("is_test")) {
      ...
      // simply set the mask and output as 0 (all drop)
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        if ((x_width & (x_width - 1)) == 0) {
          // if x_width is power of 2, use bit operation instead of mod
          bias_id = i & (x_width - 1);
        } else {
          bias_id = i % x_width;
        }
        if (dist(*engine) < dropout_prob) {
          ...
          // add bias to the output while doing the dropout
          mask_data[i] = 1;
          if (upscale_in_train) {
...
#endif
      // add bias while update the output
