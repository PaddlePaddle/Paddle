...
void call_gemm(const math::BlasT<DeviceContext, T>& blas,
               ...
               const T* B, const T beta, T* C, bool naive_gemm) {
if (!naive_gemm) {
 ...
 public:
  void Make() override {
    ...
    AddAttr<int>("out_size", "out_size: the output size")
        .SetDefault(0)
        .EqualGreaterThan(1);
    AddAttr<bool>("naive_gemm", "naive_gemm: use naive gemm")
        .SetDefault(false);
...
class CPUSearchFCOPKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    bool naive_gemm = ctx.Attr<bool>("naive_gemm");  // 100
    ...
    call_gemm(blas, CblasNoTrans, CblasTrans, batch, _out, _in, 1.0f,
              bottom_data, weights, 0.0f, top_data, naive_gemm);
    if (true) {
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    call_gemm(blas, CblasTrans, CblasNoTrans, _out, _in, batch, (T)1.0,
                          top_diff, bottom_data, (T)0.0, weights_diff, naive_gemm);

    call_gemm(blas, CblasNoTrans, CblasNoTrans, batch, _in, _out, (T)1.0, top_diff,
              weights, (T)0.0, bottom_diff, naive_gemm);
