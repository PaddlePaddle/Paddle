  ...
  */
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (x_rank > 2) {
      ...
      if (axis == 0) {
        preserved_dims = pten::slice_ddim(x_.dims(), 1, x_rank);
        x_resized_dims = {seq_length, pten::product(preserved_dims)};
        out_resized_dims = {n_frames, frame_length,
                            pten::product(preserved_dims)};
      } else {
        preserved_dims = pten::slice_ddim(x_.dims(), 0, x_rank - 1);
        x_resized_dims = {pten::product(preserved_dims), seq_length};
        out_resized_dims = {pten::product(preserved_dims), frame_length,
                            n_frames};
      }
  ...
  */
  void Compute(const framework::ExecutionContext& ctx) const {
    ...
    if (d_x_rank > 2) {
      ...
      if (axis == 0) {
        preserved_dims = pten::slice_ddim(d_x->dims(), 1, d_x_rank);
        d_x_resized_dims = {seq_length, pten::product(preserved_dims)};
        d_out_resized_dims = {n_frames, frame_length,
                              pten::product(preserved_dims)};
      } else {
        preserved_dims = pten::slice_ddim(d_x->dims(), 0, d_x_rank - 1);
        d_x_resized_dims = {pten::product(preserved_dims), seq_length};
        d_out_resized_dims = {pten::product(preserved_dims), frame_length,
                              n_frames};
      }
