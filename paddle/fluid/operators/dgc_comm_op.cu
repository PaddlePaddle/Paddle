...
class DGCCommOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    LOG(INFO) << "=======dgc comm op ======";
    LOG(INFO) << "=======dgc comm op =====input==" << *x;
    LOG(INFO) << "=======dgc comm op========gather_buff==" << gather_res;
    LOG(INFO) << "=======dgc comm op========out_numel===" << out_numel;
#if defined(PADDLE_WITH_DGC)

    LOG(INFO) << "=======dgc comm op========k==" << k;
    LOG(INFO) << "=======dgc comm op========nranks==" << nranks;
