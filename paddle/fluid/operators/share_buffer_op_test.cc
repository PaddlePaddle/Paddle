...
TEST(test_share_buffer_op, test_share_buffer_op) {
  ...
  for (size_t i = 0; i < n; ++i) {
    auto *in_tensor = scope.Var(inputs[i])->GetMutable<Tensor>();
    in_tensor->Resize(dims[i]);
    in_tensor->mutable_data<float>(place);
    scope.Var(outputs[i])->GetMutable<Tensor>();
  }
  op->Run(scope, place);
  platform::DeviceContextPool::Instance().Get(place)->Wait();

  for (size_t i = 0; i < n; ++i) {
    const auto &in_tensor = scope.Var(inputs[i])->Get<Tensor>();
    const auto &out_tensor = scope.Var(outputs[i])->Get<Tensor>();
