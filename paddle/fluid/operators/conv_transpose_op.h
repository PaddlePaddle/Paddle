...
class DepthwiseConvTransposeKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    int groups = context.Attr<int>("groups");
    PADDLE_ENFORCE_EQ(groups, filter.dims()[0], platform::errors::InvalidArgument("groups should be error to the 1st dimension of filter. But received groups is %d and filter dimension[0] is %d", groups, filter.dims()[0]));
    ...
    for (auto v : dilations) {
      PADDLE_ENFORCE_EQ(v, 1, platform::errors::InvalidArgument("dilations should be 1 in depthwise conv. But received dilations is %d", v));
    }
