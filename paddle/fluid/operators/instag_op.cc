...
namespace operators {
    ...
    public:
        ...
        void InferShape(framework::InferShapeContext* ctx) const override {
            ...
            ctx->SetOutputDim("Out", framework::make_ddim({x3_dims[0], x1_dims[0], x1_dims[1]}));
        }
    protected:
    ...
    public:
        void Make() override {
            ...
            AddInput("X3", "(LoDTensor) fc tag list");
...
or not. And the output only shares the LoD information with input Ids.                         
	...
	public:
		...
		void InferShape(framework::InferShapeContext* ctx) const override {
			...
			ctx->SetOutputDim(framework::GradVarName("X1"), framework::make_ddim({grad_out_dims[1], grad_out_dims[2]}));
		}
	protected:
		framework::OpKernelType GetExpectedKernelType(const framework::ExecutionContext& ctx) const override {
			auto data_type = framework::GetDataTypeOfVar(ctx.InputVar(framework::GradVarName("Out")));
			return framework::OpKernelType(data_type, ctx.device_context());
		}
	};
