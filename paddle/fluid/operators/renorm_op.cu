...
class CUDARenormKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    pow_value.Resize(pten::make_ddim({pre_mul, dimension_each, dim_divisor}));
    dim_value.Resize(pten::make_ddim({dimension_each}));
    pow_value.mutable_data<T>(context.GetPlace());
    out->Resize(pten::make_ddim(pten::vectorize(input_dims)));
...
class CUDAGradRenormKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    for (int i = dim + 1; i < dim_size; i++) dim_divisor *= input_dims[i];
    for (int i = 0; i < dim; i++) pre_mul *= input_dims[i];
    d_x->Resize(pten::make_ddim(pten::vectorize(input_dims)));
    T* dx_data = d_x->mutable_data<T>(ctx.GetPlace());
    framework::Tensor pow_value, mul_value, dim_value, dim_power_sum,
        weight_derivative;
    pow_value.Resize(pten::make_ddim({pre_mul, dimension_each, dim_divisor}));
    mul_value.Resize(pten::make_ddim({pre_mul, dimension_each, dim_divisor}));
    dim_value.Resize(pten::make_ddim({dimension_each}));
    dim_power_sum.Resize(pten::make_ddim({dimension_each}));
    weight_derivative.Resize(pten::make_ddim({dimension_each}));
