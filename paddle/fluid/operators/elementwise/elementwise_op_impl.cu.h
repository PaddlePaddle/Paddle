...
__global__ void VectorizedKernel(
    ElementwiseDataWrapper<ET, VecSize, InT, OutT> data, int main_tid,
    int tail_tid, Functor func) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;

  if (tid < main_tid) {
    VectorizedKernelImpl<ET, VecSize, InT, OutT, Functor>(data, func, tid);
  }
  if (tid < tail_tid) {
    ScalarKernelImpl<ET, VecSize, InT, OutT, Functor>(data, func, tid);
  }
}

template <ElementwiseType ET, typename InT, typename OutT, typename Functor>
__global__ void ScalarKernel(ElementwiseDataWrapper<ET, 1, InT, OutT> data,
                             int numel, Functor func) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid < numel) {
    ScalarKernelImpl<ET, 1, InT, OutT, Functor>(data, func, tid);
  }
}
