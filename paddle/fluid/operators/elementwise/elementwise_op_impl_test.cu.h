...
*/
// inline int GetThreadsConfig(const platform::XPUDeviceContext &ctx,
...
//    threads = platform::RoundToPowerOfTwo(active_threads_num / (sm_count <<
//    1));
...
template <typename InT, typename OutT, typename Functor, int Arity, int VecSize,
          bool IsBoundary>
__device__ void ElementVectorizeKernelImpl(const InT _global_ptr_ *in0,
                                           const InT _global_ptr_ *in1,
                                           const InT _global_ptr_ *in2,
                                           OutT _global_ptr_ *out, int num,
                                           int data_offset, Functor func) {
...
#pragma unroll
  ...
  kps::ReadData<InT, VecSize, 1, 1, IsBoundary>(args[0], in0 + data_offset,
                                                num);
  if (Arity >= 2) {
    kps::ReadData<InT, VecSize, 1, 1, IsBoundary>(args[1], in1 + data_offset,
                                                  num);
  } else if (Arity == 3) {
    kps::ReadData<InT, VecSize, 1, 1, IsBoundary>(args[2], in2 + data_offset,
                                                  num);
  }
...
template <typename InT, typename OutT, typename Functor, int Arity, int VecSize>
__global__ void ElementVectorizeKernel(const InT *in0, const InT *in1,
                                       const InT *in2, OutT *out, int size,
                                       int main_offset, Functor func) {
  int data_offset = BLOCK_ID_X * BLOCK_NUM_X * VecSize;
  int stride = BLOCK_NUM_X * GRID_NUM_X * VecSize;
  for (; data_offset < main_offset; data_offset += stride) {
    ElementVectorizeKernelImpl<InT, OutT, Functor, Arity, VecSize, false>(
        in0, in1, in2, out, VecSize * BLOCK_NUM_X, data_offset, func);
  }

  int num = size - data_offset;
  if (num > 0) {
    ElementVectorizeKernelImpl<InT, OutT, Functor, Arity, VecSize, true>(
        in0, in1, in2, out, num, data_offset, func);
  }
}

template <typename InT, typename OutT, typename Functor, int Arity, int VecSize>
void ElementwiseCudaKernel(const platform::XPUDeviceContext &ctx,
                           const std::vector<const framework::Tensor *> &ins,
                           std::vector<framework::Tensor *> *outs,
                           Functor func) {
  auto numel = ins[0]->numel();
  int block_size = 128;  // GetThreadsConfig(ctx, numel, VecSize);
  int grid_size =
  ...
  ElementVectorizeKernel<InT, OutT, Functor, Arity,
      ...
      ins_data[0], ins_data[1], ins_data[2], out_data, numel,
      main_tid * block_size * VecSize, func);

// float* output_cpu= (float*) malloc( numel * sizeof(float));
// int ret = xpu_memcpy(output_cpu, ins_data[0], numel * sizeof(float),
// XPU_DEVICE_TO_HOST);
