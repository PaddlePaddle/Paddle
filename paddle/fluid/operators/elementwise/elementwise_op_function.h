...
inline void GetBroadcastDimsArrays(const framework::DDim &x_dims,
                                   ...
                                   const int axis) {
  PADDLE_ENFORCE_GE(axis, 0, platform::errors::InvalidArgument("Axis should be great than or equal to 0, but received axis is %d", axis));
  PADDLE_ENFORCE_LT(axis, max_dim, platform::errors::InvalidArgument("Axis should be less than %d, but received axis is %d", max_dim, axis));
  ...
  for (int i = 0; i < max_dim; i++) {
    PADDLE_ENFORCE_EQ(x_dims_array[i] == y_dims_array[i] || x_dims_array[i] <= 1 ||
                       y_dims_array[i] <= 1, true, platform::errors::InvalidArgument(
...
void FusedElemwiseAndActGradComputeEx(
  ...
  if (UseIntermediateOut) {
    PADDLE_ENFORCE_EQ(intermediate_out, true, platform::errors::InvalidArgument("Intermediate out is null pointer."));
  }
  if (x_dim == y_dim) {
...
void FusedElemwiseAndActComputeEx(const framework::ExecutionContext &ctx,
                                  ...
                                  framework::Tensor *intermediate_out) {
  if (KeepIntermediateOut) {
    PADDLE_ENFORCE_EQ(intermediate_out, true, platform::errors::InvalidArgument("The save_intermediate_out is opened, intermediate out is null pointer."));
  }
