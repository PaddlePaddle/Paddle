...
class TestElementwiseOpGradGrad {
 public:
  ...
  void InitVarInScope(std::string var_name) {
    in_out_tensors_[var_name] =
        scope_.Var(var_name)->template GetMutable<framework::Tensor>();
  ...
  bool Check() {
    ...
    for (auto &out_name : outputs_) {
      auto &out_tensor =
          scope_.FindVar(out_name)->template Get<framework::Tensor>();
 ...
 protected:
  ...
  std::map<std::string, paddle::framework::Tensor *> in_out_tensors_;
