  ...
  void InferShape(framework::InferShapeContext *ctx) const override {
    ...
    if (ctx->GetInputsVarType("X").front() ==
      ...
      PADDLE_ENFORCE_EQ(
          ...
          "), Y must be scalar, the size of Y should be 1u. But reveived the size of Y = %s",
          ctx->GetInputDim("Y").size());
      PADDLE_ENFORCE_EQ(
          ctx->GetInputDim("Y")[0], 1,
          platform::errors::InvalidArgument(
          "For elementwise_op, if X is Sparse(VarType.SELECTED_ROWS"
          "), Y must be scalar, the first dimension of Y should be 1. But reveived the first dimension of Y = %s",
          ctx->GetInputDim("Y")[0]);
    } else if (ctx->GetInputsVarType("X").front() !=
               framework::proto::VarType::LOD_TENSOR) {
      PADDLE_THROW(platform::errors::InvalidArgument("Input X's type[%s] is not supported by elementwise_op. Please set its type to LOD_TENSOR.",
...
class ElementwiseOpGrad : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext *ctx) const override {
    ...
    OP_INOUT_CHECK(ctx->HasInput(out_grad_name), "Input", out_grad_name, "ElementwiseOpGrad");
 ...
 public:
  ...
  framework::OpKernelType GetExpectedKernelType(
    ...
    if (ctx.HasInput("DDX") == false) {
      OP_INOUT_CHECK(ctx->HasInput("DDY"), "Input", "DDY", "ElementwiseOpDoubleGradWithoutDXDY");
      input_data_type = OperatorWithKernel::IndicateVarDataType(ctx, "DDY");
    } else if (ctx.HasInput("DDY") == false) {
      OP_INOUT_CHECK(ctx->HasInput("DDX"), "Input", "DDX", "ElementwiseOpDoubleGradWithoutDXDY");
