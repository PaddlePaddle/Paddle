...
USE_OP_DEVICE_KERNEL(elementwise_mul_grad, NPU);
#endif
...
// Case 1: [4, 1, 1] x [4] = [4, 1, 4]

// input - X
const std::vector<int64_t> x_dim = std::vector<int64_t>{2, 3, 4};
const int64_t x_numel = std::accumulate(x_dim.begin(), x_dim.end(), 1, std::multiplies<int64_t>());
// input - Y
const std::vector<int64_t> y_dim = std::vector<int64_t>{4};
const int64_t y_numel = std::accumulate(y_dim.begin(), y_dim.end(), 1, std::multiplies<int64_t>());
// Output - Out
const std::vector<int64_t> out_dim = {2, 3, 4};
...
void TestMain(const platform::DeviceContext& ctx, std::vector<T>* out_data, std::vector<T>* x_grad_data, std::vector<T>* y_grad_data) {
  ...
  // --------------- backward ----------------------
  desc_bwd.SetType("elementwise_mul_grad");
...
TEST(test_elementwise_mul, compare_cpu_and_npu) {
  ...
  // TestMain<float>(npu_ctx, &out_npu);
