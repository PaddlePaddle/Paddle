...
void NpuBroadcast(const platform::NPUDeviceContext& dev_ctx, const Tensor* src,
  ...
  auto prev = pten::product(pten::slice_ddim(dst_dims, 0, axis));
  if (prev > 1) {
    Tensor tmp_tensor;
    auto tmp_tensor_dims =
        pten::slice_ddim(dst_dims, 0, axis + src_dims.size());
    tmp_tensor.mutable_data<T>(tmp_tensor_dims, dev_ctx.GetPlace());
    const auto& runner =
        NpuOpRunner("ExpandD", {tmp_src}, {tmp_tensor},
                    {{"shape", pten::vectorize<int64_t>(tmp_tensor_dims)}});
    runner.Run(stream);
    tmp_src.ShareDataWith(tmp_tensor);
    tmp_src.Resize(tmp_tensor_dims);
  } else {
    tmp_src.Resize(pten::slice_ddim(dst_dims, 0, axis + src_dims.size()));
  }

  // 3.expand the tail axis
  auto post = pten::product(
      pten::slice_ddim(dst_dims, axis + src_dims.size(), dst_dims.size()));
  if (post > 1) {
    auto src_dims_vec = pten::vectorize<int>(tmp_src.dims());
    src_dims_vec.push_back(1);
    tmp_src.Resize(pten::make_ddim(src_dims_vec));
...
void NpuElementWiseOpBroadcast(const platform::NPUDeviceContext& dev_ctx,
  ...
  std::vector<int> dst_dims_vec = pten::vectorize<int>(x_dims);

  if (x_dims.size() < y_dims.size()) {
    is_xsize_larger = false;
    max_dim = y_dims.size();
    dst_dims_vec = pten::vectorize<int>(y_dims);
  }
  ...
  auto dst_dims = pten::make_ddim(dst_dims_vec);
