...
class ElementwiseMulKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    PADDLE_ENFORCE_EQ(x_var != nullptr, true, platform::errors::InvalidArgument(
    ...
    if (x_var->IsType<framework::SelectedRows>()) {
      PADDLE_ENFORCE_EQ(y->dims().size() == 1 && y->dims()[0] == 1, true, platform::errors::InvalidArgument(
                     "For elementwise_op, if X is Sparse, Y must be scalar. But reveived the size of Y = %s.", y->dims().size()));
      ...
      PADDLE_THROW(platform::errors::InvalidArgument("X's type[%s] is not supported by elementwise_op. X's type should be LoDTensor or SelectedRows.",
