...
template <typename T>
void FindOutMulTensors(const framework::ExecutionContext& ctx,
                       const framework::Variable* x_var, framework::Tensor* x,
                       const framework::Tensor* y, framework::Tensor* z) {
  if (x_var->IsType<framework::SelectedRows>()) {
    PADDLE_ENFORCE_EQ(y->dims().size() == 1 && y->dims()[0] == 1, true,
                      platform::errors::InvalidArgument(
                          "For elementwise_op, "
                          "if X is Sparse, Y must be scalar. But reveived the "
                          "size of Y = %d.",
                          y->dims().size()));
    ...
    *x = x_var->Get<framework::LoDTensor>();
    ...
    PADDLE_THROW(platform::errors::InvalidArgument(
        "X's type[%s] is not "
        "supported by elementwise_op. X's type should be LoDTensor "
        "or SelectedRows.",
