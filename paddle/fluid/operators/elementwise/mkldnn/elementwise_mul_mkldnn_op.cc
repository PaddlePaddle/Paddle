...
static void ReorderInput(framework::Tensor* tensor,
  ...
  out_tensor.set_format(isFourDim ? memory::format_tag::nchw
...
class ElementwiseMulMKLDNNKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto x_int_dims = paddle::framework::vectorize(x_dims);
    ...
    const bool is_x_format_correct = x->format() == memory::format_tag::nChw16c;
    const bool is_y_format_correct = y->format() == memory::format_tag::nc;
...
#pragma omp parallel for collapse(2)
      ...
      const bool is_x_nchw = x->format() == memory::format_tag::nchw;
      const bool is_x_nc = x->format() == memory::format_tag::nc;
      const bool is_x_x = x->format() == memory::format_tag::x;
      const bool is_y_nchw = y->format() == memory::format_tag::nchw;
      const bool is_y_nc = y->format() == memory::format_tag::nc;
      const bool is_y_x = y->format() == memory::format_tag::x;
