...
template <typename T,
          ...
          int LDGS = ELTS_PER_ROW / ELTS_PER_ROW_PER_CTA>
__global__
__launch_bounds__(THREADS_PER_CTA) void fused_fast_ln_fwd_quant_dequant_kernel(
    int rows,
    int cols,
    uint64_t seed,
    const float dropout_prob,
...
#pragma unroll
    for (int it = 0, col = c; it < LDGS; it++) {  // 1
      phi::Load<int32_t, VecSize>(x_ptr + row * ELTS_PER_ROW + col * VecSize,
                                  &x_int32[it]);
      phi::Load<T, VecSize>(residual_ptr + row * ELTS_PER_ROW + col * VecSize,
                            &residual[it]);
...
#pragma unroll
        for (int jt = 0; jt < VecSize; jt++) {
          ...
          T tmp =
              (static_cast<T>(static_cast<float>(x_int32[it][jt]) *
                              quant_out_scale[it][jt]) +
               bias[it][jt]) *
                  // T tmp = (static_cast<T>(static_cast<float>(x_int32[it][jt])
                  // * 1.0f) + bias[it][jt]) *
                  static_cast<T>(mask_vec[it][jt]) * factor +
              residual[it][jt];
...
#pragma unroll
        for (int jt = 0; jt < VecSize; jt++) {
          ...
          T tmp = static_cast<T>(static_cast<float>(x_int32[it][jt]) *
                      ...
                      static_cast<T>(mask_vec[it][jt]) * factor +
                  // T tmp = static_cast<T>(static_cast<float>(x_int32[it][jt])
                  // * 1.0f) * static_cast<T>(mask_vec[it][jt]) * factor +
                  residual[it][jt];
          x[it][jt] = tmp;
          xf[it * VecSize + jt] = U(x[it][jt]);
        }
...
#pragma unroll
    ...
    Vec_int8
        x_int8[LDGS];  // = static_cast<phi::AlignedVector<int8_t, VecSize>>(x);
...
#pragma unroll
      for (int jt = 0; jt < VecSize; jt++) {
        ...
        x_int8[it][jt] =
            __float2int_rn(static_cast<float>(x[it][jt]) * quant_in_scale_data);
        // printf("\n");
        // printf("after calculate x_int8[it][jt] = %d\t",
        // static_cast<int32_t>(x_int8[it][jt])); printf("\n");
      }
    }

#pragma unroll
    for (int it = 0, col = c; it < LDGS; it++) {
      // store x_int8[it] into y_ptr, 32 % vecsize == 0 to avoid the inner loop
      // in VecSize. 32 % vecsize == 0 for jt col loop:
      phi::Store<int8_t, VecSize>(x_int8[it],
