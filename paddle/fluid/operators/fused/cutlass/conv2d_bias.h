...
cutlass::Status cutlass_nhwc_conv2d_bias(CONV_PARAMS0) {
...
void cutlass_conv2d_bias(const half *input,
                       const half *weight,
                       const half *bias,
                       half *output,
                       int batch,
 ...
 if (map_problem_conv2d_bias.count(problem_size)) {
    cutlass_conv2d_bias_all_func[map_problem_conv2d_bias.at(problem_size)](CONV_ARGS0);
    return;
 }
