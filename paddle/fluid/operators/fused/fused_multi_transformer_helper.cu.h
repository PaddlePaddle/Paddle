...
class AttnQKVHelper {
 public:
  AttnQKVHelper(const phi::GPUContext &dev_ctx,
                phi::DenseTensor &workspace,          // NOLINT
                phi::DenseTensor &tmp_quant_space,    // NOLINT
                phi::DenseTensor &tmp_dequant_space,  // NOLINT
                const float epsilon,
                const int token_num,    // m
                const int input_size,   // k
      ...
      : dev_ctx_(dev_ctx),
        ...
        pre_layer_norm_(pre_layer_norm),
        quant_round_type_(quant_round_type),
        quant_min_bound_(quant_min_bound),
        quant_max_bound_(quant_max_bound),
        is_dybatch_(is_dybatch),
        workspace_(workspace),
        tmp_quant_space_(tmp_quant_space),
        tmp_dequant_space_(tmp_dequant_space),
        is_decoder_(is_decoder) {
    int8_gemm_helper_ = std::make_unique<Int8GEMMHelper<T>>(dev_ctx_,
                                                            token_num,
                                                            input_size,
                                                            output_size,
                                                            workspace,
    ...
    gemm_helper_ = std::make_unique<LtGEMMHelper<T>>(
        dev_ctx_, token_num, input_size, output_size, transpose_qkvw);
  }
  ...
  void Compute(const phi::DenseTensor &input,
               ...
               const phi::DenseTensor &dequant_out_scales,
               const int layer_id,
               const float qkv_in_scale,
               phi::DenseTensor *output) {
    // using U = LayerNormParamType<T>;
    using U = typename PDDataTypeTraits<T>::DataType;

    phi::DenseTensor ln_out;
    if (layer_id == 0 && pre_layer_norm_) {
      auto *ln_scale_data = reinterpret_cast<const U *>(ln_scale.data<T>());
      auto *ln_bias_data = reinterpret_cast<const U *>(ln_bias.data<T>());
      ln_out.Resize(input.dims());

      PureLoad<U> load(reinterpret_cast<const U *>(input.data<T>()),
                       input_size_);
      if (qkv_in_scale > 0) {
        dev_ctx_.Alloc<int8_t>(&ln_out);
        VLOG(5) << "ln_scale_data " << ln_scale;

        AffineQuantStore<int8_t, U, U, U, true, true> store(
            ln_out.data<int8_t>(),
            input_size_,
            ln_scale_data,
            ln_bias_data,
        ...
        dev_ctx_.Alloc<T>(&ln_out);
        AffineStore<U, U, U, true, true> store(
            reinterpret_cast<U *>(ln_out.data<T>()),
            input_size_,
            ln_scale_data,
            ln_bias_data);
        DispatchLayerNorm<decltype(load), decltype(store), U>(
    ...
    if (qkv_in_scale > 0) {
      if (!is_dybatch_) {
        if (is_decoder_) {
          dev_ctx_.Alloc<int32_t>(output);
        } else {
          dev_ctx_.Alloc<T>(output);
        }
      }
      int8_gemm_helper_->Compute(
          &ln_out,
          &weight,  // int8, Need be transposed
          &dequant_out_scales,
          qkv_in_scale,                         // not used
          output,                               // int32 or T
          false,                                //  no need to quant
          is_dybatch_ ? false : !is_decoder_);  // dequant in mmha in deocder

    } else {
      dev_ctx_.Alloc<T>(output);
      gemm_helper_->Compute(&ln_out, &weight, output);
    }
  }

 private:
  const phi::GPUContext &dev_ctx_;
  bool pre_layer_norm_ = true;
  bool is_decoder_;
  bool is_dybatch_;
  float epsilon_;
  int token_num_;    // m
...
template <typename T>
class StaticInt8FFNHelper {
 public:
  StaticInt8FFNHelper(const phi::GPUContext &dev_ctx,
                      phi::DenseTensor &workspace,          // NOLINT
                      phi::DenseTensor &tmp_out,            // NOLINT
                      phi::DenseTensor &tmp_quant_space,    // NOLINT
                      phi::DenseTensor &tmp_ffn_out,        // NOLINT
                      phi::DenseTensor &tmp_dequant_space,  // NOLINT
                      phi::DenseTensor &tmp_ffn_act_out,    // NOLINT
                      int token_num,
                      int dim_embed,
                      int dim_ffn,
                      ...
                      float quant_max_bound = 127.0f,
                      float quant_min_bound = -127.0f,
                      bool is_dybatch = false)
      : dev_ctx_(dev_ctx),
        token_num_(token_num),
        dim_embed_(dim_embed),
        ...
        quant_min_bound_(quant_min_bound),
        workspace_(workspace),
        tmp_dim_ffn_(dim_ffn_),
        tmp_out_(tmp_out),
        tmp_quant_space_(tmp_quant_space),
        tmp_ffn_out_(tmp_ffn_out),
        tmp_dequant_space_(tmp_dequant_space),
        tmp_ffn_act_out_(tmp_ffn_act_out),
        is_dybatch_(is_dybatch) {
    if (act_method == "geglu" || act_method == "swiglu") {
      tmp_dim_ffn_ = dim_ffn / 2;
    }

    ffn1_int8_gemm_helper_ =
        std::make_unique<Int8GEMMHelper<T>>(dev_ctx_,
                                            token_num,
                                            dim_embed,
                                            dim_ffn,
                                            workspace,
                                            ...
                                            quant_max_bound,
                                            quant_min_bound);
    ffn1_gemm_helper_ = std::make_unique<LtGEMMHelper<T>>(
        dev_ctx_, token_num, dim_embed, dim_ffn, false);

    ffn2_int8_gemm_helper_ = std::make_unique<Int8GEMMHelper<T>>(
        dev_ctx_,
        token_num,
        tmp_dim_ffn_,
        dim_embed,
        ...
        quant_min_bound,
        is_decoder && FLAGS_use_gemm_dequant /*use_gemm_dequant*/);
    ffn2_gemm_helper_ = std::make_unique<LtGEMMHelper<T>>(
        dev_ctx_, token_num, tmp_dim_ffn_, dim_embed, false);
  }

  void Compute(const phi::DenseTensor &input,
               const phi::DenseTensor &ffn1_weight,
               const phi::DenseTensor &ffn2_weight,
               const phi::DenseTensor &ffn_ln_scale,
               const phi::DenseTensor &ffn_ln_bias,
               const phi::DenseTensor &outlinear_bias,
               const phi::DenseTensor &ffn1_bias,
               const phi::DenseTensor &residual,
               const phi::DenseTensor &ffn1_out_scales,
               const phi::DenseTensor &ffn2_out_scales,
               const phi::DenseTensor *ffn2_shift,
               const phi::DenseTensor *ffn2_smooth,
               float ffn1_in_scale,
               float ffn2_in_scale,
               phi::DenseTensor *output,
               phi::DenseTensor *residual_out) {
    using LayerNormComputeType = typename PDDataTypeTraits<T>::DataType;

    // 1. Bias residual layernorm
    auto *ln_scale_data =
        reinterpret_cast<const LayerNormComputeType *>(ffn_ln_scale.data<T>());
    auto *ln_bias_data =
        reinterpret_cast<const LayerNormComputeType *>(ffn_ln_bias.data<T>());

    phi::DenseTensor ln_out;

    SkipLoadAndStoreResidual<LayerNormComputeType> load(
        reinterpret_cast<const LayerNormComputeType *>(input.data<T>()),
        reinterpret_cast<const LayerNormComputeType *>(
            outlinear_bias.data<T>()),
        reinterpret_cast<const LayerNormComputeType *>(residual.data<T>()),
        reinterpret_cast<LayerNormComputeType *>(residual_out->data<T>()),
        0.0f,
        dim_embed_);
    if (pre_layer_norm_) {
      if (ffn1_in_scale > 0) {
        ln_out = tmp_quant_space_;
        AffineQuantStore<int8_t,
                         LayerNormComputeType,
                         LayerNormComputeType,
                         LayerNormComputeType,
                         true,
                         true>
            store(ln_out.data<int8_t>(),
                  dim_embed_,
                  ln_scale_data,
                  ln_bias_data,
                  ffn1_in_scale,
        ...
        DispatchLayerNorm<decltype(load),
                          decltype(store),
                          LayerNormComputeType>(dev_ctx_.stream(),
                                                load,
                                                store,
                                                token_num_,
                                                dim_embed_,
        ...
        ln_out = tmp_out_;
        AffineStore<LayerNormComputeType,
                    LayerNormComputeType,
                    LayerNormComputeType,
                    true,
                    true>
            store(reinterpret_cast<LayerNormComputeType *>(ln_out.data<T>()),
                  dim_embed_,
                  ln_scale_data,
                  ln_bias_data);
        DispatchLayerNorm<decltype(load),
      ...
      PADDLE_THROW(platform::errors::Unimplemented(
          "Static Int8 FFN Helper currently does not support post layer norm"));
    }
    ...
    if (ffn1_in_scale > 0) {
      ffn1_out = tmp_dequant_space_;
      ffn1_int8_gemm_helper_->Compute(&ln_out,       // int8
                                      &ffn1_weight,  // int8, Need be transposed
                                      &ffn1_out_scales,
                                      ffn1_in_scale,
                                      &ffn1_out,  // int32
                                      false,      // no need to quant
                                      false);     // no need to dequant
    } else {
      ffn1_out = tmp_ffn_out_;
      ffn1_gemm_helper_->Compute(&ln_out, &ffn1_weight, &ffn1_out);
    }

    BiasActHelper<T> bias_act_helper(
        dev_ctx_, act_method_, token_num_, dim_ffn_);

    phi::DenseTensor act_out =
        ffn2_in_scale > 0 ? tmp_quant_space_ : tmp_ffn_act_out_;

    bias_act_helper.Compute(&ffn1_out,
                            &ffn1_bias,
                            ffn1_in_scale > 0 ? &ffn1_out_scales : nullptr,
                            ffn2_shift,
                            ffn2_smooth,
                            ffn2_in_scale,
                            quant_round_type_,
    ...
    if (ffn2_in_scale > 0) {
      ffn2_int8_gemm_helper_->Compute(
          &act_out,      // int8
          &ffn2_weight,  // int8, Need be transposed
          ffn2_in_scale > 0 ? &ffn2_out_scales : nullptr,
          ffn2_in_scale,
          output,  // T
          false,   //  no need to quant
          true);   // need to dequant cause allreduce
    } else {
      ffn2_gemm_helper_->Compute(&act_out, &ffn2_weight, output);
    }
    if (nccl_comm_) {
      *output = nccl_comm_->AllReduce();
    } else {
 ...
 private:
  ...
  phi::DenseTensor &tmp_out_;            // T
  phi::DenseTensor &tmp_quant_space_;    // int8_t
  phi::DenseTensor &tmp_ffn_out_;        // T
  phi::DenseTensor &tmp_dequant_space_;  // int32_t
  phi::DenseTensor &tmp_ffn_act_out_;
  std::unique_ptr<Int8GEMMHelper<T>> ffn1_int8_gemm_helper_,
      ffn2_int8_gemm_helper_;
  std::unique_ptr<LtGEMMHelper<T>> ffn1_gemm_helper_, ffn2_gemm_helper_;
};
