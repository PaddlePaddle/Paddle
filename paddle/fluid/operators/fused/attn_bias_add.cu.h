...
__global__ void BroadcastKernelBinary(
  ...
  // load in0
  if (use_broadcast[0]) {
    pten::kps::ReadDataBc<InT, VecSize, DATA_PER_THREAD, 1, ShapeSize>(
        arg0, in0, fix, configlists[0], numel);
  } else {
    pten::kps::ReadData<InT, VecSize, 1, 1>(arg0, in0 + fix, num);
  }
  // load in1
  if (use_broadcast[1]) {
    pten::kps::ReadDataBc<InT, VecSize, DATA_PER_THREAD, 1, ShapeSize>(
    ...
    pten::kps::ReadData<InT, VecSize, 1, 1>(arg1, in1 + fix, num);
  }
  // compute
  pten::kps::ElementwiseBinary<InT, OutT, VecSize, 1, 1, Functor>(result, arg0,
                                                                  arg1, func);
  // store
  pten::kps::WriteData<OutT, VecSize, 1, 1, true>(out + fix, result, num);
}
