...
static framework::DDim GetBitmaskDims(std::vector<int> out_shape) {
  ...
  return pten::make_ddim(bitmask_shape);
}
 ...
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const {
    ...
    std::vector<int64_t> bn_param_shape =
        pten::vectorize(ctx->GetInputDim("ScaleX"));
    if (1 == bn_param_shape.size()) {
      bn_param_shape = {1, 1, 1, bn_param_shape[0]};
    }
    framework::DDim bn_param_dims = pten::make_ddim(bn_param_shape);
    ...
    auto y_dims = pten::make_ddim(out_shape);
