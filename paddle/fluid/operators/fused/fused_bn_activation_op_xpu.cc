...
template <typename T>
static double _compute_mean(const std::vector<T>& x) {
  double sum = 0;
  int length = x.size();
...
static double memory_checksum(const float* xpu_data_ptr, int len) {
  std::vector<float> cpu_data(len);
  xpu_wait();
  int ret = xpu_memcpy(static_cast<void*>(cpu_data.data()),
                       static_cast<void*>(const_cast<float*>(xpu_data_ptr)),
                       len * sizeof(float), XPU_DEVICE_TO_HOST);
  PADDLE_ENFORCE_EQ(ret, XPU_SUCCESS, "xpu_memcpy failed!");
  return _compute_mean(cpu_data);
}
