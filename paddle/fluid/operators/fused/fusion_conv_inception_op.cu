...
class CUDNNConvInceptionFusionOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    std::vector<int> in_dim = pten::vectorize<int>(input->dims());
    ...
    cudnnTensorDescriptor_t cudnn_input_desc =
        input_desc.descriptor<T>(layout, pten::vectorize<int>(input->dims()));
    cudnnTensorDescriptor_t pool_out_desc = out_pool_desc.descriptor<T>(
        layout, pten::vectorize<int>(input->dims()));
    ...
    for (int i = 0; i < 4; ++i) {
      filter_dims.push_back(pten::vectorize<int>(filters[i]->dims()));
    ...
    T* temp2_data = temp_outs[1]->mutable_data<T>(pten::make_ddim(out_dims[2]),
                                                  ctx.GetPlace());
