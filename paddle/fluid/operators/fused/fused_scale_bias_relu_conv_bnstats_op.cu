...
template <typename T>
void _FusedScaleBiasReluConvBnstatsImpl(const framework::ExecutionContext& ctx,
                                        const Tensor* input,
                                        const Tensor* filter,
                                        const Tensor* scale,
                                        const Tensor* bias,
...
template <typename T>
void _BNFinalizeImpl(const framework::ExecutionContext& ctx,
                     const Tensor* sum_tensor,
                     const Tensor* sqsum_tensor,
                     const Tensor* scale_tensor,
                     const Tensor* bias_tensor,
                     const Tensor* input_running_mean_tensor,
                     const Tensor* input_running_var_tensor,
  ...
  data_ptrs.push_back(const_cast<U*>(sum_tensor->data<U>()));
  uids.push_back(uid);

  auto sqsum_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(const_cast<U*>(sqsum_tensor->data<U>()));
  uids.push_back(uid);

  auto scale_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(const_cast<U*>(scale_tensor->data<U>()));
  uids.push_back(uid);

  auto bias_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(const_cast<U*>(bias_tensor->data<U>()));
  uids.push_back(uid);

  auto input_running_mean_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(const_cast<U*>(input_running_mean_tensor->data<U>()));
  uids.push_back(uid);

  auto input_running_var_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(const_cast<U*>(input_running_var_tensor->data<U>()));
  uids.push_back(uid);

  // outputs
  auto updated_running_mean_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(updated_running_mean_tensor->data<U>());
  uids.push_back(uid);

  auto updated_running_var_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(updated_running_var_tensor->data<U>());
  uids.push_back(uid);

  auto saved_mean_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(saved_mean_tensor->data<U>());
  uids.push_back(uid);

  auto saved_inv_var_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format_bn);
  data_ptrs.push_back(saved_inv_var_tensor->data<U>());
  uids.push_back(uid);

  auto eq_scale_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format);
  data_ptrs.push_back(eq_scale_tensor->data<T>());
  uids.push_back(uid);

  auto eq_bias_desc = helper::GetGeneralTensorDescriptor(
      dim_c, layout_format, ++uid, 16, tensor_format);
  data_ptrs.push_back(eq_bias_tensor->data<T>());
  uids.push_back(uid);
...
class FusedScaleBiasReluConvBnstatsOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto* bn_scale = ctx.Input<Tensor>("BN_Scale");
    auto* bn_bias = ctx.Input<Tensor>("BN_Bias");
    ...
    if (fuse_prologue) {
      scale = ctx.Input<Tensor>("Scale");
      bias = ctx.Input<Tensor>("Bias");
    }
    // output variables
    ...
    updated_running_mean_tensor->mutable_data<U>(ctx.GetPlace());
    updated_running_var_tensor->mutable_data<U>(ctx.GetPlace());
    saved_mean_tensor->mutable_data<U>(ctx.GetPlace());
    saved_inv_var_tensor->mutable_data<U>(ctx.GetPlace());
    eq_scale_tensor->mutable_data<T>(ctx.GetPlace());
    eq_bias_tensor->mutable_data<T>(ctx.GetPlace());
    ...
    Tensor sqsum_tensor(bn_scale->dtype());
    sum_tensor.Resize(bn_dims);
    sqsum_tensor.Resize(bn_dims);
    sum_tensor.mutable_data<U>(ctx.GetPlace());
    sqsum_tensor.mutable_data<U>(ctx.GetPlace());
    _FusedScaleBiasReluConvBnstatsImpl<T>(ctx,
                                          input,
                                          filter,
                                          scale,
                                          bias,
                                          ...
                                          &sqsum_tensor);
    // Step 2: BN Finalize
    _BNFinalizeImpl<T>(ctx,
                       &sum_tensor,
                       &sqsum_tensor,
                       bn_scale,
                       bn_bias,
                       input_running_mean_tensor,
                       input_running_var_tensor,
                       accumulation_count,
                       exp_decay,
                       epsilon,
