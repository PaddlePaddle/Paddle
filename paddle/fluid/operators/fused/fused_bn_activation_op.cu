...
class FusedBatchNormActKernel<platform::CUDADeviceContext, T>
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    T *y_data = y->template mutable_data<T>(dev_ctx.GetPlace());
    ...
    if (data_layout == DataLayout::kNHWC) {
      ...
      PADDLE_ENFORCE_EQ(
          act_type, "relu",
          phi::errors::InvalidArgument("Only bn + relu fusion is supported."));
    }
    ...
    if (reserve_space_size == 0 && !fuse_bn_act) {
      if (std::is_same<T, float>::value) {
        bool success = TryLaunchFusedNCHWFP32BNTrainingKernel(
            dev_ctx, reinterpret_cast<const float *>(x_data), nullptr,
    ...
    workspace_ptr = workspace_tensor.mutable_data<uint8_t>(ctx.GetPlace());
    PADDLE_ENFORCE_GPU_SUCCESS(
        platform::dynload::cudnnBatchNormalizationForwardTrainingEx(
            ...
            saved_mean_data, saved_variance_data, act_desc_for_bn_api,
            workspace_ptr, workspace_size, reserve_space_ptr,
            reserve_space_size));

    if (!fuse_bn_act) {
      LaunchMaskedAddReluFwdKernel<T>(dev_ctx, y_data, nullptr, y_data,
                                      mask_ptr, numel);
    }
  }
};
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    if (data_layout == DataLayout::kNHWC) {
      ...
      tmp_buffer.Resize({numel});
      auto *tmp_grad = tmp_buffer.mutable_data<T>(ctx.GetPlace());
      const auto *mask = ctx.Input<Tensor>("Mask");
      LaunchMaskedReluBwdKernel<T>(dev_ctx, dy_data, mask->data<uint8_t>(),
                                   tmp_grad, numel);
