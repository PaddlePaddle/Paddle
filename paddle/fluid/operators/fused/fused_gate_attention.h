...
class FMHAGateRef {
 public:
  ...
  void ComputeQKVTransposeBackward(const Tensor& q_transpose_out_grad,
                                   ...
                                   Tensor* v_out_grad) {
    std::vector<int> perm = {0, 1, 3, 2, 4};
    TransposeGPUKernelDriver<T>(dev_ctx_, q_transpose_out_grad, perm,
                                q_out_grad);
    TransposeGPUKernelDriver<T>(dev_ctx_, k_transpose_out_grad, perm,
                                k_out_grad);
    TransposeGPUKernelDriver<T>(dev_ctx_, v_transpose_out_grad, perm,
                                v_out_grad);
  }
  ...
  void ComputeQKVTransposeBackward(const Tensor& qkv_transpose_out_grad,
    ...
    TransposeGPUKernelDriver<T>(dev_ctx_, qkv_transpose_out_grad, perm,
                                qkv_out_grad);
  }
