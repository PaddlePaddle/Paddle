...
class FusedMultiTransformerINT8OpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    //     // dequant output scales, tensor, size = [num_layers, n], n is gemm
    //     output
    //     // size
    //     auto qkv_out_scales =
    //     ctx.MultiInput<phi::DenseTensor>("QKVOutScale"); auto
    //     out_linear_out_scales =
    //         ctx.MultiInput<phi::DenseTensor>("OutLinearOutScale");
    //     auto ffn1_out_scales =
    //     ctx.MultiInput<phi::DenseTensor>("FFN1OutScale"); auto
    //     ffn2_out_scales = ctx.MultiInput<phi::DenseTensor>("FFN2OutScale");
    ...
    //     auto ln_compute =
    //         AttnLayerNorm<T, T, int8_t>(dev_ctx, epsilon, bsz_seq,
    //         dim_embed);
    ...
    //     auto fmha_compute =
    //         FMHARef<T>(dev_ctx, bsz, seq_len, num_head, dim_head,
    //         attn_param);
    ...
    //                         0,
    //                         platform::errors::PreconditionNotMet(
    //                             "The value of time_step must > 0, but now is
    //                             %d", time_step_value));
    //       PADDLE_ENFORCE_EQ(
    //           seq_len,
    //           1,
    //           platform::errors::PreconditionNotMet(
    //               "In decode stage, the seq_len of input must be 1, but now
    //               is %d", seq_len));
    ...
    //         dev_ctx.Alloc<T>(&transpose_out_2, transpose_out_2.numel() *
    //         sizeof(T));
    ...
    //     attn_dropout_mask_out.Resize({{bsz, num_head, seq_len,
    //     out_seq_len}}); auto *attn_dropout_mask_out_data = dev_ctx.Alloc<T>(
    //         &attn_dropout_mask_out, attn_dropout_mask_out.numel() *
    //         sizeof(T));
    ...
    //     // 4. out_linear
    //     auto out_linear_weights =
    //     ctx.MultiInput<phi::DenseTensor>("OutLinearW"); auto
    //     out_linear_biases =
    ...
    //                            bias_dropout_residual_out.numel() *
    //                            sizeof(T));
    //     }
    ...
    //     // 7. ffn act + bias
    //     DropoutParam ffn1_dropout_param(true, 0, true, true, 0.0, nullptr,
    //     0); FusedDropoutHelper<T, uint8_t, int32_t, int8_t>
    //     fused_act_dropout_helper(
    //         dev_ctx, bsz_seq, dim_ffn, ffn1_dropout_param);
    //     FusedDropoutHelper<T, uint8_t>
    ...
    //     DropoutParam ffn2_dropout_param(true, 0, true, true, 0.0, nullptr,
    //     0); FusedDropoutLayerNormHelper<T, uint8_t, int32_t, int8_t>
    ...
    //     cublaslt_workspace;
    //     // for input and output transform data is CUBLASLT_ORDER_COL32
    //     format, int m_max = bsz_seq, k_max = std::max(dim_embed, dim_ffn),
    ...
    //       phi::DenseTensor *cache_kv_out = cache_kv ? cache_kv_outs[i] :
    //       nullptr;
