...
class ResNetUnitKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    auto input_x_shape = pten::vectorize<int>(input_x->dims());
    auto filter_x_shape = pten::vectorize<int>(filter_x->dims());
    ...
    if (has_shortcut) {
      ...
      auto input_z_shape = pten::vectorize<int>(input_z->dims());
      auto filter_z_shape = pten::vectorize<int>(filter_z->dims());
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    if (has_shortcut) {
      ...
      auto z_shape = pten::vectorize<int>(z->dims());
      auto filter_z_shape = pten::vectorize<int>(filter_z->dims());
