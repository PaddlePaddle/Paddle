...
class CustomFMHAXPUKernel : public framework::OpKernel<T> {
 public:
  ...
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    const phi::DenseTensor* cu_seq_len = ctx.Input<phi::DenseTensor>("CuSeqLen");
    const phi::DenseTensor* host_seq_len = ctx.Input<phi::DenseTensor>("HostSeqLen");
    // output
    phi::DenseTensor* s_out = ctx.Output<phi::DenseTensor>("SOut");
    phi::DenseTensor* dropout_mask = ctx.Output<phi::DenseTensor>("DropoutMask");
    ...
    const int* host_seq_len_ptr = reinterpret_cast<const int*>(host_seq_len->data<int>());
    const int* cu_seq_len_ptr = reinterpret_cast<const int*>(cu_seq_len->data<int>());

    XPUType* s_out_ptr = reinterpret_cast<XPUType*>(s_out->mutable_data<T>(ctx.GetPlace()));
    XPUType* dropout_mask_ptr = reinterpret_cast<XPUType*>(dropout_mask->mutable_data<T>(ctx.GetPlace()));
    XPUType* ctx_out_ptr = reinterpret_cast<XPUType*>(ctx_out->mutable_data<T>(ctx.GetPlace()));
    ...
    xpu::VectorParam<int> lod = {host_seq_len_ptr, batch_size + 1, const_cast<int*>(cu_seq_len_ptr)};
    ...
    int r = xpu::mha_fusion<XPUType, int16_t>(xpu_ctx,
                qkv_ptr, s_out_ptr, dropout_mask_ptr, ctx_out_ptr,
                batch_size, num_heads, head_size,
                lod, lod, drop_p, is_test);
...
class CustomFMHAGradXPUKernel : public framework::OpKernel<T> {
 public:
  ...
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    const phi::DenseTensor* dropout_mask = ctx.Input<phi::DenseTensor>("DropoutMask");
    ...
    const int* host_seq_len_ptr = reinterpret_cast<const int*>(host_seq_len->data<int>());
    const int* cu_seq_len_ptr = reinterpret_cast<const int*>(cu_seq_len->data<int>());
    const XPUType* s_out_ptr = reinterpret_cast<const XPUType*>(s_out->data<T>());
    const XPUType* dropout_mask_ptr = reinterpret_cast<const XPUType*>(dropout_mask->data<T>());
    const XPUType* d_ctx_out_ptr = reinterpret_cast<const XPUType*>(d_ctx_out->data<T>());

    XPUType* d_qkv_ptr = reinterpret_cast<XPUType*>(d_qkv->mutable_data<T>(ctx.GetPlace()));
    ...
    int r = xpu::mha_fusion_grad<XPUType, int16_t>(xpu_ctx,
                qkv_ptr, s_out_ptr, dropout_mask_ptr, const_cast<XPUType*>(d_ctx_out_ptr),
    ...
    r = xpu::concat<XPUType>(xpu_ctx,
                {d_q_ptr, d_k_ptr, d_v_ptr}, d_qkv_ptr,
                {dqkv_shape, dqkv_shape, dqkv_shape}, 1);
