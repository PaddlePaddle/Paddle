...
Tensor *ComputeOutputLinearBackward(const framework::ExecutionContext &ctx,
                                    const GateAttentionGradConfig<T> &config,
                                    const Tensor *fmha_out,
                                    Tensor *fmha_out_grad, bool has_gating) {
  const auto *out_grad = ctx.Input<Tensor>(framework::GradVarName("Out"));
  const auto *out_linear_weight = ctx.Input<Tensor>("OutLinearWeight");
  const auto *input = has_gating ? ctx.Input<Tensor>("GateOut") : fmha_out;
...
class FusedGateAttentionOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    // 2. FMHA
    auto fmha_compute = FMHAGateRef<T>(ctx.cuda_device_context(), merge_qkv);
...
class FusedGateAttentionGradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    const auto *query_out = ctx.Input<Tensor>("QueryOut");
    const auto *key_out = ctx.Input<Tensor>("KeyOut");
    const auto *value_out = ctx.Input<Tensor>("ValueOut");
    const auto *qkv_out = ctx.Input<Tensor>("QKVOut");

    // backward output
    auto *query_grad = ctx.Output<Tensor>(framework::GradVarName("Query"));
    auto *qktv_out_grad = ctx.Output<Tensor>(framework::GradVarName("QKTVOut"));
    auto *softmax_out_grad =
        ctx.Output<Tensor>(framework::GradVarName("SoftmaxOut"));
    auto *qkv_out_grad = ctx.Output<Tensor>(framework::GradVarName("QKVOut"));
    auto *query_out_grad =
        ctx.Output<Tensor>(framework::GradVarName("QueryOut"));
    auto *key_out_grad = ctx.Output<Tensor>(framework::GradVarName("KeyOut"));
    auto *value_out_grad =
        ctx.Output<Tensor>(framework::GradVarName("ValueOut"));

    query_grad->mutable_data<T>(ctx.GetPlace());
    qktv_out_grad->mutable_data<T>(ctx.GetPlace());
    ...
    Tensor *fmha_out_grad = config.GetFMHAOutGrad(ctx.cuda_device_context());
    Tensor *fhma_or_gate_out_grad = ComputeOutputLinearBackward<T>(
        ctx, config, fmha_out, fmha_out_grad, has_gating);
    ...
    if (merge_qkv) {
      qkv_out_grad->mutable_data<T>(ctx.GetPlace());
    } else {
      query_out_grad->mutable_data<T>(ctx.GetPlace());
      key_out_grad->mutable_data<T>(ctx.GetPlace());
      value_out_grad->mutable_data<T>(ctx.GetPlace());
    }

    // 3. Gradient of FMHA
    fmha_compute.ComputeBackward(
        query_out, key_out, value_out, qkv_out, softmax_out, fmha_out_grad,
        qktv_out_grad, softmax_out_grad, nullptr, nonbatched_bias_grad,
        query_out_grad, key_out_grad, value_out_grad, qkv_out_grad, &config);
