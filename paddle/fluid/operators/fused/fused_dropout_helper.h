 ...
 public:
  ...
  void ResidualDropoutBias(const platform::CUDADeviceContext& ctx, const T* src,
    ...
    void DropoutActBiasGrad(const platform::CUDADeviceContext& ctx,
                            const T* dout, const T* src, const T* bias,
                            const MaskType* mask, T* d_src, T* d_bias,
      ...
      if (act_method == "gelu") {
        ...
        LaunchDropoutActBiasGrad<T, MaskType, GeluGradFunctor<T>>(
            ...
            dropout_param_.is_upscale_in_train, rows_, cols_, d_src, d_bias,
            ctx);
      } else if (act_method == "relu") {
  ...
  class FusedDropoutLayerNormHelper : public FusedDropoutHelper<T, MaskType> {
   public:
    ...
    void LayernormResidualDropoutBias(
        ...
        const LayerNormParamType<T>* beta, T* dropout_out, MaskType* mask,
        T* out, LayerNormParamType<T>* mean, LayerNormParamType<T>* variance) {
      using U = LayerNormParamType<T>;
      ...
      LaunchLayernormResidualDropoutBias<T, MaskType>(
          ...
          this->dropout_param_.is_test, src, residual, bias, gamma, beta, mask,
          dropout_out, out, mean, variance, ctx);
    }
