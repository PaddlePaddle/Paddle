...
class CosSimOp : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    PADDLE_ENFORCE_EQ(true, ctx->HasOutput("XNorm"),
                      platform::errors::InvalidArgument(
                          "Output(XNorm) of CosSimOp should not be null."));
    PADDLE_ENFORCE_EQ(true, ctx->HasOutput("YNorm"),
                      platform::errors::InvalidArgument(
...
class CosSimOpGrad : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    PADDLE_ENFORCE_EQ(
        true, ctx->HasInput("Y"),
        platform::errors::InvalidArgument("Input(Y) must not be null."));
    PADDLE_ENFORCE_EQ(
        true, ctx->HasInput("XNorm"),
        platform::errors::InvalidArgument("Input(XNorm) must not be null."));
    PADDLE_ENFORCE_EQ(
        true, ctx->HasInput("YNorm"),
        platform::errors::InvalidArgument("Input(YNorm) must not be null."));
    PADDLE_ENFORCE_EQ(
        true, ctx->HasInput("Out"),
        platform::errors::InvalidArgument("Input(Out) must not be null."));
    PADDLE_ENFORCE_EQ(
        true, ctx->HasInput(framework::GradVarName("Out")),
