    ...
    if (check) {
      ...
      PADDLE_ENFORCE_GE(
          ...
          platform::errors::InvalidArgument(
              ...
              x_dims.size()));
      PADDLE_ENFORCE_EQ(
          pten::slice_ddim(x_dims, 1, x_dims.size()),
          pten::slice_ddim(y_dims, 1, y_dims.size()),
          platform::errors::InvalidArgument(
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    PADDLE_ENFORCE_EQ(
        ...
        platform::errors::InvalidArgument(
            ...
            " just 1 (which will be broadcasted to match Input(X)).",
            y_dims[0], x_dims[0]));
    auto target_xnorm_dims = pten::make_ddim({x_dims[0], 1});
    auto target_ynorm_dims = pten::make_ddim({y_dims[0], 1});
    PADDLE_ENFORCE_EQ(
