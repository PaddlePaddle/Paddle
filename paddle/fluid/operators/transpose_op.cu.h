...
struct NarrowDims2TransposeDispatch {
  static void DoTranspose(const pten::GPUContext& d, int tile_size_i,
...
void SwapDim1And2InNarrow(const pten::GPUContext& d, const T* input,
                          const Dim3& input_dims, T* output,
...
template <typename T>
void SendSwapDim1And2InTranspose(const pten::GPUContext& d, const T* input,
                                 const Dim3& input_dims, T* output) {
...
struct TransposeSimple {
  static bool run(const pten::GPUContext& ctx, const Tensor& in,
...
template <typename T>
void TransposeGPUKernelDriver(const pten::GPUContext& dev_ctx, const int ndims,
                              const Tensor& in, const std::vector<int32_t> perm,
                              Tensor* out) {
  auto ret = TransposeSimple<T>::run(dev_ctx, in, perm, out);
  if (!ret) {
    TransCompute<pten::GPUContext, T>(ndims, dev_ctx, in, out, perm);
  }
}
