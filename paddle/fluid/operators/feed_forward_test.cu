...
void GetElementwiseAddOpGrad(const std::vector<T> &dout_vec, const int bsz_seq,
  ...
  int axis = -1;
  bool use_mkldnn = false;
  std::string x_data_format = "";
  std::string y_data_format = "";
  bool use_quantizer = false;
  std::string mkldnn_data_type = "float32";
  float Scale_x = 1.0;
  float Scale_y = 1.0;
...
class TestFeedForward {
 public:
  ...
  // src: [bs, seq_len, em_dim]
  // weight: [output_size, em_dim]
  void SetUp() {
    int size_src = bsz_seq_ * dim_embed_;
    int size_weight = dim_embed_ * output_size_;
    int size_output = bsz_seq_ * output_size_;
    int size_bias = output_size_;

    base_out_vec_.resize(size_output);
    base_bias_out_vec_.resize(size_output);

    base_dinput_vec_.resize(size_src);
    base_dweight_vec_.resize(size_weight);
    base_dbias_vec_.resize(size_bias);

    src_vec_.resize(size_src);
    weight_vec_.resize(size_weight);
    bias_vec_.resize(size_bias);

    doutput_vec_.resize(size_output);
  ...
  void BaselineForward() {
    ...
    float alpha = 1;
    // call gemm-nt: inputs are all row-major
    // (bsz_seq, input_size) x (out_size, input_size) = (bsz_seq, out_size)
  ...
  void CheckOut(const T diff, bool is_relative_atol = false) {
    ...
    std::vector<T> bias_out(size_output);
  ...
  void CheckGrad(const T diff, bool is_relative_atol = false) {
    ...
    std::vector<T> h_dinput(size_src);
    ...
    std::vector<T> h_dweight(size_weight);
    ...
    if (has_bias_) {
      std::vector<T> h_dbias(size_bias);
 ...
 private:
  ...
  // results of baseline.
  std::vector<T> base_out_vec_;
  std::vector<T> base_bias_out_vec_;
...
// pass
TEST(FeedForward, GPUFeedforwardSmallSize) {
  int batch_size = 1;
  int seq_len = 2;
  int num_head = 1;
  int dim_head = 4;
...
// pass
TEST(FeedForward, GPUFeedforwardSmallSizeFp16) {
  int batch_size = 1;
  int seq_len = 2;
  int num_head = 1;
  int dim_head = 4;
...
// pass
TEST(FeedForward, GPUFeedforwardSmallSizeBias) {
  int batch_size = 1;
  int seq_len = 2;
  int num_head = 1;
  int dim_head = 4;
...
// pass
TEST(FeedForward, GPUFeedforwardSmallSizeFp16Bias) {
  int batch_size = 1;
  int seq_len = 2;
  int num_head = 1;
  int dim_head = 1;
