...
TEST(op_debug_str, test_unknown_dtype) {
  ...
  auto x_tensor = scope.Var("X")->GetMutable<framework::Tensor>();
  x_tensor->Resize(dim);
  x_tensor->mutable_data<float>(place);

  auto y_tensor = scope.Var("Y")->GetMutable<framework::Tensor>();
  y_tensor->Resize(dim);
  y_tensor->mutable_data<float>(place);

  auto out_grad_tensor =
      scope.Var(framework::GradVarName("Out"))->GetMutable<framework::Tensor>();
  out_grad_tensor->Resize(dim);
  out_grad_tensor->mutable_data<float>(place);

  scope.Var(framework::GradVarName("X"))->GetMutable<framework::Tensor>();
  scope.Var(framework::GradVarName("Y"))->GetMutable<framework::Tensor>();
