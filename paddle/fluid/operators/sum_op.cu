...
#include "paddle/fluid/operators/sum_op_gpu_kernel.h"
    ...
    if (out_var->IsType<framework::LoDTensor>()) {
      ...
      if (!in_place) {
        if (in_num == 2 && in_vars[0]->IsType<framework::LoDTensor>() &&
          ...
          KeCompute(length);
          sum_gpu<T><<<grids, blocks, 0, stream>>>(
      ...
      if (!selectrow_index.empty()) {
        ...
        if (!sr_in_data.empty() && !out_data.empty()) {
          auto tmp_sr_in_array = platform::DeviceTemporaryAllocator::Instance()
                                     .Get(dev_ctx)
                                     .Allocate(sr_in_data.size() * sizeof(T *));
          ...
          auto tmp_out_array = platform::DeviceTemporaryAllocator::Instance()
                                   .Get(dev_ctx)
                                   .Allocate(out_data.size() * sizeof(T *));
          ...
          KeCompute(length);
          sum_gpu_sr<T><<<grids, blocks, 0, stream>>>(
