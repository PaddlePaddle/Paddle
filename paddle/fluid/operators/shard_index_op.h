 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    for (int64_t i = 0; i < numel; ++i) {
      PADDLE_ENFORCE(in_data[i] >= 0 && in_data[i] < indice_num,
                     "Input indice(%d) is out of range [0,%d)", in_data[i],
                     indice_num);
