...
__global__ void KeNearestNeighborInterpFw(
  ...
  for (; tid < nthreads; tid += stride) {
    int out_id_h = tid / output_w;  // n
    int out_id_w = tid % output_w;  // cdhw
  ...
  if ("nearest" == interp_method) {
    ...
    KeNearestNeighborInterpFw<
        T><<<config.block_per_grid, config.thread_per_block, 0,
             ctx.cuda_device_context().stream()>>>(
        input_data, in_d, in_h, in_w, n, in_chw, output_data, out_d, out_h,
        out_w, n, out_chw, c, ratio_d, ratio_h, ratio_w, align_corners,
        data_layout);
...
static void Interpolate2DCUDABwd(const framework::ExecutionContext& ctx,
  ...
  if ("nearest" == interp_method) {
    ...
    KeNearestNeighborInterpBw<
        T><<<config.block_per_grid, config.thread_per_block, 0,
             ctx.cuda_device_context().stream()>>>(
        input_grad_data, in_d, in_h, in_w, n, in_chw, output_grad_data, out_d,
        out_h, out_w, n, out_chw, c, ratio_d, ratio_h, ratio_w, align_corners,
        data_layout);
  } else if ("bilinear" == interp_method) {
