...
class DetectionMAPOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    ...
    AddInput("HasState",
             "(phi::DenseTensor<int>) A tensor with shape [1], 0 means "
             "ignoring input "
             "states, which including PosCount, TruePos, FalsePos.")
        .AsDispensable();
    AddInput("PosCount",
             "(phi::DenseTensor) A tensor with shape [Ncls, 1], store the "
    ...
    AddOutput(
        "MAP",
        "(phi::DenseTensor) A tensor with shape [1], store the mAP evaluate "
