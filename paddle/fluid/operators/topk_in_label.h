...
class TopkinLabelKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    for (int i = 0; i < y_numel; i++) {
      mset.insert(y_data[i]);
    }

    auto x_numel = X->numel();
    for (int i = 0; i < x_numel; i++) {
      auto mtemp = mset.count(x_data[i]);
      out_data[i] = (mtemp>0 )? true:false;
    }
  }
};

template <typename DeviceContext, typename T>
class TopkinLabelGradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
