...
class BeamSearchFunctor<platform::NPUDeviceContext, T> {
 public:
  void operator()(const platform::NPUDeviceContext& ctx,
    ...
    selected_ids->mutable_data<int64_t>(pten::make_ddim({total_length, 1}),
                                        place);
    selected_scores->mutable_data<float>(pten::make_ddim({total_length, 1}),
                                         place);
    parent_idx->mutable_data<int64_t>(pten::make_ddim({total_length}), place);
    ...
    expand_pre_ids.Resize(pten::make_ddim({batch_size, seq_width}));
    ...
    expand_pre_scores.Resize(pten::make_ddim({batch_size, seq_width}));
    ...
    const auto& runner_fill_end_id =
        NpuOpRunner("FillD", {end_id_tmp_tensor}, {end_id_tensors},
                    {{"dims", pten::vectorize(ids_int32.dims())}});
    ...
    first_pos_false_tensors.Resize(pten::make_ddim({batch_size, 1}));
    ...
    if (seq_width > 1) {
      pos_tensors.Resize(pten::make_ddim({batch_size, seq_width}));
      ...
      second_pos_true_tensors.Resize(
          pten::make_ddim({batch_size, seq_width - 1}));
    ...
    const auto& runner_fill_ninf =
        NpuOpRunner("FillD", {ninf_tmp_tensor}, {ninf_tensors},
                    {{"dims", pten::vectorize(scores->dims())}});
    ...
    cal_scores.Resize(pten::make_ddim({num_seqs, real_beam_size * seq_width}));

    Tensor topk_scores(scores->dtype());
    topk_scores.Resize(
        pten::make_ddim({num_seqs, static_cast<int64_t>(beam_size)}));
    ...
    // real_beam_size * K]
    // real_beam_size = 1 or beam_size
    cal_ids.Resize(pten::make_ddim({num_seqs, real_beam_size * seq_width}));
    ...
    batch_ids.Resize(
        pten::make_ddim({num_seqs, static_cast<int64_t>(beam_size), 1}));
    ...
    gather_nd_score_indices.Resize(
        pten::make_ddim({num_seqs, static_cast<int64_t>(beam_size), 2}));
