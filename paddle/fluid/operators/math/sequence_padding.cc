...
void CopyValidData(framework::Tensor* dst_tensor,
                   ...
                   int pad_seq_len, int step_width, bool norm_by_len, bool size_average, bool length_average, int total_logits_len,
  ...
  for (int seq_idx = 0; seq_idx < seq_num; ++seq_idx) {
    ...
    if (length_average){
      ...
      VLOG(3) << "[warpctc grad][length_average]: scale " << scale << "total_logits_len " << total_logits_len;
    } else if (size_average){
      scale = 1.0f / static_cast<float>(seq_num);
      VLOG(3) << "[warpctc grad][size_average]: scale " << scale << "B " << seq_num;
    } else if (norm_by_len){
      scale = 1.0f / static_cast<float>(valid_seq_len);
      VLOG(3) << "[warpctc grad][norm_by_len]: scale " << scale << "T " << valid_seq_len;
    }
 ...
 public:
  void operator()(const platform::CPUDeviceContext& context,
    ...
    CopyValidData<T>(pad_tensor, &seq_tensor, seq_offsets, pad_seq_len,
                     step_width, norm_by_times, false, false, 0, kSeqToPad, layout);
  }
};
 ...
 public:
  void operator()(const platform::CPUDeviceContext& context,
    ...
    CopyValidData<T>(seq_tensor, &pad_tensor, seq_offsets, pad_seq_len,
                     step_width, norm_by_times, size_average, length_average, total_logits_len, kPadToSeq, layout);
  }
};
