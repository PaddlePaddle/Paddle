...
void TestNNZ(const std::vector<T>& dense_data, const int correct_nnz,
  ...
  auto dense_dims = pten::make_ddim({rows, cols});
  auto nnz_dims = pten::make_ddim({dense_dims[0] + 1});
...
void TestDenseToSparse(const std::vector<T>& correct_dense_data,
  ...
  auto nnz_dims = pten::make_ddim({correct_nnz});
  auto crows_dims = pten::make_ddim({rows + 1});
