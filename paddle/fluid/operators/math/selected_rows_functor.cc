...
struct SelectedRowsAdd<platform::CPUDeviceContext, T> {
  void operator()(const platform::CPUDeviceContext& context,
    ...
    memory::Copy(BOOST_GET(platform::CPUPlace, out_place), out_data,
                 BOOST_GET(platform::CPUPlace, in1_place), in1_data,
                 in1_value.numel() * sizeof(T));

    auto* in2_data = in2_value.data<T>();
    memory::Copy(BOOST_GET(platform::CPUPlace, out_place),
                 out_data + in1_value.numel(),
                 BOOST_GET(platform::CPUPlace, in2_place), in2_data,
...
struct SelectedRowsAddTo<platform::CPUDeviceContext, T> {
  void operator()(const platform::CPUDeviceContext& context,
    ...
    auto* in1_data = in1_value.data<T>();
    auto* in2_data = in2_value->data<T>();
    memory::Copy(BOOST_GET(platform::CPUPlace, in2_place),
    ...
    if (merged_row_set.size() == row_num && !sorted_result) {
      ...
      for (auto* in : inputs) {
        ...
        memory::Copy(BOOST_GET(platform::CPUPlace, out_place),
                     ...
                     BOOST_GET(platform::CPUPlace, in_place), in_data,
                     in_numel * sizeof(T));
