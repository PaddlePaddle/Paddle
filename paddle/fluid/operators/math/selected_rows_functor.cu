...
struct SelectedRowsAdd<platform::CUDADeviceContext, T> {
  void operator()(const platform::CUDADeviceContext& context,
    ...
    memory::Copy(BOOST_GET(platform::CUDAPlace, out_place), out_data,
                 BOOST_GET(platform::CUDAPlace, in1_place), in1_data,
                 in1_value.numel() * sizeof(T), context.stream());

    auto* in2_data = in2_value.data<T>();
    memory::Copy(BOOST_GET(platform::CUDAPlace, out_place),
                 out_data + in1_value.numel(),
                 BOOST_GET(platform::CUDAPlace, in2_place), in2_data,
...
struct SelectedRowsAddTo<platform::CUDADeviceContext, T> {
  void operator()(const platform::CUDADeviceContext& context,
    ...
    auto* in1_data = in1_value.data<T>();
    auto* in2_data = in2_value->data<T>();
    memory::Copy(BOOST_GET(platform::CUDAPlace, in2_place),
