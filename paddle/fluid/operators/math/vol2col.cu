 ...
 */
template <class T>
class Vol2ColFunctor<paddle::operators::math::ColFormat::kCFO,
 ...
 */
template <class T>
class Col2VolFunctor<paddle::operators::math::ColFormat::kCFO,
                     platform::CUDADeviceContext, T> {
 public:
  void operator()(const platform::CUDADeviceContext& context,
...
template <class T>
__global__ void vol2colOCF(
    int num_kernels, const T* data_vol, int depth, int height, int width,
    int dilation_d, int dilation_h, int dilation_w, int filter_depth,
    int filter_height, int filter_width, int stride_depth, int stride_height,
    int stride_width, int padding_depth, int padding_height, int padding_width,
    int output_detph, int output_height, int output_width, T* data_col,
  ...
  for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < num_kernels;
    ...
    int w_out = (data_layout == DataLayout::kNCHW
                     ? index % output_width
                     : (index / input_channels) % output_width);
    int h_out = (data_layout == DataLayout::kNCHW
                     ? (index / output_width) % output_height
                     : (index / input_channels / output_width) % output_height);
    int d_out = (data_layout == DataLayout::kNCHW
                     ? (index / output_width / output_height) % output_detph
                     : index / input_channels / output_width / output_height);
    int channel_in = (data_layout == DataLayout::kNCHW
                          ? index / output_width / output_height / output_detph
      ...
      for (int i = 0; i < filter_height; ++i) {
        for (int j = 0; j < filter_width; ++j) {
          ...
          if (data_layout == DataLayout::kNCHW) {
            vol_idx =
                ((channel_in * depth + d_in) * height + h_in) * width + w_in;
          } else {
            vol_idx =
                ((d * height + h) * width + w) * input_channels + channel_in;
          }
          int col_idx =
              ((d_out * output_height + h_out) * output_width + w_out) *
                  input_channels +
              channel_in;
          col_idx = ((col_idx * filter_depth + k) * filter_height + i) *
                        filter_width +
                    j;
          data_col[col_idx] = (d >= 0 && d < depth && h >= 0 && h < height &&
                               w >= 0 && w < width)
                                  ? data_vol[vol_idx]
                                  : 0;
        }
 ...
 */
template <class T>
class Vol2ColFunctor<paddle::operators::math::ColFormat::kOCF,
                     platform::CUDADeviceContext, T> {
 public:
  void operator()(const platform::CUDADeviceContext& context,
                  const framework::Tensor& vol,
    ...
    vol2colOCF<T><<<blocks, threads, 0, context.stream()>>>(
        num_outputs, vol.data<T>(), input_depth, input_height, input_width,
        dilations[0], dilations[1], dilations[2], filter_depth, filter_height,
        filter_width, strides[0], strides[1], strides[2], paddings[0],
        paddings[1], paddings[2], output_depth, output_height, output_width,
        col->data<T>(), data_layout);
  }
};

template <class T>
__global__ void col2volOCF(
    int num_kernels, const T* data_col, int depth, int height, int width,
    int dilation_d, int dilation_h, int dilation_w, int filter_depth,
    int filter_height, int filter_width, int stride_depth, int stride_height,
    int stride_width, int padding_depth, int padding_height, int padding_width,
    int output_detph, int output_height, int output_width, T* data_vol,
    const DataLayout data_layout) {
  const int d_filter_depth = dilation_d * (filter_depth - 1) + 1;
  const int d_filter_height = dilation_h * (filter_height - 1) + 1;
  const int d_filter_width = dilation_w * (filter_width - 1) + 1;
      ...
      for (int h_col = h_col_start; h_col < h_col_end; ++h_col) {
        for (int w_col = w_col_start; w_col < w_col_end; ++w_col) {
          ...
          if (d_off % dilation_d == 0 && h_off % dilation_h == 0 &&
            ...
            int data_col_index =
                ((d_col * output_height + h_col) * output_width + w_col) *
                    input_channels +
                c;
            data_col_index =
                (((((data_col_index * filter_depth + d_off) * filter_height +
                    h_off) *
                       filter_width +
                   w_off)));
            src_val += data_col[data_col_index];
 ...
 */
template <class T>
class Col2VolFunctor<paddle::operators::math::ColFormat::kOCF,
                     platform::CUDADeviceContext, T> {
 public:
  void operator()(const platform::CUDADeviceContext& context,
                  const framework::Tensor& col,
    ...
    col2volOCF<T><<<blocks, threads, 0, context.stream()>>>(
        num_kernels, col.data<T>(), input_depth, input_height, input_width,
        dilations[0], dilations[1], dilations[2], filter_depth, filter_height,
        filter_width, strides[0], strides[1], strides[2], paddings[0],
        paddings[1], paddings[2], output_depth, output_height, output_width,
        vol->data<T>(), data_layout);
  }
};

template class Vol2ColFunctor<paddle::operators::math::ColFormat::kOCF,
                              platform::CUDADeviceContext, float>;
template class Vol2ColFunctor<paddle::operators::math::ColFormat::kOCF,
                              platform::CUDADeviceContext, double>;
template class Col2VolFunctor<paddle::operators::math::ColFormat::kOCF,
                              platform::CUDADeviceContext, float>;
template class Col2VolFunctor<paddle::operators::math::ColFormat::kOCF,
                              platform::CUDADeviceContext, double>;
