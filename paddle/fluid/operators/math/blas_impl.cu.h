...
// here.
  ...
  // https://docs.nvidia.com/cuda/cublas/index.html#cublassetmathmode
  template <typename... ARGS>
  static void GEMM_EX(pten::CUDAContext *dev_ctx, cublasOperation_t transa,
...
template <>
template <typename T>
void Blas<pten::CUDAContext>::GEMM(CBLAS_TRANSPOSE transA,
...
#if CUDA_VERSION >= 8000
  if (FLAGS_enable_cublas_tensor_op_math && std::is_same<T, float>::value) {
    auto &cuda_ctx = const_cast<pten::CUDAContext &>(context_);
    CUBlas<T>::GEMM_EX(&cuda_ctx, cuTransB, cuTransA, N, M, K, &alpha, B,
                       CUDA_R_32F, ldb, A, CUDA_R_32F, lda, &beta, C,
                       CUDA_R_32F, N);
  } else {
...
template <>
template <>
inline void Blas<pten::CUDAContext>::GEMM(CBLAS_TRANSPOSE transA,
...
template <>
template <>
inline void Blas<pten::CUDAContext>::GEMM(
    CBLAS_TRANSPOSE transA, CBLAS_TRANSPOSE transB, int M, int N, int K,
    platform::complex<double> alpha, const platform::complex<double> *A,
    const platform::complex<double> *B, platform::complex<double> beta,
    platform::complex<double> *C) const {
...
template <>
template <typename T>
void Blas<pten::CUDAContext>::GEMM(bool transA, bool transB, int M, int N,
...
template <>
template <>
inline void Blas<pten::CUDAContext>::GEMM(bool transA, bool transB, int M,
...
template <>
template <typename T>
void Blas<pten::CUDAContext>::GEMV(bool trans_a, int M, int N, T alpha,
...
template <>
template <>
inline void Blas<pten::CUDAContext>::GEMV(bool trans_a, int M, int N,
...
template <>
template <typename T>
void Blas<pten::CUDAContext>::BatchedGEMM(CBLAS_TRANSPOSE transA,
                                          CBLAS_TRANSPOSE transB, int M, int N,
...
template <>
template <>
inline void Blas<pten::CUDAContext>::BatchedGEMM(
    CBLAS_TRANSPOSE transA, CBLAS_TRANSPOSE transB, int M, int N, int K,
    platform::float16 alpha, const platform::float16 **A,
    const platform::float16 **B, platform::float16 beta, platform::float16 **C,
    int batchCount) const {
