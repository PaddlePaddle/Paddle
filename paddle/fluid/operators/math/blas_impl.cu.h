...
struct CUBlas<float> {
  ...
  static void GEMM(ARGS... args) {
    PADDLE_ENFORCE_EQ(platform::dynload::cublasSgemm(args...), CUBLAS_STATUS_SUCCESS);
  }

  template <typename... ARGS>
  static void AXPY(ARGS... args) {
    PADDLE_ENFORCE_EQ(platform::dynload::cublasSaxpy(args...), CUBLAS_STATUS_SUCCESS);
  }

  template <typename... ARGS>
  static void GEMV(ARGS... args) {
    PADDLE_ENFORCE_EQ(platform::dynload::cublasSgemv(args...), CUBLAS_STATUS_SUCCESS);
  }

  template <typename... ARGS>
  static void GEMM_STRIDED_BATCH(ARGS... args) {
#if CUDA_VERSION >= 8000
    PADDLE_ENFORCE_EQ(platform::dynload::cublasSgemmStridedBatched(args...), CUBLAS_STATUS_SUCCESS);
#else
...
// here.
    ...
    dev_ctx->TensorCoreCublasCallIfAvailable([&](cublasHandle_t handle) {
      PADDLE_ENFORCE_EQ(platform::dynload::cublasSgemmEx(
...
struct CUBlas<double> {
  template <typename... ARGS>
  static void GEMM(ARGS... args) {
    PADDLE_ENFORCE_EQ(platform::dynload::cublasDgemm(args...), CUBLAS_STATUS_SUCCESS);
  }

  template <typename... ARGS>
  static void AXPY(ARGS... args) {
    PADDLE_ENFORCE_EQ(platform::dynload::cublasDaxpy(args...), CUBLAS_STATUS_SUCCESS);
  }

  template <typename... ARGS>
  static void GEMV(ARGS... args) {
    PADDLE_ENFORCE_EQ(platform::dynload::cublasDgemv(args...), CUBLAS_STATUS_SUCCESS);
  }

  template <typename... ARGS>
  static void GEMM_STRIDED_BATCH(ARGS... args) {
#if CUDA_VERSION >= 8000
    PADDLE_ENFORCE_EQ(platform::dynload::cublasDgemmStridedBatched(args...), CUBLAS_STATUS_SUCCESS);
#else
...
struct CUBlas<platform::float16> {
  ...
  static void GEMM_STRIDED_BATCH(cublasHandle_t handle,
                                 ...
                                 int batchCount) {
#if CUDA_VERSION >= 8000
    PADDLE_ENFORCE_EQ(platform::dynload::cublasHgemmStridedBatched(
  ...
  static void GEMM_EX(platform::CUDADeviceContext *dev_ctx,
    ...
    dev_ctx->TensorCoreCublasCallIfAvailable([&](cublasHandle_t handle) {
      PADDLE_ENFORCE_EQ(platform::dynload::cublasGemmEx(
          handle, transa, transb, m, n, k, alpha, A, Atype, lda, B, Btype, ldb,
...
void Blas<platform::CUDADeviceContext>::BatchedGEMM(
  ...
  if (FLAGS_enable_cublas_tensor_op_math && std::is_same<T, float>::value) {
    ...
    context_.TensorCoreCublasCallIfAvailable([&](cublasHandle_t handle) {
      PADDLE_ENFORCE_EQ(platform::dynload::cublasGemmStridedBatchedEx(
