...
class MHASeqDataPrepKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    MHASeqDataSingleton::Instance().Data(key).qkvo_seq_len =
        memory::Alloc(host_pinned_place, qkvo_seqlen_size);
    PADDLE_ENFORCE_CUDA_SUCCESS(cudaMemcpy(
        MHASeqDataSingleton::Instance().Data(key).qkvo_seq_len->ptr(),
        reinterpret_cast<const void*>(qo_kv_slen_data), qkvo_seqlen_size,
    ...
    MHASeqDataSingleton::Instance().Data(key).lo_hi_windows =
        memory::Alloc(host_pinned_place, low_high_windows_size);
    PADDLE_ENFORCE_CUDA_SUCCESS(cudaMemcpy(
        MHASeqDataSingleton::Instance().Data(key).lo_hi_windows->ptr(),
