...
PipeReader::PipeReader(const int pipe_fd) : framework::FileReader() {
...
void PipeReader::ReadNext(std::vector<framework::LoDTensor>* out) {
  ...
  //          lod_data: (level_size * uint32) data of each lod level
  //      }
  ...
  out_ref.resize(num_tensor);
  for (uint64_t i = 0; i < num_tensor; i++) {
    auto& tensor = out_ref[i];
    ...
    for (uint64_t j = 0; j < lod_level; j++) {
      // level size
      uint64_t level_size;
      pipe_->read(reinterpret_cast<uint8_t*>(&level_size), sizeof(level_size));
      // level data
    ...
    // dtype
    uint32_t dtype_enum;
    pipe_->read(reinterpret_cast<uint8_t*>(&dtype_enum), sizeof(dtype_enum));
