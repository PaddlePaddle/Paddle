...
BufferedReader::BufferedReader(
  ...
  if (platform::is_gpu_place(place_)) {
    int dev_idx = BOOST_GET(platform::CUDAPlace, place_).device;
    compute_stream_ =
...
void BufferedReader::ReadAsync(size_t i) {
  position_.emplace(thread_pool_.enqueue([this, i]() -> size_t {
    ...
    if (platform::is_gpu_place(place_)) {
      ...
      platform::SetDeviceId(BOOST_GET(platform::CUDAPlace, place_).device);
      ...
      for (size_t i = 0; i < cpu.size(); ++i) {
        ...
        if (platform::is_cuda_pinned_place(cpu_place)) {
          memory::Copy(BOOST_GET(platform::CUDAPlace, place_), gpu_ptr,
                       BOOST_GET(platform::CUDAPinnedPlace, cpu_place), cpu_ptr,
                       size, stream_.get());
        } else if ((platform::is_gpu_place(cpu_place))) {
          memory::Copy(BOOST_GET(platform::CUDAPlace, place_), gpu_ptr,
                       BOOST_GET(platform::CUDAPlace, cpu_place), cpu_ptr, size,
                       stream_.get());
        } else {
          ...
          memory::Copy(cuda_pinned_place, cuda_pinned_ptr,
                       BOOST_GET(platform::CPUPlace, cpu_place), cpu_ptr, size);
          memory::Copy(BOOST_GET(platform::CUDAPlace, place_), gpu_ptr,
