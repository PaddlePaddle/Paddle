...
TEST(scatter, ScatterUpdate) {
  ...
  auto* p_src = src.mutable_data<float>(pten::make_ddim({1, 4}),
                                        paddle::platform::CPUPlace());
  auto* p_index = index.mutable_data<int>(pten::make_ddim({1}),
                                          paddle::platform::CPUPlace());
  ...
  auto* p_output = output.mutable_data<float>(pten::make_ddim({4, 4}),
                                              paddle::platform::CPUPlace());
