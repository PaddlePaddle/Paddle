...
TEST(SecSaveLoadOp, CPU) {
  ...
  paddle::framework::AttributeMap attrs;
  attrs.insert({"file_path", std::string("tensor.save")});
  attrs.insert({"encrypt", true});
  attrs.insert({"decrypt", true});
  attrs.insert({"key", std::string("0", 32)});

  auto sec_save_op = paddle::framework::OpRegistry::CreateOp(
      "save", {{"X", {"test_var"}}}, {}, attrs);
  sec_save_op->Run(scope, place);

  auto load_var = scope.Var("out_var");
  auto target = load_var->GetMutable<paddle::framework::LoDTensor>();
  auto sec_load_op = paddle::framework::OpRegistry::CreateOp(
      "load", {}, {{"Out", {"out_var"}}}, attrs);
  sec_load_op->Run(scope, place);
  int* actual = target->data<int>();
  for (int64_t i = 0; i < tensor->numel(); ++i) {
    EXPECT_EQ(expect[i], actual[i]);
  }
...
TEST(SecSaveLoadOpSelectRowType, CPU) {
  paddle::framework::Scope scope;
  paddle::platform::CPUPlace place;
  ...
  auto* data = value->mutable_data<float>(
      ...
      place);
  int* expect = tensor->mutable_value()->mutable_data<int>(place);
  ...
  auto sec_load_op = paddle::framework::OpRegistry::CreateOp(
      "load", {}, {{"Out", {"out_var"}}}, attrs);
  sec_load_op->Run(scope, place);
  int* actual = target->mutable_value()->data<int>();
  for (int64_t i = 0; i < value->numel(); ++i) {
    EXPECT_EQ(expect[i], actual[i]);
  }
}
