...
class MultiHeadMatMulKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    auto *q =
        context.Input<framework::Tensor>("Q");
    auto *k =
        context.Input<framework::Tensor>("K");
    auto *v =
        context.Input<framework::Tensor>("V");
    ...
    auto &bias_qk =
        detail::Ref(context.Input<framework::Tensor>("BiasQK"), "Cannot find Y");
    ...
    math::MultiHeadGPUCompute<platform::CUDADeviceContext, T>::compute(device_ctx, head_number, q->dims(), k->dims(), v->dims(),
       q->data<T>(), k->data<T>(), v->data<T>(), bias_q.data<T>(), bias_k.data<T>(), bias_v.data<T>(), bias_qk.data<T>(), out->data<T>(), scale, T(0.0) 
      	);
    }
};
...
/*class MultiHeadMatMulOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    ...
    AddInput("BiasQ", "The first input of MatMul op");
    AddInput("BiasK", "The second input of MatMul op");
    AddInput("BiasV", "The second input of MatMul op");
    AddInput("BiasQK", "The second input of MatMul op");     
...
REGISTER_OP_CUDA_KERNEL(
    multihead_matmul, ops::MultiHeadMatMulKernel<paddle::platform::CUDADeviceContext, float>,
