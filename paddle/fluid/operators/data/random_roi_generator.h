...
class RandomROIGenerator {
 public:
  explicit RandomROIGenerator(AspectRatioRange aspect_ratio_range,
                              AreaRange area_range, int64_t seed = time(0),
                              int64_t num_attempts = 10);
 ...
 public:
  ...
  Generators* GetGenerators(const int64_t program_id, const int batch_size,
    ...
    if (iter == prog_id_to_generators_.end()) {
      prog_id_to_generators_[program_id] =
          std::unique_ptr<Generators>(new Generators(batch_size));
      ...
      for (int i = 0; i < batch_size; i++) {
        prog_id_to_generators_[program_id]->at(i).reset(
            new RandomROIGenerator(aspect_ratio_range, area_range, rands[i]));
      }
    }
