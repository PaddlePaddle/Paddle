...
class TriangularSolveOp : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    std::vector<int64_t> x_dims_vec = pten::vectorize(x_dims);
    std::vector<int64_t> y_dims_vec = pten::vectorize(y_dims);
    ...
    ctx->SetOutputDim("Out", pten::make_ddim(y_broadcast_dims));
    ctx->ShareLoD("X", /*->*/ "Out");
  }
