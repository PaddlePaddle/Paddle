...
void Compare(f::Scope *scope, const p::DeviceContext &ctx) {
  ...
  std::vector<InputVars> input_names = {
      {"x", scope->Var("x")->GetMutable<f::Tensor>()},
      {"x1", scope->Var("x1")->GetMutable<f::Tensor>()}};

  auto *scale = scope->Var("scale")->GetMutable<f::Tensor>();

  // init output
  auto *out = scope->Var("out")->GetMutable<f::Tensor>();
  auto *out1 = scope->Var("out1")->GetMutable<f::Tensor>();
  auto *found_inf = scope->Var("found_inf")->GetMutable<f::Tensor>();
