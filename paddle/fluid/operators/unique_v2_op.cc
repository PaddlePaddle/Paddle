...
#include "paddle/fluid/operators/unique_v2_op.h"

namespace paddle {
namespace operators {

class UniqueV2Op : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    OP_INOUT_CHECK(ctx->HasOutput("Inverse"), "Output", "Inverse", "unique");
    ...
    if (axis_vec.empty()) {
      ...
      if (return_inverse)
        ctx->SetOutputDim("Inverse", {framework::product(in_dims)});
    } else {
      int axis = axis_vec[0];
      if (axis < 0) {
        axis += in_dims.size();
      ...
      if (return_inverse) ctx->SetOutputDim("Inverse", {in_dims[axis]});
    }
    if (return_index) ctx->SetOutputDim("Indices", {-1});
    if (return_counts) ctx->SetOutputDim("Counts", {-1});
  }
...
class UniqueOpV2Maker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("X", "Input tensor.");
    AddOutput("Out", "A unique tensor for X.");
    AddOutput(
        "Indices",
...
REGISTER_OPERATOR(
    unique_v2, ops::UniqueV2Op, ops::UniqueOpV2Maker,
    paddle::framework::EmptyGradOpMaker<paddle::framework::OpDesc>,
    paddle::framework::EmptyGradOpMaker<paddle::imperative::OpBase>);
REGISTER_OP_CPU_KERNEL(
    unique_v2, ops::UniqueKernel<paddle::platform::CPUDeviceContext, float>,
