...
class ExpandAsV3Kernel : public framework::OpKernel<T> {
 public:
      ...
      if (vec_in_dims[i] != 1) {
        PADDLE_ENFORCE_EQ(
            ...
            platform::errors::InvalidArgument(
                ...
                "target tensor for expand_as_v3 op.",
                vec_in_dims[i], target_shape[i]));
        repeat_times[i] = 1;
      } else {
...
template <typename DeviceContext, typename T>
class ExpandAsV3GradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    auto* in0 = context.Input<Tensor>("X");
    auto target_shape = context.Attr<std::vector<int>>("target_shape");
    ...
    if (just_copy) {
      ...
      PADDLE_ENFORCE_GE(dims, 1,
                        platform::errors::InvalidArgument(
                            ...
                            "expand_as_v3_grad op must be greater than or "
                            "equal to 1, but the value received is %d.",
                            dims));
      PADDLE_ENFORCE_LE(dims, MAX_RANK_SUPPORTED,
                        platform::errors::InvalidArgument(
                            "The rank of the input 'Out@GRAD' for "
                            "expand_as_v3_grad op must be less than or equal "
