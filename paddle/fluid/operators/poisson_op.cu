...
struct PoissonCudaFunctor {
 public:
  PoissonCudaFunctor(const T* in, unsigned int seed, unsigned int offset,
                     T* out)
      : in_(in), seed_(seed), offset_(offset), out_(out) {}
...
class PoissonKernel<platform::CUDADeviceContext, T>
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    PoissonCudaFunctor<T> functor(x_data, seed, offset, out_data);
    for_range(functor);
    VLOG(0) << "gpu forward end";
  }
};
