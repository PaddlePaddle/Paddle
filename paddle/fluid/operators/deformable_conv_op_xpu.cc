...
class DeformableConvXPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    PADDLE_ENFORCE_EQ(deformable_groups == 1, true,
    ...
    int output_dim = output_shape_vec[1] * output_shape_vec[2] * output_shape_vec[3];
    std::vector<int> ksize{static_cast<int>(filter.dims()[2]), static_cast<int>(filter.dims()[3])};
    int n = im2col_step;
    int c = input->dims()[1];
    int h = input->dims()[2];
    int w = input->dims()[3];
    ...
    for (int i = 0; i < batch_size / im2col_step; ++i) {
      int r = xpu::deformable_conv<float, float, float, int>(
        ...
        n, c, h, w, f, ksize, strides, paddings, dilations, groups,
        deformable_groups, nullptr, nullptr, nullptr, true);
      PADDLE_ENFORCE_EQ(
          r, XPU_SUCCESS,
          platform::errors::External("XPU deformable_conv kernel return wrong value[%d].",
                                     r));
    }
  }
};
...
class DeformableConvGradXPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (dx_data == nullptr) {
      PADDLE_ENFORCE_EQ(
          xpu_malloc(reinterpret_cast<void **>(&dx_data), input->numel() * sizeof(T)),
          XPU_SUCCESS,
          platform::errors::ResourceExhausted("XPU has no enough memory"));
    } 
    if (dw_data == nullptr) {
      PADDLE_ENFORCE_EQ(
          xpu_malloc(reinterpret_cast<void **>(&dw_data), filter.numel() * sizeof(T)),
          XPU_SUCCESS,
          platform::errors::ResourceExhausted("XPU has no enough memory"));
    }
    if (doffset_data == nullptr) {
      PADDLE_ENFORCE_EQ(
          xpu_malloc(reinterpret_cast<void **>(&doffset_data), offset.numel() * sizeof(T)),
          XPU_SUCCESS,
          platform::errors::ResourceExhausted("XPU has no enough memory"));
    }
    if (dmask_data == nullptr) {
      PADDLE_ENFORCE_EQ(
          xpu_malloc(reinterpret_cast<void **>(&dmask_data), mask.numel() * sizeof(T)),
          XPU_SUCCESS,
          platform::errors::ResourceExhausted("XPU has no enough memory"));
    }    
    ...
    PADDLE_ENFORCE_EQ(
        xpu_malloc(reinterpret_cast<void **>(&filter_grad_tmp), filter_grad->numel() * sizeof(T)),
        XPU_SUCCESS,
        platform::errors::ResourceExhausted("XPU has no enough memory"));
    ...
    int r_filter = xpu::memset(dev_ctx.x_context(), filter_grad_tmp, zero,
                        filter.numel() * sizeof(T));
    auto ret = (r_dx == xpu::Error_t::SUCCESS) && (r_dx == r_dw) && (r_dx == r_doffset) &&
              (r_dx == r_dmask) && (r_dx == r_filter);
    ...
    for (int i = 0; i < batch_size / im2col_step; ++i) {
      int r = xpu::deformable_conv_grad<float, float, float, int>(
              ...
              n, c, h, w, f, ksize, strides, paddings, dilations, groups,
              deformable_groups, nullptr, nullptr, nullptr, nullptr, nullptr, true);
      PADDLE_ENFORCE_EQ(
          r, XPU_SUCCESS,
          platform::errors::External("XPU deformable_conv_grad kernel return wrong value[%d].",
                                     r));
      r = baidu::xpu::api::add<T>(dev_ctx.x_context(), filter_grad_tmp, dw_data, dw_data,
          filter.numel());
      PADDLE_ENFORCE_EQ(
          r, XPU_SUCCESS,
          platform::errors::External("XPU add kernel return wrong value[%d].",
                                     r));
    }
...
REGISTER_OP_XPU_KERNEL(
    deformable_conv, ops::DeformableConvXPUKernel<XPUDeviceContext, float>);
REGISTER_OP_XPU_KERNEL(
    deformable_conv_grad, ops::DeformableConvGradXPUKernel<XPUDeviceContext, float>);
