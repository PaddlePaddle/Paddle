...
USE_OP_DEVICE_KERNEL(argsort, NPU);
USE_OP_DEVICE_KERNEL(argsort_grad, NPU);
#endif
...
// input - X
const std::vector<int64_t> x_dim = std::vector<int64_t>{2, 2, 2, 2, 3};
const int64_t x_numel = std::accumulate(x_dim.begin(), x_dim.end(), 1, std::multiplies<int64_t>());
// // Output - Out
// const std::vector<int64_t> out_dim = x_dim;
// // Output - Ids
// const std::vector<int64_t> ids_dim = x_dim;

template <typename T>
void TestMain(const platform::DeviceContext& ctx, std::vector<T>* out_data, 
              std::vector<int64_t>* ids_data, std::vector<T>* dx_data) {
  ...
  // framework::DDim ids_ddim = framework::make_ddim(ids_dim);

  LOG(INFO) << "x_ddim=" << x_ddim.to_str();
  // LOG(INFO) << "out_ddim=" << out_ddim.to_str();
  // LOG(INFO) << "ids_ddim=" << ids_ddim.to_str();

  // --------------- forward ----------------------
  desc_fwd.SetType("argsort");
  desc_fwd.SetInput("X", {"X"});
  desc_fwd.SetOutput("Out", {"Out"});
  desc_fwd.SetOutput("Indices", {"Indices"});
  desc_fwd.SetAttr("axis", axis);
  desc_fwd.SetAttr("descending", descending);

  auto X = scope.Var("X")->GetMutable<framework::LoDTensor>();
  auto Out = scope.Var("Out")->GetMutable<framework::LoDTensor>();
  auto Ids = scope.Var("Indices")->GetMutable<framework::LoDTensor>();
  ...
  framework::TensorToVector(*Ids, ctx, ids_data);

  // --------------- backward ----------------------
  desc_bwd.SetType("argsort_grad");
  desc_bwd.SetInput("X", {"X"});
  desc_bwd.SetInput("Indices", {"Indices"});
  ...
  desc_bwd.SetAttr("descending", descending);
...
TEST(test_argsort, compare_cpu_and_npu) {
  platform::CPUPlace cpu_place;
  platform::CPUDeviceContext cpu_ctx(cpu_place);
  std::vector<float> out_cpu(x_numel);
  std::vector<int64_t> ids_cpu(x_numel);
  std::vector<float> dx_cpu(x_numel);
  TestMain<float>(cpu_ctx, &out_cpu, &ids_cpu, &dx_cpu);

#ifdef PADDLE_WITH_ASCEND_CL
  platform::NPUPlace npu_place(0);
  platform::NPUDeviceContext npu_ctx(npu_place);
  std::vector<float> out_npu(x_numel);
  std::vector<int64_t> ids_npu(x_numel);
  std::vector<float> dx_npu(x_numel);
  TestMain<float>(npu_ctx, &out_npu, &ids_npu, &dx_npu);

  compare_results<float>(out_cpu, out_npu, x_dim, "Out");
  compare_results<int64_t>(ids_cpu, ids_npu, x_dim, "Ids");
  compare_results<float>(dx_cpu, dx_npu, x_dim, "dX");
#endif
}
