...
struct DeviceIndependenceTensorOperations {
  ...
  framework::Tensor Matmul(const framework::Tensor& mat_a, 
    ...
    blas.MatMul(mat_a, mat_a_discrib, mat_b, mat_b_discrib, T(1.0), &ret, T(0.0)) ;
    return ret; 
  }
  ...
  framework::Tensor Transpose(const framework::Tensor& x) {
    ...
    switch(rank){
        ...
        default: {
            PADDLE_THROW(platform::errors::InvalidArgument(
                "Invalid Rank number, currently only support rank between 2~6"
            ));
        }
    }
    return ret;
  ...
  framework::Tensor Diag(const framework::Tensor& x, int offset = 0,
    ...
    PADDLE_ENFORCE_EQ(offset, 0,
                    platform::errors::InvalidArgument(
                        "Current diag only support offset = 0, you can use DiagOp instead(not recommend)"));
    ...
    if (x_rank == 2) {
      PADDLE_THROW( platform::errors::InvalidArgument(
                    "Current diag only support vector -> diagonalized matrix, not support matrix -> vector, Use DiagOp instead."));
    } else if (x_rank == 1) {
    ...
    ret = Zeros({out_shape[0], out_shape[0]}, framework::proto::VarType::FP32, 0.0); 
  ...
  void print(const framework::Tensor & x, std::string name){
    // Support tensor print in arbitrary devices.
    framework::Tensor out ;
    TensorCopy(x, platform::CPUPlace(), &out);
    VLOG(4) << "NAME: " << name << out ;
  }
  ...
  framework::Tensor Sub(const framework::Tensor& x,
    ...
    if (x.dims().size() >= y.dims().size()) {
      ElementwiseComputeEx<SubFunctor<T>, DeviceContext, T>(context, &x, &y, -1,
                                                            SubFunctor<T>(), &ret);
    } else {
      ElementwiseComputeEx<InverseSubFunctor<T>, DeviceContext, T>(  // This is copyed from elementwise_sub, which means we need reverse will xrank < yrank
  ...
  framework::Tensor Infinits(std::vector<int> shape,
    ...
    return Zeros(shape, dtype, value);
  }
 ...
 private:
  ...
  void EigenSliceWrapper(const framework::Tensor * in, 
    ...
    PADDLE_ENFORCE_EQ(start.size(), rank, 
        platform::errors::InvalidArgument("EigenSliceWrapper function' start argument must have the same length as input rank."));
    PADDLE_ENFORCE_EQ(end.size(), rank, 
        platform::errors::InvalidArgument("EigenSliceWrapper function' start argument must have the same length as input rank."));
    auto eigen_place_ptr = context.template device_context<DeviceContext>().eigen_device();
