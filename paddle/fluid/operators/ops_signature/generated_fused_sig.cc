...
NOTE: The following codes are for 'get_compat_kernel_signature.py'
All possible KernelSignatures returned by
FusedDotProductAttentionOpArgumentMapping:

return KernelSignature("fused_dot_product_attention", {"q", "k", "v", "mask"},
{"scaling_factor", "dropout_probability", "is_training", "is_causal_masking"},
{"out", "softmax_out", "rng_state"});
******************************************************************
*/

KernelSignature FusedDotProductAttentionOpArgumentMapping(
    const ArgumentMappingContext& ctx) {
  ...
  return KernelSignature("fused_dot_product_attention",
                         std::move(inputs),
                         std::move(attrs),
                         std::move(outputs));
}
...
NOTE: The following codes are for 'get_compat_kernel_signature.py'
All possible KernelSignatures returned by
FusedDotProductAttentionGradOpArgumentMapping:

return KernelSignature("fused_dot_product_attention_grad", {"q", "k", "v",
"out", "softmax_out", "rng_state", "mask", "out@GRAD"}, {"scaling_factor",
"dropout_probability", "is_causal_masking"}, {"q@GRAD", "k@GRAD", "v@GRAD"});
******************************************************************
*/

KernelSignature FusedDotProductAttentionGradOpArgumentMapping(
    const ArgumentMappingContext& ctx) {
  paddle::small_vector<const char*> inputs{
      "q", "k", "v", "out", "softmax_out", "rng_state", "mask", "out@GRAD"};
  paddle::small_vector<const char*> attrs;
  attrs.emplace_back("scaling_factor");
  attrs.emplace_back("dropout_probability");
  attrs.emplace_back("is_causal_masking");
  paddle::small_vector<const char*> outputs{"q@GRAD", "k@GRAD", "v@GRAD"};
  return KernelSignature("fused_dot_product_attention_grad",
                         std::move(inputs),
                         std::move(attrs),
                         std::move(outputs));
}
...
PD_REGISTER_ARG_MAPPING_FN(fused_dot_product_attention,
                           phi::FusedDotProductAttentionOpArgumentMapping);
...
PD_REGISTER_ARG_MAPPING_FN(fused_dot_product_attention_grad,
                           phi::FusedDotProductAttentionGradOpArgumentMapping);
