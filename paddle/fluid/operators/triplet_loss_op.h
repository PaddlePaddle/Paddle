...
//    x_t_2 = x_t.square()
//    x_t_2_sum = x_t_2.sum({1})
//    x_t_2_sum_bc = x_t_2_sum.broadcast({1,batch_size})

//    x_t_2_sum_trans = x_t_2_sum.shuffle({1,0})
//    x_t_2_sum_trans_bc = x_t_2_sum_trans.broadcast({batch_size, 1})

//    D = x_t_2_sum_bc + x_t_2_sum_trans_bc - 2*AB


    d_x_t_2_sum_bc = d_D
    d_x_t_2_sum_trans_bc = d_D
    d_AB = d_D * -2
    
    d_x_t_2_sum_trans = d_x_t_2_sum_trans_bc.sum(0)
    d_x_t_2_sum = d_x_t_2_sum_trans.shuffle({1,0})

    d_x_t_2_sum += d_x_t_2_sum_bc.sum(1)
    d_x_t_2 = d_x_t_2_sum.broadcast({1, feature_len})
    d_x_t = 2 * x_t * d_x_t_2


    d_x_t += d_AB * x_t_trans
    d_x_t_trans = d_AB * x_t.T
    d_x_t += d_x_t_trans.T
