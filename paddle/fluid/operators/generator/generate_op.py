...
def to_phi_and_fluid_op_name_without_underline(op_item):
    '''
    If the op_name ends with '_', delete the last '_'. For an example, 'sgd_' become 'sgd
    '''
    names = op_item.split('(')
    if len(names) == 1:
        op_kernel_name = to_name_without_underline(names[0].strip())
        return op_kernel_name
    else:
        op_name = to_name_without_underline(names[0].strip())
        kernel_name = to_name_without_underline(names[1].split(')')[0].strip())
...
def main(
    ...
    forward_op_dict = to_named_dict_for_op(ops)
    with open(backward_yaml_path, "rt") as f:
        backward_ops = yaml.safe_load(f)
        backward_ops = [restruct_io(op) for op in backward_ops]
    backward_op_dict = to_named_dict_for_op(backward_ops)
