 ...
 private:
  void RunImpl(const framework::Scope &scope,
    ...
    if (Inputs().find("ExecutorPrepareContext") != Inputs().end() &&
      ...
      const int64_t *data = context.data<int64_t>();
      ctx_ptr = reinterpret_cast<framework::ExecutorPrepareContext *>(data[0]);
    } else {
      ctx = executor.Prepare(*program, block->ID());
      ctx_ptr = ctx.get();
    }
    ...
    if (!is_test) {
      ...
      while (cond.data<bool>()[0]) {
        executor.RunPreparedContext(ctx_ptr, &current_scope, false, false,
                                    false);
      }
      scope.DeleteScope(&current_scope);
    }
 ...
 public:
  void Make() override {
    ...
    AddInput(
        ...
        .AsDuplicable();
    AddInput("ExecutorPrepareContext",
             "(Tensor<ExecutorPrepareContext*>, optional). If provided, "
             "executor will this context to avoid the overhead of "
             "executor.Prepare(). It is a address, and will be stored as a "
             "int64_t.")
        .AsDispensable();
    AddOutput(kOutputs,
