    ...
    private:
        void RunImpl(const framework::Scope & scope,
            ...
            if (!core_ || !platform::is_same_place(core_->GetPlace(), dev_place)) {
                ...
                // NOTE: Borrowed from `paddle/fluid/operators/controlflow/control_flow_op_helper.h`
...
class StaticPyLayerBackwardOp : public StaticPyLayerOp {
    ...
    private:
        void RunImpl(const framework::Scope & scope,
            ...
            // NOTE: It's neccessary. The reason of associating `inside_grads` and `outside_grads` at runtime `RunImpl` instead of `assgin` op at block
            // is that the Var name of grad_op's outputs may be changed in the `append_backward` function (e.g. `_addup_repetitive_outputs_`). 
