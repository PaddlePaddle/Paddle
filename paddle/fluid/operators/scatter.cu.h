...
DEFINE_int32(bad_index, -1, "the dupliate index set bad")
...
__global__ void ScatterCUDAKernel(const T* params, const IndexT* indices,
  ...
  CUDA_1D_KERNEL_LOOP(i, index_size * slice_size) {
    ...
    if (overwrite) {
      if (sctter_i != FLAGS_bad_index) {
        *(output + out_i) = *(params + i);
      }
    } else {
      paddle::platform::CudaAtomicAdd(params + i, *(output + out_i));
    }
  }
}
