...
__global__ void ScatterCUDAKernel(const T* params, const IdsT* indices,
                                  T* output, size_t index_size,
                                  size_t slice_size) {
  CUDA_1D_KERNEL_LOOP(i, index_size * slice_size) {
    int indices_i = i / slice_size;
    int slice_i = i - indices_i * slice_size;  // offset inside the slice
    IdsT scatter_i = indices[indices_i];
    IdsT out_i = scatter_i * slice_size + slice_i;
