...
class RpnTargetAssignOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("Anchor",
             "(phi::DenseTensor) input anchor is a 2-D phi::DenseTensor with "
             "shape [H*W*A, 4].");
    AddInput("GtBoxes",
    ...
    AddOutput(
        "LocationIndex",
        "(phi::DenseTensor), The indexes of foreground anchors in all RPN "
        "anchors, the "
        "shape of the LocationIndex is [F], F depends on the value of input "
        "tensor and attributes.");
    AddOutput(
        "ScoreIndex",
        "(phi::DenseTensor), The indexes of foreground and background anchors "
        "in all "
    ...
    AddOutput("TargetBBox",
              "(phi::DenseTensor), The target bbox deltas with shape "
              "[F, 4], F is the sampled foreground number.");
    AddOutput(
        "TargetLabel",
        "(phi::DenseTensor<int>), The target labels of each anchor with shape "
        "[F + B, 1], F and B are sampled foreground and background number.");
    AddOutput("BBoxInsideWeight",
              "(phi::DenseTensor), The bbox inside weight with shape "
              "[F, 4], F is the sampled foreground number.");
    AddComment(R"DOC(
...
class RetinanetTargetAssignOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    ...
    AddOutput(
        "LocationIndex",
        "(phi::DenseTensor), The indexes of foreground anchors in all anchors, "
        "the "
        "shape of the LocationIndex is [F], F depends on the value of input "
        "tensor and attributes.");
    AddOutput("ScoreIndex",
              "(phi::DenseTensor), The indexes of foreground and background "
              "anchors in all "
              "RPN anchors(The rest anchors are ignored). The shape of the "
    ...
    AddOutput("ForegroundNumber",
              "(phi::DenseTensor), The foreground number. "
              "[1, 1].");
    AddComment(R"DOC(
