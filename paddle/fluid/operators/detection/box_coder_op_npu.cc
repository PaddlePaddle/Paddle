...
void BoxCoderEnc(const framework::ExecutionContext& ctx, const Tensor* tb,
  ...
  auto M = pb->dims()[0];
  auto N = tb->dims()[0];
  auto shape_0 = pten::make_ddim({4, 2});
  Tensor m_diff;
  Tensor m_aver;
  ...
  auto shape_half = pten::make_ddim({N, M, 2});
  auto shape_full = pten::make_ddim({N, M, 4});
  ...
  if (pbv) {
    ...
    Vector2Tensor(ctx, vec_var, pten::make_ddim({1, 1, 4}), &t_var);
...
void BoxCoderDec(const framework::ExecutionContext& ctx, const Tensor* tb,
  ...
  Tensor pb_xy = F.Adds(F.Dot(*pb, m_aver), (norm ? 0 : 0.5));
  Tensor pb_wh = F.Adds(F.Dot(*pb, m_diff), (norm ? 0 : 1));
  auto pb_resize_shape = axis == 0 ? pten::make_ddim({1, pb->dims()[0], 2})
                                   : pten::make_ddim({pb->dims()[0], 1, 2});
  pb_xy.Resize(pb_resize_shape);
  pb_wh.Resize(pb_resize_shape);

  auto tbox_slice_shape = pten::make_ddim({tb->dims()[0], tb->dims()[1], 2});
  ...
  Tensor tb_wh;
  if (pbv) {
    auto pbvt_slice_shape = pten::make_ddim({pbv->dims()[0], 2});
    auto pbvt_resize_shape = axis == 0
                                 ? pten::make_ddim({1, pbv->dims()[0], 2})
                                 : pten::make_ddim({pbv->dims()[0], 1, 2});
    ...
    auto t_var_shape = pten::make_ddim({1, 1, 2});
