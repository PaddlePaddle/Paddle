...
class DistributeFpnProposalsOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    ...
    auto multi_fpn_rois = context.MultiOutput<paddle::framework::LoDTensor>("MultiFpnRois");

    auto* restore_index = context.Output<paddle::framework::Tensor>("RestoreIndex");

    int min_level = context.Attr<int>("min_level");
    int max_level = context.Attr<int>("max_level");
    int refer_level = context.Attr<int>("refer_level");
    int refer_scale = context.Attr<int>("refer_scale");
    ...
    for(int i = 0; i < fpn_rois_lod.size() - 1; ++i) {
      ...
      for(int j = 0; j < fpn_rois_slice.dims()[0]; ++j) {
        ...
        int tgt_lvl = std::floor(std::log2(roi_scale / refer_scale) + refer_level);
    ...
    for(int i = 0; i < num_level; ++i) {
      ...
      multi_fpn_rois[i]->mutable_data<T>({num_rois_level[i], kBoxDim}, context.GetPlace());
    ...
    for(int i = 0; i < fpn_rois_lod.size() - 1; ++i) {
      ...
      for(int j = 0; j < fpn_rois_slice.dims()[0]; ++j) {
        ...
        memcpy(multi_fpn_rois_data[lvl - min_level], rois_data, kBoxDim*sizeof(T));
