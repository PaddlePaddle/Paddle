 ...
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    ...
    for (int idx = 0; idx < limit; ++idx) {
      T x = x_data[idx];
      int a = idx / num_classes;  // current anchor
      ...
      float c_pos = (g == (d + 1));
      float c_neg = ((g != -1) & (g != (d + 1)));
      float fg_num = (fg_num_data[0] > 1.0) ? fg_num_data[0] : 1.0;
      float s_neg = (1.0 - alpha) / fg_num;
      float s_pos = alpha / fg_num;

      // p = 1. / 1. + expf(-x)
      float p = 1. / (1. + std::exp(-x));
 ...
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    ...
    for (int idx = 0; idx < limit; ++idx) {
      ...
      // (p**g) * (g*(1-p)*log(1-p) - p)
      float term_neg = std::pow(p, gamma) *
                       ((-1. * x * (x >= 0) -
