...
std::vector<std::vector<int>> SampleFgBgGt(
  ...
  for (int64_t i = 0; i < row; ++i) {
    if(is_cascade_rcnn && ((rpn_rois_dt[i*4+2]-rpn_rois_dt[i*4+0]+1) <= 0 ||
      (rpn_rois_dt[i*4+3]-rpn_rois_dt[i*4+1]+1) <= 0
    ){
      // cascade rcnn decode filter when refine boxes
      continue;
    }
  ...
  if(is_cascade_rcnn){
    // cascade rcnn has no sampling from second stage
...
std::vector<Tensor> SampleRoisForOneImage(
    ...
    bool is_cls_agnostic, bool is_cascade_rcnn) {
  auto im_scale = im_info.data<T>()[2];

  Tensor rpn_rois;
  Tensor rpn_rois_slice;
  if(is_cascade_rcnn){
    // cascade rcnn delete gt boxes
    rpn_rois_slice = rpn_rois_in.Slice(gt_boxes.dims()[0], rpn_rois_in.dims()[0]);
 ...
 public:
  void Make() override {
    ...
    AddAttr<bool>("is_cascade_rcnn",
                  "set True will change sampling policy.")
        .SetDefault(false);
