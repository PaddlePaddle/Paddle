...
static edge_node **bound_list(lmt_node **lmt, double y) {
  ...
  if (!*lmt) {
    ...
    gpc_malloc<lmt_node>(*lmt, sizeof(lmt_node), (char *)"LMT insertion");
    (*lmt)->y = y;
...
static void add_intersection(it_node **it, edge_node *edge0, edge_node *edge1,
  ...
  if (!*it) {
    ...
    gpc_malloc<it_node>(*it, sizeof(it_node), (char *)"IT insertion");
    (*it)->ie[0] = edge0;
    (*it)->ie[1] = edge1;
    (*it)->point.x = x;
    (*it)->point.y = y;
...
static void add_st_edge(st_node **st, it_node **it, edge_node *edge,
  ...
  if (!*st) {
    ...
    gpc_malloc<st_node>(*st, sizeof(st_node), (char *)"ST insertion");
    (*st)->edge = edge;
    (*st)->xb = edge->xb;
    (*st)->xt = edge->xt;
    (*st)->dx = edge->dx;
...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
  ...
  gpc_malloc<double>(sbt, sbt_entries * sizeof(double), (char *)"sbt creation");
