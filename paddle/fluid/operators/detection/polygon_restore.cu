...
template <typename T>
__global__ void PolygonRestoreKernel(const int n, const int h, const int w,
...
template <typename T>
class PolygonRestoreOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    PolygonRestoreKernel<T><<<numBlocks, threadsPerBlock, 0, stream>>>(
...
REGISTER_OP_CUDA_KERNEL(polygon_restore,
                        paddle::operators::PolygonRestoreOpCUDAKernel<float>,
                        paddle::operators::PolygonRestoreOpCUDAKernel<double>);
