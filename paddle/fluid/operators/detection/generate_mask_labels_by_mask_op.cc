...
class GenerateMaskLabelsByMaskOp : public framework::OperatorWithKernel {
 public:
...
std::vector<Tensor> SampleMaskForOneImage(
  ...
  Tensor rois_;
  rois_.mutable_data<T>(rois->dims(), context.GetPlace());
  framework::TensorCopy(*rois, context.GetPlace(), &rois_);
  auto rois_et = framework::EigenTensor<T, 2>::From(rois_);
  ...
  if (fg_num > 0) {
    ...
    Gather<T>(rois_.data<T>(), 4, fg_inds.data(), fg_inds.size(),
              rois_fg.data<T>());
    ...
    Gather<T>(rois_.data<T>(), 4, bg_inds.data(), bg_inds.size(),
              rois_fg.data<T>());
...
template <typename T>
class GenerateMaskLabelsByMaskKernel : public framework::OpKernel<T> {
 public:
...
class GenerateMaskLabelsByMaskOpMaker
...
REGISTER_OPERATOR(generate_mask_labels_by_mask, ops::GenerateMaskLabelsByMaskOp,
                  ops::GenerateMaskLabelsByMaskOpMaker,
                  paddle::framework::EmptyGradOpMaker);
REGISTER_OP_CPU_KERNEL(generate_mask_labels_by_mask,
                       ops::GenerateMaskLabelsByMaskKernel<float>,
                       ops::GenerateMaskLabelsByMaskKernel<double>);
