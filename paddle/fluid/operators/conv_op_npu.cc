  ...
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (filter_grad) {
      ...
      runner.SetType("DepthwiseConv2DBackpropFilterD")
          ...
          .AddAttr("filter_size", pten::vectorize(transformed_filter.dims()))
    ...
    if (input_grad) {
      ...
      runner.SetType("DepthwiseConv2DBackpropInputD")
          ...
          .AddAttr("input_size", pten::vectorize(input->dims()))
          .AddAttr("strides", strides)
          .AddAttr("dilations", dilations)
          .AddAttr("pads", padding)
          .AddAttr("data_format", data_format)
...
class NPUConvGradOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (filter_grad) {
      filter_grad->mutable_data<T>(ctx.GetPlace());
      std::vector<int> filter_shape_vec = pten::vectorize<int>(filter->dims());
    ...
    if (input_grad) {
      input_grad->mutable_data<T>(ctx.GetPlace());
      std::vector<int> input_shape_vec = pten::vectorize<int>(input->dims());
