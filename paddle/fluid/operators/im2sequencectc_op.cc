...
#include "paddle/fluid/operators/im2sequencectc_op.h"
#include <string>
...
class Im2SequenceCTCOp : public framework::OperatorWithKernel {
 public:
 ...
 protected:
  void InferShape(framework::InferShapeContext* ctx) const override {
    PADDLE_ENFORCE(ctx->HasInput("X"),
                   "Input(X) of Im2SequenceCTCOp should not be null.");
    PADDLE_ENFORCE(
        ctx->HasInput("Y"),
        "Input(Image_real_size) of Im2SequenceCTCOp should not be null.");
    PADDLE_ENFORCE(ctx->HasOutput("Out"),
                   "Output(Out) of Im2SequenceCTCOp op should not be null.");
    ...
    auto out_stride = ctx->Attrs().Get<std::vector<int>>("out_stride");
...
class Im2SequenceCTCOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("X",
    ...
    AddAttr<std::vector<int>>("paddings",
        ...
        .SetDefault({0, 0, 0, 0});
    // TODO(fuhailong): add out_stride, use this to
    ...
    AddAttr<std::vector<int>>("out_stride",
        ...
        .SetDefault({1, 1});
    AddAttr<bool>("is_inference", " nor 0  is inference, 0 is train")
        .SetDefault(false);
    AddComment(R"DOC(
...
REGISTER_OPERATOR(im2sequencectc, ops::Im2SequenceCTCOp,
                  ops::Im2SequenceCTCOpMaker,
                  paddle::framework::DefaultGradOpDescMaker<true>);
REGISTER_OP_CPU_KERNEL(
    im2sequencectc,
    ops::Im2SequenceCTCKernel<paddle::platform::CPUDeviceContext, float>);
