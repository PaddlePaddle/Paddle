...
class ShuffleBatchOp : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext *ctx) const override {
    PADDLE_ENFORCE_EQ(
        ctx->HasInput("X"), true,
        platform::errors::NotFound("Input(X) shold not be null."));
    PADDLE_ENFORCE_EQ(
        ctx->HasInput("Seed"), true,
        platform::errors::NotFound("Input(Seed) shold not be null."));
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput("Out"), true,
        platform::errors::NotFound("Output(Out) shold not be null."));
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput("ShuffleIdx"), true,
        platform::errors::NotFound("Output(ShuffleIdx) shold not be null."));
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput("SeedOut"), true,
        platform::errors::NotFound("Output(SeedOut) shold not be null."));
 ...
 public:
  ...
  void InferShape(framework::InferShapeContext *ctx) const override {
    PADDLE_ENFORCE_EQ(
        ...
        platform::errors::NotFound("Input(ShuffleIdx) shold not be null"));
    PADDLE_ENFORCE_EQ(
        ctx->HasInput(framework::GradVarName("Out")), true,
        platform::errors::NotFound("Grad Input(Out) shold not be null"));
    PADDLE_ENFORCE_EQ(
        ctx->HasOutput(framework::GradVarName("X")), true,
        platform::errors::NotFound("Grad Output(X) shold not be null"));
