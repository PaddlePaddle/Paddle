...
class SmoothL1LossV2Kernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    loss.device(*place) = errors_mat_view;
  }
};
...
template <typename DeviceContext, typename T, typename AttrType = T>
class SmoothL1LossV2GradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    auto gradients = out_grad * weights * diff_mat_view;
