...
struct AllcloseFunctor {
  void operator()(const DeviceContext& ctx, const framework::Tensor& output,
...
class AllcloseKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    framework::Tensor out_v;
    out_v.Resize({input->numel()});
    auto* out_data = out->mutable_data<bool>(ctx.GetPlace());
    // get place
    // auto& place = *ctx.template
    // device_context<DeviceContext>().eigen_device();

    AllcloseFunctor<DeviceContext, T>()(dev_ctx, out_v, *input, *other, rtol,
                                        atol, equal_nan);

    auto* out_v_data = out_v.data<bool>();
    *out_data = 1;
    for (int i = 0; i < out_v.numel(); i++) {
      *out_data &= out_v_data[i];
      printf("THis is out_v_data[i]: %d >>>>>>>>>>>>>>\n", out_v_data[i]);
    }
