...
void DeformablePSROIPoolForwardCPUKernel(
  ...
  for (int ix = 0; ix < count; ix++) {
    ...
    T trans_x = no_trans ? (T)(0) : bottom_trans[(((n * num_classes +
                class_id) * 2) * part_height + part_h) * part_width +
                part_w] * (T)trans_std;
    T trans_y = no_trans ? (T)(0) : bottom_trans[(((n * num_classes +
                class_id) * 2 + 1) * part_height + part_h) * part_width +
                part_w] * (T)trans_std;
...
void DeformablePSROIPoolBackwardAccCPUKernel(
    ...
    const int num_classes, const int channels_each_class, const int batch_size,
  ...
  for (int index = 0; index < count; index++) {
    ...
    int class_id = ctop / channels_each_class;
    T trans_x = no_trans ? (T)(0) : bottom_trans[(((n * num_classes + class_id)
                * 2) * part_height + part_h) * part_width + part_w] *
                (T)trans_std;
    T trans_y = no_trans ? (T)(0) : bottom_trans[(((n * num_classes +
                class_id) * 2 + 1) * part_height + part_h) * part_width +
                part_w] * (T)trans_std;
    ...
    for (int ih = 0; ih < sample_per_part; ih++) {
      for (int iw = 0; iw < sample_per_part; iw++) {
        ...
        if (bottom_data_diff != NULL) {
            T* offset_bottom_data_diff_addr00 = bottom_data_diff + roi_batch_ind
                                                * channels * height * width +
                                                bottom_index_base + y0 *
                                                width + x0;
            T* offset_bottom_data_diff_addr01 = bottom_data_diff + roi_batch_ind
                                                * channels * height * width +
                                                bottom_index_base + y1 *
                                                width + x0;
            T* offset_bottom_data_diff_addr10 = bottom_data_diff + roi_batch_ind
                                                * channels * height * width +
                                                bottom_index_base + y0 *
                                                width + x1;
            T* offset_bottom_data_diff_addr11 = bottom_data_diff + roi_batch_ind
                                                * channels * height * width +
                                                bottom_index_base + y1 *
                                                width + x1;
        ...
        T* offset_bottom_trans_diff_y = bottom_trans_diff + (((n *
            num_classes + class_id) * 2 + 1) * part_height + part_h) *
            part_width + part_w;
...
class DeformablePSROIPoolGradCPUKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx)  const override {
    ...
    if (trans_grad) {
        bottom_trans_diff = no_trans ? NULL : trans_grad->mutable_data<T>
                               (ctx.GetPlace());
    }
