 ...
 public:
  void operator()(framework::InferShapeContext *context) const override {
    PADDLE_ENFORCE_EQ(context->HasInput("X"), true,
                      platform::errors::NotFound(
                          "Input(x) of ShrinkRNNMrmoryOp is not found."));
    PADDLE_ENFORCE_EQ(context->HasInput("I"), true,
                      platform::errors::NotFound(
                          "Input(I) of ShrinkRNNMrmoryOp is not found."));
    PADDLE_ENFORCE_EQ(
        context->HasInput("RankTable"), true,
        platform::errors::NotFound(
            "Input(RankTable) of ShrinkRNNMrmoryOp is not found."));
  ...
  void operator()(framework::InferShapeContext *context) const override {
    PADDLE_ENFORCE_EQ(context->HasInput("X"), true,
                      platform::errors::NotFound(
                          "Input(X) of ShrinkRNNMemoryGradOp is not found."));
    PADDLE_ENFORCE_EQ(
        context->HasOutput(framework::GradVarName("X")), true,
        platform::errors::NotFound(
            "Output(X@GRAD) of ShrinkRNNMemoryGradOp is not found."));
