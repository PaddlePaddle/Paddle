...
/*
  Core function of softmax with cross entropy forward
    - softmax, CPMODE=0
    - log softmax, CPMODE=1
    - softmax with cross entropy hard label, CPMODE=2
*/
...
#pragma unroll
    for (int it = 0; it < kIterationsV; ++it) {
      ...
      if (kVSize == 1) {  // kVSize==1
        if (idx < idx_max_v[i]) {
          if (CPMODE == 1) {  // log softmax
            softmax[(first_batch + i) * stride + idx] =
                srcdata[i][it][0] - max_value[i] - sum[i];
          } else if (CPMODE == 2) {  // softmax with cross entropy hard label
...
#define SOFTMAX_WARP_FORWARD_CASE(Log2Elements, VecT, AccT)           \
  case Log2Elements:                                                  \
    WarpSoftmaxForward<T, VecT, AccT, Log2Elements,                   \
                       CPMODE><<<blocks, threads, 0, stream>>>(       \
...
*/
template <typename T, int CPMODE>
...
static void SoftmaxWithCrossEntropyHardLabel(
  ...
  if (D == 1 && dim <= max_dim) {
    ...
    CrossEntropyExpHardLabel<T><<<blocks, threads, 0, stream>>>(
...
static void SoftmaxWithCrossEntropySoftLabel(
  ...
  if (D == 1 && dim <= max_dim) {
    ...
    CrossEntropySoftLabel<
        T, T, true><<<blocks, threads, 0, ctx.cuda_device_context().stream()>>>(
  ...
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (soft_label) {
      ...
      SoftmaxWithCrossEntropySoftLabel<T>(context, rank, axis, logits_data,
                                          labels_data, softmax_data, loss_data,
                                          n, axis_dim, d / axis_dim);
    } else {
      if (!context.Attr<bool>("numeric_stable_mode")) {
        ...
        SoftmaxWithCrossEntropyHardLabel<T>(
            ...
            softmax_data, n, axis_dim, d / axis_dim, ignore_index);
      }
    }
  }
};
