...
static void DataTranspose(const framework::ExecutionContext& ctx,
  ...
  framework::DDim input_transpose_dims(pten::make_ddim(input_transpose_vec));
...
class CUDNNConvTransposeOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    std::vector<int> input_vec = pten::vectorize<int>(input->dims());
    std::vector<int> output_vec = pten::vectorize<int>(output->dims());
    ...
    input_vec = pten::vectorize<int>(transformed_input.dims());
    ...
    if (!is_sys_pad) {
      DDim transformed_output_shape(pten::make_ddim(transformed_output_vec));
      transformed_output.mutable_data<T>(transformed_output_shape,
                                         ctx.GetPlace());
    } else {
      output->mutable_data<T>(ctx.GetPlace());
      transformed_output.ShareDataWith(*output);
      transformed_output.Resize(pten::make_ddim(transformed_output_vec));
    }
    ...
    if (data_layout == platform::DataLayout::kNHWC) {
      ...
      output_nchw.Resize(pten::make_ddim(output_vec));
      if (strides.size() == 2U) {
...
class CUDNNConvTransposeGradOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    std::vector<int> input_vec = pten::vectorize<int>(input->dims());
    std::vector<int> output_vec = pten::vectorize<int>(output_grad->dims());
    if (data_layout == platform::DataLayout::kNHWC) {
      if (strides.size() == 2U) {
        std::vector<int> axis = {0, 3, 1, 2};
        for (size_t i = 0; i < axis.size(); ++i) {
    ...
    if (!is_sys_pad) {
      ...
      framework::DDim new_output_grad_shape(
          pten::make_ddim(new_output_grad_shape_vec));
    ...
    output_vec = pten::vectorize<int>(transformed_output_grad.dims());
    ...
    if (input_grad) {
      ...
      if (data_layout == platform::DataLayout::kNHWC) {
        ...
        input_grad_nchw.Resize(pten::make_ddim(input_vec));
        if (strides.size() == 2U) {
          std::vector<int> axis = {0, 2, 3, 1};
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    std::vector<int> output_vec =
        pten::vectorize<int>(transformed_dO_channel.dims());
    ...
    if (!is_sys_pad) {
      ...
      transformed_ddO_channel.Resize(pten::make_ddim(transformed_output_vec));
    }
