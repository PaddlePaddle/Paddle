...
class TreeConvKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    W.Resize(pten::flatten_to_2d(Filter->dims(), 2));
    ...
    auto edge_set_slicedim = pten::slice_ddim(
        Edges->dims(), 1, static_cast<int>(Edges->dims().size()));

    auto embedding_slicedim = pten::slice_ddim(
        Embeddings->dims(), 1, static_cast<int>(Embeddings->dims().size()));

    auto output_slicedim = pten::slice_ddim(
        output_emb->dims(), 1, static_cast<int>(output_emb->dims().size()));

    output_slicedim = pten::flatten_to_2d(output_slicedim, 1);
...
class TreeConvGradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    Tensor W;
    W.ShareDataWith(*Filter);
    W.Resize(pten::flatten_to_2d(Filter->dims(), 1));
    ...
    auto out_grad_dims = pten::slice_ddim(
        out_g->dims(), 1, static_cast<int>(out_g->dims().size()));
    out_grad_dims = pten::flatten_to_2d(out_grad_dims, 1);
    if (filter_g) {
      filter_g->mutable_data<T>(Filter->dims(), ctx.GetPlace());
      Tensor f_g;
      f_g.ShareDataWith(*filter_g);
      f_g.Resize(pten::flatten_to_2d(Filter->dims(), 2));
    ...
    if (in_g) {
      auto input_grad_dims = pten::slice_ddim(
