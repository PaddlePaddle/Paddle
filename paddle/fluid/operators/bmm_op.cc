...
class BmmOp : public framework::OperatorWithKernel {
  public:
    ...
    void InferShape(framework::InferShapeContext* ctx) const override {
      ...
      PADDLE_ENFORCE_EQ(
          x_dims[2], y_dims[1],
          "First matrix's width must be equal with second matrix's height in BmmOp.");
  ...
  public:
    ...
    void InferShape(framework::InferShapeContext* ctx) const override {
      ...
      PADDLE_ENFORCE_EQ(
          ...
          platform::errors::NotFound("Input(Y) of BmmOp should not be null"));
      PADDLE_ENFORCE_EQ(
          ctx->HasOutput(framework::GradVarName("Out")), true,
          platform::errors::NotFound("Output(Out@GRAD) of BmmOp should not be null."));
