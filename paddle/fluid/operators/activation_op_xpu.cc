...
void xpu_activation_forward(const framework::ExecutionContext &ctx,
  ...
  switch (type.type) {
      case xpu::Activation_t::HARD_SWISH: {
          ...
          PADDLE_ENFORCE_EQ(threshold, 6.0f, platform::errors::External("Not support threshold [%f] in XPU", threshold));
          PADDLE_ENFORCE_EQ(scale, 6.0f, platform::errors::External("Not support scale [%f] in XPU", scale));
          PADDLE_ENFORCE_EQ(offset, 3.0f, platform::errors::External("Not support offset [%f] in XPU", offset));

          r = xpu::hard_swish(xpu_context,
...
template <typename T>
using XPUHardSwishGradFunctor = XPUActivationGradFunc<T, xpu::Activation_t::HARD_SWISH>;
template <typename T>
...
REGISTER_ACTIVATION_XPU_KERNEL(hard_swish, XPUHardSwishFunctor, XPUHardSwishGradFunctor)
