...
static PyObject *imperative_conv2d_fusion_cutlass(PyObject *self,
                                                  PyObject *args,
                                                  PyObject *kwargs) {
  PyThreadState *tstate = nullptr;
  try {
    ...
    platform::RecordEvent op_type_record_event(
        "conv2d_fusion_cutlass pybind_imperative_func");
...
static PyObject *imperative_fused_multi_transformer(PyObject *self,
  ...
  try {
    ...
    auto RotaryPosEmb =
        GetVarBaseFromArgs(op_type, "RotaryPosEmb", args, 7, true);
    auto TimeStep = GetVarBaseFromArgs(op_type, "TimeStep", args, 8, true);
    auto SrcMask = GetVarBaseFromArgs(op_type, "SrcMask", args, 9, true);
    auto OutLinearW =
        GetVarBaseListFromArgs(op_type, "OutLinearW", args, 10, false);
    auto OutLinearBias =
        GetVarBaseListFromArgs(op_type, "OutLinearBias", args, 11, true);
    auto FFNLnScale =
        GetVarBaseListFromArgs(op_type, "FFNLnScale", args, 12, false);
    auto FFNLnBias =
        GetVarBaseListFromArgs(op_type, "FFNLnBias", args, 13, false);
    auto FFN1Weight =
        GetVarBaseListFromArgs(op_type, "FFN1Weight", args, 14, false);
    auto FFN1Bias = GetVarBaseListFromArgs(op_type, "FFN1Bias", args, 15, true);
    auto FFN2Weight =
        GetVarBaseListFromArgs(op_type, "FFN2Weight", args, 16, false);
    auto FFN2Bias = GetVarBaseListFromArgs(op_type, "FFN2Bias", args, 17, true);
    auto CacheKVOut =
        GetVarBaseListFromArgs(op_type, "CacheKVOut", args, 18, true);
    framework::AttributeMap attrs;
    ConstructAttrMapFromPyArgs(
        op_type, args, 19, PyTuple_GET_SIZE(args), attrs);
    tstate = PyEval_SaveThread();
...
static PyObject *imperative_moe(PyObject *self,
                                PyObject *args,
                                PyObject *kwargs) {
  PyThreadState *tstate = nullptr;
  try {
...
static PyMethodDef ExtestMethods[] = {
    ...
    {"conv2d_fusion_cutlass",
     (PyCFunction)(void (*)(void))imperative_conv2d_fusion_cutlass,
     METH_VARARGS | METH_KEYWORDS,
     "C++ interface function for conv2d_fusion_cutlass in dygraph."},
    {"number_count",
    ...
    {"moe",
     (PyCFunction)(void (*)(void))imperative_moe,
     METH_VARARGS | METH_KEYWORDS,
     "C++ interface function for moe in dygraph."},
    {"conj",
