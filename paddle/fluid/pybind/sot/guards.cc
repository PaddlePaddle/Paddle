...
bool LengthMatchGuard::check(PyObject* value) {
  if (PyDict_Check(value)) {
    return GetDictSizeFast(value) == expected_;
  }
...
bool AttributeMatchGuard::check(PyObject* value) {
  PyObject* attr = GetAttrFast(value, attr_name_);
  bool result = PyObject_Equal(attr, attr_ptr_);
  Py_XDECREF(attr);
  return result;
}
...
bool LayerMatchGuard::check(PyObject* value) {
  ...
  return CheckTrainingAttr(value, training_);
}
