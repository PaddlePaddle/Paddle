...
static PyObject *imperative_einsum(PyObject *self,
  ...
  try {
    ...
    auto InnerCacheNum =
        GetUnsignedLongFromArgs(op_type, "InnerCacheNum", args, 1, false);
    auto XShapeNum =
        GetUnsignedLongFromArgs(op_type, "XShapeNum", args, 2, false);
    ...
    imperative::NameVarBaseMap outs = {
        ...
        {"InnerCache", ConstructDuplicableOutput(InnerCacheNum)},
        {"XShape", ConstructDuplicableOutput(XShapeNum)}};
    ...
    return MakeReturnPyObject(
        std::make_tuple(outs["Out"][0], outs["InnerCache"], outs["XShape"]));
  } catch (...) {
    if (tstate) {
      PyEval_RestoreThread(tstate);
    }
...
static PyObject *imperative_mp_allreduce_sum(PyObject *self,
                                             PyObject *args,
                                             PyObject *kwargs) {
  PyThreadState *tstate = nullptr;
  try {
    std::string op_type = "mp_allreduce_sum";
    platform::RecordEvent op_type_record_event(
        "mp_allreduce_sum pybind_imperative_func");
...
static PyObject *imperative_mp_allreduce_sum_(PyObject *self,
                                              PyObject *args,
                                              PyObject *kwargs) {
  PyThreadState *tstate = nullptr;
  try {
...
static PyObject *imperative_fused_multi_transformer_int8(PyObject *self,
  ...
  try {
    ...
    auto QKVOutScale =
        GetVarBaseFromArgs(op_type, "QKVOutScale", args, 16, true);
    auto OutLinearOutScale =
        GetVarBaseFromArgs(op_type, "OutLinearOutScale", args, 17, true);
    auto FFN1OutScale =
        GetVarBaseFromArgs(op_type, "FFN1OutScale", args, 18, true);
    auto FFN2OutScale =
        GetVarBaseFromArgs(op_type, "FFN2OutScale", args, 19, true);
    auto CacheKVOut =
...
static PyMethodDef ExtestMethods[] = {
    ...
    {"mp_allreduce_sum",
     (PyCFunction)(void (*)(void))imperative_mp_allreduce_sum,
     METH_VARARGS | METH_KEYWORDS,
     "C++ interface function for mp_allreduce_sum in dygraph."},
    {"mp_allreduce_sum_",
     (PyCFunction)(void (*)(void))imperative_mp_allreduce_sum_,
     METH_VARARGS | METH_KEYWORDS,
