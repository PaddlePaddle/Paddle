...
PyObject* StreamBaseNew(PyTypeObject* type, PyObject* args, PyObject* kwargs) {
  ...
  if (obj) {
    ...
    if (platform::is_cpu_place(p)) {
      ...
      v->stream =
          paddle::platform::CreateDeviceContext<phi::OneDNNContext>(p, true)
              .release();
#else
      v->stream =
          paddle::platform::CreateDeviceContext<phi::CPUContext>(p, true)
              .release();
#endif
      ...
      //       v->stream =
      //       paddle::platform::CreateDeviceContext<CustomDeviceContext>(p,
      //       true).release();
      // #else
      //       PADDLE_THROW(platform::errors::Unimplemented(
