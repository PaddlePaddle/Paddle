...
void BindDistributed(py::module *m) {
      ...
      *m, "ProcessGroupNCCL", ProcessGroup)
      ...
      .def(
          ...
          [](distributed::ProcessGroupNCCL &self,
            ...
            // The window covers the whole underlying allocation rather than
            // just the logical bytes of the tensor: core.nccl_mem_alloc() pads
            // the allocation up to kNCCLWindowAlignment, so a tensor whose
            // shape is not a multiple of the alignment is still registrable.
            // Requiring the tensor to own its allocation from offset 0 keeps
            // this from ever registering memory the tensor does not own.
            PADDLE_ENFORCE_EQ(
  ...
  // ProcessGroupNCCL.register_comm_buffer(). Buffers coming from Paddle's own
  // allocator generally do not meet that alignment requirement.
#if defined(PADDLE_WITH_NCCL) && NCCL_VERSION_CODE >= 21900
  m->def(
      "nccl_mem_alloc",
      [](const std::vector<int64_t> &shape, phi::DataType dtype) -> py::object {
        PADDLE_ENFORCE_EQ(
            ...
            common::errors::Unavailable(
                ...
                "which is required to allocate registrable communication "
                "buffers."));
        size_t numel = 1;
        for (auto dim : shape) {
        ...
        size_t nbytes = numel * phi::SizeOf(dtype);
        ...
        // padded size, so register_comm_buffer() can hand the whole aligned
        // region to NCCL without the caller ever seeing the padding.
