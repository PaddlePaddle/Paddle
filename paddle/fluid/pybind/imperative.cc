...
#undef TENSOR_TO_PY_SCALAR
      ...
      .def("cpu",
           [](const std::shared_ptr<imperative::VarBase> &self, bool blocking) {
             if (platform::is_cpu_place(self->Place())) {
               return self;
             } else {
               auto new_var = self->NewVarBase(platform::CPUPlace(), blocking);
  ...
  m.def("async_read", [](imperative::VarBase &src, imperative::VarBase &dst,
    ...
    auto &dst_tensor_ptr = dst.Var().Get<framework::LoDTensor>();
    auto &buffer_tensor_ptr = buffer.Var().Get<framework::LoDTensor>();
    const auto &deviceId = paddle::platform::GetCurrentDeviceId();
    auto stream =
        paddle::platform::stream::get_current_stream(deviceId)->raw_stream();
    // VLOG(0) << "Finish INIT";
    // VLOG(0) << "buffer place:" << buffer_tensor_ptr.place();
    // VLOG(0) << "src place:" << src_tensor.place();
    // VLOG(0) << "index place:" << index_tensor.place();
    // VLOG(0) << "dst place:" << dst_tensor_ptr.place();
    ...
    auto index_select = [](const framework::Tensor &src_tensor,
                           ...
                           const framework::Tensor &buffer_tensor_ptr) {
      // VLOG(0) << "Enter index_select function";
      auto *src_data = src_tensor.data<float>();
      auto *index_data = index_tensor.data<int64_t>();
      auto *buffer_data = buffer_tensor_ptr.data<float>();
      ...
      int c = 0;
      // VLOG(0) << "Finish index_select INIT";
      // VLOG(0) << index_tensor.numel();
    ...
    index_select(src_tensor, index_tensor, buffer_tensor_ptr);
    // VLOG(0) << "Index Select";

    auto *dst_data = dst_tensor_ptr.data<float>();
    auto *dst_data_new = const_cast<float *>(dst_data);
    int64_t size = src_tensor.numel() / src_tensor.dims()[0];
    // Copy the data to device memory
    // VLOG(0) << "Begin CUDA Copy";
    cudaMemcpyAsync(dst_data_new, buffer_tensor_ptr.data<float>(),
