...
T TensorGetElement(const framework::Tensor &self, size_t offset) {
  ...
  if (platform::is_cpu_place(self.place())) {
    ...
    auto p = BOOST_GET(platform::CUDAPlace, self.place());
...
inline framework::Tensor *_getTensor(const framework::Tensor &self,
  ...
  if (platform::is_cpu_place(place)) {
    output->mutable_data(BOOST_GET(platform::CPUPlace, place), self.type());
#ifdef PADDLE_WITH_CUDA
  } else {
    if (platform::is_cuda_pinned_place(place)) {
      output->mutable_data(BOOST_GET(platform::CUDAPinnedPlace, place),
                           self.type());
    } else if ((platform::is_gpu_place(place))) {
      output->mutable_data(BOOST_GET(platform::CUDAPlace, place), self.type());
    }
#endif
  }
  return output;
