  ...
  m.def("get_fetch_variable",
        [](const Scope &scope, const std::string &var_name,
          ...
          if (data_is_lod_tensor(var)) {
            return py::cast(BOOST_GET_CONST(LoDTensor, var));
          } else {
            return py::cast(BOOST_GET_CONST(LoDTensorArray, var));
          }
        });
  ...
  py::class_<FetchList>(m, "FetchList", R"DOC( FetchList is a
           ...
           [](FetchList &self) -> py::list {
             ...
             for (size_t i = 0; i < self.size(); ++i) {
               if (data_is_lod_tensor(self[i])) {
                 auto &data = BOOST_GET_CONST(LoDTensor, self[i]);
                 res[i] = py::cast(std::move(data));
               } else {
                 auto &data = BOOST_GET_CONST(LoDTensorArray, self[i]);
      ...
      .def("append",
           [](FetchList &self, const LoDTensor &t) {
             self.emplace_back();
             auto &lod_tensor = BOOST_GET_CONST(LoDTensor, self.back());
      ...
      .def("append",
           [](FetchList &self, const LoDTensorArray &t) {
             self.emplace_back();
             auto &lod_tensor_array =
                 BOOST_GET_CONST(LoDTensorArray, self.back());
             ...
             for (size_t i = 0; i < self.size(); ++i) {
               ...
               for (size_t j = 0; j < self[i].size(); ++j) {
                 if (data_is_lod_tensor(self[i][j])) {
                   auto &var = BOOST_GET_CONST(LoDTensor, self[i][j]);
                   tmp[j] = py::cast(std::move(var));
                 } else {
                   auto &var = BOOST_GET_CONST(LoDTensorArray, self[i][j]);
           ...
           [](ParallelExecutor &self,
             ...
             if (return_merged) {
               return py::cast(std::move(
                   BOOST_GET_CONST(paddle::framework::FetchList, ret)));
             } else {
               return py::cast(std::move(
                   BOOST_GET_CONST(paddle::framework::FetchUnmergedList, ret)));
             }
           })
