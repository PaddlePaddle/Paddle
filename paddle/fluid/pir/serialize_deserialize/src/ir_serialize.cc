...
Json ProgramWriter::WriteProgram(const pir::Program* program){
    ...
    VLOG(0) << "Finish write program.";
    return program_json;
}

Json ProgramWriter::WriteRegion(const pir::Region* region, const std::string &region_name){
    Json region_json;
    ...
    VLOG(0) << "Finish write " << region_name;
    return region_json;
}

Json ProgramWriter::WriteBlock(const pir::Block* block, const std::string& block_name){
    Json block_json;
    ...
    VLOG(0) << "Finish write " << block_name;
    return block_json;
}
...
Json ProgramWriter::WriteBlockArg(const pir::Value& value){
    ...
    VLOG(0) << "Finish write blockargument " << blockarg_id_ ;
    blockarg_id_--;
...
Json ProgramWriter::WriteValue(const pir::Value& value){
    ...
    var_json["type"] = var;
    VLOG(0) << "Finish write value " << value_id_ ;
...
Json ProgramWriter::WriteOp(const pir::Operation& op){
    ...
    VLOG(0) << "Finish write Operation " << op.name() ;
    return op_json;
}
...
Json ProgramWriter::WriteOpOperand(const pir::OpOperand& op_operand){
    ...
    operand_json["id"] = id;
    VLOG(0) << "Finish write OpOperand " << id;
    return operand_json;
}
...
Json ProgramWriter::WriteAttributesMap_1(const AttributeMap& attr_map){
    ...
    for (auto& attr : attr_map){
        if (attr.first == "stop_gradient" || attr.first == "persistable"){
            operesult_attrs_json.emplace_back(WriteAttribute(attr.first, attr.second));
        }
    }
    VLOG(0) << "Finish write AttributeMap " ;
...
Json ProgramWriter::WriteAttributesMap_0(const AttributeMap& attr_map){
    ...
    VLOG(0) << "Finish write AttributeMap_0 " ;
    return attrs_json;
}

Json ProgramWriter::WriteAttribute(const std::string& op_attr_name, const pir::Attribute& attr){
    Json attr_json;
    attr_json["name"] = op_attr_name;
    attr_json["attr_type"] = pir::writeAttr(attr);
    
    VLOG(0) << "Finish write Attribute. " ;
    return attr_json;
}
...
void WriteModule(const pir::Program &program, const std::string& file_path, const uint64_t &pir_version, bool overwrite)
{
    ...
    std::string total_str = total.dump(4);
