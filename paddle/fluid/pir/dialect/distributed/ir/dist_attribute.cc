...
const std::vector<std::vector<int64_t>>& TensorDistAttribute::dims_mapping()
    const {
...
phi::distributed::Placements TensorDistAttribute::placements() const {
  ...
  for (size_t t_dim = 0; t_dim < dim_mapping.size(); t_dim++) {
    ...
    for (size_t idx = 0; idx < m_dims.size(); idx++) {
      ...
      placements[mesh_dim] =
          is_co_shard ? std::make_shared<phi::distributed::CoShard>(t_dim, idx)
...
TensorDistAttribute TensorDistAttribute::get(
  ...
  if (!placements.has_value() && !mesh.empty()) {
    ...
    return Base::get(
        ctx, mesh, dims_mapping, split_factor, partial_status, placements);
  }
}
