...
static std::unordered_map<std::string, std::string> kTypeMap = {
...
OpInfoTuple GetPirOpInfo(const std::string& pir_op_name) {
  const auto& op_meta =
  ...
  for (const auto& op_attr : op_attrs) {
    ...
    if (kTypeMap.find(attr_type_str) == kTypeMap.end()) {
    ...
    std::string attr_pir_type = kTypeMap[attr_type_str];
...
void CustomOpDialect::RegisterCustomOp(const paddle::OpMetaInfo& op_meta) {
  ...
  std::vector<pir::InterfaceValue> interface_values;
  pir::TypeId op_info_typeid = pir::TypeId::get<OpYamlInfoInterface>();
  void* op_info_model = malloc(sizeof(OpYamlInfoInterface::Concept));
  new (op_info_model) OpYamlInfoInterface::Concept(GetPirOpInfo);
  pir::InterfaceValue op_info_interface(op_info_typeid, op_info_model);
  interface_values.push_back(std::move(op_info_interface));
