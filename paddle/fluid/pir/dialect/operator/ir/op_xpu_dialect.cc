...
XpuOperatorDialect::XpuOperatorDialect(pir::IrContext *ctx)
    : pir::Dialect(name(), ctx, pir::TypeId::get<XpuOperatorDialect>()) {
  initialize();
}

void XpuOperatorDialect::initialize() {
  ...
  RegisterOps<
#define GET_OP_LIST1
#include "paddle/fluid/pir/dialect/operator/ir/xpu_op_info.cc"  // NOLINT
      >();
  RegisterOps<
#define GET_OP_LIST2
#include "paddle/fluid/pir/dialect/operator/ir/xpu_op_info.cc"  // NOLINT
...
void XpuOperatorDialect::PrintType(pir::Type type, std::ostream &os) const {
...
void XpuOperatorDialect::PrintAttribute(pir::Attribute attr,
...
pir::Attribute XpuOperatorDialect::ParseAttribute(
...
pir::OpPrintFn XpuOperatorDialect::PrintOperation(pir::Operation *op) const {
...
IR_DEFINE_EXPLICIT_TYPE_ID(paddle::dialect::XpuOperatorDialect)
