...
bool ApplyPerChannelScaleOpInferSymbolicShape(
  ...
  PADDLE_ENFORCE_EQ(x_shape.shape().size(),
                    2,
                    phi::errors::InvalidArgument(
                        "The rank of Input(x) must be 2, but received %u.",
                        x_shape.shape().size()));

  PADDLE_ENFORCE_EQ(scales_shape.shape().size(),
                    1,
                    phi::errors::InvalidArgument(
  ...
  bool Atan2OpInferSymbolicShape(
      pir::Operation * op, pir::InferSymbolicShapeContext * infer_context) {
    const auto x_shape =
        infer_context->GetShapeOrDataForValue(op->operand_source(0)).shape();
    const auto y_shape =
  ...
  bool BceLossOpInferSymbolicShape(
    ...
    for (int i = 0; i < rank; ++i) {
      infer_context->AddEqualCstr(input_shape.shape()[i],
                                  label_shape.shape()[i]);
    }
  ...
  bool Conv2dOpInferSymbolicShape(
    ...
    const std::vector<symbol::DimExpr> in_data_dims =
        channel_last
            ? std::vector<symbol::DimExpr>(in_s_or_d.shape().begin() + 1,
