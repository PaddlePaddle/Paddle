...
bool OverlapAddOpInferSymbolicShape(
  ...
  int end_axis = 0;
  if (axis == 0) {
    n_frames = static_cast<int64_t>(x_dims[0].Get<std::int64_t>());
    frame_length = static_cast<int64_t>(x_dims[0].Get<std::int64_t>());
    start_axis = 2;
    end_axis = x_rank - 1;
  } else {
    n_frames = static_cast<int64_t>(x_dims[x_rank - 1].Get<std::int64_t>());
  ...
  if (axis == 0) {
    // (seq_length, ...)
    output_shape.insert(output_shape.begin(), symbol::DimExpr(seq_length));
  } else {
    // (..., seq_length)
    output_shape.push_back(symbol::DimExpr(seq_length));
  }
...
// bool UniformRandomInplaceOpInferSymbolicShape(
//     pir::Operation *op, pir::InferSymbolicShapeContext *infer_context) {
//   // pass
//   return true;
// }

// bool UniformRandomInplace_OpInferSymbolicShape(
//     pir::Operation *op, pir::InferSymbolicShapeContext *infer_context) {
//   // pass
