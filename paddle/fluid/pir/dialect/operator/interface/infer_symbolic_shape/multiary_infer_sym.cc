...
bool PyramidHashOpInferSymbolicShape(pir::Operation *op, pir::InferSymbolicShapeContext *infer_context) {
  const auto &x_shape_or_data = infer_context->GetShapeOrDataForValue(op->operand_source(0));
  const auto &w_shape_or_data = infer_context->GetShapeOrDataForValue(op->operand_source(1));

  std::vector<symbol::DimExpr> x_shape = x_shape_or_data.shape();
  std::vector<symbol::DimExpr> w_shape = w_shape_or_data.shape();

  PADDLE_ENFORCE_EQ(x_shape.size(), 2, common::errors::InvalidArgument(
      "The rank of Input(X) of PyramidHashOP is invalid. It should be 2, but got %d", x_shape.size()));

  PADDLE_ENFORCE_EQ(w_shape.size(), 2, common::errors::InvalidArgument(
      "The rank of Input(W) of PyramidHashOP is invalid. It should be 2, but got %d", w_shape.size()));
  ...
  int white_list_len = op->attribute<pir::Int32Attribute>("white_list_len").data();
  int black_list_len = op->attribute<pir::Int32Attribute>("black_list_len").data();

  infer_context->AddEqualCstr(w_shape[0], symbol::DimExpr(space_len + rand_len));
  infer_context->AddEqualCstr(w_shape[1], symbol::DimExpr(1));

  PADDLE_ENFORCE_EQ(num_emb % rand_len, 0, common::errors::InvalidArgument(
      "The PyramidHashOP’s Attr(num_emb) should mod Attr(rand_len), but num_emb is %d, rand_len is %d", num_emb, rand_len));

  // Handle white_list constraints if present
  if (white_list_len > 0) {
    const auto &white_list_shape_or_data = infer_context->GetShapeOrDataForValue(op->operand_source(2));
    if(white_list_shape_or_data.isa<symbol::NullShapeOrDataDimExpr>())
        PADDLE_THROW("Input(WhiteList) of PyramidHashOP is not "
                                 "found but white_list_len > 0.");
    else{
        std::vector<symbol::DimExpr> wl_shape = white_list_shape_or_data.shape();
        PADDLE_ENFORCE_EQ(wl_shape.size(), 2, common::errors::InvalidArgument(
            "The rank of Input(WhiteList) of PyramidHashOP is invalid. It should be 2, but got %d", wl_shape.size()));
  ...
  if (black_list_len > 0) {
    const auto &black_list_shape_or_data = infer_context->GetShapeOrDataForValue(op->operand_source(3));
    std::vector<symbol::DimExpr> bl_dims = black_list_shape_or_data.shape();

    PADDLE_ENFORCE_EQ(bl_dims.size(), 2, common::errors::InvalidArgument(
        "The rank of Input(BlackList) of PyramidHashOP is invalid. It should be 2, but got %d", bl_dims.size()));
  ...
  infer_context->SetShapeOrDataForValue(op->result(0), symbol::ShapeOrDataDimExprs{symbol::TensorShapeOrDataDimExprs(out_shape)});
