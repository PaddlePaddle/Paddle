...
class CodeGen:
    ...
    def _gen_api_inputs(self, op_info):
        ...
        for name, type, optional in zip(name_list, type_list, optional_list):
            if optional == 'true':
                ret.append(f'{OPTIONAL_TYPE_MAP[type]} {name}')
            else:
                ret.append(f'{TYPE_MAP[type]} {name}')
        return ', '.join(ret)
        ...
        if output_num > 1:
            ...
            for name, type, intermediate in zip(
                ...
                if self._is_optinonal_output(op_info, name):
                    ret.append(OPTIONAL_TYPE_MAP[type])
                else:
                    ret.append(TYPE_MAP[type])
            return 'std::tuple<{}>'.format(', '.join(ret))
        elif output_num == 1:
            index = intermediate_list.index('false')
            name = name_list[index]
            if self._is_optinonal_output(op_info, name):
                return OPTIONAL_TYPE_MAP[type_list[index]]
            else:
                return TYPE_MAP[type_list[index]]
        elif output_num == 0:
            return 'void'
        ...
        for name, optional, type in zip(name_list, optional_list, type_list):
            if optional == 'true':
                if VECTOR_TYPE in type:
                    ret += OPTIONAL_VECTOR_OPRESULT_INPUT_TEMPLATE.format(
                        name=name
                    )
                else:
                    ret += OPTIONAL_OPRESULT_INPUT_TEMPLATE.format(name=name)
        return ret
