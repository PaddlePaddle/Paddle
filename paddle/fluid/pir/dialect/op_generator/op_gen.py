...
OpInfoTuple {op_name}::GetOpInfo() {{
  ...
  std::unordered_map<std::string, std::vector<std::string>> kernel_select_rules = {{{kernel_select_rules}}};
  return std::make_tuple(inputs, attributes, outputs, run_time_info, kernel_select_rules, "{origin_op_name}");
}}
"""
    ...
    for key, op_info in op_info_items.items():
        ...
        for op_name in op_info.op_phi_name:
            ...
            if op_kernel_map is not None:
                ...
                if 'dispatch' in op_kernel_map:
                    kernel_select_rules_list = []
                    for key, value in op_kernel_map['dispatch'].items():
                        if value is not None:
                            ...
                            select_str = (
                                '{{"{kernel}", {{ "{args}" }}}}'.format(
                                    kernel=key, args='", "'.join(arg_list)
                                )
                            )
                            kernel_select_rules_list.append(select_str)
                    kernel_select_rules_str = ", ".join(
                        kernel_select_rules_list
                    )
            ...
            op_info_func_str = OP_INFO_TEMPLATE.format(
                ...
                inplace=inplace_str,
                view=view_str,
                kernel_select_rules=kernel_select_rules_str,
