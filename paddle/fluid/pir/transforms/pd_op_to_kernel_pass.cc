    ...
    for (size_t i = 0; i < op_item->num_results(); ++i) {
      ...
      if (!result_type) {
        ...
        if (kernel_key.backend() == phi::Backend::ONEDNN) {
          ...
          op_output_types.push_back(
              BuildOutputType(result_type, out_place, ctx));
        }
#else
        ...
        for (auto& base_type : base_types) {
          if (base_type) {
            ...
            auto allocated_dense_tensor_dtype = AllocatedDenseTensorType::get(
...
void ProcessBlock(
  ...
  for (auto iter = block->begin(); iter != block->end(); ++iter) {
    ...
    auto op_output_types =
        BuildOutputs(op_item, kernel_name, kernel_key, ctx, new_vec_inputs);
