      ...
      if (shadow_value.use_count() >= 1 || val_src_place == phi::CPUPlace()) {
        ...
        for (auto use_it = shadow_value.use_begin();
          ...
          if (op_info) {
            ...
            VLOG(4) << "shadow value index" << val_index
                    << input_infos[val_index].is_mutable_attribute;
        ...
        if (all_use_is_scalar) {
          ...
          VLOG(4) << "Reset shadow_feed dst_place_type to 0 for scalar use";
