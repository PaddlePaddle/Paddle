...
class Conv2dAddXpuFusePattern : public paddle::drr::DrrPatternBase {
 ...
 public:
  ...
  void operator()(paddle::drr::DrrPatternContext *ctx) const override {
    VLOG(5) << "lyf -- [Conv2dAddFusePattern] start.";
    ...
    VLOG(5) << "lyf -- [Conv2dAddFusePattern] conv2d.";
    // add
    ...
    pat.AddConstraint([](const paddle::drr::MatchContext &match_ctx) -> bool {
      ...
      VLOG(5) << "lyf -- [Conv2dAddFusePattern] conv_input_shape: "
              << conv_input_shape.size();
      VLOG(5) << "lyf -- [Conv2dAddFusePattern] filter_shape: "
              << filter_shape.size();
    ...
    VLOG(5) << "lyf -- [Conv2dAddFusePattern] add.";
    ...
    const auto &scale_attr = res.ComputeAttr(
        [](const paddle::drr::MatchContext &match_ctx) -> int32_t {
          ...
          if (filter_dtype.isa<pir::Int16Type>()) {
            return 32767;
          } else if (filter_dtype.isa<pir::Int8Type>()) {
            return 127;
          } else {
            return 0;
          }
        });
    VLOG(5) << "lyf -- [Conv2dAddFusePattern] bit_length: ";
    // reshape scale shape
    ...
    if (!enable_int8_) {
      if (quant_post_type_.find("conv2d") != quant_post_type_.end() &&
        ...
        res.Tensor("filter_fp_max") = max_op1(res.Tensor("filter"));
        ...
        // find filter_quant_max
        res.Tensor("filter_max") = max_op1(res.Tensor("filter_quant"));
        // fp32/int16
        res.Tensor("scale_max_tmp") = div(full1(), res.Tensor("filter_max"));
        // cast fp32->int16
        res.Tensor("scale_max") = cast_op(res.Tensor("scale_max_tmp"));
        ...
        const auto &cast_op = res.Op(paddle::dialect::CastOp::name(),
                                     {{"dtype", res.DataTypeAttr("int16")}});
        res.Tensor("filter_quant") = cast_op(res.Tensor("filter_quant_tmp"));
        // find filter_quant_max,  per-channel size, not expand
        res.Tensor("filter_max") = max_op1(res.Tensor("filter_quant"));
        // fp32/int16
        res.Tensor("scale_max_tmp") = div(full1(), res.Tensor("filter_max"));
        // cast fp32->int16
...
class Conv2dAddFusePass : public pir::PatternRewritePass {
 public:
  ...
  pir::RewritePatternSet InitializePatterns(pir::IrContext *context) override {
    ...
    VLOG(5) << "lyf -- quant_post_type.find("
               ")->second:";
