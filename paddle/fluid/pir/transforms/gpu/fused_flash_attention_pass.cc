...
class TransposeSliceFlashAttnPattern : public paddle::drr::DrrPatternBase {
 public:
  ...
  void operator()(paddle::drr::DrrPatternContext *ctx) const override {
    ...
    paddle::drr::SourcePattern src = ctx->SourcePattern();
    // transpose
    const auto &transpose_qkv = src.Op("pd_op.transpose",
                                       {{"perm", src.Attr("perm")}});
    src.Tensor("qkv_transpose") = transpose_qkv(src.Tensor("qkv"));
    // slice q -> [b, head, s, head_dim]
    const auto &slice_q = src.Op(paddle::dialect::SliceOp::name(),
    ...
    src.Tensor("q") = slice_q(src.Tensor("qkv_transpose"), full_int_array_q1(), full_int_array_q2());
    // slice k -> [b, head, s, head_dim]
    const auto &slice_k = src.Op(paddle::dialect::SliceOp::name(),
    ...
    src.Tensor("k") = slice_k(src.Tensor("qkv_transpose"), full_int_array_k1(), full_int_array_k2());
    // slice v -> [b, head, s, head_dim]
    const auto &slice_v = src.Op(paddle::dialect::SliceOp::name(),
    ...
    src.Tensor("v") = slice_v(src.Tensor("qkv_transpose"), full_int_array_v1(), full_int_array_v2());
    ...
    const auto &full_scale =
        src.Op("pd_op.full", {{"value", src.Attr("scale_out_value")}});
    src.Tensor("qk_scale_out") =
        scale_out(src.Tensor("qk_out"), full_scale());
    ...
    const auto &context_matmul =
        src.Op("pd_op.matmul",
               {{"transpose_x", src.Attr("context_matmul_transpose_x")},
                {"transpose_y", src.Attr("context_matmul_transpose_y")}});
    src.Tensor("context_matmul_out") = context_matmul(
        src.Tensor("softmax_out"), src.Tensor("v"));
    ...
    src.RequireNativeCall(
        [](const paddle::drr::MatchContext &match_ctx) -> bool {
          ...
          // tensor shape
          auto q =
              pir::GetShapeFromValue(match_ctx.Tensor("q"));
          auto k =
              pir::GetShapeFromValue(match_ctx.Tensor("k"));
          auto v =
              pir::GetShapeFromValue(match_ctx.Tensor("v"));
    ...
    // transpose
    const auto &transpose_qkv_res = res.Op("pd_op.transpose",
                                           {{"perm", perm_res}});
    ...
    const auto &infer_flags_q_out = res.ComputeAttr(
        [](const paddle::drr::MatchContext &match_ctx) -> std::vector<int64_t> {
          auto infer_flags_q = match_ctx.Attr<std::vector<int64_t>>("infer_flags_q");
          return infer_flags_q;
        });
    const auto &decrease_axis_q_out = res.ComputeAttr(
        [](const paddle::drr::MatchContext &match_ctx) -> std::vector<int64_t> {
          auto decrease_axis_q = match_ctx.Attr<std::vector<int64_t>>("decrease_axis_q");
    ...
    res.Tensor("q") = slice_q_res(res.Tensor("qkv_transpose"), res.OutputNoneTensor(), res.OutputNoneTensor());
    // slice k -> [b, head, s, head_dim]
    ...
    const auto &infer_flags_k_out = res.ComputeAttr(
        [](const paddle::drr::MatchContext &match_ctx) -> std::vector<int64_t> {
          auto infer_flags_k = match_ctx.Attr<std::vector<int64_t>>("infer_flags_k");
          return infer_flags_k;
        });
    const auto &decrease_axis_k_out = res.ComputeAttr(
        [](const paddle::drr::MatchContext &match_ctx) -> std::vector<int64_t> {
          auto decrease_axis_k = match_ctx.Attr<std::vector<int64_t>>("decrease_axis_k");
    ...
    res.Tensor("k") = slice_k_res(res.Tensor("qkv_transpose"), res.OutputNoneTensor(), res.OutputNoneTensor());
    // slice v -> [b, head, s, head_dim]
    ...
    const auto &infer_flags_v_out = res.ComputeAttr(
        [](const paddle::drr::MatchContext &match_ctx) -> std::vector<int64_t> {
          auto infer_flags_v = match_ctx.Attr<std::vector<int64_t>>("infer_flags_v");
          return infer_flags_v;
        });
    const auto &decrease_axis_v_out = res.ComputeAttr(
        [](const paddle::drr::MatchContext &match_ctx) -> std::vector<int64_t> {
          auto decrease_axis_v = match_ctx.Attr<std::vector<int64_t>>("decrease_axis_v");
    ...
    res.Tensor("v") = slice_v_res(res.Tensor("qkv_transpose"), res.OutputNoneTensor(), res.OutputNoneTensor());
    printf("here");
    const auto &flash_attn = res.Op("pd_op.flash_attn",
                                    {{{"dropout", res.Float32Attr(0.0)},
                                      {"causal", res.BoolAttr(false)},
