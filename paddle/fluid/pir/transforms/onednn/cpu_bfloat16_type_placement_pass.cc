...
namespace pir {
class CpuBfloat16TypePattern : public RewritePattern {
 public:
  explicit CpuBfloat16TypePattern(IrContext* context)
  ...
  void Rewrite(Operation* op,
    ...
    auto op_info = IrContext::Instance()->GetRegisteredOpInfo(op->name());
    if (op_info) {
      for (size_t i = 0; i < op->num_results(); ++i) {
        Type type = op->result_type(i);
        if (type.isa<paddle::dialect::DenseTensorType>()) {
          ...
          auto new_vec_type =
              VectorType::get(rewriter.ir_context(), results_type);
...
class OneDNNBf16TypePass : public PatternRewritePass {
 public:
  OneDNNBf16TypePass()
      : PatternRewritePass("cpu_bfloat16_type_placement_pass", 2) {}
...
REGISTER_IR_PASS(cpu_bfloat16_type_placement_pass, pir::OneDNNBf16TypePass);
