...
static Type create_type(Type type, Type out_dtype, IrContext *ctx) {
 ...
 public:
  ...
  bool MatchAndRewrite(OpType op,
    ...
    if (!(target_op_name == "onednn_op.cast" ||
      ...
      return false;
    auto *pre_op = GetDefiningOpForInput(op, 0);
    ...
    std::string mkldnn_dtype =
        mkldnn_data_type.template dyn_cast<StrAttribute>().AsString();
    auto onednn_data_type = attributes["onednn_data_type"];
    std::string onednn_dtype =
        onednn_data_type.template dyn_cast<StrAttribute>().AsString();
    if (mkldnn_dtype != "bfloat16" && onednn_dtype != "bfloat16") return false;

    IrContext *ctx = rewriter.ir_context();
    ...
    if (dtype == DataType::FLOAT32) {
      Attribute new_dtype =
          paddle::dialect::DataTypeAttribute::get(ctx, DataType::BFLOAT16);
      attributes["dtype"] = new_dtype;
    } else {
      return false;
    }

    std::unordered_map<std::string, Attribute> q_attributes;
    ...
    Type new_type =
        create_type<DenseTensorType, paddle::dialect::DenseTensorType>(
            type, BFloat16Type::get(ctx), ctx);
...
class ConcatBf16QuantizePattern
    : public OpRewritePattern<paddle::onednn::dialect::ConcatOp> {
 public:
  using OpRewritePattern<paddle::onednn::dialect::ConcatOp>::OpRewritePattern;
  bool MatchAndRewrite(paddle::onednn::dialect::ConcatOp op,
                       PatternRewriter &rewriter) const override {  // NOLINT
    // The input should come from combine.
    CombineOp pre_op = GetDefiningOpForInput(op, 0)->dyn_cast<CombineOp>();
    ...
    for (size_t idx = 0; idx < combine_inputs.size(); idx++) {
      ...
      auto pre_pre_op = GetDefiningOpForInput(pre_op, idx);
    ...
    std::vector<Value> new_combine_inputs(combine_inputs.size());
    for (size_t idx = 0; idx < combine_inputs.size(); idx++) {
    ...
    // Create new combine
    CombineOp new_combine = rewriter.Build<CombineOp>(new_combine_inputs);
...
class SplitSliceBf16QuantizePattern
    : public OpRewritePattern<paddle::onednn::dialect::SplitOp> {
 public:
  using OpRewritePattern<paddle::onednn::dialect::SplitOp>::OpRewritePattern;
  bool MatchAndRewrite(paddle::onednn::dialect::SplitOp op,
                       PatternRewriter &rewriter) const override {  // NOLINT
    /**
    ...
    for (auto i = 0; i < static_cast<int>(next_op_list.size()); i++) {
      SliceOp next_op = (next_op_list[i].first)->dyn_cast<SliceOp>();
      if (!next_op) {
        return false;
      }
    }
    ...
    Type new_type_quantize =
        create_type<DenseTensorType, paddle::dialect::DenseTensorType>(
            quantize_type_, BFloat16Type::get(ctx), ctx);
    ...
    for (auto i = 0; i < static_cast<int>(next_op_list.size()); i++) {
      ...
      auto index =
          next_op->attribute("index").dyn_cast<Int32Attribute>().data();
      auto input_type = next_op->operand(0).type().dyn_cast<VectorType>();
      auto new_type_ = input_type[index];
      Type new_type_slice =
          create_type<DenseTensorType, paddle::dialect::DenseTensorType>(
              new_type_, BFloat16Type::get(ctx), ctx);

      auto slice_info = ctx->GetRegisteredOpInfo(SliceOp::name());

      std::vector<Type> op_item_inner_output_types;
      op_item_inner_output_types.push_back(new_type_slice);

      auto slice_attributes = next_op->attributes();
      Operation *new_slice = rewriter
...
class SplitdoubleBf16QuantizePattern
 ...
 public:
  ...
  bool MatchAndRewrite(paddle::onednn::dialect::SplitOp op,
    ...
    auto next_op_list = GetUseOpsForOutput(op, 0);
    for (auto i = 0; i < static_cast<int>(next_op_list.size()); i++) {
      SplitOp next_op = (next_op_list[i].first)->dyn_cast<SplitOp>();
      if (!next_op) {
        return false;
      }
    }
    ...
    for (size_t i = 0; i < next_op_list.size(); i++) {
      ...
      SplitOp next_split_op = rewriter.Build<SplitOp>(split_op->result(0));
...
class CPUSpecialOpsBf16Pass : public PatternRewritePass {
 public:
  CPUSpecialOpsBf16Pass()
      : PatternRewritePass("cpu_special_ops_bf16_pass", 2) {}
...
REGISTER_IR_PASS(cpu_special_ops_bf16_pass, pir::CPUSpecialOpsBf16Pass);
