...
class DeleteQuantDequantLinearOpBasePattern
 ...
 public:
  ...
  bool Constraint(const paddle::drr::MatchContext& match_ctx) const {
    ...
    auto input_scale_dtype = GetDataTypeFromValue(match_ctx.Tensor("scale"));
    ...
    if (!input_scale_dtype.isa<Float16Type>() &&
        !input_scale_dtype.isa<Float32Type>()) {
      return false;
    }
    return true;
  }
  ...
  void PostProcess(const paddle::drr::MatchContext& match_ctx) const {
    ...
    } else {  // (input_scale_dtype.isa<Float32Type>())
    ...
    this->pass_state_.get()->preserved_analyses.Preserve<pass::QuantAnalysis>();
    PADDLE_ENFORCE_EQ(
    ...
    auto next_op = GetUseOpsForOutput(op, 0)[0].first;
    IrContext* ctx = IrContext::Instance();
    auto inputs = next_op->operands_source();
    std::vector<Attribute> input_index_vec;
    std::vector<Attribute> input_scale_vec;
    for (size_t i = 0; i < inputs.size(); i++) {
      auto input = inputs[i];
      if (match_ctx.Tensor("dequantize_linear_out") == input) {
        // non-weight quant-dequant situation
        Attribute input_index = Int32Attribute::get(ctx, static_cast<int>(i));
        input_index_vec.push_back(input_index);
        Attribute input_scale_attr =
            FloatAttribute::get(IrContext::Instance(), input_scale);
        input_scale_vec.push_back(input_scale_attr);
      }
    }
    // set the scale info into op
    if (input_index_vec.size() > 0) {
      Attribute inputs_index =
          ArrayAttribute::get(IrContext::Instance(), input_index_vec);
      next_op->set_attribute("inputs_index", inputs_index);
      Attribute inputs_scale =
          ArrayAttribute::get(IrContext::Instance(), input_scale_vec);
...
class DeleteQuantDequantLinearOpPass : public PatternRewritePass {
 public:
  DeleteQuantDequantLinearOpPass()
      : PatternRewritePass("delete_quant_dequant_linear_op_pass", 1) {}
...
REGISTER_IR_PASS(delete_quant_dequant_linear_op_pass,
                 pir::DeleteQuantDequantLinearOpPass);
