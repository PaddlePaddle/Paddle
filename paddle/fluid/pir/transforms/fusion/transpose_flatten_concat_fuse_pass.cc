...
class Conv2dAddFusePattern : public paddle::drr::DrrPatternBase {
 public:
  explicit Conv2dAddFusePattern(int transpose_flatten_time) {
    transpose_flatten_time_ = transpose_flatten_time;
  }
  void operator()(paddle::drr::DrrPatternContext *ctx) const override {
    paddle::drr::SourcePattern pat = ctx->SourcePattern();
    std::vector<const drr::Op *> transpose_list;
    std::vector<const drr::Op *> flatten_list;
    std::vector<const Tensor *> combine_in;
    transpose_list.reserve(transpose_flatten_time_);
    flatten_list.reserve(transpose_flatten_time_);
    combine_in.reserve(transpose_flatten_time_);
    // create transpose op and flatten op
    for (int i = 0; i < transpose_flatten_time_; i++) {
      ...
      transpose_list[i] = transpose_op;
      flatten_list[i] = flatten_op;
    }
    ...
    // Link op
    for (int i = 0; i < transpose_flatten_time_; i++) {
      transpose_list[i]({&pat.Tensor("transpose_in_" + std::string(i))},
                        {&pat.Tensor("transpose_out_" + std::string(i))});
      flatten_list[i]({&pat.Tensor("transpose_in_" + std::string(i))},
                      {&pat.Tensor("flatten_out_" + std::string(i)),
                       &pat.Tensor("flatten_xshape_" + std::string(i))});
      combine_in[i] = &pat.Tensor("flatten_out_" + std::string(i));
    }
    ...
    pat.RequireNativeCall(
        [](const paddle::drr::MatchContext &match_ctx) -> bool {
          std::vector<int> perm_0;
          int flatten_start_0;
          int flatten_stop_0;
          for (int i = 0; i < transpose_flatten_time_; i++) {
            auto flatten_out_shape = pir::GetShapeFromValue(
                match_ctx.Tensor("flatten_out_" + std::string(i)));
            if flatten_out_shape
              .size() != 2 { return false; }
            if (i == 0) {
              perm_0 = match_ctx.GetAttr<std::vector<int>>("perm_" +
                                                           std::to_string(i));
              flatten_start_0 =
                  match_ctx.GetAttr<int>("start_axis_" + std::to_string(i));
              flatten_stop_0 =
                  match_ctx.GetAttr<int>("stop_axis_" + std::to_string(i));
            } else {
              auto tmp_perm = match_ctx.GetAttr<std::vector<int>>(
                  "perm_" + std::to_string(i));
              auto tmp_flatten_start =
                  match_ctx.GetAttr<int>("start_axis_" + std::to_string(i));
              auto tmp_flatten_top =
                  match_ctx.GetAttr<int>("stop_axis_" + std::to_string(i));
              if (perm_0.size() != tmp_perm.size()) {
                return false;
              }
              for (int j = 0; j < static_cast<int>(perm_0.size()); j++) {
                if (perm_0[j] != tmp_perm[j]) {
                  return false;
                }
              }
              if (flatten_start_0 != tmp_flatten_start ||
                  flatten_stop_0 != tmp_flatten_top) {
                return false;
              }
            }
          }
    ...
    std::vector<const Tensor *> x_in;
    x_in.reserve(transpose_flatten_time_);
    for (int i = 0; i < transpose_flatten_time_; i++) {
      x_in[i] = &res.Tensor("transpose_in_" + std::string(i));
    }

    fusion_transpose_flatten_concat_op(x_in, {&res.Tensor("add_out")});
  }
