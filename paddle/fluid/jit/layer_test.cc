...
void Print(const IValue &ival) {
  ...
  cout << t.defined() << " " << t.is_initialized() << endl;
...
TEST(layer, Construct) {
  std::string path = "/workspace/paddle-dev/paddle/fluid/jit/Testing/export";
  auto layer = jit::Load(path);
  auto inputs = PrepareInputs();
  auto outputs = layer.forward(inputs);
  // out = layer.GetFunction("infer")(inputs);
