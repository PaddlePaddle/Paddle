...
__forceinline__ __device__ void warpgroup_fence_operand(float& reg) {  // NOLINT
...
__device__ __forceinline__ void st_shared(const float2* ptr, float2 val) {
  asm volatile(
      "st.shared.v2.f32 [%0], {%1, %2};" ::"l"(ptr), "f"(val.x), "f"(val.y));
}

template <int N>
__device__ void warpgroup_wait() {
  DG_STATIC_ASSERT(N >= 0 && N <= 7, "WGMMA wait: N must be in range [0, 7]");
...
struct FP8MMA {
  ...
  __forceinline__ __device__ static void call_fma_impl(
    ...
    MMA::fma(desc_a,
             desc_b,
             d[Idx]...,
             (scale_d ? cute::SM90::GMMA::ScaleOut::One
                      : cute::SM90::GMMA::ScaleOut::Zero));
  }

  __forceinline__ __device__ static void wgmma(uint64_t const& desc_a,
                                               uint64_t const& desc_b,
                                               float* d,
                                               bool scale_d) {
    call_fma_impl(
        desc_a, desc_b, d, scale_d, std::make_index_sequence<N_ / 2>{});
  }
...
struct FP8MMASelector {
  static constexpr auto select_mma() {
    if constexpr (N == 16)
      return cute::SM90::GMMA::MMA_64x16x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 24)
      return cute::SM90::GMMA::MMA_64x24x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 32)
      return cute::SM90::GMMA::MMA_64x32x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 40)
      return cute::SM90::GMMA::MMA_64x40x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 48)
      return cute::SM90::GMMA::MMA_64x48x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 56)
      return cute::SM90::GMMA::MMA_64x56x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 64)
      return cute::SM90::GMMA::MMA_64x64x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 72)
      return cute::SM90::GMMA::MMA_64x72x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 80)
      return cute::SM90::GMMA::MMA_64x80x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 88)
      return cute::SM90::GMMA::MMA_64x88x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 96)
      return cute::SM90::GMMA::MMA_64x96x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 104)
      return cute::SM90::GMMA::MMA_64x104x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 112)
      return cute::SM90::GMMA::MMA_64x112x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 120)
      return cute::SM90::GMMA::MMA_64x120x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 128)
      return cute::SM90::GMMA::MMA_64x128x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 136)
      return cute::SM90::GMMA::MMA_64x136x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 144)
      return cute::SM90::GMMA::MMA_64x144x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 152)
      return cute::SM90::GMMA::MMA_64x152x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 160)
      return cute::SM90::GMMA::MMA_64x160x32_F32E4M3E4M3_SS_TN();
    if constexpr (N == 192)
      return cute::SM90::GMMA::MMA_64x192x32_F32E4M3E4M3_SS_TN();
  }
