...
struct Scheduler {
  ...
  __device__ __forceinline__ void get_swizzled_block_idx(
    ...
    if constexpr (kIsTMAMulticastOnA) {
      ...
      auto num_n_blocks_in_group =
          min(kNum1DBlocksPerGroup, kNumNBlocks - first_n_block_idx);
      ...
      auto num_m_blocks_in_group =
          min(kNum1DBlocksPerGroup, num_m_blocks - first_m_block_idx);
      auto in_group_idx = block_idx % num_blocks_per_group;
  ...
  __device__ __forceinline__ bool get_next_block(uint32_t& m_block_idx,
    ...
    if constexpr (kGemmType == GemmType::GroupedMasked) {
      ...
      get_swizzled_block_idx(num_m_blocks,
                             next_block_idx - curr_cumsum * kNumNBlocks,
                             m_block_idx,
                             n_block_idx);
    } else {
