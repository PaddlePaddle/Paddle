...
IMPLEMT_EQUIVALENCE_TRANS_CFIRST_FUNC(
    gcu_builder, node, map_inputs, map_inputs_formats, Conv2DEquivalenceTrans) {
  auto* op = node->Op();
  ...
  if (map_inputs.count("Input") != 0) {
    ...
    if (data_format == "NCHW") {
      ...
      PADDLE_THROW(platform::errors::InvalidArgument(
          "GCU conv2d does not support data_format %s", data_format));
    }
  } else {
    PADDLE_ENFORCE_EQ(
...
IMPLEMT_EQUIVALENCE_TRANS_CFIRST_FUNC(gcu_builder,
  ...
  builder::Op filter;
  const std::string running_mode = std::string(std::getenv(kRunningMode));
...
IMPLEMT_EQUIVALENCE_TRANS_CFIRST_FUNC(gcu_builder,
  ...
  if (map_inputs.count("Input") != 0) {
    ...
    if (data_format == "NCHW") {
      ...
      PADDLE_THROW(platform::errors::InvalidArgument(
          "GCU conv2d_transpose does not support data_format %s", data_format));
    }
  } else {
    PADDLE_ENFORCE_EQ(
        true, false, platform::errors::NotFound("lack of [Input] gcu op"));
...
EQUIVALENCE_TRANS_FUNC_REG(kConv2D, CHANNELFIRST, Conv2DEquivalenceTrans);
