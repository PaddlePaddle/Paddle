...
namespace platform {
namespace gcu {
const char *const kAdamW = "adamw";
const char *const kMergedAdamW = "merged_adamw";

IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, node, map_inputs, running_mode, AdamWEquivalenceTrans) {
  ...
  auto *op = node->Op();
  float lr_ratio = PADDLE_GET_CONST(float, op->GetAttr("lr_ratio"));    // 1.0f
  float coeff = PADDLE_GET_CONST(float, op->GetAttr("coeff"));          // 0.01f
  bool with_decay = PADDLE_GET_CONST(bool, op->GetAttr("with_decay"));  // false
  ...
  if (skip_update) {
  ...
  } else {  // Adamw algorithm
    if (!with_decay) {
      coeff = 0.0f;
    }
    auto lr_decay = builder::FullLike(learning_rate_op, lr_ratio);
    auto learning_rate_decay = learning_rate_op * lr_decay;

    auto coeff_op = builder::FullLike(learning_rate_op, coeff);
    ...
    learning_rate_decay =
        learning_rate_decay * (builder::Sqrt(const_1_op - beta_2_pow_op) /
                               (const_1_op - beta_1_pow_op));
    param_op = param_op - learning_rate_op * coeff_op * param_op;
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, node, map_inputs, running_mode, MergedAdamWEquivalenceTrans) {
  ...
  float coeff = PADDLE_GET_CONST(float, op->GetAttr("coeff"));  // 0.01f
  std::vector<float> lr_ratios =
      PADDLE_GET_CONST(std::vector<float>, op->GetAttr("lr_ratios"));
  PADDLE_ENFORCE_EQ(n,
                    lr_ratios.size(),
                    platform::errors::InvalidArgument(
                        "The size of Attr(lr_ratios) must be equal to "
                        "Input(Param), but got the size of Attr(lr_ratios) "
                        "is %d, the size of Input(Param) is %d.",
                        lr_ratios.size(),
                        n));
  std::vector<int> with_decays =
      PADDLE_GET_CONST(std::vector<int>, op->GetAttr("with_decays"));
  PADDLE_ENFORCE_EQ(n,
                    with_decays.size(),
                    platform::errors::InvalidArgument(
                        "The size of Attr(with_decays) must be equal to "
                        "Input(Param), but got the size of Attr(with_decays) "
                        "is %d, the size of Input(Param) is %d.",
                        with_decays.size(),
                        n));
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    auto lr_decay = builder::FullLike(learning_rate_op[i], lr_ratios[i]);
    auto learning_rate_decay = learning_rate_op[i] * lr_decay;
    if (with_decays[i] != 0) {
      auto coeff_op = builder::FullLike(learning_rate_op[i], coeff);
      param_op[i] = param_op[i] - learning_rate_decay * coeff_op * param_op[i];
    }
    ...
    learning_rate_decay =
        learning_rate_decay * (builder::Sqrt(const_1_op - beta_2_pow_op[i]) /
                               (const_1_op - beta_1_pow_op[i]));

    param_out_op[i] =
        param_op[i] - learning_rate_decay *
                          (moment_1_out_op[i] /
...
EQUIVALENCE_TRANS_FUNC_REG(kAdamW, INSENSITIVE, AdamWEquivalenceTrans);
EQUIVALENCE_TRANS_FUNC_REG(kMergedAdamW,
                           INSENSITIVE,
                           MergedAdamWEquivalenceTrans);
