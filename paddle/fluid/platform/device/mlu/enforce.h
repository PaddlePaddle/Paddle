...
/*************** CNNL ERROR ***************/
inline bool is_error(cnnlStatus stat) {
  return stat != CNNL_STATUS_SUCCESS;
}

inline std::string build_mlu_error_msg(cnnlStatus stat) {
  std::ostringstream sout;
  sout << "MLU CNNL error(" << stat << "), "
       << cnnlGetErrorString(stat) << ". ";
  return sout.str();
}
...
inline std::string build_mlu_error_msg(cnStatus stat) {
  ...
  std::ostringstream sout;
  sout << "MLU CN error(" << static_cast<int>(stat) << "), " 
