...
void MLUMemcpyD2HAsync(void* dst, const void* src, size_t num, mluStream stream) {
  PADDLE_ENFORCE_MLU_SUCCESS(
      cnrtMemcpyAsync(dst, const_cast<void*>(src), num, stream, cnrtMemcpyDevToHost));
}
...
void MLUMemcpyH2DAsync(void* dst, const void* src, size_t num, mluStream stream){
  PADDLE_ENFORCE_MLU_SUCCESS(
      cnrtMemcpyAsync(dst, const_cast<void*>(src), num, stream, cnrtMemcpyHostToDev));
}
...
void MLUMemcpyD2DAsync(void* dst, const void* src, size_t num, mluStream stream){
  PADDLE_ENFORCE_MLU_SUCCESS(
      cnrtMemcpyAsync(dst, const_cast<void*>(src), num, stream, cnrtMemcpyDevToDev));
}
...
void MLUMemcpyPeerAsync(void* dst, int dst_device,
                        const void* src, int src_device,
                        size_t num, mluStream stream) {
  PADDLE_ENFORCE_MLU_SUCCESS(
      cnrtMemcpyPeerAsync(dst, dst_device, const_cast<void*>(src), src_device, num, stream));
}

void MLUMemcpyPeerSync(void* dst, int dst_device,
                       const void* src, int src_device,
                       size_t num) {
  PADDLE_ENFORCE_MLU_SUCCESS(
      cnrtMemcpyPeer(dst, dst_device, const_cast<void*>(src), src_device, num));
}
 ...
 public:
  ...
  cnrtStatus Malloc(void **ptr, size_t size) {
    ...
    if (result == cnrtSuccess) {
      ...
      // Non out of memory error would be raised inside RaiseNonOutOfMemoryError.
