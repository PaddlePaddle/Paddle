...
Node *lookup_table_op_handler(Graph *graph,
  ...
  if (padding_idx_ >= 0 && padding_idx_ < table_size_) {
    ...
    if (padding_idx_ == 0) {
      auto right_slice = CreateSlice(graph,
                                     node,
                                     {GetInputVarNode("W", node)},
                                     {},
                                     std::vector<int>{int(padding_idx_) + 1},
                                     std::vector<int>{int(table_size_)},
                                     std::vector<int>{0},
                                     std::vector<int>{1});
      ...
      auto left_slice = CreateSlice(graph,
                                    node,
                                    {GetInputVarNode("W", node)},
                                    {},
                                    std::vector<int>{0},
                                    std::vector<int>{int(padding_idx_)},
                                    std::vector<int>{0},
                                    std::vector<int>{1});
      w_node = CreateBaseOp(graph,
                            node,
...
Node *slice_handler(Graph *graph, Node *node) {
  ...
  auto axes_value = BOOST_GET_CONST(std::vector<int>, op->GetAttr("axes"));
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (is_tensor) {
      ...
      if (i == 2 && !op->HasAttr("strides")) {
        ...
        slice_values[i] =
            BOOST_GET_CONST(std::vector<int>, op->GetAttr(attr_names[i]));
      }
    }
  }
...
Node *tile_handler(Graph *graph, Node *node) {
  ...
  auto repeat_times =
      BOOST_GET_CONST(std::vector<int>, op->GetAttr("repeat_times"));
