...
TEST(DeviceEvent, CUDA) {
  ...
  float *src_fp32, *dst_fp32;
  int size = 1000000 * sizeof(float);
  musaMallocHost(reinterpret_cast<void**>(&src_fp32), size);
  musaMalloc(reinterpret_cast<void**>(&dst_fp32), size);
  musaMemcpyAsync(
      dst_fp32, src_fp32, size, musaMemcpyHostToDevice, context->stream());
  event.Record(context);  // step 1. record it
  status = event.Query();
  ASSERT_EQ(status, false);
