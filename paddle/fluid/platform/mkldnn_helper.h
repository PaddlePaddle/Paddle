...
inline dnnl::memory::format_tag GetMKLDNNFormat(dnnl::memory::desc mem_desc) {
  ...
  inline MKLDNNMemoryFormat MKLDNNFormatForSize(
      size_t dims_size, MKLDNNMemoryFormat data_format) {
    if (dims_size == 1) {
  ...
  inline void AppendKey(std::string * key, const char* str) {
    key->append(str);
  }
  ...
  inline void GetGroupConvWeightsTz(
      std::vector<int64_t> & weights_tz,  // NOLINT
      const int groups) {
    if (groups > 1) {
