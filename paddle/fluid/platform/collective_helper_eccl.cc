 ...
 public:
  ...
  ~ECCLCommImpl() {
    PADDLE_ENFORCE_NPU_SUCCESS(
        platform::dynload::eccl_destroy_comm_global(comm_.c_str()));
  }
...
ECCLComm* ECCLCommContext::CreateECCLComm(PaddleEcclCommGroupIdType group_name,
                                          int nranks, int rank, int dev_id,
                                          int ring_id) {
  PADDLE_ENFORCE_GT(
      nranks, 1,
  ...
  VLOG(1) << "initialized comm: " << group_name << ", nranks: " << nranks
          << ", rank: " << rank;
  PADDLE_ENFORCE_NPU_SUCCESS(platform::dynload::eccl_init_comm_global(
      nranks, rank, "CPU", dev_id, group_name.c_str()));

  VLOG(1) << "initialized comm: " << group_name << ", nranks: " << nranks
          << ", rank: " << rank;

  auto* comm_wrapper =
      AssignECCLComm(group_name, nranks, rank, dev_id, ring_id);
...
ECCLComm* ECCLCommContext::AssignECCLComm(PaddleEcclCommGroupIdType group_name,
                                          int nranks, int rank, int dev_id,
                                          int ring_id) {
