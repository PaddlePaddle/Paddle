...
TEST(CudnnHelper, ScopedTensorDescriptor) {
  ...
  paddle::platform::dynload::miopenGetTensorNdDescriptor(
...
TEST(CudnnHelper, ScopedFilterDescriptor) {
  ...
  paddle::platform::dynload::miopenGetFilterNdDescriptor(
...
TEST(CudnnHelper, ScopedConvolutionDescriptor) {
  ...
  EXPECT_EQ(mode, HIPDNN_CROSS_CORRELATION);
}
...
TEST(CudnnHelper, ScopedPoolingDescriptor) {
  ...
  std::vector<int> pads(3);
  std::vector<int> strides(3);
  paddle::platform::dynload::mopenGet2dPoolingDescriptor(
