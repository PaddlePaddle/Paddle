...
struct XPUPlace {
  ...
  inline int GetDeviceId() const { return device; }
  // needed for variant equality comparison
  inline bool operator==(const XPUPlace &o) const {
    return device == o.device;
  }
...
class Place : public boost::variant<CUDAPlace, XPUPlace, CPUPlace, CUDAPinnedPlace> {
 private:
  using PlaceBase = boost::variant<CUDAPlace, XPUPlace, CPUPlace, CUDAPinnedPlace>;
