...
TEST(bfloat16, conversion_cpu) {
  ...
  // Conversion from int
  EXPECT_EQ(bfloat16(-1).value, 0xbf80);
  EXPECT_EQ(bfloat16(0).value, 0x0000);
  EXPECT_EQ(bfloat16(1).value, 0x3f80);
  EXPECT_EQ(bfloat16(2).value, 0x4000);
  EXPECT_EQ(bfloat16(3).value, 0x4040);

  // Conversion from bool
  EXPECT_EQ(bfloat16(true).value, 0x3f80);
  EXPECT_EQ(bfloat16(false).value, 0x0000);
...
TEST(bfloat16, lod_tensor_cpu) {
  ...
  for (size_t i = 0; i < input_data.size(); ++i) {
    ...
    EXPECT_EQ(data_ptr[i].value, input_data[i].value);
  }
}
