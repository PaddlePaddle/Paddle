 ...
 *
 * @param rank_table A string identifying the rank table file path, include file
 * name.
 ...
 * @param outputPtr A pointer identifying the output data address of the
 * operator.
 * @param inputCount An integer(u64) identifying the number of the input data.
 * @param dataType The data type of the operator, must be one of the following
 * types: int8, int32, float16, float32.
 * @param group A string identifying the group name of ranks participating in
 * the operator.
 * @param stream A pointer identifying the stream information.
 * @return hcclResult_t
 */
extern hcclResult_t hcom_all_gather(const char *tag, void *inputPtr,
                                    void *outputPtr, u64 inputCount,
                                    hcclDataType_t dataType, const char *group,
                                    rtStream_t stream);
 ...
 * @param dataType The data type of the operator, must be one of the following
 * types: int8, int32, float16, float32.
 * @param op The reduction type of the operator, must be one of the following
 * types: sum, min, max, prod.
 * @param group A string identifying the group name of ranks participating in
 * the operator.
 * @param stream A pointer identifying the stream information.
 * @return hcclResult_t
 */
extern hcclResult_t hcom_all_reduce(const char *tag, void *inputPtr,
                                    void *outputPtr, u64 count,
 ...
 * @return hcclResult_t
 */
extern hcclResult_t hcom_broadcast(const char *tag, void *ptr, u64 count,
                                   hcclDataType_t dataType, u32 root,
                                   const char *group, rtStream_t stream);
 ...
 * @return hcclResult_t
 */
extern hcclResult_t hcom_reduce_scatter(const char *tag, void *inputPtr,
                                        void *outputPtr, u64 count,
                                        hcclDataType_t dataType, hcclRedOp_t op,
                                        const char *group, rtStream_t stream);
 ...
 * @return hcclResult_t
 */
hcclResult_t hcom_get_world_rank_from_group_rank(const char *group,
                                                 u32 groupRank, u32 *worldRank);
 ...
 * @return hcclResult_t
 */
hcclResult_t hcom_get_group_rank_from_world_rank(u32 worldRank,
                                                 const char *group,
                                                 u32 *groupRank);
 ...
 * @return hcclResult_t
 */
hcclResult_t hcom_send(const char *tag, void *inputPtr, u64 count,
                       hcclDataType_t dataType, u32 destRank, u32 srTag,
                       const char *group, rtStream_t stream);
 ...
 * @return hcclResult_t
 */
hcclResult_t hcom_receive(const char *tag, void *outputPtr, u64 count,
                          hcclDataType_t dataType, u32 srcRank, u32 srTag,
                          const char *group, rtStream_t stream);
 ...
 * @param maxSegmentNum An integer(u32) identifying the max segments of
 * gradients.
 * @param segmentNum A pointer identifying the segments number of gradients.
 * @param segmentIdx A list identifying the index of end gradient in each
 * segment.
 * @return hcclResult_t
 */
hcclResult_t hcom_get_split_strategy(const char *group,
                                     const struct model_feature *feature,
                                     u32 maxSegmentNum, u32 *segmentNum,
                                     u32 *segmentIdx,
                                     GradSplitForceMode force = FORCE_NONE);

/**
 * @brief Set the gradient split strategy with in the group, according to
 * gradient index.
 *
 * @param group A string identifying the group name.
 * @param segmentNum An integer(u32) identifying the segments number of
 * gradients.
 * @param IdxList A list identifying the index of end gradient in each segment.
 * @return hcclResult_t
 */
extern hcclResult_t hcom_set_split_strategy_by_index(const char *group,
                                                     u32 segmentNum,
                                                     const u32 *IdxList);
 ...
 * @return hcclResult_t
 */
extern hcclResult_t hcom_set_split_strategy_by_size(const char *group,
                                                    u32 segmentNum,
                                                    const float *sizeList);
