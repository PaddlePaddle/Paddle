...
void ChromeTracingLogger::LogHostEventNode(const HostEventNode& host_node) {
  if (!output_file_stream_) {
    return;
  }
...
void ChromeTracingLogger::LogRuntimeEventNode(
    const CudaRuntimeEventNode& runtime_node) {
  if (!output_file_stream_) {
    return;
  }
  output_file_stream_ << string_format(
...
void ChromeTracingLogger::LogDeviceEventNode(
    const DeviceEventNode& device_node) {
  if (!output_file_stream_) {
    return;
  }
...
void ChromeTracingLogger::HandleTypeKernel(const DeviceEventNode& device_node) {
...
void ChromeTracingLogger::HandleTypeMemcpy(const DeviceEventNode& device_node) {
...
void ChromeTracingLogger::HandleTypeMemset(const DeviceEventNode& device_node) {
