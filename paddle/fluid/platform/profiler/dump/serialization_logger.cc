...
void SerializationLogger::LogHostTraceEventNode(
  ...
  host_trace_event->set_start_ns(host_node.start_ns());
  host_trace_event->set_end_ns(host_node.end_ns());
  host_trace_event->set_process_id(host_node.process_id());
  host_trace_event->set_thread_id(host_node.thread_id());
...
void SerializationLogger::LogRuntimeTraceEventNode(
  ...
  runtime_trace_event->set_start_ns(runtime_node.start_ns());
  runtime_trace_event->set_end_ns(runtime_node.end_ns());
  runtime_trace_event->set_process_id(runtime_node.process_id());
  runtime_trace_event->set_thread_id(runtime_node.thread_id());
  runtime_trace_event->set_correlation_id(runtime_node.correlation_id());
  runtime_trace_event->set_callback_id(runtime_node.callback_id());
...
void SerializationLogger::HandleTypeKernel(
  ...
  device_trace_event->set_start_ns(device_node.start_ns());
  device_trace_event->set_end_ns(device_node.end_ns());
  device_trace_event->set_device_id(device_node.device_id());
  device_trace_event->set_context_id(device_node.context_id());
  device_trace_event->set_stream_id(device_node.stream_id());
  device_trace_event->set_correlation_id(device_node.correlation_id());
  // fill KernelEventInfoProto
  KernelEventInfo info = device_node.kernel_info();
...
void SerializationLogger::HandleTypeMemcpy(
  ...
  MemcpyEventInfo info = device_node.memcpy_info();
  memcpy_info->set_num_bytes(info.num_bytes);
  memcpy_info->set_copy_kind(info.copy_kind);
  memcpy_info->set_src_kind(info.src_kind);
  memcpy_info->set_dst_kind(info.dst_kind);
  // binding
...
void SerializationLogger::HandleTypeMemset(
  ...
  MemsetEventInfo info = device_node.memset_info();
  memset_info->set_num_bytes(info.num_bytes);
  memset_info->set_memory_kind(info.memory_kind);
