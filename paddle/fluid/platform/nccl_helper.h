...
struct NCCLContext {
  ...
  int device_id() const {
    return BOOST_GET(platform::CUDAPlace, ctx_->GetPlace()).device;
  }
};
...
struct NCCLContextMap {
  ...
  explicit NCCLContextMap(const std::vector<platform::Place> &places,
    ...
    for (auto &p : places) {
      int dev_id = BOOST_GET(CUDAPlace, p).device;
  ...
  CUDADeviceContext *DevCtx(platform::Place p) const {
    return DevCtx(BOOST_GET(CUDAPlace, p).device);
  }

  const NCCLContext &at(platform::Place p) const {
    return this->at(BOOST_GET(CUDAPlace, p).device);
  }
  ...
  void InitFlatCtxs(const std::vector<platform::Place> &places,
    ...
    for (int ring_id = 0; ring_id < nrings; ++ring_id) {
      for (size_t p = 0; p < places.size(); ++p) {
        ...
        int dev_id = BOOST_GET(CUDAPlace, places[p]).device;
