...
<<<<<<< HEAD
    //   CUDADeviceContext *DevCtx(int dev_id) const { return
    //   at(dev_id).ctx_.get();
    //   }
=======
...
<<<<<<< HEAD
      //    *When nccl inits nccl comm using ncclCommInitAll, it meets error
      //    when
      //    *allreduce ophandle and sync_batch_norm_op use ncclallreduce
      //    parallelly.
      //    So
      //    *create a new nccl comm for sync_batch_norm_op. And these codes
      //    should be
      //    *polished with a unified nccl management.

      //   NCCLContextMap *GetSyncBatchNormCtx(
      //       framework::Scope *scope, const std::vector<platform::Place>
      //       &places) {
...
<<<<<<< HEAD
        ...
        //                       platform::errors::InvalidArgument(
        //                           "Hierarchical ctxs should be initialized
        //                           firstly!"));
...
>>>>>>> ce26f88... update Ascendrc hccl to 20.3 (#32126)
        ...
        void InitHierarchicalCtxs(
            const std::vector<platform::Place> &places,
            const std::vector<HcclRootInfo *> &inter_hccl_ids,
            const std::vector<HcclRootInfo *> &exter_hccl_ids,
            size_t trainers_num, size_t trainer_id, size_t inter_trainers_num,
            size_t exter_trainers_num) {
          PADDLE_ENFORCE_EQ(
              trainers_num, inter_trainers_num * exter_trainers_num,
              platform::errors::InvalidArgument(
          ...
          for (size_t i = 0; i < inter_hccl_ids.size(); i++) {
            VLOG(1) << "init inter_trainer_id:" << inter_trainer_id
                    << ", comm no:" << i;
            auto local =
                new HCCLContextMap(places, inter_hccl_ids[i],
          ...
          if (exter_trainer_id >= 0) {
            for (size_t i = 0; i < exter_hccl_ids.size(); i++) {
              auto ex =
                  new HCCLContextMap(places, exter_hccl_ids[i],
        ...
        HCCLContextMap *GetHierarchicalExterCtx(size_t run_order) const {
          PADDLE_ENFORCE_GT(
              h_exter_ctxs_.size(), 0,
              platform::errors::InvalidArgument(
                  "Hierarchical ctxs should be initialized firstly!"));
          return h_exter_ctxs_[run_order % h_exter_ctxs_.size()].get();
        }

        std::vector<std::unique_ptr<HCCLContextMap>>
       ...
       protected:
        // Support multi nccl comm on default nccl ring while HCCLContextMap
        // can't.
