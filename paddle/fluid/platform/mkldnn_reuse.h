...
class MKLDNNHandler {
 public:
  ...
  std::shared_ptr<mkldnn::memory> AcquireMemory(
    ...
    if (target_memory_p == nullptr) {
      ...
      if (md != user_md) {
        ...
        auto reorder_p = std::shared_ptr<mkldnn::reorder>(new mkldnn::reorder(*reorder_pd));
  ...
  void AcquireReorder(
    ...
    mkldnn::stream astream(engine_);
    reorder_p->execute(astream, *user_memory_p, *target_memory_p);
    astream.wait();
  }
  ...
  static void AppendKey(
      ...
      const mkldnn::memory::dims& weights_dims, const std::vector<int64_t>& strides,
      const std::vector<int64_t>& paddings, const std::vector<int64_t>& dilations,
      const int& groups, const mkldnn::memory::data_type& srcdt,
      const mkldnn::memory::format_tag& format, const std::string& fuse_activation,
    ...
    AppendKey(key, std::to_string(static_cast<int>(srcdt)));
    AppendKey(key, std::to_string(static_cast<int>(format)));
  ...
  static void AppendKeyVec(std::string* key, const std::vector<int64_t>& dims) {
    for (unsigned int i = 0; i < dims.size(); i++) {
      AppendKey(key, std::to_string(dims[i]));
    }
  }
 ...
 public:
  ...
  std::shared_ptr<mkldnn::sum::primitive_desc> AcquireSumPrimitiveDescriptor(
    ...
    if (sum_pd_ == nullptr) {
      ...
      std::vector<mkldnn::memory::desc> src_pds;
      for (auto& input_mem : src_mems) {
        src_pds.push_back(input_mem->get_desc());
      }

      sum_pd_.reset(
          new mkldnn::sum::primitive_desc(dst_md, scales, src_pds, engine_));
  ...
  std::shared_ptr<mkldnn::memory> AcquireDstMemoryFromPrimitive(void* ptr) {
    return this->AcquireMemory(sum_pd_->dst_desc(), ptr, "@dst_mem_p");
  }
 ...
 public:
  ...
  std::shared_ptr<mkldnn::eltwise_forward> AcquireActivation() {
    ...
    if (eltwise_p == nullptr) {
      eltwise_p = std::make_shared<mkldnn::eltwise_forward>(*activation_pd_);
  ...
  std::shared_ptr<mkldnn::memory> AcquireDstMemoryFromPrimitive(
    ...
    T* ptr = output->mutable_data<T>(place,
                                     activation_pd_->dst_desc().get_size());
    return this->AcquireMemory(activation_pd_->dst_desc(), ptr, "@dst_mem_p");
  }

  std::shared_ptr<mkldnn::memory> AcquireDiffSrcMemoryFromPrimitive(void* ptr) {
    return this->AcquireMemory(activation_bwd_pd_->diff_src_desc(), ptr, "@diff_src_mem_p");
  }
  ...
  std::shared_ptr<mkldnn::eltwise_backward> AcquireActivationBackward() {
    ...
    if (eltwise_bwd_p == nullptr) {
      eltwise_bwd_p =
          std::make_shared<mkldnn::eltwise_backward>(*activation_bwd_pd_);
...
class LRNMKLDNNHandler : public MKLDNNHandler {
 public:
  ...
  std::shared_ptr<mkldnn::memory> AcquireWorkspaceMemory(void) {
    ...
    if (mem_p == nullptr) {
      ...
      if (mem_p == nullptr) {
        ...
        mkldnn::memory::desc workspace_md = fwd_pd_->workspace_desc();
        mem_p = std::make_shared<mkldnn::memory>(workspace_md, engine_);
        dev_ctx_.SetBlob(local_key, mem_p);
      }
    }
    return mem_p;
  ...
  std::shared_ptr<mkldnn::lrn_forward> AcquireLRN() {
    ...
    if (lrn_p == nullptr) {
      if (is_test_) {
        lrn_p = std::make_shared<mkldnn::lrn_forward>(*fwd_pd_);
      } else {
  ...
  AcquireLRNBackwardPrimitiveDescriptor(const mkldnn::memory::desc& src_md,
    ...
    if (bwd_pd_ == nullptr) {
      ...
      auto backward_desc =
          mkldnn::lrn_backward::desc{mkldnn::algorithm::lrn_across_channels,
                                     src_md,
                                     diff_md,
                                     n,
                                     alpha,
  ...
  std::shared_ptr<mkldnn::lrn_backward> AcquireLRNBackward() {
    ...
    if (lrn_bwd_p == nullptr) {
      lrn_bwd_p = std::make_shared<mkldnn::lrn_backward>(*bwd_pd_);
...
class PoolingMKLDNNHandler : public MKLDNNHandler {
 public:
  ...
  AcquirePoolingPrimitiveDescriptor(
      ...
      const std::vector<int64_t>& paddings, bool ceil_mode) {
    ...
    if (fwd_pd_ == nullptr) {
      ...
      fwd_pd_ =
          std::static_pointer_cast<mkldnn::pooling_forward::primitive_desc>(
              dev_ctx_.GetBlob(key_pooling_pd));
      if (fwd_pd_ == nullptr) {
        std::vector<int64_t> padding_left_top(paddings);
        std::vector<int64_t> padding_right_bottom(paddings);
        if (ceil_mode) {
  ...
  std::shared_ptr<mkldnn::memory> AcquireDstMemoryFromPrimitive(void* ptr) {
    return this->AcquireMemory(fwd_pd_->dst_desc(), ptr, "@dst_mem_p");
  }

  std::shared_ptr<mkldnn::memory> AcquireWorkspaceMemory(void) {
    mkldnn::memory::desc workspace_md =
        pooling_type_ == "max"
            ? fwd_pd_->workspace_desc()
            : mkldnn::memory::desc({}, dt_, mkldnn::memory::format_tag::nchw);
    // Pooling PD has to be passed to Grad op that
    // may be executed by diffrent thread, hence
    // for that one we use key that does not contain TID
    auto local_key = key_common_ + "@workspace";
  ...
  std::shared_ptr<mkldnn::pooling_forward> AcquirePooling() {
    ...
    if (pooling_p == nullptr) {
      if (is_test_) {
        pooling_p = std::make_shared<mkldnn::pooling_forward>(*fwd_pd_);
      } else {
        // For training we need to create workspace
        // to store indices from backward
  ...
  AcquirePoolingBackwardPrimitiveDescriptor(
      ...
      const mkldnn::memory::desc& diff_src_md,
      const std::vector<int64_t>& ksize, const std::vector<int64_t>& strides,
  ...
  std::shared_ptr<mkldnn::memory> AcquireDiffDstMemoryFromDataPrimitive(
      const std::shared_ptr<mkldnn::memory> user_memory_p,
      std::vector<mkldnn::primitive>& pipeline) {  // NOLINT
    auto diff_dst_pd = bwd_pd_->diff_dst_desc();
    auto user_md = user_memory_p->get_desc();
    return this->AcquireMemory(diff_dst_pd, user_md, user_memory_p,
                               "@diff_dst_mem_p", pipeline);
  }

  std::shared_ptr<mkldnn::memory> AcquireDiffSrcMemoryFromPrimitive(void* ptr) {
    return this->AcquireMemory(bwd_pd_->diff_src_desc(), ptr, "@diff_src_mem_p");
  }
  ...
  std::shared_ptr<mkldnn::pooling_backward> AcquirePoolingBackward(
    ...
    if (pooling_bwd_p == nullptr) {
      pooling_bwd_p = std::make_shared<mkldnn::pooling_backward>(*bwd_pd_);
  ...
  static std::string GetHash(
      ...
      const std::vector<int64_t>& ksize, const std::vector<int64_t>& strides,
      const std::vector<int64_t>& paddings, const memory::data_type& dt,
      const memory::format_tag& fmt, const std::string& suffix) {
    std::string key;
    key.reserve(platform::MKLDNNHandler::MaxKeyLength);
    platform::MKLDNNHandler::AppendKeyDims(&key, input_dims);
...
class TransposeMKLDNNHandler : public MKLDNNHandler {
 public:
  ...
  std::shared_ptr<mkldnn::memory> AcquireSrcMemory(
      const mkldnn::memory::format_tag& fmt, void* ptr) {
    ...
    if (mem_p == nullptr) {
      ...
      auto src_md = fmt != mkldnn::memory::format_tag::nchw
  ...
  std::shared_ptr<mkldnn::memory> AcquireDstMemory(framework::Tensor* output,
    ...
    if (mem_p == nullptr) {
      ...
      auto dst_data = output->mutable_data<float>(place, dst_md.get_size());
  ...
  static std::string GetHash(std::vector<int64_t>& shape,  // NOLINT
                             std::vector<int>& axis,       // NOLINT
                             const std::string& suffix) {
    return dims2str(shape) +
           dims2str(std::vector<int64_t>(begin(axis), end(axis))) + suffix;
  }
 ...
 protected:
  mkldnn::memory::desc Axis2MemoryDesc(std::vector<int64_t>& nchw_tz,  // NOLINT
    ...
    mkldnn::memory::desc mem_d(nchw_tz, platform::MKLDNNGetDataType<float>(),
                               strides);
...
class ReorderMKLDNNHandler : public MKLDNNHandler {
 public:
  ...
  std::shared_ptr<mkldnn::memory> AcquireDstMemory(
      framework::Tensor* output, const mkldnn::memory::format_tag& fmt,
      platform::Place place) {
    auto local_key = key_ + "@user_dst_mem_p";
    auto mem_p =
        std::static_pointer_cast<mkldnn::memory>(dev_ctx_.GetBlob(local_key));
  ...
  static std::string GetHash(std::vector<int64_t>& shape,  // NOLINT
                             mkldnn::memory::format_tag in_fmt,
                             mkldnn::memory::format_tag out_fmt,
                             const std::string& suffix) {
    return dims2str(shape) + std::to_string(static_cast<int>(in_fmt)) + "->" +
           std::to_string(static_cast<int>(out_fmt)) + "#" + suffix;
  }
...
class ConvMKLDNNTemplateHandler : public MKLDNNHandler {
 public:
  ...
  mkldnn::memory::format_tag GetDstFormat() const {
  ...
  std::shared_ptr<mkldnn::memory> AcquireDiffWeightsMemoryFromWeightsPrimitive(
      void* ptr) {
    return this->AcquireMemory(conv_bwd_weights_pd_->diff_weights_desc(), ptr, "@diff_weights_mem_p");
  }
  ...
  std::shared_ptr<mkldnn::memory> AcquireDstMemoryFromResidualDataMemory(
      ...
      void* dst_ptr,
      std::vector<mkldnn::primitive>& pipeline) {  // NOLINT
      auto residual_dst_memory_p = this->AcquireDstMemoryFromPrimitive(dst_ptr);
      this->AcquireReorder(user_residual_memory_p, residual_dst_memory_p, "@residual_data_mem_p");

    return residual_dst_memory_p;
  }

  std::shared_ptr<mkldnn::memory> AcquireDiffSrcMemoryFromDataPrimitive(
      void* ptr) {
    return this->AcquireMemory(conv_bwd_data_pd_->diff_src_desc(), ptr, "@diff_src_mem_p");
  }

  std::shared_ptr<mkldnn::memory> AcquireDstMemoryFromPrimitive(void* ptr) {
    return this->AcquireMemory(conv_pd_->dst_desc(), ptr, "@dst_mem_p");
  }
  ...
  static std::string GetHash(mkldnn::memory::dims& input_dims,    // NOLINT
                             ...
                             std::vector<int64_t>& strides,           // NOLINT
                             std::vector<int64_t>& paddings,          // NOLINT
                             std::vector<int64_t>& dilations,         // NOLINT
...
static void SetDstMemoryQuantized(
  ...
  dst_fmt =
      platform::MKLDNNFormatForSize(dst_dims, mkldnn::memory::format_tag::nhwc);
