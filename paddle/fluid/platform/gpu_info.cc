...
int GetCUDAMultiProcessors(int id) {
  PADDLE_ENFORCE_LT(id, GetCUDADeviceCount(), "id must less than GPU count");
  int count;
  auto error_code = cudaDeviceGetAttribute(&count, cudaDevAttrMultiProcessorCount, id);
  PADDLE_ENFORCE(error_code,
...
int GetCUDAMaxThreadsPerMultiProcessor(int id) {
  PADDLE_ENFORCE_LT(id, GetCUDADeviceCount(), "id must less than GPU count");
  int count;
  auto error_code = cudaDeviceGetAttribute(&count, 
...
void GpuMemcpyAsync(void *dst, const void *src, size_t count,
  ...
  PADDLE_ENFORCE(error_code,
                 ...
                 src, dst, static_cast<int>(count), error_code, CudaErrorWebsite());
}
...
void GpuMemcpyPeerAsync(void *dst, int dst_device, const void *src,
  ...
  auto error_code = cudaMemcpyPeerAsync(dst, dst_device, src, 
                                            src_device, count, stream);
  PADDLE_ENFORCE(error_code,
                 "cudaMemcpyPeerAsync failed in paddle::platform::GpuMemcpyPeerAsync "
                 "error code : %d, %s", error_code, CudaErrorWebsite());
}
