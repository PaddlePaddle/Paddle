...
std::vector<std::vector<ir::OpResult>> MeanOp::Vjp(
  ...
  std::vector<std::vector<Tensor>> tensor_res = primitive::mean_vjp(
...
std::vector<std::vector<ir::OpResult>> DivideOp::Vjp(
    ir::Operation* op,
    const std::vector<std::vector<ir::OpResult>>& out_grads,
    const std::vector<std::vector<bool>>& stop_gradients) {
  ...
  int axis = -1;
  std::vector<std::vector<Tensor>> tensor_res =
      primitive::divide_vjp(x, y, out, out_grad, axis, stop_gradients);
