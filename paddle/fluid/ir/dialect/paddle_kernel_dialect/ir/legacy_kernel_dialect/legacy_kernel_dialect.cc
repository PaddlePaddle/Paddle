...
REGISTER_FILE_SYMBOLS(legacy_kernel_dialect);

namespace paddle {
namespace dialect {

LegacyKernelDialect::LegacyKernelDialect(ir::IrContext *context)
    : ir::Dialect(name(), context, ir::TypeId::get<LegacyKernelDialect>()) {
  initialize();
}

void LegacyKernelDialect::initialize() {
...
void LegacyKernelDialect::PrintType(ir::Type type, std::ostream &os) const {
...
void LegacyKernelDialect::PrintAttribute(ir::Attribute attr,
...
IR_DEFINE_EXPLICIT_TYPE_ID(paddle::dialect::LegacyKernelDialect)
