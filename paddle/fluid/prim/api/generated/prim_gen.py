...
from paddle.phi.api.yaml.generator import PREFIX_TENSOR_NAME, BaseAPI
...
"""
     ...
     def get_ad_func_args(self, inplace_flag=False):
        ad_func_args = self.get_input_tensor_args(inplace_flag)
    ...
    def gene_eager_prim_api_code(self):
        ...
        api_code = f"""
template <typename T>
{self.get_return_type(self.is_inplace)} {api_func_name}({self.get_define_args(self.is_inplace)}) 
"""
        api_code = api_code + '{' + '\n'
        api_code += f"""
{indent} {self.gene_ad_func_call} 
"""
        api_code += '}'
...
def generate_api(api_yaml_path, header_file_path, eager_prim_source_file_path):
    ...
    include_header_file = "#include paddle/fluid/prim/api/manual/prim_api/prim_api.h"
    ...
    eager_prim_source_file.write(namespace[1])
...
def main():
    ...
    parser.add_argument(
        ...
        default='paddle/fluid/prim/api/manual/prim_api/prim_api.h',
    )

    parser.add_argument(
        '--eager_prim_api_source_path',
        help='output of generated eager_prim_api source code file',
        default='paddle/fluid/prim/api/manual/prim_api/eager_prim_api.cc',
    )
