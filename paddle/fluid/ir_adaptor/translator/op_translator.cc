  ...
  pir::AttributeMap TranslateOpAttribute(
    ...
    for (const auto& info : op_attr_infos) {
      ...
      if (op_desc.HasAttr(legacy_attr_name)) {
        ...
        pir::Attribute new_attr =
            attribute_translator(info.type_name, legacy_attr);
        if (info.name == "alpha" || info.name == "beta") {
          new_attr = pir::DoubleAttribute::get(
              ctx,
              static_cast<double>(
                  new_attr.dyn_cast<pir::FloatAttribute>().data()));
...
struct GaussianRandomOpTranscriber : public OpTranscriber {
  pir::AttributeMap TranslateOpAttribute(
      pir::IrContext* ctx,
      const std::string& normalized_op_name,
      const OpAttributeInfoList& op_attr_infos,
    ...
    for (const auto& info : op_attr_infos) {
      ...
      if (op_desc.HasAttr(legacy_attr_name)) {
        ...
        pir::Attribute new_attr =
            attribute_translator(info.type_name, legacy_attr);
        if (info.name == "mean" || info.name == "std") {
          new_attr = pir::DoubleAttribute::get(
              ctx,
              static_cast<double>(
                  new_attr.dyn_cast<pir::FloatAttribute>().data()));
...
// `uniform_inplace` / `gaussian_inplace` widened their distribution parameters
// from float to double, so legacy programs storing them as float need the
// attribute rebuilt as a DoubleAttribute.
struct InplaceRandomOpTranscriber : public OpTranscriber {
  pir::AttributeMap TranslateOpAttribute(
      pir::IrContext* ctx,
      const std::string& normalized_op_name,
      const OpAttributeInfoList& op_attr_infos,
    ...
    for (const auto& info : op_attr_infos) {
      ...
      if (op_desc.HasAttr(legacy_attr_name)) {
        ...
        pir::Attribute new_attr =
            attribute_translator(info.type_name, legacy_attr);
        if (info.name == "mean" || info.name == "std" || info.name == "min" ||
            info.name == "max") {
          new_attr = pir::DoubleAttribute::get(
              ctx,
              static_cast<double>(
...
OpTranslator::OpTranslator() {
  ...
  special_handlers["gaussian_inplace"] = InplaceRandomOpTranscriber();
  special_handlers["gaussian_inplace_grad"] = InplaceRandomOpTranscriber();
  special_handlers["uniform_random_inplace"] = InplaceRandomOpTranscriber();
  special_handlers["uniform_random_inplace_grad"] =
      InplaceRandomOpTranscriber();
}
}  // namespace paddle::translator
