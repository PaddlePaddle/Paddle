...
pir::OpInfo OpTranscriber::LookUpOpInfo(pir::IrContext* ctx,
  ...
  for (const auto& info : input_infos) {
    ...
    PADDLE_ENFORCE(
        var != nullptr,
        platform::errors::InvalidArgument("[op:%s] Input %s should not be null",
                                          op_desc.Type(),
                                          legacy_input_vars[0]));
  ...
  PADDLE_ENFORCE(!target_op_name.empty(),
                 platform::errors::InvalidArgument(
                     "Op %d should have corresponding OpInfo %d",
                     op_desc.Type(),
                     target_op_name));
...
std::vector<pir::Value> OpTranscriber::GenerateOperationInput(
  ...
  for (const auto& info : input_infos) {
    ...
    if (!is_vector) {
      PADDLE_ENFORCE(legacy_input_vars.size() == 1u,
                     platform::errors::InvalidArgument(
                         "Input %s not found when parsing op %s",
                         info.name,
                         op_desc.Type()));
      PADDLE_ENFORCE(param_map->count(legacy_input_vars[0]),
                     platform::errors::InvalidArgument(
...
pir::Value TranslateDropOutStateIn(pir::IrContext* ctx,
  ...
  PADDLE_ENFORCE(
      ...
      platform::errors::InvalidArgument("[op:%s] Output %s should not be null",
                                        op_desc.Type(),
                                        legacy_output_vars[0]));
  auto& type_translator = TypeTranslator::instance();
  pir::Type translated_var_type =
...
struct SplitOpTranscriber : public OpTranscriber {
  std::vector<pir::Value> GenerateOperationInput(
    ...
    PADDLE_ENFORCE(
        x_input_vars.size() == 1,
        platform::errors::InvalidArgument("x input of split MUST be a tensor"));
    ...
    if (op_desc.HasInput("AxisTensor") &&
      ...
      PADDLE_ENFORCE(axis_var_list.size() == 1,
                     platform::errors::InvalidArgument(
...
ValueInfo GetTensorInfoByVarName(const OpDesc& op_desc,
  ...
  PADDLE_ENFORCE(type.isa<dialect::DenseTensorType>(),
                 platform::errors::InvalidArgument(
...
struct MulGradOpTranscriber : public OpTranscriber {
  ...
  std::vector<pir::Value> GenerateOperationInput(
    ...
    PADDLE_ENFORCE(y_num_col_dims <= static_cast<int>(y_shape.size()),
                   platform::errors::InvalidArgument(
                       "Expected op[%s]'s attr `y_num_col_dims` less than or "
                       "equal to dim of input Y %s, but got %d",
                       op_desc.Type(),
                       y_shape.size(),
                       y_num_col_dims));
  ...
  void RecordOpResultMapping(pir::IrContext* ctx,
    ...
    auto gradReshape = [&](const std::string& var_name) {
      ...
      PADDLE_ENFORCE(var_desc != nullptr,
                     platform::errors::InvalidArgument(
                         "[op:%s] Input %s should not be null",
                         op_desc.Type(),
                         var_name.substr(0, 1)));
      ...
      PADDLE_ENFORCE(value_res != nullptr,
                     platform::errors::InvalidArgument(
                         "Expected op[%s]'s input %s is not null",
                         op_desc.Type(),
                         grad_var_name));
      pir::Type grad_type = value_res.type();
      PADDLE_ENFORCE(grad_type.isa<dialect::DenseTensorType>(),
                     platform::errors::InvalidArgument(
                         "Expected op[%s]'s input %s is DenseTensor but got %s",
                         op_desc.Type(),
                         grad_var_name,
...
struct SelectInputOpTranscriber : public OpTranscriber {
  pir::Operation* operator()(pir::IrContext* ctx,
    ...
    PADDLE_ENFORCE(param_map->count(Mask_name) > 0,
                   platform::errors::InvalidArgument(
    ...
    for (auto in_name : Input_name) {
      PADDLE_ENFORCE(param_map->count(in_name) > 0,
                     platform::errors::InvalidArgument(
                         "Expected op[%s]'s input %s has been parsed",
                         op_desc.Type(),
                         in_name));
    ...
    if (input1 == input2) {
      ...
      if (tensor1.dtype() != tensor2.dtype() ||
        ...
        PADDLE_ENFORCE(
            ...
            platform::errors::InvalidArgument(
                "undefined_var %s should be generated by assign_value, but got "
                "%s",
  ...
  pir::Operation* operator()(pir::IrContext* ctx,
    ...
    PADDLE_ENFORCE(param_map->count(Mask_name) > 0,
                   platform::errors::InvalidArgument(
                       ...
                       Mask_name));
    op_inputs.push_back(param_map->at(Mask_name).value);
    PADDLE_ENFORCE(param_map->count(Input_name) > 0,
                   platform::errors::InvalidArgument(
                       "Expected op[%s]'s input %s has been parsed",
                       op_desc.Type(),
                       Input_name));
    ...
    PADDLE_ENFORCE(Out_names.size() == 2,
                   platform::errors::InvalidArgument(
...
pir::Value TranslateNumClassesForOneHot(pir::IrContext* ctx,
  ...
  if (op_desc.HasInput(legacy_tensor_name) &&
    ...
    PADDLE_ENFORCE(legacy_vars.size() == 1,
                   platform::errors::InvalidArgument(
...
struct ElementwiseTranscriber : public OpTranscriber {
  std::vector<pir::Value> GenerateOperationInput(
    ...
    PADDLE_ENFORCE(param_map->count(x_name) > 0,
                   platform::errors::InvalidArgument(
                       "Expected op[%s]'s input %s has been parsed",
                       op_desc.Type(),
                       x_name));
    ...
    PADDLE_ENFORCE(x_type.isa<dialect::DenseTensorType>(),
                   platform::errors::InvalidArgument(
                       "Expected op[%s]'s input %s is DenseTensor but got %s",
                       op_desc.Type(),
                       x_name,
    ...
    PADDLE_ENFORCE(param_map->count(y_name) > 0,
                   platform::errors::InvalidArgument(
                       "Expected op[%s]'s input %s has been parsed",
                       op_desc.Type(),
                       y_name));
    ...
    PADDLE_ENFORCE(y_type.isa<dialect::DenseTensorType>(),
                   platform::errors::InvalidArgument(
                       "Expected op[%s]'s input %s is DenseTensor but got %s",
                       op_desc.Type(),
                       y_name,
...
struct ElementwiseGradTranscriber : public OpTranscriber {
  void RecordOpResultMapping(pir::IrContext* ctx,
    ...
    PADDLE_ENFORCE(y_grad_type.isa<dialect::DenseTensorType>(),
                   platform::errors::InvalidArgument(
                       "Expected op[%s]'s input %s is DenseTensor but got %s",
                       op_desc.Type(),
                       y_grad_var_name,
...
struct SetValueWithTensorOpTranscriber : public SetValueOpTranscriber {
  ...
  InputHandlerFn GetSpecialInputHandlers(
    ...
    return [](pir::IrContext* ctx,
      ...
      std::vector<std::string> legacy_input_vars;
      PADDLE_ENFORCE(op_desc.HasInput("ValueTensor"),
                     platform::errors::InvalidArgument(
                         "[set_value] should have ValueTensor"));
      legacy_input_vars = op_desc.Input("ValueTensor", true);
      PADDLE_ENFORCE(
          legacy_input_vars.size() == 1u,
          platform::errors::InvalidArgument("[set_value][ValueTensor] should "
                                            "only have 1 variable, but got %d",
...
struct FusedFeedForwardOpTranscriber : public OpTranscriber {
  ...
  void RecordOpResultMapping(pir::IrContext* ctx,
    ...
    if (op_desc.HasOutput("Out")) {
      const auto& output_vars = op_desc.Output("Out");
      PADDLE_ENFORCE(output_vars.size() == 1,
                     platform::errors::InvalidArgument(
  ...
  InputHandlerFn GetSpecialInputHandlers(
    ...
    return [](pir::IrContext* ctx,
      ...
      PADDLE_ENFORCE(
          vars.size() == 1,
          platform::errors::InvalidArgument(
              "Input `X` should be one variable %s", op_desc.Type()));
...
struct SliceOpTranscriber : public OpTranscriber {
  pir::OpInfo LookUpOpInfo(pir::IrContext* ctx,
    ...
    PADDLE_ENFORCE(op_desc.HasInput("Input"),
                   platform::errors::InvalidArgument(
    ...
    const auto* input_var = op_desc.Block()->FindVarRecursive(input_vars[0]);
    if (input_var->GetType() == framework::proto::VarType::LOD_TENSOR_ARRAY) {
      PADDLE_ENFORCE(op_desc.HasOutput("Out"),
                     platform::errors::InvalidArgument(
