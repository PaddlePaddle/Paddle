...
void FilterAndAddOutputVars(const BlockDesc& block, const VarList& prev_input_vars, VarList* cur_output_vars) {
...
void CreateVarNodesAndAddDeps(const VarList& cur_output_vars, int64_t producer_id, RuntimeGraph* runtime_graph) {
...
InterVarNode::InterVarNode(const VarDesc& var) 
	: name_(var.Name()), var_(&var) {
...
bool TaskNode::IsSrcTask() const {
	return consumed_var_node_names_.empty();
}
...
int64_t RuntimeGraph::TaskNodesNum() const {
