...
std::shared_ptr<pten::DenseTensor> MakeTensorImpl<pten::DenseTensor, LoDTensor>(
    const LoDTensor& tensor, pten::Backend backend, pten::DataType dtype,
...
std::shared_ptr<pten::DenseTensor> MakeTensorImpl<pten::DenseTensor, Tensor>(
    const Tensor& tensor, pten::Backend backend, pten::DataType dtype,
    pten::DataLayout layout) {
  auto holder = tensor.Holder();
  auto tensor_impl = std::make_shared<pten::DenseTensor>(
      pten::TensorMeta(tensor.dims(), backend, dtype, layout, tensor.offset()),
...
std::shared_ptr<pten::TensorInterface> InputVariableToPtTensor(
...
std::shared_ptr<pten::TensorInterface> OutputVariableToPtTensor(
...
pten::KernelKey TransOpKernelTypeToPtKernelKey(
  ...
  pten::DataLayout layout = pten::TransToPtDataLayout(kernel_type.data_layout_);
  pten::DataType dtype = pten::TransToPtDataType(kernel_type.data_type_);
