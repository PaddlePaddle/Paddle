...
TEST(DataTypeTransform, GPUTransform) {
  ...
  // data type transform from/to float16
  {
    ...
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(out_data_float[i], static_cast<float>(in_ptr[i]));
    }
    ...
    double* out_data_double = out.data<double>();
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(out_data_double[i], static_cast<double>(in_ptr[i]));
    }
    ...
    int* out_data_int = out.data<int>();
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(out_data_int[i], static_cast<int>(in_ptr[i]));
    }
    ...
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(out_data_int64[i], static_cast<int64_t>(in_ptr[i]));
    }
    ...
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(out_data_bool[i], static_cast<bool>(in_ptr[i]));
    }
    ...
    in_ptr = out.data<float16>();
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(in_ptr[i].x, static_cast<float16>(in_data_float[i]).x);
    }
    ...
    in_ptr = out.data<float16>();
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(in_ptr[i].x, static_cast<float16>(in_data_double[i]).x);
    }
    ...
    in_ptr = out.data<float16>();
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(in_ptr[i].x, static_cast<float16>(in_data_int[i]).x);
    }
    ...
    in_ptr = out.data<float16>();
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(in_ptr[i].x, static_cast<float16>(in_data_int64[i]).x);
    }
    ...
    in_ptr = out.data<float16>();
    for (int i = 0; i < data_number; ++i) {
      ASSERT_EQ(in_ptr[i].x, static_cast<float16>(in_data_bool[i]).x);
    }
  }
}