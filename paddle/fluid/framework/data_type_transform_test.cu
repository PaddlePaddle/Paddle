...
TEST(DataTypeTransform, GPUTransform) {
  ...
  // data type transform from float32
  {
    ...
    float* in_ptr = in.mutable_data<float>(pten::make_ddim({2, 3}), cpu_place);
  ...
  // data type transform from/to float16
  {
    ...
    float* in_data_float =
        in.mutable_data<float>(pten::make_ddim({2, 3}), cpu_place);
    for (int i = 0; i < data_number; ++i) {
      in_data_float[i] = i;
    }
    ...
    double* in_data_double =
        in.mutable_data<double>(pten::make_ddim({2, 3}), cpu_place);
    for (int i = 0; i < data_number; ++i) {
      in_data_double[i] = i;
    }
    ...
    int* in_data_int = in.mutable_data<int>(pten::make_ddim({2, 3}), cpu_place);
    for (int i = 0; i < data_number; ++i) {
      in_data_int[i] = i;
    }
    ...
    int64_t* in_data_int64 =
        in.mutable_data<int64_t>(pten::make_ddim({2, 3}), cpu_place);
    for (int i = 0; i < data_number; ++i) {
      in_data_int64[i] = i;
    }
    ...
    bool* in_data_bool =
        in.mutable_data<bool>(pten::make_ddim({2, 3}), cpu_place);
    for (int i = 0; i < data_number; ++i) {
      in_data_bool[i] = i;
    }
