...
// WorkQueue implementation composed from a blocking and
...
class SingleThreadedWorkQueue : public WorkQueue {
 public:
  ...
  virtual void AddTask(std::function<void()> fn) override {
    queue_.AddTask(std::move(fn));
  }

  virtual void WaitQueueEmpty() override {
    queue_.WaitQueueEmpty();
  }

  virtual size_t NumThreads() override {
    return queue_.NumThreads();
  }
