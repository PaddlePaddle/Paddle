...
void SetColAttrForFetchOps(std::shared_ptr<ProgramDesc> program_desc,
                           const int64_t micro_batch_num,
                           const int64_t micro_batch_id) {
  const std::set<std::string>& valid_fetch_op_types = {"fetch", "fetch_v2"};
    ...
    if (valid_fetch_op_types.find(op_desc->Type()) !=
      ...
      PADDLE_ENFORCE_GE(
          ...
          platform::errors::InvalidArgument(
              ...
              col));
      op_desc->SetAttr(
          "col", {static_cast<int>(col * micro_batch_num + micro_batch_id)});
    }
  }
}
