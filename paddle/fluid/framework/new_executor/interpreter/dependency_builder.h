...
//   // build op dependencies and return the mapping from op to its
//   downstream-op
//   // set
//   const std::map<size_t, std::set<size_t>>& Build(const
...
//   bool is_build_;
//   const std::vector<std::unique_ptr<InstructionBase>>* instructions_;  //
//   not_own size_t op_num_;

//   // ops_behind_ is the adjacency list about op to its posterior-ops, that is
//   to
//   // say, op_behind_[i] == {a, b, c} means op[a], op[b] and op[c] depend on
//   // op[i] directly or indirectly. ops_before_ is the revered adjacency list
//   of
//   // ops_behind_.
//   std::vector<std::vector<size_t>> ops_before_;
//   std::vector<std::vector<size_t>> ops_behind_;

//   // op_downstream_map_ is the mapping from op to its downstream-op set, that
//   is
//   // to say, op_downstream_map_[i] == {a, b, c} means op[a], op[b] and op[c]
//   // depend on op[i] directly.
//   std::map<size_t, std::set<size_t>> op_downstream_map_;

//   // op_happens_before_ is a matrix form of ops_before_ and ops_behind_, it
//   is
//   // used to speed up the query.
//   std::vector<std::vector<bool>> op_happens_before_;
// };
