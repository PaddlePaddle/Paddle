...
class CounterTracker {
 public:
  explicit CounterTracker(std::atomic<uint64_t>& counter, 
                          EventCount& ec) : counter_(&counter), ec_(&ec) {
 ...
 public:
  ...
  void AddTaskWithHint(std::function<void()> fn, int start, int limit) {
    Task t = env_.CreateTask(
        [task = std::move(fn), 
            raii = CounterTracker(num_tasks_, wait_empty_ec_)] () mutable { 
          task(); 
        });
  ...
  void WaitQueueEmpty() {
    ...
    if (waiting || !wait_empty_.compare_exchange_strong(
                       waiting, true, std::memory_order_acquire)) {
      abort();
    }
    ...
    waiting = true;
    if (!waiting || !wait_empty_.compare_exchange_strong(
                        waiting, false, std::memory_order_acquire)) {
      abort();
    }
  }
