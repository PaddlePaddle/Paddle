...
paddle::framework::FetchList StandaloneExecutor::Run(
  ...
  const auto& jobs = plan_.JobList();
  std::unordered_map<std::string, size_t> type_to_id;
  ...
  if (!is_interpretercore_build_result_shared_) {
    ...
    for (size_t job_idx = 1; job_idx < jobs.size(); ++job_idx) {
      ...
      // on.
      std::string job_type = jobs[job_idx]->Type();
      if (type_to_id.count(job_type) == 0) {
        type_to_id[job_type] = job_idx;
      }
    }
  ...
  for (size_t job_idx = 0; job_idx < jobs.size(); ++job_idx) {
    ...
    if (type_to_id.count(job_type) != 0 && job_idx != type_to_id[job_type]) {
