...
TEST(DataTransform, DataLayoutFunction) {
  ...
  in.mutable_data<double>(pten::make_ddim({2, 3, 1, 2}), place);
...
TEST(DataTransformBf16, GetDataFromTensorDNNL) {
  auto place = paddle::platform::CPUPlace();
  paddle::framework::Tensor in = paddle::framework::Tensor();
  in.mutable_data<paddle::platform::bfloat16>(pten::make_ddim({2, 3, 1, 2}),
                                              place);
...
TEST(DataTransformInt32, GetDataFromTensorDNNL) {
  auto place = paddle::platform::CPUPlace();
  paddle::framework::Tensor in = paddle::framework::Tensor();
  in.mutable_data<int32_t>(pten::make_ddim({2, 3, 1, 2}), place);
