...
TEST(Attribute, GetAttrValueToAny) {
  ...
  auto rlt_float = paddle::framework::GetAttrValueForPhi(x_float);
  EXPECT_NEAR(paddle::get<float>(rlt_float), 3.14, 1e-6);

  std::string str_value("test");
  paddle::framework::Attribute x_str(str_value);
  auto rlt_str = paddle::framework::GetAttrValueForPhi(x_str);
  EXPECT_EQ(paddle::get<std::string>(rlt_str), "test");

  std::vector<int> vec_int_var(2, 100);
  paddle::framework::Attribute x_vec_int = vec_int_var;
  auto rlt_vec_int = paddle::framework::GetAttrValueForPhi(x_vec_int);
  auto vec_int = paddle::get<std::vector<int>>(rlt_vec_int);
  ...
  auto rlt_vec_float = paddle::framework::GetAttrValueForPhi(x_vec_float);
  auto vec_float = paddle::get<std::vector<float>>(rlt_vec_float);
  ...
  auto rlt_vec_str = paddle::framework::GetAttrValueForPhi(x_vec_str);
  auto vec_str = paddle::get<std::vector<std::string>>(rlt_vec_str);
  EXPECT_EQ(vec_str.size(), 2UL);
  EXPECT_EQ(vec_str[0], "test");
  EXPECT_EQ(vec_str[1], "test");

  paddle::framework::Attribute x_bool(true);
  auto rlt_bool = paddle::framework::GetAttrValueForPhi(x_bool);
  EXPECT_EQ(paddle::get<bool>(rlt_bool), true);
  ...
  auto rlt_int64 = paddle::framework::GetAttrValueForPhi(x_int64);
  EXPECT_EQ(paddle::get<int64_t>(rlt_int64), 100);

  std::vector<int64_t> vec_int64_var(2, 100);
  paddle::framework::Attribute x_vec_int64 = vec_int64_var;
  auto rlt_vec_int64 = paddle::framework::GetAttrValueForPhi(x_vec_int64);
  auto vec_int64 = paddle::get<std::vector<int64_t>>(rlt_vec_int64);
  ...
  auto rlt_vec_double = paddle::framework::GetAttrValueForPhi(x_vec_double);
  auto vec_double = paddle::get<std::vector<double>>(rlt_vec_double);
