...
void TensorCopy(const Tensor& src, const platform::Place& dst_place,
  ...
  if (platform::is_cpu_place(src_place) && platform::is_cpu_place(dst_place)) {
    memory::Copy(BOOST_GET(platform::CPUPlace, dst_place), dst_ptr,
                 BOOST_GET(platform::CPUPlace, src_place), src_ptr, size);
  }
#ifdef PADDLE_WITH_CUDA
  else if (platform::is_gpu_place(src_place) &&  // NOLINT
           platform::is_cpu_place(dst_place)) {
    auto src_gpu_place = BOOST_GET(platform::CUDAPlace, src_place);
    auto dst_cpu_place = BOOST_GET(platform::CPUPlace, dst_place);
    auto ctx_place = ctx.GetPlace();
    PADDLE_ENFORCE_EQ(platform::is_gpu_place(ctx_place), true);
    auto ctx_gpu_place = BOOST_GET(platform::CUDAPlace, ctx_place);
    ...
    auto src_cpu_place = BOOST_GET(platform::CPUPlace, src_place);
    auto dst_gpu_place = BOOST_GET(platform::CUDAPlace, dst_place);
    auto ctx_place = ctx.GetPlace();
    PADDLE_ENFORCE_EQ(platform::is_gpu_place(ctx_place), true);
    auto ctx_gpu_place = BOOST_GET(platform::CUDAPlace, ctx_place);
...
void TensorCopySync(const Tensor& src, const platform::Place& dst_place,
  ...
  if (platform::is_cpu_place(src_place) && platform::is_cpu_place(dst_place)) {
    ...
    memory::Copy(dst_gpu_place, dst_ptr, src_gpu_place, src_ptr, size, nullptr);
             ...
             platform::is_gpu_place(dst_place)) {
    auto src_pinned_place = BOOST_GET(platform::CUDAPinnedPlace, src_place);
    auto dst_gpu_place = BOOST_GET(platform::CUDAPlace, dst_place);
    ...
    if (platform::is_gpu_place(tensor.place())) {
      ...
      while (size != 0) {
        ...
        memory::Copy(cpu, buf.get(),
                     BOOST_GET(platform::CUDAPlace, tensor.place()),
