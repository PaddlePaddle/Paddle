...
void TensorCopy(const Tensor& src, const platform::Place& dst_place,
  ...
  if (platform::is_cpu_place(src_place) && platform::is_cpu_place(dst_place)) {
    ...
    memory::Copy(
        BOOST_GET_CONST(platform::CPUPlace, dst_place), dst_ptr,
        BOOST_GET_CONST(platform::XPUPlace, src_place), src_ptr,
        size);
  } else if (platform::is_cpu_place(src_place) &&
             platform::is_xpu_place(dst_place)) {
    memory::Copy(
        BOOST_GET_CONST(platform::XPUPlace, dst_place), dst_ptr,
        BOOST_GET_CONST(platform::CPUPlace, src_place), src_ptr,
        size);
  } else if (platform::is_xpu_place(src_place) &&
             platform::is_xpu_place(dst_place)) {
    if (src_ptr == dst_ptr) {
