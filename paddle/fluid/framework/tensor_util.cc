...
template <typename Predicate, typename DevCtx> struct AnyDTypeVisitor {
...
public:
  ...
  template <typename Place> bool operator()(const Place &place) const {
...
public:
  ...
  template <typename Place> void operator()(const Place &place) const {
    auto *ctx = platform::DeviceContextPool::Instance().GetByPlace(place);
    out_->Resize({1});
...
struct BothFalseVisitor : public boost::static_visitor<> {
  ...
  void
  VisitorImpl(const platform::CUDAPinnedPlace &cpu /* equals to cpu*/) const {
    bool lhs = !in_.data<bool>()[0];
    bool rhs = !out_->mutable_data<bool>(cpu)[0];
    out_->mutable_data<bool>(cpu)[0] = lhs && rhs;
  }
