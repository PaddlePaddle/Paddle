...
#include "paddle/fluid/framework/fleet/context/context_callback.h"
    ...
    for (size_t j = 0; j < resc_types_.size() + 1; ++j) {
      if (target_result[id].negtive_num_[j] == 0) {
        ...
        target_result[id].final_pn_[j] =
            (static_cast<float>)((static_cast<double>)target_result[id]
                                     .positive_num_[j] /
                                 (static_cast<double>)target_result[id]
                                     .negtive_num_[j]);
      }
      ...
      if (target_result[id].negtive_wnum_[j] <= 0) {
        ...
        target_result[id].final_wpn_[j] =
            (static_cast<float>)(target_result[id].positive_wnum_[j] /
