...
void FleetWrapper::SaveMultiTableOnePath(const std::vector<int>& table_ids, const std::string& path, const int mode) {
#ifdef PADDLE_WITH_PSLIB
  auto ret = pslib_ptr_->_worker_ptr->save_multi_table_one_path(table_ids, path, std::to_string(mode));
  ret.wait();
