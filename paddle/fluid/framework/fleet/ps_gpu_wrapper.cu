...
__global__ void PullCopy(float** dest, const FeatureValue* src,
                         ...
                         uint64_t** keys, int max_val_size, int* gpu_dim) {
  CUDA_KERNEL_LOOP(i, total_len) {
    int low = 0;
    int high = slot_num - 1;
    while (low < high) {
    ...
    mf_dim = gpu_dim[x] - 3;
    if (*(keys[x] + y) != 0) {
      if (gpu_dim[x] - 3 != feature_value_ptr->mf_dim) {
        printf("yxfttest i: %d, max_val_size: %d res: %d\n", i, max_val_size, i * max_val_size);
        printf("yxf::cur dim: %d mf_dim: %d key: %d show: %f, clk: %f mf_size: %d, mf0: %f, mf1: %f lr: %f i: %d total_len: %d\n",gpu_dim[x], feature_value_ptr->mf_dim, *(keys[x] + y), feature_value_ptr->show, feature_value_ptr->clk, feature_value_ptr->mf_size, feature_value_ptr->mf[0], feature_value_ptr->mf[1], feature_value_ptr->lr, i, total_len);
      }
    }
    if (*(keys[x] + y) == 0) {
...
void PSGPUWrapper::CopyForPull(const paddle::platform::Place& place,
  ...
  if (!multi_mf_dim_) {
    ...
    auto buf_dim = memory::Alloc(place, slot_mf_dim_vector_.size() * sizeof(int));
    int* gpu_dim = reinterpret_cast<int*>(buf_dim->ptr());
    cudaMemcpy(gpu_dim, slot_mf_dim_vector_.data(), slot_mf_dim_vector_.size() * sizeof(int),
             cudaMemcpyHostToDevice);
    PullCopy<<<(total_length + 1024 - 1) / 1024, 1024, 0, stream>>>(
