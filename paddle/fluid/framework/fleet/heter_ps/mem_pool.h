...
class MemoryPool {
 public:
  MemoryPool(size_t capacity, size_t block_size) : capacity_(capacity), block_size_(block_size) {
    VLOG(3) << "mem_pool init with block_size: " << block_size << " capacity: " << capacity;
...
class HBMMemoryPool : public managed {
 public:
  ...
  HBMMemoryPool(MemoryPool* mem_pool) {
    ...
    VLOG(3) << "hbm memory pool with capacity" << capacity_ << " bs: " << block_size_; 
    cudaMalloc(&mem_, block_size_ * capacity_);
    cudaMemcpy(mem_, mem_pool->mem(), mem_pool->byte_size(), cudaMemcpyHostToDevice);
  }
  ...
  void clear(void) {
    cudaMemset(mem_, 0, block_size_ * capacity_);
  }
  ...
  friend std::ostream& operator<<(std::ostream& out, HBMMemoryPool& p) {
    for (size_t k = 0; k < 5; k++) {
      ...
      out << "show: " << x->show << " clk: " << x->clk << " slot: " << x->slot
        << " lr: " << x->lr <<" mf_dim: " << x->mf_size <<  " mf_size: " << x->mf_size << " mf:";
