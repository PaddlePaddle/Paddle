...
class FleetWrapper {
 public:
  ...
  // Pull sparse variables from server with dependency in sync mode
  void PullSparseVarsWithDependencySync(
      const Scope& scope, const uint64_t table_id,
      const std::vector<std::string>& var_names,
      std::vector<uint64_t>* fea_keys,
      std::vector<std::vector<float>>* fea_values, int fea_dim,
      const std::vector<std::string>& var_emb_names, uint64_t dependent_table);
  ...
  // Push sparse variables with labels to server in async mode,
  // table_id has its dependent_table
  void PushSparseVarsWithLabelWithDependencyAsync(
      const Scope& scope, const uint64_t table_id,
      const std::vector<uint64_t>& fea_keys,
      const std::vector<float>& fea_labels,
      const std::vector<std::string>& sparse_key_names,
      ...
      std::vector<::std::future<int32_t>>* push_sparse_status,
      const int batch_size, const bool use_cvm, const bool dump_slot,
      std::vector<uint64_t>* sparse_push_keys, uint64_t dependent_table);
