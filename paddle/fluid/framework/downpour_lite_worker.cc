...
void DownpourLiteWorker::CollectLabelInfo(size_t table_idx) {
  if (no_cvm_) {
    return;
  }
...
void DownpourLiteWorker::FillSparseValue(size_t table_idx) {
  uint64_t table_id = static_cast<uint64_t>(
      param_.program_config(0).pull_sparse_table_id(table_idx));
...
void DownpourLiteWorker::AdjustInsWeight() {
#ifdef _LINUX
