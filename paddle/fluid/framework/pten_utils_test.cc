...
TEST(TcmptUtils, MakeTensor) {
  ...
  ASSERT_EQ(dense_x->type(), pten::DataType::kFLOAT32);
}
...
#endif
  auto tensor_def = pten::TensorArgDef(expect_backend, pten::DataLayout::kNCHW,
                                       pten::DataType::kINT32);
  // 2. test API
  auto tensor_x = InputVariableToPtTensor(v, tensor_def);
  // 3. check result
  ASSERT_EQ(tensor_x->backend(), expect_backend);
  ASSERT_EQ(tensor_x->type(), pten::DataType::kINT32);
}
