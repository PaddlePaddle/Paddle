...
void DownpourWorker::TrainFiles() {
  ...
  while ((cur_batch = device_reader_->Next()) > 0) {
    ...
    // matrix-based part, hard code: top_path_id, top_path_score
    Variable* top_path_var = thread_scope_->FindVar("top_path_id.tmp_0");
    Variable* top_score_var = thread_scope_->FindVar("top_path_score.tmp_0");
    Variable* item_id_var = thread_scope_->FindVar("17");
    if (top_path_var != nullptr && top_score_var != nullptr && item_id_var != nullptr){
      // topk_num is the global and local top number
      int32_t topk_num = 3;
      int32_t path_num = 1;
      int32_t matrix_d = 4;
      // push path
      ...
      LoDTensor* item_id_tensor = item_id_var->GetMutable<LoDTensor>();
      size_t item_id_len = item_id_tensor->numel();
      int64_t* item_ids = item_id_tensor->data<int64_t>();
      LoDTensor* top_score_tensor = top_score_var->GetMutable<LoDTensor>();
      //size_t top_score_len = top_score_tensor->numel();
      float* top_score_ids = top_score_tensor->data<float>();
      LoDTensor* top_path_tensor = top_path_var->GetMutable<LoDTensor>();
      //size_t top_path_len = top_path_tensor->numel();
      // to do: path include pre_batch 0, so if D=4, the path length is 5.
      ...
      for (int32_t id_idx = 1; id_idx < static_cast<int32_t>(item_id_len); ++id_idx) {
        ...
        if (now_item_id != last_id){
          ...
          for (auto score_idx = last_id_pos * topk_num; score_idx < id_idx * topk_num; ++score_idx){
            path_sorted_kv_list.push_back({top_score_ids[score_idx], top_path_ids + 1 + score_idx * (matrix_d + 1)}) ;
          }
          // sort by score
          std::sort(path_sorted_kv_list.begin(), path_sorted_kv_list.end(), [](const std::pair<float, int64_t*>& k1, const std::pair<float, int64_t*>& k2) {
      ...
      auto status_push = fleet_ptr_->pslib_ptr_->_worker_ptr->push_path(
                    table_id, fea_keys_push.data(), (const float**)push_g_vec.data(),
                          fea_keys_push.size());
      std::vector<::std::future<int32_t>> push_sparse_status;
      push_sparse_status.resize(0);
      ...
      //}
      //for (auto id_idx = 0u; id_idx < top_path_len; ++id_idx) {
      //  std::cout << "top_path:" << top_path_ids[id_idx] << std::endl;
      //}
    }
