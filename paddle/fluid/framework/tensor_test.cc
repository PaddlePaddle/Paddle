...
TEST(Tensor, MutableData) {
  {
    ...
    float* p2 = nullptr;
    // initialization
    p1 = src_tensor.mutable_data<float>(pten::make_ddim({1, 2, 3}),
                                        platform::CPUPlace());
    auto p1_holder = src_tensor.Holder();
    EXPECT_NE(p1, nullptr);
    // set src_tensor a new dim with large size
    // momery is supposed to be re-allocated
    p2 = src_tensor.mutable_data<float>(pten::make_ddim({3, 4}),
                                        platform::CPUPlace());
    ...
    p1 = src_tensor.mutable_data<float>(pten::make_ddim({2, 2, 3}),
                                        platform::CPUPlace());
    auto p2_holder2 = src_tensor.Holder();
    EXPECT_EQ(p2_holder1.get(), p2_holder2.get());
    // set src_tensor a new dim with smaller size
    // momery block is supposed to be unchanged
    p2 = src_tensor.mutable_data<float>(pten::make_ddim({2, 2}),
                                        platform::CPUPlace());
    ...
    auto* tmp = src_tensor.mutable_data<uint8_t>(pten::make_ddim({2, 2}),
                                                 platform::CPUPlace());
    ...
    auto* tmp2 = src_tensor.mutable_data<double>(pten::make_ddim({2, 2, 3}),
                                                 platform::CPUPlace());
  ...
  {
    framework::Tensor src_tensor;
    int8_t* p1 = src_tensor.mutable_data<int8_t>(pten::make_ddim({1}),
                                                 platform::CPUPlace());
    EXPECT_NE(p1, nullptr);
    *p1 = 1;

    uint8_t* p2 = src_tensor.mutable_data<uint8_t>(pten::make_ddim({1}),
                                                   platform::CPUPlace());
    EXPECT_NE(p2, nullptr);
...
#if defined(PADDLE_WITH_CUDA) || defined(PADDLE_WITH_HIP)
  {
    ...
    // set src_tensor a new dim with large size
    // momery is supposed to be re-allocated
    p2 = src_tensor.mutable_data<float>(pten::make_ddim({3, 1024}),
                                        platform::CUDAPlace(0));
...
TEST(Tensor, ShareDataWith) {
  {
    ...
    src_tensor.mutable_data<int>(pten::make_ddim({2, 3, 4}),
                                 platform::CPUPlace());
    dst_tensor.ShareDataWith(src_tensor);
...
TEST(Tensor, Slice) {
  {
    framework::Tensor src_tensor;
    src_tensor.mutable_data<int>(pten::make_ddim({5, 3, 4}),
                                 platform::CPUPlace());
  ...
  {
    framework::Tensor src_tensor;
    src_tensor.mutable_data<double>(pten::make_ddim({6, 9}),
                                    platform::CUDAPlace(0));
...
TEST(Tensor, Split) {
  {
    framework::Tensor src_tensor;
    src_tensor.mutable_data<int>(pten::make_ddim({6, 2}), platform::CPUPlace());
  ...
  {
    framework::Tensor src_tensor;
    src_tensor.mutable_data<double>(pten::make_ddim({6, 4}),
                                    platform::CUDAPlace(0));
    std::vector<framework::Tensor> split_tensor_list = src_tensor.Split(2, 0);
    ASSERT_EQ(split_tensor_list.size(), 3UL);
    EXPECT_EQ(split_tensor_list[0].dims()[0], 2);
