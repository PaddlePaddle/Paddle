...
void HeterCpuWorker::CollectLabelInfo(std::shared_ptr<HeterTask> task, size_t table_idx) {
  ...
  for (size_t i = 0; i < sparse_key_names_[table_id].size(); ++i) {
    ...
    Variable* emb_var =
        scope->FindVar(sparse_value_names_[table_id][i]);
    if (emb_var == nullptr) {
      continue;
    }
...
void HeterCpuWorker::FillSparseValue(std::shared_ptr<HeterTask> task, size_t table_idx) {
  ...
  for (size_t i = 0; i < sparse_key_names_[table_id].size(); ++i) {
    ...
    Variable* pin_var = scope->FindVar(emb_slot_name + "pin");
    CHECK(pin_var != nullptr);
    LoDTensor* pin_tensor = pin_var->GetMutable<LoDTensor>();
    ptr = pin_tensor->mutable_data<float>({len, table.emb_dim()}, platform::CUDAPinnedPlace());
    ...
    float* ptr1 = tensor_emb->data<float>();
    float* pin_ptr1 = pin_tensor->data<float>();
    memory::Copy(boost::get<platform::CUDAPlace>(place_),ptr1,platform::CUDAPinnedPlace(),pin_ptr1,len * table.emb_dim() * sizeof(float), copy_stream_);
...
void HeterCpuWorker::AdjustInsWeight(std::shared_ptr<HeterTask> task) {
  ...
  Variable* nid_var =
      scope->FindVar(adjust_ins_weight_config_.nid_slot());
  if (nid_var == nullptr) {
  ...
  while (1) {
    ...
    for (;;) {
      ...
      if (task->state_ == PULL_SPARSE) {
        ...
        auto* dev_ctx = static_cast<platform::CUDADeviceContext *>(platform::DeviceContextPool::Instance().Get(place_));
        PADDLE_ENFORCE_CUDA_SUCCESS(cudaEventRecord(event_, dev_ctx->stream()));
	cudaEventSynchronize(event_);
        ...
        if (need_to_push_sparse_) {
          ...
          for (int i = 0; i < param_.program_config(0).push_sparse_table_id_size();
            ...
            fleet_ptr_->HeterPushSparseVars(
                ...
                dump_slot_, no_cvm_, place_, copy_stream_, event_);
            timeline.Pause();
          ...
          for (int i = 0; i < param_.program_config(0).push_dense_table_id_size();
            ...
            fleet_ptr_->PushDenseVarsAsync(
                *(task->scope_), tid, dense_grad_names_[tid], &push_sparse_status_,
                scale_datanorm_, task->cur_batch_, place_, copy_stream_, event_);
          }
          timeline.Pause();
