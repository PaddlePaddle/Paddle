...
void NaiveExecutor::Run() {
  ...
  for (auto &op : ops_) {
    ...
    if (collect_shape_func_) {
      collect_shape_func_(op.get());
    }
  }
#ifdef PADDLE_WITH_INFERENCE_NVTX
  platform::CudaNvtxRangePop();
...
void NaiveExecutor::RegisterCollectShapeHook(
    const HookFunc &collect_shape_func) {
  collect_shape_func_ = collect_shape_func;
}
