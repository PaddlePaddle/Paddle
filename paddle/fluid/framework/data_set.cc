...
static int compute_thread_batch_nccl(
  ...
  if (gloo_wrapper->Size() > 1) {
    ...
    auto thread_max_batch_num_vec = gloo_wrapper->AllReduce(thread_avg_batch_num_vec, "max");
    auto sum_total_ins_num_vec = gloo_wrapper->AllReduce(total_instance_num_vec, "sum");
    ...
    VLOG(0) << "##########diff batch num: " << diff_batch_num << " thread max batch num: " << thread_max_batch_num << " thread avg batch num: " << thread_avg_batch_num;
    if (diff_batch_num == 0) {
...
void DatasetImpl<T>::SetHeterPs(bool enable_heterps) {
  ...
  if (enable_heterps_) {
    VLOG(0) << "enable heter PS";
    
    if (input_records_.size() == 0 && input_channel_ != nullptr && input_channel_->Size() != 0) {
      input_channel_->ReadAll(input_records_);
      VLOG(0) << "rrrrread from channel to records with records size: " << input_records_.size();
    }
    VLOG(0) << "#######input records size: " << input_records_.size();
    int64_t total_ins_num = input_records_.size();
    std::vector<std::pair<int, int>> offset;
    int default_batch_size = reinterpret_cast<MultiSlotInMemoryDataFeed*>(readers_[0].get())
        ->GetDefaultBatchSize();
    VLOG(0) << "#12 thread_num: " << thread_num_ << " memory size: " << total_ins_num << " default batch_size: " << default_batch_size;
    compute_thread_batch_nccl(thread_num_, total_ins_num, default_batch_size,
                              &offset);
    VLOG(0) << "#13 offset size: " << offset.size();
    for (int i = 0; i < thread_num_; i++) {
      reinterpret_cast<MultiSlotInMemoryDataFeed*>(readers_[i].get())->SetRecord(&input_records_[0]);
    }
    for (int i = 0; i < offset.size(); i++) {
      reinterpret_cast<MultiSlotInMemoryDataFeed*>(readers_[i % thread_num_].get())->AddBatchOffset(offset[i]);
    }
  }
}
...
void DatasetImpl<T>::ReleaseMemory() {
  ...
  if (enable_heterps_) {
    ...
    VLOG(0) << "rrrrrrelease heterps input records records size: " << input_records_.size();
  }
