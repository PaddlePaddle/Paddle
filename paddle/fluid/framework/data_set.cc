...
// Matrix Sample
template <typename T>
void DatasetImpl<T>::InitMatrixPath(
    const std::string path) {
  auto matrix_ptr = MatrixWrapper::GetInstance();
  matrix_ptr->insert(path);
  return;
}
...
// do sample
template <typename T>
void DatasetImpl<T>::MatrixPathGenerate(const uint16_t sample_slot,
                               const std::vector<uint16_t> path_slots, const uint16_t path_num) {
  ...
  for (auto i = 0; i < data.size(); i++) {
    ...
    matrix_ptr->sample(sample_slot, path_slots, &data[i], &tmp_results, path_num);
    VLOG(1) << "sample_results(" << sample_slot << ", " << ") = " << tmp_results.size();
    
    //sample_results.push_back(tmp_results);
    for (auto j = 0; j < tmp_results.size(); j++){
