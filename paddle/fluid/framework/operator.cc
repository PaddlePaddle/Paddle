...
// device, it's ugly, and we will refactor in the future.
  ...
  if (phi::KernelFactory::Instance().HasCompatiblePhiKernel(type_)) {
    // bug fix(liupeng): in some case like the pre-op is Has NO
    // CompatiblePhiKernel,
    // pt_kernel_signature_ and kernel_type_ of this op both are not null. If
    // the op
    // has no only-cpu-supported kernel on NPU platform, there will occurs an
    // error:
    // the op will still pass an NPU DeviceContext while the recieving function
    // accepts
    // a CPU ctx.
    // Further searching for the reason of this bug is needed.
    kernel_signature_.reset(nullptr);
