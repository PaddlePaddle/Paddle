...
DEFINE_bool(memory_used, false, "roughly show memory used in runtime.");
...
inline void ParallelExecutorPrivate::MemoryUsedInRuntime() {
  ...
  for (size_t i = 0; i < local_scope_sizes.size(); ++i) {
    os << "Local Scope " << i << " "
       << "(temp vars total size) : " << local_scope_sizes[i];
  }
...
void ParallelExecutor::Run(const std::vector<std::string> &fetch_tensors,
  ...
  *member_->global_scope_->Var(fetched_var_name)->GetMutable<FeedFetchList>() =
      fetch_data;
  if (FLAGS_memory_used) {
    member_->MemoryUsedInRuntime();
  }
}
