...
class OpHandleBase;
class XPUThreadedSSAGraphExecutor : public SSAGraphExecutor {
 public:
  XPUThreadedSSAGraphExecutor(const ExecutionStrategy &strategy,
 ...
 private:
  ...
  void RunOpAsync(OpHandleBase *op,
                  const std::vector<std::shared_ptr<sem_t>> &signals,
