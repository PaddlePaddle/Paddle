...
template <typename T>
std::string GetArrayReadableData(int b, int e, int w, int summarize,
                                 void* data) {
  std::stringstream ss;
...
std::string GetMatrixReadableData(const std::string& message,
  ...
  ss << message << " " << name << ":";
  ss << GetArrayReadableData<T>(t_b, t_e, w, summarize, data);

  if (b_b > t_e) {
    std::cout << "  ... " << std::endl;
    ss << GetArrayReadableData<T>(b_b, b_e, w, summarize, data);
  }
