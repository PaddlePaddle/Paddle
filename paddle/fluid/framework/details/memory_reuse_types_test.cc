...
static inline std::string PrintPool(const OrderedReusedNodePairPool& p) {
  std::stringstream ss;
...
TEST(OrderedReusedNodePairPool, Normal) {
  OrderedReusedNodePairPool pool;
  std::vector<std::unique_ptr<VarDesc>> descs;
  std::vector<std::unique_ptr<ir::Node>> nodes;

  std::vector<std::vector<int64_t>> shapes = {{-1, 10},   {-1, 20}, {1, 2},
                                              {5, 2},     {10, 20}, {-1, 2, 5},
                                              {-1, 1, 5}, {-1, 1}};
  ...
  std::cout << PrintPool(pool) << std::endl;
