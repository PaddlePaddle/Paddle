...
bool AnalysisVarPass::NodeMatching(ir::Node* var, ir::Node* cache, int* node_idx_in_pool) const {
  auto compare_node_type_and_shape = [&](ir::Node* lhs, ir::Node* rhs) {
    ...
    if(lhs_desc->GetDataType() != rhs_desc->GetDataType()
       || lhs_desc->GetType() != rhs_desc->GetType()) return false;
    auto lhs_size = std::abs(std::accumulate(lhs_shape.begin(), lhs_shape.end(), 1));
    auto rhs_size = std::abs(std::accumulate(rhs_shape.begin(), rhs_shape.end(), 1));
...
std::unique_ptr<ir::Graph> AnalysisVarPass::ApplyImpl(std::unique_ptr<ir::Graph> graph) const {
  ...
  auto var_can_reused = [&](const ir::Node* node) {
    PADDLE_ENFORCE(node->IsVar(), string::Sprintf("Expect node %s as Variable.", node->Name()));
    auto* desc = node->var();
    auto* type = desc->GetType();
    if (fetched_vars.count(node)) {return false;}
    if (desc->Persistable() || (type != proto::VarType::LOD_TENSOR && type != proto::VarType::SELECTED_ROWS)
        || desc->GetShape().size() == 0) { return false;}
    for (auto* op : node->inputs) {
      // fill_constant on cpu can not be shared.
      if (op->Name() == "fill_constant" && op->HasAttr("force_cpu")) {return false;}
    }
    return true;
  };
