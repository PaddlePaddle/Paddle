...
ControlFlowGraph::ControlFlowGraph(std::unique_ptr<ir::Graph> graph) {
  ops_ = ir::TopologySortOperations(graph.get());
  // 1. build graph
  // for (auto& op : ops) {
  //   PADDLE_ENFORCE(op->NodeType() == ir::Node::Type::kOperation);
  // }
  for(auto& op: ops_) {
    PADDLE_ENFORCE(op->NodeType() == ir::Node::Type::kOperation, string::Sprintf("Expect op, but get %s ", op->Name()));
    ops_.insert(op);
    ...
    for (auto& input_var : op->inputs) {
      ...
      PADDLE_ENFORCE(input_var->NodeType() == ir::Node::Type::kVariable,
                     string::Sprintf("Expect var, but get %s", input_var->Name()));
      uses_[op].insert(input_var);
    }
    for (auto& output_var : op->inputs) {
      PADDLE_ENFORCE(output_var->NodeType() == ir::Node::Type::kVariable,
...
void ControlFlowGraph::DataAnalysis() {
  ...
  auto set_euqal = [](const std::unordered_set<ir::Node*>& lhs,
  ...
  while(!worklist.empty()) {
    ...
    worklist.pop_front();
    PADDLE_ENFORCE(live_in_.find(op) != live_in_.end(), string::Sprintf("Expect %s in live_in, but Not Found.", op->Name()));
    node_live_in = live_in_[op];
    for(auto& s : successors_[op]) {
      std::set_union(live_in_[s].begin(), live_in_[s].end(), live_out_[op].begin(), live_out_[op].end(), live_out_[op].begin());
    }
    live_in_[op].clear();
    ...
    // std::set_difference(live_out_[op].begin(), live_out_[op].end(), defs_[op].begin(), defs_[op].end(), live_in_[op].beign());
    // std::set_union(live_in_[op].begin(), live_in_[op].end(), defs_[op].begin(), defs_[op])
  }
}

void ControlFlowGraph::UpdateGraph(ir::Node* old_node, ir::Node* new_node, int begin_idx) {
  // update graph from begin idx to the end
  for(int i = begin_idx; i != ops_.size(); ++i) {
    auto* op = ops_[i];
...
const std::unordered_set<ir::Node*>& ControlFlowGraph::LiveIn(ir::Node* op) const {
  auto it = live_in_.find(op);
  PADDLE_ENFORCE(it != live_in_.end(), string::Sprintf("Expect %s in live_in, but Not Found.", op->Name()));
  return *it;
}

const std::unordered_set<ir::Node*>& ControlFlowGraph::LiveOut(ir::Node* op) const {
  auto it = live_out_.find(op);
  PADDLE_ENFORCE(it != live_out_.end(), string::Sprintf("Expect %s in live_out, but Not Found.", op->Name()));
  return *it;
}
const std::unordered_set<ir::Node*>& ControlFlowGraph::Def(ir::Node* op) const {
  auto it = defs_.find(op);
  PADDLE_ENFORCE(it != defs_.end(), string::Sprintf("Expect %s in defs, but Not Found.", op->Name()));
  return *it;
}

const std::unordered_set<ir::Node*>& ControlFlowGraph::Use(ir::Node* op) const {
  auto it = uses_.find(op);
  PADDLE_ENFORCE(it != uses_.end(), string::Sprintf("Expect %s in live_out, but Not Found.", op->Name()));
  return *it;
}

const std::vector<ir::Node*>& ControlFlowGraph::Ops() const {
  return ops_;
}
