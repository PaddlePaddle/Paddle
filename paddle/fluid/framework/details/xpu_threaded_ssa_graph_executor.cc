...
FetchResultType XPUThreadedSSAGraphExecutor::RunMultiStream(
  ...
  InsertFetchOps(fetch_tensors, &fetches, &fetched_vars, op_deps.get(),
                 &fetch_ops, &ready_fetch_ops);
  ready_ops = bootstrap_ops_;

  ready_ops.insert(ready_ops.end(), ready_fetch_ops.begin(),
                   ready_fetch_ops.end());
  exec_op_count_ = 0;

  platform::XPUPlace cur_place;
  int all_reduce_place_index = 0;
  for (uint32_t i = 0; i < ready_ops.size(); i++) {
    auto dev_ctxes_ = ready_ops[i]->DeviceContext();
    if (dev_ctxes_.size() > 1) {
      cur_place =
          boost::get<platform::XPUPlace>(places_[all_reduce_place_index]);
      all_reduce_place_index = (all_reduce_place_index + 1) % places_.size();
    } else if (dev_ctxes_.size() == 1) {
      cur_place = boost::get<platform::XPUPlace>(dev_ctxes_.begin()->first);
    } else {
      cur_place = boost::get<platform::XPUPlace>(
          dynamic_cast<ComputationOpHandle *>(ready_ops[i])->GetPlace());
    }
    int cur_index = get_pool_thread_index(cur_place.device);
    RunOpAsync((*op_deps)[ready_ops[i]].op, (*op_deps)[ready_ops[i]].signals,
               (*op_deps)[ready_ops[i]].waits, cur_index, cur_place);
    stream_op_count_[cur_index]++;
    auto cur_op = ready_ops[i];
    auto children = get_children(cur_op);
    while (children.size() == 1 && get_parents(children[0]).size() == 1) {
      cur_op = children[0];
      RunOpAsync((*op_deps)[cur_op].op, (*op_deps)[cur_op].signals,
                 (*op_deps)[cur_op].waits, cur_index, cur_place);
      stream_op_count_[cur_index]++;
      children = get_children(cur_op);
    }

    if (children.size() == 1 && get_parents(children[0]).size() > 1) {
      if (places_.size() > 1 || multi_stream_num_ > 1) {
        std::vector<std::shared_ptr<sem_t>> signals;
        std::vector<std::shared_ptr<sem_t>> waits;
        signals.push_back(std::make_shared<sem_t>());
        sem_init(signals[0].get(), 0, 0);
        RunOpAsync(nullptr, signals, waits, cur_index, cur_place);
        (*op_deps)[children[0]].waits.push_back(signals[0]);
      }
      (*op_deps)[children[0]].dep_num--;
      if ((*op_deps)[children[0]].dep_num == 0) {
        ready_ops.push_back(children[0]);
      }
      continue;
    }
    if (children.size() > 1) {
      ...
      for (uint32_t j = 0; j < children.size(); j++) {
        if (places_.size() > 1 || multi_stream_num_ > 1) {
          signals.push_back(std::make_shared<sem_t>());
          sem_init(signals[j].get(), 0, 0);
          (*op_deps)[children[j]].waits.push_back(signals[j]);
        }
        (*op_deps)[children[j]].dep_num--;

        if ((*op_deps)[children[j]].dep_num == 0) {
          ready_ops.push_back(children[j]);
        }
      }
      if (places_.size() > 1 || multi_stream_num_ > 1) {
        RunOpAsync(nullptr, signals, waits, cur_index, cur_place);
  ...
  while (exec_op_count_ != op_deps->size()) {
  }
