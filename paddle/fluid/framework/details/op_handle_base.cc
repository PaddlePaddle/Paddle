...
void OpHandleBase::InitCUDA() {
  ...
  for (auto &p : dev_ctxes_) {
    int dev_id = BOOST_GET(platform::CUDAPlace, p.first).device;
...
void OpHandleBase::RunAndRecordEvent(const std::function<void()> &callback) {
  ...
  if (!events_.empty()) {  // Use event
    for (auto &p : dev_ctxes_) {
      auto dev_id = BOOST_GET(platform::CUDAPlace, p.first).device;
...
void OpHandleBase::RunAndRecordEvent(platform::Place p,
  ...
  if (platform::is_cpu_place(p) || events_.empty()) {
    ...
    cuda_ctx->RecordEvent(events_.at(BOOST_GET(platform::CUDAPlace, p).device),
                          callback);
  }
#else
  callback();
