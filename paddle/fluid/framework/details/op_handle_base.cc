...
void OpHandleBase::InitCUDA() {
  ...
  for (auto &p : dev_ctxes_) {
    int dev_id = static_cast<unsigned char>(p.first.device);
  ...
  if (IsMultiDeviceTransfer() && !dev_ctxes_.empty()) {
    for (auto &out_var : outputs_) {
      ...
      if (out_var_handle) {
        int dev_id = static_cast<unsigned char>(out_var_handle->place().device);
    ...
    int dev_id = static_cast<unsigned char>(place.device);
    for (auto &out_var : outputs_) {
      auto *out_var_handle = dynamic_cast<VarHandle *>(out_var);
      if (out_var_handle) {
        PADDLE_ENFORCE_EQ(
