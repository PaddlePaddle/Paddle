...
void DataBalanceOpHandle::RunImpl() {
  ...
  for (size_t i = 0; i < local_scopes_.size(); ++i) {
    ...
    PADDLE_ENFORCE(places_[i] == tensor->place());  // for test
    lod_tensors.push_back(tensor);
    device_sizes.emplace_back(tensor->lod().empty() ? tensor->dims()[0]
                                                    : tensor->NumElements());
  }
  const auto &balance_plan = GetBalancePlan(device_sizes);

  for (const auto &trans : balance_plan) {
    LoDTensor *src_tensor = lod_tensors[trans[0]];
    LoDTensor *dst_tensor = lod_tensors[trans[1]];
