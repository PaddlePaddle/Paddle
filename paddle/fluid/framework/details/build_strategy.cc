  ...
  for (std::shared_ptr<ir::Pass> &pass : pass_builder_->AllPasses()) {
    ...
    if (IsMultiDevPass(pass->Type())) {
      ...
      platform::NCCLCommunicator *nctx =
          (device == kCUDA) ? nccl_ctxs : nullptr;
      pass->Erase(kNCCLCtxs);
      pass->SetNotOwned<platform::NCCLCommunicator>(kNCCLCtxs, nctx);
#elif defined(PADDLE_WITH_XPU) && defined(PADDLE_WITH_XPU_BKCL)
      // ToDo: more check
      platform::BKCLCommunicator *bkcl_ctx =
          (device == kXPU) ? bkcl_ctxs : nullptr;
...
#elif defined(PADDLE_WITH_XPU) && defined(PADDLE_WITH_XPU_BKCL)
      platform::BKCLCommunicator *nctx = (device == kXPU) ? bkcl_ctxs : nullptr;
      pass->Erase(kBKCLCtxs);
      ...
      pass->Set<bool>("use_gpu", new bool((device == kCUDA)));
      if (device != kCUDA) {
