...
PDNode *patterns::BidirecGRU::operator()(paddle::framework::ir::PDNode *x,
                                         bool with_bias) {
  // Create shared nodes.
  auto *xnode = pattern->NewNode(xnode_repr());

#define NEW_NODE(arg__, io__) \
  auto *arg__ =               \
      pattern->NewNode(arg__##_repr())->assert_is_op_##io__("gru", #arg__);

  auto *xnode_out_var = pattern->NewNode(xnode_out_repr());
  xnode_out_var->AsIntermediate()->assert_is_op_input("mul");
  ...
  xnode->LinksFrom({x}).LinksTo({xnode_out_var});
  mul0->LinksFrom({xnode_out_var, mul0_w_var}).LinksTo({mul0_out_var});
  if (with_bias) {
  ...
  xnode->LinksFrom({x}).LinksTo({xnode_out_var});
  mul1->LinksFrom({xnode_out_var, mul1_w_var}).LinksTo({mul1_out_var});
