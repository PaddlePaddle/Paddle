...
PDNode *patterns::ElementwiseActivation::operator()(
  ...
  auto *elementwise_op = pattern->NewNode(elementwise_repr())->assert_is_op(elementwise_type);
  auto *activation_op =
      pattern->NewNode(activation_repr())->assert_is_op(activation_type);
  // Create variables
  ...
  elementwise_op->LinksFrom({elementwise_a, elementwise_b}).LinksTo({elementwise_out_var});
