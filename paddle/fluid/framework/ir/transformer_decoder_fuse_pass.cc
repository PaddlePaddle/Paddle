...
#include "paddle/fluid/framework/ir/transformer_decoder_fuse_pass.h"
...
PDNode* TransformerDecoderPattern::operator()() {
  ...
  auto* k_cache_r_var = pattern->NewNode(k_cache_r_repr())  
                             ->assert_is_op_input_debug("gather");
  auto* gather1 = pattern->NewNode(gather1_repr())->assert_is_op("gather"); 
  auto* gather1_out_var = pattern->NewNode(gather1_out_repr())  
                             ->assert_is_op_output("gather")
  ...
  auto* gather2 = pattern->NewNode(gather2_repr())->assert_is_op("gather"); 
  auto* gather2_out_var = pattern->NewNode(gather2_out_repr())  
                             ->assert_is_op_output("gather")
                             ->assert_is_op_input("concat");
  auto* gather_index_var = pattern->NewNode(gather_index_repr())                         
                             ->assert_is_op_input("gather");
  ...
  concat1->LinksFrom({transpose2_1_out_var, gather1_out_var})
      .LinksTo({concat1_out_var});
  assign1->LinksFrom({concat1_out_var}).LinksTo({k_cache_w_var});
  gather1->LinksFrom({k_cache_r_var, gather_index_var}).LinksTo({gather1_out_var});

  concat2->LinksFrom({transpose2_2_out_var, gather2_out_var})
      .LinksTo({concat2_out_var});
  assign2->LinksFrom({concat2_out_var}).LinksTo({v_cache_w_var});
  gather2->LinksFrom({v_cache_r_var, gather_index_var}).LinksTo({gather2_out_var});
...
TransformerDecoderV2FusePass::TransformerDecoderV2FusePass() {
...
int TransformerDecoderV2FusePass::BuildFusionV2(Graph* graph,
  ...
  // Create pattern.
  patterns::TransformerDecoderPattern multihead_pattern(pattern, name_scope);
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    if (!IsCompat(subgraph, g)) {
      LOG(WARNING)
          << "Op compat check in transformer_decoder_fuse_pass_v2 failed.";
      return;
    }
    ...
    GET_IR_NODE_FROM_SUBGRAPH(gather1, gather1, multihead_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(gather2, gather2, multihead_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(gather1_out, gather1_out, multihead_pattern);
...
void TransformerDecoderV2FusePass::ApplyImpl(Graph* graph) const {
  FusePassBase::Init(name_scope_, graph);
  auto* scope = param_scope();
  PADDLE_ENFORCE_NOT_NULL(
      scope,
      platform::errors::Fatal(
          "During the TransformerDecoder pass, The scope should not be null."));

  int fusion_count = BuildFusionV2(graph, name_scope_, scope);
  if (fusion_count > 0) {
    graph->Set(kTransformerDecoderPass, new bool(true));
  }
...
REGISTER_PASS(transformer_decoder_fuse_pass_v2,
              paddle::framework::ir::TransformerDecoderV2FusePass);
REGISTER_PASS(transformer_decoder_fuse_pass,
              paddle::framework::ir::TransformerDecoderV2FusePass);

REGISTER_PASS_CAPABILITY(transformer_decoder_fuse_pass_v2)
    .AddCombination(
