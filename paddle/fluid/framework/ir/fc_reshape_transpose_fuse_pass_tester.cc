...
#include "paddle/fluid/framework/ir/fc_reshape_transpose_fuse_pass.h"  // NOLINT
...
static void AddVarToScope(Scope* param_scope, const std::string& name,
                          const DDim& dims) {
...
TEST(FCReshapeTransposeFusePass, basic) {
  // inputs                           operator            output
  // --------------------------------------------------------------------
  // (x)                              dropout          -> dropout_out
  ...
  auto dropout_out = layers.dropout(x, 0.1, "downgrade_in_infer");
  ...
  auto* mul_out_0 = layers.mul(dropout_out, weights_0);
  auto* mul_out_1 = layers.mul(dropout_out, weights_1);
  auto* mul_out_2 = layers.mul(dropout_out, weights_2);
  ...
  auto pass = PassRegistry::Instance().Get("fc_reshape_transpose_fuse_pass");
  ...
  // after fuse mul=2
  int num_fused_nodes_after = GetNumOpNodes(graph, "mul");
  VLOG(3) << DebugString(graph);

  PADDLE_ENFORCE_EQ(num_nodes_before, num_nodes_after + 18);
  PADDLE_ENFORCE_EQ(num_fused_nodes_after, 2);
}
...
USE_PASS(fc_reshape_transpose_fuse_pass);
