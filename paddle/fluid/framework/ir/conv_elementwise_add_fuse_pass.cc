...
void ConvElementwiseAddFusePass::ApplyImpl(ir::Graph* graph) const {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    //set the out_threshold of the elementwise add op to be the out_threshold of the conv2d_fusion
    if(out_threshold_attr.which()) {
      VLOG(1) << "setting out_threshold: " << boost::get<float>(out_threshold_attr);
