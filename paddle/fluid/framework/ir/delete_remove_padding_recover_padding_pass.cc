...
void DeleteRemovePaddingRecoverPaddingPass::ApplyImpl(ir::Graph *graph) const {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t &subgraph,
    ...
    for (auto &output : recover_padding_out->outputs) {
      if (recover_padding_out->output->Name() == "remove_padding") {  // op_node
        auto *remove_padding_out_node =
            recover_padding_out->output->outputs[0];          // var_node
        auto *out_op_node = remove_padding_out_node->output;  // op_node
        IR_NODE_LINK_TO(recover_padding_input, out_op_node);
        del_node_set.insert(recover_padding_out->output);
