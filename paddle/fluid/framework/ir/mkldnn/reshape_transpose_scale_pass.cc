...
void ReshapeTransposeScalePass::ApplyImpl(ir::Graph* graph) const {
  ...
  patterns::ReshapeTransposeScale reshape_transpose_scale_pattern{
      gpd.mutable_pattern(), "reshape_transpose_scale_pass"};
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    transpose_op->Op()->SetOutput(
        "Out", std::vector<std::string>({scale_out->Name()}));
  ...
  PrettyLogDetail(
      "---   squashed %d reshape2-transpose2-scale pairs in "
      "reshape_transpose_scale_pass.cc",
      found_reshape_transpose_scale_count);
}
