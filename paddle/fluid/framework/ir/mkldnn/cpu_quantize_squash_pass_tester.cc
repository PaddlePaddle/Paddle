...
void EqualScaleTest(const ProgramDesc& prog, const std::string& op_name,
  ...
  for (auto* node : graph->Nodes()) {
    if (node->IsOp() &&
        BOOST_GET(std::string, node->Op()->GetAttr("name")) == op_name) {
      float op_scale = BOOST_GET(float, node->Op()->GetAttr(scale_name));
...
void CheckRequantScalesTest(const ProgramDesc& prog, float scale_in,
  ...
  for (auto* node : graph->Nodes()) {
    if (node->IsOp() && node->Op()->Type() == "requantize") {
      float op_scale_in = BOOST_GET(float, node->Op()->GetAttr("Scale_in"));
      EXPECT_EQ(op_scale_in, scale_in);
      float op_scale_out = BOOST_GET(float, node->Op()->GetAttr("Scale_out"));
