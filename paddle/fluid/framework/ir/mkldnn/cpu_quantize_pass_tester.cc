...
static const std::initializer_list<std::string> variable_names_elementwise_mul =
    {"a", "b", "c", "d", "e", "f"};

ProgramDesc BuildProgramDescElementwiseMul() {
  ProgramDesc prog;
  for (auto& v : variable_names_elementwise_mul) {
    prog.MutableBlock(0)->Var(v);
  }
  SetOp(&prog, "dequantize", "Dequantize1", {"a"}, {"b"}, true);
  SetOp(&prog, "dequantize", "Dequantize2", {"c"}, {"d"}, true);
  SetOp(&prog, "elementwise_mul", "ElementwiseMul", {"b", "d"}, {"e"}, true,
        "int8");
  SetOp(&prog, "dropout", "Dropout", {"e"}, {"f"}, true, "float32");
...
TEST(CpuQuantizePass, elementwise_mul) {
  ...
  int added_nodes = 6;
  std::unordered_map<std::string, int> expected_operators = {
      {"elementwise_mul", 1}, {"quantize", 2}, {"dequantize", 3}};
  MainTest(BuildProgramDescElementwiseMul(), variable_names_elementwise_mul,
           expected_operators, added_nodes, SCALE * S8_MAX);
}
...
TEST(CpuQuantizePass, elementwise_mul_output_scale_missing) {
  int added_nodes = 0;
  std::unordered_map<std::string, int> expected_operators = {
      {"elementwise_mul", 1}, {"quantize", 0}, {"dequantize", 2}};
  MainTest(BuildProgramDescElementwiseMul(), variable_names_elementwise_mul,
           expected_operators, added_nodes, 1.f, 1.f, "e");
}
