  ...
  for (auto* node : graph->Nodes()) {
    if (node->IsOp()) {
      ...
      if (op->Type() == "conv2d") {
        ...
        auto scale_in_attr = BOOST_GET(float, op->GetAttr("Scale_in"));
        auto scale_out_attr = BOOST_GET(float, op->GetAttr("Scale_out"));
        auto scale_weights_attr =
            BOOST_GET(std::vector<float>, op->GetAttr("Scale_weights"));
        EXPECT_EQ(scale_in_attr, scale)
            << "Scale_in for node '" + op_name + "'.";
        EXPECT_EQ(scale_out_attr, scale)
            << "Scale_out for node '" + op_name + "'.";
        EXPECT_EQ(scale_weights_attr[0], scale)
  ...
  for (auto* node : graph->Nodes()) {
    if (node->IsOp()) {
      ...
      if (op->Type() == "matmul") {
        matmul_nodes_count++;
        auto op_name = BOOST_GET(std::string, op->GetAttr("name"));
        auto scale_x_attr = BOOST_GET(float, op->GetAttr("Scale_x"));
        auto scale_y_attr = BOOST_GET(float, op->GetAttr("Scale_y"));
        auto scale_out_attr = BOOST_GET(float, op->GetAttr("Scale_out"));

        EXPECT_EQ(scale_x_attr, scale) << "Scale_x for node '" + op_name + "'.";
        EXPECT_EQ(scale_y_attr, scale) << "Scale_y for node '" + op_name + "'.";
        EXPECT_EQ(scale_out_attr, scale)
            << "Scale_out for node '" + op_name + "'.";
      } else if (op->Type() == "quantize") {
        quantize_nodes_count++;
