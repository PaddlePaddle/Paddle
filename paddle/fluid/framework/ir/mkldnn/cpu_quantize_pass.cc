...
void CPUQuantizePass::QuantizeFusionGru2(Graph* graph) const {
  GraphPatternDetector gpd;
  patterns::FusionGru2 pattern{gpd.mutable_pattern(), name_scope_};
  pattern();

  int quantize_count = 0;
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* g) {
    VLOG(4) << "Quantize bidirectional fusion_gru op";
    GET_IR_NODE_FROM_SUBGRAPH(op, op, pattern);
    ...
    if (!AreScalesPresentForNodes({x, wx1, wx2})) {
      LogCannotQuantizeOp(op);
      return;
    }
    ...
    auto weight_scale_tensor1 = GetScaleTensorForNode(wx1);
    auto weight_scale_tensor2 = GetScaleTensorForNode(wx2);
    EigenVectorArrayMap<double> eigen_tensor1{
        weight_scale_tensor1.data<double>(), weight_scale_tensor1.numel(), 1};
    EigenVectorArrayMap<double> eigen_tensor2{
        weight_scale_tensor2.data<double>(), weight_scale_tensor2.numel(), 1};
    eigen_tensor1 *= static_cast<double>(S8_MAX);
    eigen_tensor2 *= static_cast<double>(S8_MAX);
    std::vector<float> scale_weights{
        weight_scale_tensor1.data<double>(),
        weight_scale_tensor1.data<double>() + weight_scale_tensor1.numel()};
    std::vector<float> scale_weights_reverse{
        weight_scale_tensor2.data<double>(),
        weight_scale_tensor2.data<double>() + weight_scale_tensor2.numel()};

    op->Op()->SetAttr("Scale_weights", scale_weights);
    op->Op()->SetAttr("Scale_weights_reverse", scale_weights_reverse);
    // return fp32 data
    op->Op()->SetAttr("force_fp32_output", true);
  ...
  PrettyLogDetail("---    quantized %d bidirectional fusion_gru ops",
                  quantize_count);
}
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    for (int i = 0; i < wx_size; ++i) {
      ...
      EigenVectorArrayMap<double> eigen_tensor_src{
          scale_tensor_src.data<double>(), scale_tensor_src.numel(), 1};
      ...
      EigenVectorArrayMap<float> eigen_tensor_dst{
          dst_data, w_scale_tensor_dst->numel(), 1};
      eigen_tensor_dst =
