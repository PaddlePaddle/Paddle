...
class Quanter {
 public:
  void AddQuantOps() {
    ...
    for(const auto& logical_xput : op_xputs) {
      ...
      if(IsNotPermittedName(logical_xput_name))
        continue;
      ...
      for(const auto& physical_xput_name: physical_xputs_names) {
        ...
        VarDesc quant_x_desc(patterns::PDNodeName(get_op_type(), get_op_edge()));
 ...
 protected:
  ...
  Quanter(Graph& graph, ir::Node* const op, const VariableNameMap& op_xputs) : graph(graph), op(op), op_xputs(op_xputs) {};
  ...
  virtual void link_nodes(ir::Node* const physical_xput_node, ir::Node* const quant_op, ir::Node* const quant_x_node) = 0;
  virtual void set_edge(const std::string& logical_xput_name, const std::vector<std::string>& quant_xput_names) = 0;

  bool IsAlreadyLinked(const std::vector<std::string>& node_names, const std::string& node_name) const
  {
    return std::find(node_names.begin(), node_names.end(), node_name) != node_names.end();
  }

  virtual ir::Node* create_quant_op(const std::string& input_name, const std::string& output_name) const {
    OpDesc op_desc;
...
class Quantizer final : public Quanter{
 public:
  Quantizer(Graph* const graph, ir::Node* const op) : Quanter(*graph, op, op->Op()->Inputs()) {
 ...
 protected:
  ...
  bool IsNotPermittedName(const std::string& input_name) const override {
    ...
    return std::none_of(permitted_names.begin(),
                        permitted_names.end(),
                        [&input_name](const std::string& name) {
  ...
  void link_nodes(ir::Node* const physical_xput_node, ir::Node* const quant_op, ir::Node* const quant_x_node) override {
  ...
  void set_edge(const std::string& logical_xput_name, const std::vector<std::string>& quant_xput_names) override
  {
...
class DeQuantizer final : public Quanter {
 public:
  DeQuantizer(Graph* const graph, ir::Node* const op) : Quanter(*graph, op, op->Op()->Outputs()) {
 ...
 protected:
  ...
  ir::Node* create_quant_op(const std::string& input_name, const std::string& output_name) const override {
