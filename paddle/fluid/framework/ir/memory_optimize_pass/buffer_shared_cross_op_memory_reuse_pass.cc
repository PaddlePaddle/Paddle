...
std::vector<OpHandleBase *> BufferSharedCrossOpMemoryReusePass::SortOp(
  ...
  PADDLE_ENFORCE_EQ(
      ...
      platform::errors::Fatal("sorted_ops size not equal to graph op size. "
...
size_t BufferSharedCrossOpMemoryReusePass::ResolveDependencyBetween(
  ...
  for (auto *prev_op : prev_ops) {
    ...
    PADDLE_ENFORCE_EQ(
        op_dep, NodeDependency::kNoDep,
        platform::errors::Fatal("The graph has circle, this may be a bug"));
...
void BufferSharedCrossOpMemoryReusePass::BuildOpDependencyMap() const {
  PADDLE_ENFORCE_EQ(ops_.empty(), true,
                    platform::errors::Fatal("ops_ must be initialized here"));
  PADDLE_ENFORCE_EQ(
      op_to_idx_.empty(), true,
      platform::errors::Fatal("op_to_idx_ must be initialized here"));
  PADDLE_ENFORCE_EQ(deps_.empty(), true,
                    platform::errors::Fatal("deps_ must be initialized here"));
  ...
  PADDLE_ENFORCE_EQ(
      ...
      platform::errors::Fatal("preceding_ops size must equal to op_num"));
...
size_t BufferSharedCrossOpMemoryReusePass::OpIndex(
  ...
  PADDLE_ENFORCE_NE(iter, op_to_idx_[op->GetScopeIdx()].end(),
                    platform::errors::Fatal(
                        "can not find op(%s) in op_to_index", op->Name()));
  return iter->second;
}
...
NodeDependency BufferSharedCrossOpMemoryReusePass::GetOpDep(
  ...
  PADDLE_ENFORCE_EQ(
      op1->GetScopeIdx(), op2->GetScopeIdx(),
      platform::errors::Fatal("op(%s) and op(%s) must in the same scope.",
                              op1->Name(), op2->Name()));
...
void BufferSharedCrossOpMemoryReusePass::SetOpDep(
  ...
  if (op1 == op2) {
    PADDLE_ENFORCE_EQ(
        ...
        platform::errors::Fatal("Set Same Op(%s) Dep, dep must be kSame.",
                                op1->Name()));
