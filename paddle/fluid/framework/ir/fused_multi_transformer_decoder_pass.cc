...
int FusedMultiTransformerDecoderPass::BuildFusion(Graph* graph, const std::string& name_scope, Scope* scope) const {
  ...
  auto fuse_creater = [&](Node* input0,
    ...
    // Time Step
    // VarDesc timestep_desc("timestep" + std::to_string(layer_idx));
    // timestep_desc.SetDataType(proto::VarType::INT32);
    // timestep_desc.SetPersistable(true);
    // auto* timestep = graph->CreateVarNode(&timestep_desc);
    // auto* timestep_tensor = scope->Var(timestep->Name())->GetMutable<LoDTensor>();
    // timestep_tensor->Resize(DDim{1});
    // std::fill_n(timestep_tensor->mutable_data<int>(platform::CPUPlace()),
    //             timestep_tensor->numel(), layer_idx + 64);
    // fused_multi_transformer_op_desc.SetInput("TimeStep", {timestep->Name()});
