...
PDNode* TrtMultiHeadMatmulV4Pattern::operator()() {
  std::unordered_set<std::string> mul_ops{"mul", "matmul_v2"};
  std::unordered_set<std::string> matmul_ops{"matmul", "matmul_v2"};
  auto* input0 = pattern->NewNode(input0_repr());
  input0->assert_is_ops_input(mul_ops);
  VLOG(5)<<"@@@@ wangbojun, TrtMultiHeadMatmulV4Pattern";
  // First path
  auto* mul0 = pattern->NewNode(mul0_repr())->assert_is_ops(mul_ops);
  auto* mul0_w_var = pattern->NewNode(mul0_w_repr())
                         ->AsInput()
...
PDNode* TrtMultiHeadMatmulV4CrossPattern::operator()() {
  ...
  VLOG(5)<<"@@@@ wangbojun, TrtMultiHeadMatmulV4CrossPattern";
  // First path
  auto* mul0 = pattern->NewNode(mul0_repr())->assert_is_ops(mul_ops);
  auto* mul0_w_var = pattern->NewNode(mul0_w_repr())
                         ->AsInput()
...
TrtMultiHeadMatmulV4FusePass::TrtMultiHeadMatmulV4FusePass() {
  AddOpCompat(OpCompat("mul"))
      .AddInput("X")  // the shape shoule be (B, S, N*H)
      .IsTensor()
      .End()
...
int TrtMultiHeadMatmulV4FusePass::BuildFusionV4(Graph* graph,
                                                const std::string& name_scope,
                                                Scope* scope) const {
    GraphPatternDetector gpd;
    auto* pattern = gpd.mutable_pattern();
    ...
    // Create pattern.
    patterns::TrtMultiHeadMatmulV4Pattern multihead_pattern(pattern, name_scope);
  ...
  auto fuse_creater = [&](Node* input0,
        ...
        int head_number =
            PADDLE_GET_CONST(std::vector<int>, reshape_desc->GetAttr("shape"))
                .at(2);
        multihead_op_desc.SetType("multihead_matmul_v4");
        multihead_op_desc.SetInput("Input",{input0->Name()});
        ...
        // printf("@@ print q fc weight [%s] : \n ",mul0_w->Name().c_str());
        // for(int i =0;i<5;i++){
        // printf("%f ",wq_data[i]);
        // }
        // printf("\n");
        // printf("@@ print k fc weight [%s] : \n ",mul1_w->Name().c_str());
        // for(int i =0;i<5;i++){
        // printf("%f ",wk_data[i]);
        // }
        // printf("\n");
        // printf("@@ print v fc weight [%s] : \n ",mul2_w->Name().c_str());
        // for(int i =0;i<5;i++){
        // printf("%f ",wv_data[i]);
        // }
        // printf("\n");
...
void TrtMultiHeadMatmulV4FusePass::ApplyImpl(Graph* graph) const {
  FusePassBase::Init(name_scope_, graph);
  auto* scope = param_scope();
  int fusion_count = BuildFusionV4(graph, name_scope_, scope);
  AddStatis(fusion_count);
}


TrtMultiHeadMatmulV4CrossFusePass::TrtMultiHeadMatmulV4CrossFusePass() {
  AddOpCompat(OpCompat("mul"))
      .AddInput("X")  // the shape shoule be (B, S, N*H)
      .IsTensor()
      .End()
...
int TrtMultiHeadMatmulV4CrossFusePass::BuildFusionV4Cross(Graph* graph,
                                                const std::string& name_scope,
                                                Scope* scope) const {
    GraphPatternDetector gpd;
    auto* pattern = gpd.mutable_pattern();
    ...
    // Create pattern.
    patterns::TrtMultiHeadMatmulV4CrossPattern multihead_pattern(pattern, name_scope);
  ...
  auto fuse_creater = [&](Node* input0,
        ...
        int head_number =
            PADDLE_GET_CONST(std::vector<int>, reshape_desc->GetAttr("shape"))
                .at(2);
        multihead_op_desc.SetType("multihead_matmul_v4_cross");
...
void TrtMultiHeadMatmulV4CrossFusePass::ApplyImpl(Graph* graph) const {
  FusePassBase::Init(name_scope_, graph);
  auto* scope = param_scope();
  int fusion_count = BuildFusionV4Cross(graph, name_scope_, scope);
  AddStatis(fusion_count);
}
