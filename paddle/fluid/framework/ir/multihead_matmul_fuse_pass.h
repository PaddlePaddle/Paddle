...
struct MultiHeadMatmulWithAttentionPattern : public PatternBase {
  MultiHeadMatmulWithAttentionPattern(PDPattern* pattern, const std::string& name_scope)
      : PatternBase(pattern, name_scope, "multihead_matmul_with_attention") {}
...
class MultiHeadMatmulWithAttentionFusePass : public FusePassBase {
 public:
  MultiHeadMatmulWithAttentionFusePass();
 ...
 private:
  int BuildFusionWithAttention(Graph* graph,
