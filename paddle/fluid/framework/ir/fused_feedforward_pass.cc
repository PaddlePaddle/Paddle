...
ir::Graph *FusedFeedForwardPass::FusedFeedForwardFwd(
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t &subgraph,
    ...
    if (use_mp) {
      GET_IR_NODE_FROM_SUBGRAPH(
          all_reduce_op, all_reduce_op, fused_feedforward_pattern);
      ring_id = PADDLE_GET_CONST(int, all_reduce_op->Op()->GetAttr("ring_id"));
    }
    ...
    if (use_mp) {
      ...
      nodes_to_remove.insert(all_reduce_op);
    }
    if (use_dropout_1) {
      GET_IR_NODE_FROM_SUBGRAPH(
          dropout_op_1, dropout_op_1, fused_feedforward_pattern);
