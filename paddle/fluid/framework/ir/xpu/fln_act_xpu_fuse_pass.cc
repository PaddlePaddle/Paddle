...
struct FLNActXPUPattern : public PatternBase {
  FLNActXPUPattern(PDPattern* pattern,
...
FLNActXPUPattern::FLNActXPUPattern(PDPattern* pattern,
  ...
  ln->LinksFrom({ln_input, ln_bias, ln_scale})
      .LinksTo({ln_out});

  ln_out->assert_is_op_input(act_type_, "X");
  auto act = pattern->NewNode(act_repr())->assert_is_op(act_type_);
  auto act_out = pattern->NewNode(act_out_repr())
                     ->assert_is_op_output(act_type_, "Out")
                     ->AsOutput();
  act->LinksFrom({ln_out}).LinksTo({act_out});
}

}  // namespace patterns

class FLNActXPUFusePass : public FusePassBase {
 protected:
 ...
 private:
  ...
  const std::string name_scope_{"fln_act_xpu_fuse_pass"};
};

void FLNActXPUFusePass::ApplyImpl(ir::Graph* graph) const {
...
int FLNActXPUFusePass::ApplyImpl(ir::Graph* graph,
                                const std::string& act_type) const {
  
  GraphPatternDetector gpd;
  patterns::FLNActXPUPattern pattern(
      gpd.mutable_pattern(), name_scope_, act_type);
  int found_subgraph_count = 0;
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* graph) {
    VLOG(4) << "handle FLNActXPUFusePass fuse";
    // declare operator node's name
    GET_IR_NODE(ln);
    GET_IR_NODE(act);
    // declare variable node's name
    GET_IR_NODE(ln_input);
    GET_IR_NODE(ln_bias);
    ...
    fused_op_desc.SetType("fln_act_xpu");
    ...
    fused_op_desc.SetInput("x", {ln_input->Name()});
...
REGISTER_PASS(fln_act_xpu_fuse_pass, paddle::framework::ir::FLNActXPUFusePass);

REGISTER_PASS_CAPABILITY(fln_act_xpu_fuse_pass)
    .AddCombination(
