...
static std::string get_input_names(const Node* node) {
  if (node == nullptr) {
    return "{empty}";
  }
...
static std::string get_output_names(const Node* node) {
  if (node == nullptr) {
    return "{empty}";
  }
  ...
  std::stringstream ss;
  ss << "{";
  for (auto out_op : node->outputs) {
    auto out_name = out_op->Name();
    ss << out_name << ", ";
  }
  ss << "}";
  return ss.str();
...
int FusedMultiTransformerQuantPass::ApplyImpl(ir::Graph* graph,
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* graph) {
    LOG(INFO) << "handle FusedMultiTransformerQuantPass fuse";
    ...
    auto quant_func = [&](const std::string& input_name,
                          ...
                          bool need_transpose) {
      typedef int16_t TW;  // TODO(mayang02):
