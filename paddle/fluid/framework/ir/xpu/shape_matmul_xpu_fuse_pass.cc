...
#include "paddle/fluid/framework/ir/xpu/shape_matmul_xpu_fuse_pass.h"
...
Reshape2MatmulPattern::Reshape2MatmulPattern(PDPattern* pattern,
  ...
  auto* reshape2 = pattern->NewNode(reshape2_repr())
  ...
  auto* matmul = pattern->NewNode(matmul_repr())
                     ...
                     ->assert_more([](Node* node) {
                       auto* op_desc = node->Op();
                       auto matmul_shape_attr =
                           op_desc->GetAttrIfExists<std::vector<int>>("shape");
                       auto matmul_alpha_attr =
                           op_desc->GetAttrIfExists<float>("alpha");
                       return matmul_shape_attr.size() == 2 &&
                              std::abs(matmul_alpha_attr - 1.f) < 1e-5;
                     });
  auto* matmul_out = pattern->NewNode(matmul_out_repr())
                         ->assert_is_op_output("matmul", "Out")
                         ->AsOutput();
