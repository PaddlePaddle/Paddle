...
GroupNormlizeSiluXPUPattern::GroupNormlizeSiluXPUPattern(PDPattern* pattern,
  ...
  auto gn =
      pattern->NewNode(gn_repr())->assert_is_op("group_norm");
  ...
  auto gn_variance = pattern->NewNode(gn_variance_repr())
                     ...
                     ->assert_has_n_outputs(0);
  gn->LinksFrom({gn_x, gn_bias, gn_scale}).LinksTo({gn_y, gn_mean, gn_variance});
...
class GroupNormalizeSiluXPUFusePass : public FusePassBase {
 ...
 private:
  ...
  const std::string name_scope_{"gn_silu_xpu_fuse_pass"};
};
...
void GroupNormalizeSiluXPUFusePass::FuseGroupNormalizeSilu(ir::Graph* graph) const {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    int groups =
        PADDLE_GET_CONST(int, gn->Op()->GetAttr("groups"));
...
REGISTER_PASS(gn_silu_xpu_fuse_pass,
              paddle::framework::ir::GroupNormalizeSiluXPUFusePass);

REGISTER_PASS_CAPABILITY(gn_silu_xpu_fuse_pass)
    .AddCombination(
