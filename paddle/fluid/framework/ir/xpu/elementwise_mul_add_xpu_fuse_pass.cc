  ...
  mul_x       Y    mul_y
    \        /     /
     \      /     /
      ...
      ele_mul_add
*/
struct EleMulAddXPUPattern : public PatternBase {
  EleMulAddXPUPattern(PDPattern* pattern, const std::string& name_scope);
...
EleMulAddXPUPattern::EleMulAddXPUPattern(PDPattern* pattern,
...
class EleMulAddXPUFusePass : public FusePassBase {
 protected:
...
void EleMulAddXPUFusePass::ApplyImpl(ir::Graph* graph) const {
...
int EleMulAddXPUFusePass::ApplyImpl_(ir::Graph* graph) const {
  GraphPatternDetector gpd;
  patterns::EleMulAddXPUPattern pattern(gpd.mutable_pattern(), name_scope_);
  int found_subgraph_count = 0;
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* graph) {
    VLOG(4) << "handle EleMulAddXPUFusePass fuse";
    // declare operator node's name
    GET_IR_NODE(scale);
    GET_IR_NODE(ele_add);
...
REGISTER_PASS(elementwise_mul_add_xpu_fuse_pass,
              paddle::framework::ir::EleMulAddXPUFusePass);
