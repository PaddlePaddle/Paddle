...
SpatialTransformerResBlockXPUPattern::SpatialTransformerResBlockXPUPattern(
    PDPattern* pattern, const std::string& name_scope, bool conv_fix, bool input_max, 
    bool has_silu_fc_input, bool include_silu)
    : PatternBase(pattern, name_scope, name_scope), 
    conv_fix_(conv_fix), input_max_(input_max),
    has_silu_fc_input_(has_silu_fc_input), include_silu_(include_silu) {
  // gn_silu_0
  auto gn_silu_0 =
  ...
  if(conv_fix_){
    ...
    // conv2d_2
    auto conv2d_2 = pattern->NewNode(conv2d_2_repr())->assert_is_op("conv2d_xpu");
    ...
    std::vector<PDNode*> conv2d_2_input{gn_silu_0_x, conv2d_2_bias, conv2d_2_filter, conv2d_2_filter_max};
    if(input_max_){
  ...
  auto conv2d_1_out_max = pattern->NewNode(conv2d_1_out_max_repr())
                              ->assert_is_op_output("conv2d_xpu", "out_max");
  conv2d_1->LinksFrom(
          {gn_silu_1_out, conv2d_2_out, conv2d_1_bias, conv2d_1_filter, conv2d_1_filter_max})
...
class SpatialTransformerResBlockXPUFusePass : public FusePassBase {
 ...
 private:
  int FuseSpatialTransformerResBlock(ir::Graph* graph, bool conv_fix=false, bool input_max=false,
    bool has_silu_fc_input=false, bool include_silu=false) const;
        ...
        if(conv_fix == true){
          for (auto input_max : {true, false}) {
            found_subgraph_count += 
              FuseSpatialTransformerResBlock(graph, conv_fix/*true*/, input_max);
          }
        }
        else{
          found_subgraph_count += 
...
int SpatialTransformerResBlockXPUFusePass::FuseSpatialTransformerResBlock(
    ir::Graph* graph, bool conv_fix, bool input_max, bool has_silu_fc_input, bool include_silu) const {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    std::vector<std::string> conv_bias_names{conv2d_0_bias->Name(), conv2d_1_bias->Name()}; 
    ...
    if(conv_fix){
      conv2d_2_act_param = PADDLE_GET_CONST(float, conv2d_2->Op()->GetAttr("act_param"));
      conv2d_2_act_type = PADDLE_GET_CONST(int, conv2d_2->Op()->GetAttr("act_type"));  
      auto conv2d_2_dilations = PADDLE_GET_CONST(std::vector<int>, conv2d_2->Op()->GetAttr("dilations"));
      dilations.emplace_back(std::move(conv2d_2_dilations));  
      int conv2d_2_groups = PADDLE_GET_CONST(int, conv2d_2->Op()->GetAttr("groups")); 
      groups.emplace_back(std::move(conv2d_2_groups));   
      conv2d_2_out_dtype = PADDLE_GET_CONST(int, conv2d_2->Op()->GetAttr("out_dtype"));  
      auto conv2d_2_paddings = PADDLE_GET_CONST(std::vector<int>, conv2d_2->Op()->GetAttr("paddings"));
      paddings.emplace_back(std::move(conv2d_2_paddings));  
      conv2d_2_padding_algorithm = PADDLE_GET_CONST(std::string, conv2d_2->Op()->GetAttr("padding_algorithm"));
      auto conv2d_2_strides = PADDLE_GET_CONST(std::vector<int>, conv2d_2->Op()->GetAttr("strides"));
      ...
      conv_filter_max_names.emplace_back(std::move(conv2d_2_filter_max->Name()));
    }
