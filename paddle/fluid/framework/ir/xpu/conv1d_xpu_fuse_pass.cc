...
Conv1dXPUPattern::Conv1dXPUPattern(PDPattern* pattern,
  ...
  auto x = pattern->NewNode("x")
               ->assert_is_op_input("unsqueeze2_x", "X")
  ...
  ->assert_is_op_input(conv_type_, "Input")->assert_more([](Node* node) {
    auto input_shape = node->Var()->GetShape();
    size_t input_rank = input_shape.size();
    return input_rank == 4 && input_shape[2] == 1;
  });
  ...
  // squeeze2 op
  ew_bias_add_out->assert_is_op_input("squeeze2", "X") auto* squeeze2 =
...
int Conv1dXPUFusePass::ApplyImpl(ir::Graph* graph,
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    conv1d_xpu_op_desc.SetInput("x", {input->Name()});
    ...
    std::vector<int64_t> paddings =
        std::vector<int64_t>{conv_paddings[0], conv_paddings[1]};
    ...
    conv1d_xpu_op_desc.SetAttr("has_branch", has_branch);
    conv1d_xpu_op_desc.SetAttr("has_bias", has_bias);
