...
namespace ir {
namespace patterns {
struct OptimizeDataPattern : public PatternBase {
  OptimizeDataPattern(PDPattern* pattern, const std::string& name_scope);
...
OptimizeDataPattern::OptimizeDataPattern(PDPattern* pattern,
...
int XpuOptimizeDataPreparationForFillPass::ApplyOptimizeDataPass(
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* graph) {
    VLOG(4) << "handle ApplyOptimizeDataPass fuse";
...
void XpuOptimizeDataPreparationForFillPass::ApplyImpl(ir::Graph* graph) const {
  ...
  int found_subgraph_count = ApplyOptimizeDataPass(graph);
  for (size_t i = 0; i < graph->SubGraphsSize(); i++) {
    found_subgraph_count += ApplyOptimizeDataPass(graph->GetSubGraph(i));
  }
