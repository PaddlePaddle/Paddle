...
#include "paddle/fluid/framework/ir/multihead_cpu_matmul_fuse_pass.h"
#include <memory>
#include <string>
...
static int ReshapeTranspose(Graph* graph) {
  ...
  patterns::MultiheadMatmulCPUFuse rm_reshape_transpose_pattern(
      gpd.mutable_pattern(), "multihead_cpu_matmul_fuse");
  rm_reshape_transpose_pattern(x, false /*inverse*/, false);
  int found_count = 0;
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* g) {
    VLOG(4) << "handle remove reshape transpose in attention pattern";
...
static int TransposeReshape(Graph* graph) {
  ...
  patterns::MultiheadMatmulCPUFuse rm_transpose_reshape_pattern(
      gpd.mutable_pattern(), "remove_transpose_reshape");
  rm_transpose_reshape_pattern(x, true, false);
  int found_count = 0;
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* g) {
    VLOG(4) << "handle remove transpose reshape in attention pattern";
...
static int ReshapeTransposeScale(Graph* graph) {
  ...
  auto* x = gpd.mutable_pattern()
                ...
                ->AsInput()
                ->assert_is_op_input("reshape2", "X");
  patterns::MultiheadMatmulCPUFuse rm_reshape_transpose_scale_pattern(
      gpd.mutable_pattern(), "remove_reshape_transpose_scale");
  rm_reshape_transpose_scale_pattern(x, false /*inverse*/, true);
  int found_count = 0;
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* g) {
    VLOG(4) << "handle remove reshape transpose scale in attention pattern";
...
static int MultiheadStackRemove(Graph* graph) {
  GraphPatternDetector gpd;
  auto* x = gpd.mutable_pattern()
                ->NewNode("rm_stack/x")
                ->AsInput()
                ->assert_is_op_input("stack", "X");
  patterns::MultiheadStackRemove remove_stack_pattern(gpd.mutable_pattern(),
...
static int DetectStack(Graph* graph) {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* g) {
    VLOG(4) << "handle remove stack for attention pattern";
    GET_IR_NODE_FROM_SUBGRAPH(stack, stack, remove_stack_pattern);
    GET_IR_NODE_FROM_SUBGRAPH(stack_out, stack_out, remove_stack_pattern);
...
void MultiheadCPUMatmulFusePass::ApplyImpl(ir::Graph* graph) const {
...
REGISTER_PASS(multihead_cpu_matmul_fuse_pass,
              paddle::framework::ir::MultiheadCPUMatmulFusePass);
