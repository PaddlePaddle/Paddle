...
void PianoJitOpSearchAndReplacePass::ApplyImpl(ir::Graph* graph) const {
  // Step1: Collect direct connection information between jit-supported ops through a var.
  PianoJitOpSearchHelper search_helper;
  std::vector<const Node*> target_op_nodes;
  std::unordered_map<const Node*, std::unordered_set<const Node*>> connection_recorder;
  ...
  for (const Node* target_op_node : target_op_nodes) {
    if (!visited.at(target_op_node)) {
      ...
      ClusterNodesThroughDfs(target_op_node, visited, connection_recoder, cluster);
  ...
  for (const auto& cluster : clusters) {
    ...
    for (const Node* var_node : outputs_recoder) {
      if (inputs_recoder.find(var_node) != inputs_recoder.end()) {
        ...
        for (const Node* next_op_node : var_node->outputs) {
          is_only_used_internel &= (cluster.find(next_op_node) != cluster.end());
        }
    ...
    Node* piano_jit_op = graph->CreateEmptyNode(PianoJitOpName, Node::Type::kOperation);
...
REGISTER_PASS(piano_jit_op_search_and_replace_pass, paddle::framework::ir::PianoJitOpSearchAndReplacePass);
