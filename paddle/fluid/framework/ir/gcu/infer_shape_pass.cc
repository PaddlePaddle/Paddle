...
#include "paddle/fluid/framework/ir/gcu/infer_shape_pass.h"
...
void InferShapePass::ApplyImpl(ir::Graph* graph) const {
  ...
  bool need_infer_shape = true;
  // auto gcu_backend = platform::gcu::GcuBackend::GetInstance();
  // auto batch_size = gcu_backend->GetGcuStrategy().GetBatchSize();
  auto batch_size = Get<std::string>("batch_size");
  ...
  for (auto node : graph->Nodes()) {
    ...
    if (is_feed) {
      ...
      if (input_shape[0] <= -1) {
        input_shape[0] = std::atoi(batch_size.c_str());
  ...
  if (need_infer_shape) {
    ...
    for (auto node : nodes) {
      ...
      VLOG(10) << "InferShapePass: Infer shape for Op (" << node->Name()
               << ") finished";
      ConstantComputation::ConstComputeIfNecessary(op, scope, node);
    }
