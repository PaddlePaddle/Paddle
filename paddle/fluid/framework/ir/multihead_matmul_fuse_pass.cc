...
int MultiHeadMatmulV2FusePass::BuildFusionV2(Graph* graph,
  ...
  auto fuse_creater = [&](
    ...
    if (softmax_qk_op_desc->HasAttr("out_threshold")) {
      auto qkv_plugin_scale = BOOST_GET_CONST(float, softmax_qk_op_desc->GetAttr("out_threshold"));
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    fuse_creater(input0, mul0, mul1, mul2, mul0_out, mul1_out, mul2_out, mul0_w,
                 ...
                 reshape2_0, reshape2_qkv_out, scale, scale_out, softmax_qk, eltadd0, eltadd1, eltadd2);
