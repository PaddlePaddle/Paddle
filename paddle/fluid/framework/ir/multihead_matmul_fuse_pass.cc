...
static int BuildFusion(Graph* graph, const std::string& name_scope) {
  ...
  auto fuse_creater = [&](
    ...
    auto scale_attr = BOOST_GET(float, scale->Op()->GetAttr("scale"));
    // auto scale_bias = BOOST_GET(float, scale->Op()->GetAttr("bias"));
    // bool after_scale =
    //    BOOST_GET(bool, scale->Op()->GetAttr("bias_after_scale"));
    ...
    int head_number =
        BOOST_GET(std::vector<int>, reshape_desc->GetAttr("shape")).at(2);
