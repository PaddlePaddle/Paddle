...
void MultiHeadMatmulPattern::operator()(bool with_attention_output) {
  ...
  reshape2_qkv_out_var->AsOutput()->assert_is_op_input("mul");
...
int MultiHeadMatmulWithAttentionFusePass::BuildFusionWithAttention(
  ...
  auto fuse_creater = [&](
      ...
      Node* eltadd0_b, Node* eltadd1_b, Node* eltadd2_b, Node* eltadd_qk_b,
      Node* reshape2, Node* reshape2_qkv_out, Node* scale, Node* scale_out,
      Node* softmax_qk, Node* softmax_qk_out, Node* eltadd0, Node* eltadd1,
    ...
    OpDesc multihead_op_desc(mul0->Op()->Block());
    multihead_op_desc.SetType("multihead_matmul_with_attention");
    ...
    multihead_op_desc.SetOutput("Out", {reshape2_qkv_out->Name()});
    multihead_op_desc.SetOutput("Attention", {softmax_qk_out->Name()});
    ...
    IR_NODE_LINK_TO(multihead, reshape2_qkv_out);
    IR_NODE_LINK_TO(multihead, softmax_qk_out);
  };
...
void MultiHeadMatmulWithAttentionFusePass::ApplyImpl(Graph* graph) const {
  FusePassBase::Init(name_scope_, graph);
  auto* scope = param_scope();
  PADDLE_ENFORCE_NOT_NULL(
      scope,
...
REGISTER_PASS(multihead_matmul_with_attention_fuse_pass,
              paddle::framework::ir::MultiHeadMatmulWithAttentionFusePass);
...
REGISTER_PASS_CAPABILITY(multihead_matmul_fuse_pass_v2)
    .AddCombination(
        paddle::framework::compatible::OpVersionComparatorCombination()
            ...
            .LE("matmul", 1)
            .EQ("softmax", 0));
REGISTER_PASS_CAPABILITY(multihead_matmul_with_attention_fuse_pass)
    .AddCombination(
        paddle::framework::compatible::OpVersionComparatorCombination()
            .EQ("mul", 0)
            .LE("elementwise_add", 1)
