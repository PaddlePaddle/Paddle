...
void DeleteQuant(ir::Graph* graph, Scope* scope,
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    int bit_length = boost::get<int>(quant->Op()->GetAttr("bit_length"));
...
void FuseDequant(ir::Graph* graph, Scope* scope,
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    int bit_length =
        boost::get<int>(quantized_op_node->Op()->GetAttr("bit_length"));
    int range = ((1 << (bit_length - 1)) - 1);
    ...
    if (dequant_type == "fake_channel_wise_dequantize_max_abs") {
      ...
      float max_range =
          boost::get<float>(dequant_op_node->Op()->GetAttr("max_range"));
