...
void SetOp(ProgramDesc* prog, const std::string& type, const std::string& name,
           ...
           const std::vector<std::string>& outputs, boost::tribool use_cudnn) {
  auto* op = prog->MutableBlock(0)->AppendOp();

  op->SetType(type);

  if (!boost::indeterminate(use_cudnn)) op->SetAttr("use_cudnn", use_cudnn);
...
void MainTest(std::initializer_list<std::string> cudnn_enabled_op_types,
  ...
  // EXPECT_EQ(use_cudnn_true_count, expected_use_mkldnn_true_count);
}

TEST(CUDNNPlacementPass, enable_conv_relu) {
