...
void FlattenVariable(const std::vector<Variable *> &src_vars,
                     Variable *dst_var) {
  // only support cpu now
  auto cpu_place = platform::CPUPlace();
  auto numel = 0;

  for (Variable *src_var : src_vars) {
    auto &src_tensor = src_var->Get<framework::LoDTensor>();
  ...
  for (Variable *src : src_vars) {
    auto &srcs = src->Get<framework::LoDTensor>();
    auto size = srcs.numel() * SizeOfType(srcs.type());
    auto src_ptr = srcs.data<void>();

    memory::Copy(cpu_place, dst_ptr, cpu_place, src_ptr, size);
    dst_ptr = reinterpret_cast<char *>(dst_ptr) + size;
  }
}
