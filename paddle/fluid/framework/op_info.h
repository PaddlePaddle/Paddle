...
class OpInfoMap {
 public:
  ...
  const ankerl::unordered_dense::map<std::string, OpInfo>& map() const {
    return map_;
  }

  ankerl::unordered_dense::map<std::string, OpInfo>* mutable_map() {
    return &map_;
  }
 ...
 private:
  OpInfoMap() = default;
  ankerl::unordered_dense::map<std::string, OpInfo> map_;
