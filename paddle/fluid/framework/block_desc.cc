  ...
  for (const auto &src_op : block->ops_) {
    ...
    for (const auto &pair : src_op->GetAttrMap()) {
      ...
      if (attr_type == proto::AttrType::VAR ||
        ...
        framework::BlockDesc *block_desc = PADDLE_GET_CONST(BlockDesc *, attr_value);
        if (std::find(old_block_desc.begin(),
          ...
          VLOG(10) << "Set block attr save ori " << attr_name;
        }
        //if (src_op->Type() == "run_program") {
          // dst_op->SetBlockAttr(attr_name, PADDLE_GET_CONST(BlockDesc *, attr_value));
          // VLOG(10) << "Set block attr save ori " << attr_name;
        //} else {
