...
void GraphDataGenerator::AllocResource(const paddle::platform::Place& place,
  ...
  d_prefix_sum_ =
      memory::AllocShared(place_, (once_max_sample_keynum + 1) * sizeof(int64_t));
  int64_t* d_prefix_sum_ptr = reinterpret_cast<int64_t*>(d_prefix_sum_->ptr());
  cudaMemsetAsync(d_prefix_sum_ptr, 0, (once_max_sample_keynum + 1) * sizeof(int64_t),
                  stream_);
  cursor_ = 0;
  jump_rows_ = 0;
  ...
  cudaMemsetAsync(d_walk_->ptr(), 0, buf_size_ * sizeof(int64_t),
                  stream_);
  d_sample_keys_ = memory::AllocShared(place_, once_max_sample_keynum * sizeof(int64_t));
  
  d_sampleidx2rows_.push_back(memory::AllocShared(place_, once_max_sample_keynum * sizeof(int)));
  d_sampleidx2rows_.push_back(memory::AllocShared(place_, once_max_sample_keynum * sizeof(int)));
  cur_sampleidx2row_ = 0;

  d_len_per_row_ = memory::AllocShared(place_, once_max_sample_keynum * sizeof(int));
  ...
  d_random_row_ =
      memory::AllocShared(place_, (once_sample_startid_len_ * walk_degree_ * repeat_time_) * sizeof(int));
  shuffle_seed_ = 0;
