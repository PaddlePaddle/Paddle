...
bool InMemoryDataFeed<T>::Start() {
  ...
  if (batch_offsets_.size() > 0) {
    VLOG(0) << "#14 batch_size offsets: " << batch_offsets_.size();
...
int InMemoryDataFeed<T>::Next() {
  ...
  if (!enable_heterps_) {
    ...
    VLOG(0) << "#015 enable heter NEXT: " << offset_index_ << " batch_offsets: " << batch_offsets_.size();
    if (offset_index_ >= batch_offsets_.size()) {
      VLOG(0) << "offset_index: " << offset_index_ << " batch_offsets: " << batch_offsets_.size();
      return 0;
    }
    ...
    if (this->batch_size_ != 0) {
      ...
      VLOG(3) << "finish reading for heterps, batch size zero, thread_id=" << thread_id_;
    }
    /*
    ...
    */
    VLOG(0) << "#15 enable heter NEXT: " << offset_index_ << " batch_offsets: " << batch_offsets_.size() << " baych_size: " <<  this->batch_size_;
  }
