...
TEST(host_storage, internal) {
  ...
  const auto a = std::make_shared<HostAllocatorSample>();
  Storage storage(a, size);
...
TEST(host_storage, external_stroage) {
  const size_t size{100};
  const auto a = std::make_shared<HostAllocatorSample>();
  auto in_storage = make_intrusive<Storage>(a, size);
  ...
  const size_t n{10};
  auto ex_storage = make_intrusive<ExternalStorage>(in_storage, delta, n);
...
TEST(host_storage, external_vector) {
  ...
  const size_t n{10};
  auto ex_storage =
      make_intrusive<ExternalStorage>(data.data(), n, platform::CPUPlace());
  CHECK_EQ(ex_storage->size(), n);
  CHECK(platform::is_cpu_place(ex_storage->place()));
  CHECK(!ex_storage->OwnsMemory());
  for (size_t i = delta; i < delta + n; ++i) {
