...
TEST(dense_tensor, ctor) {
  ...
  auto alloc = std::make_shared<HostAllocatorSample>();
  ...
  auto check_dense_tensor = [](const DenseTensor& t,
    ...
    r = r && (t.place() == platform::CPUPlace());
    r = r && t.initialized();
    r = r && t.SharesStorageWith(t);
    return r;
  };
  ...
  DenseTensor tensor_2(make_intrusive<Storage>(alloc), meta);
