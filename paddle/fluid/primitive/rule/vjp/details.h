...
void group_norm_grad(const Tensor& x,
  ...
  auto inv_std = rsqrt<T>(variance + epsilon);
  ...
  // if (scale_grad) {
  //   if (scale) {
  //     VLOG(0) << "gn grad in ************** 7 sum_y_grad_mul_x.dtype() "<<
  //     sum_y_grad_mul_x.dtype()<< " sum_y_grad "<< sum_y_grad.dtype()<<" mean
  //     "
  //     <<mean.dtype()<<" inv_std "<<inv_std.dtype();
  ...
  //     // auto tmp1 = reshape<T>(sum_y_grad_mul_x, shape_group); // err

  //     // auto tmp1 = reshape<T>(sum_y_grad, shape_group) * reshape<T>(mean,
  //     third_shape); // 1.377532e-39 err
  //     // auto tmp1 = reshape<T>(sum_y_grad, shape_group);

  //     // auto scale_grad_tmp =
  //     //     reshape<T>(tmp1.sum({0}, scale_data.dtype(), false), {C});
  //     VLOG(0) << "gn grad in ************** 8";
  //     // all 30 ok
  //     // auto tmp = sum_y_grad.sum({0}, bias_data.dtype(), false);
  //     // set_output<T>(tmp, scale_grad);
  //     auto tmp1 = reshape<T>(inv_std, third_shape);

  //     set_output<T>(tmp1, scale_grad);
  //   }
  // }
...
//     PADDLE_THROW(phi::errors::InvalidArgument("Unsupported storage order:
//     %s",
//                                               data_layout));
//   }
//   Tensor x_data = x;
...
//       sqrt_element,
//       full<T>(
//           common::vectorize(sqrt_element.dims()), 0.5,
//           sqrt_element.dtype()));
//   auto inv_std_mul_s = inv_std / hw / g_num;
//   auto dtype = x_data.dtype();
//   auto sum_y_grad_mul_x =
...
//     if (bias_ptr) {
//       auto bias_grad_tmp =
//           sum_y_grad.sum(std::vector<int64_t>({0}), bias_ptr->dtype(),
//           false);
//       set_output<T>(bias_grad_tmp, bias_grad);
//     } else {
//       bias_grad = nullptr;
