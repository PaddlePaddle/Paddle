...
template <>
std::vector<Tensor> concat_grad<StaticTensor>(const std::vector<Tensor>& x,
                                              const Tensor& out_grad,
                                              const Tensor& axis) {
  std::vector<ir::OpResult> x_res;
  for (uint64_t idx = 0; idx < x.size(); idx++) {
    x_res.emplace_back(std::static_pointer_cast<StaticTensor>(x[idx].impl())
                           ->getValue()
                           .dyn_cast<ir::OpResult>());
  }
  ...
  ir::OpResult axis_res = std::static_pointer_cast<StaticTensor>(axis.impl())
                              ->getValue()
                              .dyn_cast<ir::OpResult>();
  ...
  for (uint64_t idx = 0; idx < op_res.size(); idx++) {
    op_result.emplace_back(
        std::make_shared<primitive::experimental::StaticTensor>(op_res[idx]));
  }
  return op_result;
}
