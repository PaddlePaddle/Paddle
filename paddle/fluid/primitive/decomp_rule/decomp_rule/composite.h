...
template <typename T>
Tensor var_decomp(const Tensor& x,
  ...
  // 1. Calculate Mean (keepdim=True for broadcasting)
  auto mean_val = mean_decomp<T>(x_tmp, axis_, true);

  // 2. Calculate (x - mean)^2
  auto sub = x_tmp - mean_val;
  auto sq = sub * sub;

  // 3. Calculate Sum of squares
  auto sum_sq = sum<T>(sq, axis_, x_tmp.dtype(), keepdim);
  ...
  if (switch_dynamic) {
    auto x_shape = shape64<T>(x);
    num_elements = slice<T>(x_shape, {0}, {axis_[0]}, {axis_[0] + 1}, {1}, {0});
    for (size_t i = 1; i < axis_.size(); ++i) {
      num_elements =
          num_elements *
          slice<T>(x_shape, {0}, {axis_[i]}, {axis_[i] + 1}, {1}, {0});
    }
    num_elements = cast<T>(num_elements, x_tmp.dtype());
  } else {
    int64_t val = 1;
    for (size_t i = 0; i < axis_.size(); i++) {
      val *= x_dim[axis_[i]];
    }
    num_elements = full_scalar<T>(val, x_tmp.dtype(), x_tmp.place());
  }

  // 5. Adjust divisor
  Tensor divisor;
  if (unbiased) {
    auto one = full_scalar<T>(1, x_tmp.dtype(), x_tmp.place());
    divisor = num_elements - one;
  } else {
    divisor = num_elements;
  }

  auto res = sum_sq / divisor;
  return ConvertToOrig<T>(res, x.dtype());
}
