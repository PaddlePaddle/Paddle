  ...
  if (forward_global_block->OpSize() > 0) {
    if (place.GetDeviceType() == "gcu") {
      ...
      PADDLE_THROW(paddle::platform::errors::Unavailable(
          "Can not run program on custom-gcu in non GCU compiled environment, "
          "please re-compile with WITH_GCU=ON."));
#endif
    } else {
      paddle::platform::RecordEvent record_event(
...
#else
      PADDLE_THROW(paddle::platform::errors::Unavailable(
          "Can not run program grad on custom-gcu in non GCU compiled "
          "environment, please re-compile with WITH_GCU=ON."));
#endif
    } else {
      paddle::platform::RecordEvent record_event(
          "interpreter_core_run",
