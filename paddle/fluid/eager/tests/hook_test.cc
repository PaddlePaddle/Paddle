...
TEST(RetainGrad, HookBeforeRetainGrad) {
  ...
  pt::Tensor leaf_tensor = pt::Tensor();
  {
      ...
      auto auto_grad_meta = std::make_shared<AutogradMeta>();
      auto_grad_meta->SetGradNode(std::dynamic_pointer_cast<GradNodeBase>(acc_node_ptr));
      auto_grad_meta->SetOutRank(0);
  ...
  PADDLE_ENFORCE(CompareGradTensorWithValue<float>(target_tensor, 4.0) == true, 
    paddle::platform::errors::Fatal("Numerical Error, Expected %f", 4.0));

  // Print leaf tensor grad
  PADDLE_ENFORCE(CompareGradTensorWithValue<float>(leaf_tensor, 23.0) == true, 
...
TEST(RetainGrad, HookAfterRetainGrad) {
  ...
  pt::Tensor leaf_tensor = pt::Tensor();
  {
      ...
      RetainGradForTensor(leaf_tensor); // RetainGrad for leaf tensor gets postponed, result: 4.0*5.0 + 3.0 = 23.0
      RegisterGradientHookForTensor(leaf_tensor, hook);
  }
