...
inline std::string GetDtypeStr(
  ...
  return framework::DataTypeToString(pt::TransToProtoVarType(tensor->type()));
}

inline bool NeedCast(
    const std::shared_ptr<paddle::experimental::Tensor>& tensor) {
  auto place = pt::TransToFluidPlace(tensor->place());
  auto data_type = pt::TransToProtoVarType(tensor->type());
...
static inline std::shared_ptr<paddle::experimental::Tensor> CastToType(
  ...
  auto in_data_type = pt::TransToProtoVarType(tensor->type());
...
static inline std::shared_ptr<paddle::experimental::Tensor> CastToFP16(
  ...
  if (NeedCast(tensor) &&
      (pt::TransToProtoVarType(tensor->type()) != dst_type)) {
...
static inline framework::proto::VarType::Type GetPromoteType(
  ...
  for (const auto& pair : ins) {
    for (const auto& tensor : pair.second) {
      if (pt::TransToProtoVarType(tensor->type()) ==
          framework::proto::VarType::FP32) {
        dst_type = pt::TransToProtoVarType(tensor->type());
        break;
      }
    }
  }
  ...
  if (op_type == "moving_average_abs_max_scale") {
    for (const auto& pair : ins) {
      if (pair.first == "X" &&
          pt::TransToProtoVarType(pair.second.front()->DataType()) ==
