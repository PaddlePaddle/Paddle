...
LocalTensorsFromDistGradNode::operator()(
    paddle::small_vector<std::vector<paddle::Tensor>,
  ...
  paddle::platform::RecordEvent node_execution_inner(
      "Local_LocalTensorsFromDistGradNode",
  ...
  //         ->dist_attr();
  auto& grad_out = hooked_grad[0][local_tensor_idx_];
  ...
  shared_ptr<phi::distributed::DistTensor> grad_local_tensor_ptr =
      std::static_pointer_cast<phi::distributed::DistTensor>(grad_out.impl());
  std::shared_ptr<phi::distributed::DistTensor> dist_grad_input =
      std::make_shared<phi::distributed::DistTensor>(
          grad_local_tensor_ptr->value(),
          input.impl()->dims(),
          global_mesh_,
          global_placements_);
  ...
  VLOG(5) << "Finish C++ API: local_tensors_from_dist_func";
