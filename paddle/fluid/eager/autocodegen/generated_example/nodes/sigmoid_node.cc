...
std::vector<std::vector<paddle::experimental::Tensor>> GradNodesigmoid::
operator()(
  ...
  std::map<std::string,
      ...
      ins = {{"Out", TensorsToVarBases(this->Out_)},
             {"Out@GRAD", TensorsToVarBases(grads[0])}};
  std::map<std::string,
           std::vector<std::shared_ptr<paddle::imperative::VarBase>>>
      outs = {
          {"X@GRAD", ConstructDuplicableOutput(this->OutputMeta()[0].Size())}};

  paddle::framework::AttributeMap attrs = {{"use_cudnn", this->use_cudnn_},
                                           {"use_mkldnn", this->use_mkldnn_}};

  tracer->TraceOp("sigmoid_grad", ins, outs, attrs, tracer->ExpectedPlace(),
                  false, {});
