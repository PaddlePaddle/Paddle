...
paddle::experimental::Tensor sigmoid_dygraph_function(
  ...
  std::map<std::string,
      ...
      ins = {{"X", TensorsToVarBases(X)}};

  paddle::framework::AttributeMap attrs = {
      {"use_mkldnn", use_mkldnn},     {"use_cudnn", use_cudnn},
      {"op_role", op_role},           {"op_role_var", op_role_var},
      {"op_namescope", op_namescope}, {"op_callstack", op_callstack},
      {"op_device", op_device},       {"with_quant_attr", with_quant_attr},
  };

  std::map<std::string,
           std::vector<std::shared_ptr<paddle::imperative::VarBase>>>
      outs = {{"Out", ConstructDuplicableOutput(1)}};

  tracer->TraceOp("sigmoid", ins, outs, attrs, tracer->ExpectedPlace(), false,
                  {});

  paddle::experimental::Tensor Out = VarBasesToTensors(outs["Out"])[0];
  ...
  if (egr::EagerUtils::ComputeRequireGrad(pp_autograd_in, 1, pp_autograd_out, 1,
                                          trace_backward)) {
    // Create GradOpNode
