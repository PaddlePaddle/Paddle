...
paddle::experimental::Tensor matmul_v2_dygraph_function(
  ...
  std::map<std::string,
      ...
      ins = {{"X", TensorsToVarBases(X)}, {"Y", TensorsToVarBases(Y)}};

  paddle::framework::AttributeMap attrs = {
      {"trans_x", trans_x},           {"trans_y", trans_y},
      {"use_mkldnn", use_mkldnn},     {"mkldnn_data_type", mkldnn_data_type},
  ...
  if (egr::EagerUtils::ComputeRequireGrad(pp_autograd_in, p_autograd_in.size(),
                                          pp_autograd_out, 1, trace_backward)) {
    // Create GradOpNode
