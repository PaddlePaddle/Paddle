...
TEST(Table, SparseGeoTable) {
  ...
  common_config->set_table_name("test_table");
  ...
  ret = table->initialize(table_config, fs_config);
  ASSERT_EQ(ret, 0); 
  ...
  std::shared_ptr<::ThreadPool> pool_ = std::make_shared<::ThreadPool>(trainers);
  ...
  for (int i = 0; i < trainers; i++) {
    ...
    for (size_t j = 0; j < geo_pull_ids[i].size(); ++j) {
      ...
      for (int k = 0; k < emb_dim; k++) {
        ASSERT_EQ(geo_pull_values[i][j * emb_dim + k], pull_values[id * emb_dim + k]);
      }
    }
  }
}
