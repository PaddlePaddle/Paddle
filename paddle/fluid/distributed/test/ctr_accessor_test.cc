...
TEST(downpour_feature_value_accessor_test, test_shrink) {
    ...
    std::cout << "size of struct: " << acc->common_feature_value.embed_sgd_dim 
...
TEST(downpour_feature_value_accessor_test, test_update) {
    ...
    std::cout << "dim: " << acc->common_feature_value.dim() << "\n";
    std::cout << "update_dim: " << acc->update_dim() << "\n";
    ...
    for (auto i = 0u; i < item_size; ++i) {
        ...
        acc->_embed_sgd_rule->update_value(&v.embed_w, &v.embed_g2sum[0], &push_v.embed_g);
        acc->_embedx_sgd_rule->update_value(&v.embedx_w[0], &v.embedx_g2sum[0], &push_v.embedx_g[0]);
    ...
    for (auto i = 0u; i < item_size; ++i) {
        for (auto j = 0u; j < acc->dim(); ++j) {
            std::cout << value[i][j] << ":" << exp_value[i][j] << " ";
