...
int GraphBrpcClient::get_server_index_by_id(uint64_t id){
    ...
    size_t server_size = get_server_nums();
    int shard_per_server = shard_num % server_size == 0 ? shard_num / server_size : shard_num / server_size + 1;
    return id % shard_num / shard_per_server;
}
//char* &buffer,int &actual_size
std::future<int32_t> GraphBrpcClient::sample(uint32_t table_id, uint64_t node_id, GraphNodeType type, int sample_size,vector<GraphNode> &res){
  ...
  DownpourBrpcClosure *closure = new DownpourBrpcClosure(
      1, [&](void *done) {
        ...
        VLOG(0) << "check sample response: " <<" "<<closure->check_response(0, PS_GRAPH_SAMPLE);
          ...
          io_buffer_itr.copy_and_forward((void *)(buffer),
                                                 bytes_size);
          int start = 0;
  ...
  std::future<int> fut = promise->get_future();   ;
      ...
      closure->request(0)->add_params((char *)&node_id,
                                      sizeof(uint64_t));      
      closure->request(0)->add_params((char *)&type_int,
                                      sizeof(int));
...
std:: future<int32_t> GraphBrpcClient::pull_graph_list(uint32_t table_id, uint64_t node_id, GraphNodeType type,int start, int size, vector<GraphNode> &res){
  int server_index = get_server_index_by_id(node_id);
  DownpourBrpcClosure *closure = new DownpourBrpcClosure(
      1, [&](void *done) {
        int ret = 0;
        ...
        VLOG(0) << "check sample response: " <<" "<<closure->check_response(0, PS_PULL_GRAPH_LIST);
          auto &res_io_buffer = closure->cntl(0)->response_attachment();
          butil::IOBufBytesIterator io_buffer_itr(res_io_buffer);
          size_t bytes_size = io_buffer_itr.bytes_left();
          char *buffer = new char[bytes_size];
  ...
  std::future<int> fut = promise->get_future();   ;
      ...
      closure->request(0)->add_params((char *)&type_int,
                                      sizeof(int));
             closure->request(0)->add_params((char *)&start,
                                      sizeof(int));
              closure->request(0)->add_params((char *)&size,
                                      sizeof(int));                               
      PsService_Stub rpc_stub(get_cmd_channel(server_index));
      closure->cntl(0)->set_log_id(butil::gettimeofday_ms());
      rpc_stub.service(closure->cntl(0), closure->request(0),
...
int32_t GraphBrpcClient::initialize(){
    ...
    return BrpcPsClient::initialize();
}
}
}