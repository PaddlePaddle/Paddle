...
int32_t MemorySparseTable::InitializeValue() {
  ...
  if (_config.enable_revert()) {
    ...
    _shard_merge_rate =
        _config.has_shard_merge_rate() ? _config.shard_merge_rate() : _shard_merge_rate;
    CHECK((_m_avg_local_shard_num =
            static_cast<int>(std::ceil(_avg_local_shard_num * _shard_merge_rate)),
        _m_avg_local_shard_num <= _avg_local_shard_num));
    CHECK((_m_real_local_shard_num =
        static_cast<int>(std::ceil(_real_local_shard_num * _shard_merge_rate)),
        _m_real_local_shard_num <= _real_local_shard_num));

    uint32_t avg_shard_server_num = _sparse_table_shard_num / _avg_local_shard_num;
    uint32_t last_server_shard_num = _sparse_table_shard_num - avg_shard_server_num * _avg_local_shard_num;
    _m_sparse_table_shard_num = avg_shard_server_num * _m_avg_local_shard_num +
...
int32_t MemorySparseTable::LoadPatch(const std::vector<std::string>& file_list, int load_param) {
    if (!_config.enable_revert()) {
    ...
    size_t feature_value_size = _value_accesor->GetAccessorInfo().size / sizeof(float);
    end_idx = end_idx < _m_sparse_table_shard_num ? end_idx : _m_sparse_table_shard_num;
    ...
    for (size_t i = start_idx; i < end_idx; ++i) {
        ...
        do {
            ...
            for (size_t j = o_start_idx; j < o_end_idx; ++j) {
                if ((j % _avg_local_shard_num) % _m_real_local_shard_num == m_local_shard_id) {
            ...
            try {
                while (read_channel->read_line(line_data) == 0 && line_data.size() > 1) {
                    ...
                    auto index_iter = global_shard_idx.find(key % _sparse_table_shard_num);
                    if (index_iter == global_shard_idx.end()) {
                        LOG(WARNING) << "MemorySparseTable key:" << key << " not match shard,"
                            << " file_idx:" << i
                ...
                if (err_no == -1) {
                    ++retry_num;
                    is_read_failed = true;
                    LOG(ERROR) << "MemorySparseTable load failed after read, retry it! path:"
                               << channel_config.path << " , retry_num=" << retry_num;
                }
            } catch(...) {
                ++retry_num;
                is_read_failed = true;
                LOG(ERROR) << "MemorySparseTable load failed, retry it! path:" << channel_config.path
                           << " , retry_num=" << retry_num;
            }
            if (retry_num > FLAGS_pserver_table_save_max_retry) {
                LOG(ERROR) << "MemorySparseTable load failed reach max limit!";
...
int32_t MemorySparseTable::Save(const std::string& dirname,
 ...
 if (_real_local_shard_num == 0) {
  ...
  if (save_param == 5) {
        ...
        _local_shards_new.reset(new shard_type[_real_local_shard_num]);
        _save_patch_model_thread = std::thread(
                std::bind(&MemorySparseTable::SavePatch, this, std::string(dirname), save_param));
        return 0;
    }
...
int32_t MemorySparseTable::SavePatch(const std::string& path, int save_param) {
    ...
    for (size_t i = 0; i < _m_real_local_shard_num; ++i) {
        ...
        do {
                    ...
                    for (auto it = shard.begin(); it != shard.end(); ++it) {
                        if (_value_accesor->Save(it.value().data(), save_param)) {
                            std::string format_value = _value_accesor->
                                ParseToString(it.value().data(), it.value().size());
            ...
            if (err_no == -1) {
                ++retry_num;
                is_write_failed = true;
                LOG(ERROR) << "MemorySparseTable save patch failed after write, retry it! "
                           << "path:" << channel_config.path << " , retry_num=" << retry_num;
            }
            if (is_write_failed) {
                _afs_client.remove(channel_config.path);
    ...
    LOG(INFO) << "MemorySparseTable save patch success, path:" <<
        paddle::string::format_string("%s/%03d/part-%03d-", path.c_str(), _config.table_id(), _shard_idx)
        << " from " << file_start_idx << " to " << file_start_idx + _m_real_local_shard_num - 1
        << ", feasign size: " << feasign_size_all;
    return 0;
}

int64_t MemorySparseTable::CacheShuffle(const std::string& path, const std::string& param, double cache_threshold,
    std::function<std::future<int32_t>(int msg_type, int to_pserver_id, std::string& msg)> send_msg_func,
    paddle::framework::Channel<std::pair<uint64_t, std::string>>& shuffled_channel, const std::vector<Table*>& table_ptrs) {
    ...
    if (!_config.enable_sparse_table_cache() || cache_threshold < 0) {
        LOG(WARNING) << "cache shuffle failed not enable table cache or cache threshold < 0 "
    ...
    std::vector<paddle::framework::ChannelWriter<std::pair<uint64_t, std::string>>> writers(_real_local_shard_num);
    std::vector<std::vector<std::pair<uint64_t, std::string>>> datas(_real_local_shard_num);
  ...
  for (size_t i = 0; i < _real_local_shard_num; ++i) {
    ...
    //    writer.reset(make_channel<std::pair<uint64_t, std::string>>());
        ...
        for (size_t idx = 0; idx < table_ptrs.size(); idx++) {
            ...
            for (auto it = shard_ptr->begin(); it != shard_ptr->end(); ++it) {
                if (value_accesor->SaveCache(it.value().data(), save_param, cache_threshold)) {
                    std::string format_value = value_accesor->
                            ParseToString(it.value().data(), it.value().size());
    ...
    //LOG(INFO) << "MemorySparseTable cache KV save success to Channel feasigh size: " << feasign_size << " and start sparse cache data shuffle real local shard num: " << _real_local_shard_num;
    std::vector<std::pair<uint64_t, std::string>> local_datas;
    for (size_t idx_shard = 0; idx_shard < _real_local_shard_num; ++idx_shard) {
        paddle::framework::ChannelWriter<std::pair<uint64_t, std::string>>& writer = writers[idx_shard];
        ...
        while (channel->Read(data)) {
            for (auto& t : data) {
                auto pserver_id = paddle::distributed::local_random_engine()() % shuffle_node_num;
...
int32_t MemorySparseTable::SaveCache(const std::string& path, const std::string& param,
    paddle::framework::Channel<std::pair<uint64_t, std::string>>& shuffled_channel) {
    if (_shard_idx >= _config.sparse_table_cache_file_num()) {
        return 0;
    }
    int save_param = atoi(param.c_str());//batch_model:0  xbox:1
    size_t file_start_idx = _avg_local_shard_num * _shard_idx;
    std::string table_path = paddle::string::format_string("%s/%03d_cache/", path.c_str(), _config.table_id());
    _afs_client.remove(paddle::string::format_string("%s/part-%03d", table_path.c_str(), _shard_idx));
    uint32_t feasign_size= 0;
    FsChannelConfig channel_config;
    // not compress cache model
    channel_config.path = paddle::string::format_string("%s/part-%03d",
                                        table_path.c_str(), _shard_idx);
    channel_config.converter = _value_accesor->Converter(save_param).converter;
    channel_config.deconverter = _value_accesor->Converter(save_param).deconverter;
    ...
    while (shuffled_channel->Read(data)) {
        for (auto& t : data) {
            ++feasign_size;
            if (0 != write_channel->write_line(
                    paddle::string::format_string("%lu %s", t.first, t.second.c_str()))) {
    ...
    write_channel->close();
    LOG(INFO) << "MemorySparseTable cache save success, feasign: " 
              << feasign_size << ", path: " << channel_config.path;
         ...
         &task_keys]() -> int {
          ...
          for (int i = 0; i < keys.size(); ++i) {
            ...
            if (_config.enable_revert()) {
              ...
              memcpy(const_cast<float*>(feature_value_new->data()), value_data, new_size * sizeof(float));
            }
