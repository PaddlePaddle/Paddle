...
int32_t MemorySparseTable::InitializeValue() {
  ...
  if (_config.enable_revert()) {
    ...
    _shard_merge_rate = _config.has_shard_merge_rate()
                            ...
                            : _shard_merge_rate;
    _m_avg_local_shard_num = static_cast<int>(std::ceil(_avg_local_shard_num * _shard_merge_rate));
    PADDLE_ENFORCE_LE(_m_avg_local_shard_num,
                      _avg_local_shard_num,
                      phi::errors::InvalidArgument(
                          "The calculated '_m_avg_local_shard_num' (%d) must be less than or equal to '_avg_local_shard_num' (%d).",
                          _m_avg_local_shard_num, _avg_local_shard_num));
    
    _m_real_local_shard_num = static_cast<int>(std::ceil(_real_local_shard_num * _shard_merge_rate));
    PADDLE_ENFORCE_LE(_m_real_local_shard_num,
                      _real_local_shard_num,
                      phi::errors::InvalidArgument(
                          "The calculated '_m_real_local_shard_num' (%d) must be less than or equal to '_real_local_shard_num' (%d).",
