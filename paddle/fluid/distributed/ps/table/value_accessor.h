...
class ValueAccessor {
 public:
  ...
  // value各个维度的size
  virtual size_t DimSize(size_t dim) = 0;
  // value各维度相加总size
  virtual size_t Size() = 0;

  // value中mf动态长度部分总size大小, sparse下生效
  virtual size_t MfSize() { return 0; }
  virtual bool NeedExtendMf(float* value) { return false; }
  virtual bool HasMf(size_t size) { return false; }
  // pull value维度
  virtual size_t SelectDim() = 0;
  // pull value各个维度的size
  virtual size_t SelectDimSize(size_t dim) = 0;
  // pull value各维度相加总size
  virtual size_t SelectSize() = 0;
  // push value维度
  virtual size_t UpdateDim() = 0;
  // push value各个维度的size
  virtual size_t UpdateDimSize(size_t dim) = 0;
  // push value各维度相加总size
  virtual size_t UpdateSize() = 0;
  // fea total for dense
  virtual size_t FeaDim() { return _config.fea_dim(); }
  // converter for save
