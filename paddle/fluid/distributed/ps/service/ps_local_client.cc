...
::std::future<int32_t> PsLocalClient::Save(const LoadSaveContext& save_context) {
  ...
  if (push_context.value_type == Dense) { // push dense
    if (push_context.training_phase == Init) {
      ...
      if (push_context.training_mode == Geo) { // geo
        float* total_send_data = reinterpret_cast<float*>(push_context.dense_values);
        size_t total_send_data_size = push_context.num;
        push_dense_raw_gradient(push_context.table, total_send_data, total_send_data_size, push_context.callback);
      } else { // async and sync
        const Region* regions = push_context.push_context.push_dense_values;
        size_t region_num = push_context.num;
