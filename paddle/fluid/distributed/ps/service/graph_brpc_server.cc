...
int32_t GraphBrpcService::add_graph_node(Table *table,
  ...
  uint64_t *node_data = reinterpret_cast<uint64_t *>(request.params(1).c_str());
...
int32_t GraphBrpcService::graph_random_sample_neighbors(
  ...
  size_t node_num = request.params(1).size() / sizeof(uint64_t);
  uint64_t *node_data = reinterpret_cast<uint64_t *>(request.params(1).c_str());
  int sample_size = *(reinterpret_cast<int *>(request.params(2).c_str()));
...
int32_t GraphBrpcService::graph_random_sample_nodes(
  ...
  int type_id = *(reinterpret_cast<int *>(request.params(0).c_str()));
  int idx_ = *(reinterpret_cast<int *>(request.params(1).c_str()));
  size_t size = *(reinterpret_cast<uint64_t *>(request.params(2).c_str()));
