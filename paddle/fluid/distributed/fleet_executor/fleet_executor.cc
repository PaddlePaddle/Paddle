...
void FleetExecutor::Init(
  ...
  // NOTE: For inference, the vars in force_root_scope_var_names shouldn't
  // be deleted during inf, for that they may be the result of the inf. If
  // they are GCed, it will cause error during ZeroCopy the result.
  ...
  for (auto pair : unused_vars) {
    ...
    for (auto name : force_root_scope_var_names) {
...
void FleetExecutor::InitCarrier(
  ...
  carrier->Init(exe_desc_.cur_rank(), runtime_graph_->interceptor_id_to_rank(),
                ...
                num_micro_batches, place, force_root_scope_var_names);
}
