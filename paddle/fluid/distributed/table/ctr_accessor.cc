...
size_t CtrDoubleUnitAccessor::dim() { return unit_feature_value.dim(); }
size_t CtrDoubleUnitAccessor::dim_size(size_t dim) {
  auto embedx_dim = _config.embedx_dim();
  return unit_feature_value.dim_size(dim, embedx_dim);
}
size_t CtrDoubleUnitAccessor::size() { return unit_feature_value.size(); }
...
int32_t CtrDoubleUnitAccessor::create(float** values, size_t num) {
  ...
  for (size_t value_item = 0; value_item < num; ++value_item) {
    ...
    *(reinterpret_cast<double*>(value + unit_feature_value.show_index())) = 0;
...
int32_t CtrDoubleUnitAccessor::select(float** select_values,
...
int32_t CtrDoubleUnitAccessor::merge(float** update_values,
...
// first dim: item
// second dim: field num
int32_t CtrDoubleUnitAccessor::update(float** update_values,
...
TODO(zhaocaibei123): const float* => double*
  ...
  if (score >= _config.embedx_threshold() && param_size > 9) {
    for (auto i = unit_feature_value.embedx_w_index();
         i < unit_feature_value.embedx_g2sum_index() +
                 unit_feature_value.embedx_sgd_dim;
         ++i) {
      os << " " << v[i];
    }
  }
...
int CtrDoubleUnitAccessor::parse_from_string(const std::string& str,
                                             float* value) {
  int embedx_dim = _config.embedx_dim();
  const int kDataLen = dim() + 2;
  float data_buff[kDataLen];
...
size_t CtrUnitAccessor::dim() { return unit_feature_value.dim(); }
...
size_t CtrUnitAccessor::size() { return unit_feature_value.size(); }
...
size_t CtrUnitAccessor::select_dim_size(size_t dim) { return sizeof(float); }

size_t CtrUnitAccessor::select_size() { return select_dim() * sizeof(float); }
...
size_t CtrUnitAccessor::update_dim_size(size_t dim) { return sizeof(float); }
...
int32_t CtrUnitAccessor::select(float** select_values, const float** values,
                                size_t num) {
  auto embedx_dim = _config.embedx_dim();
  for (size_t value_item = 0; value_item < num; ++value_item) {
    float* select_value = select_values[value_item];
    float* value = const_cast<float*>(values[value_item]);
    select_value[CtrUnitPullValue::show_index()] =
        value[unit_feature_value.show_index()];
    select_value[CtrUnitPullValue::click_index()] =
        value[unit_feature_value.click_index()];
    select_value[CtrUnitPullValue::embed_w_index()] =
        value[unit_feature_value.embed_w_index()];
...
// first dim: item
// second dim: field num
int32_t CtrUnitAccessor::merge(float** update_values,
...
// first dim: item
// second dim: field num
int32_t CtrUnitAccessor::update(float** update_values,
                                const float** push_values, size_t num) {
  auto embedx_dim = _config.embedx_dim();
  for (size_t value_item = 0; value_item < num; ++value_item) {
    float* update_value = update_values[value_item];
