...
__global__ void __launch_bounds__(kNumThreads, 1)
  ...
  if (lane_id == 0) {
    ...
    fence_barrier_init();
    EP_DEVICE_ASSERT(hidden_int4 % 2 == 0 &&
...
#pragma unroll
    ...
    EP_DEVICE_ASSERT(kNumRanks <= 32);
    EP_DEVICE_ASSERT(recv_thread_id >= 0 && num_recv_warps % kNumRanks == 0);
    ...
    while (lane_id == 0 && (total_offset = ld_volatile_global(
                                channel_start_offset.buffer())) == 0) {
    }
    while (lane_id == 0 && (num_tokens_to_recv = ld_volatile_global(
...
#pragma unroll 4
      ...
      asm volatile("bar.sync %0, %1;" ::"r"(responsible_rank),
                   "r"(num_threads_per_rank));
      if (recv_warp_id_in_rank == num_recv_warps_per_rank - 1 && lane_id == 0)
...
#pragma unroll
      ...
      for (int64_t token_idx = token_start_idx + recv_warp_id - 1;
        ...
        while (__any_sync(
            0xffffffff,
            channel_tail_idx[lane_id] <= expected_head && expected_head >= 0)) {
          // Timeout check
          if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
            printf(
...
#pragma unroll
          ...
          auto bias_1_values =
              reinterpret_cast<const dtype_t*>(&bias_1_value_int4);
#pragma unroll
