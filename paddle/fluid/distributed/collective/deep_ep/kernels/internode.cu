...
__host__ __device__ __forceinline__ std::pair<int, int> get_nvl_clean_meta(
  ...
  EP_STATIC_ASSERT(sizeof(SourceMeta) % sizeof(int) == 0,
                   "Invalid size of `SourceMeta`");
  const int num_bytes_per_token =
      is_dispatch ? get_num_bytes_per_rdma_token(
                        hidden_int4, num_scales, num_topk_idx, num_topk_weights)
                  : (hidden_int4 * sizeof(int4) + num_scales * sizeof(float) +
                     num_topk_idx * sizeof(int) +
                     num_topk_weights * sizeof(float) + sizeof(SourceMeta));
  return {
      (num_nvl_recv_buffer_tokens * num_bytes_per_token * num_nvl_ranks *
       num_channels) /
          sizeof(int),
...
__global__ void __launch_bounds__(
  ...
  if ((warp_role == WarpRole::kRDMAAndNVLForwarder ||
    ...
    fence_barrier_init();
    EP_DEVICE_ASSERT(num_bytes_per_rdma_token + sizeof(uint64_t) <=
                     kNumTMABytesPerWarp);
  }
  __syncwarp();
...
#pragma unroll
    ...
    while (__any_sync(0xffffffff, num_tokens_to_recv_from_rdma > 0)) {
      ...
      for (int i = src_rdma_head, num_tokens_sent = 0; i < src_rdma_tail; ++i) {
        ...
        auto dst_shifted =
            nvl_channel_x.buffer() + dst_slot_idx * num_bytes_per_rdma_token;
...
#pragma unroll
    ...
    while (num_tokens_to_recv > 0) {
      ...
      for (int chunk_idx = 0; chunk_idx < num_recv_tokens;
        ...
        auto shifted = nvl_channel_x.buffer() +
                       token_idx_in_buffer * num_bytes_per_rdma_token;
