...
#include "paddle/fluid/distributed/collective/flash_ep/kernels/buffer.cuh"
...
__global__ void fused_notify(const int* dispatch_num_tokens_per_rank,
  ...
  if (sm_id == 0) {
    ...
    int dispatch_num_elements = NUM_MAX_NVL_PEERS + num_rdma_experts + 1;
      ...
      for (int j = 0; j < num_pipeline_stages; ++j) {
        rdma_recv_num_tokens_mixed.send_buffer(
            i / NUM_MAX_NVL_PEERS)[dispatch_num_elements * j +
                                   (i % NUM_MAX_NVL_PEERS)] =
            dispatch_num_tokens_per_rank[j * num_ranks + i];
      }
    }
#pragma unroll
    for (int i = thread_id; i < num_experts; i += num_threads) {
      for (int j = 0; j < num_pipeline_stages; ++j) {
        rdma_recv_num_tokens_mixed.send_buffer(
            ...
            dispatch_num_tokens_per_expert[j * num_experts + i];
      }
    }
      ...
      for (int j = 0; j < num_pipeline_stages; ++j) {
        rdma_recv_num_tokens_mixed.send_buffer(
            thread_id)[dispatch_num_elements * j + NUM_MAX_NVL_PEERS +
                       num_rdma_experts] =
            dispatch_num_tokens_per_rdma_rank[j * kNumRDMARanks + thread_id];
      }
    }
    __syncthreads();
...
#pragma unroll
    ...
    EP_DEVICE_ASSERT(dispatch_nvl_reduced_num_tokens_per_expert.total_bytes +
                         dispatch_nvl_send_num_tokens_per_rank.total_bytes +
                         dispatch_nvl_send_num_tokens_per_expert.total_bytes +
...
#pragma unroll
        for (int i = 0; i < kNumRDMARanks; ++i)
          sum += rdma_recv_num_tokens_mixed.recv_buffer(
              i)[j * dispatch_num_elements + NUM_MAX_NVL_PEERS + thread_id];
        dispatch_nvl_reduced_num_tokens_per_expert[j * num_rdma_experts +
                                                   thread_id] = sum;
      }
    }
    __syncthreads();
...
#pragma unroll
      for (int i = 0; i < kNumRDMARanks; ++i) {
        sum += rdma_recv_num_tokens_mixed.recv_buffer(
            i)[thread_id * dispatch_num_elements + NUM_MAX_NVL_PEERS +
               num_rdma_experts];
        dispatch_recv_rdma_rank_prefix_sum[thread_id * kNumRDMARanks + i] = sum;
      }
      while (ld_volatile_global(dispatch_moe_recv_rdma_counter_mapped +
                                thread_id) != -1) {
      }
      dispatch_moe_recv_rdma_counter_mapped[thread_id] = sum;
    }
        ...
        for (int i = 0; i < kNumRDMARanks; ++i)
          dispatch_nvl_send_num_tokens_per_rank.buffer(
              ...
              rdma_recv_num_tokens_mixed.recv_buffer(
                  i)[j * dispatch_num_elements + thread_id];
#pragma unroll
        for (int i = 0; i < num_nvl_experts; ++i)
          dispatch_nvl_send_num_tokens_per_expert.buffer(
              nvl_rank)[j * num_nvl_experts + i] =
              dispatch_nvl_reduced_num_tokens_per_expert
                  [j * num_rdma_experts + thread_id * num_nvl_experts + i];
      }
    }
...
#pragma unroll
      ...
      tokens_cumsum[thread_id] = sum;
      while (ld_volatile_global(combine_moe_recv_rdma_counter_mapped +
                                thread_id) != -1) {
      }
      combine_moe_recv_rdma_counter_mapped[thread_id] = sum;
    }
...
#pragma unroll
      for (int i = 0; i < num_ranks; ++i) {
        ...
        sum += dispatch_nvl_recv_num_tokens_per_rank.buffer(
            src_nvl_rank)[thread_id * kNumRDMARanks + src_rdma_rank];
        dispatch_recv_gbl_rank_prefix_sum[thread_id * num_ranks + i] = sum;
      }
      while (ld_volatile_global(dispatch_moe_recv_counter_mapped + thread_id) !=
             -1) {
      }
...
#pragma unroll
      for (int i = 0; i < NUM_MAX_NVL_PEERS; ++i)
        sum += dispatch_nvl_recv_num_tokens_per_expert.buffer(i)[thread_id];
      sum = (sum + expert_alignment - 1) / expert_alignment * expert_alignment;
      while (ld_volatile_global(dispatch_moe_recv_expert_counter_mapped +
                                thread_id) != -1) {
      }
      dispatch_moe_recv_expert_counter_mapped[thread_id] = sum;
    }
...
#pragma unroll
      for (int i = 0; i < num_ranks; ++i) {
        int src_rdma_rank = i / NUM_MAX_NVL_PEERS,
            src_nvl_rank = i % NUM_MAX_NVL_PEERS;
        sum += combine_nvl_recv_num_tokens_per_rank.buffer(
            src_nvl_rank)[thread_id * kNumRDMARanks + src_rdma_rank];
        combine_recv_gbl_rank_prefix_sum[thread_id * num_ranks + i] = sum;
      }
      while (ld_volatile_global(combine_moe_recv_counter_mapped + thread_id) !=
             -1) {
      }
      combine_moe_recv_counter_mapped[thread_id] = sum;
    }
    ...
    for (int channel_id = warp_id; channel_id < num_channels;
      ...
      for (int64_t i = token_start_idx + lane_id; i < token_end_idx; i += 32) {
        ...
        auto is_token_in_rank_uint64 = *reinterpret_cast<const uint64_t*>(
            ...
            (stage_id * num_tokens + i) * num_ranks +
...
#pragma unroll
        for (int i = 0; i < NUM_MAX_NVL_PEERS; ++i) {
          dispatch_gbl_channel_prefix_matrix
              [(stage_id * num_ranks + dst_rdma_rank * NUM_MAX_NVL_PEERS + i) *
                   num_channels +
               channel_id] = per_nvl_rank_count[i];
          dispatch_rdma_channel_prefix_matrix[(stage_id * kNumRDMARanks +
                                               dst_rdma_rank) *
                                                  num_channels +
...
#pragma unroll
    ...
    if (thread_id < NUM_MAX_NVL_PEERS) {
      auto prefix_row = dispatch_gbl_channel_prefix_matrix +
                        (stage_id * num_ranks +
    ...
    for (int channel_id = warp_id; channel_id < num_channels;
      ...
      for (int i = token_start_idx + lane_id; i < token_end_idx; i += 32) {
        EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS * sizeof(bool) == sizeof(uint64_t),
                         "Invalid number of NVL peers");
        auto is_token_in_rank_uint64 = *reinterpret_cast<const uint64_t*>(
            combine_is_token_in_rank + (stage_id * num_tokens + i) * num_ranks +
            dst_rdma_rank * NUM_MAX_NVL_PEERS);
        auto is_token_in_rank_values =
            reinterpret_cast<const bool*>(&is_token_in_rank_uint64);
        ...
        combine_send_rdma_head[(stage_id * num_tokens + i) * kNumRDMARanks +
                               dst_rdma_rank] = rdma_tail_idx;
        global_rdma_tail_idx =
            __shfl_sync(mask, global_rdma_tail_idx, warp_valid_tokens - 1);

        EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS % 4 == 0,
                         "Invalid number of NVL peers");
#pragma unroll
        for (int j = 0; j < NUM_MAX_NVL_PEERS / 4; ++j) {
          int4 nvl_tail_idx_frag;
          for (int k = 0; k < 4; k++) {
            int offset = j * 4 + k;
            per_nvl_rank_count[offset] += is_token_in_rank_values[offset];
            int warp_nvl_tail_idx = (is_token_in_rank_values[offset]);
            global_nvl_tail_idx[offset] += warp_scan(warp_nvl_tail_idx, mask);
            auto nvl_tail_idx = is_token_in_rank_values[offset] == 0
                                    ? -1
                                    : global_nvl_tail_idx[offset] - 1;
            reinterpret_cast<int*>(&nvl_tail_idx_frag)[k] = nvl_tail_idx;
            global_nvl_tail_idx[offset] = __shfl_sync(
                mask, global_nvl_tail_idx[offset], warp_valid_tokens - 1);
          }
          *reinterpret_cast<int4*>(
              combine_send_nvl_head +
              (stage_id * num_tokens + i) * kNumRDMARanks * NUM_MAX_NVL_PEERS +
              dst_rdma_rank * NUM_MAX_NVL_PEERS + j * 4) = nvl_tail_idx_frag;
        }
      }
...
#pragma unroll
        for (int i = 0; i < NUM_MAX_NVL_PEERS; ++i)
          combine_gbl_channel_prefix_matrix[(stage_id * num_ranks +
                                             dst_rdma_rank * NUM_MAX_NVL_PEERS +
                                             i) *
                                                num_channels +
                                            channel_id] = per_nvl_rank_count[i];
        combine_rdma_channel_prefix_matrix[(stage_id * kNumRDMARanks +
                                            dst_rdma_rank) *
                                               num_channels +
                                           channel_id] = total_count;
      }
    ...
    if (thread_id == 0) {
      auto prefix_row =
          combine_rdma_channel_prefix_matrix +
          (stage_id * kNumRDMARanks + dst_rdma_rank) * num_channels;
#pragma unroll
      for (int i = 1; i < num_channels; ++i) {
        prefix_row[i] += prefix_row[i - 1];
    ...
    EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS <= 32, "Invalid number of NVL peers");
    if (thread_id < NUM_MAX_NVL_PEERS) {
      auto prefix_row = combine_gbl_channel_prefix_matrix +
                        (stage_id * num_ranks +
                         dst_rdma_rank * NUM_MAX_NVL_PEERS + thread_id) *
                            num_channels;
#pragma unroll
...
#define FUSE_NOTIFY_LAUNCH_CASE(num_rdma_ranks)                        \
  {                                                                    \
    auto fuse_notify_func = low_latency_mode                           \
                                ? fused_notify<true, num_rdma_ranks>   \
                                : fused_notify<false, num_rdma_ranks>; \
    LAUNCH_KERNEL(&cfg,                                                \
                  fuse_notify_func,                                    \
                  ...
                  dispatch_moe_recv_expert_counter_mapped,             \
                  num_experts,                                         \
                  dispatch_is_token_in_rank,                           \
                  ...
                  combine_gbl_channel_prefix_matrix,                   \
                  combine_recv_gbl_rank_prefix_sum,                    \
...
__global__ void fused_notify_combine_post_step(
  ...
  // NVL buffers
  auto nvl_send_buffer =
      warp_id - 1 < NUM_MAX_NVL_PEERS ? buffer_ptrs[warp_id - 1] : nullptr;
  auto nvl_recv_buffer = buffer_ptrs[nvl_rank];
  ...
  if (warp_id == 0) {  // rdma_channel_prefix_matrix data
        ...
        for (int i = 0; i < num_channels; ++i) {
          rdma_channel_meta.send_buffer(
              lane_id)[j * (num_channels + num_channels * NUM_MAX_NVL_PEERS) +
                       i] =
              -rdma_channel_prefix_matrix[(j * kNumRDMARanks + lane_id) *
                                              num_channels +
                                          i] -
              1;
        }
             ...
             NUM_MAX_NVL_PEERS + 1) {  // gbl_channel_prefix_matrix data
    // land_id -> dst_rdma_rank
    ...
    if (lane_id < kNumRDMARanks) {
      for (int j = 0; j < num_pipeline_stages; ++j) {
        ...
        dst_ptr[0] = -0 - 1;
        for (int i = 1; i < num_channels; ++i) {
          dst_ptr[i] = -gbl_channel_prefix_matrix[(j * kNumRDMARanks *
                                                       NUM_MAX_NVL_PEERS +
        ...
        for (int i = 0; i < num_channels; ++i) {
          recv_rdma_channel_prefix_matrix[(j * kNumRDMARanks + lane_id) *
              ...
              -rdma_channel_meta.recv_buffer(lane_id)
                   [j * (num_channels + num_channels * NUM_MAX_NVL_PEERS) + i] -
              1;
        }
      }
    }

  } else if (warp_id < NUM_MAX_NVL_PEERS + 1) {
    // lane_id -> src_rdma_rank
    // warp_id - 1  -> dst_nvl_rank
    if (lane_id < kNumRDMARanks) {
      for (int j = 0; j < num_pipeline_stages; ++j) {
        ...
        for (int i = 0; i < num_channels; ++i) {
          st_relaxed_sys_global(
              nvl_send_channel_meta.buffer(nvl_rank) +
                  (j * kNumRDMARanks + lane_id) * num_channels + i,
              recv_ptr[i]);
        }
      }
    }
  ...
  EP_DEVICE_ASSERT(kNumRDMARanks * NUM_MAX_NVL_PEERS <= num_threads);
...
#pragma unroll
      for (int i = 0; i < num_channels; ++i) {
        recv_gbl_channel_prefix_matrix[(j * kNumRDMARanks * NUM_MAX_NVL_PEERS +
                                        thread_id) *
                                           num_channels +
                                       i] =
            rank_offset -
            nvl_recv_channel_meta.buffer(src_nvl_rank)
                [(j * kNumRDMARanks + src_rdma_rank) * num_channels + i] -
            1;
      }
    }
  }
...
void fused_notify_combine_post_step(int num_ranks,
                                    ...
                                    cudaStream_t stream,
                                    bool low_latency_mode) {
#define NOTIFY_COMBINE_S1_LAUNCH_CASE(num_rdma_ranks)                \
  {                                                                  \
    auto notify_combine_post_step_func =                             \
        low_latency_mode                                             \
            ? fused_notify_combine_post_step<true, num_rdma_ranks>   \
            : fused_notify_combine_post_step<false, num_rdma_ranks>; \
    LAUNCH_KERNEL(&cfg,                                              \
  ...
  SWITCH_RDMA_RANKS(NOTIFY_COMBINE_S1_LAUNCH_CASE);
...
__global__ void __launch_bounds__(
  ...
  void* pipe_rdma_buffer_ptr =
      reinterpret_cast<void*>(reinterpret_cast<uint8_t*>(rdma_buffer_ptr) +
                              pipeline_stage_id * num_rdma_bytes);
  auto rdma_channel_data = SymBuffer<int8_t>(
      pipe_rdma_buffer_ptr,
      num_max_rdma_chunked_recv_tokens * num_bytes_per_rdma_token,
      kNumRDMARanks,
      channel_id,
      num_channels);
  auto rdma_channel_meta = SymBuffer<int>(pipe_rdma_buffer_ptr,
                                          NUM_MAX_NVL_PEERS * 2 + 2,
                                          kNumRDMARanks,
                                          channel_id,
                                          num_channels);
  auto rdma_channel_head = SymBuffer<uint64_t, false>(
      pipe_rdma_buffer_ptr, 1, kNumRDMARanks, channel_id, num_channels);
  ...
  if (warp_role == WarpRole::kRDMAAndNVLForwarder)
    rs_wr_buffer_ptr = reinterpret_cast<void*>(
        reinterpret_cast<uint8_t*>(buffer_ptrs[nvl_rank]) +
        pipeline_stage_id * num_nvl_bytes),
    ws_rr_buffer_ptr = reinterpret_cast<void*>(
        reinterpret_cast<uint8_t*>(buffer_ptrs[target_rank]) +
        pipeline_stage_id * num_nvl_bytes),
...
#pragma unroll
    ...
    if (lane_id < kNumRDMARanks) {
      while (true) {
        ...
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
          printf(
              "FlashEP dispatch forwarder timeout (RDMA meta), channel: %d, "
    ...
    while (__any_sync(0xffffffff, num_tokens_to_recv_from_rdma > 0)) {
      ...
      while (lane_id == 0) {
        ...
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
          printf(
              "FlashEP dispatch forwarder timeout (NVL check), channel: %d, "
      ...
      while (true) {
        ...
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES &&
          ...
          printf(
              "FlashEP dispatch forwarder timeout (RDMA check), channel: %d, "
        ...
        if (lane_id == src_rdma_rank) {
          ...
          if (kAsymmertricMode) {
            ...
            int64_t asymm_channel_start_idx =
                src_meta.src_channel_id == 0
                    ? 0
                    : ld_nc_global(
                          shifted_asymm_recv_rdma_channel_prefix_matrix +
                          src_rdma_rank * num_channels +
                          src_meta.src_channel_id - 1);
            ...
            *reinterpret_cast<int8*>(asymm_aggregated_nvl_head +
                                     asymm_aggregated_nvl_head_offset *
                                         NUM_MAX_NVL_PEERS) =
                *reinterpret_cast<int8*>(&(src_meta.send_nvl_head));
          }
        }
...
#pragma unroll
    ...
    while (lane_id < kNumRDMARanks) {
      ...
      if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
        printf(
            "FlashEP dispatch NVL receiver timeout, channel: %d, RDMA: %d, "
            "nvl: "
            ...
            ? (is_cached_dispatch                                             \
                   ...
                   : (asymmetric_mode ? dispatch<true,                        \
                                      ...
                                      : dispatch<true,                        \
                                                 ...
                                                 false>))                     \
            : (is_cached_dispatch                                             \
                   ? (asymmetric_mode ? dispatch<false,                       \
                                                 num_rdma_ranks,              \
                                                 true,                        \
                                                 kNumDispatchRDMASenderWarps, \
                                                 true>                        \
...
__global__ void cached_notify(const int rdma_clean_offset,
  ...
  if (sm_id == 0 && pipeline_stage_id == 0) {
    // Barrier for RDMA
    if (thread_id == 0)
      nvshmem_barrier_with_same_gpu_idx<kLowLatencyMode>(rdma_team);
    __syncthreads();
    ...
    auto rdma_buffer_ptr_int = reinterpret_cast<int*>(rdma_buffer_ptr);
#pragma unroll
    for (int64_t stage_id = 0; stage_id < num_pipeline_stages; stage_id++) {
      for (int64_t i = thread_id; i < rdma_num_int_clean; i += num_threads)
        rdma_buffer_ptr_int[stage_id * num_rdma_bytes / sizeof(int) +
                            rdma_clean_offset + i] = 0;
    }
    nvshmem_fence();
    __syncthreads();
    ...
    if (thread_id == 0)
      nvshmem_barrier_with_same_gpu_idx<kLowLatencyMode>(rdma_team);
  } else if (sm_id == 1 && pipeline_stage_id == 0) {
    // Barrier for NVL
    barrier_device<NUM_MAX_NVL_PEERS>(task_fifo_ptrs, head, nvl_rank);
    move_fifo_slots<NUM_MAX_NVL_PEERS>(head);
    __syncthreads();
...
#pragma unroll
    for (int64_t stage_id = 0; stage_id < num_pipeline_stages; stage_id++) {
      for (int64_t i = thread_id; i < nvl_num_int_clean; i += num_threads)
        nvl_buffer_ptr_int[stage_id * num_nvl_bytes / sizeof(int) +
                           nvl_clean_offset + i] = 0;
    }
    memory_fence();
    __syncthreads();
    ...
    if (is_cached_dispatch || sm_id == 0 || sm_id == 1) return;
...
#pragma unroll
      for (int k = 0; k < kDtypePerInt4; ++k)
        values[k] +=
            static_cast<float>(recv_value_dtypes[k]);  // 转成了float进行累加
    }
...
#pragma unroll
      ...
      for (int j = 0; j < kDtypePerInt4; ++j) values[j] += out_dtypes[j];
      st_na_global(reinterpret_cast<int8*>(combined_row) + i,
                   *reinterpret_cast<int8*>(values));
    }
  }
...
__global__ void __launch_bounds__((NUM_MAX_NVL_PEERS + 1 + kNumForwarders) * 32,
    ...
    combine(int4* combined_x,
            float* combined_topk_weights,
            const int4** x_vec,
            const float** topk_weights_vec,
            const int** combined_rdma_head_vec,
            const int** combined_nvl_head_vec,
            const int** rdma_channel_prefix_matrix_vec,
            const int** rdma_rank_prefix_sum_vec,
            const int** gbl_channel_prefix_matrix_vec,
            const int* num_tokens_vec,
            const int* num_combined_tokens_vec,
            int hidden,
            int num_topk,
            void* rdma_buffer_ptr,
            int num_max_rdma_chunked_send_tokens,
  ...
  for (int pipeline_stage_id = 0; pipeline_stage_id < num_pipeline_stages;
       ++pipeline_stage_id) {
    const int4* x = x_vec != nullptr ? x_vec[pipeline_stage_id] : nullptr;
    const float* topk_weights = topk_weights_vec != nullptr
                                    ? topk_weights_vec[pipeline_stage_id]
                                    : nullptr;
    const int* combined_rdma_head =
        combined_rdma_head_vec != nullptr
            ? combined_rdma_head_vec[pipeline_stage_id]
            : nullptr;
    const int* combined_nvl_head =
        combined_nvl_head_vec != nullptr
            ? combined_nvl_head_vec[pipeline_stage_id]
            : nullptr;
    const int* rdma_channel_prefix_matrix =
        rdma_channel_prefix_matrix_vec != nullptr
            ? rdma_channel_prefix_matrix_vec[pipeline_stage_id]
            : nullptr;
    const int* rdma_rank_prefix_sum =
        rdma_rank_prefix_sum_vec != nullptr
            ? rdma_rank_prefix_sum_vec[pipeline_stage_id]
            : nullptr;
    const int* gbl_channel_prefix_matrix =
        gbl_channel_prefix_matrix_vec != nullptr
            ? gbl_channel_prefix_matrix_vec[pipeline_stage_id]
            : nullptr;
    const int num_tokens = num_tokens_vec[pipeline_stage_id];
    const int num_combined_tokens = num_combined_tokens_vec[pipeline_stage_id];
    if (warp_role == WarpRole::kNVLSender) {
      // NVL producers
      if (lane_id == 0) {
        while (ld_volatile_global(is_tokens_ready + pipeline_stage_id) != 1) {
          // // Nanosleep and let other warps work
          //   __nanosleep(NUM_WAIT_NANOSECONDS);
        }
      }
      ...
      // NVL layouts
      // NOTES: to avoid deadlocks, we use separate NVL buffers for different
      // RDMA sources
      auto dst_buffer_ptr = reinterpret_cast<void*>(
          reinterpret_cast<uint8_t*>(buffer_ptrs[dst_nvl_rank]) +
          pipeline_stage_id * num_nvl_bytes);  // buffer_ptrs[dst_nvl_rank],
      auto local_buffer_ptr = reinterpret_cast<void*>(
          reinterpret_cast<uint8_t*>(buffer_ptrs[nvl_rank]) +
          pipeline_stage_id * num_nvl_bytes);  // buffer_ptrs[nvl_rank];
      auto nvl_channel_x =
      ...
      while (true) {
        ...
        while (true) {
          ...
          if (clock64() - start_time > NUM_TIMEOUT_CYCLES &&
              lane_id < kNumRDMARanks) {
            printf(
                "FlashEP combine NVL sender timeout, channel: %d, RDMA: %d, "
                "nvl: "
                "%d, dst NVL: %d, RDMA lane: %d, head: %d, tail: %d, start: "
                "%d, "
                "end: %d\n",
                channel_id,
                rdma_rank,
      ...
      // NVL layouts
      void* local_nvl_buffer = reinterpret_cast<void*>(
          reinterpret_cast<uint8_t*>(buffer_ptrs[nvl_rank]) +
          pipeline_stage_id * num_nvl_bytes);
...
#pragma unroll
      for (int i = 0; i < NUM_MAX_NVL_PEERS; ++i)
        nvl_buffers[i] =
            reinterpret_cast<void*>(reinterpret_cast<uint8_t*>(buffer_ptrs[i]) +
                                    pipeline_stage_id * num_nvl_bytes);
      auto nvl_channel_x =
      ...
      if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
        ...
        nvl_channel_topk_weights.advance(
            dst_rdma_rank * num_max_nvl_chunked_recv_tokens_per_rdma *
            num_topk);
        nvl_channel_head.advance(dst_rdma_rank);
        nvl_channel_tail.advance(dst_rdma_rank);

        // Clean shared memory and sync
        EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS <= 32,
                         "Invalid number of NVL peers");
        ...
        for (int token_start_idx = 0; token_start_idx < num_tokens_to_combine;
          ...
          while (sub_warp_id == 0 && lane_id == 0) {
            ...
            if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
              printf(
                  "FlashEP combine forwarder (RDMA check) timeout, channel: "
                  "%d, "
          ...
          for (int token_idx = token_start_idx + sub_warp_id;
            ...
            // Read expected head
            EP_STATIC_ASSERT(kNumRDMARanks <= 32,
            ...
            while (cached_nvl_channel_tail_idx <= expected_head) {
              ...
              if (clock64() - start_time > NUM_TIMEOUT_CYCLES &&
                  lane_id < NUM_MAX_NVL_PEERS) {
                printf(
                    "FlashEP combine forwarder (NVL check) timeout, channel: "
                    "%d, "
                    "RDMA: %d, nvl: %d, src NVL: %d, dst RDMA: %d, tail: %d, "
                    "waiting: %d, total: %d, sub: %d, large: %d, expected: "
                    "%d\n",
                    channel_id,
                    rdma_rank,
                    nvl_rank,
        ...
        for (int64_t token_idx = token_start_idx + warp_id;
          ...
          if (lane_id < kNumRDMARanks) {
            ...
            (expected_head < 0)
                ? (rdma_receiver_rdma_head[warp_id][lane_id] =
                       -expected_head - 1)
          ...
          while (cached_channel_tail_idx <= expected_head) {
            ...
            if (clock64() - start_time > NUM_TIMEOUT_CYCLES) {
              printf(
                  "FlashEP combine RDMA receiver timeout, channel: %d, RDMA: "
                  "%d, "
          ...
          // Combine current token
          auto recv_fn = [&](int src_rdma_rank,
                             int slot_idx,
                             int hidden_int4_idx) -> int4 {
            return ld_nc_global(
          ...
          if (inplace_float_combine)
            out_hidden_int4 *= sizeof(float) / sizeof(dtype_t);
