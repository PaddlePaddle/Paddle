...
#include "paddle/fluid/distributed/collective/flash_ep/include/event.h"
#include "paddle/fluid/distributed/collective/flash_ep/kernels/exception.cuh"

namespace flash_ep {

struct EventHandle {
  std::shared_ptr<flash_ep::detail::Event> event;

  EventHandle() {
    event = std::make_shared<flash_ep::detail::Event>();
    // LOG(WARNING) << "EventHandle constructor is called without record current
    // stream";
    event->record(flash_ep::detail::getCurrentCUDAStream().raw_stream());
  }
  ...
  void current_stream_wait() const {
    CUDA_CHECK(cudaStreamWaitEvent(
        flash_ep::detail::getCurrentCUDAStream().raw_stream(),
        event->cuda_event(),
        0));
  }
};
...
inline flash_ep::detail::Event create_event(const cudaStream_t& s) {
  auto event = flash_ep::detail::Event();
  event.record(s);
  return event;
}
