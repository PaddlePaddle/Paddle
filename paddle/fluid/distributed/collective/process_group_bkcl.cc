...
std::shared_ptr<ProcessGroup::Task> ProcessGroupBKCL::Reduce(
  ...
  return Collective(
      ...
      [&](phi::DenseTensor* output,
	    ...
	    output_copy_ptr = (void*) RAII_GUARD.alloc_l3_or_gm<float>(input.numel());
            num_bytes = input.numel() * sizeof(float);
	    break;
          case phi::DataType::FLOAT16:
	    output_copy_ptr = (void*) RAII_GUARD.alloc_l3_or_gm<float16>(input.numel());
            num_bytes = input.numel() * sizeof(float16);
            break;
          case phi::DataType::INT32:
	    output_copy_ptr = (void*) RAII_GUARD.alloc_l3_or_gm<int>(input.numel());
	  ...
	  xpu::copy(calc_ctx->x_context(), (int8_t*) output_copy_ptr,
			    (int8_t*) output->data(), num_bytes);
        }
        return ret;
      },
      CommType::ALLREDUCE,
