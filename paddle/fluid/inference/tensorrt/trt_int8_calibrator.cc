...
TRTInt8Calibrator::TRTInt8Calibrator(
    const std::unordered_map<std::string, size_t>& dev_buffers, int batch_size,
  ...
  int i = 0;
  VLOG(3) << "Init a new calibrator: " << engine_name_;
  // data_tensors_.resize(data_buffers_.size());
  for (const auto it : dev_buffers) {
...
bool TRTInt8Calibrator::getBatch(void** bindings, const char** names,
  ...
  data_is_set_ = false;
  calib_running_ = true;
  VLOG(3) << "get batch done: " << engine_name_;
  return true;
}
