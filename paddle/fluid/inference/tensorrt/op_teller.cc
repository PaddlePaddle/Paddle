...
// strides > 1 and 'SAME' is only supported by trt7.0 above
    ...
    if (op_type == "transpose2" || op_type == "transpose") {
      ...
      auto* x_var_desc = block->FindVar(x_var_name);
      const auto x_shape = x_var_desc->GetShape();
      VLOG(3) << "in op teller transpose2, get shape success";
      ...
      for (int i = 0; i < dims; i++) {
        perm[i] = axis[i];
        VLOG(3) << "dim:"<<i<<" axis:"<<axis[i]<<" shape:"<< x_shape[i];
      }
    ...
    if (op_type == "reshape" || op_type == "reshape2") {
      ...
      VLOG(3) << "in op teller reshape2, get attr:shape success";
      ...
      if (!with_dynamic_shape) {
        ...
        if (shape[0] == 0) {
          ...
          auto* x_var_desc = block->FindVar(x_var_name);
          const auto x_shape = x_var_desc->GetShape();
          VLOG(3) << "in op teller reshape2, get x_shape success";
          int input_num = std::accumulate(x_shape.begin() + 1, x_shape.end(), 1,
                                          std::multiplies<int>());
          int shape_num = std::accumulate(shape.begin() + 1, shape.end(), 1,
                                          std::multiplies<int>());
          VLOG(3) << "in op teller reshape2, input_num:"<<input_num<<" shape_num:"<<shape_num;
