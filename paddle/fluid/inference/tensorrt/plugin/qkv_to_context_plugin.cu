...
template <typename T>
__global__ void TransposeQkvKernel_v2(const int H, const T *input, T *output, T*output2) {
  // Input: BxSx3xNxH
  // Bias: 3xSxB
  // Output: 3xBxNxSxH
  int n = threadIdx.y;
...
inline void TransposeQKV_v2(const int batch, const int seq_len,
                         const int head_size, const int head_num,
                         const float *input, float *output, float *output_2,
                         cudaStream_t stream) {
  int scratch_size = batch * head_num * seq_len * seq_len;
  const dim3 grid(seq_len, batch, 3);
  if (head_size % 4 == 0 && scratch_size % 4 == 0) {
    ...
    PADDLE_ENFORCE_LE(h * head_num, 1024,
                      platform::errors::InvalidArgument(
                          "head_num (%d) * head_size (%d) should <= %d",
                          head_num, head_size, 1024 * 4));
    TransposeQkvKernel_v2<float4><<<grid, block, 0, stream>>>(h, input4, output4, output_2_4);
  } else if (head_size % 2 == 0 && scratch_size % 2 == 0) {
    const int h = head_size / 2;
    const float2 *input2 = reinterpret_cast<const float2 *>(input);
    float2 *output2 = reinterpret_cast<float2 *>(output);
    ...
    PADDLE_ENFORCE_LE(h * head_num, 1024,
                      platform::errors::InvalidArgument(
                          "head_num (%d) * head_size (%d) should <= %d",
                          head_num, head_size, 1024 * 2));
    TransposeQkvKernel_v2<float2><<<grid, block, 0, stream>>>(h, input2, output2, output_2_2);
  } else {
    const dim3 block(head_size, head_num, 1);
    // limit head_size * head_num to max block size(1024).
    PADDLE_ENFORCE_LE(head_size * head_num, 1024,
                      platform::errors::InvalidArgument(
                          "head_num (%d) * head_size (%d) should <= %d",
                          head_num, head_size, 1024));
    TransposeQkvKernel_v2<float><<<grid, block, 0, stream>>>(head_size, input,
                                                          output, output_2);
  }
}
...
inline void TransposeQKV_v2(const int batch, const int seq_len,
                         const int head_size, const int head_num,
                         const half *input, half *output, half* output_2, cudaStream_t stream) {
  int scratch_size = batch * head_num * seq_len * seq_len;
  const dim3 grid(seq_len, batch, 3);
  if (head_size % 8 == 0 && scratch_size % 8 == 0) {
    int h = head_size / 8;
    ...
    PADDLE_ENFORCE_LE(h * head_num, 1024,
                      platform::errors::InvalidArgument(
                          "head_num (%d) * head_size (%d) should <= %d",
                          head_num, head_size, 1024 * 8));
    TransposeQkvKernel_v2<int4><<<grid, block, 0, stream>>>(h, input4, output4, output_2_4);
  } else if (head_size % 2 == 0 && scratch_size % 2 == 0) {
    const int h = head_size / 2;
    const half2 *input2 = reinterpret_cast<const half2 *>(input);
    half2 *output2 = reinterpret_cast<half2 *>(output);
    ...
    PADDLE_ENFORCE_LE(h * head_num, 1024,
                      platform::errors::InvalidArgument(
                          "head_num (%d) * head_size (%d) should <= %d",
                          head_num, head_size, 1024 * 2));
    TransposeQkvKernel_v2<half2><<<grid, block, 0, stream>>>(h, input2, output2, output_2_2);
  } else {
    const dim3 block(head_size, head_num, 1);
    // limit head_size * head_num to max block size(1024).
    PADDLE_ENFORCE_LE(head_size * head_num, 1024,
                      platform::errors::InvalidArgument(
                          "head_num (%d) * head_size (%d) should <= %d",
                          head_num, head_size, 1024));
    TransposeQkvKernel_v2<half><<<grid, block, 0, stream>>>(head_size, input,
                                                         output, output_2);
  }
}
...
template<typename T>
__global__ void RotrayKernel(const T *inputact, const T*input1, const T*intput2, T *output, const int nElement, const int lastdim)
{
...
inline int round_up(int seq_len, int multiple = 32) {
  PADDLE_ENFORCE_GT(
      multiple, 0,
      platform::errors::InvalidArgument(
          "multiple should be a positive number锛宐ut it's (%d)", multiple));
  ...
  if (input_type == nvinfer1::DataType::kFLOAT) {
    ...
    if (ProductDim(input_desc[3].dims) == (batch * seq_len)) {
      ...
      broadcast<<<grid, block, 0, stream>>>(
          static_cast<const float *>(inputs[3]), temp_qk_bias, seq_len,
          head_number_);
    ...
    RotrayKernel<<<blocks, threads, 0, stream>>>(tmp_roformer_ptr, input_cos_data, input_sin_data, tptr, n_q, head_size_); //q
    RotrayKernel<<<blocks, threads, 0, stream>>>(tmp_roformer_ptr + n_q, input_cos_data, input_sin_data, tptr+n_q, n_q, head_size_);//k
    ...
    if (ProductDim(input_desc[3].dims) == (batch * seq_len)) {
      ...
      int block = round_up(seq_len);
      broadcast<<<grid, block, 0, stream>>>(
          static_cast<const half *>(inputs[3]), temp_qk_bias, seq_len,
          head_number_);
      qk_bias = temp_qk_bias;
    }
    ...
    multihead_compute_func(dev_ctx, batch, seq_len, head_number_, head_size_,
                           qkptr, input3_data, tptr, half(1.), half(0.0));
