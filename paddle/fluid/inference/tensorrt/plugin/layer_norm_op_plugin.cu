...
int LayerNormPlugin::enqueue(int batch_size, const void *const *inputs,
  ...
  const auto input_ddim = pten::make_ddim(input_shape);
  auto matrix_dim = pten::flatten_to_2d(input_ddim, begin_norm_axis);
  ...
  scale_t.Resize(pten::make_ddim({feature_size}));
  bias_t.Resize(pten::make_ddim({feature_size}));
  mean_t.Resize(pten::make_ddim(mean_shape_));
  variance_t.Resize(pten::make_ddim(variance_shape_));
  int device_id;
