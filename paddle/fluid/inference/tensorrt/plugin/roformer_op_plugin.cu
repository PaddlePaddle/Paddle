...
__global__ void RoformerKernel(const T *inputact,
  ...
  for (int index = _index; index < right_index; index += blockDim.x) {
    ...
    int h_index = index % h;
    int cos_index = seq_index * h + h_index;

    T left = input1[cos_index] * inputact[index];
    int col = index % lastdim;
    const int new_index = index - col + (col + half_lastdim) % lastdim;
    if (col >= half_lastdim) {
      output[index] = left + input2[cos_index] * inputact[new_index];
    } else {
      output[index] = left - input2[cos_index] * inputact[new_index];
    }
  }
...
RoformerPlugin::RoformerPlugin(const std::string &name,
                               int head_num,
                               int head_size)
...
RoformerPlugin::~RoformerPlugin() { WHERE_AM_I(); }
...
int32_t RoformerPlugin::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,
...
void RoformerPlugin::terminate() noexcept { WHERE_AM_I(); }
