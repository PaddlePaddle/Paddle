#include "generic_dynamic_plugin.h"
...
void GenericDynamicPlugin::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* in,
  ...
  CHECK(phi_kernel_context_);
  LOG(INFO) << "nb_inputs " << nb_inputs;
  LOG(INFO) << "nb_outputs " << nb_outputs;
}
...
int GenericDynamicPlugin::enqueue(const nvinfer1::PluginTensorDesc* input_desc,
  ...
  // Consider only single input and output
  LOG(INFO) << "enqueue " << op_desc_.Type();
  CHECK(phi_kernel_context_);
  std::vector<phi::DenseTensor>dense_tensor_inputs(1);
  std::vector<phi::DenseTensor>dense_tensor_outputs(1);

  // input
  for(int i = 0; i < 1; i++) {
    auto const &input_dims = input_desc[0].dims;
  ...
  // output
  for(int i = 0; i < 1; i++) {
    auto const &output_dims = output_desc[0].dims;
