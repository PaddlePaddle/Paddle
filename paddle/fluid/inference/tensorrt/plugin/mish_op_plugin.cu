...
__global__ void mish_kernel(int num, const float *input, float *output,
                             float beta) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < num) {
#if __CUDA_ARCH__ >= 350
    output[index] =
        __ldg(input + index) * tanhf(logf(1.0f + expf(__ldg(input + index))));
#else
    output[index] = input[index] * tanhf(logf(1.0f + expf(input[index])));
#endif
  }
}
...
int MishPlugin::enqueue(int batch_size, const void *const *inputs,
  ...
  mish_kernel<<<blocks, threads, 0, stream>>>(num, input, output, beta_);
