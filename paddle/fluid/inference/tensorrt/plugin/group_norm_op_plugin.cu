...
//   // Allocate shared memory for the groups. We could reduce the amount of
//   shared
//   // memory reserved.
...
//     // Store the scaled values.
//     if (ci < params.c) {
//       *reinterpret_cast<__half2 *>(&params.dst[offset]) =
  ...
  if (input_type == nvinfer1::DataType::kFLOAT) {
    ...
    if (input_desc[0].format == nvinfer1::PluginFormat::kLINEAR) {
      ...
      cudaMemsetAsync(params_.redBuffer,
                      ...
                      stream);
      phi::groupNormNHWCSum<half> groupnorm_nhwc_sum;
