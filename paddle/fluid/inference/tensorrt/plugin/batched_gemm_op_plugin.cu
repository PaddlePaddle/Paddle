...
template<typename T>
__global__ void Add_ReluKernel(T *pInput, const T* pBias, int col_bias, int col_input, int bc)
{
    //inplace
    const int tx = threadIdx.x, index = blockIdx.x * col_input + threadIdx.x;
    if(tx >= bc*col_input) return;
    int offset_bias =  blockIdx.x * col_bias + threadIdx.x % col_bias;
    int offset_input = blockIdx.x * col_input + threadIdx.x;
    T _x = pInput[offset_input];
    T _y = _x + pBias[offset_bias];
    pInput[offset_input] = fmaxf(0, _y);
    return;
}

template<typename T>
__global__ void AddKernel(T *pInput, const T* pBias, int col_bias, int col_input, int bc)
{
    //inplace
    const int tx = threadIdx.x, index = blockIdx.x * col_input + threadIdx.x;
    if(tx >= bc*col_input) return;
...
CuBLASBatchedGemmPlugin::CuBLASBatchedGemmPlugin(const std::string &name, const nvinfer1::Weights& weight, const nvinfer1::Weights& bias, const int k, const int n, const int batchcount, const bool has_relu, const bool with_fp16):
    name_(name), nK_(k), nN_(n), batchcount_(batchcount), has_relu_(has_relu) 
{
    WHERE_AM_I()
...
int32_t CuBLASBatchedGemmPlugin::enqueue(const nvinfer1::PluginTensorDesc *inputDesc, const nvinfer1::PluginTensorDesc *outputDesc, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept {
    ...
    if(with_fp16_) {
        ...
        if(has_relu_) {
            Add_ReluKernel<half><<<grid, thread>>>(output, bias, nN_, thread, batchcount_);
        } else {
            AddKernel<half><<<grid, thread>>>(output, bias, nN_, thread, batchcount_);
        }
    } else {
        ...
        if(has_relu_) {
            Add_ReluKernel<float><<<grid, thread>>>(output, bias, nN_, thread, batchcount_);
        } else {
            AddKernel<float><<<grid, thread>>>(output, bias, nN_, thread, batchcount_);
        }
    }
    return 0;
}
...
void CuBLASBatchedGemmPlugin::destroy() noexcept {
    ...
    std::cout<<weight_.values<<std::endl;
