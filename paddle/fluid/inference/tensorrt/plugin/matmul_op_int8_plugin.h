...
class MatmulPlugin : public nvinfer1::IPluginV2IOExt {
 public:
  ...
  MatmulPlugin(void const* serial_data, size_t serial_length) {
    ...
    DeserializeValue(&serial_data, &serial_length, &inscale_0_);
    DeserializeValue(&serial_data, &serial_length, &inscale_1_);
    DeserializeValue(&serial_data, &serial_length, &outscale_);
  ...
  nvinfer1::IPluginV2IOExt* clone() const TRT_NOEXCEPT override {
    ...
    ptr->m_ = m_;
    ptr->n_ = n_;
    ptr->inscale_0_ = inscale_0_;
    ptr->inscale_1_ = inscale_1_;
    ptr->outscale_ = outscale_;
    ptr->type_ = type_;
    return ptr;
  }
 ...
 protected:
  ...
  size_t getSerializationSize() const TRT_NOEXCEPT override {
    return SerializedSize(dims_x_) + SerializedSize(dims_y_) +
           ...
           SerializedSize(alpha_) + SerializedSize(batch_) +
           SerializedSize(k_) + SerializedSize(m_) + SerializedSize(n_) +
           SerializedSize(inscale_0_) + SerializedSize(inscale_1_) +
           SerializedSize(outscale_) + SerializedSize(type_);
  }
  ...
  void serialize(void* buffer) const TRT_NOEXCEPT override {
    ...
    SerializeValue(&buffer, inscale_0_);
    SerializeValue(&buffer, inscale_1_);
    SerializeValue(&buffer, outscale_);
