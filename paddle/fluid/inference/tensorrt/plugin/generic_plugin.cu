...
void BuildPhiKernelContextAttr(const framework::OpDesc& op_desc,
  ...
  for (size_t k = 0; k < attr_names.size(); ++k) {
    ...
    // LOG(INFO) << "BuildPhiKernelContextAttr " << signature.name << " " <<
    // attr_name;
...
GenericPlugin::GenericPlugin(
  ...
  LOG(INFO) << "inputs_data_type_.size() " << inputs_data_type_.size();
}
...
nvinfer1::IPluginV2DynamicExt* GenericPlugin::clone() const TRT_NOEXCEPT {
  LOG(INFO) << "clone: in_out_info_.inputs_data_type.size() "
            << in_out_info_.inputs_data_type.size();
  nvinfer1::IPluginV2DynamicExt* plugin =
      new GenericPlugin(proto_op_desc_, inputs_data_type_);
  plugin->initialize();
  return plugin;
}
...
int GenericPlugin::initialize() TRT_NOEXCEPT {
  ...
  LOG(INFO) << "initialize GenericPlugin " << op_type;

  PADDLE_ENFORCE_EQ(
      phi::OpUtilsMap::Instance().HasArgumentMappingFn(op_type),
      true,
      platform::errors::Fatal("%s has no argument mapping function!",
                              op_type.c_str()));
...
int GenericPlugin::enqueue(const nvinfer1::PluginTensorDesc* input_desc,
  ...
  for (int i = 0; i < getNbInputs(); i++) {
    ...
    LOG(INFO) << "input_shape: " << vectorToStr(input_shape);
    ...
    LOG(INFO) << "inputs_data_type_.size():" << inputs_data_type_.size();
    LOG(INFO) << "inputs_data_type_[i]:" << int(inputs_data_type_[i]);
    LOG(INFO) << "protoType2PhiType(inputs_data_type_[i]):"
              << int(protoType2PhiType(inputs_data_type_[i]));
    ...
    // dense_tensor_inputs[i] = std::move(phi::DenseTensor(input_alloc,
    // input_meta));
    // phi_kernel_context_->EmplaceBackInput(&dense_tensor_inputs[i]);
  }
  ...
  for (int i = 0; i < getNbOutputs(); i++) {
    ...
    LOG(INFO) << "output_shape: " << vectorToStr(output_shape);

    phi::DenseTensorMeta output_meta(phi::DataType::FLOAT32,
    ...
    // dense_tensor_outputs[i] = std::move(phi::DenseTensor(output_alloc,
    // output_meta));
    // phi_kernel_context_->EmplaceBackOutput(&dense_tensor_outputs[i]);
  }
