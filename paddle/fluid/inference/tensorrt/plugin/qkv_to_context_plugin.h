...
class QkvToContextAndQkPluginDynamic : public DynamicPluginTensorRT {
 public:
  ...
  const char* getPluginType() const TRT_NOEXCEPT override {
    return "qkv_to_context_and_qk_plugin";
  }
...
class QkvToContextAndQkPluginDynamicCreator : public nvinfer1::IPluginCreator {
 public:
  QkvToContextAndQkPluginDynamicCreator() {}
  const char* getPluginName() const TRT_NOEXCEPT override {
    return "qkv_to_context_and_qk_plugin";
  }
...
REGISTER_TRT_PLUGIN_V2(QkvToContextAndQkPluginDynamicCreator);
#endif
