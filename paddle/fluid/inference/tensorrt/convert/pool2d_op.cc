...
class Pool2dOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc &op,
    ...
    bool global_pooling = BOOST_GET(bool, op_desc.GetAttr("global_pooling"));
    std::string pool_type =
        BOOST_GET(std::string, op_desc.GetAttr("pooling_type"));
    std::vector<int> ksize =
        BOOST_GET(std::vector<int>, op_desc.GetAttr("ksize"));
    std::vector<int> strides =
        BOOST_GET(std::vector<int>, op_desc.GetAttr("strides"));
    std::vector<int> paddings =
        BOOST_GET(std::vector<int>, op_desc.GetAttr("paddings"));
    bool ceil_mode = BOOST_GET(bool, op_desc.GetAttr("ceil_mode"));
    bool adaptive = false;
    if (op_desc.HasAttr("adaptive"))
      adaptive = BOOST_GET(bool, op_desc.GetAttr("adaptive"));
    ...
    if (op_desc.HasAttr("enable_int8")) {
      ...
      float input_scale = BOOST_GET(float, op_desc.GetAttr("X_scale"));
