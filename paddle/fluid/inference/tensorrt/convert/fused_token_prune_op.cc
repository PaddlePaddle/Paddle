...
class FusedTokenPruneOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    bool keep_first_token = op_desc.HasAttr("keep_first_token")
    ...
    if (engine_->with_dynamic_shape()) {
      ...
      plugin::FusedTokenPrunePluginDynamic* plugin =
          new plugin::FusedTokenPrunePluginDynamic(with_fp16, keep_first_token, keep_order);
    ...
    RreplenishLayerAndOutput(layer, "fused_token_prune", {output_name, out_inds_name},
                             test_mode);
  }
};
