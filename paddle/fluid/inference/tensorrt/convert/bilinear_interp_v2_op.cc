...
class BilinearInterpolateV2OpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    scale.reserve(3);
    const std::vector<float> scale_tensor = BOOST_GET_CONST(std::vector<float>, op_desc.GetAttr("Scale"));
    auto in_dim = input->getDimensions();

    if (scale_tensor.size() != (size_t)in_dim.nbDims) {
      ...
      int w_axis = (data_layout == framework::DataLayout::kNCHW) + 1 + with_dynamic;
      float scale_h = static_cast<float>(out_h) / static_cast<float>(in_dim.d[h_axis]);
      float scale_w = static_cast<float>(out_w) / static_cast<float>(in_dim.d[w_axis]);
      ...
      if (data_layout == framework::DataLayout::kNCHW) {
        ...
        PADDLE_THROW(platform::errors::InvalidArgument("Data layout must be NCHW or NHWC."));
      }

    } else {
      std::copy(scale_tensor.begin(), scale_tensor.end(), scale.begin());
    }
