...
class LeakyReluOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    // y_relu = (x > 0) : x : 0
    auto* relu_layer = TRT_ENGINE_ADD_LAYER(
    ...
    //
    TensorRTEngine::Weight sub_scale{
        nvinfer1::DataType::kFLOAT, &alpha_data[1], 1};
    auto* scale_relu_layer = TRT_ENGINE_ADD_LAYER(engine_,
                                                  Scale,
                                                  *(relu_layer->getOutput(0)),
                                                  nvinfer1::ScaleMode::kUNIFORM,
                                                  shift.get(),
                                                  sub_scale.get(),
                                                  power.get());
