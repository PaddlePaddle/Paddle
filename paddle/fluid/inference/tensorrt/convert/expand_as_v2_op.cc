...
class ExpandAsV2OpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
                  const framework::Scope& scope,
                  bool test_mode) override {
    VLOG(3) << "convert a paddle expand_as_v2 op to trt expand layer.";
    ...
    nvinfer1::ITensor* shape_tensor = engine_->GetITensor(op_desc.Input("Y")[0]);
    int32_t shape_rank = shape_tensor->getDimensions().d[0];
    ...
    RreplenishLayerAndOutput(layer, "expand_as_v2", {output_name}, test_mode);
  }
};
