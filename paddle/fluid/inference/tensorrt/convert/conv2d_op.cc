...
void ConvertConv2d(TensorRTEngine* engine, const framework::proto::OpDesc& op,
  ...
  if (enable_int8) {
    ...
    float in_scale = BOOST_GET(float, op_desc.GetAttr("Input_scale"));
    auto weight_scale =
  ...
  const int groups = BOOST_GET(int, op_desc.GetAttr("groups"));
  const std::vector<int> dilations =
      BOOST_GET(std::vector<int>, op_desc.GetAttr("dilations"));
  const std::vector<int> strides =
      BOOST_GET(std::vector<int>, op_desc.GetAttr("strides"));
  const std::vector<int> paddings =
      BOOST_GET(std::vector<int>, op_desc.GetAttr("paddings"));
