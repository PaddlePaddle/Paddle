 ...
 */
class IndexSelectOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
                  const framework::Scope& scope,
                  bool test_mode) override {
    VLOG(4) << "convert fluid IndexSelect op to tensorrt Slice layer";
    ...
    plugin::GenericDynamicPlugin *plugin = new plugin::GenericDynamicPlugin(op);
    layer = engine_->AddDynamicPlugin(inputs, 2, plugin);
...
REGISTER_TRT_OP_CONVERTER(index_select, IndexSelectOpConverter);
