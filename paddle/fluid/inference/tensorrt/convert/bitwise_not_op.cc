 ...
 public:
  void operator()(const framework::proto::OpDesc& op,
    ...
    if (data_type == nvinfer1::DataType::kBOOL) {
      layer = TRT_ENGINE_ADD_LAYER(engine_, Unary, *input_tensor, nvinfer1::UnaryOperation::kNOT);
    } else {
      nvinfer1::Dims input_dims = input_tensor->getDimensions();

      // set up a elementwise -1 tensor, can not get the dims info for dynamic_shape so just let it broadcaste
      ...
      auto neg_one_tensor = TRT_ENGINE_ADD_LAYER(engine_, Constant, neg_one_tensor_dims, weights)->getOutput(0);

      auto mul_neg_one = TRT_ENGINE_ADD_LAYER(engine_, ElementWise, *input_tensor, *neg_one_tensor, nvinfer1::ElementWiseOperation::kPROD);
      
      layer = TRT_ENGINE_ADD_LAYER(engine_, ElementWise, *(mul_neg_one->getOutput(0)), *neg_one_tensor, nvinfer1::ElementWiseOperation::kSUM);
    }
