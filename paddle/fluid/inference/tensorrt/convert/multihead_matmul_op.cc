...
class MultiheadMatMulWithAttentionOpConverter : public OpConverter {
 public:
  void operator()(const framework::proto::OpDesc& op,
                  const framework::Scope& scope, bool test_mode) override {
    VLOG(3) << "convert a fluid multihead_matmul_with_attention op to a "
               "corresponding tensorrt network structure";
    framework::OpDesc op_desc(op, nullptr);
    // Declare inputs
    auto* input = engine_->GetITensor(op_desc.Input("Input").front());
    ...
    nvinfer1::ILayer* layer = nullptr;
    auto output_name = op_desc.Output("Out")[0];
    auto attention_name = op_desc.Output("Attention")[0];

    if (engine_->with_dynamic_shape()) {
      if (engine_->use_oss()) {
        PADDLE_THROW(platform::errors::Fatal(
            "MultiheadMatMulWithAttentionOp does not support OSS."));
      } else {
        PADDLE_ENFORCE_EQ(
            input->getDimensions().nbDims, 3,
            platform::errors::InvalidArgument(
    ...
    RreplenishLayerAndOutput(layer, "multihead_matmul_with_attention",
                             {output_name, attention_name}, test_mode);
  }
};
...
REGISTER_TRT_OP_CONVERTER(multihead_matmul_with_attention,
                          MultiheadMatMulWithAttentionOpConverter);
