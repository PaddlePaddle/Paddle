...
class NotEqualOpConverter : public OpConverter {
 public:
  ...
  void operator()(const framework::proto::OpDesc& op,
    ...
    layer = engine_->network()->add_unary(layer->getOutput(0), op=nvinfer1::UnaryOperation.ABS)
    RreplenishLayerAndOutput(layer, "not_equal", {output_name}, test_mode);
#else
    PADDLE_THROW(
        platform::errors::Fatal("ElementWise NotEqual Operation is only supported "
