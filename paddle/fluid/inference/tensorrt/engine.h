...
class TensorRTEngine {
 ...
 public:
  ...
  Weight GetFp16TrtWeight(const std::string& name,
                          const DenseTensor& weight_tensor);
  ...
  std::unordered_map<std::string /*name*/, std::unique_ptr<DenseTensor>>
      weight_map;
  ...
  void SetWeights(std::string w_name, std::unique_ptr<DenseTensor> w_tensor) {
