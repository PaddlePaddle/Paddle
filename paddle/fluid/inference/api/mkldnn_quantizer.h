...
class AnalysisPredictor::MkldnnQuantizer {
 ...
 private:
  ...
  void CalculateSingleScale(const std::string& op_name,
                            ...
                            const framework::Tensor& var_tensor,
                            bool is_unsigned);
  ...
  std::pair<bool, framework::Tensor> GetKLScalingFactor(
      const framework::Tensor& var_tensor, bool is_unsigned) const;

  std::pair<bool, framework::Tensor> GetMaxChScalingFactor(
      const framework::Tensor& var_tensor, bool is_unsigned,
      bool is_transposed) const;

  std::pair<bool, framework::Tensor> GetMaxChGRUScalingFactor(
      const framework::Tensor& wx_tensor,
      const framework::Tensor& wh_tensor) const;

  std::pair<bool, framework::Tensor> GetMaxChLSTMScalingFactor(
      const framework::Tensor& wx_tensor,
      const framework::Tensor& wh_tensor) const;

  std::pair<bool, framework::Tensor> GetMaxScalingFactor(
      const framework::Tensor& var_tensor, bool is_unsigned) const;
