...
public:
	static void Image_Read(std::string imagename, unsigned char* &imagebuf, int& imagewidth, int& imageheight, int& imagebitcount) {
		int imagelinebyte = 0;
		FILE* filep;
		...
		if (filerror == 0) {
			if (!fseek(filep, sizeof(BITMAPFILEHEADER), 0)) {
				...
				unsigned char *imagedatabuf = new unsigned char[imagewidth*imageheight * 3];
				for (int i = 0; i < imageheight; i++) {
					for (int j = 0; j < imagewidth; j++) {
						for (int k = 0; k < 3; k++) {
							imagedatabuf[k*imagewidth * imageheight + i * imagewidth + j] = imagebuf[i*imagelinebyte + j * 3 + k];
						}
					}
				}
				delete imagebuf;
	...
	static bool Image_Save(const char* imagename, unsigned char* imagebuf,
		...
		fileHead.bfSize = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + colorTablesize + imagelinebyte*imageheight;
	...
	static void Save_Bmp(const char * filename, int64_t* resultimage, int image_size) {
		...
		for (int channel = 0; channel < 3; channel++) {
			for (int height = 0; height < Image_Height; height++) {
				for (int width = 0; width < Image_Width; width++) {
					record.data[i++] = float((float)Image_Buf[channel*Image_Height*Image_Width +
																							 height*Image_Width + width] - Image_Mean[channel]);
				}
			}
		}
