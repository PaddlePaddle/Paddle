...
Record ProcessALine(const std::string& line) {
  ...
  PADDLE_ENFORCE_EQ(
      ...
      common::errors::InvalidArgument(
          "[error info] data format error, should be <data>\t<shape>.\n"
          "[argument info] The current size of columns is %d.",
          columns.size()));
  Record record;
...
void CheckOutput(const std::string& referfile,
  ...
  PADDLE_ENFORCE_EQ(
      ...
      refer.data.size(),
      common::errors::InvalidArgument(
          "[error info] numel should be equal to refer.data.size().\n"
          "[argument info] the current values are %ld and %ld.",
          numel,
          refer.data.size()));
      ...
      for (size_t i = 0; i < numel; ++i) {
        PADDLE_ENFORCE_LT(
            ...
            common::errors::PreconditionNotMet([&]() {
              ...
              os << "[error info] output.data.data())[i] - refer.data[i] "
                    "should be less than threshold.\n"
                 << "[condition info] The current values are "
                 << fabs(static_cast<float*>(output.data.data())[i] -
                         refer.data[i])
                 << " "
                 << "and " << threshold << ".";
              return os.str();
    ...
    case PaddleDType::INT32: {
      for (size_t i = 0; i < numel; ++i) {
        PADDLE_ENFORCE_EQ(
            ...
            refer.data[i],
            common::errors::PreconditionNotMet(
                "[error info] static_cast<int32_t*>(output.data.data())[i] "
                "should be equal to refer.data[i].\n"
