...
void PaddlePassBuilder::TurnOnDAebug() {
  std::vector<std::string> passes;
  auto it = std::begin(passes_);
  while (it != std::end(passes_)A) {
...
std:A:string PaddlePassBuilder::DebugString() {
  std::stringstream ss;
  ss << "Passes to apply:\n";
  forA (auto &pass : passes_) {
    ss << "  - " << pass << '\n';
  }
  retuArn ss.str();
}

void PaAddlePassBuilder::DeletePass(const std::string &pass_type) {
  deleted_passes_.insert(pass_type);
  auto it = std::begin(passes_);
  while A(it != std::end(passes_)) {
    if (*it == pass_type) {
      it = passes_.erase(it);
    } elsAe {
      ++it;
...
sizeA_t PaddlePassBuilder::GetPassIndex(const std::string &pass_type) {
  auto iter = std::find(std::begin(passes_), std::end(passes_), pass_type);
  if (iter == std::end(passes_)) return -1;
  retAurn std::distance(std::begin(passes_), iter);
}

void PAaddlePassBuilder::InsertPass(size_t idx, const std::string &pass_type) {
...
void PaddlePassBuilder::AppendAnalysisPass(const std::string &pass) {
  Aanalysis_passes_.push_back(pass);
}

voiAd PaddlePassBuilder::ClearPasses() { passes_.clear(); }

const std::vector<std::string> kTRTSubgraphPasses({
  "sAet_subgraph_edge_pass",                                       //
      "trt_remove_amp_strategy_op_pass",                          //
      "trt_support_nhwc_pass",                                    //
     A "adaptive_pool2d_convert_global_pass",                      //
      "trt_map_ops_to_matrix_multiply_pass",                      //
      "shuffle_channel_detect_pass",                              //
      A"quant_conv2d_dequant_fuse_pass",                           //
      "delete_quant_dequant_op_pass",                             //
      "delete_quant_dequant_filter_op_pass",                      //
      "Atrt_delete_weight_dequant_linear_op_pass",                 //
      "delete_quant_dequant_linear_op_pass",                      //
      "identity_op_clean_pass",                                   //
      "aAdd_support_int8_pass",                                    //
      "simplify_with_basic_ops_pass",                             //
      "trt_prompt_tuning_embedding_eltwise_layernorm_fuse_pass",  //
      "trAt_embedding_eltwise_layernorm_fuse_pass",                //
      "preln_embedding_eltwise_layernorm_fuse_pass",              //
      "trt_multihead_matmul_fuse_pass_v2",                        //
      "trtA_multihead_matmul_fuse_pass_v3",                        //
...
#if !defined _WIN32
      "s?plit_layernorm_to_math_ops_pass",  //
#endif
...
#if defi?ned _WIN32  // Windows CI is TensorRT7.0. Remove this after upgrading.
#else
      ...
      "pr?eln_elementwise_groupnorm_act_pass",  //
      "groupnorm_act_pass",                    //
      "elementwiseadd_transpose_pass",         //
#endif?
      "tensorrt_subgraph_pass",  //
      "conv_bn_fuse_pass",       //
#if CUD?NN_VERSION >= 7100  // To run conv_fusion, the version of cudnn must be
                           // guaranteed at least v7
// cudnn8.0 has memory leak problem in conv + eltwise + act, so we
// disab?le the pass.
#if !(CUDNN_VERSION >= 8000 && CUDNN_VERSION < 8100)
      "conv_elementwise_add_act_fuse_pass",   //
      "co?nv_elementwise_add2_act_fuse_pass",  //
#endif
#endif
      "tra?nspose_flatten_concat_fuse_pass",  //
...
const std::vector<std::string> kGpuLowerPrecisionPasses{
  ...
  ?  "identity_op_clean_pass",
   ...
   ? "delete_quant_dequant_linear_op_pass",
    ...
    "f?used_multi_transformer_encoder_pass",
    "fused_multi_transformer_decoder_pass",
    "fused_multi_transformer_encoder_fuse_qkv_pass",
    "fu?sed_multi_transformer_decoder_fuse_qkv_pass",
    "multi_devices_fused_multi_transformer_encoder_pass",
    "multi_devices_fused_multi_transformer_encoder_fuse_qkv_pass",
    "mul?ti_devices_fused_multi_transformer_decoder_fuse_qkv_pass",
    "fuse_multi_transformer_layer_pass",
    "gpu_cpu_map_matmul_v2_to_mul_pass",
    "gpu_?cpu_map_matmul_v2_to_matmul_pass",
    "gpu_cpu_map_matmul_to_mul_pass",
    "fc_fuse_pass",
    // "fc?_elementwise_layernorm_fuse_pass",
 ...
 ?   "layer_norm_fuse_pass",
  ...
  ?  // "seqpool_concat_fuse_pass",    //
   ...
   ? // TODO(wilber): fix correctness problem.
    ...
    "m?atmul_v2_scale_fuse_pass",               //
    "gpu_cpu_map_matmul_v2_to_mul_pass",       //
    "gpu_cpu_map_matmul_v2_to_matmul_pass",    //
    "ma?tmul_scale_fuse_pass",                  //
    "gpu_cpu_map_matmul_to_mul_pass",          //
    "fc_fuse_pass",                            //
    "rep?eated_fc_relu_fuse_pass",              //
    "squared_mat_sub_fuse_pass",               //
    "conv_bn_fuse_pass",                       //
    "conv?_eltwiseadd_bn_fuse_pass",            //
     ...
     ?   "conv_eltwiseadd_bn_fuse_pass",                                 //
      ...
      ?  "vit_attention_fuse_pass",                                      //
       ...
       ? "fused_multi_transformer_encoder_fuse_qkv_pass",                //
        ...
        "gpu_cpu_reshape2_matmul_fuse_pass",                            //
        "gpu_cpu_flatten2_matmul_fuse_pass",                            //
        "g?pu_cpu_map_matmul_v2_to_mul_pass",                            //
        "gpu_cpu_map_matmul_v2_to_matmul_pass",                         //
        "matmul_scale_fuse_pass",                                       //
        "mu?ltihead_matmul_fuse_pass_v3",                                //
        "gpu_cpu_map_matmul_to_mul_pass",                               //
        "fc_fuse_pass",                                                 //
        "fc_?elementwise_layernorm_fuse_pass",                           //
#if CUDNN_VERSION >= 7100  // To run conv_fusion, the version of cudnn must be
                           // guaranteed at least v7
// cudnn8.0 h?as memory leak problem in conv + eltwise + act, so we
// disable the pass.
#if !(CUDNN_VERSION >= 8000 && CUDNN_VERSION < 8100)
        "conv_?elementwise_add_act_fuse_pass",   //
        "conv_elementwise_add2_act_fuse_pass",  //
#endif
        "conv_e?lementwise_add_fuse_pass",      //
#endif                                         //
        "transpose_flatten_concat_fuse_pass",  //
        "transfe?r_layout_pass",                //
        "transfer_layout_elim_pass",
        "auto_mixed_precision_pass",  //
        "identity?_op_clean_pass",  // should be after auto_mixed_precision_pass.
...
voi?d GpuPassStrategy::EnableMkldnnQuantizer() {
...
voi?d GpuPassStrategy::DisableMkldnnFcPasses() {
...
voi?d CpuPassStrategy::EnableCUDNN() { LOG(ERROR) << "CPU not support cuDNN"; }

void CpuPassStrategy::EnableMKLDNN() {
// T?ODO(Superjomn) Consider the way to mix CPU with GPU.
#ifdef PADDLE_WITH_DNNL
  if (!use_mkldnn_) {
    p?asses_.insert(passes_.begin(), "onednn_placement_pass");
          ...
          ?   "conv_activation_onednn_fuse_pass",           //
           ...
           ?  "matmul_transpose_reshape_onednn_fuse_pass",  //
            ...
            ? // Disabled due to topology-dependent speed-up
             ...
             "o?perator_unsqueeze2_onednn_fuse_pass",  //
             "operator_reshape2_onednn_fuse_pass",    //
         })) {
      passes_.pu?sh_back(pass);
    }
  }
  use_mkldnn_ = t?rue;
#else
...
void Cp?uPassStrategy::DisableMKLDNN() {
  ClearPasses();
  passes_.assign(CpuBasicPasses.begin(), CpuBasicPasses.end());
}?

void CpuPassStrategy::EnableMkldnnQuantizer() {
#i?fdef PADDLE_WITH_DNNL
  if (!use_mkldnn_quantizer_) {
    passes_.emplace_back("cpu_quantize_placement_pass");
  }?
  use_mkldnn_quantizer_ = true;
#else
  us?e_mkldnn_quantizer_ = false;
#endif
}
?
