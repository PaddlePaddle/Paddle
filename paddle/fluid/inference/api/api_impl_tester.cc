...
void MainWord2Vec(bool use_gpu) {
  ...
  auto output1_tensor = BOOST_GET_CONST(paddle::framework::LoDTensor, output1);
...
void MainImageClassification(bool use_gpu) {
  ...
  float* data = static_cast<float*>(outputs[0].data.data());
  float* lod_data =
      BOOST_GET_CONST(paddle::framework::LoDTensor, output1).data<float>();
  for (size_t j = 0; j < len / sizeof(float); ++j) {
...
void MainThreadsWord2Vec(bool use_gpu) {
  ...
  for (int tid = 0; tid < num_jobs; ++tid) {
    threads.emplace_back([&, tid]() {
      ...
      auto ref_tensor =
          BOOST_GET_CONST(paddle::framework::LoDTensor, refs[tid]);
