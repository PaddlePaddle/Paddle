...
bool PaddleTensorToLoDTensor(const PaddleTensor &pt, framework::LoDTensor *t,
  ...
  if (platform::is_cpu_place(place)) {
    ...
    auto dst_gpu_place = BOOST_GET(platform::CUDAPlace, place);
...
bool AnalysisPredictor::SetFeed(const std::vector<PaddleTensor> &inputs,
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    ...
    if (config_.specify_input_name_) {
      ...
      idx = BOOST_GET(int, feeds_[i]->GetAttr("col"));
    }
...
bool AnalysisPredictor::GetFetch(std::vector<PaddleTensor> *outputs,
  ...
  for (size_t i = 0; i < fetches_.size(); ++i) {
    int idx = BOOST_GET(int, fetches_[i]->GetAttr("col"));
...
std::unique_ptr<ZeroCopyTensor> AnalysisPredictor::GetInputTensor(
  ...
  if (platform::is_cpu_place(place_)) {
    ...
    auto gpu_place = BOOST_GET(platform::CUDAPlace, place_);
  ...
  for (auto &op_desc : block.AllOps()) {
    if (op_desc->Type() == "tensorrt_engine") {
      std::string engine_name =
          BOOST_GET(std::string, op_desc->GetAttr("engine_key"));
