...
paddle::PassType GetVersionCtrlRes(bool has_trt,
  ...
  if (has_trt && has_cudnn && meet2trt_version && meet2cudnn_version) {
    ...
    return paddle::PassType::Default;
  } else {
    return paddle::PassType::Default;
  }
}
...
PassType SinglePassContorller::GetPassStatus(
    const std::string& pass_runtime_status, PassCtrlMode pass_ctrl_mode) {
...
bool PassContorller::LoadDefaultConfig() {
  ...
  try {
    ...
    for (auto ctrl_pass_name : ctrl_passes_) {
      pass_ctrl_map_.emplace(ctrl_pass_name,
                             SinglePassContorller(ctrl_pass_name));
...
void PassContorller::SetPassStatus(const std::string& pass_name,
...
bool PassContorller::LoadDefaultPassCtrl() {
    ...
    if (configFile.is_open()) {
      ...
      while (std::getline(configFile, line)) {
        ...
        if (line[0] == '[' && line.back() == ']') {
          ...
          pass_ctrl_map_.emplace(pass, SinglePassContorller(pass));
        } else {
...
const std::vector<std::string> PassContorller::GetCtrlPassList(
