...
TEST(PD_Config, interface) {
  ...
  PD_ConfigSwitchUseFeedFetchOps(config, FALSE);
  bool use_feed_fetch_ops = PD_ConfigUseFeedFetchOpsEnabled(config);
  EXPECT_FALSE(use_feed_fetch_ops);

  PD_ConfigSwitchSpecifyInputNames(config, TRUE);
  bool specify_input_names = PD_ConfigSpecifyInputName(config);
