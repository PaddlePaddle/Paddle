...
#define PADDLE_VERSION 200030000

#if PADDLE_VERSION < 200000000
  using Predictor = paddle::PaddlePredictor;
...
DEFINE_int32(max_sequence_length, 128, "Max sequence length of the inference model.");
DEFINE_string(mode, "no_opt", "Inference mode. choice = ['no_opt', 'trt', 'trt_varlen', ‘all’]");
...
#if PADDLE_VERSION < 200000000
    if (inputnames[0] != file_input["input_id"].name) 
    {
      std::cerr << "slot 0 should be input_id if paddle version < 2.1\n";
      return;
    }
    if (inputnames[1] != file_input["sent_id"].name) 
    {
      std::cerr << "slot 1 should be sent_id if paddle version < 2.1\n";
      return;
    }
    if (inputnames[2] != file_input["loc_id"].name) 
    { 
      std::cerr << "slot 2 should be loc_id if paddle version < 2.1\n";
      return;
    }
#endif
}

void process_ext_input(const std::vector<std::string> &inputnames, Predictor *predictor)
{
  ...
  for (int i = 4; i < inputnames.size(); i++) {
    for (auto const it : file_input) {
      ...
      if (er_data.name == inputnames[i]) {
      ...
      #if PADDLE_VERSION < 200000000
        auto input = predictor->GetInputTensor(er_data.name);
      #else
        ...
        if (er_data.data_type == "int32_t") {
          ...
          inum.insert(inum.end(), er_data.float_data.begin(), er_data.float_data.end());
        #if PADDLE_VERSION < 200000000
            input->copy_from_cpu(inum.data());
        #else
            input->CopyFromCpu(inum.data());
          ...
          inum.insert(inum.end(), er_data.int64_data.begin(), er_data.int64_data.end());
          #if PADDLE_VERSION < 200000000
            input->copy_from_cpu(inum.data());
          #else
            input->CopyFromCpu(inum.data());
...
std::shared_ptr<Predictor> init_pred_no_varlen() {
  ...
  if (FLAGS_mode == "trt")
  {
    ...
    config.EnableTensorRtEngine(1 << 30, 1, FLAGS_min_subgraph_size, Config::Precision::kHalf,
                                false, false);
    // dynamic shape
...
void run_no_varlen(Predictor *predictor, std::vector<float> *out_data, double &qps) {
  ...
  if (FLAGS_req_with_batch) {
    run_seq_len = static_cast<int>(file_input["input_id"].data.size()) / FLAGS_run_batch;
  } else {
  ...
  {
    i1.insert(i1.end(), file_input["input_id"].data.begin(), file_input["input_id"].data.end());
    i2.insert(i2.end(), file_input["sent_id"].data.begin(), file_input["sent_id"].data.end());
    i3.insert(i3.end(), file_input["loc_id"].data.begin(), file_input["loc_id"].data.end());
    i4.insert(i4.end(), file_input["mask"].data.begin(), file_input["mask"].data.end());
    if (FLAGS_req_with_batch) break; // insert once if req already has batched datas
  }
...
#if PADDLE_VERSION < 200000000
    // first input
    auto input_t1 = predictor->GetInputTensor(file_input["input_id"].name);
    input_t1->Reshape({FLAGS_run_batch, run_seq_len, 1});
    input_t1->copy_from_cpu(i1.data());

    // second input
    auto input_t2 = predictor->GetInputTensor(file_input["sent_id"].name);
    input_t2->Reshape({FLAGS_run_batch, run_seq_len, 1});
    input_t2->copy_from_cpu(i2.data());

    // third input
    auto input_t3 = predictor->GetInputTensor(file_input["loc_id"].name);
    input_t3->Reshape({FLAGS_run_batch, run_seq_len, 1});
    input_t3->copy_from_cpu(i3.data());

    // fourth input
    auto input_t4 = predictor->GetInputTensor(file_input["mask"].name);
    input_t4->Reshape({FLAGS_run_batch, run_seq_len, 1});
    input_t4->copy_from_cpu(i4.data());
...
void run_var_len(Predictor *predictor,  std::vector<float> *out_data, double &qps) {
...
//  const int max_seq_len = 128;
  ...
  for (int i = 0; i < FLAGS_run_batch; i++)
  {
    i1.insert(i1.end(), file_input["input_id"].varlen_data.begin(), file_input["input_id"].varlen_data.end());
    i2.insert(i2.end(), file_input["sent_id"].varlen_data.begin(), file_input["sent_id"].varlen_data.end());
    if (FLAGS_req_with_batch) break; // insert once if req already has batched datas
  }

  // shape info of this batch
  std::vector<int32_t> i3(file_input["loc_id"].varlen_data.begin(), file_input["loc_id"].varlen_data.end());
  // max_single_seq_len represents the max sentence length of all the sentences, only length of
...
void read_file() {
  ...
  while(file.good()) {
    ...
    if (type == "data") {
      ...
      while (numstr.good()) {
        if (data_type == "int32_t") numstr >> num;
        else if (data_type == "float") numstr >> fp_num;
        else if (data_type == "int64_t") numstr >> num64;
        if (!numstr.good()) break;
        numstr >> comma;
        if (data_type == "int32_t") file_input[loc_name].data.push_back(num);
        else if (data_type == "float") file_input[loc_name].float_data.push_back(fp_num);
        else if (data_type == "int64_t") file_input[loc_name].int64_data.push_back(num64);
      }
    }
    else if (type == "name") {
      ...
      if(file_input[loc_name].data_type != "float" && file_input[loc_name].data_type != "int32_t"
        && file_input[loc_name].data_type != "int64_t")
      {
        std::cerr << "unknown data type, only float32, int32, int64 is supported.\n";
      }
    }
    else if (type == "shape") {
      std::istringstream numstr(line.substr(pos +1));
  ...
  if (file_input["input_id"].data.size() > max_single_seq_len * FLAGS_run_batch ||
      file_input["input_id"].data.size() <= 0) {
      std::cerr << "Input data size should < max_single_seq_len * run_batch  and >= 0.\n";
      return ;
  }
...
void run_model(const std::string &mode, std::vector<float> &out_data, double &qps)
{
...
int main(int argc, char *argv[]) {
  ...
  if (FLAGS_mode == "all") run_modes = {"no_opt", "trt", "trt_varlen"};
  else run_modes = {FLAGS_mode};
