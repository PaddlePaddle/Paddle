...
TEST(PD_Tensor, int8) {
  ...
  PD_Tensor* tensor =
      PD_PredictorGetInputHandle(predictor, input_names->data[0]);
  int32_t shapes[4] = {1, 3, 300, 300};
  int8_t input[1 * 3 * 300 * 300] = {0};
  int32_t size;
  PD_PlaceType place;
  PD_TensorReshape(tensor, 4, shapes);
  PD_TensorCopyFromCpuInt8(tensor, input);
  int8_t* data_ptr = PD_TensorDataInt8(tensor, &place, &size);
  EXPECT_EQ(place, PD_PLACE_CPU);
  EXPECT_EQ(size, 1 * 3 * 300 * 300);
  int8_t* mutable_data_ptr = PD_TensorMutableDataInt8(tensor, place);
  EXPECT_EQ(data_ptr, mutable_data_ptr);

  PD_DataType data_type = PD_TensorGetDataType(tensor);
  EXPECT_EQ(data_type, PD_DATA_INT8);
  PD_TensorCopyToCpuInt8(tensor, input);
