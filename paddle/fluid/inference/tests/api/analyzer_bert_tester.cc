...
void profile(bool use_dnnl = false, bool use_bfloat16 = false);
std::vector<std::vector<paddle::PaddleTensor>> LoadInputData();
void CompareNativeAndAnalysisWrapper(bool use_dnnl = false);
std::vector<paddle::PaddleTensor> ParseInputStreamToVector(
    const std::string &line);

AnalysisConfig SetConfig(bool use_dnnl = false, bool use_bfloat16 = false);
...
TEST(Analyzer_bert, compare_mkldnn) {
  auto use_dnnl = true;
  CompareNativeAndAnalysisWrapper(use_dnnl);
}
#endif
...
void profile(bool use_dnnl, bool use_bfloat16) {
  auto config(SetConfig(use_dnnl, use_bfloat16));
...
void CompareNativeAndAnalysisWrapper(bool use_dnnl) {
  auto cfg(SetConfig(use_dnnl));
  auto inputs = LoadInputData();
...
AnalysisConfig SetConfig(bool use_dnnl, bool use_bfloat16) {
