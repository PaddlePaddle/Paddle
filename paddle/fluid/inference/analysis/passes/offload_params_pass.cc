...
std::list<framework::LayerIdx2ParamsTensors> CopyOffloadLayersParams2Cpu(
    std::vector<size_t> *offload_layers,
  ...
  for (auto &ele : weight_queue) {
    offload_layers->push_back(ele.first);
  }
...
std::vector<size_t> CollectOffloadLayersAndParams(
    bool is_multi_layer_fused,
  ...
  if (is_multi_layer_fused) {
    ...
    std::vector<std::string> expected_offload_ops{
        "fused_multi_transformer", "fused_attention", "fused_feedforward"};
    for (size_t i = 0; i < block_topo_order.size(); i++) {
      auto &node = block_topo_order[i];
      if (!node->IsOp()) continue;
      if (std::find(expected_offload_ops.begin(),
                    expected_offload_ops.end(),
                    node->Op()->Type()) != expected_offload_ops.end()) {
        offload_layers.push_back(i);
        size_t offload_params_size = 0;
        for (auto *var_node : node->inputs) {
          if (!var_node->Var()->Persistable()) continue;
          auto offload_var_name = var_node->Name();
          (*offload_layer_attrs)[i].push_back(offload_var_name);
          fixed_vars->erase(offload_var_name);
          auto *offload_var = scope->GetVar(offload_var_name);
          auto *offload_tensor = offload_var->GetMutable<phi::DenseTensor>();
          offload_params_size +=
...
void OffLoadParamsPass::RunImpl(Argument *argument) {
  ...
  // remove feed, fetch op for they won't be executed in runtime
  ...
  if (while_op_with_decoder) {
    ...
    while_op_with_decoder->SetAttr("offload_params", true);
    while_op_with_decoder->SetAttr("offload_vars_pool_idx", sub_block->ID());
    ...
    auto offload_decoder_layers =
        CollectOffloadLayersAndParams(argument->is_multi_layer_fused(),
                                      while_block_topo_order,
                                      sub_block->ID(),
