  ...
  for (size_t index = 0; index < block_desc->OpSize(); ++index) {
    ...
    for (int i = 0; i < op->inputs_size(); i++) {
      ...
      for (int k = 0; k < in_var->arguments_size(); k++) {  // all the arguments
        ...
        const std::string arg_value_with_id = RenameVarBeUnique(arg_value, std::to_string(var2id[arg_value]));
    ...
    if (op_desc.Type() == "conv2d" && trt_and_not_int8) {
      ...
      if (same_hierarchy_conv2d_num_map[input_var_name] > 0) {
        (*output_names_with_id)
            .insert(RenameVarBeUnique(out_var_name, std::to_string(var2id[out_var_name])));
