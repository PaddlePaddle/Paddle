...
std::string replace_name(std::string name,const char* raw,const char* new_char){
...
void DlnneSubgraphPass::CreateDlnneOp(
  ...
  auto* tmp_dump_main_block =
      tmp_dump_program_desc.MutableBlock(0);
    ...
    *op->Proto() = *node->Op()->Proto();
    ...
    //debug
    {
      ...
      for(auto argument_name:op_desc.InputArgumentNames()){
        if (std::count(graph_params.begin(), graph_params.end(),argument_name) > 0) {
  ...
  op_desc->SetInput(
      "Xs", std::vector<std::string>(valid_input_names.begin(), valid_input_names.end()));

  op_desc->SetOutput(
      "Ys", std::vector<std::string>(valid_output_names.begin(), valid_output_names.end()));
  ...
  { 
    ...
    for(auto param_name : params){
        ...
        if(var!=nullptr){
           ...
           ofstream p_stream;
           p_stream.open(dir_name+"/"+replace_name(param_name,"/","."),ios::out);
