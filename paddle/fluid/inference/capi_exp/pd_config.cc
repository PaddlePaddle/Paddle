...
static Config::Precision ConvertToCxxPrecisionType(PD_PrecisionType precision) {
  switch (precision) {
    ...
    default:
      CHECK(false) << "Unsupport paddle precision.";
      return Config::Precision::kFloat32;
  }
}
...
void PD_ConfigSwitchUseFeedFetchOps(__pd_keep PD_Config* pd_config, PD_Bool x) {
  CHECK_AND_CONVERT_PD_CONFIG;
  config->SwitchUseFeedFetchOps(x);
}
PD_Bool PD_ConfigUseFeedFetchOpsEnabled(__pd_keep PD_Config* pd_config) {
  CHECK_AND_CONVERT_PD_CONFIG;
  return config->use_feed_fetch_ops_enabled();
}

void PD_ConfigSwitchSpecifyInputNames(__pd_keep PD_Config* pd_config,
                                      PD_Bool x) {
  CHECK_AND_CONVERT_PD_CONFIG;
  return config->SwitchSpecifyInputNames(x);
}
PD_Bool PD_ConfigSpecifyInputName(__pd_keep PD_Config* pd_config) {
  CHECK_AND_CONVERT_PD_CONFIG;
  return config->specify_input_name();
}
