...
class VariableWrapper {
 public:
  ...
  void RegisterBackwardHooks(const std::function<void()>& func, int id) {
    ...
    VLOG(9) << "Register Backward Hook for Variable: " << Name() << std::endl;
  }
  ...
  void InvokeBackwardHooks() {
    ...
    for (iter = backward_hooks_.begin(); iter != backward_hooks_.end();
         iter++) {
      (iter->second)();
      VLOG(9) << "Invoke Backward Hook for Variable: " << Name() << std::endl;
    }
  }
 ...
 private:
  ...
  // add backward_hooks_ to support backward hook.
