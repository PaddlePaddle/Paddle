  ...
  while (!q.empty()) {
    ...
    for (auto &grad_pending_node : grad_pending_nodes) {
      ...
      for (auto op_base = begin;op_base != end;op_base++) {
        PADDLE_ENFORCE_EQ(op_base->Type() != "py_layer", true,
          platform::errors::PreconditionNotMet(
              "Note: Currently PyLayer is not supported in DataParallel. For "
              "using PyLayer in a DataParallel model, you can skip gradient "
              "synchronization among multiple cards by 'no_sync', and manually "
              "implement 'all_reduce' before model optimization. There is a "
              "demo showing specific implemetation processing in offical docs: "
              "https://www.paddlepaddle.org.cn/documentation/docs/api/paddle/"
              "DataParallel_cn.html"));    
      }
