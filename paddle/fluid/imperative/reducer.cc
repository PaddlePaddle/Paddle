...
void Reducer::MarkGroupReady(size_t group_index) {
  if (group_index > next_group_) {
    VLOG(1) << "It will adjust the order of group in next batch automatically";
    return;
  }
  ...
  for (; next_group_ < groups_.size() && groups_[next_group_].pending_ == 0;
    ...
    if (group.is_sparse_) {
      if (group.sparse_contents_ != nullptr) {
        VLOG(1) << "sparse group [" << next_group_
        ...
        VLOG(1) << "The sparse group[" << next_group_
                << "] has no var to allreduce";
      }
    } else {
      VLOG(1) << "dense group [" << next_group_ << "] start allreduce in ring["
              << run_order << "]";
...
// communication.
      ...
      if (platform::is_xpu_place(group.dense_tensors_[0].place())) {
        VLOG(1) << "Wait Comm init in XPU device id:"
...
void Reducer::FinalizeBackward() {
  ...
  VLOG(1) << "In the batch, Reducer is finished...";
}
