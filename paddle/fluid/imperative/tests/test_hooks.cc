...
std::shared_ptr<imperative::VariableWrapper> DoubleHook(
  ...
  auto* out_tensor = out_var->MutableVar()->GetMutable<framework::Tensor>();
  auto& tensor = var->Var().Get<framework::Tensor>();
...
TEST(TestHooks, TestGradVarLeafBackwardHook) {
  ...
  auto* x_tensor = x->MutableVar()->GetMutable<framework::Tensor>();
  auto* y_tensor = y->MutableVar()->GetMutable<framework::Tensor>();
  ...
  framework::TensorCopySync(x->GradVar().Get<framework::Tensor>(), place,
                            &x_grad);
  ...
  framework::TensorCopySync(y->GradVar().Get<framework::Tensor>(), place,
                            &y_grad);
...
void GradVarLeafBackwardHookWithGradAccmulatedTest() {
  ...
  auto* x_tensor = x->MutableVar()->GetMutable<framework::Tensor>();
  auto* y_tensor = y->MutableVar()->GetMutable<framework::Tensor>();
  auto* z_tensor = z->MutableVar()->GetMutable<framework::Tensor>();
  ...
  framework::Tensor z_grad;
  framework::TensorCopySync(z->GradVar().Get<framework::Tensor>(), place,
                            &z_grad);
