...
static void OpBaseRunImpl(const framework::OperatorBase& op,
   ...
   */
  auto prepared_op =
      PreparedOp::Prepare(ins, outs, *op_kernel, place, attrs, attrs_default);
  auto tmp_ins_ptr =
      PrepareData<VarType>(*op_kernel, ins, prepared_op.kernel_type());
  if (tmp_ins_ptr == nullptr) {
    prepared_op.Run(ins, outs, attrs, attrs_default);
  } else {
    prepared_op.Run(*tmp_ins_ptr, outs, attrs, attrs_default);
  }
