...
class sherwood_v10_table : private EntryAlloc,
 ...
 public:
  ...
  sherwood_v10_table(size_type bucket_count,
  ...
  sherwood_v10_table(std::initializer_list<T> il,
                     size_type bucket_count = 0,
                     const ArgumentHash &hash = ArgumentHash(),
                     const ArgumentEqual &equal = ArgumentEqual(),
                     const ArgumentAlloc &alloc = ArgumentAlloc())
      : sherwood_v10_table(bucket_count, hash, equal, alloc) {
    if (bucket_count == 0) reserve(il.size());
  ...
  sherwood_v10_table(sherwood_v10_table &&other,
  ...
  sherwood_v10_table &operator=(const sherwood_v10_table &other) {
    ...
    static_assert(
        AllocatorTraits::propagate_on_container_copy_assignment::value ==
            BucketAllocatorTraits::propagate_on_container_copy_assignment::
                value,
    ...
    if (AllocatorTraits::propagate_on_container_copy_assignment::value) {
      ...
      AssignIfTrue<BucketAllocator,
                   BucketAllocatorTraits::
                       propagate_on_container_copy_assignment::value>()(*this,
                                                                        other);
    }
    _max_load_factor = other._max_load_factor;
  ...
  sherwood_v10_table &operator=(sherwood_v10_table &&other) noexcept {
    static_assert(
        AllocatorTraits::propagate_on_container_move_assignment::value ==
            BucketAllocatorTraits::propagate_on_container_move_assignment::
                value,
        "The allocators have to behave the same way");
  ...
  iterator find(const FindKey &key) {
    ...
    for (EntryPointer it = *bucket; it; it = it->next) {
      if (compares_equal(key, it->value)) return {it, bucket};
    }
    return end();
  }
  ...
  std::pair<iterator, bool> emplace(Key &&key, Args &&... args) {
    ...
    EntryPointer *bucket = entries + ptrdiff_t(index);
    for (EntryPointer it = *bucket; it; it = it->next) {
      if (compares_equal(key, it->value)) return {{it, bucket}, false};
    }
    return emplace_new_key(
        bucket, std::forward<Key>(key), std::forward<Args>(args)...);
  }
  ...
  void rehash(size_t num_buckets) {
    ...
    EntryPointer *end_it =
        new_buckets + static_cast<ptrdiff_t>(num_buckets + 1);
  ...
  void reserve(size_t num_elements) {
    ...
    num_elements = static_cast<size_t>(
        std::ceil(num_elements / static_cast<double>(_max_load_factor)));
    if (num_elements > bucket_count()) rehash(num_elements);
  }
  ...
  convertible_to_iterator erase(const_iterator to_erase) {
    ...
    AllocatorTraits::destroy(*this,
                             std::addressof(to_erase.current_element->value));
  ...
  convertible_to_iterator erase(const_iterator begin_it,
 ...
 private:
  ...
  emplace_new_key(EntryPointer *bucket, Args &&... args) {
    ...
    if (is_full()) {
      ...
      try {
        AllocatorTraits::construct(*this,
                                   std::addressof(new_entry->value),
  ...
  void deallocate_data() {
    if (entries != Entry::empty_pointer()) {
      BucketAllocatorTraits::deallocate(
          *this, entries - 1, num_slots_minus_one + 2);
    }
  }
    ...
    : public detailv10::sherwood_v10_table<
          ...
          typename std::allocator_traits<A>::template rebind_alloc<
              typename std::allocator_traits<A>::template rebind_traits<
                  detailv10::sherwood_v10_entry<std::pair<K, V>,
                                                A>>::pointer>> {
