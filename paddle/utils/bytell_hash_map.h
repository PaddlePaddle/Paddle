...
class sherwood_v8_table : private ByteAlloc, private Hasher, private Equal {
 ...
 public:
  ...
  sherwood_v8_table(size_type bucket_count,
  ...
  inline std::pair<iterator, bool> emplace(Key &&key, Args &&... args) {
    ...
    for (;;) {
      ...
      if (first) {
        if ((metadata & Constants::bits_for_direct_hit) !=
            Constants::magic_for_direct_hit)
          return emplace_direct_hit({index, block},
                                    std::forward<Key>(key),
      ...
      int8_t to_next_index = metadata & Constants::bits_for_distance;
      if (to_next_index == 0)
        return emplace_new_key({index, block},
                               std::forward<Key>(key),
                               std::forward<Args>(args)...);
      index += Constants::jump_distances[to_next_index];
      index = hash_policy.keep_in_range(index, num_slots_minus_one);
...
class bytell_hash_map : public detailv8::sherwood_v8_table<
                            std::pair<K, V>,
                            K,
                            H,
