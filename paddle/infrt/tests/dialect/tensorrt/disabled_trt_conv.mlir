...
// CHECK-LABEL: @run_trt
func @run_trt(%input_tensor : !core.dense_tensor<GPU, FP32, NCHW>, %kernel_weight : !core.dense_tensor<CPU, FP32, NCHW>, %kernel_bias : !core.dense_tensor<CPU, FP32, NCHW>, %gpu_ctx : !phi.context<GPU>) {
  %a = "trt.create_engine"(%input_tensor, %kernel_weight, %kernel_bias) ({
    ...
    %2 = "trt.Convolution"(%input_tensor, %kernel_weight, %kernel_bias) {out_channel_num = 3 : si32, kernel_size = [3:i32, 3:i32]} : (!core.dense_tensor<GPU, FP32, NCHW>, !core.dense_tensor<CPU, FP32, NCHW>, !core.dense_tensor<CPU, FP32, NCHW>) -> !core.dense_tensor<GPU, FP32, NCHW>
  ...
  %res = "trt.compute"(%a, %gpu_ctx) {} : (!trt.engine, !phi.context<GPU>) -> (!core.tensor_list)
...
func @main() {
  ...
  %input_tensor = "phi_dt.create_dense_tensor.gpu" (%gpu_ctx) {
    ...
    dims=[1:i64, 3:i64, 28:i64, 28:i64], lod=[0:i64]}: (!phi.context<GPU>) -> (!core.dense_tensor<GPU, FP32, NCHW>)
  ...
  %kernel_weight = "phi_dt.create_dense_tensor.cpu"(%cpu_ctx) {
    precision=#core.precision<FP32>,
    layout=#core.layout<NCHW>,
    dims=[3:i64, 3:i64, 3:i64, 3:i64], lod=[0:i64]} : (!phi.context<CPU>) -> (!core.dense_tensor<CPU, FP32, NCHW>)
  ...
  %kernel_bias = "phi_dt.create_dense_tensor.cpu"(%cpu_ctx) {
    precision=#core.precision<FP32>,
    layout=#core.layout<NCHW>,
    dims=[3:i64], lod=[0:i64]} : (!phi.context<CPU>) -> (!core.dense_tensor<CPU, FP32, NCHW>)
  "phi_dt.fill_dense_tensor.f32"(%kernel_bias) {value=[1.:f32]} : (!core.dense_tensor<CPU, FP32, NCHW>) -> ()
  // "phi_dt.print_tensor" (%kernel_bias) : (!core.dense_tensor<CPU, FP32, NCHW>) -> ()

  core.call @run_trt(%input_tensor, %kernel_weight, %kernel_bias, %gpu_ctx) : (!core.dense_tensor<GPU, FP32, NCHW>, !core.dense_tensor<CPU, FP32, NCHW>, !core.dense_tensor<CPU, FP32, NCHW>, !phi.context<GPU>) -> ()
