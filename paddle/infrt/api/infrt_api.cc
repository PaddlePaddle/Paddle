...
int InfRtPredictor::Init(const InfRtConfig& config) {
  ...
  // Load params
  infrt::phi::DenseTensorMap tensor_map =
      ::infrt::kernel::phi::LoadCombinedParameters(config.model_dir(),
                                                   config.param_dir());

  // Create PredictExecutor
  impl_->executor.reset(new PredictExecutor(
      impl_->module_ref.get(), registry, std::move(tensor_map)));
  return 0;
}
