...
#endif  // INFRT_WITH_PHI
#ifdef INFRT_WITH_TRT
            backends::tensorrt::TrtEngine,
            kernel::tensorrt::MlirOperationWithInfrtSymbol,
#endif  // INFRT_WITH_TRT
            std::vector<int16_t>,
            std::vector<int32_t>,
            std::vector<int64_t>,
...
class Value : public common::Object {
 public:
  ...
  explicit Value(TargetType x) : data(x) {}
  explicit Value(LayoutType x) : data(x) {}
  explicit Value(PrecisionType x) : data(x) {}
...
#ifdef INFRT_WITH_PHI
  explicit Value(phi::DenseTensorMap&& x) : data(std::move(x)) {}
...
#ifdef INFRT_WITH_TRT
  explicit Value(backends::tensorrt::TrtEngine&& x) : data(std::move(x)) {}
  explicit Value(kernel::tensorrt::MlirOperationWithInfrtSymbol x) : data(x) {}
#endif  // INFRT_WITH_TRT
#endif
