 ...
 private:
  ...
  adt::Result<adt::Ok> SerializeLastExprInLambdaImpl(
    ...
    for (const auto& [var, combined] : let->bindings) {
      if (IsReturn(combined)) {
        ADT_LET_CONST_REF(value, GetReturnValue(ctx, combined));
        ctx->Generate(std::string("return ") + value);
        return adt::Ok{};
      } else if (IsIdentity(combined)) {
        ADT_LET_CONST_REF(value, GetIdentityInput(ctx, combined));
        ctx->Generate(var.value() + " = " + value);
      } else {
  ...
  adt::Result<std::string> GetReturnValue(
      LambdaBodySerializeCtx* ctx, const Combined<AnfExpr>& combined) const {
    ADT_LET_CONST_REF(call, combined.template TryGet<Call<AnfExpr>>());
    ADT_CHECK(call->args.size() == 1);
    return AtomicToString(ctx, call->args.at(0));
  }

  adt::Result<std::string> GetIdentityInput(
      LambdaBodySerializeCtx* ctx, const Combined<AnfExpr>& combined) const {
    ADT_LET_CONST_REF(call, combined.template TryGet<Call<AnfExpr>>());
    ADT_CHECK(call->args.size() == 1);
    return AtomicToString(ctx, call->args.at(0));
  ...
  bool IsReturn(const Combined<AnfExpr>& combined) const {
    if (!combined.template Has<Call<AnfExpr>>()) return false;
    const auto& call = combined.template Get<Call<AnfExpr>>();
    if (call->args.size() != 1) return false;
    if (!call->func.template Has<tVar<std::string>>()) return false;
    const auto& func_name =
        call->func.template Get<tVar<std::string>>().value();
    return func_name == axpr::kBuiltinReturn();
  }

  bool IsIdentity(const Combined<AnfExpr>& combined) const {
    if (!combined.template Has<Call<AnfExpr>>()) return false;
    const auto& call = combined.template Get<Call<AnfExpr>>();
    if (call->args.size() != 1) return false;
    if (!call->func.template Has<tVar<std::string>>()) return false;
    const auto& func_name =
        call->func.template Get<tVar<std::string>>().value();
    return func_name == axpr::kBuiltinIdentity();
  }
  ...
  adt::Result<std::string> CombinedToStringImpl(
    ...
    if (func_name == axpr::kBuiltinIdentity()) {
      ADT_CHECK(call->args.size() == 1);
      ADT_LET_CONST_REF(arg_str, AtomicToString(ctx, call->args.at(0)));
      ss << arg_str;
    } else {
      ss << func_name << "(";
      int i = 0;
...
adt::Result<std::string> AnfExprHelper::FunctionToString(
    const Lambda<AnfExpr>& lambda) const {
  return AnfExprHelperImpl{}.FunctionToString(lambda);
}

adt::Result<std::string> AnfExprHelper::FunctionToString(
    const std::string& func_name, const Lambda<AnfExpr>& lambda) const {
  return AnfExprHelperImpl{}.FunctionToString(func_name, lambda);
}
