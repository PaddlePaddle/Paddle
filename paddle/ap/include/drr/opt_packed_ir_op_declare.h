...
struct OptPackedIrOpDeclareImpl {
  ...
  adt::Result<T*> cast_data() const {
    ...
    ADT_CHECK(data.has_value()) << adt::errors::ValueError{
        std::string() + "((OptPackedIrOpDeclareImpl*)" + ThisToString() +
        ")->data is nullopt"};
...
template <typename NodeT>
struct Type<drr::OptPackedIrOpDeclare<NodeT>> : public std::monostate {
  using std::monostate::monostate;
  const char* Name() const { return "OptPackedIrOpDeclare"; }

  static axpr::TypeImpl<axpr::BuiltinClassInstance<axpr::Value>> GetClass() {
    return GetOptPackedIrOpDeclareClass();
  }
};
