...
#include "paddle/ap/include/code_gen/in_tensor_data_size_kernel_arg_id.h"
...
struct InTensorDataSizeKernelArgIdMethodClass {
  using This = InTensorDataSizeKernelArgIdMethodClass;
  using Self = InTensorDataSizeKernelArgId<BirNode>;
  ...
  static adt::Result<ValueT> ToString(const ValueT& self_val,
    ...
    ss << "<InTensorDataSizeKernelArgId object at " << ptr << ">";
    return ss.str();
  }
  ...
  static adt::Result<ValueT> GetAttr(const ValueT& self_val,
    ...
    return adt::errors::AttributeError{
        std::string() +
        "'InTensorDataSizeKernelArgId' instance has no attribute '" + attr_name +
        "'."};
  }
...
GetInTensorDataSizeKernelArgIdClass() {
  using ImplMethods = InTensorDataSizeKernelArgIdMethodClass<ValueT, BirNode>;
  static auto cls(axpr::MakeBuiltinClass<ValueT>(
      "InTensorDataSizeKernelArgId", [&](const auto& Define) {
