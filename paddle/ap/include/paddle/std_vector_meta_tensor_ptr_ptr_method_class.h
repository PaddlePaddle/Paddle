...
struct StdVectorMetaTensorPtrPtrMethodClass {
  using This = StdVectorMetaTensorPtrPtrMethodClass;
  using Self = std::vector<MetaTensorPtr>*;
  ...
  static adt::Result<axpr::Value> ToString(
    ...
    const void* ptr = self;
    ss << "<StdVectorMetaTensorPtrPtr object at " << ptr << ">";
    return ss.str();
  }
  ...
  static adt::Result<axpr::Value> CastItem(const MetaTensorPtr& elem) {
    return GetMetaTensorPtrClass().New(elem);
  }
};
