...
class Base {
 public:
  tcmpt::TypeInfo<Base<T>> type_info() const { return type_info_; }

 private:
  template <typename T1, typename T2>
  friend class tcmpt::TypeInfoTraits;
  tcmpt::TypeInfo<Base<T>> type_info_{tcmpt::TypeInfo<Base<T>>::kUnknownType};
};

template <typename T>
class DerivedA : public Base<T>,
                 public tcmpt::TypeInfoTraits<Base<T>, DerivedA<T>> {
 public:
...
class DerivedB : public Base<T>,
                 public tcmpt::TypeInfoTraits<Base<T>, DerivedB<T>> {
 public:
