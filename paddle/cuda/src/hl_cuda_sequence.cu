...
__global__ void KeMatrixAddRows(real* output,
  ...
  int idy = hipThreadIdx_y;
  int sampleId = hipBlockIdx_x + idy * gridDimX;
...
__global__ void KeSequence2Batch(real* batch,
  ...
  int idx = hipThreadIdx_x;
  int idy = hipThreadIdx_y;
  int id = hipBlockIdx_x + idy * gridDimX;
  while (id < batchCount) {
...
__global__ void KeSequence2BatchPadding(real* batch,
  ...
  if (sequenceIdx < sequenceLength) {
    if (seq2batch) {
      /* sequence -> batch */
      for (int i = hipThreadIdx_x; i < sequenceWidth; i += hipBlockDim_x) {
