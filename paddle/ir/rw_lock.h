...
struct RWLock {
  ...
  inline void RDLock() {
    int state = pthread_rwlock_rdlock(&lock_);
    if (state != 0) {
      throw("The pthread failed to acquire read lock.");
    }
  }

  inline void WRLock() {
    int state = pthread_rwlock_wrlock(&lock_);
    if (state != 0) {
      throw("The pthread failed to acquire write lock.");
    }
  }

  inline void UNLock() {
    int state = pthread_rwlock_unlock(&lock_);
    if (state != 0) {
      throw("The pthread failed to unlock.");
    }
  }
