...
void FoldBroadcastToConstantPass(DrrPassContext* ctx) {
  ...
  const auto& fill_constant =
      pat.Op("fill_constant",
  ...
  // broadcast_to(fill_constant())，注意shape属性已更新 所有 ret 参数均在Source
  // Pattern中使用，对 ret 的赋值等同于对 ret 的 producer op的删除和重连接
