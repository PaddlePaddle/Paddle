...
void DyScheduleImpl::SimpleComputeAt(const Expr& block, const Expr& loop) {
  ...
  for (int i = 0; i < loops.size(); ++i) {
    ...
    PADDLE_ENFORCE_EQ(prove_eq.has_value() && prove_eq.value(),
                      true,
                      ::common::errors::InvalidArgument(
...
void DyScheduleImpl::ComputeInline(const Expr& schedule_block) {
  ...
  PADDLE_ENFORCE_EQ(
      ...
      true,
      ::common::errors::InvalidArgument(
          "Expr param(schedule_block) must be a ScheduleBlockRealize!"));
  ...
  PADDLE_ENFORCE_EQ(inliner.BodyPatternAllowInline(),
                    true,
                    ::common::errors::InvalidArgument(
                        "Current IR can't meets the requirements of "
                        "ComputeInline!"));
