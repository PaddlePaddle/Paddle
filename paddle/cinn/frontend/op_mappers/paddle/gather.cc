...
void GatherOpMapper(const paddle::cpp::OpDesc& op_desc,
  ...
  PADDLE_ENFORCE_EQ(
      ...
      1UL,
      phi::errors::InvalidArgument("The input of gather op should be one."));
  auto x_name = op_desc.Input("X").front();
  PADDLE_ENFORCE_EQ(op_desc.Input("Index").size(),
                    1UL,
                    phi::errors::InvalidArgument(
                        "The input index of gather op should be one."));
  auto index_name = op_desc.Input("Index").front();
  PADDLE_ENFORCE_EQ(
      op_desc.Output("Out").size(),
      1UL,
      phi::errors::InvalidArgument("The output of gather op should be one."));
