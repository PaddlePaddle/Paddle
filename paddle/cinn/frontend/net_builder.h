...
#undef NETBUILDER_REDUCE_OP_DECL
  ...
  /**
   * @brief Create constant tensor with the specific value/vector and type, the
  ...
  Variable FillConstant(const cinn::utils::ShapeType& shape,
                        ...
                        const std::string& name = "",
                        bool force_cpu = false) {
    return FillConstant(shape,
                        static_cast<float>(value),
                        name,
                        common::Type2Str(common::type_of<T>()),
                        force_cpu);
  }
   ...
   * int. Default is {1, 1}.
   * @param paddings he padding size. If padding is a list/tuple, it must
