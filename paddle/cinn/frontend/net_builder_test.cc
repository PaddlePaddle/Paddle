...
TEST(net_build, program_execute_scatter) {
  ...
  Variable output =
      builder.Scatter(input1, input2, {B, H_OUT}, default_value, 1);
  auto program = builder.Build();
...
TEST(net_build, program_execute_scatter_nd) {
  ...
  Variable output =
      builder.ScatterNd(input1, input2, {B, H_OUT}, default_value, {1});
  auto program = builder.Build();
...
TEST(net_build, program_execute_flip) {
    ...
    for (int h = 0; h < H; h++) {
      ...
      for (int w = 0; w < W; w++) {
        ...
        line += (std::to_string(index) + ": " +
                 std::to_string(input_data[index]) + ", ");
      }
      VLOG(6) << line;
    }
  }
  ...
  for (int c = 0; c < C; c++) {
    int flip_c =
        std::find(axes.begin(), axes.end(), 0) == axes.end() ? c : C - c - 1;
    for (int h = 0; h < H; h++) {
      std::string line;
      int flip_h =
          std::find(axes.begin(), axes.end(), 1) == axes.end() ? h : H - h - 1;
      for (int w = 0; w < W; w++) {
        int flip_w = std::find(axes.begin(), axes.end(), 2) == axes.end()
                         ? w
                         : W - w - 1;
