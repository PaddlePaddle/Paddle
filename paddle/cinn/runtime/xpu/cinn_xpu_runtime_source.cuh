...
/**
 * \file Device-side intrinsics for XPU M100 (Houyi/XCN, CUDA 12.8-compatible)
 * kernels generated by CodeGenXpuDevice.
 *
 * Headers used:
 *   - xpu/kernel/xtdk.h   : XTDK kernel base (threadIdx, blockIdx, __global__,
 *                            __shared__, __syncthreads, warp shuffle, etc.)
 *   - cuda_runtime_api.h  : CUDA-compatible host/device type declarations
 *                           (provided by XRE xcuda runtime library)
 *
 * The bfloat16 type used here (cinn::common::bfloat16) relies on
 * __nv_bfloat16 which is available via cuda_bf16.h from the XRE.
 * The float16 type (cinn::common::float16) similarly uses __half from
 * cuda_fp16.h in the XRE.
 *
 * All PTX inline assembly (rcp.approx, tanh.approx) is valid on M100
 * since the hardware is CUDA 12.8-compatible.
 *
 * Wavefront/warp size on M100 is 32 (__XCN_WAVEFRONT_SIZE = 32), matching
 * NVIDIA CUDA warp semantics used in the Welford reduction macros below.
 */
#pragma once
