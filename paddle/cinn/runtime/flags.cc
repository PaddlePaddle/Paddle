...
unsigned long long RandomSeed::seed_ = 0ULL;  // NOLINT

unsigned long long RandomSeed::GetOrSet(unsigned long long seed) {  // NOLINT
  if (seed != 0ULL) {
    seed_ = seed;
  }
  return seed_;
...
unsigned long long RandomSeed::Clear() {  // NOLINT
  auto old_seed = seed_;
  seed_ = 0ULL;
  return old_seed;
}
