...
/**
 * \file This file contains all the intrinsics available to be used in SYCL code generated by CodeGen.
 */
...
inline int cinn_sycl_find_int(const int *buf, int size, int num) { __cinn_sycl_find_kernel(buf, size, num, 0, 1); }

inline int cinn_sycl_find_float(const float *buf, int size, float num) {
  __cinn_sycl_find_kernel(buf, size, num, 0, 1);
}

inline int cinn_sycl_find_int_nd(const int *buf, int size, int num, int begin, int stride) {
  __cinn_sycl_find_kernel(buf, size, num, begin, stride);
}

inline int cinn_sycl_find_float_nd(const float *buf, int size, float num, int begin, int stride) {
  __cinn_sycl_find_kernel(buf, size, num, begin, stride);
}

#undef __cinn_sycl_find_kernel

inline int cinn_sycl_next_smallest_int32(int *buf, int size, int num, int begin, int stride) {
  int id = -1;
...
inline int cinn_sycl_find_int_from(const int *buf, int size, int num, int begin) {
  __cinn_sycl_find_from_kernel(buf, size, num, begin);
}

inline int cinn_sycl_find_float_from(const float *buf, int size, float num, int begin) {
  __cinn_sycl_find_from_kernel(buf, size, num, begin);
}
...
#define CINN_NVGPU_LT_NUM(TYPE_SUFFIX, TYPE)                                                 \
  inline int cinn_sycl_lt_num_##TYPE_SUFFIX(                                                \
...
#define CINN_NVGPU_GT_NUM(TYPE_SUFFIX, TYPE)                                                 \
  inline int cinn_sycl_gt_num_##TYPE_SUFFIX(                                                \
      const TYPE *buf, const int size, const TYPE num, const int offset, const int stride) { \
    int out = 0;                                                                             \
    for (int i = (size - 1) * stride + offset; i >= offset; i -= stride) {                   \
      if (buf[i] > num) out++;                                                               \
