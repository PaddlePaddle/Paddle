...
// This file is a WITH_XPU_CADA-only variant of float16.h. It exists
// because xtrans/clang, unlike nvcc, applies strict C++ overload
// resolution: cinn::common::float16's non-explicit `operator float()`
// together with a non-explicit `operator double()` makes any call to a
// function overloaded on both float and double (xtrans's own
// __shfl_down/__shfl_xor, its device <cmath> max()/min(), etc.)
// ambiguous, since both implicit conversions are equally viable. Real
// __half only converts to float, so nvcc never hits this. Rather than
// changing the shared float16.h (which every CUDA/HIP/SYCL backend
// includes, and which some existing code may rely on for the implicit
// float16->double conversion), this variant makes every conversion
// explicit and is only ever selected for WITH_XPU_CADA builds (see
// paddle/cinn/backends/nvrtc/header_generator.cc).

#ifndef CINN_COMMON_FLOAT16_XPU_CADA_H
#define CINN_COMMON_FLOAT16_XPU_CADA_H
...
// Conversion operators
  ...
  // Unlike float16.h, this conversion is explicit: see the file-level
  // comment for why xtrans/clang needs it.
  ...
  // Made explicit (unlike float16.h) so it never competes with
  // `operator float()` in overload resolution.
...
#endif  // CINN_COMMON_FLOAT16_XPU_CADA_H
