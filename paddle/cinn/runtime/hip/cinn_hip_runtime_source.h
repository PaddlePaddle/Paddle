...
/**
 * \file This file contains all the intrinsics available to be used in HIP code generated by CodeGen.
 */
...
// /opt/dtk-23.04/hip/include/hip/amd_detail/amd_hip_runtime.h miss int8 and int16
...
#define EXPAND_REDUCE_INT64_MARCO(MARCO, ...)                          \
  ...
  MARCO(max_int64, std::numeric_limits<long long int>::min(), long long int, ##__VA_ARGS__) \
  MARCO(min_int64, std::numeric_limits<long long int>::max(), long long int, ##__VA_ARGS__)
...
__device__ inline int cinn_hip_find_float(const float *buf, int size, float num) {
  __cinn_hip_find_kernel(buf, size, num, 0, 1);
}

__device__ inline int cinn_hip_find_int_nd(const int *buf, int size, int num, int begin, int stride) {
  __cinn_hip_find_kernel(buf, size, num, begin, stride);
}

__device__ inline int cinn_hip_find_float_nd(const float *buf, int size, float num, int begin, int stride) {
  __cinn_hip_find_kernel(buf, size, num, begin, stride);
}

#undef __cinn_hip_find_kernel

__device__ inline int cinn_hip_next_smallest_int32(int *buf, int size, int num, int begin, int stride) {
  int id = -1;
...
__device__ inline int cinn_hip_find_int_from(const int *buf, int size, int num, int begin) {
  __cinn_hip_find_from_kernel(buf, size, num, begin);
}

__device__ inline int cinn_hip_find_float_from(const float *buf, int size, float num, int begin) {
  __cinn_hip_find_from_kernel(buf, size, num, begin);
}
...
#define CINN_HIP_LT_NUM(TYPE_SUFFIX, TYPE)                                                  \
  __device__ inline int cinn_hip_lt_num_##TYPE_SUFFIX(                                      \
...
CINN_HIP_LT_NUM(int64, long long int)
#ifdef CINN_HIP_FP16
...
#define CINN_HIP_GT_NUM(TYPE_SUFFIX, TYPE)                                                 \
  __device__ inline int cinn_hip_gt_num_##TYPE_SUFFIX(                                     \
      const TYPE *buf, const int size, const TYPE num, const int offset, const int stride) { \
    int out = 0;                                                                             \
    for (int i = (size - 1) * stride + offset; i >= offset; i -= stride) {                   \
      if (buf[i] > num) out++;                                                               \
...
CINN_HIP_GT_NUM(int64, long long int)
#ifdef CINN_HIP_FP16
...
#define CINN_HIP_INDEX_ADD(TYPE_SUFFIX, TYPE)                               \
  __device__ inline TYPE cinn_hip_index_add_##TYPE_SUFFIX(const TYPE x,     \
