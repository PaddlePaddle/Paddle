...
std::vector<ir::Expr> CustomCallArgsForCudnnPoolForward(
  ...
  bool global_pooling =
      attr_store.count("global_pooling")
          ? absl::get<bool>(attrs.attr_store.at("global_pooling"))
          : false;
  if (global_pooling) {
    if (data_format == "NCHW") {
  ...
  bool adaptive = attr_store.count("adaptive")
                      ? absl::get<bool>(attrs.attr_store.at("adaptive"))
                      : false;
  if (adaptive) {
    CHECK(kernel[0] == 1 && kernel[1] == 1)
        << "cudnn pool2d in cinn only support kernel_size=[1, 1] now, you can "
           "set "
           "FLAGS_cinn_custom_call_deny_ops=\"pool2d\" to disallow pool2d "
           "using cudnn";
    CHECK(stride[0] == 1 && stride[1] == 1)
        << "cudnn pool2d in cinn only support stride_size=[1, 1] now, you can "
           "set "
           "FLAGS_cinn_custom_call_deny_ops=\"pool2d\" to disallow pool2d "
           "using cudnn";
    CHECK(padding[0] == 0 && padding[1] == 0)
        << "cudnn pool2d in cinn only support padding_size=[0, 0] now, you can "
           "set "
           "FLAGS_cinn_custom_call_deny_ops=\"pool2d\" to disallow pool2d "
           "using cudnn";
  ...
  cudnnPoolingMode_t mode = pool_type == "max"
                                ? CUDNN_POOLING_MAX
...
std::vector<ir::Expr> CustomCallArgsForCudnnPoolBackward(
  ...
  if (adaptive) {
    CHECK(kernel[0] == 1 && kernel[1] == 1)
        << "cudnn pool2d_grad in cinn only support kernel_size=[1, 1] now, you "
           "can set "
           "FLAGS_cinn_custom_call_deny_ops=\"pool2d_grad\" to disallow "
           "pool2d_grad using cudnn";
    CHECK(stride[0] == 1 && stride[1] == 1)
        << "cudnn pool2d_grad in cinn only support stride_size=[1, 1] now, you "
           "can set "
           "FLAGS_cinn_custom_call_deny_ops=\"pool2d_grad\" to disallow "
           "pool2d_grad using cudnn";
    CHECK(padding[0] == 0 && padding[1] == 0)
        << "cudnn pool2d_grad in cinn only support padding_size=[0, 0] now, "
           "you can set "
           "FLAGS_cinn_custom_call_deny_ops=\"pool2d_grad\" to disallow "
           "pool2d_grad using cudnn";
