...
std::vector<std::vector<int>> InferShapeForGather(
  ...
  CHECK(axis >= 0 && axis < static_cast<int>(inputs_shape[0].size()))
      << "The attribute `axis` in Gather should be >= 0 and < the size of the "
         "first input shape! Please check "
         "again.";

  std::vector<int> output_shape = inputs_shape[0];
  CHECK_EQ(inputs_shape[1].size(), 1U) << "The index should be a 1-D Tensor.";
  CHECK_GT(inputs_shape[1][0], 0)
      << "The length of the index should be greater than 0.";
  output_shape[axis] = inputs_shape[1][0];
  VLOG(4) << "The output calculated in InferShapeForGather: "
          << utils::Join(output_shape, ", ");
