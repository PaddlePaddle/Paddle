...
class FlattenOpPattern
 ...
 public:
  ...
  void Rewrite(paddle::dialect::FlattenOp op,
    ...
    int end_axis = op.attribute("stop_axis").dyn_cast<::pir::Int32Attribute>().data();
    ...
    if( is_dyshape )
    {
      ...
      auto x_rank = op->operand_source(0)
                          .type()
                          .dyn_cast<paddle::dialect::DenseTensorType>().dims().size();
      auto x_shape =   rewriter.Build<paddle::dialect::ShapeOp>( op->operand_source(0)).result(0);                        
      for (size_t i = 0; i < x_rank;) {
        if (i == static_cast<size_t>(start_axis)) {          
          auto new_single_dim = rewriter.Build<cinn::dialect::SliceOp>(x_shape,
          ...
          for (auto t = start_axis + 1; t <= end_axis; ++t) {            
            auto dim_t = rewriter.Build<cinn::dialect::SliceOp>(x_shape,
                                                 std::vector<int64_t>({0}),
                                                 std::vector<int64_t>({t}),
                                                 std::vector<int64_t>({t+1}),
                                                 std::vector<int64_t>({}),
                                                 std::vector<int64_t>({})).result(0);
            new_single_dim = rewriter.Build<paddle::dialect::MultiplyOp>( new_single_dim, dim_t).result(0);
          }
          out_shape.push_back(new_single_dim);
          i = end_axis + 1;
        } else {          

          auto t = rewriter.Build<cinn::dialect::SliceOp>(x_shape,
                                                 std::vector<int64_t>({0}),
                                                 std::vector<int64_t>({i}),
                                                 std::vector<int64_t>({i+1}),
                                                 std::vector<int64_t>({}),
       ...
       auto new_shape = rewriter.Build<cinn::dialect::ConcatOp>( out_shape, -1).result(0);

       auto reshape_op = rewriter.Build<paddle::dialect::ReshapeOp>( op->operand_source(0), new_shape );
