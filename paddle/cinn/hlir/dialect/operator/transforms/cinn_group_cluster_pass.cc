...
struct GroupClusterNode {
  ...
  void MergeNode(const GroupClusterNode& node,
    ...
    if (sch_node.type != "") {
      ...
      for (auto it = alignment_schedule_info.begin();
           it != alignment_schedule_info.end();
           ++it) {
...
::pir::Operation* ReplaceWithGroupOp(
  ...
  auto new_group_op = builder.Build<cinn::dialect::GroupOp>(
      output_types, alignment_schedule_info);
...
std::vector<GroupClusterNode> GroupSplit(::pir::Operation* input_op) {
  ...
  for (auto* op : op_list) {
    ...
    for (size_t i = 0; i < op->num_operands(); ++i) {
      ...
      op_list.insert(op_list.end(),
                     op_path[pre_op].ops.begin(),
                     op_path[pre_op].ops.end());

      cluster_node.alignment_schedule_info =
          op_path[pre_op].alignment_schedule_info;
  ...
  for (auto& node : first_stage_output) {
    ...
    all_needed_values.insert(node_outside_input.begin(),
  ...
  for (size_t i = 0; i < first_stage_output.size(); ++i) {
    ...
    for (auto out_val : node.output_value) out_value_to_node_id[out_val] = i;
  }
    ...
    for (auto it = block.begin(); it != block.end();) {
      ...
      if (base_it->isa<cinn::dialect::GroupOp>()) {
        ...
        for (auto& node : split_res) {
          ...
          insert_point = ReplaceWithGroupOp(&block,
                                            tmp_ops,
                                            node.output_value,
                                            node.alignment_schedule_info,
                                            insert_point,
                                            &ir_mapping);
          ...
          for (size_t i = 0; i < yield_op->num_operands(); ++i) {
            if (local_outs.count(yield_op->operand_source(i))) {
              base_it->result(i).ReplaceAllUsesWith(
                  insert_point->result(local_index++));
            }
          }
        }
