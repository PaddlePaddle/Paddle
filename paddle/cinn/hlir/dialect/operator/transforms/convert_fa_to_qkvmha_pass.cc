...
class ConvertFA2QKVMHAPattern : public pir::OpRewritePattern<
                                 paddle::dialect::FlashAttnOp> {
 public:
  using pir::OpRewritePattern<
      paddle::dialect::FlashAttnOp>::OpRewritePattern;
  ...
  bool Match(paddle::dialect::FlashAttnOp op) const override {
    ...
    auto in_shape = shape_analysis.GetShapeOrDataForValue(op->operand_source(0));

    auto dim1 = in_shape.shape()[1];

    if( dim1.isa<int64_t>() && ( dim1.dyn_cast<int64_t>() == static_cast<int64_t>(1) ) )
    {
      return true;
    }
  ...
  void Rewrite(paddle::dialect::FlashAttnOp op,
    ...
    pir::Value attn_mask;
    auto fa = rewriter.Build<paddle::dialect::QkvMhaOp>(
        q, k, v, attn_mask);
