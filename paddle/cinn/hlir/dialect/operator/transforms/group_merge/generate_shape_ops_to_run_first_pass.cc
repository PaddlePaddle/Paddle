  ...
  bool MatchAndRewrite(cinn::dialect::GroupOp group_op,
    ...
    ShapeOrDataDimExprsAccessor dim_exprs_accessor{
      .GetShapeOrDataDimExprs=[=](pir::Value value) -> const symbol::ShapeOrDataDimExprs& {
        return shape_analysis_->GetShapeOrDataForValue(&value);
      },
      .SetShapeOrDataDimExprs=[=](pir::Value value, const symbol::ShapeOrDataDimExprs& dim_exprs) {
        shape_analysis_->SetShapeOrDataForValue(&value, dim_exprs);
      }
    };
...
class GenerateShapeOpsToRunFirstPass : public pir::PatternRewritePass {
 public:
  GenerateShapeOpsToRunFirstPass(
      const std::shared_ptr<pir::ShapeConstraintIRAnalysis>& shape_analysis)
  ...
  pir::RewritePatternSet InitializePatterns(pir::IrContext* context) override {
    ...
    ps.Add<GroupOpGenerateShapeOpsPattern>(context, shape_analysis_);
    return ps;
  }
...
std::unique_ptr<::pir::Pass> CreateGenerateShapeOpsToRunFirstPass(
    const std::shared_ptr<pir::ShapeConstraintIRAnalysis>& shape_analysis) {
  return std::make_unique<GenerateShapeOpsToRunFirstPass>(shape_analysis);
}
