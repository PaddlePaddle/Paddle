...
//                           "constraint after split by '==' should be 2, but
//                           now " "is %d, original constrain is '%s'",
//                           expr_pair.size(),
//                           constraint));
...
//   // if the priority is same, we compare the string value to find the
//   smallest
//   // one
...
//       if
//       (!std::holds_alternative<symbol::Equal<symbol::DimExpr>>(constraint)) {
//         VLOG(0) << "The DimExprConstraint type is no Equal<DimExpr>, this
//         part "
//                    "is to be completed.";
//         return;
//       }
...
//             SubstituteShapeOrData(origin_shape_or_data,
//             substitution_pattern);
//         VLOG(8) << op->name()
