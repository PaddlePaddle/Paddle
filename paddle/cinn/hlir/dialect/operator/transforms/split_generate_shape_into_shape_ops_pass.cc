...
using TensorDim4SymbolNameT = std::function<std::optional<TensorDim>(const std::string& symbol_name)>;
...
struct CachedDimExprToValueConverter {
  ...
  // TODO(): Refactor to cached version if std::hash<symbol::DimExpr>() is ready.
  // std::unordered_map<symbol::DimExpr, pir::Value> symbol_names2cached_value_;

  pir::Value ConvertToValue(const symbol::DimExpr& dim_expr) {
    // TODO():  cache the returned value if std::hash<symbol::DimExpr>() is ready
    return std::visit([&](const auto& impl) {
      return ConvertToValueImpl(impl);
    }, dim_expr.variant());
  }
  ...
  pir::Value GetInputShapeByInputTensor(pir::Value input_tensor) {
    ...
    if (iter == tensor2shape_.end()) {
      pir::Value input_shape = rewriter->Build<paddle::dialect::ShapeOp>(input_tensor).out();
 ...
 private:
   std::unordered_map<pir::Value/*input tensor*/, pir::Value/*input shape tensor*/> tensor2shape_;

  pir::Value ConvertToValueImpl(int64_t dim_expr) {
    return rewriter->Build<paddle::dialect::FullIntArrayOp>(std::vector{dim_expr}, phi::DataType::INT64).out();
  }
  ...
  pir::Value ConvertToValueImpl(const std::string& symbol_name) {
    ...
    PADDLE_ENFORCE(
      ...
      phi::errors::PreconditionNotMet(
          "symbol [%s] are not bound to any input of generate_shape op", symbol_name));
    return std::visit([&](const auto& impl) {
      return ConvertTensorDimToValue(impl);
    }, tensor_dim.value());
  }
  ...
  pir::Value ConvertTensorDimToValue(const TensorDimInShape& tensor_dim) {
    ...
    return ConvertTensorDimToValue(TensorDimInData{
      .value=input_shape,
      .axis=tensor_dim.axis
    });
  }

  pir::Value ConvertTensorDimToValue(const TensorDimInData& tensor_dim) {
    return rewriter->Build<paddle::dialect::SliceOp>(
      tensor_dim.value, std::vector{0LL}, std::vector{tensor_dim.axis},
      std::vector{tensor_dim.axis+1}, std::vector<int64_t>{},
      std::vector<int64_t>{}).out();
  }

  pir::Value ConvertToValueImpl(const symbol::Negative<symbol::DimExpr>& dim_expr) {
    LOG(FATAL) << "Dead code. This logical should handled by ConvertToValueImpl(symbol::Add<symbol::DimExpr>)";
  }

  pir::Value ConvertToValueImpl(const symbol::Reciprocal<symbol::DimExpr>& dim_expr) {
    LOG(FATAL) << "Dead code. This logical should handled by ConvertToValueImpl(symbol::Mul<symbol::DimExpr>)";
  }
  ...
  pir::Value ConvertToValueImpl(const symbol::Add<symbol::DimExpr>& dim_expr) {
    ...
    for (int i = 1; i < operands->size(); ++i) {
      if (operands->at(i).isa<symbol::Negative<symbol::DimExpr>>()) {
        const auto& [operand] = *operands->at(i).dyn_cast<symbol::Negative<symbol::DimExpr>>();
        pir::Value operand_value = ConvertToValue(operand);
        acc = rewriter->Build<paddle::dialect::SubtractOp>(acc, operand_value).out();
      } else {
  ...
  pir::Value ConvertToValueImpl(const symbol::Mul<symbol::DimExpr>& dim_expr) {
    ...
    for (int i = 1; i < operands->size(); ++i) {
      if (operands->at(i).isa<symbol::Reciprocal<symbol::DimExpr>>()) {
        const auto& [operand] = *operands->at(i).dyn_cast<symbol::Negative<symbol::DimExpr>>();
        pir::Value operand_value = ConvertToValue(operand);
        prod = rewriter->Build<paddle::dialect::DivideOp>(prod, operand_value).out();
      } else {
        pir::Value operand_value = ConvertToValue(operands->at(i));
        prod = rewriter->Build<paddle::dialect::MultiplyOp>(prod, operand_value).out();
      }
    }
    return prod;
  }
  ...
  pir::Value ConvertToValueImpl(const symbol::Broadcast<symbol::DimExpr>& dim_expr) {
    const auto& [operands] = dim_expr;
    CHECK_GT(operands->size(), 0);
    pir::Value broadcasted = ConvertToValue(operands->at(0));
    for (int i = 1; i < operands->size(); ++i) {
      pir::Value operand_value = ConvertToValue(operands->at(i));
      broadcasted = rewriter->Build<paddle::dialect::ShapeBroadcastOp>(broadcasted, operand_value).out();
    }
    return broadcasted;
  }
...
class SplitGenerateShapeIntoShapeOps : public pir::OpRewritePattern<cinn::dialect::GenerateShapeOp> {
 public:
  ...
  bool MatchAndRewrite(cinn::dialect::GenerateShapeOp op,
    ...
    std::optional<pir::Value> out_replacement = GetOutReplacement(op, &rewriter);
  ...
  TensorDim4SymbolNameT MakeGetterTensorDim4SymbolName(cinn::dialect::GenerateShapeOp op) const {
    ...
    PADDLE_ENFORCE(
      (iter != attr_map.end()),
      phi::errors::PreconditionNotMet(
          "attr symbol_bindings MUST in attribute map for [%s] op", op->name()));
    pir::Attribute attr = iter->second;
    auto* Convert = &cinn::dialect::GenerateShapeOp::ConvertAttributeToSymbolBindings;
    const auto& symbol_bindings = Convert(attr);
    PADDLE_ENFORCE(
      symbol_bindings.has_value(),
      phi::errors::PreconditionNotMet(
          "attr symbol_bindings in op [%s] can not be converted to symbol bindings", op->name()));
    for (const auto& symbol_binding : symbol_bindings.value()) {
      InsertSymbolBinding(op, symbol_binding, &symbol_name2tenso_dim);
    }
    return [map = std::move(symbol_name2tenso_dim)](const std::string& symbol_name) {
  ...
  std::vector<symbol::DimExpr> GetOutDimExprs(cinn::dialect::GenerateShapeOp op) const {
    const auto& attr_map = op->attributes();
    const auto& iter = attr_map.find("output_dim_exprs");
    PADDLE_ENFORCE(
      (iter != attr_map.end()),
      phi::errors::PreconditionNotMet(
          "attr output_dim_exprs MUST in attribute map for [%s] op", op->name()));
    pir::Attribute output_dim_exprs_attr = iter->second;
    PADDLE_ENFORCE(output_dim_exprs_attr.isa<pir::ArrayAttribute>(),
      phi::errors::PreconditionNotMet(
          "attr output_dim_exprs for [%s] op must be an pir::ArrayAttribute", op->name()));
    std::vector<symbol::DimExpr> ret{};
    const auto& output_dim_exprs = output_dim_exprs_attr.dyn_cast<pir::ArrayAttribute>();
  ...
  pir::Value GetValueOfRewritedOps(
    ...
    return converter->rewriter->Build<pir::CombineOp>(values_from_dim_exprs).out();
  }
