...
//         auto reducer = FindReducerInRoute(producer, ops_set,
//         GetConsumersInSet); if (reducer) {
...
//     if (!found && r_op) {
//       auto r_input_shape =
//       CompatibleInfo::ValueShape(r_op->operand_source(0)); if
//       (CompatibleInfo::ShapeProduct(output_shape) ==
...
//     }
//   }
//   // Establish virtual consumer relationships between output nodes with the
//   same
//   // shape. This allows the calculation of output nodes without affiliation
//   to
...
//   std::priority_queue<OpWithPriority, std::vector<OpWithPriority>,
//   Comparator>
//       priority_candidates;
...
//     // Some nodes may have multiple edges between them, resulting in
//     duplicates
...
//         --degree_map[op];
//         // Ensure that each node is accessed only once and maintain
//         topological
//         // order.
...
//     }
//     MergeLoops(
//         ir_sch.GetModule().GetExprs().at(0), node_loops, master_loops,
//         index);
