...
std::vector<std::vector<int>> GetMulNewShapes(
  ...
  PADDLE_ENFORCE_EQ(
      inputs_shape.size(),
      2UL,
      common::errors::InvalidArgument("The mul should only have two inputs."));
  const auto &x_shape = inputs_shape[0], &y_shape = inputs_shape[1];
  ...
  auto flatten_shape = [&](const std::vector<int>& shape, int num_col_dims) {
    ...
    PADDLE_ENFORCE_GT(
        num_col_dims,
        0,
        common::errors::InvalidArgument("The [num_col_dims] should not be 0 in "
                                        "mul op. Please check."));
    PADDLE_ENFORCE_LT(
        num_col_dims,
        shape.size(),
...
ir::Tensor Gather(const ir::Tensor& x,
  ...
  PADDLE_ENFORCE_EQ(
      x->shape.size(),
      index->shape.size(),
      common::errors::InvalidArgument("The rank of x and index must be same."));
...
ir::Tensor ScatterAdd(const ir::Tensor& input,
  ...
  auto ScatterAddNvHygon = [&] {
    ...
    PADDLE_ENFORCE_EQ(
        input->type(),
        updates->type(),
        common::errors::InvalidArgument("The data types for input and updates "
