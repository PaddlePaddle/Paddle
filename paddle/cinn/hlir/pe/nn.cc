...
std::vector<ir::Tensor> Conv2d_NCHW(const ir::Tensor &input,
  ...
  PADDLE_ENFORCE_EQ(
      weights->shape.size(),
      4,
      common::errors::InvalidArgument("The dimension of weights should be 4."));
...
std::vector<ir::Tensor> Conv2d_NCHW_5D(const ir::Tensor &input,
  ...
  PADDLE_ENFORCE_EQ(
      tensors.size(),
      2U,
      common::errors::InvalidArgument("Conv2d_NCHWc should return 2 tensors"));
...
std::vector<Tensor> GlobalPool2d(const Tensor &tensor,
  ...
  if (pool_type == "max") {
    ...
    PADDLE_THROW(common::errors::InvalidArgument("unsupported pooling type."));
  }
  return {};
}
