...
void CodeGenC::Compile(const ir::LoweredFunc &function) {
  PADDLE_ENFORCE_EQ(
      function.defined(),
      true,
      platform::errors::InvalidArgument("The function is not defined."));
...
void CodeGenC::Visit(const ir::Load *op) {
  ...
  if (dense_strided_ramp.defined()) {  // Loading a continuous Ramp address.
    PADDLE_ENFORCE_EQ(op->type().is_vector(),
                      true,
                      platform::errors::InvalidArgument(
                          "The operation type is invalid. The operation type "
...
std::string ReadWholeFile(const std::string &path) {
  ...
  PADDLE_ENFORCE_EQ(file.is_open(),
                    true,
                    platform::errors::InvalidArgument("Failed to open file: %s",
                                                      path.c_str()));
  std::stringstream ss;
  ss << file.rdbuf();
  return ss.str();
