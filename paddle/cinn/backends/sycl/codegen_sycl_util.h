...
struct SYCLCollectHostFunctionVisitor : public ir::IRMutator<> {
  explicit SYCLCollectHostFunctionVisitor(const std::string& module_name)
 ...
 protected:
  ...
  Expr CreateHostFunctionGivenDeviceKernel(const ir::_LoweredFunc_* func) {
    ...
    auto call_extern_api =
        ir::Call::Make(Void(),
                       runtime::intrinsic::call_sycl_kernel,
                       {kernel_ptr,
                        kernel_args,
                        kernel_args_num,
