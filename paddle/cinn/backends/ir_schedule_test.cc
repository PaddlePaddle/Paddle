...
TEST(IrSchedule, dy_reorder1) {
  ...
  auto fused = ir_sch.Reorder("B", {1, 0});
  std::cout << "After Reorder IR is : " << ir_sch.GetModule().GetExprs()[0];
}

// TEST(IrSchedule, dy_fuse1) {
//   Context::Global().ResetNameId();
//   Expr M(32);
//   Expr N(32);
//   Expr P(32);
...
//   auto func = cinn::lang::LowerVec(
//       "test_dy_fuse1", stages, {A, B}, {}, {}, nullptr, target, true);
//   auto ast_expr = func[0]->body;
//   std::vector<Expr> vec_ast{ast_expr};
//   ir::ModuleExpr mod_expr(vec_ast);
//   ir::IRSchedule
//   ir_sch(mod_expr,-1,false,cinn::utils::ErrorMessageLevel::kGeneral,true);
//   auto fused = ir_sch.Fuse("B", {0, 1});
//   std::cout<<"After fused IR is : "<<ir_sch.GetModule().GetExprs()[0];
// }

// TEST(IrSchedule, split_and_bind) {
...
//   auto B = Compute(
//       {M}, [&](Var i) { return A(i); }, "B");

//   auto stages = CreateStages({A, B});

//   auto func = cinn::lang::LowerVec(
//       "test_split_and_bind", stages, {A, B}, {}, {}, nullptr, target, true);
//   auto ast_expr = func[0]->body;
//   std::vector<Expr> vec_ast{ast_expr};
//   ir::ModuleExpr mod_expr(vec_ast);
//   std::cout << "hello world" << __LINE__ << std::endl;
...
//   std::cout << "hello world" << __LINE__ << std::endl;
//   std::cout << "loops[0] is "<< loops[0] << "\n";
//   Var D(1024,"DDim");
//   Expr DD(D);
//   std::cout << "DD is "<< DD << "\n";
//   // loops[0].As<ir::For>()->extent = Expr(DD);
//   std::cout << "loops[0] is "<< loops[0] << "\n";
//   std::cout << "before Split IR is : "<< ir_sch.GetModule().GetExprs().at(0);
//   auto splited = ir_sch.Split(loops[0], {-1, 32, 32});
//   std::cout << "After Split IR is : "
//           << ir_sch.GetModule().GetExprs().at(0);
//   loops = ir_sch.GetLoops("B");
//   ir_sch.Bind(loops[1],"blockIdx.x");
//   ir_sch.Bind(loops[2],"threadIdx.x");
//   std::cout << "After Bind IR is :"
//           << ir_sch.GetModule().GetExprs().at(0);
// }

// TEST(IrSchedule, dy_compute_inline1) {
//   Context::Global().ResetNameId();
//   Expr M(32);
//   Expr N(32);
//   Expr P(32);
...
//   // std::cout << "After ComputeInline, IR is : "
//   //       << ir_sch.GetModule().GetExprs().at(0);
...
//   auto func = cinn::lang::LowerVec(
//       "test_simple_compute_at", stages, {A, C}, {}, {}, nullptr, target,
//       true);
