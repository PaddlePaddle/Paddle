...
void detail::CollectBucketStrategyHostFunctionVisitor::ProcessLoweredFunc(
  ...
  if (!temp_space_infer_shape_stmts.empty()) {
    ...
    if (temp_space_infer_shape_body_.defined()) {
      temp_space_infer_shape_body_ = ir::IfThenElse::Make(
          predicate, if_body, temp_space_infer_shape_body_);
    } else {
      temp_space_infer_shape_body_ = ir::IfThenElse::Make(predicate, if_body);
    }
  }
}
