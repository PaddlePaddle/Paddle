 ...
 */
class CodeGenHIP_Dev : public CodeGenC {
 public:
  ...
  //! Compile on hiprtc.
  std::string Compile(const ir::Module& module, bool for_hiprtc = true);
 ...
 protected:
  ...
  // Print element access at a hip built-in vector on a load/store node
 ...
 private:
  Target target_;
  bool for_hiprtc_{false};
  // names of vectorized tensors from `Let` statements where dtypes of the
  // tensors are customized_type with customized_type::khip_builtin_vector_t
  // prefix
