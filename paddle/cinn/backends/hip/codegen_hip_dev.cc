...
std::string CodeGenHIP_Dev::Compile(const ir::Module &module, bool for_hiprtc) {
  for_hiprtc_ = for_hiprtc;
...
void CodeGenHIP_Dev::Compile(const ir::LoweredFunc &func) {
  dyn_shared_mem_offset_ = Expr(-1);
  IrPrinter::Visit(Expr(func));
}

std::vector<Expr> CodeGenHIP_Dev::GenerateBufferAliasExprs(
...
void CodeGenHIP_Dev::Visit(const ir::_LoweredFunc_ *op) {
...
void CodeGenHIP_Dev::Visit(const ir::Free *op) {
  str_ += "delete [] ";
  str_ += op->destination.As<ir::_Buffer_>()->name;
  str_ += ";\n";
}

void CodeGenHIP_Dev::Visit(const ir::_Var_ *op) {
...
void CodeGenHIP_Dev::Visit(const ir::Min *op) {
  str_ += "min(";
...
void CodeGenHIP_Dev::Visit(const ir::Max *op) {
  str_ += "max(";
  IrPrinter::Visit(op->a());
  str_ += ", ";
  IrPrinter::Visit(op->b());
...
void CodeGenHIP_Dev::PrintFunctionDeclaration(const ir::_LoweredFunc_ *op) {
  str_ += "void ";
...
void CodeGenHIP_Dev::PrintFuncArg(const ir::Argument &arg) {
  if (arg.is_buffer()) {
    // In HIP kernel, only primitive type is supported, so we replace the
    // buffer with T*j
...
void CodeGenHIP_Dev::Visit(const ir::Call *op) {
  str_ += op->name;
  str_ += "(";
...
bool CodeGenHIP_Dev::PrintBuiltinVectorAccess(const ir::LoadStoreAddrMnger *op,
  ...
  // the index can't exceed the range of hip built-in vector type
...
void CodeGenHIP_Dev::Visit(const ir::Load *op) {
  // overload this visit function to especially deal with the case when it
  // accesses element at a hip built-in vector, others still resolve to
  // CodeGenC
...
void CodeGenHIP_Dev::Visit(const ir::Store *op) {
  // overload this visit function to especially deal with the case when it
  // accesses element at a hip built-in vector, others still resolve to
  // CodeGenC
