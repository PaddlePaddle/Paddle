  ...
  std::optional<DimExpr> SubstituteVariadic(const T& dim_expr) {
    ...
    for (const auto& operand : operands) {
      ...
      substituted_operands.push_back(
        substituted_operand.has_value() ? substituted_operand.value() : operand);
    }
...
symbol::DimExpr SubstituteDimExpr(
    ...
    const std::unordered_map<symbol::DimExpr, symbol::DimExpr>& pattern_to_replacement) {
  const auto& opt_replaced = SubstituteDimExprHelper(pattern_to_replacement).Substitute(dim_expr);
