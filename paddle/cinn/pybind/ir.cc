...
void BindIrVisitor(py::module *m) {
  py::class_<ir::IRVisitorBase<void>> ir_visitor(*m, "IRVisitor");
  ir_visitor.def(py::init<>())
      .def(
          "visit",
          py::overload_cast<const ir::Expr *>(&ir::IRVisitorBase<void>::Visit));
#define DEFINE_VISIT_FN(__ty) \
  ir_visitor.def(             \
      "visit",                \
      py::overload_cast<const ir::__ty *>(&ir::IRVisitorBase<void>::Visit));
