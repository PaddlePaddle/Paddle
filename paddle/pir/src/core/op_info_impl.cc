...
char* raw_base_ptr = base_ptr.get();  // 获取指向原始内存的指针
if (traits_num > 0) {
  std::vector<TypeId> trait_data(trait_set.begin(), trait_set.end());
  auto p_first_trait = reinterpret_cast<TypeId *>(raw_base_ptr);
  std::copy(trait_data.begin(), trait_data.end(), p_first_trait);  // 使用std::copy代替memcpy
