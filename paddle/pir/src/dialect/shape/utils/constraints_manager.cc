...
bool CanSubstituteInShapeAnalysis(const DimExpr& lhs, const DimExpr& rhs) {
  int lhs_priority = GetDimExprPriority(lhs);
  int rhs_priority = GetDimExprPriority(rhs);
  if (lhs.isa<Broadcast<DimExpr>>() && rhs_priority < 2) {
    return true;
  }
  if (lhs_priority >= 2 || rhs_priority >= 2) {
    return false;
...
void ConstraintsManager::AddEqCstr(const DimExpr& lhs, const DimExpr& rhs) {
  ...
  if (simplify_result.has_value()) {
    VLOG(0) << "#### SimpliyEqualCstr: " << lhs << " == " << rhs << " TO "
            << simplify_result->first << " == " << simplify_result->second;
    AddEqCstr(simplify_result->first, simplify_result->second);
  } else {
    equals_.Union(lhs, rhs);
    VLOG(0) << "### AddEqCstr the constraint: " << lhs << " == " << rhs;
    ...
    if (CanSubstituteInConstraint(origin, subsutituted)) {
      VLOG(0) << "#### SubstituteDimExprInConstraint" << origin << " -> "
              << subsutituted;
      SubstituteDimExprInConstraint(origin, subsutituted);
    }
    if (equal_callback_func_ &&
        CanSubstituteInShapeAnalysis(origin, subsutituted)) {
      VLOG(0) << "#### CanSubstituteInShapeAnalysis" << origin << " -> "
              << subsutituted;
...
std::optional<std::pair<DimExpr, DimExpr>> ConstraintsManager::SimpliyEqualCstr(
  ...
  if (lhs.isa<Add<DimExpr>>()) {
    ...
    rhs_list = rhs.Get<Add<DimExpr>>().operands;
  } else {
    lhs_list = lhs.Get<Mul<DimExpr>>().operands;
    rhs_list = rhs.Get<Mul<DimExpr>>().operands;
  }
  ...
  for (const auto& rhs_dim_expr : *rhs_list) {
    VLOG(0) << "#### rhs_dim_expr : " << rhs_dim_expr;
    if (lhs_hash.count(rhs_dim_expr)) {
      VLOG(0) << "#### lhs_hash.at(rhs_dim_expr) : "
              << lhs_hash.at(rhs_dim_expr);
      if (lhs_hash.at(rhs_dim_expr) >= 1) {
  ...
  for (const auto& lhs_dim_expr : *lhs_list) {
    VLOG(0) << "#### lhs_dim_expr : " << lhs_dim_expr
            << "  count : " << lhs_hash.at(lhs_dim_expr);
  ...
  if (lhs.isa<Add<DimExpr>>()) {
    lhs_diff =
        lhs_diffs->size() == 1 ? lhs_diffs->at(0) : Add<DimExpr>{lhs_diffs};
    rhs_diff =
        rhs_diffs->size() == 1 ? rhs_diffs->at(0) : Add<DimExpr>{rhs_diffs};
  }
  if (lhs.isa<Mul<DimExpr>>()) {
    lhs_diff =
        lhs_diffs->size() == 1 ? lhs_diffs->at(0) : Mul<DimExpr>{lhs_diffs};
    rhs_diff =
        rhs_diffs->size() == 1 ? rhs_diffs->at(0) : Mul<DimExpr>{rhs_diffs};
  }
