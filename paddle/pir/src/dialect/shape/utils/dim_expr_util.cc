...
class ContainsDimExprHelper final {
 public:
  bool Contains(const DimExpr& dim_expr, const DimExpr& target) {
    if (dim_expr == target) return true;
    return std::visit(
        [&](const auto& dim_expr) { return ContainsImpl(dim_expr, target); },
        dim_expr.variant());
  }

 private:
  bool ContainsImpl(const std::int64_t& dim_expr, const DimExpr& target) {
    return false;
  }
  bool ContainsImpl(const std::string& dim_expr, const DimExpr& target) {
    return false;
  }
  bool ContainsImpl(const Negative<DimExpr>& dim_expr, const DimExpr& target) {
    return UnaryContains(dim_expr, target);
  }
  bool ContainsImpl(const Add<DimExpr>& dim_expr, const DimExpr& target) {
    return VariadicContains(dim_expr, target);
  }
  bool ContainsImpl(const Mul<DimExpr>& dim_expr, const DimExpr& target) {
    return VariadicContains(dim_expr, target);
  }
  bool ContainsImpl(const Div<DimExpr>& dim_expr, const DimExpr& target) {
    return BinaryContains(dim_expr, target);
  }
  bool ContainsImpl(const Max<DimExpr>& dim_expr, const DimExpr& target) {
    return VariadicContains(dim_expr, target);
  }
  bool ContainsImpl(const Min<DimExpr>& dim_expr, const DimExpr& target) {
    return VariadicContains(dim_expr, target);
  }
  bool ContainsImpl(const Broadcast<DimExpr>& dim_expr, const DimExpr& target) {
    return VariadicContains(dim_expr, target);
  }

  template <typename T>
  bool UnaryContains(const T& dim_expr, const DimExpr& target) {
    const auto& operand = dim_expr->data;
    return Contains(operand, target);
  }

  template <typename T>
  bool BinaryContains(const T& dim_expr, const DimExpr& target) {
    const auto& lhs = dim_expr->lhs;
    const auto& rhs = dim_expr->rhs;
    return Contains(lhs, target) || Contains(rhs, target);
  }

  template <template <typename> class OpT>
  bool VariadicContains(const OpT<DimExpr>& dim_expr, const DimExpr& target) {
    const auto& operands = *(dim_expr.operands);
    for (const auto& operand : operands) {
      if (Contains(operand, target)) return true;
    }
    return false;
  }
};
...
IR_API bool ContainsDimExpr(const DimExpr& dim_expr, const DimExpr& target) {
  return ContainsDimExprHelper().Contains(dim_expr, target);
}
