...
OpInfo OpInfoImpl::Create(Dialect *dialect,
  ...
  size_t base_size = sizeof(detail::InterfaceValue) * interfaces_num +
  ...
  if (interfaces_num > 0) {
    ...
    for (size_t index = 0; index < interfaces_num; ++index) {
      new (base_ptr + index * sizeof(detail::InterfaceValue))
          detail::InterfaceValue(std::move(interface_map[index]));
    }
    base_ptr += interfaces_num * sizeof(detail::InterfaceValue);
  }
  if (traits_num > 0) {
...
bool OpInfoImpl::HasInterface(TypeId interface_id) const {
  if (num_interfaces_ > 0) {
    const detail::InterfaceValue *p_first_interface =
        reinterpret_cast<const detail::InterfaceValue *>(
            reinterpret_cast<const char *>(this) -
            sizeof(pir::TypeId) * num_traits_ -
            sizeof(detail::InterfaceValue) * num_interfaces_);
    return std::binary_search(p_first_interface,
                              p_first_interface + num_interfaces_,
                              detail::InterfaceValue(interface_id));
  }
  return false;
}
...
void OpInfoImpl::Destroy() {
  ...
  char *base_ptr = reinterpret_cast<char *>(this) -
                   ...
                   sizeof(detail::InterfaceValue) * num_interfaces_;
  if (num_interfaces_ > 0) {
    detail::InterfaceValue *p_interface_val =
        reinterpret_cast<detail::InterfaceValue *>(base_ptr);
