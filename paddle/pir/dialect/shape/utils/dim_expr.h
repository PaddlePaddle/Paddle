...
class ShapeOrData {
 public:
  ...
  using ShapeT = std::variant<VecT, std::vector<VecT>>;
  using DataT = std::optional<std::variant<VecT, std::vector<VecT>>>;

  explicit ShapeOrData(const VecT& shape)
      : shape_(shape), data_(std::nullopt) {}
  explicit ShapeOrData(const VecT& shape, const VecT& data)
  ...
  // Tensor's real shape for non-tensor-list case.
  const VecT& shape() const {
    IR_ENFORCE(
        std::get_if<VecT>(&shape_),
        "Shape of ShapeOrData is not a vector, check wheather the value is a "
        "tensor-list or not. If so, use shape(int64_t i) to get each shape.");
    return std::get<VecT>(shape_);
  }

  // Return i-th tensor's shape for both tensor and tensor-list case.
  const VecT& shape(int64_t i) const {
    IR_ENFORCE(i >= 0, "Index should be non-negative.");
    if (i == 0) {
      return shape();
    } else {
  ...
  // Tensor's real shape for non-tensor-list case.
  const std::optional<VecT>& data() const {
    if (data_.has_value()) {
      IR_ENFORCE(
          std::get_if<VecT>(&data_.value()),
          "Data of ShapeOrData is not a vector, check wheather the value is a "
          "tensor-list or not. If so, use data(int64_t i) to get each data.");
      return std::get<VecT>(data_.value());
    }
    return std::nullopt;
  }

  const std::optional<VecT>& data(int64_t i) const {
    IR_ENFORCE(i >= 0, "Index should be non-negative.");
    if (data_.has_value()) {
      if (i == 0) {
        return shape();
  ...
  void SetData(const VecT& data) { data_ = data; }

 private:
  // Shape of a tensor or a tensor-list.
  ShapeT shape_;

  // Specfic for Tensor generated by shape-relevant ops
  DataT data_;
};
