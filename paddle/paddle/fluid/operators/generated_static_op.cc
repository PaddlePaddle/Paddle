...
DECLARE_INFER_SHAPE_FUNCTOR(range, RangeInferShapeFunctor,
                            PD_INFER_META(phi::ArangeTensorInferMeta));
...
class AssignValueOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    ...
    ;
    AddInput("ValuesTensor", "attribute 2 for assign_value op from 0D Tensor.")
        .AsDispensable();
    AddAttr<std::vector<Scalar>>("values", "(std::vector<Scalar>), attribute 2 for assign_value op.")
        .SetDefault({});
    AddComment(R"DOC(
...
class Conv2dTransposeBiasOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("Input", "(Tensor), input 0 of conv2d_transpose_bias op.");
    AddInput("Filter", "(Tensor), input 1 of conv2d_transpose_bias op.");
    AddInput("Bias", "(Tensor), input 2 of conv2d_transpose_bias op.");
    AddOutput("Output", "(Tensor), output 0 of conv2d_transpose_bias op.");
    AddAttr<std::vector<int>>("strides", "(std::vector<int>), attribute 0 for conv2d_transpose_bias op.")
        .SetDefault({1, 1});
    AddAttr<std::vector<int>>("paddings", "(std::vector<int>), attribute 1 for conv2d_transpose_bias op.")
        .SetDefault({0, 0});
    AddAttr<std::vector<int>>("output_padding", "(std::vector<int>), attribute 2 for conv2d_transpose_bias op.")
        .SetDefault({});
    AddAttr<std::vector<int>>("output_size", "(std::vector<int>), attribute 3 for conv2d_transpose_bias op.")
        .SetDefault({})
        .SupportTensor();
    AddAttr<std::string>("padding_algorithm", "(std::string), attribute 4 for conv2d_transpose_bias op.")
        .SetDefault("EXPLICIT");
    AddAttr<int>("groups", "(int), attribute 5 for conv2d_transpose_bias op.")
        .SetDefault(1);
    AddAttr<std::vector<int>>("dilations", "(std::vector<int>), attribute 6 for conv2d_transpose_bias op.")
        .SetDefault({1, 1});
    AddAttr<std::string>("data_format", "(std::string), attribute 7 for conv2d_transpose_bias op.")
        .SetDefault("NCHW");
    AddComment(R"DOC(
TODO: Documentation of conv2d_transpose_bias op.
)DOC");
  }
};


class Conv2dTransposeBiasOp : public framework::OperatorWithKernel {
 public:
  using framework::OperatorWithKernel::OperatorWithKernel;
 protected:
  phi::KernelKey GetExpectedKernelType(
...
DECLARE_INFER_SHAPE_FUNCTOR(conv2d_transpose_bias, Conv2dTransposeBiasInferShapeFunctor,
                            PD_INFER_META(phi::Conv2dTransposeInferMeta));
...
class QuantLinearOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("x", "(Tensor), input 0 of quant_linear op.");
    AddInput("w", "(Tensor), input 1 of quant_linear op.");
    AddInput("bias", "(Tensor), input 2 of quant_linear op.")
        .AsDispensable();
    AddOutput("out", "(Tensor), output 0 of quant_linear op.");
    AddAttr<int>("in_num_col_dims", "(int), attribute 0 for quant_linear op.")
        .SetDefault(1);
    AddAttr<std::string>("activation_type", "(std::string), attribute 1 for quant_linear op.")
        .SetDefault("");
    AddAttr<bool>("padding_weights", "(bool), attribute 2 for quant_linear op.")
        .SetDefault(false);
    AddAttr<float>("scale_in", "(float), attribute 3 for quant_linear op.")
        .SetDefault(1.0f);
    AddAttr<std::vector<float>>("scale_weights", "(std::vector<float>), attribute 4 for quant_linear op.")
        .SetDefault({1.0f});
    AddAttr<int>("quant_round_type", "(int), attribute 5 for quant_linear op.")
        .SetDefault(1);
    AddAttr<float>("quant_max_bound", "(float), attribute 6 for quant_linear op.")
        .SetDefault(127.0f);
    AddAttr<float>("quant_min_bound", "(float), attribute 7 for quant_linear op.")
        .SetDefault(-127.0f);
    AddComment(R"DOC(
TODO: Documentation of quant_linear op.
)DOC");
  }
};


class QuantLinearOp : public framework::OperatorWithKernel {
 public:
  using framework::OperatorWithKernel::OperatorWithKernel;
 protected:
};
...
DECLARE_INFER_SHAPE_FUNCTOR(quant_linear, QuantLinearInferShapeFunctor,
                            PD_INFER_META(phi::QuantLinearInferMeta));
...
class MatmulV2GradGradOpMaker : public framework::SingleGradOpMaker<T> {
 ...
 protected:
  void Apply(GradOpPtr<T> grad_op) const override {
    ...
    if (this->HasOutput(GradVarName("Y"))) {
...
REGISTER_OP_VERSION(assign_value)
  .AddCheckpoint(
    R"ROC(Upgrade assign_value, remove plain attributes in favor of generic attribute.)ROC",
      paddle::framework::compatible::OpVersionDesc()
        .NewAttr("values", "replace generic types with scalar.", std::vector<paddle::experimental::Scalar>())
...
REGISTER_OPERATOR(conv2d_transpose_bias, ops::Conv2dTransposeBiasOp,
                  ops::Conv2dTransposeBiasOpMaker,
                  paddle::framework::EmptyGradOpMaker<paddle::framework::OpDesc>,
                  paddle::framework::EmptyGradOpMaker<paddle::imperative::OpBase>,
                  ops::Conv2dTransposeBiasInferShapeFunctor);
...
REGISTER_OPERATOR(quant_linear, ops::QuantLinearOp,
                  ops::QuantLinearOpMaker,
                  paddle::framework::EmptyGradOpMaker<paddle::framework::OpDesc>,
                  paddle::framework::EmptyGradOpMaker<paddle::imperative::OpBase>,
                  ops::QuantLinearInferShapeFunctor);
